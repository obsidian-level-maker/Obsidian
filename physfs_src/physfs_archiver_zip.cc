...
static int zip_prep_crypto_keys(ZIPfileinfo *finfo,
    ...
    const PHYSFS_uint8 verifier = (PHYSFS_uint8)(
        (usedate ? (entry->dos_mod_time >> 8) : (entry->crc >> 24)) & 0xFF);
    ...
    if (finalbyte != verifier) BAIL(PHYSFS_ERR_BAD_PASSWORD, 0);
...
static PHYSFS_sint64 ZIP_read(PHYSFS_Io *_io, void *buf, PHYSFS_uint64 len) {
    ...
    PHYSFS_sint64 avail =
        entry->uncompressed_size - finfo->uncompressed_position;

    if (avail < maxread) maxread = avail;
    ...
    if (entry->compression_method == COMPMETH_NONE)
        ...
        while (retval < maxread) {
            ...
            if (finfo->stream.avail_in == 0) {
                ...
                if (br > 0) {
                    if (br > ZIP_READBUFSIZE) br = ZIP_READBUFSIZE;

                    br = zip_read_decrypt(finfo, finfo->buffer,
                                          (PHYSFS_uint64)br);
                    if (br <= 0) break;
    ...
    if (retval > 0) finfo->uncompressed_position += (PHYSFS_uint32)retval;
...
static int ZIP_seek(PHYSFS_Io *_io, PHYSFS_uint64 offset) {
    ...
    if (!encrypted && (entry->compression_method == COMPMETH_NONE)) {
        ...
        if (offset < finfo->uncompressed_position) {
            ...
            if (zlib_err(inflateInit2(&str, -MAX_WBITS)) != Z_OK) return 0;

            if (!io->seek(io, entry->offset + (encrypted ? 12 : 0))) return 0;
        ...
        while (finfo->uncompressed_position != offset) {
            ...
            if (ZIP_read(_io, buf, maxread) != maxread) return 0;
        } /* while */
    }     /* else */
...
failed:
    if (finfo != NULL) {
        if (finfo->io != NULL) finfo->io->destroy(finfo->io);
...
static void ZIP_destroy(PHYSFS_Io *io) {
    ...
    if (finfo->buffer != NULL) allocator.Free(finfo->buffer);
...
static PHYSFS_sint64 zip_find_end_of_central_dir(PHYSFS_Io *io,
    ...
    while ((totalread < filelen) && (totalread < 65557)) {
        ...
        if (totalread != 0) {
            if (!__PHYSFS_readAll(io, buf, maxread - 4)) return -1;
            memcpy(&buf[maxread - 4], &extra, sizeof(extra));
            totalread += maxread - 4;
        } /* if */
        else {
            if (!__PHYSFS_readAll(io, buf, maxread)) return -1;
            totalread += maxread;
        } /* else */
...
static ZIPentry *zip_follow_symlink(PHYSFS_Io *io, ZIPinfo *info, char *path) {
    ...
    if (entry != NULL) {
        if (!zip_resolve(io, info, entry)) /* recursive! */
            ...
            if (entry->symlink != NULL) entry = entry->symlink;
        } /* else */
    }     /* if */
...
static int zip_resolve(PHYSFS_Io *io, ZIPinfo *info, ZIPentry *entry) {
    ...
    if (resolve_type == ZIP_DIRECTORY) return 1; /* we're good. */
...
static ZIPentry *zip_load_entry(ZIPinfo *info, const int zip64,
    ...
    BAIL_IF_ERRPASS(!readui16(io, &entry.general_bits),
                    NULL); /* general bits */
...
static int zip_load_entries(ZIPinfo *info, const PHYSFS_uint64 data_ofs,
    ...
    for (i = 0; i < entry_count; i++) {
        ...
        if (zip_entry_is_tradional_crypto(entry)) info->has_crypto = 1;
    } /* for */
...
static PHYSFS_sint64 zip64_find_end_of_central_dir(PHYSFS_Io *io,
    ...
    if (ui32 == ZIP64_END_OF_CENTRAL_DIR_SIG) return offset;
    ...
    if (pos > 56) {
        ...
        BAIL_IF_ERRPASS(!readui32(io, &ui32), -1);
        if (ui32 == ZIP64_END_OF_CENTRAL_DIR_SIG) return pos - 56;
    } /* if */
    ...
    if (pos > 84) {
        ...
        BAIL_IF_ERRPASS(!readui32(io, &ui32), -1);
        if (ui32 == ZIP64_END_OF_CENTRAL_DIR_SIG) return pos - 84;
    } /* if */
...
static int zip64_parse_end_of_central_dir(ZIPinfo *info,
    ...
    if ((pos < 0) || (!io->seek(io, pos))) return 0;
...
static void ZIP_closeArchive(void *opaque) {
    ...
    if (info->io) info->io->destroy(info->io);
