...
static BOOL winSetFilePointer(HANDLE h, const PHYSFS_sint64 pos,
    ...
    if (_newpos) *_newpos = (PHYSFS_sint64)linewpos.QuadPart;
    return rc;
#else
    ...
    if (_newpos) *_newpos = ((PHYSFS_sint64)rc) | (((PHYSFS_sint64)high) << 32);
...
static DWORD pollDiscDrives(void) {
    ...
    if (lib) stem = (fnSTEM)GetProcAddress((HMODULE)lib, "SetThreadErrorMode");
    ...
    for (i = 'A'; i <= 'Z'; i++) {
        DWORD tmp = 0;
        drive[0] = (char)i;
        if (GetDriveTypeA(drive) != DRIVE_CDROM) continue;
...
static DWORD WINAPI detectCDThread(LPVOID arg) {
    ...
    detectCDHwnd =
        CreateWindowExA(0, classname, winname, WS_OVERLAPPEDWINDOW,
...
static void detectAvailableCDs(PHYSFS_StringCallback cb, void *data) {
    ...
    if (!detectCDThreadHandle) {
        ...
        if (!initialDetectDone) return; /* oh well. */
        ...
        if (!detectCDThreadHandle) return; /* oh well. */
    }                                      /* if */
...
static void deinitCDThread(void) {
    if (detectCDThreadHandle) {
        if (detectCDHwnd) PostMessageW(detectCDHwnd, WM_QUIT, 0, 0);
...
char *__PHYSFS_platformCalcUserDir(void) {
    ...
    if (accessToken) CloseHandle(accessToken);
...
PHYSFS_EnumerateCallbackResult __PHYSFS_platformEnumerate(
    const char *dirname, PHYSFS_EnumerateCallback callback, const char *origdir,
    void *callbackdata) {
    ...
    BAIL_IF(dir == INVALID_HANDLE_VALUE, errcodeFromWinApi(),
            PHYSFS_ENUM_ERROR);
...
int __PHYSFS_platformStat(const char *filename, PHYSFS_Stat *st,
    ...
    if (issymlink) {
        ...
        st->filesize = (((PHYSFS_uint64)winstat.nFileSizeHigh) << 32) |
