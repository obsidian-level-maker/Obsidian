...
static PHYSFS_sint64 memoryIo_read(PHYSFS_Io *io, void *buf,
    ...
    if (avail == 0) return 0; /* we're at EOF; nothing to do. */
...
static void handleIo_destroy(PHYSFS_Io *io) {
    if (io->opaque != NULL) PHYSFS_close((PHYSFS_File *)io->opaque);
    allocator.Free(io);
} /* handleIo_destroy */
...
static void enumStringListCallback(void *data, const char *str) {
    ...
    if (pecd->errcode) return;

    ptr = allocator.Realloc(pecd->list, (pecd->size + 2) * sizeof(char *));
    newstr = (char *)allocator.Malloc(strlen(str) + 1);
    if (ptr != NULL) pecd->list = (char **)ptr;
...
void __PHYSFS_sort(void *entries, size_t max,
     ...
     */
    if (max > 0) __PHYSFS_quick_sort(entries, 0, max - 1, cmpfn, swapfn);
} /* __PHYSFS_sort */
...
static ErrState *findErrorForCurrentThread(void) {
    ...
    if (errorLock != NULL) __PHYSFS_platformGrabMutex(errorLock);
    ...
    if (errorStates != NULL) {
        ...
        for (i = errorStates; i != NULL; i = i->next) {
            if (i->tid == tid) {
                if (errorLock != NULL) __PHYSFS_platformReleaseMutex(errorLock);
                return i;
            } /* if */
        }     /* for */
    }         /* if */
...
static DirHandle *tryOpenDir(PHYSFS_Io *io, const PHYSFS_Archiver *funcs,
    ...
    if (io != NULL) BAIL_IF_ERRPASS(!io->seek(io, 0), NULL);
...
static DirHandle *openDirectory(PHYSFS_Io *io, const char *d, int forWriting) {
    ...
    if (io == NULL) {
        ...
        if (statbuf.filetype == PHYSFS_FILETYPE_DIRECTORY) {
            retval =
                tryOpenDir(io, &__PHYSFS_Archiver_DIR, d, forWriting, &claimed);
            if (retval || claimed) return retval;
        } /* if */
    ...
    if ((!retval) && (created_io)) io->destroy(io);
...
static int partOfMountPoint(DirHandle *h, char *fname) {
    ...
    if ((len + 1) == mntpntlen) return 0;

    rc = strncmp(fname, h->mountPoint, len); /* !!! FIXME: case insensitive? */
    if (rc != 0) return 0;                   /* not a match. */
...
static DirHandle *createDirHandle(PHYSFS_Io *io, const char *newDir,
    ...
    if ((mountPoint != NULL) && (*mountPoint != '\0')) {
        dirHandle->mountPoint =
            (char *)allocator.Malloc(strlen(mountPoint) + 2);
...
static int initializeMutexes(void) {
    ...
    if (errorLock == NULL) goto initializeMutexes_failed;

    stateLock = __PHYSFS_platformCreateMutex();
    if (stateLock == NULL) goto initializeMutexes_failed;

    return 1; /* success. */

initializeMutexes_failed:
    if (errorLock != NULL) __PHYSFS_platformDestroyMutex(errorLock);

    if (stateLock != NULL) __PHYSFS_platformDestroyMutex(stateLock);
...
int PHYSFS_init(const char *argv0) {
    ...
    if (!externalAllocator) setDefaultAllocator();
...
const char *PHYSFS_getPrefDir(const char *org, const char *app) {
    ...
    *endstr = '\0'; /* mask out the final dirsep for now. */
    ...
    if (!__PHYSFS_platformStat(prefDir, &statbuf, 1)) {
        for (ptr = strchr(prefDir, dirsep); ptr;
             ptr = strchr(ptr + 1, dirsep)) {
            *ptr = '\0';
...
const char *PHYSFS_getWriteDir(void) {
    ...
    __PHYSFS_platformGrabMutex(stateLock);
    if (writeDir != NULL) retval = writeDir->dirName;
    __PHYSFS_platformReleaseMutex(stateLock);
...
int PHYSFS_unmount(const char *oldDir) {
    ...
    for (i = searchPath; i != NULL; i = i->next) {
        if (strcmp(i->dirName, oldDir) == 0) {
            next = i->next;
            BAIL_IF_MUTEX_ERRPASS(!freeDirHandle(i, openReadList), stateLock,
                                  0);
    ...
    if (!allowSymLinks) {
        while (1) {
            ...
            if (!retval) {
                 ...
                 *  for writing...
                 */
                if ((end == NULL) || (allowMissing)) retval = 1;
                break;
            } /* if */
...
static int doMkdir(const char *_dname, char *dname) {
    ...
    while (1) {
        ...
        if (!exists) retval = h->funcs->mkdir(h->opaque, dname);
...
static PHYSFS_EnumerateCallbackResult enumFilesCallback(void *data,
    ...
    if ((ptr == NULL) || (newstr == NULL)) {
        if (newstr) allocator.Free(newstr);
 ...
 */
static PHYSFS_EnumerateCallbackResult enumerateFromMountPoint(
    DirHandle *i, const char *arcfname, PHYSFS_EnumerateCallback callback,
        ...
        for (i = searchPath; (retval == PHYSFS_ENUM_OK) && i; i = i->next) {
            ...
            if (partOfMountPoint(i, arcfname))
                ...
                if (statbuf.filetype != PHYSFS_FILETYPE_DIRECTORY)
                    ...
                    retval =
                        i->funcs->enumerate(i->opaque, arcfname, cb, _fn, data);
                } /* else */
            }     /* else if */
        }         /* for */
    ...
    for (i = *list; i != NULL; i = i->next) {
        if (i == handle) /* handle is in this list? */
        {
            ...
            if (!handle->forReading) {
                if (!PHYSFS_flush((PHYSFS_File *)handle)) return -1;
...
PHYSFS_sint64 PHYSFS_readBytes(PHYSFS_File *handle, void *buffer,
    ...
    if (fh->buffer) return doBufferedRead(fh, buffer, len);
...
PHYSFS_sint64 PHYSFS_writeBytes(PHYSFS_File *handle, const void *buffer,
    ...
    BAIL_IF_ERRPASS(len == 0, 0);
    if (fh->buffer) return doBufferedWrite(handle, buffer, len);
...
int PHYSFS_eof(PHYSFS_File *handle) {
    ...
    if (fh->bufpos == fh->buffill) {
        ...
        if ((pos < 0) || (len < 0)) return 0; /* beats me. */
        return (pos >= len);
    } /* if */
        ...
        if (*fname == '\0') {
            ...
            for (i = searchPath; ((i != NULL) && (!exists)); i = i->next) {
                ...
                if (exists) {
                    ...
                    if ((retval) ||
                        (currentErrorCode() != PHYSFS_ERR_NOT_FOUND))
                        exists = 1;
                } /* else if */
            }     /* for */
            __PHYSFS_platformReleaseMutex(stateLock);
...
void __PHYSFS_smallFree(void *ptr) {
    if (ptr != NULL) {
        ...
        if (useHeap) allocator.Free(block);
...
int PHYSFS_setAllocator(const PHYSFS_Allocator *a) {
    ...
    if (externalAllocator) memcpy(&allocator, a, sizeof(PHYSFS_Allocator));
...
static void *mallocAllocatorMalloc(PHYSFS_uint64 s) {
    if (!__PHYSFS_ui64FitsAddressSpace(s)) BAIL(PHYSFS_ERR_OUT_OF_MEMORY, NULL);
#undef malloc
...
int __PHYSFS_DirTreeInit(__PHYSFS_DirTree *dt, const size_t entrylen) {
    ...
    dt->root->isdir = 1;
    dt->hashBuckets = 64;
    if (!dt->hashBuckets) dt->hashBuckets = 1;
