...
static void OuterWorldVar(boolean isGlobal)
{
	...
	do
	{
		...
		TK_NextTokenMustBe(TK_COLON, (error_t)(ERR_MISSING_WVAR_COLON+isGlobal));
		TK_NextTokenMustBe(TK_IDENTIFIER, ERR_INVALID_IDENTIFIER);
...
static void LeadingLineSpecial(boolean executewait)
{
	...
	else
	{
		...
		else
		{
			...
			PC_AppendCmd((pcd_t)((argCount-1)+(useintform?PCD_LSPEC1DIRECT:PCD_LSPEC1DIRECTB)));
			PC_AppendByte((U_BYTE)specialValue);
		}
		if (useintform)
		{
...
static void InitializeArray(symbolNode_t *sym, int dims[MAX_ARRAY_DIMS], int size)
{
	...
	if(lastsize < size)
	{
		entries = (int*)MS_Realloc(entries, sizeof(int)*size, ERR_OUT_OF_MEMORY);
		lastsize = size;
	}
...
static void AddScriptFuncRef(symbolNode_t *sym, int address, int argcount)
{
	prefunc_t *fillin = (prefunc_t*)MS_Alloc(sizeof(*fillin), ERR_OUT_OF_MEMORY);
	fillin->next = NULL;
	fillin->sym = sym;
