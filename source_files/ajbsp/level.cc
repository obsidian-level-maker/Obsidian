...
static void WriteBlockmap(void)
{
    ...
    for (i = 0; i < block_count; i++)
    {
        ...
        if (ptr == 0) BugError("WriteBlockmap: offset %d not set.\n", i);
...
void GetVertices()
{
    ...
    if (lump) count = lump->Length() / (int)sizeof(raw_vertex_t);
    ...
    if (!lump->Seek(0)) cur_info->FatalError("Error seeking to vertices.\n");
...
void GetSectors()
{
    ...
    if (lump) count = lump->Length() / (int)sizeof(raw_sector_t);

    if (lump == NULL || count == 0) return;

    if (!lump->Seek(0)) cur_info->FatalError("Error seeking to sectors.\n");
...
void GetThings()
{
    ...
    if (lump) count = lump->Length() / (int)sizeof(raw_thing_t);

    if (lump == NULL || count == 0) return;

    if (!lump->Seek(0)) cur_info->FatalError("Error seeking to things.\n");
...
void GetThingsHexen()
{
    ...
    if (lump) count = lump->Length() / (int)sizeof(raw_hexen_thing_t);
...
void GetSidedefs()
{
    ...
    if (lump) count = lump->Length() / (int)sizeof(raw_sidedef_t);

    if (lump == NULL || count == 0) return;

    if (!lump->Seek(0)) cur_info->FatalError("Error seeking to sidedefs.\n");
...
void GetLinedefs()
{
    ...
    if (lump) count = lump->Length() / (int)sizeof(raw_linedef_t);

    if (lump == NULL || count == 0) return;

    if (!lump->Seek(0)) cur_info->FatalError("Error seeking to linedefs.\n");
...
void GetLinedefsHexen()
{
    ...
    if (lump) count = lump->Length() / (int)sizeof(raw_hexen_linedef_t);
...
void PutVertices(const char *name, int do_gl)
{
    ...
    for (i = 0, count = 0; i < num_vertices; i++)
    {
        ...
        if ((do_gl ? 1 : 0) != (vert->is_new ? 1 : 0)) { continue; }
...
{
    if (v->is_new) return (uint16_t)(v->index | 0x8000U);
...
{
    if (v->is_new) return (uint32_t)(v->index | 0x80000000U);
...
{
    if (v->is_new) return (uint32_t)(num_old_vert + v->index);
...
void PutSegs()
{
    ...
    for (int i = 0; i < num_segs; i++)
    {
        ...
        cur_info->Debug(
            ...
            seg->index, LE_U16(raw.start), LE_U16(raw.end), LE_U16(raw.linedef),
            seg->side ? "L" : "R", LE_U16(raw.angle), seg->start->x,
...
void PutGLSegs_V2()
{
    ...
    for (int i = 0; i < num_segs; i++)
    {
        ...
        cur_info->Debug(
            ...
            "(%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
            seg->index, LE_U16(raw.linedef), seg->side ? "L" : "R",
            LE_U16(raw.partner), seg->start->x, seg->start->y, seg->end->x,
            seg->end->y);
#endif
    }
...
void PutGLSegs_V5()
{
    ...
    for (int i = 0; i < num_segs; i++)
    {
        ...
        cur_info->Debug(
            ...
            "(%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
            seg->index, LE_U16(raw.linedef), seg->side ? "L" : "R",
            LE_U32(raw.partner), seg->start->x, seg->start->y, seg->end->x,
            seg->end->y);
#endif
    }
...
void PutSubsecs(const char *name, int do_gl)
{
    ...
    for (int i = 0; i < num_subsecs; i++)
    {
        ...
        cur_info->Debug("PUT SUBSEC %04X  First %04X  Num %04X\n", sub->index,
                        LE_U16(raw.first), LE_U16(raw.num));
#endif
    }
...
static void PutOneNode(node_t *node, Lump_c *lump)
{
    ...
    cur_info->Debug(
        ...
        "(%d,%d) -> (%d,%d)\n",
        node->index, LE_U16(raw.left), LE_U16(raw.right), node->x, node->y,
...
{
    if (node->r.node) PutOneNode_V5(node->r.node, lump);

    if (node->l.node) PutOneNode_V5(node->l.node, lump);
    ...
    cur_info->Debug(
        ...
        "(%d,%d) -> (%d,%d)\n",
        node->index, LE_U32(raw.left), LE_U32(raw.right), node->x, node->y,
        node->x + node->dx, node->y + node->dy);
#endif
}
...
{
    int struct_size =
        do_v5 ? (int)sizeof(raw_v5_node_t) : (int)sizeof(raw_node_t);
...
void CheckLimits()
{
    ...
    {
        if (num_old_vert > 32767 || num_new_vert > 32767 || num_segs > 65535 ||
            num_nodes > 32767)
        {
    ...
    {
        if (num_old_vert > 32767 || num_new_vert > 32767 || num_segs > 32767 ||
            num_nodes > 32767)
        {
...
void PutZSubsecs()
{
    ...
    for (int i = 0; i < num_subsecs; i++)
    {
        ...
        int count = 0;
        for (const seg_t *seg = sub->seg_list; seg;
             seg              = seg->next, cur_seg_index++)
        {
...
static void PutOneZNode(node_t *node, bool do_xgl3)
{
    ...
    if (node->r.node) PutOneZNode(node->r.node, do_xgl3);

    if (node->l.node) PutOneZNode(node->l.node, do_xgl3);
...
void SaveXGL3Format(Lump_c *lump, node_t *root_node)
{
    ...
    if (cur_info->force_compress) { lump->Write(lev_ZGL3_magic, 4); }
    else { lump->Write(lev_XGL3_magic, 4); }
...
void LoadLevel()
{
    ...
    if (lev_format == MAPF_UDMF) { ParseUDMF(); }
    else
    {
        GetVertices();
        GetSectors();
...
void UpdateGLMarker(Lump_c *marker)
{
    ...
    if (lev_long_name) { marker->Printf("LEVEL=%s\n", lev_current_name); }
...
{
    if (cur_wad->LevelLookupLump(lev_current_idx, name) >= 0) return;
...
void ZLibBeginLump(Lump_c *lump)
{
    ...
    if (!cur_info->force_compress) return;
...
Lump_c *CreateLevelLump(const char *name, int max_size)
{
    ...
    else
    {
        ...
        if (lev_format != MAPF_UDMF) last_idx += 1;
...
void SetInfo(buildinfo_t *info) { cur_info = info; }
...
{
    if (cur_info->cancelled) return BUILD_Cancelled;
    ...
    {
        cur_info->Print(
            2, "    Built %d NODES, %d SSECTORS, %d SEGS, %d VERTEXES\n",
