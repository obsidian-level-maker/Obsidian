...
void DetectOverlappingVertices(void)
{
    ...
    for (int i = 0; i < num_linedefs; i++)
    {
        ...
        while (L->start->overlap) { L->start = L->start->overlap; }

        while (L->end->overlap) { L->end = L->end->overlap; }
    }
}
...
void DetectOverlappingLines(void)
{
    ...
    for (int i = 0; i < num_linedefs - 1; i++)
    {
        ...
        for (int k = i + 1; k < num_linedefs; k++)
        {
            ...
            if (B->MinX() > A->MinX() + DIST_EPSILON) break;
...
void vertex_t::AddWallTip(double dx, double dy, bool open_left, bool open_right)
{
    ...
    {
        if (tip_set != NULL) tip_set->prev = tip;
...
void CalculateWallTips()
{
    for (int i = 0; i < num_linedefs; i++)
    {
        ...
        if (L->overlap || L->zero_len) continue;
...
vertex_t *NewVertexFromSplitSeg(seg_t *seg, double x, double y)
{
    ...
    {
        const sidedef_t *front =
            seg->side ? seg->linedef->left : seg->linedef->right;
        const sidedef_t *back =
            seg->side ? seg->linedef->right : seg->linedef->left;
...
vertex_t *NewVertexDegenerate(vertex_t *start, vertex_t *end)
{
    ...
    if (dlen == 0) BugError("NewVertexDegenerate: bad delta!\n");
