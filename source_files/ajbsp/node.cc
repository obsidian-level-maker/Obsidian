...
void seg_t::Recompute()
{
    ...
    if (p_length <= 0) BugError("Seg %p has zero p_length.\n", this);
...
//
inline void ComputeIntersection(seg_t *seg, seg_t *part, double perp_c,
...
void AddIntersection(intersection_t **cut_list, vertex_t *vert, seg_t *part,
                     bool self_ref)
{
...
//
bool EvalPartitionWorker(quadtree_c *tree, seg_t *part, double best_cost,
                         eval_info_t *info)
{
...
double EvalPartition(quadtree_c *tree, seg_t *part, double best_cost)
{
    ...
    if (info.real_left == 0 || info.real_right == 0) { return -1.0; }
    ...
    if (part->pdx != 0 && part->pdy != 0) info.cost += 25.0;

    return info.cost;
}

void EvaluateFastWorker(quadtree_c *tree, seg_t **best_H, seg_t **best_V,
                        int mid_x, int mid_y)
{
    for (seg_t *part = tree->list; part; part = part->next)
    {
        /* ignore minisegs as partition candidates */
        if (part->linedef == NULL) continue;
...
bool PickNodeWorker(quadtree_c *part_list, quadtree_c *tree, seg_t **best,
                    double *best_cost)
{
    /* try each Seg as partition */
    for (seg_t *part = part_list->list; part; part = part->next)
    {
        if (cur_info->cancelled) return false;
...
//       or be split.
//
void DivideOneSeg(seg_t *seg, seg_t *part, seg_t **left_list,
                  seg_t **right_list, intersection_t **cut_list)
{
    ...
    if (seg->source_line == part->source_line) a = b = 0;
...
void SeparateSegs(quadtree_c *tree, seg_t *part, seg_t **left_list,
                  seg_t **right_list, intersection_t **cut_list)
{
...
void AddMinisegs(intersection_t *cut_list, seg_t *part, seg_t **left_list,
                 seg_t **right_list)
{
    ...
    {
        cur_info->Debug(
            "  Vertex %8X (%1.1f,%1.1f)  Along %1.2f  [%d/%d]  %s\n",
            cut->vertex->index, cut->vertex->x, cut->vertex->y, cut->along_dist,
            cut->open_before ? 1 : 0, cut->open_after ? 1 : 0,
...
#if DEBUG_CUTLIST
        cur_info->Debug(
            "AddMiniseg: %p RIGHT  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
            seg->start->x, seg->start->y, seg->end->x, seg->end->y);

        cur_info->Debug(
            "AddMiniseg: %p LEFT   (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
...
int seg_t::PointOnLineSide(double x, double y) const
{
    ...
    if (fabs(perp) <= DIST_EPSILON) return 0;
...
seg_t *CreateSegs()
{
    ...
    for (int i = 0; i < num_linedefs; i++)
    {
        ...
        if (hypot(line->start->x - line->end->x,
                  line->start->y - line->end->y) >= 32000)
        {
        ...
        else { Warning("Linedef #%d has no right sidedef!\n", line->index); }
...
{
    quadtree_c *tree =
        new quadtree_c(bounds->minx, bounds->miny, bounds->maxx, bounds->maxy);
...
void subsec_t::ClockwiseOrder()
{
    ...
    for (seg = seg_list; seg; seg = seg->next)
    {
        // compute angles now
        seg->cmp_angle =
            ComputeAngle(seg->start->x - mid_x, seg->start->y - mid_y);
    ...
    {
        cur_info->Debug(
            "  Seg %p: Angle %1.6f  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n", seg,
            seg->cmp_angle, seg->start->x, seg->start->y, seg->end->x,
            seg->end->y);
    }
#endif
}
...
void subsec_t::SanityCheckClosed() const
{
    ...
    for (seg = seg_list; seg; seg = seg->next)
    {
        ...
        if (fabs(dx) > DIST_EPSILON || fabs(dy) > DIST_EPSILON) gaps++;
                          ...
                          node_t **N, subsec_t **S)
{
    ...
    *N           = node;
    ...
    /* sanity checks... */
    if (rights == NULL) BugError("Separated seg-list has empty RIGHT side\n");
    ...
    ret = BuildNodes(lefts, depth + 1, &node->l.bounds, &node->l.node,
                     &node->l.subsec);
    if (ret != BUILD_OK) return ret;
    ...
    ret = BuildNodes(rights, depth + 1, &node->r.bounds, &node->r.node,
                     &node->r.subsec);
    if (ret != BUILD_OK) return ret;
...
void subsec_t::RoundOff()
{
    ...
    for (seg = seg_list; seg; seg = seg->next)
    {
            ...
            I_ROUND(seg->start->y) == I_ROUND(seg->end->y))
        {
            ...
            if (seg->linedef != NULL) last_real_degen = seg;

            degen_total++;
            continue;
        }

        if (seg->linedef != NULL) real_total++;
    }
    ...
    if (real_total == 0)
    {
        ...
        last_real_degen->end =
            NewVertexDegenerate(last_real_degen->start, last_real_degen->end);
