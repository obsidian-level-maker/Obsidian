...
#else
void FileMessage(const char *fmt, ...) { (void)fmt; }

void LumpWarning(const char *fmt, ...) { (void)fmt; }
#endif
...
bool Lump_c::GetLine(char *buffer, size_t buf_size)
{
    ...
    if (cur_pos >= l_length) return false;  // EOF
    ...
    for (; cur_pos < l_length && dest < dest_end; cur_pos++)
    {
        *dest++ = fgetc(parent->fp);
        ...
        if (ferror(parent->fp)) return false;

        if (feof(parent->fp)) break;
    }
...
Wad_file::~Wad_file()
{
    ...
    // free the directory
    for (int k = 0; k < NumLumps(); k++) delete directory[k];
...
bool Wad_file::Validate(const char *filename)
{
    ...
    if (!(header.ident[1] == 'W' && header.ident[2] == 'A' &&
          header.ident[3] == 'D'))
    {
        fclose(fp);
        return false;
...
static bool IsDummyMarker(const char *name)
{
    ...
    if (!strchr("PSF", toupper(name[0]))) return false;
...
void Wad_file::RemoveLumps(int index, int count)
{
    ...
    for (i = 0; i < count; i++) { delete directory[index + i]; }
...
void Wad_file::RemoveGLNodes(int lev_num)
{
    ...
    while (start <= finish && IsLevelLump(directory[start]->name)) { start++; }
...
void Wad_file::FixGroup(std::vector<int> &group, int index, int num_added,
                        int num_removed)
{
...
Lump_c *Wad_file::AddLump(const char *name, int max_size)
{
    ...
    if (insert_point >= NumLumps()) insert_point = -1;
...
Lump_c *Wad_file::AddLevel(const char *name, int max_size, int *lev_num)
{
    ...
    if (lev_num) { *lev_num = (int)levels.size(); }
...
int Wad_file::FindFreeSpace(int length)
{
    ...
    for (unsigned int k = 0; k < sorted_dir.size(); k++)
    {
        ...
        if (l_start >= offset + length) continue;
...
void Wad_file::WriteDirectory()
{
    ...
    if (total_size < 0) cur_info->FatalError("Error determining WAD size.\n");
