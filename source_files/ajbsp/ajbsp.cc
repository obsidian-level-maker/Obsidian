...
{
    if (strlen(name) != strlen(range->low)) return false;

    if (strcmp(name, range->low) < 0) return false;

    if (strcmp(name, range->high) > 0) return false;
...
build_result_e BuildFile(buildinfo_t *build_info)
{
    ...
    {
        if (!CheckMapInMaplist(n)) continue;

        visited += 1;

        build_info->ProgressUpdate(visited, num_levels);

        if (n > 0 && build_info->verbosity >= 2) build_info->Print(0, "\n");
    ...
    if (res == BUILD_Cancelled) return res;
...
void VisitFile(std::filesystem::path filename, buildinfo_t *build_info)
{
    ...
    if (res == BUILD_Cancelled) build_info->FatalError("CANCELLED\n");
}
...
{
    if (strlen(name) < 2 || strlen(name) > 8) return false;
...
void ParseMapRange(char *tok, buildinfo_t *build_info)
{
    ...
    if (low[0] != high[0])
        build_info->FatalError(
            "bad map range (%s and %s start with different letters)\n", low,
            high);
...
void CheckTypeSizes(buildinfo_t *build_info)
{
    ...
    assert_size(ajbsp::raw_linedef_t, 14);
    assert_size(ajbsp::raw_sector_s, 26);
    assert_size(ajbsp::raw_sidedef_t, 30);
    assert_size(ajbsp::raw_thing_t, 10);
    assert_size(ajbsp::raw_vertex_t, 4);
}
