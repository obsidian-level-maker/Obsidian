...
int FNodeBuilder::CreateSeg(int linenum, int sidenum) {
    ...
    D(printf(
        "Seg %4d: From line %d, side %s (%5d,%5d)-(%5d,%5d)  "
        "[%08x,%08x]-[%08x,%08x]\n",
        segnum, linenum, sidenum ? "back " : "front", Vertices[seg.v1].x >> 16,
        Vertices[seg.v1].y >> 16, Vertices[seg.v2].x >> 16,
        Vertices[seg.v2].y >> 16, Vertices[seg.v1].x, Vertices[seg.v1].y,
        Vertices[seg.v2].x, Vertices[seg.v2].y));

    return segnum;
}

// Group colinear segs together so that only one seg per line needs to be
// checked by SelectSplitter().
...
// Find "loops" of segs surrounding polyobject's origin. Note that a
// polyobject's origin is not solely defined by the polyobject's anchor, but
// also by the polyobject itself. For the split avoidance to work properly, you
// must have a convex, complete loop of segs surrounding the polyobject origin.
// All the maps in hexen.wad have complete loops of segs around their
// polyobjects, but they are not all convex: The doors at the start of MAP01 and
// some of the pillars in MAP02 that surround the entrance to MAP06 are not
// convex. Heuristic() uses some special weighting to make these cases work
// properly.

void FNodeBuilder::FindPolyContainers(TArray<FPolyStart> &spots,
    ...
    for (unsigned int i = 0; i < spots.Size(); ++i) {
        ...
        if (GetPolyExtents(spot->polynum, bbox)) {
            ...
            if (j < anchors.Size()) {
                ...
                // Scan right for the seg closest to the polyobject's center
                // after it gets moved to its start spot.
                fixed_t closestdist = FIXED_MAX;
                DWORD closestseg = 0;

                P(Printf("start %d,%d -- center %d, %d\n", spot->x >> 16,
                         spot->y >> 16, center.x >> 16, center.y >> 16));
                ...
                for (unsigned int j = 0; j < Segs.Size(); ++j) {
                    ...
                    if ((v1->y < center.y && v2->y < center.y) ||
                        (v1->y > center.y &&
                         v2->y > center.y)) {  // Not crossed
                        continue;
                    }

                    fixed_t dx = v2->x - v1->x;

                    if (PointOnSide(center.x, center.y, v1->x, v1->y, dx, dy) <=
                        0) {
...
int FNodeBuilder::MarkLoop(DWORD firstseg, int loopnum) {
    ...
    do {
        ...
        P(Printf("Mark seg %d (%d,%d)-(%d,%d)\n", seg, Vertices[s1->v1].x >> 16,
                 Vertices[s1->v1].y >> 16, Vertices[s1->v2].x >> 16,
                 Vertices[s1->v2].y >> 16));
...
bool FNodeBuilder::GetPolyExtents(int polynum, fixed_t bbox[4]) {
    ...
    if (i < Segs.Size()) {
        ...
        unsigned int vert;
        unsigned int count =
            Segs.Size();  // to prevent endless loops. Stop when this reaches
                          // the number of segs.
...
FNodeBuilder::FVertexMap::FVertexMap(FNodeBuilder &builder, fixed_t minx,
                                     fixed_t miny, fixed_t maxx, fixed_t maxy)
    : MyBuilder(builder) {
    MinX = minx;
    MinY = miny;
    BlocksWide =
        int(((double(maxx) - minx + 1) + (BLOCK_SIZE - 1)) / BLOCK_SIZE);
    BlocksTall =
        int(((double(maxy) - miny + 1) + (BLOCK_SIZE - 1)) / BLOCK_SIZE);
    MaxX = MinX + BlocksWide * BLOCK_SIZE - 1;
    MaxY = MinY + BlocksTall * BLOCK_SIZE - 1;
    VertexGrid = new TArray<int>[BlocksWide * BlocksTall];
}

FNodeBuilder::FVertexMap::~FVertexMap() { delete[] VertexGrid; }
...
int FNodeBuilder::FVertexMap::InsertVertex(FNodeBuilder::FPrivVert &vert) {
    ...
    int blk[4] = {GetBlock(minx, miny), GetBlock(maxx, miny),
                  GetBlock(minx, maxy), GetBlock(maxx, maxy)};
    unsigned int blkcount[4] = {
        VertexGrid[blk[0]].Size(), VertexGrid[blk[1]].Size(),
