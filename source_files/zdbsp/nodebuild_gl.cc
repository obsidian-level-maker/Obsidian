...
double FNodeBuilder::AddIntersection(const node_t &node, int vertex) {
    ...
    double dist = (double(v->x) - node.x) * (node.dx) +
                  (double(v->y) - node.y) * (node.dy);
...
void FNodeBuilder::FixSplitSharers() {
    ...
    for (unsigned int i = 0; i < SplitSharers.Size(); ++i) {
        ...
        D(printf("Considering events on seg %d(%d[%d,%d]->%d[%d,%d]) [%g:%g]\n",
                 seg, Segs[seg].v1, Vertices[Segs[seg].v1].x >> 16,
                 Vertices[Segs[seg].v1].y >> 16, Segs[seg].v2,
        ...
        while (event != NULL && next != NULL && event->Info.Vertex != v2) {
            D(printf(
                "Forced split of seg %d(%d[%d,%d]->%d[%d,%d]) at "
                "%d(%d,%d):%g\n",
                seg, Segs[seg].v1, Vertices[Segs[seg].v1].x >> 16,
                Vertices[Segs[seg].v1].y >> 16, Segs[seg].v2,
                Vertices[Segs[seg].v2].x >> 16, Vertices[Segs[seg].v2].y >> 16,
                event->Info.Vertex, Vertices[event->Info.Vertex].x >> 16,
                Vertices[event->Info.Vertex].y >> 16, event->Distance));
...
void FNodeBuilder::AddMinisegs(const node_t &node, DWORD splitseg, DWORD &fset,
                               DWORD &bset) {
    ...
    while (event != NULL) {
        if (prev != NULL) {
            ...
            // Minisegs should only be added when they can create valid loops on
            // both the front and back of the splitter. This means some
            // subsectors could be unclosed if their sectors are unclosed, but
            // at least we won't be needlessly creating subsectors in void
            // space. Unclosed subsectors can be closed trivially once the BSP
            // tree is complete.

            if ((fseg1 = CheckLoopStart(node.dx, node.dy, prev->Info.Vertex,
                                        event->Info.Vertex)) != DWORD_MAX &&
                (bseg1 = CheckLoopStart(-node.dx, -node.dy, event->Info.Vertex,
                                        prev->Info.Vertex)) != DWORD_MAX &&
                (fseg2 = CheckLoopEnd(node.dx, node.dy, event->Info.Vertex)) !=
                    DWORD_MAX &&
                (bseg2 = CheckLoopEnd(-node.dx, -node.dy, prev->Info.Vertex)) !=
                    DWORD_MAX) {
                // Add miniseg on the front side
                fnseg = AddMiniseg(prev->Info.Vertex, event->Info.Vertex,
                                   DWORD_MAX, fseg1, splitseg);
                Segs[fnseg].next = fset;
                fset = fnseg;

                // Add miniseg on the back side
                bnseg = AddMiniseg(event->Info.Vertex, prev->Info.Vertex, fnseg,
                                   bseg1, splitseg);
                ...
                if (fsector != bsector && fsector != Segs[fseg1].backsector &&
                    ...
                    Warn(
                        "Sectors %d at (%d,%d) and %d at (%d,%d) don't "
                        "match.\n",
                        Segs[fseg1].frontsector,
                        Vertices[prev->Info.Vertex].x >> FRACBITS,
                        Vertices[prev->Info.Vertex].y >> FRACBITS,
                        Segs[bseg1].frontsector,
                        Vertices[event->Info.Vertex].x >> FRACBITS,
                        Vertices[event->Info.Vertex].y >> FRACBITS);
                }

                D(Printf("**Minisegs** %d/%d added %d(%d,%d)->%d(%d,%d)\n",
                         fnseg, bnseg, prev->Info.Vertex,
                         Vertices[prev->Info.Vertex].x >> 16,
                         Vertices[prev->Info.Vertex].y >> 16,
                         event->Info.Vertex,
...
DWORD FNodeBuilder::AddMiniseg(int v1, int v2, DWORD partner, DWORD seg1,
                               DWORD splitseg) {
    DWORD nseg;
...
DWORD FNodeBuilder::CheckLoopStart(fixed_t dx, fixed_t dy, int vertex,
                                   int vertex2) {
    FPrivVert *v = &Vertices[vertex];
    ...
    while (segnum != DWORD_MAX) {
        ...
        angle_t segAngle = PointToAngle(Vertices[seg->v1].x - v->x,
                                        Vertices[seg->v1].y - v->y);
        angle_t diff = splitAngle - segAngle;

        if (diff < ANGLE_EPSILON &&
            PointOnSide(Vertices[seg->v1].x, Vertices[seg->v1].y, v->x, v->y,
                        dx, dy) == 0) {
    ...
    while (segnum != DWORD_MAX) {
        ...
        angle_t segAngle = PointToAngle(Vertices[seg->v2].x - v->x,
                                        Vertices[seg->v2].y - v->y);
        angle_t diff = splitAngle - segAngle;
