...
FLevel::FLevel() { memset(this, 0, sizeof(*this)); }
...
FProcessor::FProcessor(FWadReader &inwad, int lump) : Wad(inwad), Lump(lump) {
    ...
    if (Level.NumLines() == 0 || Level.NumVertices == 0 ||
        Level.NumSides() == 0 || Level.NumSectors() == 0) {
...
void FLevel::RemoveExtraSides() {
    ...
    for (i = 0; i < NumLines(); ++i) {
        if (Lines[i].sidenum[0] != NO_INDEX) {
            ...
            printf(
                "   Line %d needs a front sidedef before it will run with "
                "ZDoom.\n",
                i);
        }
...
void FLevel::RemoveExtraSectors() {
    ...
    for (i = 0; i < NumSides(); ++i) {
        if ((DWORD)Sides[i].sector != NO_INDEX) {
            ...
            printf(
                "   Sidedef %d needs a front sector before it will run with "
                "ZDoom.\n",
                i);
        }
    }
...
void FProcessor::Write(FWadWriter &out) {
    if (Level.NumLines() == 0 || Level.NumSides() == 0 ||
        Level.NumSectors() == 0 || Level.NumVertices == 0) {
        if (!isUDMF) {
            ...
            for (int i = Lump;
                 strcasecmp(Wad.LumpName(i), "ENDMAP") && i < Wad.NumLumps();
                 i++) {
                out.CopyLump(Wad, i);
            }
    ...
    if (BuildNodes) {
        ...
        try {
            builder = new FNodeBuilder(Level, PolyStarts, PolyAnchors,
                                       Wad.LumpName(Lump), BuildGLNodes);
            if (builder == NULL) {
            ...
            if (ConformNodes) {
                // When the nodes are "conformed", the normal and GL nodes use
                // the same basic information. This creates normal nodes that
                // are less "good" than possible, but it makes it easier to
                // compare the two sets of nodes to determine the correctness of
                // the GL nodes.
                builder->GetNodes(Level.Nodes, Level.NumNodes, Level.Segs,
                                  Level.NumSegs, Level.Subsectors,
                                  Level.NumSubsectors);
                ...
                if (BuildGLNodes) {
                    ...
                    if (!GLOnly) {
                        ...
                        delete builder;
                        builder =
                            new FNodeBuilder(Level, PolyStarts, PolyAnchors,
                                             Wad.LumpName(Lump), false);
                        if (builder == NULL) {
                            throw std::runtime_error(
                                "   Not enough memory to build regular nodes!");
                        }
                        delete[] Level.Vertices;
                        builder->GetVertices(Level.Vertices, Level.NumVertices);
                    }
        ...
        switch (RejectMode) {
            ...
            case ERM_DontTouch: {
                ...
                if (lump >= 0) {
                    ...
                    if (Level.RejectSize !=
                        (Level.NumOrgSectors * Level.NumOrgSectors + 7) / 8) {
                        // If the reject is the wrong size, don't use it.
                        delete[] Level.Reject;
                        Level.Reject = NULL;
                        if (Level.RejectSize !=
                            0) {  // Do not warn about 0-length rejects
                            printf(
                                "   REJECT is the wrong size, so it will be "
                                "removed.\n");
                        }
                        Level.RejectSize = 0;
                        ...
                        Level.RejectSize =
                            (Level.NumSectors() * Level.NumSectors() + 7) / 8;
                    }
                }
            } break;
    ...
    if (!isUDMF) {
        ...
        // If the GL nodes are compressed, then the regular nodes must also be
        // compressed.
        compress = CompressNodes || compressGL || (Level.NumVertices > 65535) ||
        ...
        WriteSides(out);
        WriteVertices(
            out, compress || GLOnly ? Level.NumOrgVerts : Level.NumVertices);
        if (BuildNodes) {
            if (!compress) {
                if (!GLOnly) {
                    WriteSegs(out);
...
MapSubsectorEx *FProcessor::SubsectorsToEx(const MapSubsector *ssec,
                                           int count) {
    if (count == 0) {
        return NULL;
    }
...
void FProcessor::WriteSSectors2(FWadWriter &out, const char *name,
                                const MapSubsectorEx *subs, int count) const {
    int i;
    MapSubsector *ssec;
...
void FProcessor::WriteSSectors5(FWadWriter &out, const char *name,
                                const MapSubsectorEx *subs, int count) const {
    int i;
    MapSubsectorEx *ssec;
...
void FProcessor::WriteNodes2(FWadWriter &out, const char *name,
                             const MapNodeEx *zaNodes, int count) const {
    int i, j;
    short *onodes, *nodes;
    ...
    for (i = 0; i < count; ++i) {
        ...
        for (j = 0; j < 2; ++j) {
            ...
            if (child & NFX_SUBSECTOR) {
                *nodes++ =
                    LittleShort(WORD(child - (NFX_SUBSECTOR + NF_SUBSECTOR)));
            } else {
...
void FProcessor::WriteNodes5(FWadWriter &out, const char *name,
                             const MapNodeEx *zaNodes, int count) const {
    int i, j;
...
void FProcessor::WriteBlockmap(FWadWriter &out) {
    ...
    if (count >= 65536) {
        printf(
            ...
            "   Vanilla Doom cannot handle it at all. If this map is for ZDoom "
            "2+,\n"
...
void FProcessor::WriteGLSegs(FWadWriter &out, bool v5) {
    ...
    for (i = 0; i < count; ++i) {
        if (Level.GLSegs[i].v1 < (DWORD)Level.NumOrgVerts) {
            ...
            segdata[i].v1 = LittleShort(
                0x8000 | (WORD)(Level.GLSegs[i].v1 - Level.NumOrgVerts));
        }
        if (Level.GLSegs[i].v2 < (DWORD)Level.NumOrgVerts) {
            segdata[i].v2 = (WORD)LittleShort(Level.GLSegs[i].v2);
        } else {
            segdata[i].v2 = LittleShort(
                0x8000 | (WORD)(Level.GLSegs[i].v2 - Level.NumOrgVerts));
        }
...
void FProcessor::WriteGLSegs5(FWadWriter &out) {
    ...
    for (i = 0; i < count; ++i) {
        if (Level.GLSegs[i].v1 < (DWORD)Level.NumOrgVerts) {
            ...
            segdata[i].v1 = LittleLong(
                0x80000000u | ((int)Level.GLSegs[i].v1 - Level.NumOrgVerts));
        }
        if (Level.GLSegs[i].v2 < (DWORD)Level.NumOrgVerts) {
            segdata[i].v2 = LittleLong(Level.GLSegs[i].v2);
        } else {
            segdata[i].v2 = LittleLong(
                0x80000000u | ((int)Level.GLSegs[i].v2 - Level.NumOrgVerts));
        }
...
void FProcessor::WriteGLSSect(FWadWriter &out, bool v5) {
    if (!v5) {
        WriteSSectors2(out, "GL_SSECT", Level.GLSubsectors,
                       Level.NumGLSubsectors);
    } else {
        WriteSSectors5(out, "GL_SSECT", Level.GLSubsectors,
                       Level.NumGLSubsectors);
    }
}
...
void FProcessor::WriteBSPZ(FWadWriter &out, const char *label) {
    ...
    WriteVerticesZ(zout, &Level.Vertices[Level.NumOrgVerts], Level.NumOrgVerts,
...
void FProcessor::WriteGLBSPZ(FWadWriter &out, const char *label) {
    ...
    WriteVerticesZ(zout, &Level.GLVertices[Level.NumOrgVerts],
                   Level.NumOrgVerts, Level.NumGLVertices - Level.NumOrgVerts);
...
void FProcessor::WriteVerticesZ(ZLibOut &out, const WideVertex *verts,
                                int orgverts, int newverts) {
...
void FProcessor::WriteSubsectorsZ(ZLibOut &out, const MapSubsectorEx *subs,
                                  int numsubs) {
    out << (DWORD)numsubs;
...
void FProcessor::WriteGLSegsZ(ZLibOut &out, const MapSegGLEx *segs, int numsegs,
    ...
    if (nodever < 2) {
        for (int i = 0; i < numsegs; ++i) {
            out << (DWORD)segs[i].v1 << (DWORD)segs[i].partner
                << (WORD)segs[i].linedef << (BYTE)segs[i].side;
        }
    } else {
        for (int i = 0; i < numsegs; ++i) {
            out << (DWORD)segs[i].v1 << (DWORD)segs[i].partner
                << (DWORD)segs[i].linedef << (BYTE)segs[i].side;
        }
    }
}
...
void FProcessor::WriteBSPX(FWadWriter &out, const char *label) {
    ...
    WriteVerticesX(out, &Level.Vertices[Level.NumOrgVerts], Level.NumOrgVerts,
                   Level.NumVertices - Level.NumOrgVerts);
...
void FProcessor::WriteGLBSPX(FWadWriter &out, const char *label) {
    ...
    if (!CompressGLNodes) {
        printf(
            "   GL Nodes are so big that extended format has been forced.\n");
    }
    ...
    WriteVerticesX(out, &Level.GLVertices[Level.NumOrgVerts], Level.NumOrgVerts,
...
void FProcessor::WriteVerticesX(FWadWriter &out, const WideVertex *verts,
                                int orgverts, int newverts) {
    out << (DWORD)orgverts << (DWORD)newverts;
...
void FProcessor::WriteSubsectorsX(FWadWriter &out, const MapSubsectorEx *subs,
                                  int numsubs) {
    out << (DWORD)numsubs;
...
void FProcessor::WriteSegsX(FWadWriter &out, const MapSegEx *segs,
                            int numsegs) {
    out << (DWORD)numsegs;
...
void FProcessor::WriteGLSegsX(FWadWriter &out, const MapSegGLEx *segs,
                              int numsegs, int nodever) {
    out << (DWORD)numsegs;
...
void FProcessor::WriteNodesX(FWadWriter &out, const MapNodeEx *nodes,
                             int numnodes, int nodever) {
    out << (DWORD)numnodes;
...
bool FProcessor::CheckForFracSplitters(const MapNodeEx *nodes, int numnodes) {
    for (int i = 0; i < numnodes; ++i) {
        if (0 != ((nodes[i].x | nodes[i].y | nodes[i].dx | nodes[i].dy) &
                  0x0000FFFF)) {
            return true;
        }
    }
