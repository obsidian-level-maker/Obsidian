...
int BoxOnSide(int bx1, int by1, int bx2, int by2, int lx1, int ly1, int lx2,
              int ly2) {
    int p1;
    int p2;
        ...
        for (i = 0; i < stuff[2] * stuff[3]; ++i) {
            ...
            while (stuff[i1] != -1) {
                ...
                if (BlockMap[i2] == 0xffff) {
                    ...
                    x = i - y * stuff[2];
                    int l = stuff[i1];
                    // If a diagonal line passed near a block (within 2 or 4
                    // units, I think), it could be considered in the block even
                    // if it's really outside it, so if things differ, see if
                    // DoomBSP was at fault.
                    if (BoxOnSide(
                        ...
                        printf(
                            "not in cell %4d: line %4d [%2d,%2d] : "
                            "(%5d,%5d)-(%5d,%5d)\n",
                            i, stuff[i1], x, y, stuff[0] + 128 * x,
                            stuff[1] + 128 * y, stuff[0] + 128 * x + 127,
                            stuff[1] + 128 * y + 127);
                    }
                }
                i1++;
            ...
            while (BlockMap[i1] != 0xffff) {
                i2 = stuff[4 + i] + 1;
                while (stuff[i2] != -1) {
                    if ((WORD)stuff[i2] == BlockMap[i1]) break;
                    i2++;
                }
                if (stuff[i2] == -1) {
                    y = i / BlockMap[2];
                    ...
                    int l = BlockMap[i1];
                    if (!BoxOnSide(
                            (short)BlockMap[0] + 128 * x,
                            (short)BlockMap[1] + 128 * y,
                            (short)BlockMap[0] + 128 * x + 127,
                            (short)BlockMap[1] + 128 * y + 127,
                            Vertices[Lines[l].v1].x, Vertices[Lines[l].v1].y,
                            Vertices[Lines[l].v2].x, Vertices[Lines[l].v2].y)) {
                        printf(
                            "EXT in cell %4d: line %4d [%2d,%2d] : "
                            "(%5d,%5d)-(%5d,%5d)\n",
                            i, (short)BlockMap[i1], x, y,
                            (short)BlockMap[0] + 128 * x,
                            (short)BlockMap[1] + 128 * y,
                            (short)BlockMap[0] + 128 * x + 127,
                            (short)BlockMap[1] + 128 * y + 127);
    ...
    for (line = 0; line < Level.NumLines(); ++line) {
        ...
        } else  // Diagonal line
        {
            ...
            if (adx >= ady)  // X-major
            {
                ...
                do {
                    int stop =
                        (Scale((by << BLOCKBITS) + yadd - (y1 - miny), dx, dy) +
                         (x1 - minx)) >>
                        BLOCKBITS;
                    while (bx != stop) {
                        block->Push(line);
            ...
            } else  // Y-major
            {
                ...
                do {
                    int stop =
                        (Scale((bx << BLOCKBITS) + xadd - (x1 - minx), dy, dx) +
                         (y1 - miny)) >>
                        BLOCKBITS;
                    while (by != stop) {
                        block->Push(line);
...
void FBlockmapBuilder::CreateUnpackedBlockmap(TArray<WORD> *blocks,
                                              int bmapwidth, int bmapheight) {
    TArray<WORD> *block;
    WORD zero = 0;
...
void FBlockmapBuilder::CreatePackedBlockmap(TArray<WORD> *blocks, int bmapwidth,
                                            int bmapheight) {
    WORD buckets[4096];
