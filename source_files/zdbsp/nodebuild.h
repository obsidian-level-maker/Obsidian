...
extern "C" {
int ClassifyLine2(node_t &node, const FSimpleVert *v1, const FSimpleVert *v2,
                  int sidev[2]);
#ifdef BACKPATCH
#ifdef __GNUC__
int ClassifyLineBackpatch(node_t &node, const FSimpleVert *v1,
                          const FSimpleVert *v2, int sidev[2])
    __attribute__((noinline));
#else
int __declspec(noinline)
    ClassifyLineBackpatch(node_t &node, const FSimpleVert *v1,
                          const FSimpleVert *v2, int sidev[2]);
#endif
#endif
}
...
class FNodeBuilder {
    struct FPrivSeg {
        ...
        DWORD nextforvert;
        DWORD nextforvert2;
        int loopnum;    // loop number for split avoidance (0 means splitting is
                        // okay)
    ...
    class FVertexMap {
       public:
        FVertexMap(FNodeBuilder &builder, fixed_t minx, fixed_t miny,
                   fixed_t maxx, fixed_t maxy);
        ~FVertexMap();
   ...
   public:
    ...
    FNodeBuilder(FLevel &level, TArray<FPolyStart> &polyspots,
                 TArray<FPolyStart> &anchors, const char *name,
                 bool makeGLnodes);
    ~FNodeBuilder();

    void GetVertices(WideVertex *&verts, int &count);
    void GetNodes(MapNodeEx *&nodes, int &nodeCount, MapSegEx *&segs,
                  int &segCount, MapSubsectorEx *&ssecs, int &subCount);

    void GetGLNodes(MapNodeEx *&nodes, int &nodeCount, MapSegGLEx *&segs,
                    int &segCount, MapSubsectorEx *&ssecs, int &subCount);
   ...
   private:
    ...
    size_t InitialVertices;  // Number of vertices in a map that are connected
                             // to linedefs
    ...
    TArray<FSplitSharer>
        SplitSharers;  // Segs collinear with the current splitter
    ...
    void GroupSegPlanes();
    void FindPolyContainers(TArray<FPolyStart> &spots,
    ...
    bool CheckSubsectorOverlappingSegs(DWORD set, node_t &node,
                                       DWORD &splitseg);
    bool ShoveSegBehind(DWORD set, node_t &node, DWORD seg, DWORD mate);
    int SelectSplitter(DWORD set, node_t &node, DWORD &splitseg, int step,
                       bool nosplit);
    void SplitSegs(DWORD set, node_t &node, DWORD splitseg, DWORD &outset0,
                   DWORD &outset1, unsigned int &count0, unsigned int &count1);
    ...
    inline int ClassifyLine(node_t &node, const FPrivVert *v1,
                            const FPrivVert *v2, int sidev[2]);

    void FixSplitSharers();
    double AddIntersection(const node_t &node, int vertex);
    void AddMinisegs(const node_t &node, DWORD splitseg, DWORD &fset,
                     DWORD &rset);
    ...
    int RemoveMinisegs(MapNodeEx *nodes, TArray<MapSegEx> &segs,
                       MapSubsectorEx *subs, int node, short bbox[4]);
    ...
    void PushConnectingGLSeg(int subsector, TArray<MapSegGLEx> &segs, int v1,
                             int v2);
    int OutputDegenerateSubsector(TArray<MapSegGLEx> &segs, int subsector,
                                  bool bForward, double lastdot,
                                  FPrivSeg *&prev);
...
// Vertices within this distance of each other will be considered as the same
// vertex.
#define VERTEX_EPSILON 6  // This is a fixed_t value

inline int FNodeBuilder::PointOnSide(int x, int y, int x1, int y1, int dx,
                                     int dy) {
    ...
    if (fabs(s_num) < 17179869184.f)  // 4<<32
    {
        ...
        double l =
            d_dx * d_dx + d_dy * d_dy;  // double l = sqrt(d_dx*d_dx+d_dy*d_dy);
...
inline int FNodeBuilder::ClassifyLine(node_t &node, const FPrivVert *v1,
                                      const FPrivVert *v2, int sidev[2]) {
