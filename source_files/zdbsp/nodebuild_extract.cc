...
int FNodeBuilder::CloseSubsector(TArray<MapSegGLEx> &segs, int subsector) {
    ...
    // Calculate the midpoint of the subsector and also check for degenerate
    // subsectors. A subsector is degenerate if it exists in only one dimension,
    // which can be detected when all the segs lie in the same plane. This can
    // happen if you have outward-facing lines in the void that don't point
    // toward any sector. (Some of the polyobjects in Hexen are constructed like
    // this.)
    for (i = first; i < max; ++i) {
    ...
    prevAngle =
        PointToAngle(Vertices[seg->v1].x - midx, Vertices[seg->v1].y - midy);
    ...
    for (j = first; j < max; ++j) {
        ...
        angle_t ang = PointToAngle(Vertices[seg->v1].x - midx,
                                   Vertices[seg->v1].y - midy);
        printf(
            "%d%c %5d(%5d,%5d)->%5d(%5d,%5d) - %3.5f  %d,%d  "
            "[%08x,%08x]-[%08x,%08x]\n",
            j, seg->linedef == -1 ? '+' : ':', seg->v1,
            Vertices[seg->v1].x >> 16, Vertices[seg->v1].y >> 16, seg->v2,
            Vertices[seg->v2].x >> 16, Vertices[seg->v2].y >> 16,
            double(ang / 2) * 180 / (1 << 30), seg->planenum, seg->planefront,
            Vertices[seg->v1].x, Vertices[seg->v1].y, Vertices[seg->v2].x,
            Vertices[seg->v2].y);
    }
#endif

    if (diffplanes) {  // A well-behaved subsector. Output the segs sorted by
                       // the angle formed by connecting the subsector's center
        ...
        printf("\n");
#endif
    } else {  // A degenerate subsector. These are handled in three stages:
              // Stage 1. Proceed in the same direction as the start seg until
              // we
        ...
        // Stage 2. Go backward.
        count +=
            OutputDegenerateSubsector(segs, subsector, false, DBL_MAX, prev);

        // Stage 3. Go forward again.
        count +=
            OutputDegenerateSubsector(segs, subsector, true, -DBL_MAX, prev);
    }
    ...
    for (i = segs.Size() - count; i < (int)segs.Size(); ++i) {
        printf("  Seg %5d%c(%5d,%5d)-(%5d,%5d)  [%08x,%08x]-[%08x,%08x]\n", i,
               ...
               Vertices[segs[i].v1].x >> 16, Vertices[segs[i].v1].y >> 16,
               Vertices[segs[i].v2].x >> 16, Vertices[segs[i].v2].y >> 16,
               Vertices[segs[i].v1].x, Vertices[segs[i].v1].y,
...
int FNodeBuilder::OutputDegenerateSubsector(TArray<MapSegGLEx> &segs,
                                            int subsector, bool bForward,
                                            double lastdot, FPrivSeg *&prev) {
...
DWORD FNodeBuilder::PushGLSeg(TArray<MapSegGLEx> &segs, const FPrivSeg *seg) {
    ...
    // When a level is sidedef compressed both sides may well have the same
    // sidedef.
    ...
    if (newseg.linedef != NO_INDEX) {
        ...
        if (ld->sidenum[0] == ld->sidenum[1]) {
            // When both sidedefs are the same a quick check doesn't work so
            // this has to be done by comparing the distances of the seg's end
            // point to the line's start.
...
void FNodeBuilder::PushConnectingGLSeg(int subsector, TArray<MapSegGLEx> &segs,
                                       int v1, int v2) {
    MapSegGLEx newseg;
...
int FNodeBuilder::RemoveMinisegs(MapNodeEx *nodes, TArray<MapSegEx> &segs,
                                 MapSubsectorEx *subs, int node,
                                 short bbox[4]) {
    if (node & NFX_SUBSECTOR) {
        ...
        int child0 = RemoveMinisegs(nodes, segs, subs, orgnode->intchildren[0],
                                    newnode->bbox[0]);
        int child1 = RemoveMinisegs(nodes, segs, subs, orgnode->intchildren[1],
                                    newnode->bbox[1]);
        ...
        bbox[BOXBOTTOM] =
            MIN(newnode->bbox[0][BOXBOTTOM], newnode->bbox[1][BOXBOTTOM]);
        bbox[BOXLEFT] =
            MIN(newnode->bbox[0][BOXLEFT], newnode->bbox[1][BOXLEFT]);
        bbox[BOXRIGHT] =
            MAX(newnode->bbox[0][BOXRIGHT], newnode->bbox[1][BOXRIGHT]);

        return node;
    }
}

int FNodeBuilder::StripMinisegs(TArray<MapSegEx> &segs, int subsector,
                                short bbox[4]) {
    int count, i, max;
    ...
    for (count = 0; i < max; ++i) {
        ...
        if (org->linedef == -1) {
            ...
            // Just checking the sidedef to determine the side is insufficient.
            // When a level is sidedef compressed both sides may well have the
            // same sidedef.
            ...
            if (ld->sidenum[0] == ld->sidenum[1]) {
                // When both sidedefs are the same a quick check doesn't work so
                // this has to be done by comparing the distances of the seg's
                // end point to the line's start.
                WideVertex *lv1 = &Level.Vertices[ld->v1];
...
void FNodeBuilder::DumpNodes(MapNodeEx *outNodes, int nodeCount) {
    for (unsigned int i = 0; i < Nodes.Size(); ++i) {
        printf("Node %d:  Splitter[%08x,%08x] [%08x,%08x]\n", i, outNodes[i].x,
               outNodes[i].y, outNodes[i].dx, outNodes[i].dy);
        for (int j = 1; j >= 0; --j) {
