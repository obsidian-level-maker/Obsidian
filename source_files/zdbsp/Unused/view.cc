...
void ResizeDialog(HWND hDlg) {
    ...
    SetWindowPos(control, 0, 0, 0, client.right - MapWidthDiff,
                 client.bottom - MapHeightDiff,
                 SWP_NOMOVE | SWP_NOREPOSITION | SWP_NOZORDER);

    control = GetDlgItem(hDlg, IDOK);
    SetWindowPos(control, 0, ButtonsLeftOffset,
                 client.bottom - ButtonsTopOffset, 0, 0,
                 SWP_NOSIZE | SWP_NOREPOSITION | SWP_NOZORDER);

    control = GetDlgItem(hDlg, IDC_STATICNUMBER);
    SetWindowPos(control, 0, StaticNumberLeft, client.bottom - ButtonsTopOffset,
                 0, 0, SWP_NOSIZE | SWP_NOREPOSITION | SWP_NOZORDER);

    control = GetDlgItem(hDlg, IDC_COMBO1);
    SetWindowPos(control, 0, StaticComboLeft, client.bottom - ButtonsTopOffset,
                 0, 0, SWP_NOSIZE | SWP_NOREPOSITION | SWP_NOZORDER);
}
...
INT_PTR CALLBACK ViewDialogFunc(HWND hDlg, UINT uMsg, WPARAM wParam,
                                LPARAM lParam) {
    HWND control;
    RECT rect1, rect2;
                ...
                case IDC_COMBO1:
                    if (HIWORD(wParam) == CBN_SELENDOK) {
                        ...
                        if (newmode != Viewing) {
                            Viewing = newmode;
                            ResetViews();
                            InvalidateRect(GetDlgItem(hDlg, IDC_MAPVIEW), NULL,
                                           TRUE);
                        }
                    }
                    break;
...
static inline int VERTX(int i) { return Level->Vertices[i].x >> FRACBITS; }

static inline int VERTY(int i) { return Level->Vertices[i].y >> FRACBITS; }

static inline int GLVERTX(int i) { return Level->GLVertices[i].x >> FRACBITS; }

static inline int GLVERTY(int i) { return Level->GLVertices[i].y >> FRACBITS; }
...
static void DrawSubsector(HDC dc, int ssec) {
    for (DWORD i = 0; i < Level->Subsectors[ssec].numlines; ++i) {
        ...
        if (Level->Segs[seg].side == 0) {
            MoveToEx(dc, VERTX(Level->Segs[seg].v1), VERTY(Level->Segs[seg].v1),
                     NULL);
            LineTo(dc, VERTX(Level->Segs[seg].v2), VERTY(Level->Segs[seg].v2));
        } else {
            MoveToEx(dc, VERTX(Level->Segs[seg].v2), VERTY(Level->Segs[seg].v2),
                     NULL);
...
static void DrawSubsectorGL(HDC dc, int ssec, HPEN miniPen, HPEN badPen) {
    ...
    MoveToEx(dc, GLVERTX(Level->GLSegs[seg].v1), GLVERTY(Level->GLSegs[seg].v1),
             NULL);
    for (DWORD i = 0; i < Level->GLSubsectors[ssec].numlines; ++i) {
        ...
        if (Level->GLSegs[seg].side == 0 || 1) {
            // MoveToEx (dc, GLVERTX(Level->GLSegs[seg].v1),
            // GLVERTY(Level->GLSegs[seg].v1), NULL);
            LineTo(dc, GLVERTX(Level->GLSegs[seg].v2),
                   GLVERTY(Level->GLSegs[seg].v2));
        } else {
            // MoveToEx (dc, GLVERTX(Level->GLSegs[seg].v2),
            // GLVERTY(Level->GLSegs[seg].v2), NULL);
            LineTo(dc, GLVERTX(Level->GLSegs[seg].v1),
                   GLVERTY(Level->GLSegs[seg].v1));
        }
        if (oldPen != NULL) {
    ...
    if (node & NFX_SUBSECTOR) {
        ...
        if (Viewing == ViewNodes) {
            if (node == DesiredNode) {
                ...
                RealDrawNode(dc, Level->Nodes[node].children[1], LeftPen1,
                             LeftPen2);
                SelectObject(dc, RightOfSplitter);
                RealDrawNode(dc, Level->Nodes[node].children[0], RightPen1,
                             RightPen2);
                return;
            } else {
            ...
            if (node == DesiredNode) {
                SelectObject(dc, LeftOfSplitter);
                RealDrawNode(dc, Level->GLNodes[node].children[1], LeftPen1,
                             LeftPen2);
                SelectObject(dc, RightOfSplitter);
                RealDrawNode(dc, Level->GLNodes[node].children[0], RightPen1,
                             RightPen2);
                return;
            } else {
...
static void SetDesiredSubsector(int x, int y) {
    if (Viewing == ViewSubsectors) {
        ...
        while (!(node & NFX_SUBSECTOR)) {
            node = Level->Nodes[node]
                       .children[PointOnSide(x, y, Level->Nodes[node])];
        }
        ...
        while (!(node & NFX_SUBSECTOR)) {
            node = Level->GLNodes[node]
                       .children[PointOnSide(x, y, Level->GLNodes[node])];
        }
        DesiredSubsector = node & ~NFX_SUBSECTOR;
    }
}
...
static void DrawLevelReject(HDC dc) {
    ...
    for (int i = 0; i < Level->NumLines(); ++i) {
        ...
        if (Level->Lines[i].sidenum[0] != NO_INDEX) {
            if (Level->Sides[Level->Lines[i].sidenum[0]].sector ==
                DesiredSector) {
                choice = SEE_FROM;
            } else if (Level->Reject != NULL) {
                int pnum = seeFromRow +
                           Level->Sides[Level->Lines[i].sidenum[0]].sector;
        ...
        if (Level->Lines[i].sidenum[1] != NO_INDEX && choice < SEE_FROM) {
            if (Level->Sides[Level->Lines[i].sidenum[1]].sector ==
                DesiredSector) {
                choice = SEE_FROM;
            } else if (Level->Reject != NULL && choice < CAN_SEE) {
                int pnum = seeFromRow +
                           Level->Sides[Level->Lines[i].sidenum[1]].sector;
                if (Level->Reject[pnum >> 3] & (1 << (pnum & 7))) {
                    choice = CANT_SEE;
                } else {
                    choice = CAN_SEE;
        ...
        if (choice != UNDECIDED) {
            if (choice != prevchoice) {
                ...
                switch (choice) {
                    case CANT_SEE:
                        SelectObject(dc, cantSee);
                        break;
                    case CAN_SEE:
                        SelectObject(dc, canSee);
                        break;
                    case SEE_FROM:
                        SelectObject(dc, seeFrom);
                        break;
                    default:
                        break;
                }
            }
            MoveToEx(dc, VERTX(Level->Lines[i].v1), VERTY(Level->Lines[i].v1),
                     NULL);
...
static void SetDesiredSector(int x, int y) {
    ...
    while (!(node & NFX_SUBSECTOR)) {
        node =
            Level->Nodes[node].children[PointOnSide(x, y, Level->Nodes[node])];
    }
    node &= ~NFX_SUBSECTOR;
...
static void DrawLevel(HDC dc) {
    ...
    {
            MoveToEx (dc, VERTX(Level->Segs[i].v1), VERTY(Level->Segs[i].v1),
    NULL); LineTo (dc, VERTX(Level->Segs[i].v2), VERTY(Level->Segs[i].v2));
    }
    */
}
...
void SizeView(HWND wnd, bool firstTime) {
    ...
    if (firstTime) {
        ...
        ScrollWindowEx(wnd, 0, (delta + Divisor - 1) / Divisor, NULL, NULL,
    ...
    if (firstTime) {
        ...
        int delta = sinfo.nPos - sinfo.nMax + (int)sinfo.nPage;
        ScrollWindowEx(wnd, (delta + Divisor - 1) / Divisor, 0, NULL, NULL,
                       NULL, NULL, SW_INVALIDATE | SW_ERASE);
        sinfo.nPos = sinfo.nMax - sinfo.nPage;
    }
...
LRESULT CALLBACK MapViewFunc(HWND hWnd, UINT uMsg, WPARAM wParam,
                             LPARAM lParam) {
    ...
    switch (uMsg) {
        ...
        case WM_PAINT:
            if (GetUpdateRect(hWnd, NULL, FALSE)) {
                ...
                if (dc != NULL) {
                    ...
                    SetWindowOrgEx(dc, pos + MapBounds.left,
                                   MapBounds.bottom - sinfo.nPos, NULL);
                    DrawLevel(dc);
                    EndPaint(hWnd, &paint);
                }
            }
        ...
        case WM_HSCROLL:
            ...
            if (pos != sinfo.nPos) {
                ...
                ScrollWindowEx(hWnd, oldx - newx, 0, NULL, NULL, NULL, NULL,
        ...
        case WM_VSCROLL:
            ...
            if (pos != sinfo.nPos) {
                ...
                ScrollWindowEx(hWnd, 0, (sinfo.nPos - pos) / Divisor, NULL,
        ...
        case WM_RBUTTONDOWN:
            dragging = true;
            dragpos_lp = lParam;  // also sets dragpos; avoids type-punned
                                  // dereference warning from GCC
        ...
        case WM_MOUSEMOVE:
            if (!(wParam & MK_RBUTTON)) {
                ...
                if (delta) {
                    ...
                    if (pos != sinfo.nPos) {
                        SetScrollPos(hWnd, SB_HORZ, pos, TRUE);
                        ScrollWindowEx(hWnd, (sinfo.nPos - pos) / Divisor, 0,
                                       NULL, NULL, NULL, NULL,
                                       SW_INVALIDATE | SW_ERASE);
                        UpdateWindow(hWnd);
                    }
                ...
                if (delta) {
                    ...
                    if (pos != sinfo.nPos) {
                        SetScrollPos(hWnd, SB_VERT, pos, TRUE);
                        ScrollWindowEx(hWnd, 0, (sinfo.nPos - pos) / Divisor,
                                       NULL, NULL, NULL, NULL,
                                       SW_INVALIDATE | SW_ERASE);
                        UpdateWindow(hWnd);
                    }
...
void ShowView(FLevel *level) {
    ...
    if (RegisterClass(&MapViewerClass)) {
        ...
        if (level->Blockmap != NULL) {
            ...
            MapBounds.right = short(level->Blockmap[0]) +
                              (level->Blockmap[2] << BLOCKBITS) + 8;
            MapBounds.top = short(level->Blockmap[1]) - 8;
            MapBounds.bottom = short(level->Blockmap[1]) +
                               (level->Blockmap[3] << BLOCKBITS) + 8;
        } else {
        ...
        ResetViews();
        DialogBox(GetModuleHandle(0), MAKEINTRESOURCE(IDD_MAPVIEW), NULL,
                  ViewDialogFunc);
