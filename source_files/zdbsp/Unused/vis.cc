...
void FRejectBuilder::RunThreadsOnIndividual(int workcnt, bool showpacifier,
                                            void (FRejectBuilder::*func)(int)) {
    int work;
...
==============
*/
int FRejectBuilder::LeafVectorFromPortalVector(BYTE *portalbits,
                                               BYTE *leafbits) {
    int i, j, leafnum;
    VPortal *p;
    int c_leafs;
    ...
    for (i = 0; i < leaf->numportals; i++) {
        ...
        try {
            if (p->status != STAT_Done) {
                throw std::runtime_error("portal not done");
            }
...
void FRejectBuilder::CalcPortalVis() {
    ...
    RunThreadsOnIndividual(numportals, false, &FRejectBuilder::PortalFlow);
#else
    RunThreadsOnIndividual(numportals, true, &FRejectBuilder::PortalFlow);
#endif
}
...
#ifdef MREDEBUG
    _printf("%6d portals out of %d", 0, numportals);
    RunThreadsOnIndividual(numportals, false, &FRejectBuilder::CreatePassages);
    _printf("\n");
    _printf("%6d portals out of %d", 0, numportals);
    RunThreadsOnIndividual(numportals, false,
                           &FRejectBuilder::PassagePortalFlow);
    _printf("\n");
#else
    RunThreadsOnIndividual(numportals, true, &FRejectBuilder::CreatePassages);
    printf(" Vis 3: ");
...
=============
*/
bool FRejectBuilder::Winding_PlanesConcave(const FWinding *w1,
                                           const FWinding *w2,
                                           const FLine &line1,
                                           const FLine &line2) {
    // check if one of the points of winding 1 is at the front of the line of
    // winding 2
    ...
    // check if one of the points of winding 2 is at the front of the line of
    // winding 1
...
bool FRejectBuilder::TryMergeLeaves(int l1num, int l2num) {
    ...
    for (i = 0; i < l1->numportals; i++) {
        ...
        for (j = 0; j < l2->numportals; j++) {
            ...
            //
            if (Winding_PlanesConcave(&p1->winding, &p2->winding, p1->line,
                                      p2->line))
                return false;
        }
    }
...
FRejectBuilder::FWinding *FRejectBuilder::TryMergeWinding(FWinding *f1,
                                                          FWinding *f2,
                                                          const FLine &line) {
...
found:
    ...
    if (PointOnSide(f2->points[0], line) != 0 ||
        PointOnSide(f2->points[1], line) != 0) {  // not colinear
        return NULL;
    }
        ...
        for (j = 0; j < leaf->numportals; j++) {
            ...
            for (k = j + 1; k < leaf->numportals; k++) {
                ...
                if (p1->leaf == p2->leaf) {
                    ...
                    if (w) {
                        p1->winding = *w;
                        if (p1->hint && p2->hint) hintsmerged++;
                        p1->hint |= p2->hint;
                        p2->removed = true;
                        nummerges++;
                        i--;
