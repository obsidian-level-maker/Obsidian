...
void FRejectBuilder::CheckStack(FLeaf *leaf, FThreadData *thread) {
    ...
    try {
        for (p = thread->pstack_head.next; p != NULL; p = p->next) {
            //		_printf ("=");
            if (p->leaf == leaf)
                throw std::runtime_error("CheckStack: leaf recursion");
            for (p2 = thread->pstack_head.next; p2 != p; p2 = p2->next)
                if (p2->leaf == p->leaf)
                    throw std::runtime_error("CheckStack: late leaf recursion");
        }
...
FRejectBuilder::FWinding *FRejectBuilder::AllocStackWinding(
    PStack *stack) const {
    try {
        for (int i = 0; i < 3; i++) {
...
void FRejectBuilder::FreeStackWinding(FWinding *w, PStack *stack) const {
    try {
        ...
        if (stack->freewindings[i])
            throw std::runtime_error("FreeStackWinding: already free");
...
==============
*/
FRejectBuilder::FWinding *FRejectBuilder::VisChopWinding(FWinding *in,
                                                         PStack *stack,
                                                         FLine *split) {
    int side1, side2;
    FPoint mid;
    ...
    if (side1 <= 0 && side2 <= 0) {  // completely on front side
        return in;
    }

    if (side1 >= 0 && side2 >= 0) {  // completely on back side
...
==============
*/
FRejectBuilder::FWinding *FRejectBuilder::ClipToSeperators(FWinding *source,
                                                           FWinding *pass,
                                                           FWinding *target,
                                                           bool flipclip,
                                                           PStack *stack) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        for (j = 0; j < 2; j++) {
            ...
            if (d > 0) {  // source is on the back side, so we want all
                          // pass and target on the front side
                fliptest = false;
            } else if (d < 0) {  // source in on the front side, so we want all
                                 // pass and target on the back side
                fliptest = true;
            } else {  // colinear with source portal
                continue;
            }
            ...
            if (d >= 0) {  // == 0: colinear with seperating plane
            ...
            if (!target) {  // target is not visible
                return NULL;
            }
...
==================
*/
void FRejectBuilder::RecursiveLeafFlow(int leafnum, FThreadData *thread,
                                       PStack *prevstack) {
    PStack stack;
    VPortal *p;
    FLine backline;
    ...
    for (i = 0; i < leaf->numportals; i++) {
        ...
        /* MrE: portal trace debug code
        {
                ...
                for (j = 0; s->next && j < sizeof(portaltrace)/sizeof(int) - 1;
        j++, s = s->next)
                {
        ...
        */
        ...
        if (!more && (thread->base->portalvis[pnum >> 3] &
                      (1 << (pnum & 7)))) {  // can't see anything new
            continue;
        }
        ...
        stack.pass = VisChopWinding(&p->winding, &stack,
                                    &thread->pstack_head.portalline);
        if (!stack.pass) {
            continue;
        }
        ...
        if (stack.numseperators[0]) {
            for (n = 0; n < stack.numseperators[0]; n++) {
                stack.pass =
                    VisChopWinding(stack.pass, &stack, &stack.seperators[0][n]);
        ...
        if (stack.numseperators[1]) {
            for (n = 0; n < stack.numseperators[1]; n++) {
                stack.pass =
                    VisChopWinding(stack.pass, &stack, &stack.seperators[1][n]);
                if (!stack.pass) break;  // target is not visible
            }
        } else {
...
==================
*/
void FRejectBuilder::RecursivePassageFlow(VPortal *portal, FThreadData *thread,
                                          PStack *prevstack) {
    PStack stack;
    VPortal *p;
    FLeaf *leaf;
    ...
    for (i = 0; i < leaf->numportals; i++, passage = nextpassage) {
        ...
        portalvis = (p->status == STAT_Done) ? (long *)p->portalvis
                                             : (long *)p->portalflood;
        more = 0;
        for (j = 0; j < portallongs; j++) {
            if (*might) {
...
==================
*/
void FRejectBuilder::RecursivePassagePortalFlow(VPortal *portal,
                                                FThreadData *thread,
                                                PStack *prevstack) {
    PStack stack;
    VPortal *p;
    ...
    for (i = 0; i < leaf->numportals; i++, passage = nextpassage) {
        ...
        if (!prevstack
                 ->pass) {  // the second leaf can only be blocked if colinear
...
FRejectBuilder::FWinding *FRejectBuilder::PassageChopWinding(FWinding *in,
                                                             FWinding *out,
                                                             FLine *split) {
    int side1, side2;
    FPoint mid;
...
===============
*/
int FRejectBuilder::AddSeperators(FWinding *source, FWinding *pass,
                                  bool flipclip, FLine *seperators,
                                  int maxseperators) {
    int i, j;
    FLine line;
    int d;
        ...
        try {
            ...
            for (j = 0; j < 2; j++) {
                ...
                if (d > 0) {  // source is on the back side, so we want all
                              // pass and target on the front side
                    fliptest = false;
                } else if (d < 0) {  // source in on the front side, so we want
                                     // all pass and target on the back side
                    fliptest = true;
                } else {  // colinear with source portal
                    continue;
                }
                ...
                d = PointOnSide(pass->points[!j], line);
                if (d >=
                    0) {  // == 0: colinear with seperating plane
                          //  > 0: points on back side; not a seperating plane
                    continue;
                }
                ...
                if (numseperators >= maxseperators)
                    throw std::runtime_error("max seperators");
...
MrE: create passages from one portal to all the portals in the leaf the portal
leads to every passage has a cansee bit string with all the portals that can be
...
void FRejectBuilder::CreatePassages(int portalnum) {
    ...
    for (i = 0; i < leaf->numportals; i++) {
        ...
        numseperators = AddSeperators(&portal->winding, &target->winding, false,
                                      seperators, MAX_SEPERATORS * 2);
        numseperators += AddSeperators(&target->winding, &portal->winding, true,
                                       &seperators[numseperators],
        ...
        for (j = 0; j < numportals; j++) {
            p = &portals[j];
            if (p->removed) continue;
            if (!(target->portalflood[j >> 3] & (1 << (j & 7)))) continue;
            if (!(portal->portalflood[j >> 3] & (1 << (j & 7)))) continue;
            ...
            for (k = 0; k < numseperators; k++) {
                ...
                if (res == &out) memcpy(&in, &out, sizeof(FWinding));
                if (res == NULL) break;
            }
...
==================
*/
void FRejectBuilder::RecursiveLeafBitFlow(int leafnum, BYTE *mightsee,
                                          BYTE *cansee) {
    VPortal *p;
    FLeaf *leaf;
    int i, j;
    ...
    for (i = 0; i < leaf->numportals; i++) {
        ...
        if (!(mightsee[pnum >> 3] & (1 << (pnum & 7)))) continue;
