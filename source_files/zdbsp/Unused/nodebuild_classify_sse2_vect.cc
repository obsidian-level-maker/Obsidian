...
extern "C" int ClassifyLineSSE2(node_t &node, const FSimpleVert *v1,
                                const FSimpleVert *v2, int sidev[2]) {
    __m128d xy, dxy, xyv1, xyv2;

    // Why does this intrinsic go through an MMX register, when it can just go
    // through memory? That would let it work with x64, too. (This only applies
    // to VC++. GCC is smarter and can load directly from memory without
    // touching the MMX registers.)
    ...
    if ((sv1 | sv2) ==
        0) {  // seg is coplanar with the splitter, so use its orientation to
              // determine which child it ends up in. If it faces the same
              // direction as the splitter, it goes in front. Otherwise, it goes
              // in back.

        if (node.dx != 0) {
            if ((node.dx > 0 && v2->x > v1->x) ||
                (node.dx < 0 && v2->x < v1->x)) {
                return 0;
            } else {
                return 1;
