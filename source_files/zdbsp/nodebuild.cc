...
FNodeBuilder::FNodeBuilder(FLevel &level, TArray<FPolyStart> &polyspots,
                           TArray<FPolyStart> &anchors, const char *name,
                           bool makeGLnodes)
    : Level(level), SegsStuffed(0), MapName(name) {
    VertexMap =
        new FVertexMap(*this, Level.MinX, Level.MinY, Level.MaxX, Level.MaxY);
    GLNodes = makeGLnodes;
...
DWORD FNodeBuilder::CreateNode(DWORD set, unsigned int count, fixed_t bbox[4]) {
    ...
    // When building GL nodes, count may not be an exact count of the number of
    // segs in this set. That's okay, because we just use it to get a skip
    // count, so an estimate is fine.
    skip = int(count / MaxSegs);

    if ((selstat = SelectSplitter(set, node, splitseg, skip, true)) > 0 ||
        (skip > 0 &&
         (selstat = SelectSplitter(set, node, splitseg, 1, true)) > 0) ||
        (selstat < 0 &&
         (SelectSplitter(set, node, splitseg, skip, false) > 0 ||
        ...
        D(Printf("(%d,%d) delta (%d,%d) from seg %d\n", node.x >> 16,
                 node.y >> 16, node.dx >> 16, node.dy >> 16, splitseg));
        D(PrintSet(2, set2));
...
DWORD FNodeBuilder::CreateSubsector(DWORD set, fixed_t bbox[4]) {
    ...
    for (s1 = set; s1 != DWORD_MAX; s1 = Segs[s1].next) {
        for (s2 = Segs[s1].next; s2 != DWORD_MAX; s2 = Segs[s2].next) {
            if (Segs[s1].v1 == Segs[s2].v1)
                printf(
                    "Segs %d%c and %d%c have duplicate start vertex %d (%d, "
                    "%d)\n",
                    s1, Segs[s1].linedef == -1 ? '*' : ' ', s2,
                    Segs[s2].linedef == -1 ? '*' : ' ', Segs[s1].v1,
                    Vertices[Segs[s1].v1].x >> 16,
                    Vertices[Segs[s1].v1].y >> 16);
            if (Segs[s1].v2 == Segs[s2].v2)
                printf(
                    "Segs %d%c and %d%c have duplicate end vertex %d (%d, "
                    "%d)\n",
                    s1, Segs[s1].linedef == -1 ? '*' : ' ', s2,
                    Segs[s2].linedef == -1 ? '*' : ' ', Segs[s1].v2,
                    Vertices[Segs[s1].v2].x >> 16,
                    Vertices[Segs[s1].v2].y >> 16);
        }
    }
#endif
    ...
    D(Printf("bbox (%d,%d)-(%d,%d)\n", bbox[BOXLEFT] >> 16,
             bbox[BOXBOTTOM] >> 16, bbox[BOXRIGHT] >> 16, bbox[BOXTOP] >> 16));
...
bool FNodeBuilder::CheckSubsector(DWORD set, node_t &node, DWORD &splitseg) {
    ...
    do {
        D(Printf(" - seg %d%c(%d,%d)-(%d,%d) line %d front %d back %d\n", seg,
                 ...
                 Segs[seg].linedef, Segs[seg].frontsector,
                 Segs[seg].backsector));
    ...
    if (seg == DWORD_MAX) {  // It's a valid non-GL subsector, and probably a
                             // valid GL subsector too.
        if (GLNodes) {
...
bool FNodeBuilder::CheckSubsectorOverlappingSegs(DWORD set, node_t &node,
                                                 DWORD &splitseg) {
    int v1, v2;
    DWORD seg1, seg2;
    ...
    for (seg1 = set; seg1 != DWORD_MAX; seg1 = Segs[seg1].next) {
        ...
        v1 = Segs[seg1].v1;
        v2 = Segs[seg1].v2;
        for (seg2 = Segs[seg1].next; seg2 != DWORD_MAX;
             seg2 = Segs[seg2].next) {
            if (Segs[seg2].v1 == v1 && Segs[seg2].v2 == v2) {
                if (Segs[seg2].linedef ==
                    -1) {  // Do not put minisegs into a new subsector.
                    swap(seg1, seg2);
                }
                D(Printf(
                    "Need to synthesize a splitter for set %d on seg %d (ov)\n",
                    set, seg2));
                splitseg = DWORD_MAX;
...
bool FNodeBuilder::ShoveSegBehind(DWORD set, node_t &node, DWORD seg,
                                  DWORD mate) {
...
// according to the planes that they lie on. Because one seg on the plane is
// just as good as any other seg on the plane at defining a split, only one seg
// from each unique plane needs to be considered as a splitter. A result of 0
// means this set is a convex region. A result of -1 means that there were
// possible splitters, but they all split segs we want to keep intact.
int FNodeBuilder::SelectSplitter(DWORD set, node_t &node, DWORD &splitseg,
                                 int step, bool nosplit) {
    int stepleft;
    int bestvalue;
    DWORD bestseg;
    DWORD seg;
        ...
        if (--stepleft <= 0) {
            ...
            if (l < 0 || (PlaneChecked[l] & r) == 0) {
                ...
                D(Printf("Seg %5d, ld %d (%5d,%5d)-(%5d,%5d) scores %d\n", seg,
                         Segs[seg].linedef, node.x >> 16, node.y >> 16,
                         (node.x + node.dx) >> 16, (node.y + node.dy) >> 16,
                         value));
...
int FNodeBuilder::Heuristic(node_t &node, DWORD set, bool honorNoSplit) {
    // Set the initial score above 0 so that near vertex anti-weighting is less
    // likely to produce a negative score.
    int score = 1000000;
    int segsInSet = 0;
    int counts[2] = {0, 0};
    int realSegs[2] = {0, 0};
    ...
    while (i != DWORD_MAX) {
        ...
        if (HackSeg == i) {
            ...
            side = ClassifyLine(node, &Vertices[test->v1], &Vertices[test->v2],
                                sidev);
        }
        ...
        switch (side) {
            ...
            case 1:
                // If we don't split this line, but it abuts the splitter, also
                // reject it. The "right" thing to do in this case is to only
                // reject it if there is another nosplit seg from the same
                // sector at this vertex. Note that a line that lies exactly on
                // top of the splitter is okay.
                if (test->loopnum && honorNoSplit &&
                    (sidev[0] == 0 || sidev[1] == 0)) {
            ...
            default:  // Seg is cut by the partition
                ...
                if (frac < 0.001 || frac > 0.999) {
                    ...
                    x += frac * (v2->x - x);
                    y += frac * (v2->y - y);
                    if (fabs(x - v1->x) < VERTEX_EPSILON + 1 &&
                        fabs(y - v1->y) < VERTEX_EPSILON + 1) {
                        D(
                            Printf("Splitter will produce same start vertex as "
                                   "seg %d\n",
                                   i));
                        return -1;
                    }
                    if (fabs(x - v2->x) < VERTEX_EPSILON + 1 &&
                        fabs(y - v2->y) < VERTEX_EPSILON + 1) {
                        D(Printf(
                            "Splitter will produce same end vertex as seg %d\n",
                            i));
                        return -1;
                    }
                    if (frac > 0.999) {
                    ...
                    D(
                        Printf("Penalized splitter by %d for being near endpt "
                               "of seg %d (%f).\n",
                               penalty, i, frac));
                }
    ...
    // subsectors have a better chance of choosing the correct sector. This
    // situation is not neccesarily bad, just undesirable.
    if (honorNoSplit &&
        (specialSegs[0] == realSegs[0] || specialSegs[1] == realSegs[1])) {
        D(Printf("Leaves a side with only special segs\n"));
        return -1;
    }

    // If this splitter intersects any vertices of segs that should not be
    // split, check if it is also colinear with another seg from the same
    // sector. If it is, the splitter is okay. If not, it should be rejected.
    // Why? Assuming that polyobject containers are convex (which they should
    // be), a splitter that is colinear with one of the sector's segs and
    // crosses the vertex of another seg of that sector must be crossing the
    // container's corner and does not actually split the container.
...
void FNodeBuilder::SplitSegs(DWORD set, node_t &node, DWORD splitseg,
                             DWORD &outset0, DWORD &outset1,
    ...
    while (set != DWORD_MAX) {
        ...
        if (HackSeg == set) {
            ...
            side = ClassifyLine(node, &Vertices[seg->v1], &Vertices[seg->v2],
                                sidev);
            hack = false;
        }
        ...
        switch (side) {
            ...
            default:  // seg needs to be split
                ...
                if (seg->loopnum) {
                    Printf(
                        "   Split seg %lu (%d,%d)-(%d,%d) of sector %d on line "
                        "%d\n",
                        (unsigned long)set, Vertices[seg->v1].x >> 16,
                        Vertices[seg->v1].y >> 16, Vertices[seg->v2].x >> 16,
                        Vertices[seg->v2].y >> 16, seg->frontsector,
                        seg->linedef);
                }
                ...
                newvert.x +=
                    fixed_t(frac * (double(Vertices[seg->v2].x) - newvert.x));
                newvert.y +=
                    fixed_t(frac * (double(Vertices[seg->v2].y) - newvert.y));
                newvert.index = 0;
                ...
                if (Segs[set].partner != DWORD_MAX) {
                    ...
                    // yet. If it had been, then the front seg would have
                    // already been split, and we would not be in this default
                    // case. Moreover, the back seg may not even be in the set
                    // being split, so we must not move its pieces into the out
                    // sets.
        ...
        if (hack && GLNodes) {
            ...
            if (HackMate == DWORD_MAX) {
                newfront = AddMiniseg(Segs[set].v1, Segs[set].v2, newback, set,
                                      splitseg);
...
double FNodeBuilder::InterceptVector(const node_t &splitter,
                                     const FPrivSeg &seg) {
...
void FNodeBuilder::PrintSet(int l, DWORD set) {
    ...
    for (; set != DWORD_MAX; set = Segs[set].next) {
        Printf("\t%5lu(%d)%c%d(%d,%d)-%d(%d,%d)\n", (unsigned long)set,
               Segs[set].frontsector, Segs[set].linedef == -1 ? '+' : ':',
               Segs[set].v1, Vertices[Segs[set].v1].x >> 16,
               Vertices[Segs[set].v1].y >> 16, Segs[set].v2,
...
#ifdef __GNUC__
extern "C" int ClassifyLineBackpatch(node_t &node, const FSimpleVert *v1,
                                     const FSimpleVert *v2, int sidev[2])
#else
static int *CallerOffset;
int ClassifyLineBackpatchC(node_t &node, const FSimpleVert *v1,
                           const FSimpleVert *v2, int sidev[2])
#endif
{
    ...
    calleroffset--;
#ifdef _WIN32
    if (VirtualProtect(calleroffset, sizeof(void *), PAGE_EXECUTE_READWRITE,
                       &oldprotect))
#else
    ...
    size_t protectlen =
        (intptr_t)calleroffset + sizeof(void *) - (intptr_t)callerpage;
    int ptect;
    if (!(ptect = mprotect(callerpage, protectlen,
...
#ifndef __GNUC__
// The ClassifyLineBackpatch() function here is a stub that uses inline assembly
// and nakedness to retrieve the return address of the stack before sending
// control to the real ClassifyLineBackpatchC() function. Since BACKPATCH
// shouldn't be defined on 64-bit builds, we're okay that VC++ can't do inline
// assembly on that target.

extern "C" __declspec(noinline) __declspec(naked) int ClassifyLineBackpatch(
    node_t &node, const FSimpleVert *v1, const FSimpleVert *v2, int sidev[2]) {
    // We store the return address in a global, so as not to need to mess with
    // the parameter list.
    __asm
    {
		mov eax, [esp]
