...
static char *cvtCodepageToUtf8(const char *cpstr) {
    ...
    if (!uconvdll) {
        ...
        rc = pUniUconvToUcs(uconv, (void **)&cpstr, &cplen, &uc2str, &unilen,
                            &subs);
...
#undef LOAD
    ...
    if (!ok || (pUniCreateUconvObject(defstr, &uconv) != ULS_SUCCESS)) {
        ...
        if (uconvdll) {
            if (uconv) pUniFreeUconvObject(uconv);
...
PHYSFS_EnumerateCallbackResult __PHYSFS_platformEnumerate(
    ...
    while (count == 1) {
        ...
        if (retval != PHYSFS_ENUM_OK) break;
...
PHYSFS_sint64 __PHYSFS_platformRead(void *opaque, void *buf,
    ...
    ULONG br = 0;
    APIRET rc;
    BAIL_IF(!__PHYSFS_ui64FitsAddressSpace(len), PHYSFS_ERR_INVALID_ARGUMENT,
            -1);
