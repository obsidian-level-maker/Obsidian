...
static PHYSFS_uint32 utf8codepoint(const char **_str) {
    ...
    else if (octet < 224) /* two octets */
    {
        ...
        *_str += 1; /* skip to next possible start of codepoint. */
        ...
        if ((retval >= 0x80) && (retval <= 0x7FF)) return retval;
    } /* else if */
    ...
    else if (octet < 248) /* four octets */
    {
        ...
        *_str += 3; /* skip to next possible start of codepoint. */
        ...
        if ((retval >= 0x10000) && (retval <= 0x10FFFF)) return retval;
    } /* else if */
...
int PHYSFS_caseFold(const PHYSFS_uint32 from, PHYSFS_uint32 *to) {
    ...
    if (from < 128) /* low-ASCII, easy! */
    {
        ...
        {
            const CaseFoldHashBucket2_16 *bucket =
                &case_fold_hash2_16[hash & 15];
            const int count = (int)bucket->count;
            for (i = 0; i < count; i++) {
        ...
        {
            const CaseFoldHashBucket3_16 *bucket =
                &case_fold_hash3_16[hash & 3];
            const int count = (int)bucket->count;
            for (i = 0; i < count; i++) {
