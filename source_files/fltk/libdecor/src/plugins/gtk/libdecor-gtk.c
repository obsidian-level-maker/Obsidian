...
static void find_widget_by_name(GtkWidget *widget, void *data) {
...
static struct header_element_data find_widget_by_type(GtkWidget *widget, enum header_element type) {
  char *name = NULL;
  switch (type) {
    case HDR_HDR:
...
static bool in_region(const cairo_rectangle_int_t *rect, const int *x, const int *y) {
  return (*x >= rect->x) & (*y >= rect->y) & (*x < (rect->x + rect->width)) &
         (*y < (rect->y + rect->height));
}

static struct header_element_data get_header_focus(const GtkHeaderBar *header_bar, const int x,
                                                   const int y) {
  /* we have to check child widgets (buttons, title) before the 'HDR_HDR' root widget */
  static const enum header_element elems[] = {HDR_TITLE, HDR_MIN, HDR_MAX, HDR_CLOSE};
...
enum decoration_type { DECORATION_TYPE_NONE, DECORATION_TYPE_ALL, DECORATION_TYPE_TITLE_ONLY };
...
static bool moveable(struct libdecor_frame_gtk *frame_gtk) {
  return libdecor_frame_has_capability(&frame_gtk->frame, LIBDECOR_ACTION_MOVE);
}

static bool resizable(struct libdecor_frame_gtk *frame_gtk) {
...
static bool closeable(struct libdecor_frame_gtk *frame_gtk) {
...
#else
struct libdecor_plugin *libdecor_plugin_new(struct libdecor *context);
#endif
...
static void libdecor_plugin_gtk_destroy(struct libdecor_plugin *plugin) {
  ...
  wl_list_for_each_safe(output, output_tmp, &plugin_gtk->output_list, link) {
    wl_output_destroy(output->wl_output);
    free(output);
  }

  wl_list_for_each_safe(frame, frame_tmp, &plugin_gtk->visible_frame_list, link) {
...
static struct libdecor_frame_gtk *libdecor_frame_gtk_new(struct libdecor_plugin_gtk *plugin_gtk) {
  ...
  frame_gtk->shadow_blur = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, size, size);
...
static int libdecor_plugin_gtk_get_fd(struct libdecor_plugin *plugin) {
  struct libdecor_plugin_gtk *plugin_gtk = (struct libdecor_plugin_gtk *)plugin;
  struct wl_display *wl_display = libdecor_get_wl_display(plugin_gtk->context);
...
static struct buffer *create_shm_buffer(struct libdecor_plugin_gtk *plugin_gtk, int width,
...
static void free_border_component(struct border_component *border_component) {
  ...
  if (border_component->output_list.next != NULL) {
    wl_list_for_each_safe(surface_output, surface_output_tmp, &border_component->output_list,
                          link) {
...
static void libdecor_plugin_gtk_frame_free(struct libdecor_plugin *plugin,
                                           struct libdecor_frame *frame) {
...
static bool is_border_surfaces_showing(struct libdecor_frame_gtk *frame_gtk) {
  return frame_gtk->shadow_showing;
}

static void hide_border_component(struct border_component *border_component) {
  if (!border_component->wl_surface)
    return;
...
static void hide_border_surfaces(struct libdecor_frame_gtk *frame_gtk) {
  hide_border_component(&frame_gtk->shadow);
  frame_gtk->shadow_showing = false;
}

static struct border_component *get_component_for_surface(struct libdecor_frame_gtk *frame_gtk,
...
static bool redraw_scale(struct libdecor_frame_gtk *frame_gtk, struct border_component *cmpnt) {
...
static bool add_surface_output(struct libdecor_plugin_gtk *plugin_gtk, struct wl_output *wl_output,
                               struct wl_list *list) {
  struct output *output;
  struct surface_output *surface_output;
...
static void surface_enter(void *data, struct wl_surface *wl_surface, struct wl_output *wl_output) {
  ...
  if (!add_surface_output(frame_gtk->plugin_gtk, wl_output, &cmpnt->output_list))
    return;
...
static void ensure_component(struct libdecor_frame_gtk *frame_gtk, struct border_component *cmpnt) {
  if (!cmpnt->wl_surface) {
    wl_list_init(&cmpnt->output_list);
    cmpnt->scale = 1;
    create_surface_subsurface_pair(frame_gtk, &cmpnt->wl_surface, &cmpnt->wl_subsurface);
    wl_surface_add_listener(cmpnt->wl_surface, &surface_listener, frame_gtk);
  }
}

static void ensure_border_surfaces(struct libdecor_frame_gtk *frame_gtk) {
  frame_gtk->shadow.type = SHADOW;
  frame_gtk->shadow.opaque = false;
  ensure_component(frame_gtk, &frame_gtk->shadow);
}

static void ensure_title_bar_surfaces(struct libdecor_frame_gtk *frame_gtk) {
  ...
  g_object_get(gtk_widget_get_settings(frame_gtk->window), "gtk-double-click-time",
               &frame_gtk->plugin_gtk->double_click_time_ms, NULL);
  /* set as "default" decoration */
  g_object_set(frame_gtk->header, "title", libdecor_frame_get_title(&frame_gtk->frame),
               "has-subtitle", FALSE, "show-close-button", TRUE, NULL);
...
static void calculate_component_size(struct libdecor_frame_gtk *frame_gtk, enum component component,
                                     int *component_x, int *component_y, int *component_width,
  ...
  const int title_height =
      GTK_IS_WIDGET(frame_gtk->header) ? gtk_widget_get_allocated_height(frame_gtk->header) : 0;
#else
  const int title_height =
      frame_gtk->header ? gtk_widget_get_allocated_height(frame_gtk->header) : 0;
#endif
...
static void array_append(enum header_element **array, size_t *n, enum header_element item) {
  (*n)++;
  *array = realloc(*array, (*n) * sizeof(enum header_element));
  (*array)[(*n) - 1] = item;
}

static void draw_header(struct libdecor_frame_gtk *frame_gtk, cairo_t *cr,
...

  {
    ...
    gtk_widget_get_allocation(find_widget_by_type(frame_gtk->header, HDR_TITLE).widget,
                              &allocation);

    /* create subsection in which to draw label */
    label_surface = cairo_surface_create_for_rectangle(surface, allocation.x, allocation.y,
...

  {
    ...
    for (size_t i = 0; i < nbuttons; i++) {
      ...
      GtkAllocation allocation;
      {
        ...
        gtk_render_background(button_style, cr, allocation.x, allocation.y, allocation.width,
                              allocation.height);
        gtk_render_frame(button_style, cr, allocation.x, allocation.y, allocation.width,
                         allocation.height);
...

      {
...

        {
          ...
          icon_info = gtk_icon_theme_lookup_icon_for_scale(
              gtk_icon_theme_get_default(), icon_name, icon_width, scale, (GtkIconLookupFlags)0);
        }

        {
          ...
          GdkPixbuf *icon_pixbuf =
              gtk_icon_info_load_symbolic_for_context(icon_info, button_style, NULL, NULL);
          ...
          GtkBorder border;
          gtk_style_context_get_border(button_style, gtk_style_context_get_state(button_style),
                                       &border);
          left += border.left;
          right += border.right;
          top += border.top;
          ...
          GtkBorder padding;
          gtk_style_context_get_padding(button_style, gtk_style_context_get_state(button_style),
                                        &padding);
          left += padding.left;
          right += padding.right;
          top += padding.top;
          ...
          gtk_render_icon_surface(gtk_widget_get_style_context(icon_widget), cr, icon_surface,
                                  allocation.x + iconXPosition, allocation.y + iconYPosition);
          cairo_paint(cr);
...
static void draw_component_content(struct libdecor_frame_gtk *frame_gtk, struct buffer *buffer,
  ...
  switch (component) {
    case NONE:
      break;
    case SHADOW:
      render_shadow(cr, frame_gtk->shadow_blur, -(int)SHADOW_MARGIN / 2, -(int)SHADOW_MARGIN / 2,
                    buffer->width + SHADOW_MARGIN, buffer->height + SHADOW_MARGIN, 64, 64);
      break;
    case HEADER:
  ...
  if (component == SHADOW) {
    ...
    calculate_component_size(frame_gtk, component, &component_x, &component_y, &component_width,
                             &component_height);
...
static void set_component_input_region(struct libdecor_frame_gtk *frame_gtk,
  ...
  if (border_component->type == SHADOW && frame_gtk->shadow_showing) {
    ...
    calculate_component_size(frame_gtk, border_component->type, &component_x, &component_y,
...
static void draw_border_component(struct libdecor_frame_gtk *frame_gtk,
  ...
  if (!buffer)
    buffer = create_shm_buffer(plugin_gtk, component_width, component_height,
                               border_component->opaque, border_component->scale);

  draw_component_content(frame_gtk, buffer, component_width, component_height, component);
  ...
  buffer->in_use = true;
  wl_surface_commit(border_component->wl_surface);
  wl_surface_damage_buffer(border_component->wl_surface, 0, 0, component_width * scale,
                           component_height * scale);
  wl_subsurface_set_position(border_component->wl_subsurface, component_x, component_y);
...
static void draw_decoration(struct libdecor_frame_gtk *frame_gtk) {
  switch (frame_gtk->decoration_type) {
    ...
    case DECORATION_TYPE_ALL:
      ...
      /* link frame */
      if (frame_gtk->link.next == NULL)
        wl_list_insert(&frame_gtk->plugin_gtk->visible_frame_list, &frame_gtk->link);
      break;
...
static void libdecor_plugin_gtk_frame_commit(struct libdecor_plugin *plugin,
                                             struct libdecor_frame *frame,
  ...
  if (!resizable(frame_gtk)) {
    libdecor_frame_set_min_content_size(frame, frame_gtk->content_width, frame_gtk->content_height);
    libdecor_frame_set_max_content_size(frame, frame_gtk->content_width, frame_gtk->content_height);
  }
}

static void libdecor_plugin_gtk_frame_property_changed(struct libdecor_plugin *plugin,
                                                       struct libdecor_frame *frame) {
  struct libdecor_frame_gtk *frame_gtk = (struct libdecor_frame_gtk *)frame;
...
static void libdecor_plugin_gtk_frame_translate_coordinate(struct libdecor_plugin *plugin,
                                                           struct libdecor_frame *frame,
...
static void update_component_focus(struct libdecor_frame_gtk *frame_gtk, struct wl_surface *surface,
                                   struct seat *seat) {
  ...
  wl_list_for_each(child_component, &border_component->child_components, link) {
    ...
    calculate_component_size(frame_gtk, child_component->type, &component_x, &component_y,
                             &component_width, &component_height);
...
static void sync_active_component(struct libdecor_frame_gtk *frame_gtk, struct seat *seat) {
...
static void libdecor_plugin_gtk_frame_popup_grab(struct libdecor_plugin *plugin,
                                                 struct libdecor_frame *frame,
...
static void libdecor_plugin_gtk_frame_popup_ungrab(struct libdecor_plugin *plugin,
                                                   struct libdecor_frame *frame,
                                                   const char *seat_name) {
  struct libdecor_frame_gtk *frame_gtk = (struct libdecor_frame_gtk *)frame;
  struct libdecor_plugin_gtk *plugin_gtk = frame_gtk->plugin_gtk;
...
static bool libdecor_plugin_gtk_frame_get_window_size_for(struct libdecor_plugin *plugin,
                                                          struct libdecor_frame *frame,
                                                          struct libdecor_state *state,
...
#else
  const int title_bar_height =
      gtk_widget_get_allocated_height(((struct libdecor_frame_gtk *)frame)->header);
#endif
  ...
  switch (window_state_to_decoration_type(window_state)) {
    ...
    case DECORATION_TYPE_TITLE_ONLY:
      *window_width = libdecor_state_get_content_width(state);
      *window_height = libdecor_state_get_content_height(state) + title_bar_height;
      break;
  }
...
static struct libdecor_plugin_interface gtk_plugin_iface = {
    ...
    .frame_translate_coordinate = libdecor_plugin_gtk_frame_translate_coordinate,
    .frame_popup_grab = libdecor_plugin_gtk_frame_popup_grab,
    .frame_popup_ungrab = libdecor_plugin_gtk_frame_popup_ungrab,

    .configuration_get_content_size = libdecor_plugin_gtk_configuration_get_content_size,
    .frame_get_window_size_for = libdecor_plugin_gtk_frame_get_window_size_for,
};

static void init_wl_compositor(struct libdecor_plugin_gtk *plugin_gtk, uint32_t id,
                               uint32_t version) {
  plugin_gtk->wl_compositor =
      wl_registry_bind(plugin_gtk->wl_registry, id, &wl_compositor_interface, MIN(version, 4));
}

static void init_wl_subcompositor(struct libdecor_plugin_gtk *plugin_gtk, uint32_t id,
                                  uint32_t version) {
  plugin_gtk->wl_subcompositor =
      wl_registry_bind(plugin_gtk->wl_registry, id, &wl_subcompositor_interface, 1);
}
...
static void init_wl_shm(struct libdecor_plugin_gtk *plugin_gtk, uint32_t id, uint32_t version) {
  ...
  plugin_gtk->wl_shm = wl_registry_bind(plugin_gtk->wl_registry, id, &wl_shm_interface, 1);
  wl_shm_add_listener(plugin_gtk->wl_shm, &shm_listener, plugin_gtk);

  plugin_gtk->globals_callback_shm = wl_display_sync(wl_display);
  wl_callback_add_listener(plugin_gtk->globals_callback_shm, &shm_callback_listener, plugin_gtk);
}
...
static bool ensure_cursor_theme(struct seat *seat) {
  ...
  theme = wl_cursor_theme_load(plugin_gtk->cursor_theme_name, plugin_gtk->cursor_size * scale,
                               plugin_gtk->wl_shm);
  if (theme == NULL)
    return false;
...
static bool update_local_cursor(struct seat *seat) {
  ...
  struct libdecor_frame_gtk *frame_gtk = wl_surface_get_user_data(seat->pointer_focus);
  ...
  if (frame_gtk->active->type == SHADOW && is_border_surfaces_showing(frame_gtk) &&
      resizable(frame_gtk)) {
    enum libdecor_resize_edge edge;
    edge = component_edge(frame_gtk->active, seat->pointer_x, seat->pointer_y, SHADOW_MARGIN);
...
static void pointer_motion(void *data, struct wl_pointer *wl_pointer, uint32_t time,
  ...
  if (frame_gtk->active->type == HEADER) {
    struct header_element_data new_focus =
        get_header_focus(GTK_HEADER_BAR(frame_gtk->header), seat->pointer_x, seat->pointer_y);
  ...
  if (button == BTN_LEFT) {
    if (state == WL_POINTER_BUTTON_STATE_PRESSED) {
          ...
          switch (frame_gtk->hdr_focus.type) {
            ...
            default:
              if (time - seat->pointer_button_time_stamp <
                ...
                libdecor_frame_move(&frame_gtk->frame, seat->wl_seat, serial);
              }
              break;
          }
          break;
      ...
      if (edge != LIBDECOR_RESIZE_EDGE_NONE && resizable(frame_gtk)) {
        libdecor_frame_resize(&frame_gtk->frame, seat->wl_seat, serial, edge);
      }
      ...
      switch (frame_gtk->active->type) {
        case HEADER:
          switch (frame_gtk->hdr_focus.type) {
            case HDR_MIN:
              if (minimizable(frame_gtk))
                libdecor_frame_set_minimized(&frame_gtk->frame);
              break;
            case HDR_MAX:
              toggle_maximized(&frame_gtk->frame);
              break;
    ...
    const int title_height = gtk_widget_get_allocated_height(frame_gtk->header);
#endif
    libdecor_frame_show_window_menu(&frame_gtk->frame, seat->wl_seat, serial, seat->pointer_x,
...
static void init_wl_seat(struct libdecor_plugin_gtk *plugin_gtk, uint32_t id, uint32_t version) {
  struct seat *seat;

  if (version < 3) {
    char *err_msg;
    asprintf(&err_msg, "%s version 3 required but only version %i is available\n",
             wl_seat_interface.name, version);
    libdecor_notify_plugin_error(plugin_gtk->context, LIBDECOR_ERROR_COMPOSITOR_INCOMPATIBLE,
                                 err_msg);
    free(err_msg);
  }
  ...
  seat->wl_seat = wl_registry_bind(plugin_gtk->wl_registry, id, &wl_seat_interface, 3);
...
static void output_done(void *data, struct wl_output *wl_output) {
  ...
  wl_list_for_each(frame_gtk, &output->plugin_gtk->visible_frame_list, link) {
    bool updated = false;
...
static void init_wl_output(struct libdecor_plugin_gtk *plugin_gtk, uint32_t id, uint32_t version) {
  ...
  if (version < 2) {
    char *err_msg;
    asprintf(&err_msg, "%s version 2 required but only version %i is available\n",
  ...
  output->id = id;
  output->wl_output = wl_registry_bind(plugin_gtk->wl_registry, id, &wl_output_interface, 2);
...
LIBDECOR_EXPORT
  ...
  libdecor_plugin_init(&plugin_gtk->plugin, context, &gtk_plugin_iface);
  ...
  if (!libdecor_get_cursor_settings(&plugin_gtk->cursor_theme_name, &plugin_gtk->cursor_size)) {
  ...
  wl_registry_add_listener(plugin_gtk->wl_registry, &registry_listener, plugin_gtk);

  plugin_gtk->globals_callback = wl_display_sync(wl_display);
  wl_callback_add_listener(plugin_gtk->globals_callback, &globals_callback_listener, plugin_gtk);
