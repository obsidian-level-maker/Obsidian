...
void nsvgRasterizeXY(NSVGrasterizer *r, NSVGimage *image, float tx, float ty, float sx, float sy,
                     unsigned char *dst, int w, int h, int stride);
...
static void nsvg__flattenShape(NSVGrasterizer *r, NSVGshape *shape, float sx, float sy) {
  ...
  for (path = shape->paths; path != NULL; path = path->next) {
    ...
    for (i = 0; i < path->npts - 1; i += 3) {
      ...
      nsvg__flattenCubicBez(r, p[0] * sx, p[1] * sy, p[2] * sx, p[3] * sy, p[4] * sx, p[5] * sy,
                            p[6] * sx, p[7] * sy, 0, 0);
    }
    // Close path
    nsvg__addPathPoint(r, path->pts[0] * sx, path->pts[1] * sy, 0);
...
static void nsvg__flattenShapeStroke(NSVGrasterizer *r, NSVGshape *shape, float sx, float sy) {
  ...
  for (path = shape->paths; path != NULL; path = path->next) {
    ...
    for (i = 0; i < path->npts - 1; i += 3) {
      float *p = &path->pts[i * 2];
      nsvg__flattenCubicBez(r, p[0] * sx, p[1] * sy, p[2] * sx, p[3] * sy, p[4] * sx, p[5] * sy,
                            p[6] * sx, p[7] * sy, 0, NSVG_PT_CORNER);
    }
    if (r->npoints < 2)
      continue;
...
static void nsvg__rasterizeSortedEdges(NSVGrasterizer *r, float tx, float ty, float sx, float sy,
