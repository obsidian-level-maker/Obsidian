 ...
 */
void Fl_Color_Chooser::hsv2rgb(double H, double S, double V, double &R, double &G, double &B) {
  if (S < 5.0e-6) {
    R = G = B = V;
  } else {
    int i = (int)H;
 ...
 */
void Fl_Color_Chooser::rgb2hsv(double R, double G, double B, double &H, double &S, double &V) {
  double maxv = R > G ? R : G;
  if (B > maxv)
    maxv = B;
  V = maxv;
...
static const Fl_Menu_Item mode_menu[] = {{"rgb"}, {"byte"}, {"hex"}, {"hsv"}, {0}};
...
int Fl_Color_Chooser::hsv(double H, double S, double V) {
  ...
  if (V < 0.0)
    ...
    V = 1.0;
  if (H == hue_ && S == saturation_ && V == value_)
    return 0;
  double ph = hue_;
  double ps = saturation_;
  double pv = value_;
...
int Flcc_HueBox::handle(int e) {
  ...
  switch (e) {
    ...
    case FL_DRAG: {
      ...
      tohs(Xf, Yf, H, S);
      if (fabs(H - ih) < 3 * 6.0 / w())
        H = ih;
      if (fabs(S - is) < 3 * 1.0 / h())
        S = is;
      if (Fl::event_state(FL_CTRL))
        H = ih;
      if (c->hsv(H, S, c->value()))
        c->do_callback();
    }
      return 1;
...
int Flcc_ValueBox::handle(int e) {
  ...
  switch (e) {
    ...
    case FL_DRAG: {
      ...
      if (fabs(Yf - iv) < (3 * 1.0 / h()))
        Yf = iv;
      if (c->hsv(c->hue(), c->saturation(), Yf))
        c->do_callback();
    }
      return 1;
    case FL_FOCUS: /* FALLTHROUGH */
...
public:
  uchar r, g, b;
  ColorChip(int X, int Y, int W, int H)
    : Fl_Widget(X, Y, W, H) {
...
void ColorChip::draw() {
  if (damage() & FL_DAMAGE_ALL)
    draw_box();
  fl_rectf(x() + Fl::box_dx(box()), y() + Fl::box_dy(box()), w() - Fl::box_dw(box()),
