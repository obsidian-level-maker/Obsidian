...
*/
double Fl_Input_::expandpos(const char *p,   // real string
...
void Fl_Input_::drawtext(int X, int Y, int W, int H) {
  ...
  if (Fl::focus() != this && !size()) {
    if (do_mu) { // we have to erase it if cursor was there
      draw_box(box(), X - Fl::box_dx(box()), Y - Fl::box_dy(box()), W + Fl::box_dw(box()),
               H + Fl::box_dh(box()), color());
    }
    return;
  }
  ...
  for (; ypos < H;) {
    ...
    if (ypos <= -height)
      goto CONTINUE; // clipped off top

    if (do_mu) {                       // for minimal update:
      const char *pp = value() + mu_p; // pointer to where minimal update starts
      if (e < pp)
        goto CONTINUE2; // this line is before the changes
      if (readonly())
        erase_cursor_only = 0; // this isn't the most efficient way
      if (erase_cursor_only && p > pp)
        goto CONTINUE2; // this line is after
      // calculate area to erase:
      float r = (float)(X + W);
      float xx;
      if (p >= pp) {
        xx = (float)X;
        if (erase_cursor_only)
          r = xpos + 2;
        else if (readonly())
          xx -= 3;
      } else {
    ...
    if (selstart < selend && selstart <= e - value() && selend > p - value()) {
      ...
      int offset2;
      if (pp <= e)
        x2 = xpos + (float)expandpos(p, pp, buf, &offset2);
      else
        offset2 = (int)strlen(buf);
  ...
  CONTINUE2:
    // draw the cursor:
    if (Fl::focus() == this &&
        ((Fl::screen_driver()->has_marked_text() && Fl::compose_state) || selstart == selend) &&
        position() >= p - value() && position() <= e - value()) {
      fl_color(cursor_color());
      // cursor position may need to be recomputed (see STR #2486)
      curx = int(expandpos(p, value() + position(), buf, 0) + .5);
      if (readonly()) {
        fl_line((int)(xpos + curx - 2.5f), Y + ypos + height - 1, (int)(xpos + curx + 0.5f),
                Y + ypos + height - 4, (int)(xpos + curx + 3.5f), Y + ypos + height - 1);
      } else {
  ...
  fl_pop_clip();
  if (Fl::focus() == this) {
    fl_set_spot(textfont(), textsize(), (int)xpos + curx, Y + ypos_cur - fl_descent(), W, H,
                window()); // fix issue #270
  }
}
...
void Fl_Input_::handle_mouse(int X, int Y, int /*W*/, int /*H*/, int drag) {
  ...
  int theline =
      (input_type() == FL_MULTILINE_INPUT) ? (Fl::event_y() - Y + yscroll_) / fl_height() : 0;
  ...
  if (Fl::event_clicks()) {
    ...
    if (!drag && (mark() > position() ? (newmark >= position() && newpos <= mark())
...
int Fl_Input_::position(int p, int m) {
  ...
  while (p < position_ && p > 0 && (size() - p) > 0 && (fl_utf8len((char)(value() + p)[0]) < 1)) {
    p--;
  }
  ...
  while (m < mark_ && m > 0 && (size() - m) > 0 && (fl_utf8len((char)(value() + m)[0]) < 1)) {
    m--;
  }
  ...
  if (p != m) {
    if (p != position_)
      minimal_update(position_, p);
    if (m != mark_)
      minimal_update(mark_, m);
  } else {
...
int Fl_Input_::replace(int b, int e, const char *text, int ilen) {
  ...
  while (b != e && b > 0 && (size_ - b) > 0 && (fl_utf8len((value_ + b)[0]) < 1)) {
    b--;
  }
  ...
  while (p < (char *)(value_ + size_)) {
    if (p == (char *)(value_ + b)) { // skip removed part
      p = (char *)(value_ + e);
      if (p >= (char *)(value_ + size_))
        break;
    }
    int ulen = fl_utf8len(*p);
    if (ulen < 1)
      ulen = 1; // invalid UTF-8 character: count as 1
    nchars++;
    p += ulen;
  }
...
int Fl_Input_::undo() {
  was_up_down = 0;
  if (undowidget != this || (!undocut && !undoinsert))
    return 0;
...
int Fl_Input_::copy_cuts() {
  ...
  if (!yankcut || input_type() == FL_SECRET_INPUT)
    return 0;
  ...
  switch (event) {
    ...
    case FL_PASTE: {
      ...
      if (input_type() == FL_INT_INPUT) {
        ...
        if (strncmp(p, "0x", 2) == 0) {
          p += 2;
          while (isxdigit(*p & 255) && p < e)
            p++;
        } else {
          while (isdigit(*p & 255) && p < e)
            p++;
        }
        if (p < e) {
          fl_beep(FL_BEEP_ERROR);
...
void Fl_Input_::put_in_buffer(int len) {
  ...
  if (!bufsize) {
    if (len > size_)
      len += 9; // let a few characters insert before realloc
    bufsize = len + 1;
