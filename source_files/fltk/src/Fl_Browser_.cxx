...
void Fl_Browser_::resize(int X, int Y, int W, int H) {
  ...
  bbox(X, Y, W, H);
  scrollbar.resize(scrollbar.align() & FL_ALIGN_LEFT ? X - scrollsize : X + W, Y, scrollsize, H);
  hscrollbar.resize(X, scrollbar.align() & FL_ALIGN_TOP ? Y - scrollsize : Y + H, W, scrollsize);
  max_width = 0;
}
...
J1:
  ...
  if (dy < 10)
    dy = 10;
  if (scrollbar.visible()) {
    scrollbar.damage_resize(scrollbar.align() & FL_ALIGN_LEFT ? X - scrollsize : X + W, Y,
                            scrollsize, H);
  ...
  if (hscrollbar.visible()) {
    hscrollbar.damage_resize(X, scrollbar.align() & FL_ALIGN_TOP ? Y - scrollsize : Y + H, W,
                             scrollsize);
...
int Fl_Browser_::select(void *item, int val, int docallbacks) {
  if (type() == FL_MULTI_BROWSER) {
    ...
    if ((!val) == (!item_selected(item)))
      return 0;
    item_select(item, val);
    redraw_line(item);
  } else {
    if (val && selection_ == item)
      return 0;
    if (!val && selection_ != item)
      return 0;
    if (selection_) {
  ...
  switch (event) {
    ...
    case FL_DRAG:
      ...
      if (type() == FL_NORMAL_BROWSER || !top_)
        ...
        void *l1 = selection_;
        void *l = (Fl::event_x() < x() || Fl::event_x() > x() + w()) ? selection_ : find_item(my);
        change = (l != l1);
