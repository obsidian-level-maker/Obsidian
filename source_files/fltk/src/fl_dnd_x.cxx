...
extern void fl_sendClientMessage(Window window, Atom message, unsigned long d0,
                                 unsigned long d1 = 0, unsigned long d2 = 0, unsigned long d3 = 0,
                                 unsigned long d4 = 0);
...
static int dnd_aware(Window &window) {
  ...
  XGetWindowProperty(fl_display, window, fl_XdndAware, 0, 4, False, XA_ATOM, &actual, &format,
...
int Fl_X11_Screen_Driver::dnd(int unused) {
  ...
  while (Fl::pushed()) {
    ...
    for (Window child = RootWindow(fl_display, fl_screen);;) {
      Window root;
      unsigned int junk3;
      XQueryPointer(fl_display, child, &root, &child, &Fl::e_x_root, &Fl::e_y_root, &dest_x,
                    &dest_y, &junk3);
      if (!child) {
        if (!new_window && (new_version = dnd_aware(root)))
          new_window = root;
        break;
      }
      new_window = child;
      if ((new_local_window = fl_find(child)))
        break;
      if ((new_version = dnd_aware(new_window)))
        break;
    }
#if USE_XFT
    if (new_local_window) {
      float s =
          Fl::screen_driver()->scale(Fl_Window_Driver::driver(new_local_window)->screen_num());
      Fl::e_x_root /= s;
      Fl::e_y_root /= s;
    }
#endif
    ...
    if (new_window != target_window) {
      ...
      if (local_window) {
        ...
        if ((!strncmp(fl_selection_buffer[0], "file:///", 8) ||
            ...
            !strchr(fl_selection_buffer[0], ' ') && strstr(fl_selection_buffer[0], "\r\n")) {
          // Send file/URI list...
...
#endif
      fl_sendClientMessage(target_window, fl_XdndPosition, source_window, 0,
                           (exroot << 16) | eyroot, fl_event_time, fl_XdndActionCopy);
    }
    Fl::wait();
  }
  ...
  if (local_window) {
    ...
    if (local_handle(FL_DND_RELEASE, local_window))
      Fl::paste(*Fl::belowmouse(), 0);
  } else if (dndversion) {
    fl_sendClientMessage(target_window, fl_XdndDrop, source_window, 0, fl_event_time);
    ...
    Fl_Window *target = fl_find(target_window);
    if (target)
      s = Fl::screen_driver()->scale(Fl_Window_Driver::driver(target)->screen_num());
#endif
