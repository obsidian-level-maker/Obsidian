...
/* Note about implementing GL text support for a new platform
 ...
 is available, no platform-specific code is needed, besides support for fl_draw() and
 Fl_Image_Surface for the platform.
  ...
  if (once) {
    ...
    if (Fl::draw_GL_text_with_textures()) {
      ...
      has_texture_rectangle =
          (strstr((const char *)glGetString(GL_EXTENSIONS), "GL_EXT_texture_rectangle") != NULL);
...
void gl_draw(const char *str, int n) {
  if (n > 0) {
    if (has_texture_rectangle)
      Fl_Gl_Window_Driver::draw_string_with_texture(str, n);
    else
...
void gl_color(Fl_Color i) {
  if (Fl_Gl_Window_Driver::global()->overlay_color(i))
    return;
...
#if !defined( \
    FL_DOXYGEN) // do not want too much of the gl_texture_fifo internals in the documentation
...
gl_texture_fifo::~gl_texture_fifo() {
  for (int i = 0; i < size_; i++) {
    ...
    if (textures_generated)
      glDeleteTextures(1, &fifo[i].texName);
  }
  free(fifo);
}
...
int gl_texture_fifo::already_known(const char *str, int n) {
  ...
  for (rank = 0; rank <= last; rank++) {
    if ((fifo[rank].str_len == n) && (fifo[rank].fdesc == gl_fontsize) &&
...
int gl_texture_fifo::compute_texture(const char *str, int n) {
  ...
  fl_font(fl_font(),
          int(fs * Fl_Gl_Window_Driver::gl_scale)); // the font size to use in the GL scene
  ...
  // GL_ALPHA8 is defined in GL/gl.h of X11 and of MinGW32 and of MinGW64 and of OpenGL.framework
  // for MacOS
  glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_ALPHA8, w, h, 0, GL_ALPHA, GL_UNSIGNED_BYTE,
               alpha_buf);
  /* For the record: texture construction if an alpha-only-texture is not possible
   glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA8, w, h, 0, GL_BGRA_EXT,
   GL_UNSIGNED_INT_8_8_8_8_REV, rgba_buf); and also, replace GL_SRC_ALPHA by GL_ONE in glBlendFunc()
   call of display_texture()
   */
...
char *Fl_Gl_Window_Driver::alpha_mask_for_string(const char *str, int n, int w, int h,
  ...
  for (int idx = 0; idx < w * h;
       ++idx) { // Fake up the alpha component using the green component's value
...
/* Platform-independent, no-texture, GL text drawing procedure when there's no OS support
 whatsoever: glutStrokeString() is used to draw text. It's possible to vary text size, but not text
 font, and only ASCII characters can be drawn.
 */
void Fl_Gl_Window_Driver::draw_string_legacy_glut(const char *str, int n) {
  ...
  gl_scale = (gwin ? gwin->pixels_per_unit() : 1);
  float ratio =
      float(fl_width((char *)str_nul, n) * gl_scale / glutStrokeLength(GLUT_STROKE_ROMAN, str_nul));
  Fl_Surface_Device::pop_current();
