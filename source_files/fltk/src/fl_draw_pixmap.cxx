...
int fl_measure_pixmap(const char *const *cdata, int &w, int &h) {
  ...
  if (i < 4 || w <= 0 || h <= 0 || (chars_per_pixel != 1 && chars_per_pixel != 2))
    return w = 0;
  return 1;
}
...
int fl_convert_pixmap(const char *const *cdata, uchar *out, Fl_Color bg) {
  ...
  uchar *transparent_c =
      (uchar *)0; // such that transparent_c[0,1,2] are the RGB of the transparent color
