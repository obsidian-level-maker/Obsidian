...
int Fl::damage_, Fl::e_number, Fl::e_x, Fl::e_y, Fl::e_x_root, Fl::e_y_root, Fl::e_dx, Fl::e_dy,
    Fl::e_state, Fl::e_clicks, Fl::e_is_click, Fl::e_keysym, Fl::e_original_keysym,
...
void Fl::flush() {
  if (damage()) {
    ...
    for (Fl_X *i = Fl_X::first; i; i = i->next) {
      Fl_Window *wi = i->w;
      if (Fl_Window_Driver::driver(wi)->wait_for_expose_value) {
        damage_ = 1;
        continue;
      }
      if (!wi->visible_r())
...
void Fl::focus(Fl_Widget *o) {
  if (grab())
    return; // don't do anything while grab is on
...
void fl_fix_focus() {
  ...
  if (w) {
    ...
    if (Fl::e_keysym < (FL_Button + FL_LEFT_MOUSE) || Fl::e_keysym > (FL_Button + FL_RIGHT_MOUSE))
...
void fl_throw_focus(Fl_Widget *o) {
  ...
  if (o->contains(Fl::pushed()))
    Fl::pushed_ = 0;
  if (o->contains(fl_selection_requestor))
    fl_selection_requestor = 0;
  if (o->contains(Fl::belowmouse()))
    Fl::belowmouse_ = 0;
  if (o->contains(Fl::focus()))
    Fl::focus_ = 0;
  if (o == fl_xfocus)
    fl_xfocus = 0;
  if (o == Fl_Tooltip::current())
...
int Fl::handle_(int e, Fl_Window *window) {
  ...
  switch (e) {
    ...
    case FL_KEYBOARD:
      ...
      for (wi = grab() ? grab() : focus(); wi; wi = wi->parent()) {
        if (send_event(FL_KEYBOARD, wi, window))
          return 1;
      }
    ...
    case FL_SHORTCUT:
      ...
      for (; wi; wi = wi->parent()) {
        if (send_event(FL_SHORTCUT, wi, wi->window()))
          return 1;
      }

      // try using add_handle() functions:
      if (send_handlers(FL_SHORTCUT))
        return 1;
    ...
    case FL_MOUSEWHEEL:
      ...
      if (grab() && grab() != modal() && grab() != window) {
        if (send_event(FL_MOUSEWHEEL, grab(), window))
          return 1;
      }
      ...
      if (send_event(FL_MOUSEWHEEL, window, window->top_window()))
        return 1;
    default:
      break;
  }
...
void Fl::delete_widget(Fl_Widget *wi) {
  ...
  if (win && win->shown())
    win->hide(); // case of iconified window
...
#ifdef DEBUG_WATCH
  printf("\nwatch_widget_pointer:   (%d/%d) %8p => %8p\n", num_widget_watch, num_widget_watch, wp,
         *wp);
  fflush(stdout);
#endif // DEBUG_WATCH
}
...
void Fl::release_widget_pointer(Fl_Widget *&w) {
  ...
  for (i = 0; i < num_widget_watch; ++i) {
    if (widget_watch[i] != wp) {
      if (j < i)
        widget_watch[j] = widget_watch[i]; // fill gap
      j++;
    }
#ifdef DEBUG_WATCH
