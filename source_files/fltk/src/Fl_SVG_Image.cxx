...
/** The constructor loads the SVG image from the given .svg/.svgz filename or in-memory data.
 ...
 This parameter allows to load an SVG image from in-memory data, and is used when \p filename is
 NULL. \note In-memory SVG data is parsed by the object constructor and is not used after
 construction.
 */
Fl_SVG_Image::Fl_SVG_Image(const char *filename, const char *svg_data)
...
static char *svg_inflate(const char *fname) {
  ...
  gzclose(gzf);
  if (!direct)
    out = (char *)realloc(out, (p - out) + 1);
  return out;
}
#endif

void Fl_SVG_Image::init_(const char *filename, const char *in_filedata,
                         const Fl_SVG_Image *copy_source) {
  if (copy_source) {
...
void Fl_SVG_Image::rasterize_(int W, int H) {
  ...
  nsvgRasterizeXY(rasterizer, counted_svg_image_->svg_image, 0, 0, float(fx), float(fy),
                  (uchar *)array, W, H, W * 4);
  alloc_array = 1;
  ...
  if (average_weight_ < 1)
    Fl_RGB_Image::color_average(average_color_, average_weight_);
  rasterized_ = true;
  raster_w_ = W;
  raster_h_ = H;
}
...
void Fl_SVG_Image::resize(int width, int height) {
  ...
  w(w1);
  h(h1);
  if (rasterized_ && w1 == raster_w_ && h1 == raster_h_)
    return;
  if (array) {
    delete[] array;
    array = NULL;
...
void Fl_SVG_Image::cache_size_(int &width, int &height) {
  if (proportional) {
    ...
    if (height * f >= width)
      width = int(height * f + 0.5);
    else
      height = int(width / f + 0.5);
  }
}
...
/** Makes sure the object is fully initialized.
 This function rasterizes the SVG image, and consequently initializes its \ref array member, if that
 was not done before. */
