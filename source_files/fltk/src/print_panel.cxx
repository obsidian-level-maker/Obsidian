...
static const char *const idata_print_color[] = {"24 24 17 1",
                                                " \tc None",
                                                ".\tc #FFFF00",
                                                "+\tc #C8FF00",
                                                "@\tc #00FF00",
...
static const char *const idata_print_gray[] = {"24 24 17 1",
                                               " \tc None",
                                               ".\tc #E3E3E3",
                                               "+\tc #D2D2D2",
                                               "@\tc #969696",
...
static void cb_Save(Fl_Return_Button *, void *) {
  ...
  for (val = 0; val < 4; val++) {
    if (print_output_mode[val]->value())
      break;
  }
...
Fl_Double_Window *make_print_panel() {
  {
    print_panel = new Fl_Double_Window(465, 235, Fl_Printer::dialog_title);
    {
      ...
      {
        Fl_Return_Button *o =
            new Fl_Return_Button(279, 201, 100, 25, Fl_Printer::dialog_print_button);
...
printing_style print_load() { // return whether SystemV or BSD printing style is used
  ...
  if ((lpstat = popen("LC_MESSAGES=C LANG=C /bin/sh -c '(lpstat -p -d ) 2>&-'", "r")) !=
      NULL) { // try first with SystemV printing system
    while (fgets(line, sizeof(line), lpstat)) {
      if (!strncmp(line, "printer ", 8) && sscanf(line + 8, "%s", name) == 1) {
  ...
  if (print_choice->size() == 2 &&
      (lpstat = fopen("/etc/printcap", "r"))) { // try next with BSD printing system
    while (fgets(line, sizeof(line), lpstat)) { // get names of all known printers
      if (*line == '#' || (p = strchr(line, '|')) == NULL)
        continue;
      *p = 0;
...
void print_update_status() {
  ...
  if (print_choice->value()) {
    ...
    snprintf(command, sizeof(command), "/bin/sh -c \"(lpstat -p '%s' ) 2>&-\" ",
