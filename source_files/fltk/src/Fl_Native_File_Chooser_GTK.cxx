...
private:
  ...
  static int custom_gtk_filter_function(const GtkFileFilterInfo *,
                                        Fl_GTK_Native_File_Chooser_Driver::pair *);
...
// void gtk_file_chooser_set_do_overwrite_confirmation(GtkFileChooser *chooser, gboolean
...
// gulong g_signal_connect_data(gpointer, const gchar *, GCallback, gpointer, GClosureNotify,
// GConnectFlags);
typedef gulong (*XX_g_signal_connect_data)(gpointer, const gchar *, GCallback, gpointer,
...
static XX_gtk_file_chooser_set_preview_widget_active fl_gtk_file_chooser_set_preview_widget_active =
    NULL;

// GdkPixbuf* gtk_file_chooser_set_preview_widget(GtkFileChooser *, GtkWidget *);
typedef GdkPixbuf *(*XX_gtk_file_chooser_set_preview_widget)(GtkFileChooser *gtkw_ptr,
                                                             GtkWidget *preview);
...
// GdkPixbuf *gdk_pixbuf_new_from_data(const uchar *,GdkColorspace, gboolean, int, int, int, int,
// void*, void*);
typedef GdkPixbuf *(*XX_gdk_pixbuf_new_from_data)(const uchar *, int /*GdkColorspace*/, gboolean,
...
typedef void (*XX_gtk_table_attach_defaults)(GtkTable *, GtkWidget *, guint left_attach,
...
Fl_GTK_Native_File_Chooser_Driver::Fl_GTK_Native_File_Chooser_Driver(int val)
  : Fl_Native_File_Chooser_FLTK_Driver(-1)
...
void Fl_GTK_Native_File_Chooser_Driver::changed_output_type(const char *filter) {
  if (!(options() & Fl_Native_File_Chooser::USE_FILTER_EXT))
    return;
  if (strchr(filter, '(') || strchr(filter, '{') || strchr(filter + 1, '*') ||
      strncmp(filter, "*.", 2))
    return;
...
gboolean Fl_GTK_Native_File_Chooser_Driver::custom_gtk_filter_function(
    const GtkFileFilterInfo *info, Fl_GTK_Native_File_Chooser_Driver::pair *p) {
...
static char *text_file_preview(const char *fname) {
  if (!strcmp(fl_filename_ext(fname), ".svg"))
    return NULL;
  if (!strcmp(fl_filename_ext(fname), ".xpm"))
    return NULL;
  ...
  if ((int)strlen(text) < len)
    text[0] = 0; // presence of null byte in file --> not text
  char *p = text;
  int count = 0;
...
static void update_preview_cb(GtkFileChooser *file_chooser, GtkImage *gtkimg) {
  ...
  if (Fl_GTK_Native_File_Chooser_Driver::want_preview)
    filename = fl_gtk_file_chooser_get_preview_filename(file_chooser); // 2.4
  if (filename) {
  ...
  if (preview_text || (img && !img->fail())) {
    int width = preview_zoom * 175,
        height = preview_zoom * 225; // same size as Fl_File_Chooser's preview box
    if (preview_text)
      height = 225;
    if (img) {
    ...
    delete surf;
    GdkPixbuf *pixbuf = fl_gdk_pixbuf_new_from_data(
        rgb->array, 0 /*GDK_COLORSPACE_RGB*/, rgb->d() == 4, 8, rgb->data_w(), rgb->data_h(),
        rgb->ld() ? rgb->ld() : rgb->data_w() * rgb->d(), (void *)&delete_rgb_image, rgb);
    if (pixbuf) {
      fl_gtk_image_set_from_pixbuf(gtkimg, pixbuf);
      fl_g_object_unref(pixbuf);
      if (preview_zoom < 4)
        fl_gtk_widget_set_sensitive(plus_button, true);
      if (preview_zoom > 1)
        fl_gtk_widget_set_sensitive(minus_button, true);
      have_preview = true;
    }
  }
  if (img)
...
int Fl_GTK_Native_File_Chooser_Driver::fl_gtk_chooser_wrapper() {
  ...
  // create a new dialog
  gtkw_ptr = fl_gtk_file_chooser_dialog_new(gtkw_title, NULL, /* parent_window */
  ...
  switch (_btype) {
    ...
    case Fl_Native_File_Chooser::BROWSE_SAVE_FILE:
      if (_preset_file)
        fl_gtk_file_chooser_set_current_name((GtkFileChooser *)gtkw_ptr,
    ...
    case Fl_Native_File_Chooser::BROWSE_SAVE_DIRECTORY:
      ...
      fl_gtk_file_chooser_set_do_overwrite_confirmation(
          ...
          (_options & Fl_Native_File_Chooser::SAVEAS_CONFIRM) ? TRUE : FALSE);
      break;
  ...
  if (_parsedfilt) {
    ...
    while (p) {
      ...
      *q = 0;
      fl_gtk_file_filter_add_custom(
          filter_tab[count], GTK_FILE_FILTER_FILENAME,
    ...
    free(filter);
    fl_gtk_file_chooser_set_filter((GtkFileChooser *)gtkw_ptr,
                                   filter_tab[_filtvalue < _nfilters ? _filtvalue : 0]);
    previous_filter = NULL;
  ...
  fl_g_signal_connect_data(show_hidden_button, "toggled", G_CALLBACK(hidden_files_cb), gtkw_ptr,
  ...
  if (Fl_Image::register_images_done) {
    ...
    fl_g_signal_connect_data((GtkFileChooser *)gtkw_ptr, "update-preview",
                             G_CALLBACK(update_preview_cb), preview, NULL, (GConnectFlags)0);
    ...
    fl_g_signal_connect_data(preview_button, "toggled", G_CALLBACK(preview_cb), gtkw_ptr, NULL,
                             (GConnectFlags)0);
    fl_gtk_table_attach_defaults(table, preview_button, 0, 1, 0, 1);

    plus_button = fl_gtk_button_new_with_label("<--->");
    fl_g_signal_connect_data(plus_button, "clicked", G_CALLBACK(plus_cb), preview, NULL,
                             (GConnectFlags)0);
    fl_gtk_table_attach_defaults(table, plus_button, 1, 2, 0, 1);
    minus_button = fl_gtk_button_new_with_label(">---<");
    fl_g_signal_connect_data(minus_button, "clicked", G_CALLBACK(minus_cb), preview, NULL,
                             (GConnectFlags)0);
  ...
  fl_g_signal_connect_data(gtkw_ptr, "response", G_CALLBACK(run_response_handler), &response_id,
                           NULL, (GConnectFlags)0);
  while (response_id == GTK_RESPONSE_NONE) { // loop that shows the GTK dialog window
    ...
    while (Fl::ready())
      Fl::check(); // queued iterations of the FLTK event loop
  }
    ...
    if (val != BROWSE_MULTI_DIRECTORY) {
      if (!Fl_Zenity_Native_File_Chooser_Driver::have_looked_for_zenity) {
        ...
        if (pipe) {
          ...
          if (p && strlen(line) > 0)
            Fl_Zenity_Native_File_Chooser_Driver::did_find_zenity = true;
          pclose(pipe);
        }
      ...
      if (Fl_Zenity_Native_File_Chooser_Driver::did_find_zenity)
        platform_fnfc = new Fl_Zenity_Native_File_Chooser_Driver(val);
    }
#endif // USE_ZENITY
#if USE_KDIALOG
    ...
    if (!platform_fnfc && desktop && strcmp(desktop, "KDE") == 0 && val != BROWSE_MULTI_DIRECTORY) {
      if (!Fl_Kdialog_Native_File_Chooser_Driver::have_looked_for_kdialog) {
        ...
        if (pipe) {
          ...
          p = fgets(line, sizeof(line), pipe);
          if (p && strlen(line) > 0)
            Fl_Kdialog_Native_File_Chooser_Driver::did_find_kdialog = true;
          pclose(pipe);
        }
      ...
      if (Fl_Kdialog_Native_File_Chooser_Driver::did_find_kdialog)
        platform_fnfc = new Fl_Kdialog_Native_File_Chooser_Driver(val);
    }
#endif // USE_KDIALOG
#if HAVE_DLSYM && HAVE_DLFCN_H
    if (!platform_fnfc) {
      ...
      if (Fl_GTK_Native_File_Chooser_Driver::did_find_GTK_libs)
        platform_fnfc = new Fl_GTK_Native_File_Chooser_Driver(val);
    }
#endif // HAVE_DLSYM && HAVE_DLFCN_H
  }
  if (!platform_fnfc)
