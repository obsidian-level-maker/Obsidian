...
void Fl_Text_Buffer::copy(Fl_Text_Buffer *fromBuf, int fromStart, int fromEnd, int toPos) {
  ...
  if (fromEnd <= fromBuf->mGapStart) {
    ...
    memcpy(&mBuf[toPos], &fromBuf->mBuf[fromStart + (fromBuf->mGapEnd - fromBuf->mGapStart)],
           copiedLength);
  } else {
 ...
 */
void Fl_Text_Buffer::add_predelete_callback(Fl_Text_Predelete_Cb bufPreDeleteCB, void *cbArg) {
...
void Fl_Text_Buffer::remove_predelete_callback(Fl_Text_Predelete_Cb bufPreDeleteCB, void *cbArg) {
  ...
  for (i = 0; i < mNPredeleteProcs; i++) {
    if (mPredeleteProcs[i] == bufPreDeleteCB && mPredeleteCbArgs[i] == cbArg) {
      toRemove = i;
      break;
    }
  }
 ...
 */
int Fl_Text_Buffer::search_forward(int startPos, const char *searchString, int *foundPos,
                                   int matchCase) const {
  IS_UTF8_ALIGNED2(this, (startPos))
...
int Fl_Text_Buffer::search_backward(int startPos, const char *searchString, int *foundPos,
                                    int matchCase) const {
  IS_UTF8_ALIGNED2(this, (startPos))
  IS_UTF8_ALIGNED(searchString)
...
/*
 ...
 Unicode safe.
 */
void Fl_Text_Buffer::call_modify_callbacks(int pos, int nDeleted, int nInserted, int nRestyled,
                                           const char *deletedText) const {
  IS_UTF8_ALIGNED2(this, pos)
  for (int i = 0; i < mNModifyProcs; i++)
    (*mModifyProcs[i])(pos, nInserted, nDeleted, nRestyled, deletedText, mCbArgs[i]);
}
...
void Fl_Text_Buffer::reallocate_with_gap(int newGapStart, int newGapLen) {
  ...
  if (newGapStart <= mGapStart) {
    ...
    memcpy(&newBuf[newGapEnd + mGapStart - newGapStart], &mBuf[mGapEnd], mLength - mGapStart);
  } else { /* newGapStart > mGapStart */
    memcpy(newBuf, mBuf, mGapStart);
    memcpy(&newBuf[mGapStart], &mBuf[mGapEnd], newGapStart - mGapStart);
    memcpy(&newBuf[newGapEnd], &mBuf[mGapEnd + newGapStart - mGapStart], mLength - newGapStart);
  }
  free((void *)mBuf);
  mBuf = newBuf;
...
unsigned cp1252toucs(char *&p) {
  ...
  // to Unicode
  static unsigned cp1252[32] = {0x20ac, 0x0081, 0x201a, 0x0192, 0x201e, 0x2026, 0x2020, 0x2021,
...
static int general_input_filter(char *buffer, int buflen, char *line, int sline, char *&endline,
                                unsigned (*p_trf)(char *&), FILE *fp) {
 ...
 */
static int
utf8_input_filter(char *buffer,           // result buffer we fill with utf8 encoded text
                  ...
                  int *input_was_changed) // returned flag: 'true' if buffer[] different from file
                                          // due to utf8 encoding
{
  ...
  while (q < buffer + buflen) {
    if (p >= endline) {                   // walked off end of input file's line buffer?
      ...
      endline = line + r;
      if (r == 0)
        return (int)(q - buffer); // EOF? return bytes read into buffer[]
      p = line;
    }
...
int Fl_Text_Buffer::insertfile(const char *file, int pos, int buflen) {
  ...
  while (true) {
    ...
    l = general_input_filter(buffer, buflen, line, sizeof(line), endline,
