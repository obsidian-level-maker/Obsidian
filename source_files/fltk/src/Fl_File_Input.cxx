...
void Fl_File_Input::draw_buttons() {
  ...
  for (X = 0, i = 0; buttons_[i]; i++) {
    if ((X + buttons_[i]) > xscroll()) {
      if (X < xscroll()) {
        draw_box(pressed_ == i ? fl_down(down_box()) : down_box(), x(), y(),
                 X + buttons_[i] - xscroll(), DIR_HEIGHT, FL_GRAY);
      } else if ((X + buttons_[i] - xscroll()) > w()) {
        draw_box(pressed_ == i ? fl_down(down_box()) : down_box(), x() + X - xscroll(), y(),
                 w() - X + xscroll(), DIR_HEIGHT, FL_GRAY);
      } else {
        draw_box(pressed_ == i ? fl_down(down_box()) : down_box(), x() + X - xscroll(), y(),
                 buttons_[i], DIR_HEIGHT, FL_GRAY);
      }
    }
...
void Fl_File_Input::update_buttons() {
  ...
  for (i = 0, start = value(); start && i < (int)(sizeof(buttons_) / sizeof(buttons_[0]) - 1);
       start = end, i++) {
    //    printf("    start = \"%s\"\n", start);
...
void Fl_File_Input::draw() {
  ...
  char must_trick_fl_input_ = Fl::focus() != this && !size() && !(damage() & FL_DAMAGE_ALL);
...
Fl_File_Input::handle_button(int event) // I - Event
{
  ...
  for (X = 0, i = 0; buttons_[i]; i++) {
    ...
    if (X > xscroll() && Fl::event_x() < (x() + X - xscroll()))
      break;
  }
  ...
  if (!buttons_[i] || event != FL_RELEASE)
    return 1;
