...
int Fl_WinAPI_Native_File_Chooser_Driver::showfile() {
  ...
  switch (_btype) {
    ...
    case Fl_Native_File_Chooser::BROWSE_SAVE_FILE:
      ...
      if (unixpath)
        Win2Unix(_pathnames[_tpathnames - 1]); // preserve unix style path
      break;
    case Fl_Native_File_Chooser::BROWSE_MULTI_FILE: {
...
int Fl_WinAPI_Native_File_Chooser_Driver::showdir() {
  ...
  else if (_btype == Fl_Native_File_Chooser::BROWSE_SAVE_DIRECTORY)
...
// ADD A FILTER
void Fl_WinAPI_Native_File_Chooser_Driver::add_filter(
...
void Fl_WinAPI_Native_File_Chooser_Driver::parse_filter(const char *in) {
  ...
  for (; 1; in++) {
    ...
    // DEBUG
    // printf("WORKING ON '%c': mode=<%c> name=<%s> wildprefix=<%s> nwildcards=%d
    // wildcards[n]=<%s>\n",
