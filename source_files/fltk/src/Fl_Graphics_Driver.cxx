...
void Fl_Graphics_Driver::copy_offscreen(int x, int y, int w, int h, Fl_Offscreen pixmap, int srcx,
                                        int srcy) {
  ...
  if (px + pw > px_width / s) {
    pw = int(px_width / s) - px;
  }
  if (py + ph > px_height / s) {
    ph = int(px_height / s) - py;
  }
...
void Fl_Graphics_Driver::cache_size(Fl_Image *img, int &width, int &height) {
  ...
  width = (fs - int(fs) < 0.001 ? int(fs) : int((width + 1) * s));
  fs = height * s;
  height = (fs - int(fs) < 0.001 ? int(fs) : int((height + 1) * s));
 ...
 */
void Fl_Graphics_Driver::draw_pixmap(Fl_Pixmap *pxm, int XP, int YP, int WP, int HP, int cx,
                                     int cy) {
  int X, Y, W, H;
  ...
  if (!*id(pxm)) {
    if (pxm->data_w() != w2 || pxm->data_h() != h2) { // build a scaled id_ & mask_ for pxm
      ...
      *id(pxm2) = 0;
      *pw = w2;
      *ph = h2; // memorize size of cached form of pixmap
      *mask(pxm) = *mask(pxm2);
      *mask(pxm2) = 0;
      delete pxm2;
    } else
...
/** Draws an Fl_Bitmap object using this graphics driver.
 ...
 the image offset by the cx and cy arguments.
 */
void Fl_Graphics_Driver::draw_bitmap(Fl_Bitmap *bm, int XP, int YP, int WP, int HP, int cx,
                                     int cy) {
  int X, Y, W, H;
  ...
  if (!*id(bm)) {
    if (bm->data_w() != w2 || bm->data_h() != h2) { // build a scaled id_ for bm
      ...
      *id(bm2) = 0;
      *pw = w2;
      *ph = h2; // memorize size of cached form of bitmap
      delete bm2;
    } else
      cache(bm);
  }
...
/** Draws an Fl_RGB_Image object using this graphics driver.
 ...
 the image offset by the cx and cy arguments.
 */
void Fl_Graphics_Driver::draw_rgb(Fl_RGB_Image *img, int XP, int YP, int WP, int HP, int cx,
                                  int cy) {
...
void Fl_Graphics_Driver::push_no_clip() {
  if (rstackptr < region_stack_max)
    ...
    Fl::warning("Fl_Graphics_Driver::push_no_clip: clip stack overflow!\n");
  restore_clip();
}
...
void Fl_Graphics_Driver::pop_clip() {
  if (rstackptr > 0) {
    ...
    Fl::warning("Fl_Graphics_Driver::pop_clip: clip stack underflow!\n");
  restore_clip();
}

/** Sets the current value of the scaling factor */
void Fl_Graphics_Driver::scale(float f) {
  scale_ = f;
}

/** Return whether the graphics driver can do alpha blending */
char Fl_Graphics_Driver::can_do_alpha_blending() {
  return 0;
}

void Fl_Graphics_Driver::draw_fixed(Fl_Pixmap *pxm, int XP, int YP, int WP, int HP, int cx,
                                    int cy) {}

void Fl_Graphics_Driver::draw_fixed(Fl_Bitmap *bm, int XP, int YP, int WP, int HP, int cx, int cy) {
}

void Fl_Graphics_Driver::draw_fixed(Fl_RGB_Image *rgb, int XP, int YP, int WP, int HP, int cx,
                                    int cy) {}

void Fl_Graphics_Driver::make_unused_color_(unsigned char &r, unsigned char &g, unsigned char &b,
                                            int color_count, void **data) {}
...
void Fl_Graphics_Driver::draw_image_mono(const uchar *buf, int X, int Y, int W, int H, int D,
                                         int L) {}

/** see fl_draw_image(Fl_Draw_Image_Cb cb, void* data, int X,int Y,int W,int H, int D) */
void Fl_Graphics_Driver::draw_image(Fl_Draw_Image_Cb cb, void *data, int X, int Y, int W, int H,
                                    int D) {}

/** see fl_draw_image_mono(Fl_Draw_Image_Cb cb, void* data, int X,int Y,int W,int H, int D) */
void Fl_Graphics_Driver::draw_image_mono(Fl_Draw_Image_Cb cb, void *data, int X, int Y, int W,
                                         int H, int D) {}
...
void Fl_Graphics_Driver::fixloop() { // remove equal points from closed path
  while (n > 2 && xpoint[n - 1].x == xpoint[0].x && xpoint[n - 1].y == xpoint[0].y)
    n--;
}
...
void Fl_Graphics_Driver::end_loop() {
  fixloop();
  if (n > 2)
    transformed_vertex(xpoint[0].x, xpoint[0].y);
  end_line();
}
...
void Fl_Graphics_Driver::gap() {
  while (n > gap_ + 2 && xpoint[n - 1].x == xpoint[gap_].x && xpoint[n - 1].y == xpoint[gap_].y)
    n--;
  if (n > gap_ + 2) {
...
void Fl_Graphics_Driver::color(Fl_Color c) {
  color_ = c;
}
...
int Fl_Graphics_Driver::has_feature(driver_feature feature) {
  return 0;
}

/** see fl_font(Fl_Font, Fl_Fontsize) */
void Fl_Graphics_Driver::font(Fl_Font face, Fl_Fontsize fsize) {
  font_ = face;
  size_ = fsize;
}

/** see fl_font(void) */
Fl_Font Fl_Graphics_Driver::font() {
  return font_;
}

/** Return the current font size */
Fl_Fontsize Fl_Graphics_Driver::size() {
  return size_;
}

/** Compute the width of the first \p n bytes of the string \p str if drawn with current font */
double Fl_Graphics_Driver::width(const char *str, int nChars) {
  return 0;
}
...
int Fl_Graphics_Driver::height() {
  return size();
}

/** Return the current line descent */
int Fl_Graphics_Driver::descent() {
  return 0;
}

/** Set the current Fl_Font_Descriptor */
void Fl_Graphics_Driver::font_descriptor(Fl_Font_Descriptor *d) {
  font_descriptor_ = d;
}
...
uchar **Fl_Graphics_Driver::mask_bitmap() {
  return 0;
}
...
/** Support for PostScript drawing */
float Fl_Graphics_Driver::scale_bitmap_for_PostScript() {
  return 2;
}

/** Support for Fl::get_font_name() */
const char *Fl_Graphics_Driver::get_font_name(Fl_Font fnum, int *ap) {
  return NULL;
}

/** Support for Fl::get_font_sizes() */
int Fl_Graphics_Driver::get_font_sizes(Fl_Font fnum, int *&sizep) {
  return 0;
}

/** Support for Fl::set_fonts() */
Fl_Font Fl_Graphics_Driver::set_fonts(const char *name) {
  return 0;
}

/** Some platforms may need to implement this to support fonts */
Fl_Fontdesc *Fl_Graphics_Driver::calc_fl_fonts(void) {
  return NULL;
}

/** Support for Fl::get_font() */
const char *Fl_Graphics_Driver::font_name(int num) {
  return NULL;
}
...
float Fl_Graphics_Driver::override_scale() {
  return scale();
}
...
void Fl_Scalable_Graphics_Driver::rectf(int x, int y, int w, int h) {
  if (w <= 0 || h <= 0)
    return;
  rectf_unscaled(this->floor(x), this->floor(y), this->floor(x + w) - this->floor(x),
                 this->floor(y + h) - this->floor(y));
}
...
void Fl_Scalable_Graphics_Driver::line(int x, int y, int x1, int y1) {
  if (y == y1)
    ...
    line_unscaled(this->floor(x), this->floor(y), this->floor(x1), this->floor(y1));
}

void Fl_Scalable_Graphics_Driver::line(int x, int y, int x1, int y1, int x2, int y2) {
  line_unscaled(this->floor(x), this->floor(y), this->floor(x1), this->floor(y1), this->floor(x2),
                this->floor(y2));
}
...
void Fl_Scalable_Graphics_Driver::xyline(int x, int y, int x1) {
  ...
  if (s != s_int && line_width_ <= s_int) {
    ...
    void *data = NULL;
    if (need_change_width)
      data = change_pen_width(lwidth);
    xyline_unscaled(this->floor(xx), this->floor(y) + int(lwidth / 2.f), this->floor(xx1 + 1) - 1);
    if (need_change_width)
      reset_pen_width(data);
  } else {
    y = this->floor(y);
    if (line_width_ <= s_int)
      y += int(s / 2.f);
...
void *Fl_Scalable_Graphics_Driver::change_pen_width(int lwidth) {
  return NULL;
}
...
void Fl_Scalable_Graphics_Driver::loop(int x0, int y0, int x1, int y1, int x2, int y2, int x3,
                                       int y3) {
  int X, Y, W, H;
  if (x0 == x3 && x1 == x2 && y0 == y1 && y3 == y2) { // rectangular loop
    ...
    loop_unscaled(floor(x0), floor(y0), floor(x1), floor(y1), floor(x2), floor(y2), floor(x3),
                  floor(y3));
  }
}
...
void Fl_Scalable_Graphics_Driver::polygon(int x0, int y0, int x1, int y1, int x2, int y2, int x3,
                                          int y3) {
  polygon_unscaled(floor(x0), floor(y0), floor(x1), floor(y1), floor(x2), floor(y2), floor(x3),
                   floor(y3));
}
...
void Fl_Scalable_Graphics_Driver::font(Fl_Font face, Fl_Fontsize size) {
  if (!font_descriptor())
    fl_open_display(); // to catch the correct initial value of scale_
...
void Fl_Scalable_Graphics_Driver::text_extents(const char *str, int n, int &dx, int &dy, int &w,
                                               int &h) {
...
void Fl_Scalable_Graphics_Driver::draw(const char *str, int n, int x, int y) {
  if (!size_ || !font_descriptor())
    font(FL_HELVETICA, FL_NORMAL_SIZE);
...
void Fl_Scalable_Graphics_Driver::line_style(int style, int width, char *dashes) {
  if (width == 0)
    line_width_ = int(scale() < 2 ? 0 : scale());
  else
    line_width_ = int(width > 0 ? width * scale() : -width * scale());
  line_style_unscaled(style, line_width_, dashes);
}

/* read the image data from a pointer or with a callback, scale it, and draw it */
void Fl_Scalable_Graphics_Driver::draw_image_rescale(void *buf, Fl_Draw_Image_Cb cb, int X, int Y,
                                                     int W, int H, int D, int L, bool mono) {
  int aD = abs(D);
  if (L == 0)
    L = W * aD;
  ...
  Fl_RGB_Scaling keep = Fl_Image::RGB_scaling();
  Fl_Image::RGB_scaling(Fl_Image::scaling_algorithm());
  Fl_RGB_Image *scaled_rgb =
      (Fl_RGB_Image *)rgb->copy(floor(X + W) - floor(X), floor(Y + H) - floor(Y));
  Fl_Image::RGB_scaling(keep);
  delete rgb;
  if (scaled_rgb) {
    Fl_Region r2 = scale_clip(scale());
    draw_image_unscaled(scaled_rgb->array, floor(X), floor(Y), scaled_rgb->w(), scaled_rgb->h(),
                        depth);
    unscale_clip(r2);
    delete scaled_rgb;
  }
...
void Fl_Scalable_Graphics_Driver::draw_image(const uchar *buf, int X, int Y, int W, int H, int D,
                                             int L) {
  if (scale() == 1) {
...
void Fl_Scalable_Graphics_Driver::draw_image(Fl_Draw_Image_Cb cb, void *data, int X, int Y, int W,
                                             int H, int D) {
  if (scale() == 1) {
...
void Fl_Scalable_Graphics_Driver::draw_image_mono(const uchar *buf, int X, int Y, int W, int H,
                                                  int D, int L) {
  if (scale() == 1) {
...
void Fl_Scalable_Graphics_Driver::draw_image_mono(Fl_Draw_Image_Cb cb, void *data, int X, int Y,
                                                  int W, int H, int D) {
  if (scale() == 1) {
...
void Fl_Scalable_Graphics_Driver::vertex(double x, double y) {
  transformed_vertex0(float((x * m.a + y * m.c + m.x) * scale()),
                      float((x * m.b + y * m.d + m.y) * scale()));
}
...
Fl_Region Fl_Scalable_Graphics_Driver::scale_clip(float f) {
  return 0;
}
...
double Fl_Scalable_Graphics_Driver::width_unscaled(const char *str, int n) {
  return 0.0;
}

double Fl_Scalable_Graphics_Driver::width_unscaled(unsigned int c) {
  return 0.0;
}

Fl_Fontsize Fl_Scalable_Graphics_Driver::size_unscaled() {
  return 0;
}

void Fl_Scalable_Graphics_Driver::text_extents_unscaled(const char *str, int n, int &dx, int &dy,
                                                        int &w, int &h) {}

int Fl_Scalable_Graphics_Driver::height_unscaled() {
  return 0;
}

int Fl_Scalable_Graphics_Driver::descent_unscaled() {
  return 0;
}
...
void Fl_Scalable_Graphics_Driver::draw_image_unscaled(const uchar *buf, int X, int Y, int W, int H,
                                                      int D, int L) {}

void Fl_Scalable_Graphics_Driver::draw_image_unscaled(Fl_Draw_Image_Cb cb, void *data, int X, int Y,
                                                      int W, int H, int D) {}

void Fl_Scalable_Graphics_Driver::draw_image_mono_unscaled(const uchar *buf, int x, int y, int w,
                                                           int h, int d, int l) {}

void Fl_Scalable_Graphics_Driver::draw_image_mono_unscaled(Fl_Draw_Image_Cb cb, void *data, int X,
