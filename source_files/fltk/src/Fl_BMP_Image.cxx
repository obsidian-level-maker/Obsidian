...
/**
  ...
  \see Fl_BMP_Image::Fl_BMP_Image(const char* imagename, const unsigned char *data, const long
  length = -1);
*/
...
#define CHECK_ERROR                                                                           \
  if (rdr.error()) {                                                                          \
    Fl::error("[%d] Fl_BMP_Image: %s - unexpected EOF or read error at offset %ld", __LINE__, \
              rdr.name(), rdr.tell());                                                        \
    ld(ERR_FORMAT);                                                                           \
    return;                                                                                   \
  }
...
void Fl_BMP_Image::load_bmp_(Fl_Image_Reader &rdr) {
  ...
  h(0);
  d(0);
  ld(0); // make sure these are all zero
  ...
  if (info_size < 40) {
    ...
    if (!compression && depth >= 8 && width > 32 / depth) {
      int Bpp = depth / 8;
      int maskSize =
          (((width * Bpp + 3) & ~3) * height) + (((((width + 7) / 8) + 3) & ~3) * height);
  ...
  //  printf("width =%4d, height =%4d, depth = %2d, compression = %d, colors_used = %3d, repcount =
  //  %2d, row_order = %2d\n",
