  ...
  if (draw_mode) {
    ...
    if (mouse_x >= x && mouse_x < x + w) {
      if (mouse_y >= y - fl_height() + fl_descent() && mouse_y <= y + fl_descent()) {
        ...
        int l =
            (int)(f +
                  strlen(t)); // use 'quote_char' to calculate the true length of the HTML string
        if (draw_mode == 1) {
...
void Fl_Help_View::draw() {
  ...
  fl_push_clip(x() + Fl::box_dx(b), y() + Fl::box_dy(b), ww - Fl::box_dw(b), hh - Fl::box_dh(b));
  fl_color(textcolor_);
      ...
      for (ptr = block->start, buf.clear(); ptr < block->end;) {
        if ((*ptr == '<' || isspace((*ptr) & 255)) && buf.size() > 0) {
          if (!head && !pre) {
            ...
            if (underline) {
              ...
              fl_xyline(xx + x() - leftline_, yy + y() + 1, xx + x() - leftline_ + ww + xtra_ww);
            }
            ...
            while (isspace((*ptr) & 255)) {
              if (*ptr == '\n') {
                ...
                if (underline)
                  fl_xyline(xx + x() - leftline_, yy + y() + 1, xx + x() - leftline_ + buf.width());
                buf.clear();
                current_pos = (int)(ptr - value_);
                if (line < 31)
                  line++;
        ...
        if (*ptr == '<') {
          ...
          if (buf.cmp("HEAD"))
            ...
            fl_line(block->x + x(), yy + y(), block->w + x(), yy + y());
            ...
            yy += 2 * fsize; // hh;
            hh = 0;
          } else if (buf.cmp("CENTER") || buf.cmp("P") || buf.cmp("H1") || buf.cmp("H2") ||
                     buf.cmp("H3") || buf.cmp("H4") || buf.cmp("H5") || buf.cmp("H6") ||
                     buf.cmp("UL") || buf.cmp("OL") || buf.cmp("DL") || buf.cmp("LI") ||
            ...
            pushfont(font, fsize);
            buf.clear();
          } else if (buf.cmp("A") && get_attr(attrs, "HREF", attr, sizeof(attr)) != NULL) {
            fl_color(linkcolor_);
            underline = 1;
          } else if (buf.cmp("/A")) {
            fl_color(textcolor_);
            ...
            if (get_attr(attrs, "FACE", attr, sizeof(attr)) != NULL) {
              if (!strncasecmp(attr, "helvetica", 9) || !strncasecmp(attr, "arial", 5) ||
                  !strncasecmp(attr, "sans", 4))
                font = FL_HELVETICA;
              else if (!strncasecmp(attr, "times", 5) || !strncasecmp(attr, "serif", 5))
                font = FL_TIMES;
            ...
            head = 0;
          else if (buf.cmp("/H1") || buf.cmp("/H2") || buf.cmp("/H3") || buf.cmp("/H4") ||
                   buf.cmp("/H5") || buf.cmp("/H6") || buf.cmp("/B") || buf.cmp("/STRONG") ||
                   buf.cmp("/I") || buf.cmp("/EM") || buf.cmp("/CODE") || buf.cmp("/TT") ||
            ...
            if (img) {
              img->draw(xx + x() - leftline_, yy + y() - fl_height() + fl_descent() + 2);
            }
        ...
        } else if (*ptr == '&') // process html entity
        {
          ...
          if (qch < 0)
            ...
            ptr = strchr(ptr, ';') + 1;
            entity_extra_length +=
                int(ptr - (oldptr - 1)) - utf8l; // extra length between html entity and UTF-8
          }
    ...
    for (ptr = value_, buf.clear(); *ptr;) {
      ...
      if (*ptr == '<') {
        ...
        if (buf.cmp("HEAD"))
          ...
          char *st;
          for (st = title_; *ptr != '<' && *ptr && st < (title_ + sizeof(title_) - 1);
               *st++ = *ptr++) { /*empty*/
          }
          ...
          *st = '\0';
          ...
          linkcolor_ =
              get_color(get_attr(attrs, "LINK", attr, sizeof(attr)), fl_contrast(FL_BLUE, color()));
        } else if (buf.cmp("BR")) {
          line = do_align(block, line, xx, newalign, links);
          xx = block->x;
          block->h += hh;
          ...
          hh = 0;
                   ...
                   buf.cmp("H3") || buf.cmp("H4") || buf.cmp("H5") || buf.cmp("H6") ||
                   buf.cmp("UL") || buf.cmp("OL") || buf.cmp("DL") || buf.cmp("LI") ||
                   buf.cmp("DD") || buf.cmp("DT") || buf.cmp("HR") || buf.cmp("PRE") ||
                   buf.cmp("TABLE")) {
          block->end = start;
          line = do_align(block, line, xx, newalign, links);
          ...
          if ((tolower(buf[0]) == 'h' && isdigit(buf[1])) || buf.cmp("DD") || buf.cmp("DT") ||
              buf.cmp("P"))
            yy += fsize + 2;
          else if (buf.cmp("HR")) {
            hh += 2 * fsize;
          ...
          //#if defined(__GNUC__)
          //#warning FIXME this isspace & 255 test will probably not work on a utf8 stream... And we
          // use it everywhere! #endif /*__GNUC__*/
          while (isspace((*ptr) & 255))
            ptr++;
    ...
    if (*ptr == '<') {
      ...
      if (buf.cmp("BR") || buf.cmp("HR")) {
        ...
        break;
               ...
               buf.cmp("H3") || buf.cmp("H4") || buf.cmp("H5") || buf.cmp("H6") || buf.cmp("UL") ||
               buf.cmp("OL") || buf.cmp("DL") || buf.cmp("LI") || buf.cmp("DD") || buf.cmp("DT") ||
               buf.cmp("PRE")) {
        width = 0;
        needspace = 0;
                        ...
                        Fl_Color c)    // I - Default color value
{
  ...
  static const struct { // Color name table
    ...
    int r, g, b;
      ...
      {"black", 0x00, 0x00, 0x00},   {"red", 0xff, 0x00, 0x00},    {"green", 0x00, 0x80, 0x00},
      {"yellow", 0xff, 0xff, 0x00},  {"blue", 0x00, 0x00, 0xff},   {"magenta", 0xff, 0x00, 0xff},
      {"fuchsia", 0xff, 0x00, 0xff}, {"cyan", 0x00, 0xff, 0xff},   {"aqua", 0x00, 0xff, 0xff},
      {"white", 0xff, 0xff, 0xff},   {"gray", 0x80, 0x80, 0x80},   {"grey", 0x80, 0x80, 0x80},
      {"lime", 0x00, 0xff, 0x00},    {"maroon", 0x80, 0x00, 0x00}, {"navy", 0x00, 0x00, 0x80},
      {"olive", 0x80, 0x80, 0x00},   {"purple", 0x80, 0x00, 0x80}, {"silver", 0xc0, 0xc0, 0xc0},
      {"teal", 0x00, 0x80, 0x80}};
...
void Fl_Help_View::follow_link(Fl_Help_Link *linkp) {
  ...
  if (strcmp(linkp->filename, filename_) != 0 && linkp->filename[0]) {
    ...
    if (strchr(directory_, ':') != NULL && strchr(linkp->filename, ':') == NULL) {
      if (linkp->filename[0] == '/') {
        strlcpy(temp, directory_, sizeof(temp));
        if ((tempptr = strrchr(strchr(directory_, ':') + 3, '/')) != NULL)
    ...
    if (linkp->name[0])
      snprintf(temp + strlen(temp), sizeof(temp) - strlen(temp), "#%s", linkp->name);
...
int Fl_Help_View::load(const char *f) {
  ...
  if (strncmp(f, "ftp:", 4) == 0 || strncmp(f, "http:", 5) == 0 || strncmp(f, "https:", 6) == 0 ||
      strncmp(f, "ipp:", 4) == 0 || strncmp(f, "mailto:", 7) == 0 || strncmp(f, "news:", 5) == 0) {
                          ...
                          int hh) // I - New height
{
  ...
  int scrollsize = scrollbar_size_ ? scrollbar_size_ : Fl::scrollbar_size();
  scrollbar_.resize(x() + w() - scrollsize - Fl::box_dw(b) + Fl::box_dx(b), y() + Fl::box_dy(b),
                    scrollsize, h() - scrollsize - Fl::box_dh(b));
  hscrollbar_.resize(x() + Fl::box_dx(b), y() + h() - scrollsize - Fl::box_dh(b) + Fl::box_dy(b),
...
quote_char(const char *p) { // I - Quoted string
  ...
  static const struct {
    ...
    int code;
      ...
      names[] = {
          // Quoting names
          {"Aacute;", 7, 193},    {"aacute;", 7, 225},  {"Acirc;", 6, 194},  {"acirc;", 6, 226},
          {"acute;", 6, 180},     {"AElig;", 6, 198},   {"aelig;", 6, 230},  {"Agrave;", 7, 192},
          {"agrave;", 7, 224},    {"amp;", 4, '&'},     {"Aring;", 6, 197},  {"aring;", 6, 229},
          {"Atilde;", 7, 195},    {"atilde;", 7, 227},  {"Auml;", 5, 196},   {"auml;", 5, 228},
          {"brvbar;", 7, 166},    {"bull;", 5, 0x2022}, {"Ccedil;", 7, 199}, {"ccedil;", 7, 231},
          {"cedil;", 6, 184},     {"cent;", 5, 162},    {"copy;", 5, 169},   {"curren;", 7, 164},
          {"dagger;", 7, 0x2020}, {"deg;", 4, 176},     {"divide;", 7, 247}, {"Eacute;", 7, 201},
          {"eacute;", 7, 233},    {"Ecirc;", 6, 202},   {"ecirc;", 6, 234},  {"Egrave;", 7, 200},
          {"egrave;", 7, 232},    {"ETH;", 4, 208},     {"eth;", 4, 240},    {"Euml;", 5, 203},
          {"euml;", 5, 235},      {"euro;", 5, 0x20ac}, {"frac12;", 7, 189}, {"frac14;", 7, 188},
          {"frac34;", 7, 190},    {"gt;", 3, '>'},      {"Iacute;", 7, 205}, {"iacute;", 7, 237},
          {"Icirc;", 6, 206},     {"icirc;", 6, 238},   {"iexcl;", 6, 161},  {"Igrave;", 7, 204},
          {"igrave;", 7, 236},    {"iquest;", 7, 191},  {"Iuml;", 5, 207},   {"iuml;", 5, 239},
          {"laquo;", 6, 171},     {"lt;", 3, '<'},      {"macr;", 5, 175},   {"micro;", 6, 181},
          {"middot;", 7, 183},    {"nbsp;", 5, ' '},    {"not;", 4, 172},    {"Ntilde;", 7, 209},
          {"ntilde;", 7, 241},    {"Oacute;", 7, 211},  {"oacute;", 7, 243}, {"Ocirc;", 6, 212},
          {"ocirc;", 6, 244},     {"Ograve;", 7, 210},  {"ograve;", 7, 242}, {"ordf;", 5, 170},
          {"ordm;", 5, 186},      {"Oslash;", 7, 216},  {"oslash;", 7, 248}, {"Otilde;", 7, 213},
          {"otilde;", 7, 245},    {"Ouml;", 5, 214},    {"ouml;", 5, 246},   {"para;", 5, 182},
          {"permil;", 7, 0x2030}, {"plusmn;", 7, 177},  {"pound;", 6, 163},  {"quot;", 5, '\"'},
          {"raquo;", 6, 187},     {"reg;", 4, 174},     {"sect;", 5, 167},   {"shy;", 4, 173},
          {"sup1;", 5, 185},      {"sup2;", 5, 178},    {"sup3;", 5, 179},   {"szlig;", 6, 223},
          {"THORN;", 6, 222},     {"thorn;", 6, 254},   {"times;", 6, 215},  {"trade;", 6, 0x2122},
          {"Uacute;", 7, 218},    {"uacute;", 7, 250},  {"Ucirc;", 6, 219},  {"ucirc;", 6, 251},
          {"Ugrave;", 7, 217},    {"ugrave;", 7, 249},  {"uml;", 4, 168},    {"Uuml;", 5, 220},
          {"uuml;", 5, 252},      {"Yacute;", 7, 221},  {"yacute;", 7, 253}, {"yen;", 4, 165},
          {"Yuml;", 5, 0x0178},   {"yuml;", 5, 255}};
