...
int Fl_Quartz_Native_File_Chooser_Driver::get_saveas_basename(void) {
  ...
  if (!(_options & Fl_Native_File_Chooser::SAVEAS_CONFIRM)) {
    ...
    if (memcmp(q + l, UNLIKELYPREFIX, lu) == 0)
...
- (BOOL)panel:(id)sender shouldShowFilename:(NSString *)filename {
  if ([nspopup indexOfSelectedItem] == [nspopup numberOfItems] - 1)
    return YES;
  BOOL isdir = NO;
...
// correspondingly changes the extension of the output file name
{
  ...
  *q = 0;
  ...
  if (fl_mac_os_version >= 100900)
    [dialog setAllowedFileTypes:[NSArray arrayWithObject:[NSString stringWithUTF8String:p]]];
  free(s);
...
int Fl_Quartz_Native_File_Chooser_Driver::runmodal() {
  ...
  if (_directory && !dir)
    dir = [[NSString alloc] initWithUTF8String:_directory];
  ...
  if (fl_mac_os_version >= 100600) {
    ...
    if (usepath) {
      ...
      if (dir)
        [_panel setDirectoryURL:[NSURL fileURLWithPath:dir]]; // 10.6
      if (fname)
        [_panel setNameFieldStringValue:fname]; // 10.6
    }
    [path release];
    if ([NSApp mainWindow]) {
      ...
      [_panel
          beginSheetModalForWindow:[NSApp mainWindow]
                 completionHandler:^(NSInteger returnCode) {
                   complete = returnCode; // this block runs after OK or Cancel was triggered in
                                          // file dialog
                 }]; // 10.6 this message returns immediately and begins the file dialog as a sheet
      while (complete == -1)
        Fl::wait(100); // loop until end of file dialog
      retval = complete;
    } else {
...
int Fl_Quartz_Native_File_Chooser_Driver::post() {
  ...
  if (_title) {
    SEL title_or_message = (is_open_panel && fl_mac_os_version >= 101200) ? @selector(setMessage:)
                                                                          : @selector(setTitle:);
  ...
  if (is_open_panel) {
    ...
    if (_filt_total) {
      if (_filt_value >= _filt_total)
        _filt_value = _filt_total - 1;
      char *t = prepareMacFilter(_filt_total, _filter, _filt_patt);
