...
Fl_File_Icon::~Fl_File_Icon() {
  ...
  for (current = first_, prev = (Fl_File_Icon *)0; current != this && current != (Fl_File_Icon *)0;
                        ...
                        int active)  // I - Active or inactive?
{
  ...
  fl_push_matrix();
  fl_translate((float)x + 0.5 * ((float)w - scale), (float)y + 0.5 * ((float)h + scale));
  ...
  while (d < dend)
    switch (*d) {
        ...
        if (prim)
          switch (*prim) {
            ...
            case OUTLINEPOLYGON:
              ...
              oc = (Fl_Color)((((unsigned short *)prim)[1] << 16) | ((unsigned short *)prim)[2]);
              if (active) {
                if (oc == FL_ICON_COLOR)
                  fl_color(ic);
                else
      ...
      case COLOR:
        c = (Fl_Color)((((unsigned short *)d)[1] << 16) | ((unsigned short *)d)[2]);
