...
Fl_Wayland_Window_Driver::Fl_Wayland_Window_Driver(Fl_Window *win)
...
void Fl_Wayland_Window_Driver::delete_cursor_() {
  if (cursor_) {
    ...
    struct fl_wld_buffer *offscreen =
        (struct fl_wld_buffer *)wl_buffer_get_user_data(new_image->buffer);
    ...
    if (scr_driver->default_cursor() == cursor_)
...
struct xdg_toplevel *Fl_Wayland_Window_Driver::xdg_toplevel() {
  ...
  if (w->kind == DECORATED)
    top = libdecor_frame_get_xdg_toplevel(w->frame);
  else if (w->kind == UNFRAMED)
    top = w->xdg_toplevel;
  return top;
}
...
void Fl_Wayland_Window_Driver::flush_overlay() {
  ...
  wl_surface_damage_buffer(xid->wl_surface, 0, 0, pWindow->w() * xid->scale,
                           pWindow->h() * xid->scale);
}
...
void Fl_Wayland_Window_Driver::shape_bitmap_(Fl_Image *b) { // needs testing
  ...
  cairo_surface_t *mask_surf =
      cairo_image_surface_create_for_data(bits, CAIRO_FORMAT_A1, w, h, bytesperrow);
...
void Fl_Wayland_Window_Driver::draw_end() {
  if (shape_data_ && shape_data_->mask_pattern_) {
    ...
    cairo_matrix_init_scale(&matrix, double(shape_data_->lw_) / pWindow->w(),
                            double(shape_data_->lh_) / pWindow->h());
 ...
 */
void Fl_Wayland_Window_Driver::capture_titlebar_and_borders(Fl_RGB_Image *&top, Fl_RGB_Image *&left,
                                                            Fl_RGB_Image *&bottom,
                                                            Fl_RGB_Image *&right) {
...
void Fl_Wayland_Window_Driver::make_current() {
  ...
  if (window && window->buffer) {
    ((Fl_Cairo_Graphics_Driver *)fl_graphics_driver)
        ->needs_commit_tag(&window->buffer->draw_buffer_needs_commit);
  }
  ...
  if (!window->buffer) {
    window->buffer =
        Fl_Wayland_Graphics_Driver::create_shm_buffer(pWindow->w() * scale, pWindow->h() * scale);
    ((Fl_Cairo_Graphics_Driver *)fl_graphics_driver)
        ->needs_commit_tag(&window->buffer->draw_buffer_needs_commit);
  }
...
void Fl_Wayland_Window_Driver::flush() {
  ...
  if (pWindow->as_gl_window()) {
    ...
    if (plugin) {
      ...
      if (scale != fl_graphics_driver->scale() || W != pWindow->w() || H != pWindow->h())
        plugin->invalidate(pWindow);
    }
    return;
  }
  struct wld_window *window = fl_xid(pWindow);
  if (!window || !window->configured_width)
    return;
  ...
  if (r && window->buffer) {
    ...
    wl_surface_damage_buffer(window->wl_surface, 0, 0, pWindow->w() * window->scale * f,
                             pWindow->h() * window->scale * f);
    // fprintf(stderr, "damage 0x0 %dx%d\n", pWindow->w() * window->scale, pWindow->h() *
    // window->scale);
  }
...
void Fl_Wayland_Window_Driver::hide() {
  ...
  if (wld_win) { // this test makes sure ip->xid has not been destroyed already
    ...
    // wld_win->xdg_popup, wld_win->wl_surface);
...
void Fl_Wayland_Window_Driver::map() {
  ...
  if (wl_win->kind == DECORATED)
    libdecor_frame_map(wl_win->frame); // needs checking
...
void Fl_Wayland_Window_Driver::unmap() {
  ...
  if (wl_win->kind == DECORATED && wl_win->frame) {
    libdecor_frame_close(wl_win->frame); // needs checking
...
int Fl_Wayland_Window_Driver::scroll(int src_x, int src_y, int src_w, int src_h, int dest_x,
...
static void handle_error(struct libdecor *libdecor_context, enum libdecor_error error,
                         const char *message) {
...
static void handle_configure(struct libdecor_frame *frame,
  ...
  if (!window->xdg_surface)
    window->xdg_surface = libdecor_frame_get_xdg_surface(frame);
  ...
  // during resizable window creation (see
  // https://gitlab.freedesktop.org/wayland/wayland-protocols/-/issues/6). Consequently,
  // libdecor_configuration_get_content_size() may return false twice. In that case
  // libdecor_frame_get_content_{width,height}() give the desired window size
  ...
  if (ceil(width / f) != window->configured_width ||
  ...
  // fprintf(stderr, "handle_configure fl_win=%p size:%dx%d state=%x wait_for_expose_value=%d
  // is_2nd_run=%d\n", window->fl_win, width,height,window_state,driver->wait_for_expose_value,
  // is_2nd_run);
  ...
  if (window_state & LIBDECOR_WINDOW_STATE_MAXIMIZED)
    ...
    state = libdecor_state_new(int(ceil(width / f) * f), int(ceil(height / f) * f));
...
static void handle_commit(struct libdecor_frame *frame, void *user_data) {
  struct wld_window *wl_win = (struct wld_window *)user_data;
  if (wl_win->wl_surface)
    wl_surface_commit(wl_win->wl_surface);
}

static void handle_dismiss_popup(struct libdecor_frame *frame, const char *seat_name,
                                 void *user_data) {}
...
static Fl_Window *calc_transient_parent(int &center_x, int &center_y) {
  ...
  while (target) {
    ...
    while (child) {
      if (child->window() == target &&
          child->user_data() != &Fl_Screen_Driver::transient_scale_display &&
          child->x() <= center_x && child->x() + child->w() > center_x && child->y() <= center_y &&
          child->y() + child->h() > center_y) {
        break; // child contains the center of top
      }
      child = Fl::next_window(child);
    }
    if (!child)
      break; // no more subwindow contains the center of top
    target = child;
...
Fl_X *Fl_Wayland_Window_Driver::makeWindow() {
  ...
  if (pWindow->menu_window() || pWindow->tooltip_window()) { // a menu window or tooltip
    ...
    new_window->xdg_surface =
        xdg_wm_base_get_xdg_surface(scr_driver->xdg_wm_base, new_window->wl_surface);
    ...
    Fl_Widget *target =
        (pWindow->tooltip_window() ? Fl_Tooltip::current() : Fl_Window_Driver::menu_parent());
    if (!target)
      target = Fl::belowmouse();
    if (!target)
      target = Fl::first_window();
    Fl_Window *parent_win = target->top_window();
    while (parent_win && parent_win->menu_window())
    ...
    if (!scr_driver->libdecor_context)
      scr_driver->libdecor_context =
          libdecor_new(Fl_Wayland_Screen_Driver::wl_display, &libdecor_iface);
    ...
    // pWindow->x(), pWindow->y());
    libdecor_frame_set_app_id(new_window->frame,
                              get_prog_name()); // appears in the Gnome desktop menu bar
    ...
    struct wld_window *parent = fl_xid(pWindow->window());
    new_window->subsurface = wl_subcompositor_get_subsurface(
        scr_driver->wl_subcompositor, new_window->wl_surface, parent->wl_surface);
    ...
    // "A sub-surface becomes mapped, when a non-NULL wl_buffer is applied and the parent surface is
    // mapped."
    ...
    if (pWindow->label())
      xdg_toplevel_set_title(new_window->xdg_toplevel, pWindow->label());
    wl_surface_commit(new_window->wl_surface);
    pWindow->border(0);
  }
  ...
  if (pWindow->modal() || pWindow->non_modal()) {
    ...
    if (new_window->kind == DECORATED && first_xid && first_xid->kind == DECORATED) {
      ...
      if (top_dr->xdg_toplevel())
        xdg_toplevel_set_parent(new_window->xdg_toplevel, top_dr->xdg_toplevel());
    }
  }
...
Fl_Wayland_Window_Driver::type_for_resize_window_between_screens
    Fl_Wayland_Window_Driver::data_for_resize_window_between_screens_ = {0, false};
...
int Fl_Wayland_Window_Driver::set_cursor(Fl_Cursor c) {
  ...
  switch (c) {
    case FL_CURSOR_ARROW:
      if (!scr_driver->xc_arrow)
        scr_driver->xc_arrow = scr_driver->cache_cursor("left_ptr");
      scr_driver->default_cursor(scr_driver->xc_arrow);
      break;
    case FL_CURSOR_NS:
      if (!scr_driver->xc_ns)
        scr_driver->xc_ns = scr_driver->cache_cursor("sb_v_double_arrow");
      if (!scr_driver->xc_ns)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_ns);
      break;
    case FL_CURSOR_CROSS:
      if (!scr_driver->xc_cross)
        scr_driver->xc_cross = scr_driver->cache_cursor("cross");
      if (!scr_driver->xc_cross)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_cross);
      break;
    case FL_CURSOR_WAIT:
      if (!scr_driver->xc_wait)
        scr_driver->xc_wait = scr_driver->cache_cursor("wait");
      if (!scr_driver->xc_wait)
        scr_driver->xc_wait = scr_driver->cache_cursor("watch");
      if (!scr_driver->xc_wait)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_wait);
      break;
    case FL_CURSOR_INSERT:
      if (!scr_driver->xc_insert)
        scr_driver->xc_insert = scr_driver->cache_cursor("xterm");
      if (!scr_driver->xc_insert)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_insert);
      break;
    case FL_CURSOR_HAND:
      if (!scr_driver->xc_hand)
        scr_driver->xc_hand = scr_driver->cache_cursor("hand");
      if (!scr_driver->xc_hand)
        scr_driver->xc_hand = scr_driver->cache_cursor("hand1");
      if (!scr_driver->xc_hand)
        return 0;
    ...
    case FL_CURSOR_HELP:
      if (!scr_driver->xc_help)
        scr_driver->xc_help = scr_driver->cache_cursor("help");
      if (!scr_driver->xc_help)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_help);
      break;
    case FL_CURSOR_MOVE:
      if (!scr_driver->xc_move)
        scr_driver->xc_move = scr_driver->cache_cursor("move");
      if (!scr_driver->xc_move)
        return 0;
    ...
    case FL_CURSOR_WE:
      if (!scr_driver->xc_we)
        scr_driver->xc_we = scr_driver->cache_cursor("sb_h_double_arrow");
      if (!scr_driver->xc_we)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_we);
      break;
    case FL_CURSOR_N:
      if (!scr_driver->xc_north)
        scr_driver->xc_north = scr_driver->cache_cursor("top_side");
      if (!scr_driver->xc_north)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_north);
      break;
    case FL_CURSOR_E:
      if (!scr_driver->xc_east)
        scr_driver->xc_east = scr_driver->cache_cursor("right_side");
      if (!scr_driver->xc_east)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_east);
      break;
    case FL_CURSOR_W:
      if (!scr_driver->xc_west)
        scr_driver->xc_west = scr_driver->cache_cursor("left_side");
      if (!scr_driver->xc_west)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_west);
      break;
    case FL_CURSOR_S:
      if (!scr_driver->xc_south)
        scr_driver->xc_south = scr_driver->cache_cursor("bottom_side");
      if (!scr_driver->xc_south)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_south);
      break;
    case FL_CURSOR_NESW:
      if (!scr_driver->xc_nesw)
        scr_driver->xc_nesw = scr_driver->cache_cursor("fd_double_arrow");
      if (!scr_driver->xc_nesw)
        return 0;
      scr_driver->default_cursor(scr_driver->xc_nesw);
      break;
    case FL_CURSOR_NWSE:
      if (!scr_driver->xc_nwse)
        scr_driver->xc_nwse = scr_driver->cache_cursor("bd_double_arrow");
      if (!scr_driver->xc_nwse)
        return 0;
    ...
    case FL_CURSOR_SW:
      if (!scr_driver->xc_sw)
        scr_driver->xc_sw = scr_driver->cache_cursor("bottom_left_corner");
      if (!scr_driver->xc_sw)
        return 0;
    ...
    case FL_CURSOR_SE:
      if (!scr_driver->xc_se)
        scr_driver->xc_se = scr_driver->cache_cursor("bottom_right_corner");
      if (!scr_driver->xc_se)
        return 0;
    ...
    case FL_CURSOR_NE:
      if (!scr_driver->xc_ne)
        scr_driver->xc_ne = scr_driver->cache_cursor("top_right_corner");
      if (!scr_driver->xc_ne)
        return 0;
    ...
    case FL_CURSOR_NW:
      if (!scr_driver->xc_nw)
        scr_driver->xc_nw = scr_driver->cache_cursor("top_left_corner");
      if (!scr_driver->xc_nw)
        return 0;
...
void Fl_Wayland_Window_Driver::fullscreen_on() {
  ...
  if (xdg_toplevel()) {
    ...
    wl_display_roundtrip(
        Fl_Wayland_Screen_Driver::wl_display); // OK, but try to find something more specific
    wl_display_roundtrip(Fl_Wayland_Screen_Driver::wl_display);
...
int Fl_Wayland_Window_Driver::set_cursor(const Fl_RGB_Image *rgb, int hotx, int hoty) {
  ...
  struct fl_wld_buffer *offscreen = Fl_Wayland_Graphics_Driver::create_shm_buffer(
      new_image->image.width, new_image->image.height);
  ...
  Fl_Image_Surface *img_surf =
      new Fl_Image_Surface(new_image->image.width, new_image->image.height, 0, offscreen);
    ...
    if (is_a_resize) {
      if (fl_win->kind == DECORATED) { // a decorated window
        ...
        if (!in_handle_configure && xdg_toplevel()) {
          ...
          libdecor_frame_commit(fl_win->frame, state,
                                NULL); // necessary only if resize is initiated by prog
          libdecor_state_free(state);
...
void Fl_Wayland_Window_Driver::reposition_menu_window(int x, int y) {
  ...
  xid_menu->xdg_surface =
      xdg_wm_base_get_xdg_surface(scr_driver->xdg_wm_base, xid_menu->wl_surface);
  ...
  xdg_positioner_set_gravity(positioner, XDG_POSITIONER_GRAVITY_BOTTOM_RIGHT);
  if (y_offset)
    xdg_positioner_set_offset(positioner, 0, y_offset);
  xid_menu->xdg_popup =
      xdg_surface_get_popup(xid_menu->xdg_surface, parent_xid->xdg_surface, positioner);
  xdg_positioner_destroy(positioner);
...
void Fl_Wayland_Window_Driver::menu_window_area(int &X, int &Y, int &W, int &H, int nscreen) {
  ...
  if (parent) {
    ...
    Fl_Window_Driver::menu_window_area(X, Y, W, H, nscreen);
}
