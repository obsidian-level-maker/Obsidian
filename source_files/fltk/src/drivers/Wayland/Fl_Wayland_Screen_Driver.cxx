 ...
 * KDE uses SSD (server-side decoration) which means the OS draws titlebars according to its own
rules and triggers resize, minimization and maximization events.

- Function registry_handle_global() runs within fl_open_display() and sets public static variable
 Fl_Wayland_Screen_Driver::compositor to either Fl_Wayland_Screen_Driver::MUTTER, ::WESTON, or
::KDE.
 ...
 * When a libdecor-framed window is minimized under Weston, the frame remains on display. To avoid
 that, function libdecor_frame_set_minimized() is modified so it turns off the frame's visibility,
with function libdecor_frame_set_visibility(), when the window is minimized. That's implemented in
file libdecor/build/fl_libdecor.c. The modified libdecor_frame_set_minimized() function, part of
libdecor.so, needs access to variable Fl_Wayland_Screen_Driver::compositor, part of libfltk.a. This
is achieved calling FLTK function fl_libdecor_using_weston() which returns whether the running
compositor is Weston. This Weston bug has been corrected in Weston version 10. Thus, this special
processing is not performed when Weston version is ≥ 10.
...
- Synchronization between drawing to buffer and committing buffer to screen.
 ...
 compositor when it's ready to commit a new graphics scene. This function schedules a new frame
callback and commits the buffer to the display. A frame callback is also created by
Fl_Wayland_Window_Driver::flush() when a window redraw operation is needed. FLTK processes wayland
events until the compositor is ready for commit and then commits the new window content.

 - Support of Fl_Window::border(int) :
 FLTK uses libdecor_frame_set_visibility() to show or hide a toplevel window's frame. This doesn't
work with KDE which uses Server-Side Decoration. In that case, FLTK hides and re-shows the window to
toggle between presence and absence of a window's frame.
*/
...
static void set_event_xy(Fl_Window *win) {
  ...
  if (abs(Fl::e_x_root - px) + abs(Fl::e_y_root - py) > 3 || wld_event_time >= ptime + 1000) {
    Fl::e_is_click = 0;
...
static Fl_Window *event_coords_from_surface(struct wl_surface *surface, wl_fixed_t surface_x,
...
static void pointer_button(void *data, struct wl_pointer *wl_pointer, uint32_t serial,
  ...
  if (button == BTN_LEFT && state == WL_POINTER_BUTTON_STATE_PRESSED &&
      seat->pointer_focus == NULL && fl_xid(win)->kind == Fl_Wayland_Window_Driver::DECORATED) {
    // click on titlebar
...
static void init_cursors(struct seat *seat) {
  ...
  if (theme != NULL) {
    if (seat->cursor_theme) {
      // caution to destroy theme because Fl_Wayland_Window_Driver::set_cursor(Fl_Cursor) caches
      // used cursors
...
static void wl_keyboard_keymap(void *data, struct wl_keyboard *wl_keyboard, uint32_t format,
  ...
  struct xkb_keymap *xkb_keymap = xkb_keymap_new_from_string(
      seat->xkb_context, map_shm, XKB_KEYMAP_FORMAT_TEXT_V1, XKB_KEYMAP_COMPILE_NO_FLAGS);
  munmap(map_shm, size);
  close(fd);
...
static void key_repeat_timer_cb(key_repeat_data_t *key_repeat_data) {
  if ((Fl::event() == FL_KEYDOWN || (Fl_Window_Driver::menu_parent() && Fl::event() == FL_ENTER)) &&
      wld_event_time == key_repeat_data->time) {
  ...
  if (scr_driver->seat->text_input) {
    ...
    if (zwp_text_input_v3_get_user_data(scr_driver->seat->text_input)) {
      zwp_text_input_v3_set_cursor_rectangle(scr_driver->seat->text_input, insertion_point_x,
                                             insertion_point_y, insertion_point_width,
...
{
  if (!insertion_point_location_is_valid)
    return false;
...
int Fl_Wayland_Screen_Driver::compose(int &del) {
  ...
  int condition =
      (Fl::e_state & (FL_ALT | FL_META | FL_CTRL)) && ascii < 128;       // letter+modifier key
  ...
  if (ascii && (!Fl::compose_state) && (ascii <= 31 || ascii == 127)) {
    del = 0;
    return 0;
  }
  return 1;
...
static dead_key_struct dead_keys[] = {
    {XKB_KEY_dead_grave, "`"},     {XKB_KEY_dead_acute, "´"},       {XKB_KEY_dead_circumflex, "^"},
    {XKB_KEY_dead_tilde, "~"},     {XKB_KEY_dead_perispomeni, "~"}, // alias for dead_tilde
    {XKB_KEY_dead_macron, "¯"},    {XKB_KEY_dead_breve, "˘"},       {XKB_KEY_dead_abovedot, "˙"},
    {XKB_KEY_dead_diaeresis, "¨"}, {XKB_KEY_dead_abovering, "˚"},   {XKB_KEY_dead_doubleacute, "˝"},
    {XKB_KEY_dead_caron, "ˇ"},     {XKB_KEY_dead_cedilla, "¸"},     {XKB_KEY_dead_ogonek, "˛"},
    {XKB_KEY_dead_iota, "ι"},      {XKB_KEY_dead_doublegrave, " ̏"},
};
...
static void wl_keyboard_key(void *data, struct wl_keyboard *wl_keyboard, uint32_t serial,
  ...
  fprintf(stderr, "key %s: sym: %-12s(%d) code:%u fl_win=%p, ", action, buf, sym, keycode,
  Fl_Wayland_Screen_Driver::surface_to_window(seat->keyboard_surface));*/
  ...
  if (state == WL_KEYBOARD_KEY_STATE_PRESSED && !(sym >= FL_Shift_L && sym <= FL_Alt_R) &&
    ...
    if (status == XKB_COMPOSE_COMPOSING) {
      if (Fl::e_length == 0) { // dead keys produce e_length = 0
        ...
        for (i = 0; i < dead_key_count; i++) {
          if (dead_keys[i].keysym == sym)
            break;
        }
        if (i < dead_key_count)
          strcpy(buf, dead_keys[i].marked_text);
        else
          buf[0] = 0;
        Fl::e_length = strlen(buf);
        Fl::compose_state = 0;
    ...
    // seat->xkb_context, seat->xkb_compose_state, Fl::e_length, buf);
  }
...
void text_input_done(void *data, struct zwp_text_input_v3 *zwp_text_input_v3, uint32_t serial) {
...
static void seat_capabilities(void *data, struct wl_seat *wl_seat, uint32_t capabilities) {
  ...
  if (scr_driver->text_input_base) {
    seat->text_input =
        zwp_text_input_manager_v3_get_text_input(scr_driver->text_input_base, seat->wl_seat);
...
static void output_done(void *data, struct wl_output *wl_output) {
  ...
  while (xp) { // all mapped windows
    ...
    wl_list_for_each(window_output, &(win->outputs),
                     link) { // all Fl_Wayland_Window_Driver::window_output for this window
      if (window_output->output == output) {
        Fl_Wayland_Window_Driver *win_driver = Fl_Wayland_Window_Driver::driver(win->fl_win);
        if (output->wld_scale != win->scale)
...
static void registry_handle_global(void *user_data, struct wl_registry *wl_registry, uint32_t id,
  ...
  if (strcmp(interface, "wl_compositor") == 0) {
    ...
    scr_driver->wl_compositor =
        (struct wl_compositor *)wl_registry_bind(wl_registry, id, &wl_compositor_interface, 4);

  } else if (strcmp(interface, "wl_subcompositor") == 0) {
    scr_driver->wl_subcompositor = (struct wl_subcompositor *)wl_registry_bind(
        wl_registry, id, &wl_subcompositor_interface, 1);

  } else if (strcmp(interface, "wl_shm") == 0) {
    scr_driver->wl_shm = (struct wl_shm *)wl_registry_bind(wl_registry, id, &wl_shm_interface, 1);
    ...
    if (!scr_driver->seat)
      scr_driver->seat = (struct seat *)calloc(1, sizeof(struct seat));
    ...
    struct xkb_compose_table *table = xkb_compose_table_new_from_locale(
        scr_driver->seat->xkb_context, locale, XKB_COMPOSE_COMPILE_NO_FLAGS);
    ...
    if (scr_driver->seat->data_device_manager) {
      scr_driver->seat->data_device = wl_data_device_manager_get_data_device(
          scr_driver->seat->data_device_manager, scr_driver->seat->wl_seat);
    ...
    if (!scr_driver->seat)
      scr_driver->seat = (struct seat *)calloc(1, sizeof(struct seat));
    scr_driver->seat->data_device_manager = (struct wl_data_device_manager *)wl_registry_bind(
        wl_registry, id, &wl_data_device_manager_interface, 3);
    ...
    output->wld_scale = 1;
    output->wl_output =
        (struct wl_output *)wl_registry_bind(wl_registry, id, &wl_output_interface, 2);
    ...
    scr_driver->xdg_wm_base =
        (struct xdg_wm_base *)wl_registry_bind(wl_registry, id, &xdg_wm_base_interface, 1);
    ...
    scr_driver->text_input_base = (struct zwp_text_input_manager_v3 *)wl_registry_bind(
        wl_registry, id, &zwp_text_input_manager_v3_interface, 1);
...
static void registry_handle_global_remove(void *data, struct wl_registry *registry,
                                          uint32_t name) { // TODO to be tested
  Fl_Wayland_Screen_Driver::output *output;
  ...
  wl_list_for_each(output, &(scr_driver->outputs), link) { // all screens of the system
    if (output->id == name) {                              // the screen being removed
      ...
      while (xp) { // all mapped windows
        struct wld_window *win = xp->xid;
        wl_list_for_each_safe(window_output, tmp, &(win->outputs),
                              link) { // all Fl_Wayland_Window_Driver::window_output for this window
          if (window_output->output == output) {
...
static void fd_callback(int fd, struct wl_display *display) {
  ...
  do {
    if (wl_display_dispatch(display) == -1) {
      Fl::fatal("Fatal error while communicating with the Wayland server: %s", strerror(errno));
    }
  } while (poll(&fds, 1, 0) > 0);
}


Fl_Wayland_Screen_Driver::Fl_Wayland_Screen_Driver()
  : Fl_Screen_Driver() {
...
static void getsyscolor(const char *key1, const char *key2, const char *arg, const char *defarg,
...
Fl_RGB_Image *Fl_Wayland_Screen_Driver::read_win_rectangle(int X, int Y, int w, int h,
                                                           Fl_Window *win, bool ignore,
                                                           bool *p_ignore) {
  Window xid = win ? fl_xid(win) : NULL;
  struct fl_wld_buffer *buffer =
      win ? xid->buffer : (Fl_Offscreen)Fl_Surface_Device::surface()->driver()->gc();
  float s =
      win ? xid->scale * scale(win->screen_num()) : Fl_Surface_Device::surface()->driver()->scale();
  int Xs, Ys, ws, hs;
  if (s == 1) {
    Xs = X;
    Ys = Y;
...
int Fl_Wayland_Screen_Driver::screen_num_unscaled(int x, int y) {
  ...
  wl_list_for_each(output, &outputs, link) {
    ...
    int sx = output->x_org / s, sy = output->y_org / s, sw = output->width / s,
        sh = output->height / s;
...
void Fl_Wayland_Screen_Driver::reset_cursor() {
  xc_arrow = xc_ns = xc_wait = xc_insert = xc_hand = xc_help = xc_cross = xc_move = xc_north =
      xc_south = xc_west = xc_east = xc_we = xc_nesw = xc_nwse = xc_sw = xc_se = xc_ne = xc_nw =
          NULL;
}
...
void Fl_Wayland_Screen_Driver::set_spot(int font, int height, int x, int y, int w, int h,
                                        Fl_Window *win) {
