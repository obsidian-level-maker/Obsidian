...
static void data_source_handle_send(void *data, struct wl_data_source *source,
  ...
  if (strcmp(mime_type, wld_plain_text_clipboard) == 0 || strcmp(mime_type, "text/plain") == 0 ||
...
int Fl_Wayland_Screen_Driver::dnd(int use_selection) {
  ...
  if (use_selection) {
    ...
    dnd_icon = NULL;
  doing_dnd = true;
  wl_data_device_start_drag(scr_driver->seat->data_device, source, scr_driver->seat->pointer_focus,
                            dnd_icon, scr_driver->seat->serial);
  if (use_selection) {
...
static void data_offer_handle_offer(void *data, struct wl_data_offer *offer,
  ...
  } else if (strcmp(mime_type, "image/bmp") == 0 &&
             (!fl_selection_offer_type || strcmp(fl_selection_offer_type, "image/png"))) {
    fl_selection_type[1] = Fl::clipboard_image;
...
static void data_device_handle_selection(void *data, struct wl_data_device *data_device,
  ...
  if (fl_selection_offer)
    wl_data_offer_destroy(fl_selection_offer);
  fl_selection_offer = offer;
  // if (offer == NULL) fprintf(stderr, "Clipboard is empty\n");
}


static size_t convert_crlf(char *s,
                           size_t len) { // turn \r characters into \n and "\r\n" sequences into \n:
  char *p;
  size_t l = len;
  while ((p = strchr(s, '\r'))) {
    if (*(p + 1) == '\n') {
...
static void data_device_handle_motion(void *data, struct wl_data_device *data_device, uint32_t time,
  ...
  uint32_t supported_actions =
      ret ? WL_DATA_DEVICE_MANAGER_DND_ACTION_COPY : WL_DATA_DEVICE_MANAGER_DND_ACTION_NONE;
  uint32_t preferred_action = supported_actions;
  wl_data_offer_set_actions(current_drag_offer, supported_actions, preferred_action);
  wl_display_roundtrip(Fl_Wayland_Screen_Driver::wl_display);
  if (ret && current_drag_offer)
    wl_data_offer_accept(current_drag_offer, fl_dnd_serial, "text/plain");
}
...
static int get_clipboard_image() {
  ...
  if (strcmp(fl_selection_offer_type, "image/png") == 0) {
    ...
    Fl_RGB_Image *image =
        new Fl_RGB_Image(rgb, shared->w(), shared->h(), shared->d(), shared->ld());
    shared->release();
    image->alloc_array = 1;
    ...
    if (n == sizeof(buf)) {
      ...
      int R = ((3 * w + 3) / 4) *
              4; // the number of bytes per row of BMP image, rounded up to multiple of 4
      bmp = new char[R * h + 54];
      memcpy(bmp, buf, 54);
      char *from = bmp + 54;
      rest = R * h;
    ...
    close(fds[0]);
    if (!rest)
      Fl::e_clipboard_data = Fl_Unix_System_Driver::own_bmp_to_RGB(bmp);
    delete[] bmp;
    if (rest)
      return 1;
  }
