//
// Classes Fl_PostScript_File_Device and Fl_PostScript_Graphics_Driver for the Fast Light Tool Kit
// (FLTK).
//
...
Fl_Font Fl_PostScript_Graphics_Driver::font() {
...
void Fl_PostScript_Graphics_Driver::text_extents(const char *c, int n, int &dx, int &dy, int &w,
                                                 int &h) {
...
Fl_Color Fl_PostScript_Graphics_Driver::color() {
...
int Fl_PostScript_Graphics_Driver::clip_box(int x, int y, int w, int h, int &X, int &Y, int &W,
                                            int &H) {
  if (!clip_) {
    X = x;
    Y = y;
...
static const char *prolog_2 = // prolog relevant only if lang_level >1
    ...
    "/ToISO { dup findfont dup length dict copy begin /Encoding ISOLatin1Encoding def currentdict "
    "end definefont pop } def\n"
    ...
    " /Emacron  /emacron  /Ebreve  /ebreve  /Edotaccent  /edotaccent  /Eogonek  /eogonek  /Ecaron  "
    "/ecaron\n"
    " /Gcircumflex  /gcircumflex  /Gbreve  /gbreve  /Gdotaccent  /gdotaccent  /Gcommaaccent  "
    "/gcommaaccent \n"
    ...
    " /Lacute  /lacute  /Lcommaaccent  /lcommaaccent   /Lcaron  /lcaron  /Ldotaccent /ldotaccent   "
    "/Lslash  /lslash \n"
    ...
    " /florin  /circumflex  /caron  /breve  /dotaccent  /ring \n" // remaining characters from
                                                                  // PostScript standard text fonts
    ...
    " /PSname2 { dup mycharstrings exch known {LatinExtA 3 -1 roll 3 -1 roll put}{pop pop} ifelse "
    "} def \n"
    " 16#20 /Gdot PSname2 16#21 /gdot PSname2 16#30 /Idot PSname2 16#3F /Ldot PSname2 16#40 /ldot "
    "PSname2 16#7F /slong PSname2 \n"

    // proc that gives LatinExtA encoding to a font
    "/ToLatinExtA { findfont dup length dict copy begin /Encoding LatinExtA def currentdict end "
    "definefont pop } def\n"
    // create Ext-versions of standard fonts that use LatinExtA encoding \n"
    "/HelveticaExt /Helvetica ToLatinExtA \n"
    "/Helvetica-BoldExt /Helvetica-Bold ToLatinExtA /Helvetica-ObliqueExt /Helvetica-Oblique "
    "ToLatinExtA  \n"
    ...
    "/Courier-ObliqueExt /Courier-Oblique ToLatinExtA /Courier-BoldObliqueExt /Courier-BoldOblique "
    "ToLatinExtA \n"
...
static const char
    *prolog_2_pixmap = // prolog relevant only if lang_level == 2 for pixmaps/masked color images
...
static const char *prolog_3 = // prolog relevant only if lang_level >2
    ...
    // masked color images
    "/CIM {GS /inter exch def /my exch def /mx exch def /py exch def /px exch def /sy exch def /sx "
    "exch def \n"
    "translate \n"
    "sx sy scale\n"
    "/DeviceRGB setcolorspace\n"
    ...
    //  masked gray images
    "/GIM {GS /inter exch def /my exch def /mx exch def /py exch def /px exch def /sy exch def /sx "
    "exch def \n"
    "translate \n"
    "sx sy scale\n"
    "/DeviceGray setcolorspace\n"
...
void Fl_PostScript_Graphics_Driver::loop(int x0, int y0, int x1, int y1, int x2, int y2, int x3,
                                         int y3) {
  fprintf(output, "GS\n");
  fprintf(output, "BP\n");
  fprintf(output, "%i %i MT\n", x0, y0);
...
void Fl_PostScript_Graphics_Driver::polygon(int x0, int y0, int x1, int y1, int x2, int y2, int x3,
                                            int y3) {
  fprintf(output, "GS\n");
  fprintf(output, "BP\n");
  fprintf(output, "%i %i MT\n", x0, y0);
...
static uchar *calc_mask(uchar *img, int w, int h, Fl_Color bg) {
  ...
  while (h-- > 0) {               // for each row
    ...
    if (bit != 0x80)
      *q++ = byte; // enter last columns' byte in mask
  }
  return mask;
}

// write to PostScript a bitmap image of a UTF8 string
void Fl_PostScript_Graphics_Driver::transformed_draw_extra(const char *str, int n, double x,
                                                           double y, int w, bool rtl) {
...
void Fl_PostScript_Graphics_Driver::concat() {
  clocale_printf("[%g %g %g %g %g %g] CT\n", fl_matrix->a, fl_matrix->b, fl_matrix->c, fl_matrix->d,
                 fl_matrix->x, fl_matrix->y);
}

void Fl_PostScript_Graphics_Driver::reconcat() {
  clocale_printf("[%g %g %g %g %g %g] RCT\n", fl_matrix->a, fl_matrix->b, fl_matrix->c,
                 fl_matrix->d, fl_matrix->x, fl_matrix->y);
}
...
void Fl_PostScript_Graphics_Driver::curve(double x, double y, double x1, double y1, double x2,
...
static cairo_status_t write_to_cairo_stream(FILE *output, unsigned char *data,
                                            unsigned int length) {
  size_t l = fwrite(data, 1, length, output);
  return (l == length ? CAIRO_STATUS_SUCCESS : CAIRO_STATUS_WRITE_ERROR);
}

static int init_cairo_postscript(FILE *output, cairo_t *&cairo_, PangoLayout *&pango_layout, int w,
                                 int h) {
  cairo_surface_t *cs =
      cairo_ps_surface_create_for_stream((cairo_write_func_t)write_to_cairo_stream, output, w, h);
...
// returns 0 iff OK
{
  ...
  if (init_cairo_postscript(output, cairo_, pango_layout_,
                            Fl_Paged_Device::page_formats[format].width,
  ...
  sprintf(feature,
          ...
          Fl_Paged_Device::page_formats[format].name, Fl_Paged_Device::page_formats[format].width,
          Fl_Paged_Device::page_formats[format].height);
...
int Fl_PostScript_Graphics_Driver::start_eps(int width, int height) {
  pw_ = width;
  ph_ = height;
  if (init_cairo_postscript(output, cairo_, pango_layout_, width, height))
    return 1;
                                        ...
                                        int *bottom) // to implement
{
  Fl_PostScript_Graphics_Driver *ps = driver();
  if (left)
    *left = (int)(ps->left_margin / ps->scale_x + .5);
  if (right)
    *right = (int)(ps->left_margin / ps->scale_x + .5);
  if (top)
    *top = (int)(ps->top_margin / ps->scale_y + .5);
  if (bottom)
    *bottom = (int)(ps->top_margin / ps->scale_y + .5);
}
...
// returns 0 iff OK
{
  Fl_PostScript_Graphics_Driver *ps = driver();
  if (w)
    *w = (int)((ps->pw_ - 2 * ps->left_margin) / ps->scale_x + .5);
  if (h)
    *h = (int)((ps->ph_ - 2 * ps->top_margin) / ps->scale_y + .5);
  return 0;
}
...
void Fl_PostScript_File_Device::scale(float s_x, float s_y) {
  ...
  ps->clocale_printf("GR GR GS %d %d TR  %f %f SC %f rotate GS\n", ps->left_margin, ps->top_margin,
                     ps->scale_x, ps->scale_y, ps->angle);
#endif
}
...
void Fl_PostScript_File_Device::rotate(float rot_angle) {
  ...
  ps->clocale_printf("GR GR GS %d %d TR  %f %f SC %d %d TR %f rotate GS\n", ps->left_margin,
                     ps->top_margin, ps->scale_x, ps->scale_y, x_offset, y_offset, ps->angle);
#endif
}
...
Fl_EPS_File_Surface::Fl_EPS_File_Surface(int width, int height, FILE *eps, Fl_Color background,
                                         Fl_PostScript_Close_Command closef)
