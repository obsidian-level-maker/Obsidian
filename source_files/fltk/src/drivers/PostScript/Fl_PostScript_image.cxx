...
void Fl_PostScript_Graphics_Driver::draw_image(const uchar *data, int ix, int iy, int iw, int ih,
                                               int D, int LD) {
  if (abs(D) < 3) { // mono
...
void Fl_PostScript_Graphics_Driver::draw_pixmap(Fl_Pixmap *pxm, int XP, int YP, int WP, int HP,
                                                int cx, int cy) {
...
void Fl_PostScript_Graphics_Driver::draw_rgb(Fl_RGB_Image *rgb, int XP, int YP, int WP, int HP,
                                             int cx, int cy) {
  draw_rgb_bitmap_(rgb, XP, YP, WP, HP, cx, cy);
}


void Fl_PostScript_Graphics_Driver::draw_bitmap(Fl_Bitmap *bitmap, int XP, int YP, int WP, int HP,
                                                int cx, int cy) {
  draw_rgb_bitmap_(bitmap, XP, YP, WP, HP, cx, cy);
}


void Fl_PostScript_Graphics_Driver::draw_rgb_bitmap_(Fl_Image *img, int XP, int YP, int WP, int HP,
                                                     int cx, int cy) {
  ...
  if (cairo_surface_status(surf) == CAIRO_STATUS_SUCCESS) {
    ...
    cairo_matrix_init_scale(&matrix, double(img->data_w()) / (img->w() + 1),
                            double(img->data_h()) / (img->h() + 1));
...
static int convert85(const uchar *bytes4, uchar *chars5) {
  ...
  unsigned val =
      bytes4[0] * (256 * 256 * 256) + bytes4[1] * (256 * 256) + bytes4[2] * 256 + bytes4[3];
...
void Fl_PostScript_Graphics_Driver::write85(void *data, const uchar *p,
...
void Fl_PostScript_Graphics_Driver::close85(
    void *data) // stops ASCII85-encoding after processing remaining unencoded input bytes, if any
{
  struct85 *big = (struct85 *)data;
  int l;
  if (big->l4) { // # of remaining unencoded input bytes
    l = big->l4;
    while (l < 4)
      big->bytes4[l++] = 0;                  // complete them with 0s
...
void *
Fl_PostScript_Graphics_Driver::prepare_rle85() // prepare to produce RLE+ASCII85-encoded output
{
...
void Fl_PostScript_Graphics_Driver::draw_image(Fl_Draw_Image_Cb call, void *data, int ix, int iy,
                                               int iw, int ih, int D) {
...
void Fl_PostScript_Graphics_Driver::draw_image_mono(const uchar *data, int ix, int iy, int iw,
                                                    int ih, int D, int LD) {
  double x = ix, y = iy, w = iw, h = ih;
...
void Fl_PostScript_Graphics_Driver::draw_image_mono(Fl_Draw_Image_Cb call, void *data, int ix,
...
void Fl_PostScript_Graphics_Driver::draw_pixmap(Fl_Pixmap *pxm, int XP, int YP, int WP, int HP,
                                                int cx, int cy) {
  if (scale_for_image_(pxm, XP, YP, WP, HP, cx, cy))
    return;
  const char *const *di = pxm->data();
  int w, h;
  if (!fl_measure_pixmap(di, w, h))
    return;
...
void Fl_PostScript_Graphics_Driver::draw_rgb(Fl_RGB_Image *rgb, int XP, int YP, int WP, int HP,
                                             int cx, int cy) {
  if (scale_for_image_(rgb, XP, YP, WP, HP, cx, cy))
    return;
...
void Fl_PostScript_Graphics_Driver::draw_bitmap(Fl_Bitmap *bitmap, int XP, int YP, int WP, int HP,
                                                int cx, int cy) {
  if (scale_for_image_(bitmap, XP, YP, WP, HP, cx, cy))
    return;
...
int Fl_PostScript_Graphics_Driver::scale_for_image_(Fl_Image *img, int XP, int YP, int WP, int HP,
                                                    int cx, int cy) {
  int X, Y, W, H;
  ...
  clocale_printf("GS %d %d TR  %f %f SC GS\n", X - cx, Y - cy, float(img->w()) / img->data_w(),
                 float(img->h()) / img->data_h());
  return 0;
}
