...
int Fl_Posix_System_Driver::run_program(const char *program, char **argv, char *msg, int msglen) {
  ...
  while (waitpid(pid, &status, 0) < 0) {
    if (errno != EINTR) {
      ...
      if (msg)
        snprintf(msg, msglen, "waitpid(%ld) failed: %s", (long)pid, strerror(errno));
...
/**
 ...
 \param lib_name shared library name (without its extension) or NULL to search the function in the
 running program \param func_name  function name or NULL \return the address of the function (when
 func_name != NULL) or of the shared library, or NULL if not found.
 */
...
#else
  if (lib_name)
    lib_address = quadruple_dlopen(lib_name);
#endif // __APPLE_CC__
  if (func_name && lib_address)
    return ::dlsym(lib_address, func_name);
#endif // HAVE_DLFCN_H
  return lib_address;
}
...
bool Fl_Posix_System_Driver::probe_for_GTK(int major, int minor, void **p_ptr_gtk) {
  ...
  *p_ptr_gtk = Fl_Posix_System_Driver::ptr_gtk;
  // The point here is that after running gtk_init_check, the calling program's current locale can
  // be modified. To avoid that, we memorize the calling program's current locale and restore the
