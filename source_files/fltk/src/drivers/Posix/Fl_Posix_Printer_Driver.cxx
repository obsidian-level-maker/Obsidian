...
public:
  ...
  typedef GtkPrintJob *(*gtk_print_job_new_t)(const char *, GtkPrinter *, GtkPrintSettings *,
                                              GtkPageSetup *);
  ...
  typedef int (*gtk_print_job_set_source_file_t)(GtkPrintJob *job, const char *filename,
                                                 GError **error);
  typedef void (*gtk_print_job_send_t)(GtkPrintJob *, void *, gboolean *, void *);
  typedef void (*gtk_print_settings_set_t)(GtkPrintSettings *settings, const char *key,
                                           const char *value);
  ...
  typedef void (*gtk_print_unix_dialog_set_embed_page_setup_t)(GtkPrintUnixDialog *dialog,
                                                               gboolean embed);
...
int Fl_GTK_Printer_Driver::begin_job(int pagecount, int *firstpage, int *lastpage,
  ...
  GtkPrintUnixDialog *pdialog =
      CALL_GTK(gtk_print_unix_dialog_new)(Fl_Printer::dialog_title, NULL); // 2.10
  ...
  if (response_id == GTK_RESPONSE_OK) {
    ...
    if (orient == GTK_PAGE_ORIENTATION_LANDSCAPE)
    ...
    if (strcmp(pname, GTK_PAPER_NAME_LETTER) == 0)
    ...
    if (firstpage && lastpage) {
      *firstpage = 1;
      *lastpage = pagecount;
      if (CALL_GTK(gtk_print_settings_get_print_pages)(psettings) ==
          GTK_PRINT_PAGES_RANGES) { // 2.10
        int num_ranges;
        GtkPageRange *ranges =
            CALL_GTK(gtk_print_settings_get_page_ranges)(psettings, &num_ranges); // 2.10
        if (num_ranges > 0) {
...
void Fl_GTK_Printer_Driver::end_job() {
  ...
  if (gb) {
    gb = false;
    CALL_GTK(gtk_print_job_send)
    (pjob, (void *)pJobCompleteFunc, &gb, (void *)pDestroyNotify); // 2.10
    gtk_main_iteration_t fl_gtk_main_iteration = CALL_GTK(gtk_main_iteration);
    while (!gb) {
      fl_gtk_main_iteration();
    }
...
#if HAVE_DLSYM && HAVE_DLFCN_H
  static bool gtk =
      (Fl::option(Fl::OPTION_PRINTER_USES_GTK) ? Fl_GTK_Printer_Driver::probe_for_GTK() : false);
  if (gtk)
    return new Fl_GTK_Printer_Driver();
#endif
...
int Fl_Posix_Printer_Driver::begin_job(int pages, int *firstpage, int *lastpage,
  ...
  char command[1024];
  if (style == SystemV)
    snprintf(command, sizeof(command), "lp -s -d %s -n %d -t '%s' -o media=%s", printer,
             print_collate_button->value() ? 1 : (int)(print_copies->value() + 0.5), "FLTK", media);
  else
    snprintf(command, sizeof(command), "lpr -h -P%s -#%d -T FLTK ", printer,
             print_collate_button->value() ? 1 : (int)(print_copies->value() + 0.5));
