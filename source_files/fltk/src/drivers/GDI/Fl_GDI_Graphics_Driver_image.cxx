...
static void innards(const uchar *buf, int X, int Y, int W, int H, int delta, int linedelta,
                    int depth, Fl_Draw_Image_Cb cb, void *userdata, HDC gc) {
  char indexed = 0;
  ...
  for (int j = 0; j < h;) {
    ...
    if (fl_graphics_driver->has_feature(Fl_Graphics_Driver::PRINTER)) {
      ...
      StretchDIBits(gc, x, y + j - k, w, k, 0, 0, w, k,
                    (LPSTR)((uchar *)buffer + (blocking - k) * linesize), &bmi,
#if USE_COLORMAP
...
void Fl_GDI_Graphics_Driver::draw_image_unscaled(const uchar *buf, int x, int y, int w, int h,
                                                 int d, int l) {
...
void Fl_GDI_Graphics_Driver::draw_image_unscaled(Fl_Draw_Image_Cb cb, void *data, int x, int y,
                                                 int w, int h, int d) {
  if (fl_abs(d) & FL_IMAGE_WITH_ALPHA) {
    d ^= FL_IMAGE_WITH_ALPHA;
...
void Fl_GDI_Graphics_Driver::draw_image_mono_unscaled(const uchar *buf, int x, int y, int w, int h,
                                                      int d, int l) {
  if (fl_abs(d) & FL_IMAGE_WITH_ALPHA) {
    d ^= FL_IMAGE_WITH_ALPHA;
...
void Fl_GDI_Graphics_Driver::draw_image_mono_unscaled(Fl_Draw_Image_Cb cb, void *data, int x, int y,
                                                      int w, int h, int d) {
  if (fl_abs(d) & FL_IMAGE_WITH_ALPHA) {
    d ^= FL_IMAGE_WITH_ALPHA;
...
void Fl_GDI_Graphics_Driver::colored_rectf(int x, int y, int w, int h, uchar r, uchar g, uchar b) {
  ...
  if (fl_palette) {
    ...
    c[1] = g;
    c[2] = b;
    innards(c, floor(x), floor(y), floor(x + w) - floor(x), floor(y + h) - floor(y), 0, 0, 0, 0, 0,
            (HDC)gc());
    return;
  }
...
HBITMAP Fl_GDI_Graphics_Driver::create_bitmask(int w, int h, const uchar *data) {
  ...
  HBITMAP bm;
  static uchar hiNibble[16] = {0x00, 0x80, 0x40, 0xc0, 0x20, 0xa0, 0x60, 0xe0,
                               0x10, 0x90, 0x50, 0xd0, 0x30, 0xb0, 0x70, 0xf0};
  static uchar loNibble[16] = {0x00, 0x08, 0x04, 0x0c, 0x02, 0x0a, 0x06, 0x0e,
...
Fl_GDI_Printer_Graphics_Driver::transparent_f_type
Fl_GDI_Printer_Graphics_Driver::TransparentBlt() {
  HMODULE hMod;
  static transparent_f_type fpter =
      ((hMod = LoadLibrary("MSIMG32.DLL"))
           ? (transparent_f_type)GetProcAddress(hMod, "TransparentBlt")
           : NULL);
  return fpter;
}

void Fl_GDI_Printer_Graphics_Driver::draw_bitmap(Fl_Bitmap *bm, int XP, int YP, int WP, int HP,
                                                 int cx, int cy) {
  int X, Y, W, H;
  ...
  BitBlt(off_gc, 0, 0, bm->data_w(), bm->data_h(), tempdc, 0, 0,
         0xE20746L); // draw bitmap to offscreen
  Fl_Surface_Device::pop_current();
  SelectObject(
      tempdc, (HGDIOBJ)img_surf
                  ->offscreen()); // use offscreen data
  ...
  fl_TransparentBlt(gc_, X, Y, W, H, tempdc, int(cx * scaleW), int(cy * scaleH), int(W * scaleW),
                    int(H * scaleH), RGB(r, g, b));
  delete img_surf;
  RestoreDC(tempdc, save);
  DeleteDC(tempdc);
  if (recache)
...
HBITMAP Fl_GDI_Graphics_Driver::create_alphamask(int w, int h, int d, int ld, const uchar *array) {
  ...
  const uchar *dataptr;
  int x, y;
  static uchar dither[16][16] = {
      // Simple 16x16 Floyd dither
...
void Fl_GDI_Graphics_Driver::cache(Fl_RGB_Image *img) {
  ...
  if ((img->d() == 2 || img->d() == 4) && fl_can_do_alpha_blending()) {
    fl_draw_image(img->array, 0, 0, img->data_w(), img->data_h(), img->d() | FL_IMAGE_WITH_ALPHA,
                  img->ld());
  } else {
    fl_draw_image(img->array, 0, 0, img->data_w(), img->data_h(), img->d(), img->ld());
    if (img->d() == 2 || img->d() == 4) {
      *Fl_Graphics_Driver::mask(img) = (fl_uintptr_t)create_alphamask(
          img->data_w(), img->data_h(), img->d(), img->ld(), img->array);
    }
  }
  Fl_Surface_Device::pop_current();
...
void Fl_GDI_Graphics_Driver::draw_fixed(Fl_RGB_Image *img, int X, int Y, int W, int H, int cx,
                                        int cy) {
  X = this->floor(X);
  Y = this->floor(Y);
  cache_size(img, W, H);
...
void Fl_GDI_Graphics_Driver::draw_rgb(Fl_RGB_Image *rgb, int XP, int YP, int WP, int HP, int cx,
                                      int cy) {
  ...
  if ((rgb->d() % 2) == 0) {
    alpha_blend_(this->floor(XP), this->floor(YP), WP, HP, new_gc, 0, 0, rgb->data_w(),
                 rgb->data_h());
  } else {
    SetStretchBltMode(gc_, HALFTONE);
    StretchBlt(gc_, this->floor(XP), this->floor(YP), WP, HP, new_gc, 0, 0, rgb->data_w(),
               rgb->data_h(), SRCCOPY);
  }
  RestoreDC(new_gc, save);
  DeleteDC(new_gc);
...
void Fl_GDI_Printer_Graphics_Driver::draw_rgb(Fl_RGB_Image *rgb, int XP, int YP, int WP, int HP,
                                              int cx, int cy) {
  if (Fl_Graphics_Driver::start_image(rgb, XP, YP, WP, HP, cx, cy, XP, YP, WP, HP)) {
    return;
  }
  ...
  if (*id(rgb)) {
    int *pw, *ph;
    cache_w_h(rgb, pw, ph);
    if (*pw != rgb->data_w() || *ph != rgb->data_h())
      rgb->uncache();
  }
  if (!*id(rgb))
    cache(rgb);
...
static HBITMAP fl_create_bitmap(int w, int h, const uchar *data) {
  ...
  for (int y = 0; y < h; y++) {
    for (int n = 0; n < w1; n++, src++)
      *dest++ = (uchar)((reverse[*src & 0x0f] & 0xf0) | (reverse[(*src >> 4) & 0x0f] & 0x0f));
    dest += w2 - w1;
  }
...
void Fl_GDI_Graphics_Driver::cache(Fl_Bitmap *bm) {
  ...
  *pw = bm->data_w();
  *ph = bm->data_h();
  *Fl_Graphics_Driver::id(bm) =
      (fl_uintptr_t)fl_create_bitmap(bm->data_w(), bm->data_h(), bm->array);
}

void Fl_GDI_Graphics_Driver::draw_fixed(Fl_Pixmap *pxm, int X, int Y, int W, int H, int cx,
                                        int cy) {
  X = this->floor(X);
  Y = this->floor(Y);
  cache_size(pxm, W, H);
 ...
 */
void Fl_GDI_Printer_Graphics_Driver::draw_pixmap(Fl_Pixmap *pxm, int XP, int YP, int WP, int HP,
                                                 int cx, int cy) {
  int X, Y, W, H;
  ...
  if (fl_TransparentBlt) {
    ...
    fl_TransparentBlt(gc_, X, Y, W, H, new_gc, int(cx * scaleW), int(cy * scaleH), int(W * scaleW),
                      int(H * scaleH), need_pixmap_bg_color);
    RestoreDC(new_gc, save);
    DeleteDC(new_gc);
...
void Fl_GDI_Graphics_Driver::make_unused_color_(uchar &r, uchar &g, uchar &b, int color_count,
                                                void **data) {
  typedef struct {
    uchar r;
    uchar g;
  ...
  while (1) {
    for (i = 0; i < color_count; i++)
      if (used_colors[i].r == r && used_colors[i].g == g && used_colors[i].b == b)
        break;
    if (i >= color_count) {
