...
void Fl_GDI_Graphics_Driver::focus_rect(int x, int y, int w, int h) {
  ...
  for (xx = 0; xx < w; xx++, i++)
    if (i & 1)
      SetPixel(gc_, x + xx, y, c);
  for (yy = 0; yy < h; yy++, i++)
    if (i & 1)
      SetPixel(gc_, x + w, y + yy, c);
  for (xx = w; xx > 0; xx--, i++)
    if (i & 1)
      SetPixel(gc_, x + xx, y + h, c);
  for (yy = h; yy > 0; yy--, i++)
    if (i & 1)
      SetPixel(gc_, x, y + yy, c);
}
...
void *Fl_GDI_Graphics_Driver::change_pen_width(
    int width) { // set the width of the pen, return previous pen
...
void Fl_GDI_Graphics_Driver::loop_unscaled(int x, int y, int x1, int y1, int x2, int y2, int x3,
                                           int y3) {
  MoveToEx(gc_, x, y, 0L);
  LineTo(gc_, x1, y1);
  LineTo(gc_, x2, y2);
...
void Fl_GDI_Graphics_Driver::polygon_unscaled(int x, int y, int x1, int y1, int x2, int y2, int x3,
                                              int y3) {
  POINT p[4];
  p[0].x = x;
  p[0].y = y;
...
void Fl_GDI_Graphics_Driver::push_clip(int x, int y, int w, int h) {
  ...
  if (rstackptr < region_stack_max)
    ...
    Fl::warning("Fl_GDI_Graphics_Driver::push_clip: clip stack overflow!\n");
  fl_restore_clip();
}
...
int Fl_GDI_Graphics_Driver::clip_box(int x, int y, int w, int h, int &X, int &Y, int &W, int &H) {
  ...
  if (CombineRgn(temp, rr, r, RGN_AND) == NULLREGION) { // disjoint
    ...
    if (Fl_Surface_Device::surface() !=
        Fl_Display_Device::display_device()) { // if print context, convert coords from device to
                                               // logical
...
int Fl_GDI_Graphics_Driver::not_clipped(int x, int y, int w, int h) {
  ...
  RECT rect;
  if (Fl_Surface_Device::surface() !=
      Fl_Display_Device::display_device()) { // in case of print context, convert coords from
                                             // logical to device
...
void Fl_GDIplus_Graphics_Driver::line(int x, int y, int x1, int y1) {
  if (!active)
    return Fl_Scalable_Graphics_Driver::line(x, y, x1, y1);
  bool AA = !(x == x1 || y == y1);
...
void Fl_GDIplus_Graphics_Driver::line(int x, int y, int x1, int y1, int x2, int y2) {
  if (!active)
    return Fl_Scalable_Graphics_Driver::line(x, y, x1, y1, x2, y2);
  line(x, y, x1, y1);
  line(x1, y1, x2, y2);
}
...
void Fl_GDIplus_Graphics_Driver::loop(int x0, int y0, int x1, int y1, int x2, int y2) {
  if (!active)
    return Fl_Scalable_Graphics_Driver::loop(x0, y0, x1, y1, x2, y2);
  Gdiplus::GraphicsPath path;
  Gdiplus::Point gdi2_p[3] = {Gdiplus::Point(x0, y0), Gdiplus::Point(x1, y1),
                              Gdiplus::Point(x2, y2)};
...
void Fl_GDIplus_Graphics_Driver::loop(int x0, int y0, int x1, int y1, int x2, int y2, int x3,
                                      int y3) {
  if ((x0 == x3 && x1 == x2 && y0 == y1 && y3 == y2) ||
    ...
    if (!active)
      return Fl_Scalable_Graphics_Driver::loop(x0, y0, x1, y1, x2, y2, x3, y3);
    Gdiplus::GraphicsPath path;
    Gdiplus::PointF gdi2_p[4] = {
        Gdiplus::PointF(x0 + 1 - line_width_ / 2.f, y0 + 1 - line_width_ / 2.f),
        Gdiplus::PointF(x1 + 1 - line_width_ / 2.f, y1 + 1 - line_width_ / 2.f),
        Gdiplus::PointF(x2 + 1 - line_width_ / 2.f, y2 + 1 - line_width_ / 2.f),
        Gdiplus::PointF(x3 + 1 - line_width_ / 2.f, y3 + 1 - line_width_ / 2.f)};
...
void Fl_GDIplus_Graphics_Driver::polygon(int x0, int y0, int x1, int y1, int x2, int y2) {
  if (!active)
    return Fl_Scalable_Graphics_Driver::polygon(x0, y0, x1, y1, x2, y2);
  Gdiplus::GraphicsPath path;
...
void Fl_GDIplus_Graphics_Driver::polygon(int x0, int y0, int x1, int y1, int x2, int y2, int x3,
                                         int y3) {
  if ((x0 == x3 && x1 == x2 && y0 == y1 && y3 == y2) ||
    ...
    if (!active)
      return Fl_Scalable_Graphics_Driver::polygon(x0, y0, x1, y1, x2, y2, x3, y3);
    Gdiplus::GraphicsPath path;
    path.AddLine(x0, y0, x1, y1);
    path.AddLine(x1, y1, x2, y2);
