...
static void clear_xmap(Fl_XMap &xmap) {
  if (xmap.pen) {
    ...
    if (oldpen != xmap.pen)
      SelectObject(gc, oldpen); // Put it back if it is not the one we are about to delete
...
static void set_xmap(Fl_XMap &xmap, COLORREF c, int lw) {
  ...
  if (xmap.pen) {
    HDC gc = (HDC)fl_graphics_driver->gc();
    HGDIOBJ oldpen =
        SelectObject(gc, GetStockObject(BLACK_PEN)); // replace current pen with safe one
    if (oldpen != xmap.pen)
      SelectObject(gc, oldpen); // if old one not xmap.pen, need to put it back
...
void Fl_GDI_Graphics_Driver::color(Fl_Color i) {
  if (i & 0xffffff00) {
    ...
    int tw = line_width_ ? line_width_ : int(scale());
    if (!tw)
      tw = 1;
