...
void Fl_GDI_Graphics_Driver::line_style_unscaled(int style, int width, char *dashes) {
  ...
  if (dashes && dashes[0]) {
    s1 |= PS_USERSTYLE;
    for (n = 0; n < 16 && *dashes; n++)
      a[n] = *dashes++;
  } else {
    s1 |= style & 0xff; // allow them to pass any low 8 bits for style
  }
  if ((style || n) && !width)
    width = int(scale()); // fix cards that do nothing for 0?
  if (!width)
    width = 1;
  if (!fl_current_xmap)
    color(FL_BLACK);
...
void Fl_GDIplus_Graphics_Driver::line_style(int style, int width, char *dashes) {
  if (!active)
    return Fl_Scalable_Graphics_Driver::line_style(style, width, dashes);
  ...
  if (dashes && *dashes) {
    ...
    for (int i = 0; i < n; i++)
      gdi_dashes[i] = dashes[i] / float(gdi_width);
