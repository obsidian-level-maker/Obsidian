...
Fl_WinAPI_Window_Driver::border_width_title_bar_height(int &bx, // left and right border width
  ...
  if (win->shown() && !win->parent() && win->border() && win->visible()) {
    ...
    typedef HRESULT(WINAPI * DwmGetWindowAttribute_type)(HWND hwnd, DWORD dwAttribute,
                                                         PVOID pvAttribute, DWORD cbAttribute);
    static DwmGetWindowAttribute_type DwmGetWindowAttribute =
        dwmapi_dll ? (DwmGetWindowAttribute_type)GetProcAddress(dwmapi_dll, "DwmGetWindowAttribute")
                   : NULL;
    int need_r = 1;
    if (DwmGetWindowAttribute) {
      const DWORD DWMWA_EXTENDED_FRAME_BOUNDS = 9;
      if (DwmGetWindowAttribute(fl_xid(win), DWMWA_EXTENDED_FRAME_BOUNDS, &r, sizeof(RECT)) ==
          S_OK) {
        need_r = 0;
      }
    }
...
void Fl_WinAPI_Window_Driver::draw_begin() {
  if (shape_data_) {
    ...
    if ((shape_data_->lw_ != s * w() || shape_data_->lh_ != s * h()) && shape_data_->shape_) {
      ...
      SetWindowRgn(fl_xid(pWindow), region,
                   TRUE); // the system deletes the region when it's no longer needed
      delete temp;
    }
  }
}
...
void Fl_WinAPI_Window_Driver::hide() {
  ...
  if (private_dc)
    fl_release_dc(ip->xid, private_dc);
...
void Fl_WinAPI_Window_Driver::fullscreen_off(int X, int Y, int W, int H) {
  ...
  float s = Fl::screen_driver()->scale(screen_num());
  int scaledX = int(ceil(X * s)), scaledY = int(ceil(Y * s)), scaledW = int(ceil(W * s)),
      scaledH = int(ceil(H * s));
...
int Fl_WinAPI_Window_Driver::scroll(int src_x, int src_y, int src_w, int src_h, int dest_x,
...
Fl_WinAPI_Window_Driver::type_for_resize_window_between_screens
    Fl_WinAPI_Window_Driver::data_for_resize_window_between_screens_ = {0, false};
...
void Fl_WinAPI_Window_Driver::resize_after_screen_change(void *data) {
  ...
  Fl_Window_Driver::driver(win)->resize_after_scale_change(ns, old_f,
                                                           Fl::screen_driver()->scale(ns));
