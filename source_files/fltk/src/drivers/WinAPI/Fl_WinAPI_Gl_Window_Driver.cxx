...
public:
  Fl_WinAPI_Gl_Choice(int m, const int *alistp, Fl_Gl_Choice *n)
...
Fl_Gl_Choice *Fl_WinAPI_Gl_Window_Driver::find(int m, const int *alistp) {
  ...
  for (int i = 1;; i++) {
    ...
    if (!DescribePixelFormat(gc, i, sizeof(pfd), &pfd))
      break;
    ...
    if (pixelformat) {
      ...
      if (!(chosen_pfd.dwFlags & PFD_GENERIC_FORMAT) && (pfd.dwFlags & PFD_GENERIC_FORMAT))
        continue;
...
GLContext Fl_WinAPI_Gl_Window_Driver::create_gl_context(Fl_Window *window, const Fl_Gl_Choice *g,
  ...
  if (!hdc) {
    ...
    SetPixelFormat(hdc, ((Fl_WinAPI_Gl_Choice *)g)->pixelformat,
                   (PIXELFORMATDESCRIPTOR *)(&((Fl_WinAPI_Gl_Choice *)g)->pfd));
#if USE_COLORMAP
    if (fl_palette)
      SelectPalette(hdc, fl_palette, FALSE);
#endif
  }
...
void Fl_WinAPI_Gl_Window_Driver::swap_buffers() {
  ...
  BOOL ret = wglSwapLayerBuffers(Fl_WinAPI_Window_Driver::driver(pWindow)->private_dc,
                                 WGL_SWAP_MAIN_PLANE);
