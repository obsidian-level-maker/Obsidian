...
public:
  Fl_Cocoa_Gl_Choice(int m, const int *alistp, Fl_Gl_Choice *n)
...
GLContext Fl_Cocoa_Gl_Window_Driver::create_gl_context(Fl_Window *window, const Fl_Gl_Choice *g,
  ...
  if (!shared_ctx)
    gl_texture_reset();
  context = Fl_Cocoa_Window_Driver::create_GLcontext_for_window(
      ((Fl_Cocoa_Gl_Choice *)g)->pixelformat, shared_ctx, window);
  if (!context)
    return 0;
  add_context(context);
  return (context);
...
float Fl_Cocoa_Gl_Window_Driver::pixels_per_unit() {
  int retina = (fl_mac_os_version >= 100700 && Fl::use_high_res_GL() && Fl_X::i(pWindow) &&
                Fl_Cocoa_Window_Driver::driver(pWindow)->mapped_to_retina())
                   ? 2
                   : 1;
...
void Fl_Cocoa_Gl_Window_Driver::swap_buffers() {
  if (overlay() != NULL) {
    ...
    Fl_Cocoa_Window_Driver::flush_context(
        pWindow->context()); // aglSwapBuffers((AGLContext)context_);
}

char Fl_Cocoa_Gl_Window_Driver::swap_type() {
  return copy;
}
...
Fl_RGB_Image *Fl_Cocoa_Gl_Window_Driver::capture_gl_rectangle(int x, int y, int w, int h) {
  ...
  Fl_Cocoa_Window_Driver::flush_context(
      glw->context()); // to capture also the overlay and for directGL demo
  ...
  glReadPixels(x, glw->pixel_h() - (y + h), w, h, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV,
               baseAddress);
  glPopClientAttrib();
