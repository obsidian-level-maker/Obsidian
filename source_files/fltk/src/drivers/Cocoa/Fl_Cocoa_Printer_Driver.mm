...
typedef OSStatus (*PMSessionSetDocumentFormatGeneration_type)(PMPrintSession printSession,
                                                              CFStringRef docFormat,
                                                              CFArrayRef graphicsContextTypes,
                                                              CFTypeRef options);
typedef OSStatus (*PMSessionBeginDocumentNoDialog_type)(PMPrintSession printSession,
                                                        PMPrintSettings printSettings,
                                                        PMPageFormat pageFormat);
typedef OSStatus (*PMSessionGetGraphicsContext_type)(PMPrintSession printSession,
...
- (void)printPanelDidEnd:(NSPrintPanel *)printPanel
             ...
             contextInfo:(NSInteger *)contextInfo;
@end
...
// returns 0 iff OK
{
  ...
  if (fl_mac_os_version >= 100500) {
    ...
    [panel setOptions:NSPrintPanelShowsCopies | NSPrintPanelShowsPageRange |
                      NSPrintPanelShowsPageSetupAccessory | NSPrintPanelShowsOrientation |
    ...
    if (topage) {
      *topage = (int)to32;
      if (*topage > pagecount && pagecount > 0)
        *topage = pagecount;
    }
...
#endif
  {
    ...
    if (!f)
      f = (dialog_f)Fl_Darwin_System_Driver::get_carbon_function("PMSessionPageSetupDialog");
    ...
    if (!f2)
      f2 = (dialog_f2)Fl_Darwin_System_Driver::get_carbon_function("PMSessionPrintDialog");
    ...
    PMSessionSetDocumentFormatGeneration_type PMSessionSetDocumentFormatGeneration =
        (PMSessionSetDocumentFormatGeneration_type)Fl_Darwin_System_Driver::get_carbon_function(
            "PMSessionSetDocumentFormatGeneration");
    status =
        PMSessionSetDocumentFormatGeneration(printSession, kPMDocumentFormatDefault, array, NULL);
    CFRelease(array);
    PMSessionBeginDocumentNoDialog_type PMSessionBeginDocumentNoDialog =
        (PMSessionBeginDocumentNoDialog_type)Fl_Darwin_System_Driver::get_carbon_function(
  ...
  if (status != noErr) {
    if (perr_message) {
      ...
      if (s)
        *perr_message = fl_strdup([s UTF8String]);
    }
    return 2;
  }
  y_offset = x_offset = 0;
...
void Fl_Cocoa_Printer_Driver::margins(int *left, int *top, int *right, int *bottom) {
  ...
  if (orientation == kPMPortrait) {
    if (left)
      *left = (int)(margins.left / scale_x + 0.5);
    if (top)
      *top = (int)(margins.top / scale_y + 0.5);
    if (right)
      *right = (int)(margins.right / scale_x + 0.5);
    if (bottom)
      *bottom = (int)(margins.bottom / scale_y + 0.5);
  } else {
    if (left)
      *left = (int)(margins.top / scale_x + 0.5);
    if (top)
      *top = (int)(margins.left / scale_y + 0.5);
    if (right)
      *right = (int)(margins.bottom / scale_x + 0.5);
    if (bottom)
      *bottom = (int)(margins.right / scale_y + 0.5);
  }
}
...
int Fl_Cocoa_Printer_Driver::begin_page(void) {
...
#endif
  {
    ...
    PMSessionGetGraphicsContext_type PMSessionGetGraphicsContext =
        (PMSessionGetGraphicsContext_type)Fl_Darwin_System_Driver::get_carbon_function(
