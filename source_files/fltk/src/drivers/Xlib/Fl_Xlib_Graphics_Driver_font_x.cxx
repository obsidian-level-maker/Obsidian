...
static int attribute(int n, const char *p) {
  ...
  if (!*p || *p == '-' || *p == '*')
    return 0;
  if (n == 3) { // weight
    if (!strncmp(p, "normal", 6) || !strncmp(p, "light", 5) || !strncmp(p, "medium", 6) ||
        !strncmp(p, "book", 4))
      return 0;
    if (!strncmp(p, "bold", 4) || !strncmp(p, "demi", 4))
      return FL_BOLD;
  } else if (n == 4) { // slant
    if (*p == 'r')
      return 0;
...
static int to_canonical(char *to, const char *from, size_t tolen) {
  ...
  if (from[0] == '-') {
    ...
    *to++ = '*';
    ...
    if (*endptr && !use_registry(endptr + 1))
      endptr = "";
  }
  int n = c - from;
  // MRS: we want strncpy here, not strlcpy...
  ...
  for (int i = 0; i < xlistsize;) {
    ...
    if (size >= 0) {
      for (;;) { // find all matching fonts:
        ...
        if (to_canonical(this_canon, q, sizeof(this_canon)) < 0)
          break;
        if (strcmp(canon, this_canon))
          break;
        i++;
      }
...
int Fl_Xlib_Graphics_Driver::get_font_sizes(Fl_Font fnum, int *&sizep) {
  ...
  if (!s->name)
    s = ((Fl_Xlib_Fontdesc *)fl_fonts); // empty slot in table, use entry 0
  if (!s->xlist) {
    fl_open_display();
  ...
  for (int i = 0; i < listsize; i++) {
    ...
    if (!numsizes || sizes[numsizes - 1] < s) {
      ...
      int n;
      for (n = numsizes - 1; n > 0; n--)
        if (sizes[n - 1] < s)
          break;
      if (sizes[n] != s) {
        for (int m = numsizes; m > n; m--)
          sizes[m] = sizes[m - 1];
        sizes[n] = s;
        numsizes++;
      }
...
Fl_Xlib_Font_Descriptor::Fl_Xlib_Font_Descriptor(const char *name)
  : Fl_Font_Descriptor(name, 0) {
...
char *fl_get_font_xfld(int fnum, int size) {
  Fl_Xlib_Fontdesc *s = ((Fl_Xlib_Fontdesc *)fl_fonts) + fnum;
  if (!s->name)
    s = (Fl_Xlib_Fontdesc *)fl_fonts; // use font 0 if still undefined
  fl_open_display();
...
void Fl_Xlib_Graphics_Driver::font_unscaled(Fl_Font fnum, Fl_Fontsize size) {
  ...
  if (fnum == Fl_Graphics_Driver::font() && size == Fl_Graphics_Driver::size())
    return;
...
double Fl_Xlib_Graphics_Driver::width_unscaled(const char *c, int n) {
  if (font_descriptor())
    return (double)XUtf8TextWidth(((Fl_Xlib_Font_Descriptor *)font_descriptor())->font, c, n);
  else
    return -1;
}
...
double Fl_Xlib_Graphics_Driver::width_unscaled(unsigned int c) {
  if (font_descriptor())
    return (double)XUtf8UcsWidth(((Fl_Xlib_Font_Descriptor *)font_descriptor())->font, c);
  else
    return -1;
}
...
void Fl_Xlib_Graphics_Driver::text_extents_unscaled(const char *c, int n, int &dx, int &dy, int &W,
  ...
  xx = yy = ww = hh = 0;
  if (gc_)
    XUtf8_measure_extents(fl_display, fl_window,
                          ((Fl_Xlib_Font_Descriptor *)font_descriptor())->font, gc_, &xx, &yy, &ww,
                          &hh, c, n);
...
void Fl_Xlib_Graphics_Driver::draw_unscaled(const char *c, int n, int x, int y) {
  ...
  if (gc_)
    XUtf8DrawString(fl_display, fl_window, ((Fl_Xlib_Font_Descriptor *)font_descriptor())->font,
                    gc_, x1, y1, c, n);
}
...
void Fl_Xlib_Graphics_Driver::draw_unscaled(int angle, const char *str, int n, int x, int y) {
  ...
  if (!warning && angle != 0) {
    warning = 1;
    fprintf(stderr, "libfltk: rotated text not implemented by X backend.\n"
...
void Fl_Xlib_Graphics_Driver::rtl_draw_unscaled(const char *c, int n, int x, int y) {
  ...
  if (gc_)
    XUtf8DrawRtlString(fl_display, fl_window, ((Fl_Xlib_Font_Descriptor *)font_descriptor())->font,
                       gc_, x1, y1, c, n);
}
