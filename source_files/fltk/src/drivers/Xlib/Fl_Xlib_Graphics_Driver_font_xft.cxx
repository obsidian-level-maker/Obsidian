...
static void make_raw_name(char *raw, char *pretty) {
  ...
  if (nm2)
    *nm2 = 0; // terminate name after first entry
  raw[0] = ' ';
...
Fl_Font Fl_Xlib_Graphics_Driver::set_fonts(const char *pattern_name) {
  ...
  if (fnt_set) {
    ...
    for (j = 0; j < font_count; j++) {
      ...
      if (first == (char *)font) { // The listed name is still OK
        full_list[j] = (char *)font;
      } else { // The listed name has been modified
...
static XftFont *fontopen(const char *name, /*Fl_Fontsize*/ double size, bool core, int angle) {
  ...
  for (unsigned idx = 0; idx < len; idx++) {
    if (name[idx] == '-')
      hyphen_count++; // check for XLFD hyphens
    if (name[idx] == ',')
      comma_count++; // are there multiple names?
  }
  if (hyphen_count >= 14)
  ...
  if (!is_xlfd) {                             // Not an XLFD - open as a XFT style name
    ...
    if (comma_count) {                    // multiple comma-separated names were passed
      ...
      do {
        ...
        if (nxt)
          curr = nxt; // move onto next name (if it exists)
    ...
    if (!match_pat || !the_font) {
      ...
      the_font = XftFontOpen(fl_display, fl_screen, XFT_FAMILY, XftTypeString, "sans", XFT_SIZE,
...
Fl_Xlib_Font_Descriptor::Fl_Xlib_Font_Descriptor(const char *name, Fl_Fontsize fsize, int fangle)
  : Fl_Font_Descriptor(name, fsize) {
...
static void utf8extents(Fl_Xlib_Font_Descriptor *desc, const char *str, int n,
                        XGlyphInfo *extents) {
...
int Fl_Xlib_Graphics_Driver::height_unscaled() {
  if (font_descriptor())
    return ((Fl_Xlib_Font_Descriptor *)font_descriptor())->font->ascent +
           ((Fl_Xlib_Font_Descriptor *)font_descriptor())->font->descent;
  else
    return -1;
}
...
int Fl_Xlib_Graphics_Driver::descent_unscaled() {
  if (font_descriptor())
    return ((Fl_Xlib_Font_Descriptor *)font_descriptor())->font->descent;
  else
    return -1;
}
...
void Fl_Xlib_Graphics_Driver::text_extents_unscaled(const char *c, int n, int &dx, int &dy, int &w,
                                                    int &h) {
  if (!font_descriptor()) {
    w = h = 0;
    dx = dy = 0;
...
void Fl_Xlib_Graphics_Driver::draw_unscaled(const char *str, int n, int x, int y) {
  ...
  if (x1 < clip_min() || x1 > clip_max())
    return;

  int y1 = y + floor(offset_y_);
  if (y1 < clip_min() || y1 > clip_max())
    return;
  ...
  if (!(region && XEmptyRegion(region))) {
    ...
    const wchar_t *buffer = utf8reformat(str, n);
#ifdef __CYGWIN__
    XftDrawString16(draw_, &color, ((Fl_Xlib_Font_Descriptor *)font_descriptor())->font, x1, y1,
                    (XftChar16 *)buffer, n);
#else
    XftDrawString32(draw_, &color, ((Fl_Xlib_Font_Descriptor *)font_descriptor())->font, x1, y1,
                    (XftChar32 *)buffer, n);
#endif
  }
}
...
void Fl_Xlib_Graphics_Driver::drawUCS4(const void *str, int n, int x, int y) {
  ...
  uchar r, g, b;
  Fl::get_color(this->color(), r, g, b);
  color.color.red = ((int)r) * 0x101;
  color.color.green = ((int)g) * 0x101;
  color.color.blue = ((int)b) * 0x101;
  color.color.alpha = 0xffff;

  XftDrawString32(draw_, &color, ((Fl_Xlib_Font_Descriptor *)font_descriptor())->font,
                  x + floor(offset_x_), y + floor(offset_y_), (FcChar32 *)str, n);
}
...
int Fl_Xlib_Graphics_Driver::get_font_sizes(Fl_Font fnum, int *&sizep) {
  ...
  if (!s->name)
    s = fl_fonts; // empty slot in table, use entry 0

  fl_open_display();
  XftFontSet *fs = XftListFonts(fl_display, fl_screen, XFT_FAMILY, XftTypeString, s->name + 1,
                                (void *)0, XFT_PIXEL_SIZE, (void *)0);
  static int *array = 0;
...
const char *Fl_Xlib_Graphics_Driver::get_font_name(Fl_Font fnum, int *ap) {
  ...
  if (!f->fontname[0]) {
    ...
    if (strstr(p, " Bold"))
      type = FL_BOLD;
    if (strstr(p, " Italic") || strstr(p, " Oblique"))
      type += FL_ITALIC;
...
static void fl_xft_font(Fl_Xlib_Graphics_Driver *driver, Fl_Font fnum, Fl_Fontsize size,
                        int angle) {
  ...
  if (fnum == driver->Fl_Graphics_Driver::font() && size == driver->size_unscaled() && f &&
      f->angle == angle)
    return;
...
static XFontStruct *load_xfont_for_xft2(Fl_Graphics_Driver *driver) {
  ...
  if (!xgl_font)
    xgl_font = XLoadQueryFont(fl_display, "fixed");
...
static XFontStruct *fl_xxfont(Fl_Graphics_Driver *driver) {
  ...
  if ((!xgl_font) || (glsize != ((Fl_Xlib_Graphics_Driver *)driver)->size_unscaled()) ||
      (glfont != driver->font())) {
    // create a dummy XLFD for some font of the appropriate size...
    if (xgl_font)
      XFreeFont(fl_display, xgl_font); // font already loaded, free it - this *might* be a Bad Idea
  ...
  if (driver->font_descriptor()->font->core) {
    return driver->font_descriptor()
        ->font->u.core.font; // is the current font a "core" font? If so, use it.
  }
  static XftFont *xftfont;
  if (xftfont)
    XftFontClose(fl_display, xftfont);
  xftfont =
      fontopen(fl_fonts[driver->font()].name, ((Fl_Xlib_Graphics_Driver *)driver)->size_unscaled(),
               true, 0); // else request XFT to load a suitable "core" font instead.
...
XFontStruct *Fl_XFont_On_Demand::value() {
  if (!ptr)
    ptr = fl_xxfont(fl_graphics_driver);
  return ptr;
}
...
void Fl_Xlib_Graphics_Driver::font_unscaled(Fl_Font fnum, Fl_Fontsize size) {
  ...
  if (this->Fl_Graphics_Driver::font() == fnum && this->size_unscaled() == size &&
  ...
  if (fnum >= pfd_array_length) {
    ...
    PangoFontDescription **data =
        (PangoFontDescription **)calloc(new_length, sizeof(PangoFontDescription *));
  ...
  if (!fd->height_) {
    ...
    if (fd->height_ == 0) { // this occurs with some fonts
      PangoFontMetrics *pfmt = pango_font_get_metrics(
          pfont, pango_script_get_sample_language(PANGO_SCRIPT_LATIN) /* 1.4 */);
      fd->descent_ = pango_font_metrics_get_descent(pfmt) / PANGO_SCALE;
      fd->height_ = (pango_font_metrics_get_ascent(pfmt) + pango_font_metrics_get_descent(pfmt)) /
                    PANGO_SCALE;
 ...
 */
static void fl_pango_layout_get_pixel_extents(PangoLayout *layout, int &dx, int &dy, int &w, int &h,
                                              int desc, int lheight, int &y_correction) {
...
void Fl_Xlib_Graphics_Driver::do_draw(int from_right, const char *str, int n, int x, int y) {
  ...
  const char *old = 0;
  if (!str2)
    old = pango_layout_get_text(playout_);
  if (!old || (int)strlen(old) != n ||
      memcmp(str, old, n)) // do not re-set text if equal to text already in layout
    pango_layout_set_text(playout_, str, n);
  if (str2)
    free(str2);
...
void Fl_Xlib_Graphics_Driver::text_extents_unscaled(const char *str, int n, int &dx, int &dy,
                                                    int &w, int &h) {
  if (!playout_)
    context();
  pango_layout_set_font_description(playout_, pfd_array[font_]);
  pango_layout_set_text(playout_, str, n);
  int y_correction;
  fl_pango_layout_get_pixel_extents(playout_, dx, dy, w, h, descent_unscaled(), height_unscaled(),
                                    y_correction);
  dy -= y_correction;
  correct_extents(scale(), dx, dy, w, h);
}
...
int Fl_Xlib_Graphics_Driver::height_unscaled() {
  if (font_descriptor())
    return ((Fl_Xlib_Font_Descriptor *)font_descriptor())->height_;
  else
    return -1;
}
...
int Fl_Xlib_Graphics_Driver::descent_unscaled() {
  if (font_descriptor())
    return ((Fl_Xlib_Font_Descriptor *)font_descriptor())->descent_;
  else
    return -1;
}
...
static int font_name_process(const char *name, char &face) {
  int l = strlen(name);
  face = ' ';
  if (l > 8 && !memcmp(name + l - 8, " Regular", 8))
    l -= 8;
  else if (l > 6 && !memcmp(name + l - 6, " Plain", 6))
    l -= 6;
  else if (l > 12 && !memcmp(name + l - 12, " Bold Italic", 12)) {
    l -= 12;
    face = 'P';
  } else if (l > 7 && !memcmp(name + l - 7, " Italic", 7)) {
    l -= 7;
    face = 'I';
  } else if (l > 5 && !memcmp(name + l - 5, " Bold", 5)) {
    l -= 5;
    face = 'B';
  }
  return l;
