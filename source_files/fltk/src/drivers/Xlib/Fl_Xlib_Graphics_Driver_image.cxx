...
static void color16_converter(const uchar *from, uchar *to, int w, int delta) {
  ...
  for (; w--; from += d, t += td) {
    ...
    if (b > 255)
      b = 255;
    OUTASSIGN((((r & fl_redmask) << fl_redshift) + ((g & fl_greenmask) << fl_greenshift) +
...
static void mono16_converter(const uchar *from, uchar *to, int w, int delta) {
  ...
  for (; w--; from += d, t += td) {
    ...
    if (r > 255)
      r = 255;
    uchar m = r & mask;
    OUTASSIGN(((m << fl_redshift) + (m << fl_greenshift) + (m << fl_blueshift)) >> fl_extrashift);
  }
  ri = r;
}
...
static void argb_premul_converter(const uchar *from, uchar *to, int w, int delta) {
  INNARDS32((unsigned(from[3]) << 24) + (((from[0] * from[3]) / 255) << 16) +
            (((from[1] * from[3]) / 255) << 8) + ((from[2] * from[3]) / 255));
}

static void depth2_to_argb_premul_converter(const uchar *from, uchar *to, int w, int delta) {
  INNARDS32((unsigned(from[1]) << 24) + (((from[0] * from[1]) / 255) << 16) +
            (((from[0] * from[1]) / 255) << 8) + ((from[0] * from[1]) / 255));
}
...
static void color32_converter(const uchar *from, uchar *to, int w, int delta) {
  INNARDS32((from[0] << fl_redshift) + (from[1] << fl_greenshift) + (from[2] << fl_blueshift));
}

static void mono32_converter(const uchar *from, uchar *to, int w, int delta) {
  INNARDS32((*from << fl_redshift) + (*from << fl_greenshift) + (*from << fl_blueshift));
}
...
static void figure_out_visual() {
  ...
  if (!pfvlist)
    pfvlist = XListPixmapFormats(fl_display, &FL_NUM_pfv);
  ...
  if (xi.bits_per_pixel & 7)
    bytes_per_pixel = 0; // produce fatal error
  else
...
static void innards(const uchar *buf, int X, int Y, int W, int H, int delta, int linedelta,
                    int mono, Fl_Draw_Image_Cb cb, void *userdata, const bool alpha, GC gc) {
  if (!linedelta)
...
void Fl_Xlib_Graphics_Driver::draw_image_unscaled(const uchar *buf, int x, int y, int w, int h,
                                                  int d, int l) {
...
void Fl_Xlib_Graphics_Driver::draw_image_unscaled(Fl_Draw_Image_Cb cb, void *data, int x, int y,
                                                  int w, int h, int d) {
...
void Fl_Xlib_Graphics_Driver::draw_image_mono_unscaled(const uchar *buf, int x, int y, int w, int h,
                                                       int d, int l) {
  innards(buf, x + floor(offset_x_), y + floor(offset_y_), w, h, d, l, 1, 0, 0, 0, gc_);
}

void Fl_Xlib_Graphics_Driver::draw_image_mono_unscaled(Fl_Draw_Image_Cb cb, void *data, int x,
...
unsigned long Fl_Xlib_Graphics_Driver::create_bitmask(int w, int h, const uchar *data) {
  return XCreateBitmapFromData(fl_display, fl_window, (const char *)data, (w + 7) & -8, h);
}
...
void Fl_Xlib_Graphics_Driver::draw_fixed(Fl_Bitmap *bm, int X, int Y, int W, int H, int cx,
                                         int cy) {
...
static void alpha_blend(Fl_RGB_Image *img, int X, int Y, int W, int H, int cx, int cy) {
  ...
  XGetGeometry(fl_display, fl_window, &root_return, &x_return, &y_return, &winW, &winH,
...
void Fl_Xlib_Graphics_Driver::cache(Fl_RGB_Image *img) {
  ...
  if (depth == 1 || depth == 3) {
    ...
    Fl_Offscreen pixmap = XCreatePixmap(fl_display, RootWindow(fl_display, fl_screen),
                                        img->data_w(), img->data_h(), 32);
...
void Fl_Xlib_Graphics_Driver::draw_fixed(Fl_RGB_Image *img, int X, int Y, int W, int H, int cx,
                                         int cy) {
  X = floor(X) + floor(offset_x_);
  Y = floor(Y) + floor(offset_y_);
  cache_size(img, W, H);
...
void Fl_Xlib_Graphics_Driver::draw_rgb(Fl_RGB_Image *rgb, int XP, int YP, int WP, int HP, int cx,
                                       int cy) {
  ...
  scale_and_render_pixmap(*Fl_Graphics_Driver::id(rgb), rgb->d(), rgb->data_w() / double(Wfull),
                          rgb->data_h() / double(Hfull), Xs + this->floor(offset_x_) - offset,
                          Ys + this->floor(offset_y_) - offset, Wfull, Hfull);
  if (need_clip)
    pop_clip();
}

/* Draws with Xrender an Fl_Offscreen with optional scaling and accounting for transparency if
 necessary. XP,YP,WP,HP are in drawing units
 */
int Fl_Xlib_Graphics_Driver::scale_and_render_pixmap(Fl_Offscreen pixmap, int depth, double scale_x,
                                                     double scale_y, int XP, int YP, int WP,
                                                     int HP) {
  ...
  if (scale_x != 1 || scale_y != 1) {
    XTransform mat = {{{XDoubleToFixed(scale_x), XDoubleToFixed(0), XDoubleToFixed(0)},
  ...
  XRenderComposite(fl_display, (has_alpha ? PictOpOver : PictOpSrc), src, None, dst, 0, 0, 0, 0, XP,
                   YP, WP, HP);
...
void Fl_Xlib_Graphics_Driver::draw_fixed(Fl_Pixmap *pxm, int X, int Y, int W, int H, int cx,
                                         int cy) {
  X = floor(X) + floor(offset_x_);
  Y = floor(Y) + floor(offset_y_);
  cache_size(pxm, W, H);
  ...
  if (*Fl_Graphics_Driver::mask(pxm)) {
    ...
    if (clip_region()) {
      ...
      for (int i = 0; i < r->numRects; i++) {
        ...
        XCopyArea(fl_display, *Fl_Graphics_Driver::id(pxm), fl_window, gc_, cx + (X1 - X),
                  cy + (Y1 - Y), W1, H1, X1, Y1);
      }
      XDestroyRegion(r);
    } else {
...
void Fl_Xlib_Graphics_Driver::cache(Fl_Pixmap *pxm) {
  ...
  *pbitmap = (uchar *)1; // will instruct fl_draw_pixmap() to compute the image's mask
  ...
  if (bitmap) {
    *Fl_Graphics_Driver::mask(pxm) =
        (fl_uintptr_t)create_bitmask(pxm->data_w(), pxm->data_h(), bitmap);
    delete[] bitmap;
  }
  *pbitmap = 0;
  Fl_Surface_Device::pop_current();
