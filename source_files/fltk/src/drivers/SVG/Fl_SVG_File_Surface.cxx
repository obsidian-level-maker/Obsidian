//
// Implementation of classes Fl_SVG_Graphics_Driver and Fl_SVG_File_Surface in the Fast Light Tool
// Kit (FLTK).
//
...
void Fl_SVG_Graphics_Driver::rectf(int x, int y, int w, int h) {
  fprintf(out_,
          "<rect x=\"%.3f\" y=\"%.3f\" width=\"%d\" height=\"%d\" "
          "fill=\"rgb(%u,%u,%u)\" />\n",
...
void Fl_SVG_Graphics_Driver::line(int x1, int y1, int x2, int y2) {
  fprintf(out_,
          ...
          "style=\"stroke:rgb(%u,%u,%u);stroke-width:%d;stroke-linecap:%s;stroke-linejoin:%s;"
          "stroke-dasharray:%s\" />\n",
...
void Fl_SVG_Graphics_Driver::line(int x1, int y1, int x2, int y2, int x3, int y3) {
  fprintf(out_,
          ...
          "style=\"stroke:rgb(%u,%u,%u);fill:none;stroke-width:%d;stroke-linecap:%s;stroke-"
          "linejoin:%s;stroke-dasharray:%s\" />\n",
...
Fl_Font Fl_SVG_Graphics_Driver::font() {
  return Fl_Graphics_Driver::font();
}

void Fl_SVG_Graphics_Driver::compute_dasharray(float s, char *dashes) {
  if (user_dash_array_ && user_dash_array_ != dashes) {
    free(user_dash_array_);
    user_dash_array_ = NULL;
  }
  if (dashes && *dashes) {
    if (dasharray_)
      free(dasharray_);
    ...
    if (user_dash_array_ != dashes)
      user_dash_array_ = fl_strdup(dashes);
    return;
  }
  ...
  if (dash_part == FL_SOLID) {
    ...
    if (dash_part == FL_DOT)
      sprintf(dasharray_, "%.3f,%.3f", dot, gap);
    else if (dash_part == FL_DASH)
      sprintf(dasharray_, "%.3f,%.3f", big, gap);
    else if (dash_part == FL_DASHDOT)
      sprintf(dasharray_, "%.3f,%.3f,%.3f,%.3f", big, gap, dot, gap);
    else
      sprintf(dasharray_, "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f", big, gap, dot, gap, dot, gap);
  }
}
...
void Fl_SVG_Graphics_Driver::draw(const char *str, int n, int x, int y) {
  ...
  // work-around: replace all spaces by no-break space = U+00A0 = 0xC2-0xA0 (UTF-8) before sending
  // to IE
  fprintf(out_,
          "<text x=\"%d\" y=\"%d\" font-family=\"%s\"%s%s font-size=\"%d\" "
          "xml:space=\"preserve\" "
          " fill=\"rgb(%u,%u,%u)\" textLength=\"%d\">",
          x, y, family_, bold_, style_, size(), red_, green_, blue_, (int)width(str, n));
  for (int i = 0; i < n; i++) {
    if (str[i] == '&')
      fputs("&amp;", out_);
    else if (str[i] == '<')
...
Fl_Color Fl_SVG_Graphics_Driver::color() {
...
Fl_SVG_File_Surface::Fl_SVG_File_Surface(int w, int h, FILE *f, int (*closef)(FILE *))
  : Fl_Widget_Surface(new Fl_SVG_Graphics_Driver(f)) {
  closef_ = closef;
...
static void user_write_data(png_structp png_ptr, png_bytep data, png_size_t length) {
  ...
  if (svg_base64_data->lbuf == 1 && length >= 2) {
    svg_base64_data->buff[1] = *data++;
    length--;
    svg_base64_data->buff[2] = *data++;
    length--;
...
static void user_flush_data(png_structp png_ptr) {
  svg_base64_t *svg_base64_data = (svg_base64_t *)png_get_io_ptr(png_ptr);
  if (svg_base64_data->lbuf)
    to_base64(svg_base64_data->buff, svg_base64_data->lbuf, svg_base64_data);
}
...
void Fl_SVG_Graphics_Driver::define_rgb_png(Fl_RGB_Image *rgb, const char *name, int x, int y) {
  ...
  float f = rgb->data_w() > rgb->data_h() ? float(rgb->w()) / rgb->data_w()
                                          : float(rgb->h()) / rgb->data_h();
  if (name)
    fprintf(out_, "<defs><image id=\"%s\" ", name);
  else
    fprintf(out_, "<image x=\"%d\" y=\"%d\" ", x, y);
  ...
  png_set_IHDR(png_ptr, info_ptr, rgb->data_w(), rgb->data_h(), 8, color_type, PNG_INTERLACE_NONE,
               PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
  const uchar **row_pointers = new const uchar *[rgb->data_h()];
  int ld = rgb->ld() ? rgb->ld() : rgb->d() * rgb->data_w();
  for (int i = 0; i < rgb->data_h(); i++)
    row_pointers[i] = (rgb->array + i * ld);
...
static size_t process_jpeg_chunk(jpeg_compress_struct *cinfo, size_t length) {
  ...
  if (length >= 3) {
    ...
    if (new_l)
      memmove(client_data->JPEG_BUFFER, data + length - new_l, new_l);
  }
...
void Fl_SVG_Graphics_Driver::draw_rgb(Fl_RGB_Image *rgb, int XP, int YP, int WP, int HP, int cx,
                                      int cy) {
#if defined(HAVE_LIBPNG)
  char name[24];
  ...
  if (!p || !last_rgb_name_ || strcmp(name, last_rgb_name_) != 0) {
    ...
    if (rgb->d() == 3 || rgb->d() == 1)
      define_rgb_jpeg(rgb, *name ? name : NULL, XP - cx, YP - cy);
    else
#endif // HAVE_LIBJPEG
...
void Fl_SVG_Graphics_Driver::draw_pixmap(Fl_Pixmap *pxm, int XP, int YP, int WP, int HP, int cx,
                                         int cy) {
#if defined(HAVE_LIBPNG)
  char name[24];
...
void Fl_SVG_Graphics_Driver::draw_bitmap(Fl_Bitmap *bm, int XP, int YP, int WP, int HP, int cx,
                                         int cy) {
#if defined(HAVE_LIBPNG)
  char name[45];
  ...
  if (p)
    sprintf(name, "FLbm%p%X", p, fl_color());
  else
    name[0] = 0;
  if (!p || !last_rgb_name_ || strcmp(name, last_rgb_name_) != 0) {
    uchar R, G, B;
...
void Fl_SVG_Graphics_Driver::draw_image(const uchar *buf, int x, int y, int w, int h, int d,
                                        int l) {
  if (d < 0) {
...
void Fl_SVG_Graphics_Driver::draw_image(Fl_Draw_Image_Cb cb, void *data, int x, int y, int w, int h,
                                        int d) {
...
void Fl_SVG_Graphics_Driver::draw_image_mono(const uchar *buf, int x, int y, int w, int h, int d,
                                             int l) {
...
void Fl_SVG_Graphics_Driver::draw_image_mono(Fl_Draw_Image_Cb cb, void *data, int x, int y, int w,
                                             int h, int d) {
  uchar *buf = new uchar[w * h * d];
  for (int j = 0; j < h; j++) {
    cb(data, 0, j, w, buf + j * w * d);
...
void Fl_SVG_Graphics_Driver::push_clip(int x, int y, int w, int h) {
  ...
  clip_ = c;
  fprintf(out_,
          "<clipPath id=\"%s\"><rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\"/></clipPath><g "
          "clip-path=\"url(#%s)\">\n",
...
void Fl_SVG_Graphics_Driver::polygon(int x0, int y0, int x1, int y1, int x2, int y2) {
  fprintf(out_, "<path d=\"M %d %d L %d %d L %d %d z\" fill=\"rgb(%u,%u,%u)\" />\n", x0, y0, x1, y1,
          x2, y2, red_, green_, blue_);
}

void Fl_SVG_Graphics_Driver::polygon(int x0, int y0, int x1, int y1, int x2, int y2, int x3,
                                     int y3) {
  fprintf(out_, "<path d=\"M %d %d L %d %d L %d %d L %d %d z\" fill=\"rgb(%u,%u,%u)\" />\n", x0, y0,
          x1, y1, x2, y2, x3, y3, red_, green_, blue_);
}

void Fl_SVG_Graphics_Driver::loop(int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3) {
  fprintf(out_,
          "<path d=\"M %d %d L %d %d L %d %d L %d %d z\" fill=\"none\" stroke=\"rgb(%u,%u,%u)\" "
          "stroke-width=\"%d\" stroke-linejoin=\"%s\" stroke-linecap=\"%s\" "
          "stroke-dasharray=\"%s\"/>\n",
          x0, y0, x1, y1, x2, y2, x3, y3, red_, green_, blue_, width_, linejoin_, linecap_,
          dasharray_);
}

void Fl_SVG_Graphics_Driver::loop(int x0, int y0, int x1, int y1, int x2, int y2) {
  fprintf(out_,
          "<path d=\"M %d %d L %d %d L %d %d z\" fill=\"none\" stroke=\"rgb(%u,%u,%u)\" "
          "stroke-width=\"%d\" stroke-linejoin=\"%s\" stroke-linecap=\"%s\" "
          "stroke-dasharray=\"%s\"/>\n",
...
void Fl_SVG_Graphics_Driver::end_points() {
  for (int i = 0; i < n; i++) {
    fprintf(out_,
            "<path d=\"M %f %f L %f %f\" fill=\"none\" stroke=\"rgb(%u,%u,%u)\" "
            "stroke-width=\"%d\" />\n",
...
void Fl_SVG_Graphics_Driver::circle(double x, double y, double r) {
  ...
  if (what == POLYGON)
    ...
    fprintf(
        out_,
        " fill=\"none\" stroke-width=\"%d\" stroke-dasharray=\"%s\" stroke-linecap=\"%s\" stroke",
        width_, dasharray_, linecap_);
...
void Fl_SVG_Graphics_Driver::arc_pie(char AorP, int x, int y, int w, int h, double a1, double a2) {
  ...
  // CGContextAddArc() to the endpoint parameterization used by the "elliptical arc curve" command
  // of SVG (A).
  if (w <= 0 || h <= 0)
    return;
  ...
  if (AorP == 'A')
    compute_dasharray((sx + sy) / 2, user_dash_array_);
  if (full) {
    fprintf(out_, "<circle cx=\"0\" cy=\"0\" r=\"0.5\" style=\"fill");
    if (AorP == 'A')
      fprintf(out_, ":none;stroke-width:%f;stroke-linecap:%s;stroke-dasharray:%s;stroke",
              stroke_width, linecap_, dasharray_);
  } else {
    ...
    if (AorP == 'A')
      fprintf(out_,
              "<path d=\"M %f,%f A 0.5,0.5 0 %d,0 %f,%f\" "
              "style=\"fill:none;stroke-width:%f;stroke-linecap:%s;stroke-dasharray:%s;stroke",
              x1, y1, fA, x2, y2, stroke_width, linecap_, dasharray_);
    else
      fprintf(out_, "<path d=\"M 0,0 L %f,%f A 0.5,0.5 0 %d,0 %f,%f z\" style=\"fill", x1, y1, fA,
              x2, y2);
  }
  fprintf(out_, ":rgb(%u,%u,%u)\"/>\n</g>\n", red_, green_, blue_);
  if (AorP == 'A')
    compute_dasharray(1., user_dash_array_);
}
