...
public:
  Fl_X11_Gl_Choice(int m, const int *alistp, Fl_Gl_Choice *n)
...
void Fl_X11_Gl_Window_Driver::get_list(Fl_Font_Descriptor *fd, int r) {
  ...
  for (int i = 0; i < 0x400; i++) {
    ...
    if (font)
      glXUseXFont(font->fid, id, 1, gl_fd->listbase + ii);
    ii++;
  }
#endif
}
...
Fl_Gl_Choice *Fl_X11_Gl_Window_Driver::find(int m, const int *alistp) {
...
#if defined(GLX_VERSION_1_1) && defined(GLX_SGIS_multisample)
      if (m & FL_MULTISAMPLE)
        return find(m & ~FL_MULTISAMPLE, 0);
#endif
      return 0;
    }
  }
  ...
  if (/*MaxCmapsOfScreen(ScreenOfDisplay(fl_display,fl_screen))==1 && */
      visp->visualid == fl_visual->visualid && !fl_getenv("MESA_PRIVATE_CMAP"))
    g->colormap = fl_colormap;
  else
    g->colormap =
        XCreateColormap(fl_display, RootWindow(fl_display, fl_screen), visp->visual, AllocNone);
  return g;
}
...
GLContext Fl_X11_Gl_Window_Driver::create_gl_context(Fl_Window *window, const Fl_Gl_Choice *g,
                                                     int layer) {
  (void)window;
  (void)layer;
  ...
  typedef GLContext (*glXCreateContextAttribsARBProc)(Display *, GLXFBConfig, GLContext, Bool,
                                                      const int *);
  ...
  if (((Fl_X11_Gl_Choice *)g)->best_fb && strstr(glxExts, "GLX_ARB_create_context") &&
    ...
    ctx = glXCreateContextAttribsARB(fl_display, ((Fl_X11_Gl_Choice *)g)->best_fb, shared_ctx, true,
                                     context_attribs);
...
int Fl_X11_Gl_Window_Driver::mode_(int m, const int *a) {
  int oldmode = mode();
  if (a) { // when the mode is set using the a array of system-dependent values, and if asking for
           // double buffer,
    // the FL_DOUBLE flag must be set in the mode_ member variable
    const int *aa = a;
    while (*aa) {
      if (*(aa++) == GLX_DOUBLEBUFFER) {
        m |= FL_DOUBLE;
        break;
      }
    }
