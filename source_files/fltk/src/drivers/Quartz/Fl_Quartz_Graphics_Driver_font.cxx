...
Fl_Quartz_Font_Descriptor::Fl_Quartz_Font_Descriptor(const char *name, Fl_Fontsize Size)
...
static Fl_Font_Descriptor *find(Fl_Font fnum, Fl_Fontsize size) {
  if (!fl_fonts)
    fl_fonts = Fl_Graphics_Driver::default_driver().calc_fl_fonts();
  Fl_Fontdesc *s = fl_fonts + fnum;
  if (!s->name)
...
void Fl_Quartz_Graphics_Driver::text_extents(const char *txt, int n, int &dx, int &dy, int &w,
                                             int &h) {
  if (CoreText_or_ATSU == use_ATSU) {
...
void Fl_Quartz_Graphics_Driver::ADD_SUFFIX(descriptor_init,
                                           _CoreText)(const char *name, Fl_Fontsize size,
                                                      Fl_Quartz_Font_Descriptor *d) {
  ...
  d->q_width = w + 0.5;
  for (unsigned i = 0; i < sizeof(d->width) / sizeof(float *); i++)
    d->width[i] = NULL;
  if (!attributes) {
...
static CGFloat surrogate_width(const UniChar *txt, Fl_Quartz_Font_Descriptor *fl_fontsize) {
  ...
  if (b)
    CTFontGetAdvancesForGlyphs(font2, kCTFontHorizontalOrientation, glyphs, &a, 1);
  else
  ...
  for (i = 0; i < n; i++) { // loop over txt
    ...
    if (!fl_fontsize->width[r]) { // this character block has not been hit yet
      ...
      for (int j = 0; j < block; j++) { // loop over the block
        ...
        if (b)
          CTFontGetAdvancesForGlyphs(fl_fontsize->fontref, kCTFontHorizontalOrientation, &glyph,
                                     &advance_size, 1);
        else
...
void Fl_Quartz_Graphics_Driver::ADD_SUFFIX(draw, _CoreText)(const char *str, int n, float x,
                                                            float y) {
  Fl_Quartz_Font_Descriptor *fl_fontsize = valid_font_descriptor();
  ...
  CFMutableStringRef str16 =
      CFStringCreateMutableWithExternalCharactersNoCopy(NULL, uniStr, n, n, kCFAllocatorNull);
  if (str16 == NULL)
...
static char *skip(char *p, int &derived) {
  //                  0    5    10
  //                  |    |    |
  if (strncmp(p, "-BoldItalic", 11) == 0) {
    p += 11;
    derived = 3;
  } else if (strncmp(p, "-BoldOblique", 12) == 0) {
    p += 12;
    derived = 3;
  } else if (strncmp(p, "-Bold", 5) == 0) {
    p += 5;
    derived = 1;
  } else if (strncmp(p, "-Italic", 7) == 0) {
    p += 7;
    derived = 2;
  } else if (strncmp(p, "-Oblique", 8) == 0) {
    p += 8;
    derived = 2;
  } else if (strncmp(p, "-Regular", 8) == 0) {
    p += 8;
  } else if (strncmp(p, "-Roman", 6) == 0) {
    p += 6;
  }
  return p;
}
...
#pragma unused(xstarname)
  ...
  CFDictionaryRef dict =
      CFDictionaryCreate(NULL, (const void **)kCTFontCollectionRemoveDuplicatesOption,
  ...
  for (i = 0; i < count; i++) {
    ...
    CFRelease(font);
    CFDataRef cfdata =
        CFStringCreateExternalRepresentation(NULL, cfname, kCFStringEncodingUTF8, '?');
...
void Fl_Quartz_Graphics_Driver::ADD_SUFFIX(descriptor_init, _ATSU)(const char *name,
  ...
  ATSUFontID fontID;
  ATSUFindFontFromName(name, strlen(name), kFontFullName, kFontMacintoshPlatform, kFontNoScriptCode,
  ...
  if (fontID != kATSUInvalidFontID)
    err = ATSUSetAttributes(d->style, 1, sTag, sBytes, sAttr); // set the font attribute
  err = ATSUSetAttributes(d->style, 2, sTag + 1, sBytes + 1,
  ...
  err = ATSUGetUnjustifiedBounds(d->layout, kATSUFromTextBeginning, 1, &bBefore, &bAfter, &bAscent,
                                 &bDescent);
...
double Fl_Quartz_Graphics_Driver::ADD_SUFFIX(width, _ATSU)(const UniChar *txt, int n) {
  ...
  err = ATSUGetUnjustifiedBounds(layout, kATSUFromTextBeginning, n, &bBefore, &bAfter, &bAscent,
                                 &bDescent);
...
void Fl_Quartz_Graphics_Driver::ADD_SUFFIX(text_extents, _ATSU)(const char *str8, int n, int &dx,
                                                                int &dy, int &w, int &h) {
  Fl_Quartz_Font_Descriptor *fl_fontsize = valid_font_descriptor();
  UniChar *txt = mac_Utf8_to_Utf16(str8, n, &n);
  OSStatus err;
...
#pragma unused(xstarname)
  ...
  for (ItemCount idx = 0; idx < oFontCount; idx++) {
    ...
    //    ATSUFindFontName (oFontIDs[idx], kFontFullName, kFontMacintoshPlatform, kFontRomanScript,
    ...
    char oName[512];
    ATSUFindFontName(oFontIDs[idx], kFontFullName, kFontMacintoshPlatform, kFontRomanScript,
                     kFontEnglishLanguage, actualLength, oName, &actualLength, &oCountAgain);
