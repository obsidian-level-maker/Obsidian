...
public:
  ...
  HRESULT STDMETHODCALLTYPE DragOver(DWORD /*grfKeyState*/, POINTL pt, DWORD *pdwEffect) {
    ...
    if (fl_dnd_target_window) {
      float s =
          Fl::screen_driver()->scale(Fl_Window_Driver::driver(fl_dnd_target_window)->screen_num());
      Fl::e_x_root = int(Fl::e_x_root / s);
      Fl::e_y_root = int(Fl::e_y_root / s);
  ...
  HRESULT STDMETHODCALLTYPE Drop(IDataObject *data, DWORD /*grfKeyState*/, POINTL pt,
                                 DWORD * /*pdwEffect*/) {
    if (!fl_dnd_target_window)
      return S_OK;
...
public:
  ...
  HRESULT STDMETHODCALLTYPE GetData(FORMATETC *pformatetcIn, STGMEDIUM *pmedium) {
    if ((pformatetcIn->dwAspect & DVASPECT_CONTENT) && (pformatetcIn->tymed & TYMED_HGLOBAL) &&
      ...
      fl_utf8toUtf16(fl_selection_buffer[0], fl_selection_length[0], (unsigned short *)pMem,
                     utf16_len + 1);
  ...
  //  HRESULT STDMETHODCALLTYPE EnumFormatEtc( DWORD dir, IEnumFORMATETC** ppenumFormatEtc)
  //  {*ppenumFormatEtc = m_EnumF; return S_OK;}
...
int Fl_WinAPI_Screen_Driver::dnd(int unused) {
  ...
  HRESULT ret =
      DoDragDrop(fdo, fds, DROPEFFECT_MOVE | DROPEFFECT_LINK | DROPEFFECT_COPY, &dropEffect);
