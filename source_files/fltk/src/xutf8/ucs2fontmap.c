...
#define NEED_TOMB /* indicates what part of these include files is needed here (avoid compilation \
                     warnings) */
...
static int encoding_number(const char *enc) {
  if (!enc || !strncmp(enc, "iso10646-1", 10)) {
    ...
    return 14;
  } else if (!strcmp(enc, "big5-0") || !strcmp(enc, "big5.eten-0") || !strcmp(enc, "big5p-0")) {
    return 15;
