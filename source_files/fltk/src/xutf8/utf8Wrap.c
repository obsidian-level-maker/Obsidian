...
typedef struct {
  unsigned char byte1;
  unsigned char byte2;
} __attribute__((packed)) Fl_XChar2b;
#else
...
/*********************************************************************/
static int get_font_list(const char *base_font_name_list, char ***flist) {
  const char *ptr;
  const char *p;
  int nb;
  int nb_name;
...
/*********************************************************************/
static int font_spec_enc(char *font) {
  int ret;
  char *enc;
  char *end;
...
/*********************************************************************/
static void get_range(const char *enc, int *min, int *max) {
...
/*********************************************************************/
static int *get_encodings(char **font_name_list, int *ranges, int nb_font) {
      ...
      if (*ptr == '-') {
        ...
        if (ec == 13) {
          ...
          if (font_encoding_list[i] == 0) {
            get_range(ptr + 1, ranges + i * 2, ranges + i * 2 + 1);
          }
          break;
        }
      }
...
/** load all fonts                                                  **/
/*********************************************************************/
static void load_fonts(Display *dpy, XUtf8FontStruct *font_set) {

  int i = 0;

  font_set->fonts = (XFontStruct **)malloc(sizeof(XFontStruct *) * font_set->nb_font);

  font_set->ranges = (int *)malloc(sizeof(int) * font_set->nb_font * 2);
  ...
  while (i < font_set->nb_font) {
    ...
    fnt = font_set->fonts[i] = find_best_font(dpy, &(font_set->font_name_list[i]));
    if (fnt) {
  ...
  font_set->encodings =
      get_encodings(font_set->font_name_list, font_set->ranges, font_set->nb_font);
  ...
  for (i = 0; i < font_set->nb_font; i++) {
    if (font_set->font_name_list[i]) {
      int j;
      for (j = 0; j < i; j++) {
        if (font_set->font_name_list[j] && font_set->encodings[j] == font_set->encodings[i] &&
            font_set->ranges[2 * j] == font_set->ranges[2 * i] && font_set->ranges[(2 * j) + 1] &&
...
XUtf8FontStruct *XCreateUtf8FontStruct(Display *dpy, const char *base_font_name_list) {
  ...
  font_set->nb_font = get_font_list(base_font_name_list, &font_set->font_name_list);
...
/*****************************************************************************/
void XUtf8DrawRtlString(Display *display, Drawable d, XUtf8FontStruct *font_set, GC gc, int x,
                        int y, const char *string, int num_bytes) {
...
void XUtf8_measure_extents(Display *display, Drawable d, XUtf8FontStruct *font_set, GC gc,
  ...
  /* FC: the man does not specify error codes for it, but X will generate X errors like BadGC or
   * BadFont. */
  ...
  while (num_bytes > 0) {
    ...
    if (i > 120) {
      ...
      if (hs > ht)
        ht = hs; /* new height exceeds previous height */
      if (yt > (-sizes.ascent))
        yt = -sizes.ascent; /* delta y offset */
      i = 0;
    }
...
/*****************************************************************************/
int fl_XGetUtf8FontAndGlyph(XUtf8FontStruct *font_set, unsigned int ucs, XFontStruct **fnt,
                            unsigned short *id) {
...
/*****************************************************************************/
void XUtf8DrawImageString(Display *display, Drawable d, XUtf8FontStruct *font_set, GC gc, int x,
                          int y, const char *string, int num_bytes) {
  ...
  XGetGCValues(display, gc, GCFunction | GCForeground | GCBackground | GCFillStyle, &xgcv);
  ...
  XChangeGC(display, gc, GCFunction | GCForeground | GCBackground | GCFillStyle, &xgcv);

  XFillRectangle(display, d, gc, x, y - font_set->ascent, (unsigned)w,
                 (unsigned)(font_set->ascent + font_set->descent));
