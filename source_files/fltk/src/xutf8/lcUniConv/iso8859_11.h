...
static int iso8859_11_mbtowc(conv_t conv, ucs4_t *pwc, const unsigned char *s, int n) {
  unsigned char c = *s;
  if (c < 0x80) {
    *pwc = (ucs4_t)c;
    return 1;
...
static int iso8859_11_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, int n) {
  (void)conv;
  (void)n;
  unsigned char c = 0;
