...
public:
  ...
  int shortcutWidth;
  const Fl_Menu_Item *menu;
  menuwindow(const Fl_Menu_Item *m, int X, int Y, int W, int H, const Fl_Menu_Item *picked,
             const Fl_Menu_Item *title, int menubar = 0, int menubar_title = 0, int right_edge = 0);
  ~menuwindow();
...
void Fl_Menu_Item::draw(int x, int y, int w, int h, const Fl_Menu_ *m, int selected) const {
  ...
  if (selected) {
    ...
    if (selected == 2) { // menu title
      ...
      fl_draw_box(b, x + 1, y - (Fl::menu_linespacing() - 2) / 2, w - 2,
                  h + (Fl::menu_linespacing() - 2), r);
    }
  }
...
menuwindow::menuwindow(const Fl_Menu_Item *m, int X, int Y, int Wp, int Hp,
                       const Fl_Menu_Item *picked, const Fl_Menu_Item *t, int menubar,
                       int menubar_title, int right_edge)
  ...
  if (m)
    for (; m->text; m = m->next()) {
      ...
      if (hh + Fl::menu_linespacing() > itemheight)
        itemheight = hh + Fl::menu_linespacing();
...
void menuwindow::drawentry(const Fl_Menu_Item *m, int n, int eraseit) {
  ...
  if (eraseit && n != selected) {
    fl_push_clip(xx + 1, yy - (Fl::menu_linespacing() - 2) / 2, ww - 2,
                 hh + (Fl::menu_linespacing() - 2));
  ...
  if (m->submenu()) {
    ...
    Fl_Font f = m->labelsize_ || m->labelfont_ ? (Fl_Font)m->labelfont_
                : button                       ? button->textfont()
                                               : FL_HELVETICA;
    fl_font(f, m->labelsize_ ? m->labelsize_ : button ? button->textsize() : FL_NORMAL_SIZE);
    const char *k, *s = fl_shortcut_label(m->shortcut_, &k);
    if (fl_utf_nb_char((const unsigned char *)k, (int)strlen(k)) <= 4) {
...
const Fl_Menu_Item *Fl_Menu_Item::pulldown(int X, int Y, int W, int H,
                                           ...
                                           const Fl_Menu_ *pbutton, const Fl_Menu_Item *title,
                                           int menubar) const {
    ...
    if (m->submenu()) {
      ...
      if (initial_item) { // bring up submenu containing initial item:
        ...
        while (pp.nummenus > pp.menu_number + 1)
          delete pp.p[--pp.nummenus];
        pp.p[pp.nummenus++] = new menuwindow(menutable, nX, nY, title ? 1 : 0, 0, 0, title, 0,
                                             menubar, (title ? 0 : cw.x()));
      }
    } else { // !m->submenu():
      while (pp.nummenus > pp.menu_number + 1)
        delete pp.p[--pp.nummenus];
      if (!pp.menu_number && pp.menubar) {
        ...
        pp.fakemenu = new menuwindow(0, cw.x() + cw.titlex(pp.item_number), cw.y() + cw.h(), 0, 0,
                                     0, m, 0, 1);
...
*/
const Fl_Menu_Item *Fl_Menu_Item::popup(int X, int Y, const char *title, const Fl_Menu_Item *picked,
                                        const Fl_Menu_ *menu_button) const {
...
const Fl_Menu_Item *Fl_Menu_Item::find_shortcut(int *ip, const bool require_alt) const {
  ...
  if (m)
    for (int ii = 0; m->text; m = next_visible_or_not(m), ii++) {
      if (m->active()) {
        if (Fl::test_shortcut(m->shortcut_) || Fl_Widget::test_shortcut(m->text, require_alt)) {
          if (ip)
            *ip = ii;
          return m;
        }
