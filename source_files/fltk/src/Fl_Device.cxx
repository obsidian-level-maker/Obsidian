//
// implementation of classes Fl_Surface_Device and Fl_Display_Device for the Fast Light Tool Kit
// (FLTK).
//
...
/* Inheritance diagram.
  ...
  +- Fl_Surface_Device: any kind of surface that we can draw onto -> uses an Fl_Graphics_Driver
      ...
      +- Fl_Widget_Surface: any FLTK widget can be drawn to it
          |
          +- Fl_Copy_Surface: draw into the clipboard (in vectorial form if the platform supports
  it)
          +- Fl_Copy_Surface_Driver: helper class interfacing FLTK with draw-to-clipboard operations
              |
              +- Fl_..._Copy_Surface_Driver: platform-specific implementation of
  Fl_Copy_Surface_Driver
          ...
          +- Fl_Image_Surface_Driver: helper class interfacing FLTK with draw-to-image operations
              |
              +- Fl_..._Image_Surface_Driver: platform-specific implementation of
  Fl_Image_Surface_Driver
          ...
          +- Fl_Paged_Device: output to a page-structured surface
              ...
              +- Fl_WinAPI_Printer_Driver: Windows-specific helper class interfacing FLTK with print
  operations
              +- Fl_Cocoa_Printer_Driver: macOS-specific helper class interfacing FLTK with print
  operations
  ...
  +- Fl_Graphics_Driver -> directed to an Fl_Surface_Device object
      ...
      +- Fl_SVG_Graphics_Driver: platform-independent graphics driver for Scalable Vector Graphics
  drawing
      ...
      +- Fl_Scalable_Graphics_Driver: helper class to support GUI scaling
          ...
          +- Fl_GDI_Graphics_Driver: Windows-specific graphics driver
              +- Fl_GDI_Printer_Graphics_Driver: re-implements a few member functions especially for
  output to printer
...
/** Make this surface the current drawing surface.
 ...
 \li make current another surface, e.g., an Fl_Printer or an Fl_Image_Surface object,  calling
 set_current() on this object; \li draw to that surface; \li make the display current again with
 Fl_Display_Device::display_device()->set_current();  . Don't do any other call to set_current()
 before this one.

 Other scenarios of drawing surface changes should be performed via Fl_Surface_Device::push_current(
 ) / Fl_Surface_Device::pop_current().
 */
void Fl_Surface_Device::set_current(void) {
  if (surface_)
    surface_->end_current();
...
Fl_Display_Device *Fl_Display_Device::display_device() {
  static Fl_Display_Device *display =
      new Fl_Display_Device(Fl_Graphics_Driver::newMainGraphicsDriver());
  return display;
};
...
/** Pushes \p new_current on top of the stack of current drawing surfaces, and makes it current.
 ...
 Any call to push_current() must be matched by a subsequent call to
 Fl_Surface_Device::pop_current(). The max height of this stack is 16. \version 1.4.0
 */
...
/** Removes the top element from the current drawing surface stack, and makes the new top element
 current. \return A pointer to the new current drawing surface. \see
 Fl_Surface_Device::push_current(Fl_Surface_Device *) \version 1.4.0 */
Fl_Surface_Device *Fl_Surface_Device::pop_current() {
  if (surface_stack_height > 0)
    surface_stack[--surface_stack_height]->set_current();
  return surface_;
}
