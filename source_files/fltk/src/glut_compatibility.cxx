...
static void default_reshape(int w, int h) {
  glViewport(0, 0, w, h);
}
...
int Fl_Glut_Window::handle(int event) {
  ...
  switch (event) {
    ...
    case FL_MOUSEWHEEL:
      ...
      while (button < 0) {
        if (mouse)
          mouse(3, GLUT_DOWN, ex, ey);
        ++button;
      }
      while (button > 0) {
        if (mouse)
          mouse(4, GLUT_DOWN, ex, ey);
        --button;
      }
      return 1;
    ...
    case FL_KEYBOARD:
      ...
      if (Fl::event_command() && Fl_Screen_Driver::scale_handler(FL_SHORTCUT))
        return 1;
      if (Fl::event_text()[0]) {
        if (keyboard) {
...
static void domenu(int n, int ex, int ey) {
  ...
  if (g && g->callback_)
    ((void (*)(int))(g->callback_))(int(g->argument()));
  if (glut_menustatus_function)
...
void glutRemoveMenuItem(int item) {
  menu *m = &menus[glut_menu];
  if (item > m->size || item < 1)
    return;
  for (int i = item - 1; i <= m->size; i++)
    m->m[i] = m->m[i + 1];
  m->size--;
}
