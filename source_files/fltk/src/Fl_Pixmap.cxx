...
void Fl_Pixmap::copy_data() {
  ...
  if (ncolors < 0)
    new_data = new char *[data_h() + 2];
  else
    new_data = new char *[data_h() + ncolors + 1];
...
Fl_Image *Fl_Pixmap::copy(int W, int H) const {
  ...
  for (dy = H, sy = 0, yerr = H; dy > 0; dy--, new_row++) {
    *new_row = new char[chars_per_line];
    ...
    for (dx = W, xerr = W, old_ptr = data()[sy + ncolors + 1]; dx > 0; dx--) {
      for (c = 0; c < chars_per_pixel; c++)
        *new_ptr++ = old_ptr[c];
    ...
    for (color = 0; color < ncolors; color++) {
      ...
      if (fl_parse_color(p, r, g, b)) {
        ...
        if (chars_per_pixel > 1)
          sprintf(line, "%c%c c #%02X%02X%02X", data()[color + 1][0], data()[color + 1][1], r, g,
                  b);
        else
          sprintf(line, "%c c #%02X%02X%02X", data()[color + 1][0], r, g, b);
  ...
  if (ncolors < 0) {
    ...
    for (i = 0; i < ncolors; i++) {
      ...
      if (fl_parse_color(p, r, g, b)) {
        ...
        if (chars_per_pixel > 1)
          sprintf(line, "%c%c c #%02X%02X%02X", data()[i + 1][0], data()[i + 1][1], g, g, g);
        else
          sprintf(line, "%c c #%02X%02X%02X", data()[i + 1][0], g, g, g);
