...
void Fl_Counter::draw() {
  ...
  if (Fl::focus() == this)
    draw_focus(boxtype[0], xx[0], y(), ww[0], h());
...
int Fl_Counter::calc_mouseobj() {
  if (type() == FL_NORMAL_COUNTER) {
    ...
    if (Fl::event_inside(x(), y(), W, h()))
      return 1;
    if (Fl::event_inside(x() + W, y(), W, h()))
      return 2;
    if (Fl::event_inside(x() + w() - 2 * W, y(), W, h()))
      return 3;
    if (Fl::event_inside(x() + w() - W, y(), W, h()))
      return 4;
  } else {
    int W = w() * 20 / 100;
    if (Fl::event_inside(x(), y(), W, h()))
