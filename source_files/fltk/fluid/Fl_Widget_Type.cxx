...
void Fl_Widget_Type::ideal_size(int &w, int &h) {
  h = o->labelsize();
  o->measure_label(w, h);
...
void Fl_Widget_Type::ideal_spacing(int &x, int &y) {
  if (o->labelsize() < 10)
    x = y = 0;
...
Fl_Type *Fl_Widget_Type::make(Strategy strategy) {
  ...
  while (qq && (!qq->is_widget() || qq->is_menu_item()))
    qq = qq->parent;
  if (!qq) {
...
void *const LOAD =
    (void *)"LOAD";     // "magic" pointer to indicate that we need to load values into the dialog
...
static int bbox_min(int a, int b) {
  return (a < b) ? a : b;
}
static int bbox_max(int a, int b) {
  return (a > b) ? a : b;
}
...
Fluid_Coord_Input_Vars widget_vars[] = {
    ...
    {"y", vars_y_cb},   {"w", vars_w_cb},
    {"h", vars_h_cb},   {"px", vars_px_cb}, // position and size of parent widget
    {"py", vars_py_cb}, {"pw", vars_pw_cb},
    {"ph", vars_ph_cb}, {"sx", vars_sx_cb}, // position and size of previous sibling
    {"sy", vars_sy_cb}, {"sw", vars_sw_cb},
    {"sh", vars_sh_cb}, {"cx", vars_cx_cb}, // position and size of bounding box of all children
    {"cy", vars_cy_cb}, {"cw", vars_cw_cb},
    {"ch", vars_ch_cb}, {0}};
...
Fl_Menu_Item boxmenu[] = {{"NO_BOX", 0, 0, (void *)ZERO_ENTRY},
...
Fl_Menu_Item whenmenu[] = {{"Never", 0, 0, (void *)ZERO_ENTRY},
...
Fl_Menu_Item fontmenu[] = {{"Helvetica"},
                           {"Helvetica bold"},
                           {"Helvetica italic"},
...
Fl_Menu_Item labeltypemenu[] = {{"NORMAL_LABEL", 0, 0, (void *)0},
...
void color_cb(Fl_Button *i, void *v) {
  ...
  i->color(c);
  i->labelcolor(fl_contrast(FL_BLACK, c));
  i->redraw();
}
...
void v_input_cb(Fl_Input *i, void *v) {
  ...
  if (v == LOAD) {
    ...
    if (d) {
      fl_message("Error in %s: %s", i->label(), d);
      haderror = 1;
      return;
    }
    for (Fl_Type *o = Fl_Type::first; o; o = o->next) {
...
void slider_size_cb(Fl_Value_Input *i, void *v) {
  if (v == LOAD) {
    if (current_widget->is_window())
      ...
      i->parent()->show();
    if (current_widget->is_valuator() < 2) {
      i->deactivate();
      return;
    }
    i->activate();
...
Fl_Menu_Item *Fl_Widget_Type::subtypes() {
  return 0;
}

void subtype_cb(Fl_Choice *i, void *v) {
  static Fl_Menu_Item empty_type_menu[] = {{"Normal", 0, 0, (void *)0}, {0}};
    ...
    if (!m) {
      ...
      for (j = 0;; j++) {
        ...
        if (current_widget->is_spinner()) {
          if (m[j].argument() == ((Fl_Spinner *)current_widget->o)->type())
            break;
        } else {
          if (m[j].argument() == current_widget->o->type())
            break;
        }
      }
      i->value(j);
  ...
  if (Fl_Type::current) {
    ...
    for (Fl_Type *o = Fl_Type::first; o; o = o->next) {
      if (o->is_widget() && o->selected) {
        numselected++;
        if (!current_widget)
          current_widget = (Fl_Widget_Type *)o;
      }
    }
  }
  if (numselected)
...
int isdeclare(const char *c) {
  ...
  if (*c == '#')
    return 1;
  if (!strncmp(c, "extern", 6))
    return 1;
  if (!strncmp(c, "typedef", 7))
    return 1;
  if (!strncmp(c, "using", 5))
    return 1;
  return 0;
}
...
void Fl_Widget_Type::write_color(const char *field, Fl_Color color) {
  ...
  switch (color) {
    case FL_FOREGROUND_COLOR:
      color_name = "FL_FOREGROUND_COLOR";
      break;
    case FL_BACKGROUND2_COLOR:
      color_name = "FL_BACKGROUND2_COLOR";
      break;
    case FL_INACTIVE_COLOR:
      color_name = "FL_INACTIVE_COLOR";
      break;
    case FL_SELECTION_COLOR:
      color_name = "FL_SELECTION_COLOR";
      break;
    case FL_GRAY0:
      color_name = "FL_GRAY0";
      break;
    ...
    case FL_BACKGROUND_COLOR:
      color_name = "FL_BACKGROUND_COLOR";
      break;
    case FL_LIGHT1:
      color_name = "FL_LIGHT1";
      break;
    case FL_LIGHT2:
      color_name = "FL_LIGHT2";
      break;
    case FL_LIGHT3:
      color_name = "FL_LIGHT3";
      break;
    case FL_BLACK:
      color_name = "FL_BLACK";
      break;
    ...
    case FL_GREEN:
      ...
      break;
    case FL_YELLOW:
      color_name = "FL_YELLOW";
      break;
    case FL_BLUE:
      color_name = "FL_BLUE";
      break;
    case FL_MAGENTA:
      color_name = "FL_MAGENTA";
      break;
    case FL_CYAN:
      color_name = "FL_CYAN";
      break;
    case FL_DARK_RED:
      color_name = "FL_DARK_RED";
      break;
    case FL_DARK_GREEN:
      color_name = "FL_DARK_GREEN";
      break;
    case FL_DARK_YELLOW:
      color_name = "FL_DARK_YELLOW";
      break;
    case FL_DARK_BLUE:
      color_name = "FL_DARK_BLUE";
      break;
    case FL_DARK_MAGENTA:
      color_name = "FL_DARK_MAGENTA";
      break;
    case FL_DARK_CYAN:
      color_name = "FL_DARK_CYAN";
      break;
    case FL_WHITE:
      color_name = "FL_WHITE";
      break;
...
void Fl_Widget_Type::write_widget_code() {
  ...
  int shortcut = 0;
  if (is_button())
    shortcut = ((Fl_Button *)o)->shortcut();
  else if (is_input())
    shortcut = ((Fl_Input_ *)o)->shortcut();
  else if (is_value_input())
    shortcut = ((Fl_Value_Input *)o)->shortcut();
  else if (is_text_display())
    shortcut = ((Fl_Text_Display *)o)->shortcut();
  if (shortcut) {
  ...
  if (image)
    image->write_code(var);
  if (inactive)
    inactive->write_code(var, 1);
...

  {
    ...
    if (textstuff(4, ff, fs, fc)) {
      ...
      if (c != fc)
        write_color("textcolor", c);
    }
  }
  const char *ud = user_data();
  if (class_name(1) && !parent->is_widget())
    ud = "this";
  if (callback()) {
  ...
  if (o->align() != tplate->align() || subclass()) {
    int i = o->align();
    write_c("%s%s->align(Fl_Align(%s", indent(), var, item_name(alignmenu, i & ~FL_ALIGN_INSIDE));
    if (i & FL_ALIGN_INSIDE)
  ...
  if (ww != tplate->when() || subclass())
    write_c("%s%s->when(%s);\n", indent(), var, item_name(whensymbolmenu, ww));
...
void Fl_Widget_Type::write_block_close() {
  indentation--;
  write_c("%s} // %s* %s\n", indent(), subclassname(this), name() ? name() : "o");
}
...
void Fl_Widget_Type::write_properties() {
  ...
  if (is_valuator()) {
    ...
    Fl_Valuator *f = (Fl_Valuator *)(tplate);
    if (v->minimum() != f->minimum())
      write_string("minimum %g", v->minimum());
    if (v->maximum() != f->maximum())
      write_string("maximum %g", v->maximum());
    if (v->step() != f->step())
      write_string("step %g", v->step());
    if (v->value() != 0.0)
      write_string("value %g", v->value());
    if (is_valuator() >= 2) {
      double x = ((Fl_Slider *)v)->slider_size();
      double y = ((Fl_Slider *)f)->slider_size();
      if (x != y)
        write_string("slider_size %g", x);
    }
  }
  if (is_spinner()) {
    Fl_Spinner *v = (Fl_Spinner *)o;
  ...
  }
  {
    ...
    if (textstuff(4, ff, fs, fc)) {
      ...
      textstuff(0, f, s, c);
      if (f != ff)
        write_string("textfont %d", f);
      if (s != fs)
        write_string("textsize %d", s);
      if (c != fc)
        write_string("textcolor %d", c);
    }
  }
  if (!o->visible())
    write_string("hide");
  ...
  if (hotspot())
    write_string(is_menu_item() ? "divider" : "hotspot");
  for (int n = 0; n < NUM_EXTRA_CODE; n++)
    if (extra_code(n)) {
      write_indent(level + 1);
...
void Fl_Widget_Type::read_property(const char *c) {
  ...
  if (!strcmp(c, "private")) {
    ...
    if (!strcmp(c, "image")) {
      ...
      if (!i)
        read_error("Image file '%s' not found", label());
      else
        setimage(i);
      image_name(label());
      label("");
    ...
    if (is_button())
      ((Fl_Button *)o)->shortcut(shortcut);
    else if (is_input())
      ((Fl_Input_ *)o)->shortcut(shortcut);
    else if (is_value_input())
      ((Fl_Value_Input *)o)->shortcut(shortcut);
    else if (is_text_display())
      ((Fl_Text_Display *)o)->shortcut(shortcut);
  } else {
    if (!strncmp(c, "code", 4)) {
      int n = atoi(c + 4);
...
int Fl_Widget_Type::read_fdesign(const char *propname, const char *value) {
  ...
  if (!strcmp(propname, "box")) {
    ...
    if (value[0]) {
      char buf[128];
      sprintf(buf, "o->shortcut(\"%s\");", value);
      extra_code(0, buf);
    }
  } else if (!strcmp(propname, "style")) {
    if (!strncmp(value, "FL_NORMAL", 9))
      return 1;
    if (!lookup_symbol(value, v, 1))
      return 0;
    o->labelfont(v);
    o->labeltype((Fl_Labeltype)(v >> 8));
  } else if (!strcmp(propname, "size")) {
    if (!lookup_symbol(value, v, 1))
      return 0;
    o->labelsize(v);
  } else if (!strcmp(propname, "type")) {
    if (!strncmp(value, "NORMAL", 6))
      return 1;
    if (lookup_symbol(value, v, 1)) {
      o->type(v);
      return 1;
    }
    if (!strcmp(value + strlen(value) - 5, "FRAME"))
      goto TRY_BOXTYPE;
    if (!strcmp(value + strlen(value) - 3, "BOX"))
      goto TRY_BOXTYPE;
    return 0;
  ...
  TRY_BOXTYPE:
    ...
    if (!x) {
      x = item_number(boxmenu1, value);
      if (x < 0)
        return 0;
    }
    if (x == ZERO_ENTRY) {
      x = 0;
      if (o->box() != ((Fl_Widget_Type *)factory)->o->box())
        return 1; // kludge for frame
    }
    o->box((Fl_Boxtype)x);
  } else {
    return 0;
