...
/// Hold the default extension for source code  files, or the entire filename if set via command
/// line.
 ...
 */
char position_window(Fl_Window *w, const char *prefsName, int Visible, int X, int Y, int W = 0,
                     int H = 0) {
...
void save_template_cb(Fl_Widget *, void *) {
  ...
  if ((pixels = wt->read_image(w, h)) == NULL)
    return;
...
void revert_cb(Fl_Widget *, void *) {
  if (modflag) {
    if (!fl_choice("This user interface has been changed. Really revert?", "Cancel", "Revert",
                   NULL))
      return;
  }
  undo_suspend();
...
void paste_cb(Fl_Widget *, void *) {
  ...
  if (gridx > 1)
    pasteoffset = ((pasteoffset - 1) / gridx + 1) * gridx;
  if (gridy > 1)
...
void print_menu_cb(Fl_Widget *, void *) {
  ...
  for (t = Fl_Type::first, num_windows = 0; t; t = t->next) {
    if (t->is_window()) {
      ...
      if (!((Fl_Window *)(windows[num_windows]->o))->shown())
        continue;
      num_windows++;
    }
  }
  ...
  Fl_Printer printjob;
  if (printjob.start_job(num_windows, &frompage, &topage))
    return;
  int pagecount = 0;
  for (winpage = 0; winpage < num_windows; winpage++) {
    float scale = 1, scale_x = 1, scale_y = 1;
    if (winpage + 1 < frompage || winpage + 1 > topage)
      continue;
    printjob.start_page();
...
Fl_Menu_Item Main_Menu[] = {
    ...
    {"Show Source Code...", FL_ALT + FL_SHIFT + 's', (Fl_Callback *)toggle_sourceview_cb, 0,
     FL_MENU_DIVIDER},
    ...
    {"&Tiny", FL_ALT + '1', (Fl_Callback *)widget_size_cb, (void *)8, 0, FL_NORMAL_LABEL,
     FL_HELVETICA, 8},
    {"&Small", FL_ALT + '2', (Fl_Callback *)widget_size_cb, (void *)11, 0, FL_NORMAL_LABEL,
     FL_HELVETICA, 11},
    {"&Normal", FL_ALT + '3', (Fl_Callback *)widget_size_cb, (void *)14, 0, FL_NORMAL_LABEL,
     FL_HELVETICA, 14},
    {"&Medium", FL_ALT + '4', (Fl_Callback *)widget_size_cb, (void *)18, 0, FL_NORMAL_LABEL,
     FL_HELVETICA, 18},
    {"&Large", FL_ALT + '5', (Fl_Callback *)widget_size_cb, (void *)24, 0, FL_NORMAL_LABEL,
     FL_HELVETICA, 24},
    {"&Huge", FL_ALT + '6', (Fl_Callback *)widget_size_cb, (void *)32, 0, FL_NORMAL_LABEL,
     FL_HELVETICA, 32},
    {0},
...
void toggle_sourceview_cb(Fl_Double_Window *, void *) {
  if (!sourceview_panel) {
    ...
    if (!position_window(sourceview_panel, "sourceview_pos", 0, 320, 120, 550, 500))
      return;
  }
...
void load_history() {
  ...
  for (i = 0; i < max_files; i++) {
    ...
    if (absolute_history[i][0]) {
      ...
      fl_filename_relative(relative_history[i], sizeof(relative_history[i]), absolute_history[i]);
  ...
  for (; i < 10; i++) {
    if (i)
      history_item[i - 1].flags |= FL_MENU_DIVIDER;
    history_item[i].hide();
  }
}
...
void update_history(const char *flname) {
  ...
  for (i = 0; i < max_files; i++)
    ...
    if (!strcasecmp(absolute, absolute_history[i]))
      break;
#else
    if (!strcmp(absolute, absolute_history[i]))
      break;
  ...
  memmove(absolute_history + 1, absolute_history, i * sizeof(absolute_history[0]));
  memmove(relative_history + 1, relative_history, i * sizeof(relative_history[0]));

  // Put the new file at the top...
  strlcpy(absolute_history[0], absolute, sizeof(absolute_history[0]));

  fl_filename_relative(relative_history[0], sizeof(relative_history[0]), absolute_history[0]);
...
void set_modflag(int mf, int mfc) {
  ...
  if (main_window) {
    ...
    snprintf(title, sizeof(title), "%s%c  %s%c", basename, mod_star, code_ext, mod_c_star);
