...
void Fl_Input_Choice_Type::build_menu() {
  ...
  if (!n) {
    if (menusize)
      delete_menu((Fl_Menu_Item *)(w->menu()));
    w->menu(0);
    menusize = 0;
  } else {
    ...
    if (menusize < n) {
      ...
      if (menusize)
        delete_dependents((Fl_Menu_Item *)(w->menu()));
    }
    ...
    for (q = next; q && q->level > level; q = q->next) {
      ...
      m++;
      int l1 = (q->next && q->next->is_menu_item()) ? q->next->level : level;
      while (lvl > l1) {
        m->label(0);
        m++;
        lvl--;
...
Fl_Type *Fl_Menu_Item_Type::make(Strategy strategy) {
  ...
  t->add(p, strategy);
  if (!reading_file)
    t->label(submenuflag ? "submenu" : "item");
  return t;
}
...
void Fl_Menu_Item_Type::write_static() {
  ...
  for (Fl_Type *q = t->next; q && q->is_menu_item(); q = q->next) {
    ...
    if (q->is_parent())
      thislevel++;
    int nextlevel = (q->next && q->next->is_menu_item()) ? q->next->level : t->level + 1;
...
void Fl_Menu_Item_Type::write_item() {
  static const char *const labeltypes[] = {
      "FL_NORMAL_LABEL",   "FL_NO_LABEL",    "FL_SHADOW_LABEL", "FL_ENGRAVED_LABEL",
  ...
  if (user_data())
    ...
    write_c(" 0,");
  write_c(" %d, (uchar)%s, %d, %d, %d", flags(), labeltypes[o->labeltype()], o->labelfont(),
          o->labelsize(), o->labelcolor());
  write_c("},\n");
}
...
void Fl_Menu_Item_Type::write_code1() {
    ...
    if (label() && label()[0]) {
      ...
      if (i18n_type == 0) {
        ...
        write_c("%sml->labelb = catgets(%s,%s,i+%d,o->label());\n", indent(),
                i18n_file[0] ? i18n_file : "_catalog", i18n_set, msgnum());
      }
  ...
  if (i18n_type && label() && label()[0]) {
    ...
    } else if (t == FL_NORMAL_LABEL || t == FL_SHADOW_LABEL || t == FL_ENGRAVED_LABEL ||
               t == FL_EMBOSSED_LABEL) {
      start_menu_initialiser(menuItemInitialized, mname, i);
      if (i18n_type == 1) {
...
Fl_Type *Fl_Menu_Type::click_test(int, int) {
  ...
  if (m) {
    ...
    if (m->flags & (FL_MENU_RADIO | FL_MENU_TOGGLE))
      build_menu();
...
Fl_Menu_Item button_type_menu[] = {{"normal", 0, 0, (void *)0},
  ...
  if (v == LOAD) {
    ...
    for (Fl_Type *o = Fl_Type::first; o; o = o->next)
      if (o->selected && o->is_button()) {
        ...
        if (b->shortcut() != i->svalue)
          mod = 1;
        b->shortcut(i->svalue);
        if (o->is_menu_item())
          ((Fl_Widget_Type *)o)->redraw();
