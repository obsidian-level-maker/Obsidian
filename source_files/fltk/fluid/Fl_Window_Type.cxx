...
Fl_Menu_Item window_type_menu[] = {
    {"Single", 0, 0, (void *)FL_WINDOW}, {"Double", 0, 0, (void *)(FL_WINDOW + 1)}, {0}};
  ...
  if (show_guides && drag) {
    ...
    if (numselected == 1 && selection && !(drag & DRAG)) {
      ...
      if (drag & (TOP | BOTTOM)) {
        ...
        // Draw height guide
        draw_height(x < 50 ? x + 10 : x - 10, y, t, x < 50 ? FL_ALIGN_RIGHT : FL_ALIGN_LEFT);
      }
      ...
      if (drag & (LEFT | RIGHT)) {
        ...
        // Draw width guide
        draw_width(x, y < 50 ? y + 10 : y - 10, r, y < 50 ? FL_ALIGN_BOTTOM : FL_ALIGN_TOP);
      }
    }
      ...
      for (Fl_Type *q = next; q && q->level > level; q = q->next)
        if (q != selection && q->is_widget()) {
          ...
          if (!(qw->o->align() & FL_ALIGN_INSIDE)) {
            ...
            if (qw->o->align() & FL_ALIGN_TOP)
              qy -= hh;
            if (qw->o->align() & FL_ALIGN_BOTTOM)
              qt += hh;
          }

          // Do horizontal alignment when the widget is within 25
          // pixels vertically...
          if (fl_min(abs(qy - mysel->o->y() - mysel->o->h()), abs(mysel->o->y() - qt)) < 25) {
...
void check_redraw_corresponding_parent(Fl_Type *s) {
  ...
  if (!s || !s->selected || !s->is_widget())
    return;
  for (Fl_Type *i = s; i && i->parent; i = i->parent) {
    if (i->is_group() && prev_parent &&
        (!strcmp(i->type_name(), "Fl_Tabs") || !strcmp(i->type_name(), "Fl_Wizard"))) {
...
int Fl_Window_Type::handle(int event) {
  ...
  switch (event) {
    ...
    case FL_PASTE: {
      ...
      if (Fl_Type::current_dnd->group() && selection->group() &&
    ...
    case FL_PUSH:
      ...
      if (numselected && !(Fl::event_state(FL_SHIFT)) && mx <= br + snap && mx >= bx - snap &&
          my <= bt + snap && my >= by - snap) {
      ...
      // do object-specific selection of other objects:
      {
        ...
        if (t) {
          ...
          if (!drag)
            drag = BOX; // if all else fails, start a new selection region
        }
      }
      return 1;
      ...
      if (drag != BOX && (dx || dy || !Fl::event_is_click())) {
        ...
        for (Fl_Type *i = next; i && i->level > level; i = i->next)
          if (i->is_widget() && !i->is_menu_item()) {
            ...
            if (Fl::event_inside(myo->o))
              selection = myo;
            if (myo->o->x() >= x1 && myo->o->y() > y1 && myo->o->x() + myo->o->w() < mx &&
                myo->o->y() + myo->o->h() < my) {
              n++;
...
void Fl_Window_Type::write_code2() {
  ...
  if (sr_max_w || sr_max_h) {
    write_c("%s%s->size_range(%d, %d, %d, %d);\n", indent(), var, sr_min_w, sr_min_h, sr_max_w,
            sr_max_h);
