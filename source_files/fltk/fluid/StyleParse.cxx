...
static void *search_keywords(char *find) {
  return bsearch(&find, code_keywords, sizeof(code_keywords) / sizeof(code_keywords[0]),
...
static void *search_types(char *find) {
  return bsearch(&find, code_types, sizeof(code_types) / sizeof(code_types[0]),
...
int StyleParse::parse_over_angles(char s) {
  ...
  while (len > 0 && *tbuff != '>') {
    if (!parse_over_char())
      return 0;
  } // parse over '<' and angle content
  if (!parse_over_char())
    return 0; // parse over trailing '>'
  style = save;
  return 1;
}
...
int StyleParse::parse_quoted_string(char quote_char, // e.g. '"' or '\''
  ...
  if (!parse_over_char())
    return 0; // parse over opening quote
  ...
  while (len > 0) {
    ...
    if (c == quote_char) { // Closing quote? Parse and done
      if (!parse_over_char())
        return 0; // close quote
      break;
    } else if (c == '\\') { // Escape sequence? Parse over, continue
      if (!parse_over_char())
        return 0; // escape
      if (!parse_over_char())
        return 0; // char being escaped
      continue;
    }
...
int StyleParse::parse_directive() {
  ...
  if (!parse_over_char())
    return 0; // Parse over '#'
  if (!parse_over_white())
    return 0; // Parse over any whitespace after '#'
  if (!parse_over_alpha())
    return 0;  // Parse over the directive
  style = 'A'; // revert normal style
  if (!parse_over_white())
    return 0; // Parse over white after directive
  if (!parse_over_angles('D'))
    return 0; // #include <..> (if any)
  return 1;
}
...
int StyleParse::parse_escape() {
  ...
  if (!parse_over_char(no_crlf))
    return 0; // backslash
  if (!parse_over_char(no_crlf))
    return 0; // char escaped
  return 1;
}
