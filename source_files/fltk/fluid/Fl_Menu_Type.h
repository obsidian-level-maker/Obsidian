...
public:
  ...
  ~Fl_Menu_Type() {
    if (menusize)
      delete[](Fl_Menu_Item *)(((Fl_Menu_ *)o)->menu());
  }
...
public:
  ...
  Fl_Widget *widget(int X, int Y, int W, int H) { return new Fl_Menu_Button(X, Y, W, H, "menu"); }
...
class Fl_Input_Choice_Type : public Fl_Menu_Type {
  int textstuff(int w, Fl_Font &f, int &s, Fl_Color &c) {
    Fl_Input_Choice *myo =
        (Fl_Input_Choice *)(w == 4 ? ((Fl_Widget_Type *)this->factory)->o : this->o);
    switch (w) {
      case 4:
      case 0:
