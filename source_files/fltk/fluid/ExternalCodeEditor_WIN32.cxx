...
int ExternalCodeEditor::handle_changes(const char **code, int force) {
  ...
  if (changed || force) {
    ...
    if (ReadFile(fh, buf, buflen, &count, 0) == 0) {
      fl_alert("ERROR: ReadFile() failed for %s: %s", filename(), get_ms_errmsg());
      free((void *)buf);
      buf = 0;
...
int ExternalCodeEditor::remove_tmpfile() {
  ...
  if (is_file(tmpfile)) {
    ...
    if (G_debug)
      printf("remove_tmpfile(): is_file(%s) failed\n", tmpfile);
  }
  set_filename(0);
  memset(&file_mtime_, 0, sizeof(file_mtime_));
  memset(&file_size_, 0, sizeof(file_size_));
...
const char *ExternalCodeEditor::tmpdir_name() {
  ...
  _snprintf(dirname, sizeof(dirname), "%s.fluid-%ld", tempdir, (long)GetCurrentProcessId());
  if (G_debug)
    printf("tmpdir_name(): '%s'\n", dirname);
  return dirname;
}
...
void ExternalCodeEditor::tmpdir_clear() {
  ...
  if (is_dir(tmpdir)) {
    if (G_debug)
      printf("Removing tmpdir '%s'\n", tmpdir);
    if (RemoveDirectory(tmpdir) == 0) {
      fl_alert("WARNING: Can't RemoveDirectory() '%s': %s", tmpdir, get_ms_errmsg());
    }
  }
}
...
const char *ExternalCodeEditor::create_tmpdir() {
  ...
  if (!is_dir(dirname)) {
    if (CreateDirectory(dirname, 0) == 0) {
      fl_alert("can't create directory '%s': %s", dirname, get_ms_errmsg());
      return NULL;
    }
  }
  return dirname;
...
static int save_file(const char *filename, const char *code,
  ...
  HANDLE fh = CreateFile(filename,              // filename
                         ...
                         NULL);                 // templates
  if (fh == INVALID_HANDLE_VALUE) {
    fl_alert("ERROR: couldn't create file '%s': %s", filename, get_ms_errmsg());
    return (-1);
  }
  ...
  if (WriteFile(fh, code, clen, &count, NULL) == 0) {
    ...
    fl_alert("ERROR: WriteFile() '%s': wrote only %lu bytes, expected %lu", filename,
             (unsigned long)count, (unsigned long)clen);
    ret = -1; // fallthru to CloseHandle()
  }
...
//
int ExternalCodeEditor::start_editor(const char *editor_cmd, const char *filename) {
  if (G_debug)
  ...
  if (CreateProcess(NULL,            // app name
    ...
    fl_alert("CreateProcess() failed to start '%s': %s", cmd, get_ms_errmsg());
    return (-1);
  }
...
int ExternalCodeEditor::reap_editor(DWORD *pid_reaped) {
  ...
  switch (err = WaitForSingleObject(pinfo_.hProcess, msecs_wait)) {
    ...
    case WAIT_OBJECT_0: {              // reaped
      ...
      reap_cleanup();                  // clears pinfo_
      if (pid_reaped)
        *pid_reaped = wpid; // return pid to caller
      if (G_debug)
...
int ExternalCodeEditor::open_editor(const char *editor_cmd, const char *code) {
  ...
  if (L_update_timer_cb && !Fl::has_timeout(L_update_timer_cb)) {
    if (G_debug)
      printf("--- Editor opened: STARTING UPDATE TIMER\n");
...
void ExternalCodeEditor::start_update_timer() {
  if (!L_update_timer_cb)
    return;
  if (G_debug)
    printf("--- TIMER: STARTING UPDATES\n");
  Fl::add_timeout(2.0, L_update_timer_cb);
}
