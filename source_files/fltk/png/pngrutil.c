...
png_uint_32(PNGAPI png_get_uint_32)(png_const_bytep buf) {
  png_uint_32 uval = ((png_uint_32)(*(buf)) << 24) + ((png_uint_32)(*(buf + 1)) << 16) +
...
png_read_chunk_header(png_structrp png_ptr) {
  ...
  png_debug2(0, "Reading %lx chunk, length = %lu", (unsigned long)png_ptr->chunk_name,
 ...
 */
static int
png_decompress_chunk(png_structrp png_ptr, png_uint_32 chunklength, png_uint_32 prefix_size,
      ...
      if (ret == Z_STREAM_END) {
        ...
        if (inflateReset(&png_ptr->zstream) == Z_OK) {
          ...
          if (text != NULL) {
            ...
            ret = png_inflate(png_ptr, png_ptr->chunk_name, 1 /*finish*/,
                              png_ptr->read_buffer + prefix_size, &lzsize, text + prefix_size,
                              newlength);
...
static int png_inflate_read(png_structrp png_ptr, png_bytep read_buffer, uInt read_size,
                            png_uint_32p chunk_bytes, png_bytep next_out,
                            png_alloc_size_t *out_size, int finish) {
  if (png_ptr->zowner == png_ptr->chunk_name) {
    ...
    do {
       ...
       */
      ret =
          PNG_INFLATE(png_ptr, *chunk_bytes > 0 ? Z_NO_FLUSH : (finish ? Z_FINISH : Z_SYNC_FLUSH));
...
png_handle_cHRM(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  if (xy.whitex == PNG_FIXED_ERROR || xy.whitey == PNG_FIXED_ERROR || xy.redx == PNG_FIXED_ERROR ||
      xy.redy == PNG_FIXED_ERROR || xy.greenx == PNG_FIXED_ERROR || xy.greeny == PNG_FIXED_ERROR ||
        ...
        if (png_inflate_claim(png_ptr, png_iCCP) == Z_OK) {
          ...
          if (size == 0) {
            ...
            if (png_icc_check_length(png_ptr, &png_ptr->colorspace, keyword, profile_length) != 0) {
               ...
               * byte header.
               */
              if (png_icc_check_header(png_ptr, &png_ptr->colorspace, keyword, profile_length,
                ...
                if (profile != NULL) {
                   ...
                   */
                  if (size == 0) {
                    if (png_icc_check_tag_table(png_ptr, &png_ptr->colorspace, keyword,
                      ...
                      /* But otherwise allow extra data: */
                        ...
                        if (info_ptr != NULL) {
                          ...
                          info_ptr->iccp_name =
                              png_voidcast(char *, png_malloc_base(png_ptr, keyword_length + 1));
...
png_handle_bKGD(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  else if ((png_ptr->mode & PNG_HAVE_IDAT) != 0 || (png_ptr->color_type == PNG_COLOR_TYPE_PALETTE &&
...
png_handle_pCAL(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
  ...
  png_set_pCAL(png_ptr, info_ptr, (png_charp)buffer, X0, X1, type, nparams, (png_charp)units,
               params);
...
png_handle_zTXt(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
   ...
   */
     ...
     * and text chunks.
     */
    if (png_decompress_chunk(png_ptr, length, keyword_length + 2, &uncompressed_length,
...
png_handle_iTXt(png_structrp png_ptr, png_inforp info_ptr, png_uint_32 length) {
   ...
   */
    ...
    if (compressed == 0 && prefix_length <= length)
       ...
       */
      if (png_decompress_chunk(png_ptr, length, prefix_length, &uncompressed_length,
                               1 /*terminate*/) == Z_STREAM_END)
        buffer = png_ptr->read_buffer;
      ...
      if (ret < 0)
        ...
        if (keep < PNG_HANDLE_CHUNK_IF_SAFE) {
          ...
          if (png_ptr->unknown_default < PNG_HANDLE_CHUNK_IF_SAFE) {
            ...
            png_app_warning(png_ptr, "forcing save of an unhandled chunk;"
...
png_check_chunk_length(png_const_structrp png_ptr, png_uint_32 length) {
  ...
  if (png_ptr->chunk_name == png_IDAT) {
    ...
    size_t row_factor =
        (size_t)png_ptr->width * (size_t)png_ptr->channels * (png_ptr->bit_depth > 8 ? 2 : 1) + 1 +
...
#define S_MASK(p, d, s)                                                             \
  MASK_EXPAND(S_MASKx(p, 0, d, s) + S_MASKx(p, 1, d, s) + S_MASKx(p, 2, d, s) +     \
                  S_MASKx(p, 3, d, s) + S_MASKx(p, 4, d, s) + S_MASKx(p, 5, d, s) + \
                  S_MASKx(p, 6, d, s) + S_MASKx(p, 7, d, s),                        \
              d)

#define B_MASK(p, d, s)                                                             \
  MASK_EXPAND(B_MASKx(p, 0, d, s) + B_MASKx(p, 1, d, s) + B_MASKx(p, 2, d, s) +     \
                  B_MASKx(p, 3, d, s) + B_MASKx(p, 4, d, s) + B_MASKx(p, 5, d, s) + \
...
#define S_MASKS(d, s)                                                                    \
  {                                                                                      \
    S_MASK(0, d, s), S_MASK(1, d, s), S_MASK(2, d, s), S_MASK(3, d, s), S_MASK(4, d, s), \
        S_MASK(5, d, s)                                                                  \
  }
    ...
    else /* pixel_depth >= 8 */
    {
      ...
      switch (bytes_to_copy) {
        ...
        default:
           ...
           */
          if (bytes_to_copy < 16 /*else use memcpy*/ && png_isaligned(dp, png_uint_16) &&
              png_isaligned(sp, png_uint_16) && bytes_to_copy % (sizeof(png_uint_16)) == 0 &&
              bytes_to_jump % (sizeof(png_uint_16)) == 0) {
            /* Everything is aligned for png_uint_16 copies, but try for
             * png_uint_32 first.
             */
            if (png_isaligned(dp, png_uint_32) && png_isaligned(sp, png_uint_32) &&
...
png_read_start_row(png_structrp png_ptr) {
  ...
  if (png_ptr->interlaced != 0) {
    if ((png_ptr->transformations & PNG_INTERLACE) == 0)
      png_ptr->num_rows =
          (png_ptr->height + png_pass_yinc[0] - 1 - png_pass_ystart[0]) / png_pass_yinc[0];
