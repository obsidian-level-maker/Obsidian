...
PNG_FUNCTION(png_structp, PNGAPI png_create_read_struct,
  ...
  png_structp png_ptr =
      png_create_png_struct(user_png_ver, error_ptr, error_fn, warn_fn, NULL, NULL, NULL);
#else
...
PNG_FUNCTION(png_structp, PNGAPI png_create_read_struct_2,
             ...
             PNG_ALLOCATED) {
  png_structp png_ptr = png_create_png_struct(user_png_ver, error_ptr, error_fn, warn_fn, mem_ptr,
                                              malloc_fn, free_fn);
#endif /* USER_MEM */
...
void PNGAPI png_read_row(png_structrp png_ptr, png_bytep row, png_bytep dsp_row) {
  ...
  png_debug2(1, "in png_read_row (row %lu, pass %d)", (unsigned long)png_ptr->row_number,
             png_ptr->pass);
  ...
  if (png_ptr->row_buf[0] > PNG_FILTER_VALUE_NONE) {
    if (png_ptr->row_buf[0] < PNG_FILTER_VALUE_LAST)
      png_read_filter_row(png_ptr, &row_info, png_ptr->row_buf + 1, png_ptr->prev_row + 1,
                          png_ptr->row_buf[0]);
    else
...
void PNGAPI png_read_rows(png_structrp png_ptr, png_bytepp row, png_bytepp display_row,
...
void PNGAPI png_read_png(png_structrp png_ptr, png_inforp info_ptr, int transforms, voidp params) {
  ...
  if (info_ptr->row_pointers == NULL) {
    ...
    info_ptr->row_pointers =
        png_voidcast(png_bytepp, png_malloc(png_ptr, info_ptr->height * (sizeof(png_bytep))));
...
static int png_image_read_init(png_imagep image) {
  if (image->opaque == NULL) {
    png_structp png_ptr =
        png_create_read_struct(PNG_LIBPNG_VER_STRING, image, png_safe_error, png_safe_warning);
    ...
    if (png_ptr != NULL) {
      ...
      if (info_ptr != NULL) {
        png_controlp control =
            png_voidcast(png_controlp, png_malloc_warn(png_ptr, (sizeof *control)));
...
static void png_image_skip_unused_chunks(png_structrp png_ptr) {
   ...
   */
  {
    ...
    png_set_keep_unknown_chunks(png_ptr, PNG_HANDLE_CHUNK_AS_DEFAULT, chunks_to_process,
...
static void png_create_colormap_entry(png_image_read_control *display, png_uint_32 ip,
  ...
  if (encoding == P_LINEAR) {
    if (convert_to_Y != 0) {
      ...
      png_uint_32 y =
          (png_uint_32)6968 * red + (png_uint_32)23434 * green + (png_uint_32)2366 * blue;
        ...
        if (png_ptr->num_trans > 0) {
          ...
          if ((output_format & PNG_FORMAT_FLAG_ALPHA) != 0)
            ...
            if (back_r == back_g && back_g == back_b) {
               ...
               * doesn't.
               */
              png_set_background_fixed(png_ptr, &c, PNG_BACKGROUND_GAMMA_SCREEN, 0 /*need_expand*/,
                                       0 /*gamma: not used*/);
           ...
           */
          png_create_colormap_entry(display, 254, back_r, back_g, back_b, back_alpha,
                                    output_encoding);
        }
        ...
        if ((output_format & PNG_FORMAT_FLAG_COLOR) == 0 ||
          ...
          for (a = 1; a < 5; ++a) {
            ...
            for (g = 0; g < 6; ++g) {
              ...
              png_create_colormap_entry(display, i++, PNG_sRGB_FROM_LINEAR(gray + back_rx),
        ...
        if ((png_ptr->color_type == PNG_COLOR_TYPE_RGB_ALPHA || png_ptr->num_trans > 0) &&
          ...
          if (png_ptr->color_type == PNG_COLOR_TYPE_RGB_ALPHA || png_ptr->num_trans > 0) {
            ...
            if (data_encoding == P_FILE) /* from the fixup above */
            {
              ...
              gray = PNG_DIV257(
                  png_gamma_16bit_correct(gray, png_ptr->colorspace.gamma)); /* now P_FILE */

              /* And make sure the corresponding palette entry contains
               * exactly the required sRGB value.
               */
              png_create_colormap_entry(display, gray, back_g, back_g, back_g, 0 /*unused*/,
                                        output_encoding);
            }
      ...
      else /* output is color */
      {
        ...
        if (png_ptr->color_type == PNG_COLOR_TYPE_RGB_ALPHA || png_ptr->num_trans > 0) {
          ...
          if ((output_format & PNG_FORMAT_FLAG_ALPHA) != 0) {
            ...
            png_create_colormap_entry(display, cmap_entries, back_r, back_g, back_b, 0 /*unused*/,
                                      output_encoding);
            ...
            if (memcmp((png_const_bytep)display->colormap + sample_size * cmap_entries,
                       (png_const_bytep)display->colormap + sample_size * PNG_RGB_INDEX(r, g, b),
                       sample_size) != 0) {
                ...
                for (g = 0; g < 256; g = (g << 1) | 0x7f) {
                  ...
                  for (b = 0; b < 256; b = (b << 1) | 0x7f)
                    png_create_colormap_entry(
                        ...
                        png_colormap_compose(display, r, P_sRGB, 128, back_r, output_encoding),
                        png_colormap_compose(display, g, P_sRGB, 128, back_g, output_encoding),
                        png_colormap_compose(display, b, P_sRGB, 128, back_b, output_encoding),
       ...
       */
      {
        ...
        for (i = 0; i < cmap_entries; ++i) {
          if (do_background != 0 && i < num_trans && trans[i] < 255) {
            ...
            png_create_colormap_entry(display, i, colormap[i].red, colormap[i].green,
  ...
  /* Now check the format to see if it was modified. */
  {
    ...
    if ((change & PNG_FORMAT_FLAG_COLOR) != 0) {
      ...
      if ((format & PNG_FORMAT_FLAG_COLOR) != 0)
        ...
        png_set_rgb_to_gray_fixed(png_ptr, PNG_ERROR_ACTION_NONE, PNG_RGB_TO_GRAY_DEFAULT,
    ...
    if (do_local_background != 0) {
       ...
       * final value.
       */
      if (png_muldiv(&gtest, output_gamma, png_ptr->colorspace.gamma, PNG_FP_1) != 0 &&
...
int PNGAPI png_image_finish_read(png_imagep image, png_const_colorp background, void *buffer,
