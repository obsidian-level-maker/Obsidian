...
void PNGAPI png_set_background(png_structrp png_ptr, png_const_color_16p background_color,
                               int background_gamma_code, int need_expand,
...
void PNGAPI png_set_quantize(png_structrp png_ptr, png_colorp palette, int num_palette,
  ...
  if (full_quantize == 0) {
    ...
    png_ptr->quantize_index = (png_bytep)png_malloc(
        png_ptr, (png_alloc_size_t)((png_uint_32)num_palette * (sizeof(png_byte))));
  ...
  if (full_quantize != 0) {
    ...
    png_ptr->palette_lookup =
        (png_bytep)png_calloc(png_ptr, (png_alloc_size_t)(num_entries * (sizeof(png_byte))));
...
void PNGFAPI png_set_rgb_to_gray_fixed(png_structrp png_ptr, int error_action, png_fixed_point red,
  ...
  {
    {
      ...
      if ((png_ptr->transformations & PNG_EXPAND_tRNS) == 0) {
        png_ptr->trans_color.red = png_ptr->trans_color.green = png_ptr->trans_color.blue =
...
#undef CHOP
    ...
    if ((png_ptr->transformations & PNG_COMPOSE) != 0) {
      ...
      else /* color_type != PNG_COLOR_TYPE_PALETTE */
      {
        ...
        if ((png_ptr->background.red != png_ptr->background.green) ||
          ...
          png_ptr->background_1.red = png_ptr->background_1.green = png_ptr->background_1.blue =
  ...
  if ((png_ptr->transformations & PNG_SHIFT) != 0 && (png_ptr->transformations & PNG_EXPAND) == 0 &&
      (png_ptr->color_type == PNG_COLOR_TYPE_PALETTE)) {
    int i;
...
#ifdef PNG_READ_EXPAND_16_SUPPORTED
  if ((png_ptr->transformations & PNG_EXPAND_16) != 0 && info_ptr->bit_depth == 8 &&
      info_ptr->color_type != PNG_COLOR_TYPE_PALETTE) {
      ...
      if (row_info->bit_depth == 8) {
        ...
        if (gamma_table != NULL) {
          ...
          for (i = 0; i < row_width; i++, sp += 3) {
            if (*sp == png_ptr->trans_color.red && *(sp + 1) == png_ptr->trans_color.green &&
      ...
      } else /* if (row_info->bit_depth == 16) */
      {
        ...
        if (gamma_16 != NULL) {
          ...
          for (i = 0; i < row_width; i++, sp += 6) {
            ...
            if (r == png_ptr->trans_color.red && g == png_ptr->trans_color.green &&
      ...
      } else /* if (row_info->bit_depth == 16) */
      {
        ...
        if (gamma_16 != NULL && gamma_16_from_1 != NULL && gamma_16_to_1 != NULL) {
          ...
          for (i = 0; i < row_width; i++, sp += 8) {
            png_uint_16 a =
                (png_uint_16)(((png_uint_16)(*(sp + 6)) << 8) + (png_uint_16)(*(sp + 7)));
      ...
      case PNG_COLOR_TYPE_GRAY: {
        if (row_info->bit_depth == 2) {
          ...
          for (i = 0; i < row_width; i += 4) {
            ...
            *sp =
                (png_byte)(((((int)gamma_table[a | (a >> 2) | (a >> 4) | (a >> 6)])) & 0xc0) |
 ...
 */
static void png_do_expand_palette(png_structrp png_ptr, png_row_infop row_info, png_bytep row,
                                  png_const_colorp palette, png_const_bytep trans_alpha,
                                  int num_trans) {
  int shift, value;
  png_bytep sp, dp;
  png_uint_32 i;
    ...
    if (row_info->color_type == PNG_COLOR_TYPE_RGB && palette_lookup) {
      ...
      for (i = 0; i < row_width; i++) {
        ...
        p = (((r >> (8 - PNG_QUANTIZE_RED_BITS)) & ((1 << PNG_QUANTIZE_RED_BITS) - 1))
            ...
            (((g >> (8 - PNG_QUANTIZE_GREEN_BITS)) & ((1 << PNG_QUANTIZE_GREEN_BITS) - 1))
...
png_do_read_transformations(png_structrp png_ptr, png_row_infop row_info) {
...
#endif
      png_do_expand_palette(png_ptr, row_info, png_ptr->row_buf + 1, png_ptr->palette,
                            png_ptr->trans_alpha, png_ptr->num_trans);
    }
  ...
  if ((png_ptr->transformations & PNG_GAMMA) != 0 &&
       ...
       */
      !((png_ptr->transformations & PNG_COMPOSE) != 0 &&
        ((png_ptr->num_trans != 0) || (png_ptr->color_type & PNG_COLOR_MASK_ALPHA) != 0)) &&
#endif
