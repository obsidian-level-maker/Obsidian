...
void PNGAPI png_write_chunk_start(png_structrp png_ptr, png_const_bytep chunk_string,
                                  png_uint_32 length) {
...

  {
    ...
    if ((png_ptr->flags & PNG_FLAG_ZSTREAM_INITIALIZED) != 0 &&
        (png_ptr->zlib_set_level != level || png_ptr->zlib_set_method != method ||
         png_ptr->zlib_set_window_bits != windowBits || png_ptr->zlib_set_mem_level != memLevel ||
 ...
 */
void /* PRIVATE */
png_write_IHDR(png_structrp png_ptr, png_uint_32 width, png_uint_32 height, int bit_depth,
...
png_write_PLTE(png_structrp png_ptr, png_const_colorp palette, png_uint_32 num_pal) {
  ...
  max_palette_length = (png_ptr->color_type == PNG_COLOR_TYPE_PALETTE) ? (1 << png_ptr->bit_depth)
 ...
 */
void /* PRIVATE */
png_compress_IDAT(png_structrp png_ptr, png_const_bytep input, png_alloc_size_t input_len,
                  int flush) {
  if (png_ptr->zowner != png_IDAT) {
    ...
    if (png_ptr->zbuffer_list == NULL) {
      png_ptr->zbuffer_list = png_voidcast(
          png_compression_bufferp, png_malloc(png_ptr, PNG_COMPRESSION_BUFFER_SIZE(png_ptr)));
...
png_write_sBIT(png_structrp png_ptr, png_const_color_8p sbit, int color_type) {
  ...
  if ((color_type & PNG_COLOR_MASK_COLOR) != 0) {
    ...
    if (sbit->red == 0 || sbit->red > maxbits || sbit->green == 0 || sbit->green > maxbits ||
...
void /* PRIVATE */
png_write_tRNS(png_structrp png_ptr, png_const_bytep trans_alpha, png_const_color_16p tran,
...
/* Write an iTXt chunk */
void /* PRIVATE */
png_write_iTXt(png_structrp png_ptr, int compression, png_const_charp key, png_const_charp lang,
...
void /* PRIVATE */
png_write_pCAL(png_structrp png_ptr, png_charp purpose, png_int_32 X0, png_int_32 X1, int type,
  ...
  params_len = (png_size_tp)png_malloc(
      png_ptr, (png_alloc_size_t)((png_alloc_size_t)nparams * (sizeof(size_t))));
...
void /* PRIVATE */
png_write_pHYs(png_structrp png_ptr, png_uint_32 x_pixels_per_unit, png_uint_32 y_pixels_per_unit,
               int unit_type) {
  png_byte buf[9];
...
png_write_tIME(png_structrp png_ptr, png_const_timep mod_time) {
  ...
  if (mod_time->month > 12 || mod_time->month < 1 || mod_time->day > 31 || mod_time->day < 1 ||
...
png_write_start_row(png_structrp png_ptr) {
  ...
  if (png_ptr->interlaced != 0) {
    if ((png_ptr->transformations & PNG_INTERLACE) == 0) {
      ...
      png_ptr->usr_width =
          (png_ptr->width + png_pass_inc[0] - 1 - png_pass_start[0]) / png_pass_inc[0];
    }
...
png_do_write_interlace(png_row_infop row_info, png_bytep row, int pass) {
  ...
  if (pass < 6) {
    ...
    /* Set new row width */
    row_info->width =
        (row_info->width + png_pass_inc[pass] - 1 - png_pass_start[pass]) / png_pass_inc[pass];
...
png_setup_up_row(png_structrp png_ptr, size_t row_bytes, size_t lmins) {
  ...
  for (i = 0, rp = png_ptr->row_buf + 1, dp = png_ptr->try_row + 1, pp = png_ptr->prev_row + 1;
