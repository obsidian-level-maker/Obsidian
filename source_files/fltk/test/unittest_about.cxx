...
public:
  static Fl_Widget *create() { return new About(TESTAREA_X, TESTAREA_Y, TESTAREA_W, TESTAREA_H); }
  About(int x, int y, int w, int h)
    ...
    value(
        ...
        "<p>Designing a good user interface is an art. Widgets must be selected and carefully "
        "positioned "
        "to create a consistent look and feel for the user. Text must fit into given boxes and "
        "graphic "
        ...
        "<h3>The Developer</h3>\n"
        "<p>Unittest is also a great help when implementing new graphics drivers. The tests are "
        "sorted "
        "in the same order in which a new graphics driver could be implemented. Most tests rely "
        "on the previous test to function correctly, so sticking to the given order is a good "
        "idea.</p>\n"
        "<h3>Conventions</h3>\n"
        "<p>Two layers of graphics are drawn for most tests. The lower layer contains "
        "red and green pixels. The upper layer contains black pixels. The test is rendered "
        "correctly "
        "if all red pixels are covered, but none of the green pixels. The top graphics layer can "
        "be "
