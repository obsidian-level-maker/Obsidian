...
public:
  ...
  void draw() {
    ...
    if (damage() & FL_DAMAGE_ALL)
      draw_box(b, color());
    drawtext(x() + Fl::box_dx(b) + 3, y() + Fl::box_dy(b), w() - Fl::box_dw(b) - 6,
             h() - Fl::box_dh(b));
  }
...
int main(int argc, char **argv) {
  ...
  int l = fl_utf8froma(utf8, (unsigned int)strlen(latin1) * 5 + 1, latin1,
                       (unsigned int)strlen(latin1));
  make_font_chooser();
