...
public:
  MyWidget2(int x, int y)
    : Fl_Box(x, y, 100, 100, "Integer primitives") {
    labelsize(10);
    align(FL_ALIGN_TOP);
  }
};
...
public:
  MyWidget3(int x, int y)
    : Fl_Box(x, y, 100, 100,
             "Sub-pixel drawing of\nlines 1.63 points apart\nOn the screen you\ncan see aliasing, "
...
public:
  MyWidget4(int x, int y)
    : Fl_Box(x, y + 10, 150, 150, "Line styles") {
    labelsize(10);
    align(FL_ALIGN_TOP);
  }
};
...
public:
  MyWidget5(int x, int y)
    : Fl_Box(x, y, 230, 250,
             "Complex (double) drawings:\nBlue ellipse may not be\ncorrectly transformed\ndue to "
             "non-orthogonal\ntransformation") {
    labelsize(10);
    align(FL_ALIGN_TOP);
  }
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
      ...
      *p++ = uchar(255 * (X * Y));             // blue in lower-right
      ...
      if (alpha < 255)
        *p++ = uchar(alpha); // alpha transparency
      else
        *p++ = 255;
      Y += 0.5;
    }
...
void copy(Fl_Widget *, void *data) {
  ...
  if (strcmp(operation, "Fl_Copy_Surface") == 0) {
    ...
    if (target->as_window() && !target->parent()) {
      copy_surf = new Fl_Copy_Surface(target->as_window()->decorated_w(),
                                      target->as_window()->decorated_h());
  ...
  if (strcmp(operation, "Fl_EPS_File_Surface") == 0) {
    ...
    if (!fnfc.show()) {
      ...
      if (eps) {
        ...
        if (p.file()) {
          if (target->as_window())
            p.draw_decorated_window(target->as_window());
          else
            p.draw(target);
        }
      }
  ...
  if (strcmp(operation, "Fl_SVG_File_Surface") == 0) {
    ...
    if (!fnfc.show()) {
      ...
      if (svg) {
        ...
        if (surface.file()) {
          if (target->as_window())
            surface.draw_decorated_window(target->as_window());
          else
            surface.draw(target);
          if (surface.close())
            fl_message("Error while writing to SVG file %s", fnfc.filename());
        }
      }
    }
  }
...
public:
  My_Button(int x, int y, int w, int h, const char *label = 0)
...
int main(int argc, char **argv) {
  ...
  Fl_Group *group;
  {
    Fl_Group *o = new Fl_Group(360, 495 + 16, 120, 40);
    group = o;
    o->box(FL_UP_BOX);
    {
  ...
  Fl_Box tx(120, 492 + 16, 230, 50,
            "Background is not printed because\nencapsulating group, which we are\n printing, has "
            "not set the box type");
  ...
  rb->set();
  rb->callback(operation_cb, NULL);
  operation = rb->label();
  rb->labelsize(12);
  rb = new Fl_Radio_Round_Button(170, 4, 150, 12, "Fl_Copy_Surface");
  rb->callback(operation_cb, NULL);
  rb->labelsize(12);
  rb = new Fl_Radio_Round_Button(5, 17, 150, 12, "Fl_Printer");
  rb->callback(operation_cb, NULL);
  rb->labelsize(12);
  rb = new Fl_Radio_Round_Button(170, 17, 150, 12, "Fl_PostScript_File_Device");
  rb->callback(operation_cb, NULL);
  rb->labelsize(12);
  rb = new Fl_Radio_Round_Button(5, 30, 150, 12, "Fl_EPS_File_Surface");
  rb->callback(operation_cb, NULL);
  rb->labelsize(12);
  rb = new Fl_Radio_Round_Button(170, 30, 150, 12, "Fl_SVG_File_Surface");
  rb->callback(operation_cb, NULL);
  rb->labelsize(12);
  rb = new Fl_Radio_Round_Button(5, 43, 150, 12, "fl_capture_window()");
  rb->callback(operation_cb, NULL);
  rb->labelsize(12);
  g1->end();
