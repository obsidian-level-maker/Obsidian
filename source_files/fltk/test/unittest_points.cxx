...
public:
  ...
  PointTest(int x, int y, int w, int h)
    : Fl_Group(x, y, w, h) {
    ...
    t->tooltip( // Title:
        ...
        "Black and white points should be the same size of one unit (1 pixel in regular mode, 2x2 "
        "pixels in hidpi mode)."
        "If black points are smaller than white in hidpi mode, point size must be increased.\n\n"
        "Points should not be blurry. Antialiasing should be switched of and the point coordinates "
        "should be centered on the pixel(s).\n\n"
    ...
    t->tooltip( // Title:
        ...
        // Things to look out for:
        "If the order of colors is different, the byte order when writing into the pixel buffer "
        "should be fixed.");
    ...
    t->tooltip( // Title:
        ...
        // Things to look out for:
        "If parts of the black frame are clipped by the window and not visible, pixel offsets must "
        "be adjusted.");
    ...
    t->tooltip( // Title:
        ...
        "This draws three squares in red, green, and blue.\n\n"
        // Things to look out for:
        "If the order of colors is different, the color component order when writing into the "
        "pixel buffer should be fixed.");
