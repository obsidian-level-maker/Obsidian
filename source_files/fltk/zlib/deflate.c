...
z_streamp strm;
{
  ...
  if (s == Z_NULL || s->strm != strm ||
      (s->status != INIT_STATE &&
       ...
       s->status != GZIP_STATE &&
#endif
       s->status != EXTRA_STATE && s->status != NAME_STATE && s->status != COMMENT_STATE &&
       s->status != HCRC_STATE && s->status != BUSY_STATE && s->status != FINISH_STATE))
    return 1;
  return 0;
}
...
int flush;
{
  ...
  if (left >= min_block || ((left || flush == Z_FINISH) && flush != Z_NO_FLUSH &&
