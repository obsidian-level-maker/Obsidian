...
/* Process a SOFn marker */
{
  ...
  for (ci = 0; ci < cinfo->num_components; ci++) {
    ...
    TRACEMS4(cinfo, 1, JTRC_SOF_COMPONENT, compptr->component_id, compptr->h_samp_factor,
...
/* Process a SOS marker */
{
  ...
  id_found:
    ...
    TRACEMS3(cinfo, 1, JTRC_SOS_COMPONENT, compptr->component_id, compptr->dc_tbl_no,
             compptr->ac_tbl_no);
  }
...
/* Process a DHT marker */
{
  ...
  while (length > 16) {
    ...
    TRACEMS8(cinfo, 2, JTRC_HUFFBITS, bits[1], bits[2], bits[3], bits[4], bits[5], bits[6], bits[7],
             bits[8]);
    TRACEMS8(cinfo, 2, JTRC_HUFFBITS, bits[9], bits[10], bits[11], bits[12], bits[13], bits[14],
             bits[15], bits[16]);
  ...
  while (length > 0) {
    ...
    if (cinfo->err->trace_level >= 2) {
      for (i = 0; i < DCTSIZE2; i += 8) {
        TRACEMS8(cinfo, 2, JTRC_QUANTVALS, quant_ptr->quantval[i], quant_ptr->quantval[i + 1],
 ...
 */
{
  ...
  if (datalen >= APP0_DATA_LEN && GETJOCTET(data[0]) == 0x4A && GETJOCTET(data[1]) == 0x46 &&
    ...
    TRACEMS5(cinfo, 1, JTRC_JFIF, cinfo->JFIF_major_version, cinfo->JFIF_minor_version,
...
read_markers(j_decompress_ptr cinfo) {
  ...
  for (;;) {
    ...
    switch (cinfo->unread_marker) {
      ...
      case M_APP14:
      case M_APP15:
        if (!(*((my_marker_ptr)cinfo->marker)->process_APPn[cinfo->unread_marker - (int)M_APP0])(
                cinfo))
          return JPEG_SUSPENDED;
        break;
