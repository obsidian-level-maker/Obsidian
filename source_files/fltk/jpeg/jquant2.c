...
METHODDEF(void)
prescan_quantize(j_decompress_ptr cinfo, JSAMPARRAY input_buf, JSAMPARRAY output_buf,
                 int num_rows) {
...
LOCAL(void)
find_best_colors(j_decompress_ptr cinfo, int minc0, int minc1, int minc2, int numcolors,
  ...
  if (is_pre_scan) {
    ...
    if (cinfo->dither_mode == JDITHER_FS) {
      ...
      if (cquantize->fserrors == NULL)
        cquantize->fserrors =
            (FSERRPTR)(*cinfo->mem->alloc_large)((j_common_ptr)cinfo, JPOOL_IMAGE, arraysize);
...
jinit_2pass_quantizer(j_decompress_ptr cinfo) {
  ...
  cquantize->histogram = (hist3d)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE,
  ...
  if (cinfo->enable_2pass_quant) {
    ...
    if (desired > MAXNUMCOLORS)
      ERREXIT1(cinfo, JERR_QUANT_MANY_COLORS, MAXNUMCOLORS);
    cquantize->sv_colormap = (*cinfo->mem->alloc_sarray)((j_common_ptr)cinfo, JPOOL_IMAGE,
