...
/* Allocate a 2-D sample array */
{
  ...
  while (currow < numrows) {
    ...
    workspace = (JSAMPROW)alloc_large(
        cinfo, pool_id, (size_t)rowsperchunk * (size_t)samplesperrow * SIZEOF(JSAMPLE));
...
/* Allocate a 2-D coefficient-block array */
{
  ...
  while (currow < numrows) {
    ...
    workspace = (JBLOCKROW)alloc_large(
        cinfo, pool_id, (size_t)rowsperchunk * (size_t)blocksperrow * SIZEOF(JBLOCK));
    for (i = rowsperchunk; i > 0; i--) {
      result[currow++] = workspace;
...
/* Do backing store read or write of a virtual sample array */
{
  ...
  for (i = 0; i < (long)ptr->rows_in_mem; i += ptr->rowsperchunk) {
    ...
    if (writing)
      (*ptr->b_s_info.write_backing_store)(cinfo, &ptr->b_s_info, (void FAR *)ptr->mem_buffer[i],
                                           file_offset, byte_count);
    else
      (*ptr->b_s_info.read_backing_store)(cinfo, &ptr->b_s_info, (void FAR *)ptr->mem_buffer[i],
                                          file_offset, byte_count);
    file_offset += byte_count;
  }
}
...
METHODDEF(JSAMPARRAY)
access_virt_sarray(j_common_ptr cinfo, jvirt_sarray_ptr ptr, JDIMENSION start_row,
...
METHODDEF(JBLOCKARRAY)
access_virt_barray(j_common_ptr cinfo, jvirt_barray_ptr ptr, JDIMENSION start_row,
                   JDIMENSION num_rows, boolean writable)
