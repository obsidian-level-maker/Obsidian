...
#
# * SOURCES: list of source files, separated by ';' (needs quotes) Sources can
#   be: - .c/.cxx files, e.g. 'hello.cxx' - .fl (fluid) files, e.g. 'radio.fl' -
#   .plist file (macOS), e.g. 'editor.plist' - .icns file (macOS Icon), e.g.
#   'checkers.icns' - .rc file (Windows resource file, e.g. icon definition)
#
# Order of sources doesn't matter, multiple .cxx and .fl files are supported,
# but only one .plist and one .icns file.
#
# File name (type), e.g. '.icns' matters, it is parsed internally: File types
# .fl, .plist, and .icns are treated specifically, all other file types are
# added to the target's source files.
#
# macOS specific .icns and .plist files are ignored on other platforms. These
# files must resided in the subdirectory 'mac-resources'.
#
# * LIBRARIES: List of libraries (CMake target names), separated by ';'. Needs
...
# the option ANDROID_OK is set if CREATE_EXAMPLE creates code for Android builds
# in addition to the native build
#
# ##############################################################################
...
macro(CREATE_EXAMPLE NAME SOURCES LIBRARIES)
  ...
  # rename target name "help" (reserved since CMake 2.8.12) FIXME: not necessary
  # in FLTK 1.4 but left for compatibility (06/2020)
  ...
  if(MAC_BUNDLE)
    add_executable(${TARGET_NAME} MACOSX_BUNDLE ${ICON_PATH} ${srcs})
  else()
    add_executable(${TARGET_NAME} WIN32 ${ICON_PATH} ${srcs})
  endif(MAC_BUNDLE)
  ...
  if(ICON_PATH)
    set_target_properties(
      ${TARGET_NAME} PROPERTIES MACOSX_BUNDLE_ICON_FILE ${ICON_NAME}
    )
  ...
  elseif(MAC_BUNDLE)
    set_target_properties(
      ${TARGET_NAME} PROPERTIES MACOSX_BUNDLE_BUNDLE_NAME "${TARGET_NAME}"
    )
    set_target_properties(
      ${TARGET_NAME} PROPERTIES MACOSX_BUNDLE_GUI_IDENTIFIER
                                "org.fltk.${TARGET_NAME}"
    )
  endif()

  # ############################################################################
  # Copy macOS "bundle wrapper" (shell script) to target directory. The "custom
  # command" will be executed "POST_BUILD".
  # ############################################################################
  ...
  if(MAC_BUNDLE)
    ...
    add_custom_command(
      TARGET ${TARGET_NAME}
      POST_BUILD
      COMMAND cp ${CMAKE_CURRENT_SOURCE_DIR}/../CMake/macOS-bundle-wrapper.in
              ${WRAPPER}
