...
protected:
  ...
  const struct tm *GetTimeStruct() {
    time_t t = time(NULL);
    if (strcmp(time_format, "Local") == 0)
      return localtime(&t);
    if (strcmp(time_format, "GMT") == 0)
      return gmtime(&t);
    return 0;
  }

public:
  MyTimeItem(Fl_Tree *tree, const char *time_format)
    : Fl_Tree_Item(tree) {
    label(time_format);
  ...
  int draw_item_content(int render) {
    ...
    if (render)
      fl_draw(s, X, Y + 4, W, H, FL_ALIGN_LEFT | FL_ALIGN_TOP);
    sprintf(s, "Time: %02d:%02d:%02d", tm->tm_hour, tm->tm_min, tm->tm_sec);
    if (render)
      fl_draw(s, X, Y + H / 2, W, H / 2, FL_ALIGN_LEFT | FL_ALIGN_TOP);
    int lw2 = 0, lh2 = 0;
