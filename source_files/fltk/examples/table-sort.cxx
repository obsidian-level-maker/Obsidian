...
void MyTable::autowidth(int pad) {
  ...
  for (int r = 0; r < (int)rowdata_.size(); r++) {
    for (int c = 0; c < (int)rowdata_[r].cols.size(); c++) {
      w = 0;
      fl_measure(rowdata_[r].cols[c].c_str(), w, h, 0); // pixel width of row text
...
void MyTable::load_command(const char *cmd) {
  ...
  for (int line = 0; fgets(s, sizeof(s) - 1, fp); line++) {
    ...
    // WINDOWS
    if (line < 5)
      continue; // skip DIR's 5 line header
#else
    // UNIX
    if (line == 0 && strncmp(s, "total ", 6) == 0)
      continue;
#endif
    // Add a new row
    Row newrow;
    rowdata_.push_back(newrow);
