//
//
// Demonstrate resizing Fl_Table_Row right column to fit window to avoid appearance of horiz
// scrollbar
//
//     Date      Author   Description
//     --------  ------   -----------
//     01/03/17  erco     Modified version of examples/simple-table.cxx for Duncan's question on
//     fltk.general
      ...
      case CONTEXT_CELL: // Draw data in cells
        switch (COL) {
          case 0:
            ...
            break;
          case 1:
            sprintf(s, " 2017-01-02 / 09:20:25.%d", ROW);
            DrawData(s, X, Y, W, H);
            break;
        }
        return;
...
public:
  ...
  void FixColumnSize() {
    ...
    int off = (X + W) -
              (tox + tow);   // we just need X pos and width. Compute offset from table's outer size
...
int main(int argc, char **argv) {
  ...
  Fl_Button more(10, 360, 100, 25, "+ Row");
  more.callback(More_CB, (void *)table);
  Fl_Button less(220, 360, 100, 25, "- Row");
  less.callback(Less_CB, (void *)table);
  win->end();
  win->resizable(table);
  win->show(argc, argv);
  return (Fl::run());
