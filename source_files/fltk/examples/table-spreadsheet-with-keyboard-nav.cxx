...
//
//      Fl_Table[1.00/LGPL] 04/18/03 Mister Satan      -- Initial implementation, submitted to erco
//      for Fl_Table Fl_Table[1.10/LGPL] 05/17/03 Greg Ercolano     -- Small mods to follow changes
//      to Fl_Table Fl_Table[1.20/LGPL] 02/22/04 Jean-Marc Lienher -- Keyboard nav and mouse
//      selection Fl_Table[1.21/LGPL] 02/22/04 Greg Ercolano     -- Small reformatting mods,
//      comments
//         FLTK[1.3.0/LGPL] 10/26/10 Greg Ercolano     -- Moved from Fl_Table to FLTK 1.3.x, CMP
//         compliance
//
...
public:
  ...
  void start_editing(int R, int C) {
    ...
    char s[30];
    sprintf(s, "%d", values[R][C]); // Load input widget with cell's current value
    input->value(s);
