/*
 * Copyright 2000 David Chess; Copyright 2005-2007 Sam Trenholme
 *
...
#else
  *(int *)tbuf = swap_32s(texturecount); tbuf += sizeof(int);
#endif
...
#else
    *(int *)tbuf = swap_32s(isize);  tbuf += sizeof(int);
#endif
...
#else
    *(short *)tbuf = swap_16(ct->xsize);  tbuf += sizeof(short);
    *(short *)tbuf = swap_16(ct->ysize);  tbuf += sizeof(short);
#endif
    *(short *)tbuf = 0;  tbuf += sizeof(short);
    *(short *)tbuf = 0;  tbuf += sizeof(short);
...
#else
    *(short *)tbuf = swap_16(patchcount);  tbuf += sizeof(short);
#endif
...
#else
      *(short *)tbuf = swap_16(p->x);  tbuf += sizeof(short);
      *(short *)tbuf = swap_16(p->y);  tbuf += sizeof(short);
      *(short *)tbuf = swap_16(p->number);  tbuf += sizeof(short);
      *(short *)tbuf = swap_16(1);  tbuf += sizeof(short);
#endif
      *(short *)tbuf = 0;  tbuf += sizeof(short);
    }
  }
...
#else
    *(short *)p = swap_16(columns);         p += sizeof(short);   /* Width */
    *(short *)p = swap_16(rows);            p += sizeof(short);   /* Height */
    *(short *)p = swap_16((columns>>1)-1);  p += sizeof(short);   /* Width offset */
    *(short *)p = swap_16(rows-5);          p += sizeof(short);   /* Magic */
#endif
...
#ifndef ENDIAN_BIG
      *(int *)p = z;  p += sizeof(int);
#else
      *(int *)p = swap_32s(z);  p += sizeof(int);
#endif
    }
