...
static void correctstack(lua_State *L, StkId oldstack, StkId newstack) {
    CallInfo *ci;
    UpVal *up;
    if (oldstack == newstack) return; /* stack address did not change */
...
int luaD_reallocstack(lua_State *L, int newsize, int raiseerror) {
    int lim = stacksize(L);
    StkId newstack = luaM_reallocvector(L, L->stack, lim + EXTRA_STACK,
...
void luaD_hook(lua_State *L, int event, int line, int ftransfer,
    ...
    if (hook && L->allowhook) { /* make sure there is a hook */
        ...
        if (L->top + LUA_MINSTACK > ci->top) ci->top = L->top + LUA_MINSTACK;
...
static StkId rethook(lua_State *L, CallInfo *ci, StkId firstres, int nres) {
    ...
    if (isLuacode(ci)) {
        ...
        if (L->top < ci->top) L->top = ci->top; /* correct top to run hook */
    }
    ...
    switch (wanted) { /* handle typical cases separately */
        ...
        default: /* multiple results (or to-be-closed variables) */
            if (hastocloseCfunc(wanted)) { /* to-be-closed variables? */
                ...
                if (wanted == LUA_MULTRET) wanted = nres;
            }
            break;
    }
...
void luaD_poscall(lua_State *L, CallInfo *ci, int nres) {
    if (L->hookmask) L->top = rethook(L, ci, L->top - nres, nres);
...
static void ccall(lua_State *L, StkId func, int nResults, int inc) {
    CallInfo *ci;
    L->nCcalls += inc;
    if (unlikely(getCcalls(L) >= LUAI_MAXCCALLS)) luaE_checkcstack(L);
