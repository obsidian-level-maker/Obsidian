...
typedef enum { ERRINFO_GCC, ERRINFO_VCC } errorInfo_e;
...
static struct {
    ...
    {ERR_PREVIOUS_NOT_VOID,
     "Previous use of function expected a return value."},
    ...
    {ERR_NOT_A_CHAR_ARRAY,
     "%s has %d dimensions. Use %d subscripts to get a char array."},
    ...
    {ERR_SPECIAL_RANGE,
     "Line specials with values higher than 255 require #nocompact."},
...
void ERR_ErrorV(error_t error, boolean info, va_list args) {
    ...
    if (info == YES) {
        ...
        if (showedInfo == NO) {  // Output info compatible with older ACCs
    ...
    if (error != ERR_NONE) {
        ...
        eprintf("\n");
        if (showLine) {
            // deal with master source line and position indicator - Ty
            // 07jan2000
            MasterSourceLine[MasterSourcePos] =
                '\0';                                 // pre-incremented already
