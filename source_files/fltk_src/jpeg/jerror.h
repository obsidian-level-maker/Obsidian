    ...
    JMESSAGE(JERR_BAD_ALIGN_TYPE, "ALIGN_TYPE is wrong, please fix") JMESSAGE(
        ...
        "MAX_ALLOC_CHUNK is wrong, please fix") JMESSAGE(JERR_BAD_BUFFER_MODE,
                                                         ...
                                                         "mode") JMESSAGE(JERR_BAD_COMPONENT_ID,
                                                                          ...
                                                                          "%d "
                                                                          "in "
                                                                          "SOS") JMESSAGE(JERR_BAD_CROP_SPEC,
        ...
        JMESSAGE(JERR_BAD_DCT_COEF, "DCT coefficient out of range") JMESSAGE(
            ...
            JMESSAGE(JERR_BAD_HUFF_TABLE, "Bogus Huffman table definition") JMESSAGE(
                ...
                JMESSAGE(JERR_BAD_LENGTH, "Bogus marker length") JMESSAGE(
                    ...
                    "Wrong JPEG library version: library is %d, caller expects "
                    "%d") JMESSAGE(JERR_BAD_MCU_SIZE,
                                   "Sampling factors too large for interleaved "
                                                        ...
                                                        JMESSAGE(
                                                            ...
                                                            "change") JMESSAGE(JERR_NOTIMPL,
                                                                               "Not implemented yet") JMESSAGE(JERR_NOT_COMPILED,
                                                                                                               "Requested feature was omitted at compile time") JMESSAGE(JERR_NO_ARITH_TABLE,
...
#define ERREXIT6(cinfo, code, p1, p2, p3, p4, p5, p6)                        \
    ((cinfo)->err->msg_code = (code), (cinfo)->err->msg_parm.i[0] = (p1),    \
     (cinfo)->err->msg_parm.i[1] = (p2), (cinfo)->err->msg_parm.i[2] = (p3), \
     (cinfo)->err->msg_parm.i[3] = (p4), (cinfo)->err->msg_parm.i[4] = (p5), \
...
#define TRACEMS3(cinfo, lvl, code, p1, p2, p3)                              \
    MAKESTMT(int *_mp = (cinfo)->err->msg_parm.i; _mp[0] = (p1);            \
             _mp[1] = (p2); _mp[2] = (p3); (cinfo)->err->msg_code = (code); \
             (*(cinfo)->err->emit_message)((j_common_ptr)(cinfo), (lvl));)
...
#define TRACEMS5(cinfo, lvl, code, p1, p2, p3, p4, p5)                   \
    MAKESTMT(int *_mp = (cinfo)->err->msg_parm.i; _mp[0] = (p1);         \
             _mp[1] = (p2); _mp[2] = (p3); _mp[3] = (p4); _mp[4] = (p5); \
             (cinfo)->err->msg_code = (code);                            \
             (*(cinfo)->err->emit_message)((j_common_ptr)(cinfo), (lvl));)
#define TRACEMS8(cinfo, lvl, code, p1, p2, p3, p4, p5, p6, p7, p8)       \
    MAKESTMT(int *_mp = (cinfo)->err->msg_parm.i; _mp[0] = (p1);         \
             _mp[1] = (p2); _mp[2] = (p3); _mp[3] = (p4); _mp[4] = (p5); \
             _mp[5] = (p6); _mp[6] = (p7); _mp[7] = (p8);                \
             (cinfo)->err->msg_code = (code);                            \
             (*(cinfo)->err->emit_message)((j_common_ptr)(cinfo), (lvl));)
