...
/* Normal case, sYCC */
{
    ...
    for (i = 0, x = -CENTERJSAMPLE; i <= MAXJSAMPLE; i++, x++) {
        ...
        cconvert->Cr_r_tab[i] =
            (int)RIGHT_SHIFT(FIX(1.402) * x + ONE_HALF, SCALEBITS);
        /* Cb=>B value is nearest int to 1.772 * x */
        cconvert->Cb_b_tab[i] =
            (int)RIGHT_SHIFT(FIX(1.772) * x + ONE_HALF, SCALEBITS);
...
/* Wide gamut case, bg-sYCC */
{
    ...
    cconvert->range_limit = (JSAMPLE *)(*cinfo->mem->alloc_small)(
    ...
    for (i = 0, x = -CENTERJSAMPLE; i <= MAXJSAMPLE; i++, x++) {
        ...
        cconvert->Cr_r_tab[i] =
            (int)RIGHT_SHIFT(FIX(2.804) * x + ONE_HALF, SCALEBITS);
        /* Cb=>B value is nearest int to 3.544 * x */
        cconvert->Cb_b_tab[i] =
            (int)RIGHT_SHIFT(FIX(3.544) * x + ONE_HALF, SCALEBITS);
    ...
    for (i = 0; i <= MAXJSAMPLE; i++) cconvert->range_limit[i] = (JSAMPLE)i;
    /* End of range limit table: limit[x] = MAXJSAMPLE for x > MAXJSAMPLE */
    for (; i < 3 * (MAXJSAMPLE + 1); i++) cconvert->range_limit[i] = MAXJSAMPLE;
}
...
ycc_rgb_convert(j_decompress_ptr cinfo, JSAMPIMAGE input_buf,
    ...
    while (--num_rows >= 0) {
        ...
        for (col = 0; col < num_cols; col++) {
             ...
             * losses, for extended gamut (sYCC) and wide gamut (bg-sYCC)
             * encodings.
             */
...
METHODDEF(void)
rgb1_rgb_convert(j_decompress_ptr cinfo, JSAMPIMAGE input_buf,
                 JDIMENSION input_row, JSAMPARRAY output_buf, int num_rows) {
    register int r, g, b;
    register JSAMPROW outptr;
    register JSAMPROW inptr0, inptr1, inptr2;
...
METHODDEF(void)
rgb1_gray_convert(j_decompress_ptr cinfo, JSAMPIMAGE input_buf,
                  JDIMENSION input_row, JSAMPARRAY output_buf, int num_rows) {
    my_cconvert_ptr cconvert = (my_cconvert_ptr)cinfo->cconvert;
    register INT32 *ctab = cconvert->rgb_y_tab;
    register int r, g, b;
...
METHODDEF(void)
rgb_convert(j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION input_row,
...
ycck_cmyk_convert(j_decompress_ptr cinfo, JSAMPIMAGE input_buf,
    ...
    while (--num_rows >= 0) {
        ...
        for (col = 0; col < num_cols; col++) {
            ...
            cr = GETJSAMPLE(inptr2[col]);
            /* Range-limiting is essential due to noise introduced by DCT
             * losses, and for extended gamut encodings (sYCC).
             */
