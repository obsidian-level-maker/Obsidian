...
/* Returns the precision used (0 = 8bits, 1 = 16bits) for baseline checking */
{
    ...
    for (i = 0; i <= cinfo->lim_Se; i++) {
        if (qtbl->quantval[cinfo->natural_order[i]] > 255) prec = 1;
    }

    if (!qtbl->sent_table) {
        emit_marker(cinfo, M_DQT);

        emit_2bytes(cinfo, prec ? cinfo->lim_Se * 2 + 2 + 1 + 2
                                : cinfo->lim_Se + 1 + 1 + 2);
...
/* one DAC marker.  Therefore this routine does its own scan of the table. */
{
    ...
    for (i = 0; i < cinfo->comps_in_scan; i++) {
        ...
        if (cinfo->Se) ac_in_use[compptr->ac_tbl_no] = 1;
    }
    ...
    if (length) {
        ...
        for (i = 0; i < NUM_ARITH_TBLS; i++) {
            if (dc_in_use[i]) {
                emit_byte(cinfo, i);
                emit_byte(cinfo,
                          cinfo->arith_dc_L[i] + (cinfo->arith_dc_U[i] << 4));
            }
            if (ac_in_use[i]) {
...
write_frame_header(j_compress_ptr cinfo) {
    ...
    if (cinfo->color_transform) emit_lse_ict(cinfo);
...
write_scan_header(j_compress_ptr cinfo) {
    ...
    if (cinfo->arith_code) {
        ...
        for (i = 0; i < cinfo->comps_in_scan; i++) {
            ...
            /* AC needs no table when not present */
            if (cinfo->Se) emit_dht(cinfo, compptr->ac_tbl_no, TRUE);
        }
    }
