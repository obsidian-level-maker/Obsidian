...
Fl_Pixmap *pixmap[] = {0,
                       ...
                       &button_pixmap,
                       &checkbutton_pixmap,
                       &roundbutton_pixmap, /* 0..4 */
                       &box_pixmap,
                       &group_pixmap,
                       &function_pixmap,
                       &code_pixmap,
                       &codeblock_pixmap,
                       &declaration_pixmap, /* 5..10 */
                       ...
                       &lightbutton_pixmap,
                       &repeatbutton_pixmap,
                       &menubutton_pixmap, /* 22..26 */
                       &output_pixmap,
                       &textdisplay_pixmap,
                       &textedit_pixmap,
                       &fileinput_pixmap,
                       &browser_pixmap, /* 27..32 */
                       &checkbrowser_pixmap,
                       &filebrowser_pixmap,
                       &clock_pixmap,
                       &help_pixmap,
...
void select_only(Fl_Type *o) { widget_browser->select_only(o, 1); }
...
void Widget_Browser::item_draw(void *v, int X, int Y, int, int) const {
    ...
    if (show_comments && l->comment()) {
        ...
        if (l->new_selected)
            ...
            fl_color(fl_contrast(FL_DARK_GREEN, color()));
    ...
    if (l->is_parent()) {
        ...
        if (!l->next || l->next->level <= l->level) {
            if (l->open_ != (l == pushedtitle)) {
                // an outlined triangle to the right indicates closed item, no
                // children
                fl_loop(X, Y + 7, X + 5, Y + 12, X + 10, Y + 7);
            } else {
                // an outlined triangle to the bottom indicates open item, no
                // children
                fl_loop(X + 2, Y + 2, X + 7, Y + 7, X + 2, Y + 12);
            }
        } else {
            if (l->open_ != (l == pushedtitle)) {
                // a filled triangle to the right indicates closed item, with
                // children
                fl_polygon(X, Y + 7, X + 5, Y + 12, X + 10, Y + 7);
            } else {
                // a filled triangle to the bottom indicates open item, with
                // children
    ...
    switch (l->is_public()) {
        case 0:
            lock_pixmap.draw(X - 17, Y);
            break;
        case 2:
            protected_pixmap.draw(X - 17, Y);
            break;
    }

    // Indent=12 per level: Now write the text that comes after the graphics
    // representation
    Y += comment_incr;
    if (l->is_widget() || l->is_class()) {
        ...
        fl_font(textfont() | (l->is_code_block() &&
                                      (l->level == 0 || l->parent->is_class())
                                  ? 0
                                  : FL_BOLD),
                textsize());
        fl_draw(buf, X, Y + 13);
...
void redraw_browser() { widget_browser->redraw(); }

void Widget_Browser::callback() { selection_changed((Fl_Type *)selection()); }
    ...
    switch (e) {
        ...
        case FL_RELEASE:
            if (!title) {
                l = (Fl_Type *)find_item(Fl::event_y());
                if (l && l->new_selected &&
                    (Fl::event_clicks() || Fl::event_state(FL_CTRL)))
                    l->open();
                break;
            }
            l = pushedtitle;
...
int storestring(const char *n, const char *&p, int nostrip) {
    ...
    if (n) {  // see if blank, strip leading & trailing blanks
        if (!nostrip)
            while (isspace((int)(unsigned char)*n)) n++;
        const char *e = n + strlen(n);
        if (!nostrip)
            while (e > n && isspace((int)(unsigned char)*(e - 1))) e--;
        length = e - n;
        if (!length) n = 0;
    }
    if (n == p) return 0;
...
void Fl_Type::callback(const char *n) { storestring(n, callback_); }

void Fl_Type::user_data(const char *n) { storestring(n, user_data_); }

void Fl_Type::user_data_type(const char *n) { storestring(n, user_data_type_); }
...
static void delete_children(Fl_Type *p) {
    Fl_Type *f;
    for (f = p; f && f->next && f->next->level > p->level;
         f = f->next) { /*empty*/
    }
    for (; f != p;) {
        Fl_Type *g = f->prev;
...
/**
  Return 1 if the list contains a function with the given signature at the top
  level.
 */
int has_toplevel_function(const char *rtype, const char *sig) {
    Fl_Type *child;
    for (child = Fl_Type::first; child; child = child->next) {
        if (!child->is_in_class() &&
            strcmp(child->type_name(), "Function") == 0) {
            const Fl_Function_Type *fn = (const Fl_Function_Type *)child;
            if (fn->has_signature(rtype, sig)) return 1;
        }
    }
    return 0;
}
...
/**
  ...
  \see leave_live_mode()
*/
Fl_Widget *Fl_Type::enter_live_mode(int) { return 0L; }
    ...
    for (Fl_Type *p = Fl_Type::first; p; p = p->next)
        if (strcmp(p->type_name(), "Function") == 0 && p->name() != 0)
            if (strncmp(p->name(), cbname, strlen(cbname)) == 0)
                if (p->name()[strlen(cbname)] == '(') return 1;
    return 0;
}
