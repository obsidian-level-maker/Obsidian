...
const char *c_check(const char *c, int type) { return _c_check(c, type); }
...
void Fl_Function_Type::write_code1() {
    ...
    // the function either. This is great if you decide to implement that
    ...
    if (havechildren) write_c("\n");
    if (ismain()) {
        if (havechildren) write_c("int main(int argc, char **argv) {\n");
    } else {
        ...
        if (k) {
            if (havechildren) write_comment_c();
            ...
            if (!constructor) {
                ...
                if (havechildren) write_c("%s%s ", rtype, star);
            }
            ...
            *sptr = '\0';
            ...
            if (public_) {
                ...
                if (havechildren) write_c("static ");
            }
            ...
            *sptr = '\0';
            ...
            if (havechildren) write_c("%s%s %s {\n", rtype, star, s);
        }
    }

    if (havewidgets && !child->name())
        write_c("  %s* w;\n", subclassname(child));
    indentation += 2;
}
...
void Fl_Code_Type::write() {
    ...
    if (handle_editor_changes() == 1) {
        main_window->redraw();  // tell fluid to redraw; edits may affect tree's
                                // contents
    }
    Fl_Type::write();
}
    ...
    for (;;) {  // repeat as long as there are errors
        ...
        for (;;) {
            ...
            if (w == data_panel_cancel)
                ...
                goto_source_dir();
                const char *fn = fl_file_chooser("Load Binary Data", 0L,
                                                 data_filename->value(), 1);
                leave_source_dir();
                if (fn) {
                    if (strcmp(fn, data_filename->value())) set_modflag(1);
...
void Fl_Comment_Type::open() {
    ...
    {
        int i = 0, n = 0;
        Fl_Preferences menu(Fl_Preferences::USER, "fltk.org",
                            "fluid_comments_menu");
                ...
                if (comment_predefined->value() == 1) {
                    ...
                    if (xname) {
                        ...
                        for (char *s = name; *s; s++)
                            if (*s == ':') *s = ';';
                        int n;
                        Fl_Preferences db(Fl_Preferences::USER, "fltk.org",
                                          "fluid_comments");
                    ...
                    if (itempath[0] == 0 || last_selected_item == 0) {
                    ...
                    } else if (fl_choice("Are you sure that you want to delete "
                                         "the entry\n"
                                         "\"%s\"\nfrom the database?",
                        ...
                        Fl_Preferences menu(Fl_Preferences::USER, "fltk.org",
                                            "fluid_comments_menu");
                        int i, n;
                        for (i = 4, n = 0; i < comment_predefined->size();
                             i++) {
                            const Fl_Menu_Item *mi =
                                comment_predefined->menu() + i;
                            if (comment_predefined->item_pathname(itempath, 255,
                                                                  mi) == 0) {
                                if (itempath[0] == '/')
                                    memmove(itempath, itempath + 1, 255);
                                if (itempath[0])
                                    menu.set(Fl_Preferences::Name(n++),
                                             itempath);
                            }
                        }
                        menu.set("n", n);
...
const char *Fl_Type::class_name(const int need_nest) const {
    ...
    while (p) {
        if (p->is_class()) {
            // see if we are nested in another class, we must fully-qualify
            // name: this is lame but works...
            const char *q = 0;
...
/**
 * If this Type resides inside a class, this function returns the class type, or
 * null.
 */
