    ...
    if (editors_open > 0) {
        ...
        for (Fl_Type *p = Fl_Type::first; p; p = p->next) {
            if (p->is_code()) {
                ...
                if (code->handle_editor_changes()) {  // updates ram, file
                                                      // size/mtime
                    modified++;
                }
                if (code->is_editing()) {  // editor open?
                    code->reap_editor();   // Try to reap; maybe it recently
                                           // closed
                }
            }
        }
...
void save_cb(Fl_Widget *, void *v) {
    ...
    if (v || !c || !*c) {
        ...
        if (!access(c, 0)) {
            const char *basename;
            if ((basename = strrchr(c, '/')) != NULL) basename++;
            ...
            if (fl_choice("The file \"%s\" already exists.\n"
                          ...
                          "Cancel", "Replace", NULL, basename) == 0)
                return;
        }
...
void exit_cb(Fl_Widget *, void *) {
    ...
    if (modflag)
        switch (
            fl_choice("Do you want to save changes to this user\n"
    ...
    if (about_panel) delete about_panel;
    if (help_dialog) delete help_dialog;
    ...
    if (tname) {
        ...
        if (iname && *iname) {
            ...
            while (fgets(line, sizeof(line), infile)) {
                ...
                for (ptr = line; (next = strstr(ptr, "@INSTANCE@")) != NULL;
                     ptr = next + 10) {
...
static void sort_cb(Fl_Widget *, void *) { sort((Fl_Type *)0); }
    ...
    if (f) {
        ...
        if (strcmp(name, "fluid.html") == 0) {
            if (!Fl_Shared_Image::find("embedded:/fluid-org.png"))
                new Fl_PNG_Image("embedded:/fluid-org.png", fluid_org_png,
                                 sizeof(fluid_org_png));
            help_dialog->value(
                "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 "
                "Transitional//EN\">\n"
                "<html><head><title>FLTK: Programming with "
                "FLUID</title></head><body>\n"
                "<h2>What is FLUID?</h2>\n"
                "The Fast Light User Interface Designer, or FLUID, is a "
                "graphical editor "
                "that is used to produce FLTK source code. FLUID edits and "
                "saves its state "
                "in <code>.fl</code> files. These files are text, and you can "
                "(with care) "
                "edit them in a text editor, perhaps to get some special "
                "effects.<p>\n"
                "FLUID can \"compile\" the <code>.fl</code> file into a "
                "<code>.cxx</code> "
                "and a <code>.h</code> file. The <code>.cxx</code> file "
                "defines all the "
                "objects from the <code>.fl</code> file and the "
                "<code>.h</code> file "
                "declares all the global ones. FLUID also supports "
                "localization "
                "(Internationalization) of label strings using message files "
                "and the GNU "
                "gettext or POSIX catgets interfaces.<p>\n"
                "A simple program can be made by putting all your code "
                "(including a <code>"
                "main()</code> function) into the <code>.fl</code> file and "
                "thus making the "
                "<code>.cxx</code> file a single source file to compile. Most "
                "programs are "
                "more complex than this, so you write other <code>.cxx</code> "
                "files that "
                "call the FLUID functions. These <code>.cxx</code> files must "
                "<code>"
                "#include</code> the <code>.h</code> file or they can "
                "<code>#include</code> "
                "the <code>.cxx</code> file so it still appears to be a single "
                "source file.<p>"
                "<img src=\"embedded:/fluid-org.png\"></p>"
                "<p>More information is available online at <a href="
                "\"http://www.fltk.org/doc-1.3/fluid.html\">http://"
                "www.fltk.org/</a>"
                "</body></html>");
            ...
            snprintf(helpname, sizeof(helpname), "http://www.fltk.org/%s",
                     name);
...
void help_cb(Fl_Widget *, void *) { show_help("fluid.html"); }

void manual_cb(Fl_Widget *, void *) { show_help("index.html"); }
...
Fl_Menu_Item Main_Menu[] = {
    ...
    {"&Write Strings...", FL_COMMAND + FL_SHIFT + 'w', write_strings_cb, 0,
     FL_MENU_DIVIDER},
    {relative_history[0], FL_COMMAND + '0', open_history_cb,
     absolute_history[0]},
    {relative_history[1], FL_COMMAND + '1', open_history_cb,
     absolute_history[1]},
    {relative_history[2], FL_COMMAND + '2', open_history_cb,
     absolute_history[2]},
    {relative_history[3], FL_COMMAND + '3', open_history_cb,
     absolute_history[3]},
    {relative_history[4], FL_COMMAND + '4', open_history_cb,
     absolute_history[4]},
    {relative_history[5], FL_COMMAND + '5', open_history_cb,
     absolute_history[5]},
    {relative_history[6], FL_COMMAND + '6', open_history_cb,
     absolute_history[6]},
    {relative_history[7], FL_COMMAND + '7', open_history_cb,
     absolute_history[7]},
    {relative_history[8], FL_COMMAND + '8', open_history_cb,
     absolute_history[8]},
    {relative_history[9], FL_COMMAND + '9', open_history_cb,
     absolute_history[9], FL_MENU_DIVIDER},
    ...
    {"Select &None", FL_COMMAND + FL_SHIFT + 'a', select_none_cb, 0,
     FL_MENU_DIVIDER},
    ...
    {"GU&I Settings...", FL_ALT + FL_SHIFT + 'p', show_settings_cb, 0,
     FL_MENU_DIVIDER},
    {"Global &FLTK Settings...", FL_ALT + FL_SHIFT + 'g',
    ...
    {"&Tiny", FL_ALT + '1', (Fl_Callback *)widget_size_cb, (void *)8, 0,
     FL_NORMAL_LABEL, FL_HELVETICA, 8},
    {"&Small", FL_ALT + '2', (Fl_Callback *)widget_size_cb, (void *)11, 0,
     FL_NORMAL_LABEL, FL_HELVETICA, 11},
    {"&Normal", FL_ALT + '3', (Fl_Callback *)widget_size_cb, (void *)14, 0,
     FL_NORMAL_LABEL, FL_HELVETICA, 14},
    {"&Medium", FL_ALT + '4', (Fl_Callback *)widget_size_cb, (void *)18, 0,
     FL_NORMAL_LABEL, FL_HELVETICA, 18},
    {"&Large", FL_ALT + '5', (Fl_Callback *)widget_size_cb, (void *)24, 0,
     FL_NORMAL_LABEL, FL_HELVETICA, 24},
    {"&Huge", FL_ALT + '6', (Fl_Callback *)widget_size_cb, (void *)32, 0,
     FL_NORMAL_LABEL, FL_HELVETICA, 32},
    {0},
...
void toggle_widgetbin_cb(Fl_Widget *, void *) {
    if (!widgetbin_panel) {
        make_widgetbin();
        if (!position_window(widgetbin_panel, "widgetbin_pos", 1, 320, 30))
            return;
    }
...
void toggle_sourceview_cb(Fl_Double_Window *, void *) {
    if (!sourceview_panel) {
        ...
        int tab;
        svp.get("tab", tab, 0);
        if (tab >= 0 && tab < sv_tab->children())
            sv_tab->value(sv_tab->child(tab));
        if (!position_window(sourceview_panel, "sourceview_pos", 0, 320, 120,
                             550, 500))
            return;
    }
...
void toggle_sourceview_b_cb(Fl_Button *, void *) { toggle_sourceview_cb(0, 0); }
...
void make_main_window() {
    ...
    if (!main_window) {
        ...
        sourceview_item = (Fl_Menu_Item *)main_menubar->find_item(
            (Fl_Callback *)toggle_sourceview_cb);
        main_menubar->global();
        fill_in_New_Menu();
        main_window->end();
    }
...
void load_history() {
    ...
    for (i = 0; i < max_files; i++) {
        fluid_prefs.get(Fl_Preferences::Name("file%d", i), absolute_history[i],
                        "", sizeof(absolute_history[i]));
        if (absolute_history[i][0]) {
            // Make a relative version of the filename for the menu...
            fl_filename_relative(relative_history[i],
                                 sizeof(relative_history[i]),
                                 absolute_history[i]);

            if (i == 9)
                history_item[i].flags = FL_MENU_DIVIDER;
            else
                history_item[i].flags = 0;
        } else
            break;
    }
...
FILE *Fl_Process::popen(const char *cmd, const char *mode) {
    ...
    if (CreateProcess(NULL, (LPTSTR)cmd, NULL, NULL, TRUE, DETACHED_PROCESS,
                      NULL, NULL, &si, &pi)) {
...
static bool prepare_shell_command(
    const char *&command) {  // common pre-shell command code all platforms
    shell_window->hide();
    if (s_proc.desc()) {
...
void shell_pipe_cb(FL_SOCKET, void *) {
    ...
    shell_run_display->scroll(
        shell_run_display->count_lines(0, shell_run_buffer->length(), 1), 0);
}
...
void set_filename(const char *c) {
    ...
    if (filename && !batch_mode) update_history(filename);
...
void update_sourceview_position() {
    if (!sourceview_panel || !sourceview_panel->visible()) return;
    if (sv_autoposition->value() == 0) return;
...
void update_sourceview_timer(void *) { update_sourceview_cb(0, 0); }
...
void set_modflag(int mf) {
    ...
    if (main_window) {
        ...
        if (modflag) {
            ...
            main_window->label(basename);
    }
    // if the UI was modified in any way, update the Source View panel
    if (sourceview_panel && sourceview_panel->visible() &&
        sv_autorefresh->value()) {
        // we will only update earliest 0.5 seconds after the last change, and
        // only if no other change was made, so dragging a widget will not
        // generate any CPU load
...
int main(int argc, char **argv) {
    ...
    if (!Fl::args(argc, argv, i, arg) || i < argc - 1) {
        static const char *msg =
            ...
            " -u : update .fl file and exit (may be combined with '-c' or "
            "'-cs')\n"
