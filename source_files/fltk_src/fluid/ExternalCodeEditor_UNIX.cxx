...
int ExternalCodeEditor::is_editing() { return ((pid_ != -1) ? 1 : 0); }
...
void ExternalCodeEditor::kill_editor() {
    ...
    while (pid_ != -1) {  // and wait for it to finish..
        ...
        switch (reap_editor()) {
            ...
            case 0:  // process still running
                if (++wcount >
                    3) {  // retry 3x with 1/10th delay before showing dialog
                    fl_alert(
                        "Can't seem to close editor of file: %s\n"
                        "Please close editor and hit OK",
                        filename());
...
int ExternalCodeEditor::handle_changes(const char **code, int force) {
    ...
    int changed = 0;
    {
        ...
        if (stat(filename(), &sbuf) < 0)
            return (-1);  // TODO: show fl_alert(), do this in win32 too, adjust
                          // func call docs above
        time_t now_mtime = sbuf.st_mtime;
        size_t now_size = sbuf.st_size;
        // OK, now see if file changed; update records if so
        if (now_mtime != file_mtime_) {
            changed = 1;
            file_mtime_ = now_mtime;
        }
        if (now_size != file_size_) {
            changed = 1;
            file_size_ = now_size;
        }
    }
...
int ExternalCodeEditor::remove_tmpfile() {
    ...
    if (is_file(tmpfile)) {
        ...
        if (remove(tmpfile) < 0) {
            fl_alert("WARNING: Can't remove() '%s': %s", tmpfile,
                     strerror(errno));
            return -1;
        }
    }
...
void ExternalCodeEditor::tmpdir_clear() {
    ...
    if (is_dir(tmpdir)) {
        ...
        if (rmdir(tmpdir) < 0) {
            fl_alert("WARNING: Can't rmdir() '%s': %s", tmpdir,
                     strerror(errno));
        }
    }
}
...
static int make_args(char *s,     // string containing words (gets trashed!)
    ...
    int t;
    for (t = 0; (t == 0) ? (ss = strtok(s, " \t")) : (ss = strtok(0, " \t"));
         t++) {
        argv[t] = ss;
    }
    argv[t] = 0;
