...
void grid_cb(Fl_Int_Input *i, long v) {
    ...
    for (p = Fl_Type::first; p; p = p->next) {
        if (p->is_window()) {
            w = (Fl_Window_Type *)p;
            ((Fl_Window *)(w->o))
                ->size_range(gridx, gridy, Fl::w(), Fl::h(), gridx, gridy, 0);
        }
    }
}
...
void i18n_int_cb(Fl_Int_Input *i, void *) {
    ...
    if (i == i18n_set_input) i18n_set = i->value();
...
void show_grid_cb(Fl_Widget *, void *) {
    char buf[128];
    sprintf(buf, "%d", gridx);
    horizontal_input->value(buf);
    sprintf(buf, "%d", gridy);
    vertical_input->value(buf);
    sprintf(buf, "%d", snap);
    snap_input->value(buf);
    guides_toggle->value(show_guides);
    int s = Fl_Widget_Type::default_size;
    if (s <= 8)
        def_widget_size[0]->setonly();
    else if (s <= 11)
        def_widget_size[1]->setonly();
    else if (s <= 14)
        def_widget_size[2]->setonly();
    else if (s <= 18)
        def_widget_size[3]->setonly();
    else if (s <= 24)
        def_widget_size[4]->setonly();
    else if (s <= 32)
        def_widget_size[5]->setonly();
...
void code_input_cb(Fl_Input *i, void *) {
    if (code_file_name && strcmp(code_file_name, i->value())) set_modflag(1);
...
Fl_Menu_Item window_type_menu[] = {{"Single", 0, 0, (void *)FL_WINDOW},
...
void Overlay_Window::draw_overlay() { window->draw_overlay(); }
...
Fl_Type *Fl_Window_Type::make() {
    ...
    ((Fl_Window *)(this->o))
        ->size_range(gridx, gridy, 3072, 2048, gridx, gridy, 0);
    myo->factory = this;
    myo->drag = 0;
    myo->numselected = 0;
...
void Fl_Window_Type::newposition(Fl_Widget_Type *myo, int &X, int &Y, int &R,
                                 int &T) {
    X = myo->o->x();
    Y = myo->o->y();
    R = X + myo->o->w();
    ...
    if (recalc) {
        ...
        for (Fl_Type *q = next; q && q->level > level; q = q->next)
            if (q->selected && q->is_widget() && !q->is_menu_item()) {
                ...
                if (myo->o->x() + myo->o->w() > br)
                    br = myo->o->x() + myo->o->w();
                if (myo->o->y() + myo->o->h() > bt)
                    bt = myo->o->y() + myo->o->h();
            }
        recalc = 0;
        sx = bx;
    ...
    for (Fl_Type *q = next; q && q->level > level; q = q->next)
        if (q->selected && q->is_widget() && !q->is_menu_item()) {
            ...
            if (!(myo->o->align() & FL_ALIGN_INSIDE)) {
                ...
                if (myo->o->align() & FL_ALIGN_TOP)
                    y -= hh;
                else if (myo->o->align() & FL_ALIGN_BOTTOM)
                    t += hh;
                else if (myo->o->align() & FL_ALIGN_LEFT)
                    x -= ww + 4;
                else if (myo->o->align() & FL_ALIGN_RIGHT)
                    r += ww + 4;
            }
            if (x < mybx) mybx = x;
            if (y < myby) myby = y;
            ...
            for (Fl_Type *q = next; q && q->level > level; q = q->next)
                if (q != selection && q->is_widget()) {
                    ...
                    if (fl_min(abs(qy - mysel->o->y() - mysel->o->h()),
                        ...
                        if ((drag & (RIGHT | DRAG)) && abs(d = qr - mybr) < 3) {
                            dx += d;
                            if (drag & DRAG) mybx += d;
                            mybr += d;
...
void check_redraw_corresponding_parent(Fl_Type *s) {
    ...
    for (Fl_Type *i = s; i && i->parent; i = i->parent) {
        ...
        if (i->is_group() && s->is_widget()) prev_parent = (Fl_Widget_Type *)i;
    }
}
...
void toggle_overlays(Fl_Widget *, void *) {
    ...
    if (overlays_invisible)
        Main_Menu[40].label("Show O&verlays");
    else
        Main_Menu[40].label("Hide O&verlays");
    ...
    switch (event) {
        case FL_PUSH:
                ...
                for (Fl_Type *i = next; i && i->level > level; i = i->next)
                    if (i->is_widget() && !i->is_menu_item()) {
                        ...
                        if (Fl::event_inside(myo->o)) {
                            selection = myo;
                            if (Fl::event_clicks() == 1) reveal_in_browser(myo);
                        }
                    CONTINUE2:;
                    }
            }
            ...
            if (numselected && !(Fl::event_state(FL_SHIFT)) &&
                mx <= br + snap && mx >= bx - snap && my <= bt + snap &&
                my >= by - snap) {
                int snap1 = snap > 5 ? snap : 5;
                int w1 = (br - bx) / 4;
                if (w1 > snap1) w1 = snap1;
                if (mx >= br - w1)
                    drag |= RIGHT;
                else if (mx < bx + w1)
                    drag |= LEFT;
            ...
            // do object-specific selection of other objects:
            {
                ...
                if (t) {
                    // if (t == selection) return 1; // indicates mouse eaten
                    // w/o change
                    ...
                    if (!drag)
                        drag = BOX;  // if all else fails, start a new selection
                                     // region
                }
            }
            return 1;
        ...
        case FL_KEYBOARD: {
            ...
            switch (Fl::event_key()) {
                ...
                case FL_Tab: {
                    ...
                    while (i && (!i->is_widget() || i->is_menu_item()))
                        i = i->parent;
                    if (!i) return 0;
                    Fl_Type *p = i->parent;
                ...
                case FL_Down:
                    ...
                    goto ARROW;
                ARROW:
                    // for some reason BOTTOM/TOP are swapped... should be
                    // fixed...
...
void Fl_Window_Type::write_code1() { Fl_Widget_Type::write_code1(); }
...
void Fl_Window_Type::write_properties() {
    ...
    if (sr_min_w || sr_min_h || sr_max_w || sr_max_h)
        write_string("size_range {%d %d %d %d}", sr_min_w, sr_min_h, sr_max_w,
                     sr_max_h);
...
Fl_Widget *Fl_Window_Type::enter_live_mode(int) {
    ...
    if (live_widget) {
        ...
        for (n = next; n && n->level > level; n = n->next) {
            if (n->level == level + 1) n->enter_live_mode();
        }
        win->end();
    }
    return live_widget;
