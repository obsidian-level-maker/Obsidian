            ...
            if (left != max)
                for (Fl_Type *o = Fl_Type::first; o; o = o->next)
                    if (o->selected && o->is_widget()) {
                        ...
                        Fl_Widget *w = ((Fl_Widget_Type *)o)->o;
                        if (o->next && o->next->level > o->level &&
                            !o->next->selected && !o->is_menu_button()) {
                            // When resizing a group, make sure we also move the
                            // children...
                            ((igroup *)w)
                                ->full_resize(left, w->y(), w->w(), w->h());
                        } else {
        ...
        case 11:  // align h.center
            ...
            for (o = Fl_Type::first; o; o = o->next)
                if (o->selected && o->is_widget()) {
                    Fl_Widget *w = ((Fl_Widget_Type *)o)->o;
                    if (w->x() < left) left = w->x();
                    if (w->x() + w->w() > right) right = w->x() + w->w();
                    BREAK_ON_FIRST;
                }
            if (left != max) {
                ...
                for (Fl_Type *o = Fl_Type::first; o; o = o->next)
                    if (o->selected && o->is_widget()) {
                        ...
                        if (o->next && o->next->level > o->level &&
                            ...
                            // children...
                            ((igroup *)w)
                                ->full_resize((center2 - w->w()) / 2, w->y(),
                                              w->w(), w->h());
                        } else {
                            // Otherwise, just do the widget...
                            w->resize((center2 - w->w()) / 2, w->y(), w->w(),
                                      w->h());
                        }
                        w->redraw();
                        if (w->window()) w->window()->redraw();
                ...
                for (Fl_Type *o = Fl_Type::first; o; o = o->next)
                    if (o->selected && o->is_widget()) {
                        ...
                        if (o->next && o->next->level > o->level &&
                            ...
                            // children...
                            ((igroup *)w)
                                ->full_resize(w->x(), (center2 - w->h()) / 2,
                                              w->w(), w->h());
                        } else {
                            // Otherwise, just do the widget...
                            w->resize(w->x(), (center2 - w->h()) / 2, w->w(),
                                      w->h());
                        }
                        w->redraw();
                        if (w->window()) w->window()->redraw();
                ...
                for (Fl_Type *o = Fl_Type::first; o; o = o->next)
                    if (o->selected && o->is_widget()) {
                        ...
                        if (o->next && o->next->level > o->level &&
                            ...
                            // children...
                            ((igroup *)w)
                                ->full_resize(left + wsum + wdt * cnt / n,
                                              w->y(), w->w(), w->h());
                        } else {
                            // Otherwise, just do the widget...
                            w->resize(left + wsum + wdt * cnt / n, w->y(),
                                      w->w(), w->h());
                        }
                        w->redraw();
                        if (w->window()) w->window()->redraw();
                ...
                for (Fl_Type *o = Fl_Type::first; o; o = o->next)
                    if (o->selected && o->is_widget()) {
                        ...
                        if (o->next && o->next->level > o->level &&
                            ...
                            // Otherwise, just do the widget...
                            w->resize(w->x(), top + hsum + hgt * cnt / n,
                                      w->w(), w->h());
                        }
                        w->redraw();
                        if (w->window()) w->window()->redraw();
...
void widget_size_cb(Fl_Widget *, long size) {
    ...
    for (Fl_Type *o = Fl_Type::first; o; o = o->next) {
        if (o->selected && o->is_widget()) {
            ...
            w->redraw();
            // since this may be a major change, the whole window should be
            // redrawn
            if (w->window()) w->window()->redraw();
        }
    }
