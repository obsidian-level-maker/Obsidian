...
void Fl_Widget_Type::subclass(const char *n) {
    if (storestring(n, subclass_) && visible) redraw_browser();
}
        ...
        for (Fl_Type *o = Fl_Type::first; o; o = o->next) {
            if (o->selected && o->is_widget()) {
                ...
                if (w->is_in_class()) {
                    ...
                    // if this is not in a class, it can be only private or
                    // public
                    w->public_ = (i->value() > 0);
                }
                mod = 1;
...
void inactive_cb(Fl_Input *i, void *v) {
    if (v == LOAD) {
        ...
        if (current_widget->is_widget() && !current_widget->is_window()) {
            i->activate();
            i->static_value(
                ((Fl_Widget_Type *)current_widget)->inactive_name());
        } else
            i->deactivate();
    } else {
        int mod = 0;
    ...
    if (v == LOAD) {
        ...
        for (Fl_Type *o = Fl_Type::first; o; o = o->next) {
            if (o->selected && o->is_widget()) {
                ...
                if (o->is_window()) {
                    ((Fl_Window *)w)
                        ->size_range(gridx, gridy, Fl::w(), Fl::h(), gridx,
                                     gridy, 0);
                }
                mod = 1;
            }
...
void wc_relative_cb(Fl_Light_Button *i, void *v) {
    if (v == LOAD) {
        ...
        for (Fl_Type *o = Fl_Type::first; o; o = o->next) {
            if (o->selected &&
                !strcmp(current_widget->type_name(), "widget_class")) {
...
void labeltype_cb(Fl_Choice *i, void *v) {
    if (v == LOAD) {
        ...
        for (int j = 0; j < int(sizeof(labeltypemenu) / sizeof(*labeltypemenu));
             j++)
...
static Fl_Menu_Item alignmenu[] = {
    ...
    {"FL_ALIGN_TEXT_OVER_IMAGE", 0, 0,
     (void *)(fl_intptr_t)(FL_ALIGN_TEXT_OVER_IMAGE)},
    ...
    {"FL_ALIGN_BOTTOM_RIGHT", 0, 0,
     (void *)(fl_intptr_t)(FL_ALIGN_BOTTOM_RIGHT)},
    ...
    {"FL_ALIGN_RIGHT_BOTTOM", 0, 0,
     (void *)(fl_intptr_t)(FL_ALIGN_RIGHT_BOTTOM)},
    {0}};
...
void align_position_cb(Fl_Choice *i, void *v) {
    if (v == LOAD) {
        ...
        for (; mi->text; mi++) {
            if ((Fl_Align)(mi->argument()) == b) i->value(mi);
        }
    } else {
    ...
    if (v == LOAD) {
        ...
        for (j = 0;; j++) {
            ...
            if (current_widget->is_spinner()) {
                if (m[j].argument() ==
                    ((Fl_Spinner *)current_widget->o)->type())
                    break;
            } else {
...
void overlay_cb(Fl_Button *o, void *v) { toggle_overlays(o, v); }
    ...
    if (o->value()) {
        if (numselected == 1) {
            ...
            if (live_widget) {
                ...
                int w = live_widget->w();
                int h = live_widget->h();
                live_window = new Fl_Double_Window(w + 20, h + 55,
                                                   "Fluid Live Mode Widget");
                ...
                Fl_Button *btn =
                    new Fl_Button(10, h + 20, 100, 25, "Exit Live Mode");
                btn->labelsize(12);
        ...
        if (live_type) live_type->leave_live_mode();
        if (live_window) {
            live_window->hide();
...
const char *array_name(Fl_Widget_Type *o) {
    ...
    snprintf(buffer + (d - c + 1), sizeof(buffer) - (d - c + 1), "%d]",
             num + 1);
    return buffer;
}
...
void Fl_Widget_Type::write_static() {
    ...
    if (callback() && is_name(callback())) {
        ...
        if (cc) {
            ...
            if (has_toplevel_function(0L, buf)) write_extern_declaration = 0;
        }
    ...
    if (callback() && !is_name(callback())) {
        ...
        if (*(d - 1) != ';') {
            ...
            if (p)
                ...
                p = callback();
            // Only add trailing semicolon if the last line is not a
    ...
    if (label() && *label()) {
        ...
        switch (i18n_type) {
            ...
            case 2: /* POSIX catgets */
                write_c("catgets(%s,%s,%d,",
                        i18n_file[0] ? i18n_file : "_catalog", i18n_set,
                        msgnum());
                write_cstring(label());
                write_c(")");
                break;
...
void Fl_Widget_Type::write_widget_code() {
    ...
    if (is_spinner() &&
        ((Fl_Spinner *)o)->type() != ((Fl_Spinner *)tplate)->type())
    ...
    if (is_valuator()) {
        ...
        if (v->value()) {
            if (is_valuator() ==
                3) {  // Fl_Scrollbar::value(double) is nott available
    ...
    if (callback()) {
        write_c("%s%s->callback((Fl_Callback*)%s", indent(), var,
                callback_name());
        if (ud)
            write_c(", (void*)(%s));\n", ud);
        else
    ...
    Fl_When ww = o->when();
    if (ww == FL_WHEN_NOT_CHANGED) ww = FL_WHEN_NEVER;
    if (ww != tplate->when() || subclass())
        write_c("%s%s->when(%s);\n", indent(), var,
                item_name(whensymbolmenu, ww));
    if (!o->visible() && o->parent()) write_c("%s%s->hide();\n", indent(), var);
    if (!o->active()) write_c("%s%s->deactivate();\n", indent(), var);
...
void Fl_Widget_Type::write_properties() {
    ...
    if (o->color() != tplate->color()) write_string("color %d", o->color());
    ...
    if (o->align() != tplate->align()) write_string("align %d", o->align());
    if (o->when() != tplate->when()) write_string("when %d", o->when());
    if (is_valuator()) {
        Fl_Valuator *v = (Fl_Valuator *)o;
        Fl_Valuator *f = (Fl_Valuator *)(tplate);
        if (v->minimum() != f->minimum())
...
int Fl_Widget_Type::read_fdesign(const char *propname, const char *value) {
    ...
    if (!strcmp(propname, "box")) {
        ...
        if (!lookup_symbol(value, v)) {
            ...
            int v1 = atoi(value);
            if (v1 <= 0 && strcmp(value, "0")) return 0;
            v = 0;
            if (v1 >= 5) {
                v = FL_ALIGN_INSIDE;
                v1 -= 5;
...
void leave_live_mode_cb(Fl_Widget *, void *) { live_mode_cb(0, 0); }

Fl_Widget *Fl_Widget_Type::enter_live_mode(int) {
    live_widget = widget(o->x(), o->y(), o->w(), o->h());
    if (live_widget) copy_properties();
    return live_widget;
}
...
void Fl_Widget_Type::copy_properties() {
    ...
    if (is_value_input()) {
        Fl_Value_Input *d = (Fl_Value_Input *)live_widget,
                       *s = (Fl_Value_Input *)o;
        d->shortcut(s->shortcut());
    }
    ...
    if (is_text_display()) {
        Fl_Text_Display *d = (Fl_Text_Display *)live_widget,
                        *s = (Fl_Text_Display *)o;
        d->shortcut(s->shortcut());
    }
    ...
    if (!o->visible()) w->hide();
    if (!o->active()) w->deactivate();
    if (resizable() && w->parent()) w->parent()->resizable(o);
}
