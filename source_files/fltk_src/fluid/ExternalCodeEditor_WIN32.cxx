...
static const char *get_ms_errmsg() {
    ...
    LPSTR mbuf = 0;
    DWORD size = FormatMessageA(flags, 0, lastErr,
...
// [Static/Local] Terminate_app()'s callback to send WM_CLOSE to a single
// window.
...
// [Static/Local] Handle sending WIN_CLOSE to /all/ windows matching specified
// pid. Wait up to msecTimeout for process to close, and if it doesn't, use
// TerminateProcess().
//
static int terminate_app(DWORD pid, DWORD msecTimeout) {
    ...
    // Wait on handle. If it closes, great. If it times out, use
    // TerminateProcess()
    int ret = 0;
    if (WaitForSingleObject(hProc, msecTimeout) != WAIT_OBJECT_0) {
        if (G_debug) {
            printf(
                "WARNING: sent WIN_CLOSE, but timeout after %ld msecs.."
...
void ExternalCodeEditor::close_editor() {
    ...
    while (is_editing()) {
        switch (reap_editor()) {
            case -1:  // error
                fl_alert(
                    "Error reaping external editor\n"
                    "pid=%ld file=%s",
                    long(pinfo_.dwProcessId), filename());
                break;
            case 0:  // process still running
                switch (
                    fl_choice("Please close external editor\npid=%ld file=%s",
...
void ExternalCodeEditor::kill_editor() {
    ...
    switch (terminate_app(pinfo_.dwProcessId,
                          500)) {  // kill editor, wait up to 1/2 sec to die
        case -1: {                 // error
            fl_alert(
                "Can't seem to close editor of file: %s\n"
...
int ExternalCodeEditor::handle_changes(const char **code, int force) {
    ...
    if (GetFileSizeEx(fh, &fsize) == 0) {
        ...
        CloseHandle(fh);
        SetLastError(
            err);  // return error from GetFileSizeEx(), not CloseHandle()
        return -1;
    }
    // Get file time
    ...
    if (GetFileTime(fh, &ftCreate, &ftAccess, &ftWrite) == 0) {
        ...
        CloseHandle(fh);
        SetLastError(
            err);  // return error from GetFileTime(), not CloseHandle()
        return -1;
    }
    ...
    if (changed || force) {
        ...
        if (ReadFile(fh, buf, fsize.QuadPart, &count, 0) == 0) {
            fl_alert("ERROR: ReadFile() failed for %s: %s", filename(),
                     get_ms_errmsg());
            free((void *)buf);
            buf = 0;
...
const char *ExternalCodeEditor::tmpdir_name() {
    ...
    _snprintf(dirname, sizeof(dirname), "%s.fluid-%ld", tempdir,
...
void ExternalCodeEditor::tmpdir_clear() {
    ...
    if (is_dir(tmpdir)) {
        ...
        if (RemoveDirectory(tmpdir) == 0) {
            fl_alert("WARNING: Can't RemoveDirectory() '%s': %s", tmpdir,
                     get_ms_errmsg());
        }
    }
}
...
const char *ExternalCodeEditor::create_tmpdir() {
    ...
    if (!is_dir(dirname)) {
        if (CreateDirectory(dirname, 0) == 0) {
            fl_alert("can't create directory '%s': %s", dirname,
                     get_ms_errmsg());
            return NULL;
        }
    }
...
static int save_file(
    ...
    if (fh == INVALID_HANDLE_VALUE) {
        fl_alert("ERROR: couldn't create file '%s': %s", filename,
                 get_ms_errmsg());
        return (-1);
    }
...
int ExternalCodeEditor::start_editor(const char *editor_cmd,
    ...
    if (CreateProcess(NULL,             // app name
        ...
        fl_alert("CreateProcess() failed to start '%s': %s", cmd,
                 get_ms_errmsg());
        return (-1);
    }
...
//
int ExternalCodeEditor::open_editor(const char *editor_cmd, const char *code) {
    ...
    if (is_file(filename())) {
        if (is_editing()) {
            ...
            switch (wpid) {
                case -1:  // wait failed
                    fl_alert(
                        "ERROR: WaitForSingleObject() failed: %s\nfile='%s', "
                        "pid=%ld",
    ...
    if (start_editor(editor_cmd, filename()) <
        0) {  // open file in external editor
...
//
int ExternalCodeEditor::editors_open() { return L_editors_open; }
