...
struct id {
    ...
    void *object;
    id *left, *right;
    id(const char *t, void *o) : text(strdup(t)), object(o) {
        left = right = 0;
    }
    ~id();
};
...
const char *unique_id(void *o, const char *type, const char *name,
                      const char *label) {
    char buffer[128];
    char *q = buffer;
    while (*type) *q++ = *type++;
    ...
    for (; p < e;) {
        ...
        switch (c) {
            ...
            default:
                ...
                // We must not put more numbers after it, because some C
                // compilers consume them as part of the quoted sequence.  Use
                // string constant pasting to avoid this:
                c = *p;
                if (p < e &&
                    ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') ||
                     (c >= 'A' && c <= 'F'))) {
...
void write_cdata(const char *s, int length) {
    ...
    if (write_sourceview) {
        if (length >= 0)
            fprintf(code_file, "{ /* ... %d bytes of binary data... */ }",
                    length);
        else
...
static Fl_Type *write_code(Fl_Type *p) {
    ...
    if (!(p == Fl_Type::last && p->is_comment())) p->write_code1();
...
int write_code(const char *s, const char *t) {
    ...
    if (write_sourceview) filemode = "wb";
    write_number++;
    delete id_root;
    id_root = 0;
    indentation = 0;
