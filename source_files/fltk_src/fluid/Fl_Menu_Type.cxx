...
Fl_Menu_Item menu_item_type_menu[] = {{"Normal", 0, 0, (void *)0},
...
Fl_Type *Fl_Menu_Item_Type::make() {
    ...
    Fl_Type *p = q;
    if (p) {
        if ((force_parent && q->is_menu_item()) || !q->is_parent())
            p = p->parent;
    }
    force_parent = 0;
    ...
    Fl_Menu_Item_Type *t =
        submenuflag ? new Fl_Submenu_Type() : new Fl_Menu_Item_Type();
...
const char *Fl_Menu_Item_Type::menu_name(int &i) {
    ...
    while (t && t->is_menu_item()) {
        ...
        else if (t->level == t->next->level && t->is_parent())
            i++;
        t = t->prev;
        i++;
    }
...
void Fl_Menu_Item_Type::write_static() {
    ...
    Fl_Type *t = prev;
    while (t && t->is_menu_item()) t = t->prev;
    for (Fl_Type *q = t->next; q && q->is_menu_item(); q = q->next) {
        ((Fl_Menu_Item_Type *)q)->write_item();
        int thislevel = q->level;
        if (q->is_parent()) thislevel++;
        int nextlevel = (q->next && q->next->is_menu_item()) ? q->next->level
                                                             : t->level + 1;
...
void Fl_Menu_Item_Type::write_item() {
    static const char *const labeltypes[] = {
        "FL_NORMAL_LABEL",   "FL_NO_LABEL",       "FL_SHADOW_LABEL",
        "FL_ENGRAVED_LABEL", "FL_EMBOSSED_LABEL", "FL_MULTI_LABEL",
        "FL_ICON_LABEL",     "FL_IMAGE_LABEL"};
    ...
    if (user_data())
        ...
        write_c(" 0,");
    write_c(" %d, (uchar)%s, %d, %d, %d", flags(), labeltypes[o->labeltype()],
            o->labelfont(), o->labelsize(), o->labelcolor());
    write_c("},\n");
}

void Fl_Menu_Item_Type::write_code1() {
    int i;
    const char *mname = menu_name(i);
    ...
    if (next && next->is_menu_item()) {
        if (i18n_type) {
            ...
            for (Fl_Type *q = next; q && q->is_menu_item(); q = q->next) {
                ...
                int thislevel = q->level;
                if (q->is_parent()) thislevel++;
                int nextlevel = (q->next && q->next->is_menu_item())
                                    ? q->next->level
                                    : next->level + 1;
                nItem +=
                    1 + ((thislevel > nextlevel) ? (thislevel - nextlevel) : 0);
            }
            if (nLabel) {
                ...
                switch (i18n_type) {
                    ...
                    case 2:
                        write_c(
                            "%s      "
                            "%s[i].label(catgets(%s,%s,i+%d,%s[i].label()));\n",
                            indent(), mName,
...
void Fl_Input_Choice_Type::copy_properties() {
    Fl_Widget_Type::copy_properties();
    Fl_Input_Choice *s = (Fl_Input_Choice *)o,
                    *d = (Fl_Input_Choice *)live_widget;
    d->menu(s->menu());
    d->down_box(s->down_box());
    d->textcolor(s->textcolor());
    d->textfont(s->textfont());
...
void Shortcut_Button::draw() {
    ...
    if (use_FL_COMMAND && (svalue & (FL_CTRL | FL_META))) {
        char buf[1024];
        fl_snprintf(buf, 1023, "Command+%s",
                    fl_shortcut_label(svalue & ~(FL_CTRL | FL_META)));
        fl_draw(buf, x() + 6, y(), w(), h(), FL_ALIGN_LEFT);
    } else {
        fl_draw(fl_shortcut_label(svalue), x() + 6, y(), w(), h(),
                FL_ALIGN_LEFT);
    }
}
    ...
    if (v == LOAD) {
        ...
        for (Fl_Type *o = Fl_Type::first; o; o = o->next)
            if (o->selected && o->is_button()) {
                ...
                Fl_Value_Input *b =
                    (Fl_Value_Input *)(((Fl_Widget_Type *)o)->o);
                if (b->shortcut() != i->svalue) mod = 1;
                b->shortcut(i->svalue);
            } else if (o->selected && o->is_text_display()) {
                Fl_Text_Display *b =
                    (Fl_Text_Display *)(((Fl_Widget_Type *)o)->o);
                if (b->shortcut() != i->svalue) mod = 1;
                b->shortcut(i->svalue);
            }
