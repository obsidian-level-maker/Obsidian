...
void Fluid_Image::write_static() {
    ...
    if (img->count() > 1) {
        ...
        write_c(";\n");
        write_initializer("Fl_Bitmap", "%s, %d, %d", idata_name, img->w(),
                          img->h());
        ...
        write_c(";\n");
        write_initializer("Fl_JPEG_Image", "\"%s\", %s",
                          fl_filename_name(name()), idata_name);
    } else {
        // Write image data...
        write_c("\n");
        ...
        write_cdata(img->data()[0],
                    (img->w() * img->d() + extra_data) * img->h());
        write_c(";\n");
        write_initializer("Fl_RGB_Image", "%s, %d, %d, %d, %d", idata_name,
...
void Fluid_Image::write_initializer(const char *type_name, const char *format,
                                    ...) {
    /* Outputs code that returns (and initializes if needed) an Fl_Image as
     follows: static Fl_Image *'function_name_'() { static Fl_Image *image = new
     'type_name'('product of format and remaining args'); return image;
     } */
    va_list ap;
    va_start(ap, format);
    write_c("static Fl_Image *%s() {\n  static Fl_Image *image = new %s(",
            function_name_, type_name);
    vwrite_c(format, ap);
    write_c(");\n  return image;\n}\n");
    va_end(ap);
}
...
void Fluid_Image::write_code(const char *var, int inactive) {
    /* Outputs code that attaches an image to an Fl_Widget or Fl_Menu_Item.
     This code calls a function output before by
     Fluid_Image::write_initializer() */
    if (img)
        write_c("%s%s->%s( %s() );\n", indent(), var,
                inactive ? "deimage" : "image", function_name_);
}
...
Fluid_Image *Fluid_Image::find(const char *iname) {
    ...
    if (numimages > tablesize) {
        ...
        if (images)
            images = (Fluid_Image **)realloc(images,
                                             tablesize * sizeof(Fluid_Image *));
        else
...
void Fluid_Image::increment() { ++refcount; }
...
Fluid_Image *ui_find_image(const char *oldname) {
    ...
    const char *name = fl_file_chooser(
        "Image?", "Image Files (*.{bm,bmp,gif,jpg,pbm,pgm,png,ppm,xbm,xpm})",
        oldname, 1);
