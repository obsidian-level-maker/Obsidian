...
Fl_Cairo_State Fl::cairo_state_;  ///< contains all necessary info for current
                                  ///< cairo context mapping
...
#else
    Fl::fatal(
        "In Fl::autolink(bool) : Cairo autolink() feature is only "
...
cairo_t *Fl::cairo_make_current(Fl_Window *wi) {
    ...
    if (fl_gc == Fl::cairo_state_.gc() &&
        fl_xid(wi) == (Window)Fl::cairo_state_.window())
        return Fl::cairo_cc();
...
#if defined(USE_X11)
    return cairo_xlib_surface_create(fl_display, fl_window, fl_visual->visual,
                                     W, H);
#elif defined(WIN32)
