...
#else   // if the X11/Xregion.h header is not available, we assume this is the
        // layout of an X11 Region:
typedef struct {
    short x1, x2, y1, y2;
} BOX;
struct _XRegion {
...
static int start(Fl_Pixmap *pxm, int XP, int YP, int WP, int HP, int w, int h,
                 int &cx, int &cy, int &X, int &Y, int &W, int &H) {
    // ignore empty or bad pixmap data:
    if (!pxm->data()) {
        return 2;
    }
...
int Fl_Pixmap::prepare(int XP, int YP, int WP, int HP, int &cx, int &cy, int &X,
                       int &Y, int &W, int &H) {
    if (w() < 0) measure();
...
void Fl_GDI_Graphics_Driver::draw(Fl_Pixmap *pxm, int XP, int YP, int WP,
                                  int HP, int cx, int cy) {
    int X, Y, W, H;
    if (pxm->prepare(XP, YP, WP, HP, cx, cy, X, Y, W, H)) return;
    if (pxm->mask_) {
...
void Fl_GDI_Printer_Graphics_Driver::draw(Fl_Pixmap *pxm, int XP, int YP,
                                          int WP, int HP, int cx, int cy) {
    int X, Y, W, H;
    if (pxm->prepare(XP, YP, WP, HP, cx, cy, X, Y, W, H)) return;
    typedef BOOL(WINAPI * fl_transp_func)(HDC, int, int, int, int, HDC, int,
                                          int, int, int, UINT);
    ...
    if (!hMod) {
        ...
        if (hMod)
            fl_TransparentBlt =
                (fl_transp_func)GetProcAddress(hMod, "TransparentBlt");
    }
...
void Fl_Xlib_Graphics_Driver::draw(Fl_Pixmap *pxm, int XP, int YP, int WP,
                                   int HP, int cx, int cy) {
    int X, Y, W, H;
    if (pxm->prepare(XP, YP, WP, HP, cx, cy, X, Y, W, H)) return;
    if (pxm->mask_) {
        ...
        if (clip_region()) {
            // At this point, XYWH is the bounding box of the intersection
            // between the current clip region and the (portion of the) pixmap
            // we have to draw. The current clip region is often a rectangle.
            // But, when a window with rounded corners is moved above another
            // window, expose events may create a complex clip region made of
            // several (e.g., 10) rectangles. We have to draw only in the clip
            // region, and also to mask out the transparent pixels of the image.
            // This can't be done in a single Xlib call for a multi-rectangle
            // clip region. Thus, we process each rectangle of the intersection
            // between the clip region and XYWH. See also STR #3206.
            ...
            for (int i = 0; i < r->numRects; i++) {
                ...
                copy_offscreen(X1, Y1, W1, H1, pxm->id_, cx + (X1 - X),
                               cy + (Y1 - Y));
            }
            XDestroyRegion(r);
        } else {
...
Fl_Image *Fl_Pixmap::copy(int W, int H) {
    ...
    for (dy = H, sy = 0, yerr = H; dy > 0; dy--, new_row++) {
        *new_row = new char[chars_per_line];
        ...
        for (dx = W, xerr = W, old_ptr = data()[sy + ncolors + 1]; dx > 0;
             dx--) {
        ...
        for (color = 0; color < ncolors; color++) {
            ...
            if (fl_parse_color(p, r, g, b)) {
                ...
                if (chars_per_pixel > 1)
                    sprintf(line, "%c%c c #%02X%02X%02X", data()[color + 1][0],
                            data()[color + 1][1], r, g, b);
                else
                    sprintf(line, "%c c #%02X%02X%02X", data()[color + 1][0], r,
                            g, b);
    ...
    if (ncolors < 0) {
        ...
        for (i = 0; i < ncolors; i++) {
            ...
            if (fl_parse_color(p, r, g, b)) {
                ...
                if (chars_per_pixel > 1)
                    sprintf(line, "%c%c c #%02X%02X%02X", data()[i + 1][0],
                            data()[i + 1][1], g, g, g);
                else
                    sprintf(line, "%c c #%02X%02X%02X", data()[i + 1][0], g, g,
                            g);
