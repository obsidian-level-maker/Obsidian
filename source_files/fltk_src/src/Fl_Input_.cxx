...
/** \internal
  ...
  This copies the text from \p p to \p buf, replacing characters with
  <tt>^X</tt> and <tt>\\nnn</tt> as necessary.
    ...
    if (input_type() == FL_SECRET_INPUT) {
        ...
        while (o < e) {
            if (wrap() && (p >= value_ + size_ || isspace(*p & 255))) {
                ...
                width_to_lastspace +=
                    (int)fl_width(lastspace_out, (int)(o - lastspace_out));
                if (p > lastspace + 1) {
...
*/
void Fl_Input_::setfont() const { fl_font(textfont(), textsize()); }
...
void Fl_Input_::drawtext(int X, int Y, int W, int H) {
    ...
    int selstart, selend;
    if (Fl::focus() != this &&
        /*Fl::selection_owner()!=this &&*/ Fl::pushed() != this)
        selstart = selend = 0;
    ...
    for (; ypos < H;) {
        ...
        if (do_mu) {  // for minimal update:
            const char *pp =
                value() + mu_p;  // pointer to where minimal update starts
            ...
            fl_push_clip((int)xx - 1 - height / 8, Y + ypos,
        ...
        if (selstart < selend && selstart <= e - value() &&
            selend > p - value()) {
...
#endif
            {
                ...
                fl_rectf((int)(x1 + 0.5), Y + ypos, (int)(x2 - x1 + 0.5),
                         height);
                fl_color(fl_contrast(textcolor(), selection_color()));
            }
            fl_draw(buf + offset1, offset2 - offset1, x1,
                    (float)(Y + ypos + desc));
#ifdef __APPLE__  // Mac OS: underline marked ( = selected + Fl::compose_state
                  // != 0) text
            if (Fl::compose_state) {
                ...
                fl_line(x1, Y + ypos + height - 1, x1 + width,
                        Y + ypos + height - 1);
            }
#endif
            if (pp < e) {
...
#ifdef __APPLE__
            Fl::insertion_point_location(xpos + curx, Y + ypos + height,
                                         height);
#endif
        }
    ...
    fl_pop_clip();
    if (Fl::focus() == this) {
        fl_set_spot(textfont(), textsize(), (int)xpos + curx,
                    Y + ypos - fl_descent(), W, H, window());
    }
}
...
int Fl_Input_::line_start(int i) const {
    ...
    while (j > 0 && index(j - 1) != '\n') j--;
    if (wrap()) {
        // now measure lines until we get past i, start of that line is real
        // eol:
        setfont();
        for (const char *p = value() + j;;) {
            char buf[MAXBUF];
...
void Fl_Input_::handle_mouse(int X, int Y, int /*W*/, int /*H*/, int drag) {
    ...
    int theline = (input_type() == FL_MULTILINE_INPUT)
                      ? (Fl::event_y() - Y + yscroll_) / fl_height()
                      : 0;
    ...
    if (Fl::event_clicks()) {
        if (newpos >= newmark) {
            ...
            if (Fl::event_clicks() > 1) {
                ...
                newmark =
                    strict_word_end(value(), size(), newmark, input_type());
            }
        }
...
int Fl_Input_::replace(int b, int e, const char *text, int ilen) {
    ...
    if (wrap()) {
        // if there is a space in the pasted text, the whole line may have
        // rewrapped
        int i;
        for (i = 0; i < ilen; i++)
            if (text[i] == ' ') break;
...
int Fl_Input_::handletext(int event, int X, int Y, int W, int H) {
    switch (event) {
        ...
        case FL_HIDE:
            fl_reset_spot();
            if (!readonly() && (when() & FL_WHEN_RELEASE)) maybe_do_callback();
            return 1;
...
int Fl_Input_::static_value(const char *str, int len) {
    ...
    if (len) {  // non-empty new value:
        if (xscroll_ || yscroll_) {
            ...
            if (value_) {
                for (; i < size_ && i < len && str[i] == value_[i];
                     i++) { /*empty*/
                }
