...
static void WIN_SetupPrinterDeviceContext(HDC prHDC) {
    ...
    // of paper thus the logical unit is the point (= 1/72 inch)
    SetWindowExtEx(prHDC, 720, 720, NULL);
    SetViewportExtEx(prHDC, 10 * GetDeviceCaps(prHDC, LOGPIXELSX),
                     10 * GetDeviceCaps(prHDC, LOGPIXELSY), NULL);
}
...
// returns 0 iff OK
{
    ...
    BOOL b = PrintDlg(&pd);
    if (pd.hwndOwner) {  // restore the correct state of mouse buttons and
                         // keyboard modifier keys (STR #3221)
        WNDPROC windproc =
            (WNDPROC)GetWindowLongPtrW(pd.hwndOwner, GWLP_WNDPROC);
...
void Fl_System_Printer::absolute_printable_rect(int *x, int *y, int *w,
                                                int *h) {
    POINT physPageSize;
    POINT pixelsPerInch;
    XFORM transform;
