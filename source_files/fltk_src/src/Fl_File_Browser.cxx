...
Fl_File_Browser::full_height() const {
    ...
    for (i = 0, th = 0; i < size(); i++) th += item_height(find_line(i));
...
Fl_File_Browser::item_height(void *p) const  // I - List item data
{
    ...
    if (Fl_File_Icon::first() != NULL && height < iconsize_) height = iconsize_;
...
Fl_File_Browser::item_width(void *p) const  // I - List item data
{
    ...
    // If we have enabled icons then add space for them...
    if (Fl_File_Icon::first() != NULL) width += iconsize_ + 8;
...
void Fl_File_Browser::item_draw(void *p,    // I - List item data
                                ...
                                int) const  // I - Height of item
{
    ...
    if (Fl_File_Icon::first() == NULL) {
        ...
        if (line->data)
            ((Fl_File_Icon *)line->data)
                ->draw(X, Y, iconsize_, iconsize_,
                       (line->flags & SELECTED) ? FL_YELLOW : FL_LIGHT2,
                       active_r());
        ...
        if (height < iconsize_) Y += (iconsize_ - height) / 2;
    }
                      ...
                      Fl_File_Sort_F *sort)   // I - Sort function to use
{
    ...
    if (directory_[0] == '\0') {
        ...
        for (i = 'A'; i <= 'Z'; i++, drives >>= 1)
            if (drives & 1) {
                ...
                if (i < 'C')  // see also: GetDriveType and GetVolumeInformation
                              // in WIN32
                    add(filename, icon);
                else
                    add(filename, icon);
        ...
        // But reading the list of mounted filesystems from the kernel is
        // possible:
    ...
    (__NetBSD_Version__ >= 300000000)
        // NetBSD don't write the mounted filesystems to a file like
        // '/etc/mnttab'. Since NetBSD 3.0 the system call getvfsstat(2) has
        // replaced getfsstat(2) that is used by getmntinfo(3):
        ...
        if (mtab == NULL) mtab = fl_fopen("/etc/mtab", "r");  // More standard
        if (mtab == NULL)
            mtab =
        ...
        if (mtab != NULL) {
            ...
            while (fgets(line, sizeof(line), mtab) != NULL) {
                ...
                if (strcmp("/", filename) == 0)
                    continue;  // root was added before
                // Add a trailing slash (except for the root filesystem)
                strlcat(filename, "/", sizeof(filename));

                //        printf("Fl_File_Browser::load() - adding \"%s\" to
                //        list...\n", filename);
                add(filename, icon);
                num_files++;
            }
...
void Fl_File_Browser::filter(const char *pattern)  // I - Pattern string
{
