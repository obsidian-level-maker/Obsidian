...
#define USE_XRANDR \
    (HAVE_DLSYM && \
     HAVE_DLFCN_H)  // means attempt to dynamically load libXrandr.so
#if USE_XRANDR
#include <dlfcn.h>
...
void Fl::add_fd(int n, int events, void (*cb)(int, void *), void *v) {
    ...
    if (i >= fd_array_size) {
        ...
        if (!fd)
            ...
            temp = (FD *)realloc(fd, fd_array_size * sizeof(FD));
        ...
        if (!pollfds)
            tpoll = (pollfd *)malloc(fd_array_size * sizeof(pollfd));
        else
            tpoll = (pollfd *)realloc(pollfds, fd_array_size * sizeof(pollfd));
...
void Fl::remove_fd(int n) { remove_fd(n, -1); }
...
static void do_queued_events() {
    ...
    while (XEventsQueued(fl_display, QueuedAfterReading)) {
        ...
        if (fl_send_system_handlers(&xevent)) continue;
        fl_handle(xevent);
    }
...
// FIXME: warning XFT support here
    ...
    if (!fs) {
        fnt = (char *)"-misc-fixed-*";
        fs = XCreateFontSet(fl_display, fnt, &missing_list, &missing_count,
                            &def_string);
    }
#else
    if (!fs) {
    ...
    preedit_attr =
        XVaCreateNestedList(0, XNSpotLocation, &spot, XNFontSet, fs, NULL);
    status_attr =
        XVaCreateNestedList(0, XNAreaNeeded, &status_area, XNFontSet, fs, NULL);

    if (!XGetIMValues(fl_xim_im, XNQueryInputStyle, &xim_styles, NULL, NULL)) {
        int i;
        XIMStyle *style;
    ...
    if (sarea) {
        fl_xim_ic =
            XCreateIC(fl_xim_im, XNInputStyle,
                      (XIMPreeditPosition | XIMStatusArea), XNPreeditAttributes,
                      preedit_attr, XNStatusAttributes, status_attr, NULL);
    }
...
static void fl_init_xim() {
    ...
    if (fl_xim_im) {
        ...
        if (xim_warning) Fl::warning("XOpenIM() failed");
    ...
    if (xim_styles && xim_styles->count_styles) {
        ...
        if (xim_warning) Fl::warning("No XIM style found");
        XCloseIM(fl_xim_im);
        fl_xim_im = NULL;
        // if xim_styles is allocated, free it now
        if (xim_styles) XFree(xim_styles);
    ...
    if (!fl_xim_ic) {
        if (xim_warning) Fl::warning("XCreateIC() failed");
        XCloseIM(fl_xim_im);
        fl_xim_im = NULL;
    }
...
void fl_xim_activate(Window xid) {
    ...
    if (fl_xim_win != xid) {
        ...
        XSetICValues(fl_xim_ic, XNFocusWindow, fl_xim_win, XNClientWindow,
                     fl_xim_win, NULL);
    }
...
void Fl::disable_im() { fl_xim_deactivate(); }
...
void fl_open_display(Display *d) {
    ...
    fl_Xatextplainutf2 =
        XInternAtom(d, "text/plain;charset=utf-8",
                    0);  // Firefox/Thunderbird needs this - See STR#2930
    ...
    fl_NET_WM_FULLSCREEN_MONITORS =
        XInternAtom(d, "_NET_WM_FULLSCREEN_MONITORS", 0);
    ...
    if (sizeof(Atom) < 4) atom_bits = sizeof(Atom) * 8;
    ...
    if (libxrandr_addr) {
        ...
        XRRUpdateConfiguration_f = (XRRUpdateConfiguration_type)dlsym(
            libxrandr_addr, "XRRUpdateConfiguration");
        if (XRRQueryExtension_f && XRRSelectInput_f &&
            XRRQueryExtension_f(d, &randrEventBase, &error_base))
            XRRSelectInput_f(d, RootWindow(d, fl_screen),
                             RRScreenChangeNotifyMask);
        else
            XRRUpdateConfiguration_f = NULL;
    }
#endif
...
static void fl_init_workarea() {
    ...
    /* If there are several screens, the _NET_WORKAREA property
     ...
     together. Therefore, we use this property only when there is a single
     screen, and fall back to the main screen full area when there are several
     screens.
     */
    if (Fl::screen_count() > 1 ||
        XGetWindowProperty(fl_display, RootWindow(fl_display, fl_screen),
                           fl_NET_WORKAREA, 0, 4, False, XA_CARDINAL, &actual,
                           &format, &count, &remaining,
                           (unsigned char **)&xywh) ||
        !xywh || !xywh[2] || !xywh[3]) {
        Fl::screen_xywh(fl_workarea_xywh[0], fl_workarea_xywh[1],
                        fl_workarea_xywh[2], fl_workarea_xywh[3], 0);
    } else {
...
void Fl::paste(Fl_Widget &receiver, int clipboard, const char *type) {
    if (fl_i_own_selection[clipboard]) {
        ...
        if (fl_selection_type[clipboard] != Fl::clipboard_plain_text)
            return;  // TODO: allow copy/paste of image within same app
...
int Fl::clipboard_contains(const char *type) {
    ...
    XConvertSelection(fl_display, CLIPBOARD, TARGETS, CLIPBOARD, fl_xid(win),
                      CurrentTime);
    XFlush(fl_display);
    do {
    ...
    XGetWindowProperty(fl_display, event.xselection.requestor,
                       event.xselection.property, 0, 4000, 0, 0, &actual,
    ...
    if (strcmp(type, Fl::clipboard_plain_text) == 0) {
        for (i = 0; i < count; i++) {  // searching for text data
            t = ((Atom *)portion)[i];
            if (t == fl_Xatextplainutf || t == fl_Xatextplainutf2 ||
                t == fl_Xatextplain || t == fl_XaUtf8String) {
                retval = 1;
                break;
            }
        }
...
static Atom
    *fl_dnd_source_types;  // null-terminated list of data types being supplied
static Atom fl_dnd_type;
static Atom fl_dnd_source_action;
static Atom fl_dnd_action;

void fl_sendClientMessage(Window window, Atom message, unsigned long d0,
                          unsigned long d1 = 0, unsigned long d2 = 0,
                          unsigned long d3 = 0, unsigned long d4 = 0) {
    XEvent e;
...
static unsigned char *create_bmp(const unsigned char *data, int W, int H,
                                 int *return_size) {
    int R = (3 * W + 3) / 4 *
            4;  // the number of bytes per row, rounded up to multiple of 4
    int s = H * R;
    int fs = 14 + 40 + s;
...
void Fl::copy_image(const unsigned char *data, int W, int H, int clipboard) {
    ...
    delete[] fl_selection_buffer[clipboard];
    fl_selection_buffer[clipboard] =
        (char *)create_bmp(data, W, H, &fl_selection_length[clipboard]);
...
static void poll_clipboard_owner(void) {
    ...
    if (have_xfixes) return;
#endif

    // No one is interested, so no point polling
    if (fl_clipboard_notify_empty()) return;
...
static void handle_clipboard_timestamp(int clipboard, Time time) {
    ...
    *timestamp = time;
    ...
    if (time > fl_event_time) fl_event_time = time;
...
static int wasXExceptionRaised() { return xerror; }
}
...
static long getIncrData(uchar *&data, const XSelectionEvent &selevent,
                        long lower_bound) {
    ...
    for (;;) {
        ...
        if (event.type == PropertyNotify) {
            ...
            do {
                XGetWindowProperty(
                    fl_display, selevent.requestor, selevent.property, offset,
                    70000, True, AnyPropertyType, &actual_type, &actual_format,
...
int fl_handle(const XEvent &thisevent) {
    ...
    if (fl_xim_ic && xevent.type == DestroyNotify && xid != fl_xim_win &&
        !fl_find(xid)) {
        XIM xim_im;
    ...
    if (fl_xim_ic && (xevent.type == FocusIn)) fl_xim_activate(xid);

    if (fl_xim_ic && XFilterEvent((XEvent *)&xevent, 0)) return (1);

#if USE_XRANDR
    if (XRRUpdateConfiguration_f &&
        xevent.type == randrEventBase + RRScreenChangeNotify) {
    ...
    if (xevent.type == PropertyNotify &&
        xevent.xproperty.atom == fl_NET_WORKAREA) {
        fl_init_workarea();
    }
    ...
    switch (xevent.type) {
        ...
        case SelectionNotify: {
            ...
            long bytesread = 0;
            if (fl_xevent->xselection.property)
                for (;;) {
                    // The Xdnd code pastes 64K chunks together, possibly to
                    // avoid bugs in X servers, or maybe to avoid an extra
                    // round-trip to get the property length.  I copy this here:
                    Atom actual;
                    int format;
                    unsigned long count, remaining;
                    unsigned char *portion = NULL;
                    if (XGetWindowProperty(
                            fl_display, fl_xevent->xselection.requestor,
                            fl_xevent->xselection.property, bytesread / 4,
                            65536, 1, AnyPropertyType, &actual, &format, &count,
                            &remaining, &portion))
                        break;  // quit on error
                    ...
                    if (actual == TARGETS || actual == XA_ATOM) {
                        /*for (unsigned i = 0; i<count; i++) {
                          fprintf(stderr," %s", XGetAtomName(fl_display,
                        ((Atom*)portion)[i]) );
                          }
                        fprintf(stderr,"\n");*/
                        Atom t, type = XA_STRING;
                        if (Fl::e_clipboard_type ==
                            Fl::clipboard_image) {  // searching for image data
                            for (unsigned i = 0; i < count; i++) {
                                t = ((Atom *)portion)[i];
                                if (t == fl_XaImageBmp || t == fl_XaImagePNG) {
                                    type = t;
                        ...
                        for (unsigned i = 0; i < count;
                            ...
                            if (t == fl_XaText || t == fl_XaTextUriList ||
                                t == fl_XaCompoundText)
                                type = t;
                        }
                    found:
                    ...
                    if (actual == fl_INCR) {
                        bytesread = getIncrData(sn_buffer, xevent.xselection,
                                                *(long *)portion);
                        XFree(portion);
                        break;
                    }
                    ...
                    sn_buffer = (unsigned char *)realloc(
                        sn_buffer, bytesread + count + remaining + 1);
            ...
            if (Fl::e_clipboard_type == Fl::clipboard_image) {
                ...
                memcpy(rgb, shared->data()[0],
                       shared->w() * shared->h() * shared->d());
                Fl_RGB_Image *image = new Fl_RGB_Image(
                    rgb, shared->w(), shared->h(), shared->d());
                shared->release();
                image->alloc_array = 1;
            ...
            // XA_SECONDARY for that) and send an XdndFinished message. It is
            // not clear if this has to be delayed until now or if it can be
            // done immediately after calling XConvertSelection.
            ...
            if (fl_selection_type[clipboard] == Fl::clipboard_plain_text) {
                if (e.target == TARGETS) {
                    ...
                    XChangeProperty(fl_display, e.requestor, e.property,
                                    XA_ATOM, atom_bits, 0, (unsigned char *)a,
                                    3);
                } else {
                    if (/*e.target == XA_STRING &&*/ fl_selection_length
                            [clipboard]) {
                        if (e.target == fl_XaUtf8String ||
                            ...
                            // clobber the target type, this seems to make some
                            // applications behave that insist on asking for
                            // XA_TEXT instead of UTF8_STRING Does not change
                            // XA_STRING as that breaks xclipboard.
                            if (e.target != XA_STRING)
                                e.target = fl_XaUtf8String;
                            XChangeProperty(
                                fl_display, e.requestor, e.property, e.target,
                                8, 0,
                ...
                if (e.target == TARGETS) {
                    ...
                    if (e.target == fl_XaImageBmp &&
                        fl_selection_length[clipboard]) {
                        XChangeProperty(
                            fl_display, e.requestor, e.property, e.target, 8, 0,
                            (unsigned char *)fl_selection_buffer[clipboard],
                            fl_selection_length[clipboard]);
                    } else {
                        e.property = 0;
    ...
    if (window) switch (xevent.type) {
            case DestroyNotify: {  // an X11 window was closed externally from
                                   // the program
                Fl::handle(FL_CLOSE, window);
                Fl_X *X = Fl_X::i(window);
                if (X) {  // indicates the FLTK window was not closed
                    X->xid = (Window)0;  // indicates the X11 window was already
                                         // destroyed
                    window->hide();
                    int oldx = window->x(), oldy = window->y();
                    window->position(0, 0);
                    window->position(oldx, oldy);
                    window->show();  // recreate the X11 window in support of
                                     // the FLTK window
                }
                return 1;
            }
            case ClientMessage: {
                ...
                if ((Atom)(data[0]) == WM_DELETE_WINDOW) {
                    ...
                    if (data[1] & 1) {
                        ...
                        XGetWindowProperty(fl_display, fl_dnd_source_window,
                                           fl_XdndTypeList, 0, 0x8000000L,
                                           False, XA_ATOM, &actual, &format,
                                           &count, &remaining, &cm_buffer);
                        if (actual != XA_ATOM || format != 32 || count < 4 ||
                            !cm_buffer) {
                            if (cm_buffer) {
                                XFree(cm_buffer);
                                cm_buffer = 0;
                    ...
                    FAILED:
                        ...
                        if (!fl_dnd_source_types)
                            fl_dnd_source_types = new Atom[4];
                    ...
                    // Loop through the source types and pick the first text
                    // type...
                    unsigned i;
                    ...
                    fl_sendClientMessage(
                        data[0], fl_XdndStatus, fl_xevent->xclient.window,
                        accept ? 1 : 0,
                    ...
                    if (Fl::handle(FL_DND_RELEASE, window)) {
                        ...
                        XConvertSelection(fl_display, fl_XdndSelection,
                                          fl_dnd_type, XA_SECONDARY, to_window,
                                          fl_event_time);
                    } else {
                        // Send the finished message if I refuse the drop.
                        // It is not clear whether I can just send finished
                        // always, or if I have to wait for the SelectionNotify
                        // event as the code is currently doing.
                        fl_sendClientMessage(fl_dnd_source_window,
                                             fl_XdndFinished, to_window);
    ...
    if (Fl::first_window()->non_modal() && window != Fl::first_window())
      Fl::first_window()->show();
            ...
            case GraphicsExpose:
                window->damage(FL_DAMAGE_EXPOSE, xevent.xexpose.x,
            ...
            KEYPRESS:
                ...
                if (xevent.type == KeyPress) {
                    ...
                    if (fl_xim_ic) {
                        ...
                        len = XUtf8LookupString(
                            fl_xim_ic, (XKeyPressedEvent *)&xevent.xkey,
                        ...
                        while (status == XBufferOverflow &&
                            ...
                            kp_buffer =
                                (char *)realloc(kp_buffer, kp_buffer_len);
                            len = XUtf8LookupString(
                                fl_xim_ic, (XKeyPressedEvent *)&xevent.xkey,
                                kp_buffer, kp_buffer_len, &keysym, &status);
                        }
                        ...
                        len = XLookupString((XKeyEvent *)&(xevent.xkey),
                                            kp_buffer, kp_buffer_len, &keysym,
                                            0 /*&compose*/);
                        if (keysym &&
                            keysym <
                                0x400) {  // a character in latin-1,2,3,4 sets
                            // force it to type a character (not sure if this
                            // ever is needed): if (!len) {kp_buffer[0] =
                            // char(keysym); len = 1;}
                            len =
                                fl_utf8encode(XKeysymToUcs(keysym), kp_buffer);
                            if (len < 1) len = 1;
                            // ignore all effects of shift on the keysyms, which
                            // makes it a lot easier to program shortcuts and is
                            // Windoze-compatible:
                            keysym = fl_KeycodeToKeysym(fl_display, keycode, 0);
                        }
                    }
                    ...
                    // Stupid X sends fake key-up events when a repeating key is
                    // held down, probably due to some back compatibility
                    // problem. Fortunately we can detect this because the
                    // repeating KeyPress event is in the queue, get it and
                    // execute it instead:

                    // Bool XkbSetDetectableAutoRepeat ( display, detectable,
                    // supported_rtrn ) Display * display ; Bool detectable ;
                    // Bool * supported_rtrn ;
                    // ...would be the easy way to correct this issue.
                    // Unfortunately, this call is also broken on many Unix
                    // distros including Ubuntu and Solaris (as of Dec 2009)

                    // Bogus KeyUp events are generated by repeated KeyDown
                    // events. One necessary condition is an identical key event
                    // pending right after the bogus KeyUp. The new code
                    // introduced Dec 2009 differs in that it only checks the
                    // very next event in the queue, not the entire queue of
                    // events. This function wrongly detects a repeat key if a
                    // software keyboard sends a burst of events containing two
                    // consecutive equal keys. However, in every non-gaming
                    // situation, this is no problem because both KeyPress
                    // events will cause the expected behavior.
                    XEvent peekevent;
                    if (XPending(fl_display)) {
                        XPeekEvent(fl_display, &peekevent);
                        if ((peekevent.type ==
                             KeyPress)  // must be a KeyPress event
                            && (peekevent.xkey.keycode ==
                                xevent.xkey.keycode)  // must be the same key
                            &&
                            (peekevent.xkey.time ==
                             xevent.xkey
                                 .time)  // must be sent at the exact same time
                        ) {
                            XNextEvent(fl_display, &xevent);
                            goto KEYPRESS;
                        }
...
#ifdef __sgi
                // You can plug a microsoft keyboard into an sgi but the extra
                // shift keys are not translated.  Make them translate like
                // XFree86 does:
                ...
                // Attempt to fix keyboards that send "delete" for the key in
                // the upper-right corner of the main keyboard.  But it appears
                // that very few of these remain?
...
#endif
                // For the first few years, there wasn't a good consensus on
                // what the Windows keys should be mapped to for X11. So we need
                // to help out a bit and map all variants to the same FLTK
                // key...
                switch (keysym) {
                    case XK_Meta_L:
                    case XK_Hyper_L:
                ...
                // We have to get rid of the XK_KP_function keys, because they
                // are not produced on Windoze and thus case statements tend not
                // to check for them.  There are 15 of these in the range 0xff91
                // ... 0xff9f
                if (keysym >= 0xff91 && keysym <= 0xff9f) {
                    // Map keypad keysym to character or keysym depending on
                    // numlock state...
                    unsigned long keysym1 =
                        fl_KeycodeToKeysym(fl_display, keycode, 1);
                    if (keysym1 <= 0x7f ||
                        (keysym1 > 0xff9f && keysym1 <= FL_KP_Last))
                ...
                // replace XK_ISO_Left_Tab (Shift-TAB) with FL_Tab (modifier
                // flags are set correctly by X11)
            ...
            case PropertyNotify:
                if (xevent.xproperty.atom == fl_NET_WM_STATE) {
                    ...
                    if (xevent.xproperty.state != PropertyDelete) {
                        ...
                        if (0 == get_xwinprop(xid, fl_NET_WM_STATE, 64, &nitems,
                                              &words)) {
                            for (unsigned long item = 0; item < nitems;
                                 item++) {
            ...
            // I now think this is an unavoidable problem with X: it is
            // impossible for a window manager to prevent the "real" notify
            // event from being sent when it resizes the contents, even though
            // it can send an artificial event with the correct position
            // afterwards (and some window managers do not send this fake event
            // anyway) So anyway, do a round trip to find the correct x,y:
            case MapNotify:
                event = FL_SHOW;
            ...
            case ReparentNotify: {
                ...
                // on some systems, the ReparentNotify event is not handled as
                // we would expect.
                XErrorHandler oldHandler = XSetErrorHandler(catchXExceptions());

                // ReparentNotify gives the new position of the window relative
                // to the new parent. FLTK cares about the position on the root
                // window.
    ...
    switch (xevent.type - xfixes_event_base) {
        case XFixesSelectionNotify: {
            ...
            if ((selection_notify->selection == XA_PRIMARY) &&
                ...
                handle_clipboard_timestamp(
                    1, selection_notify->selection_timestamp);
...
static void send_wm_event(Window wnd, Atom message, unsigned long d0,
                          unsigned long d1 = 0, unsigned long d2 = 0,
                          unsigned long d3 = 0, unsigned long d4 = 0) {
    XEvent e;
    e.xany.type = ClientMessage;
...
int Fl_X::ewmh_supported() {
    ...
    if (result == -1) {
        ...
        if (0 == get_xwinprop(XRootWindow(fl_display, fl_screen),
                              fl_NET_SUPPORTING_WM_CHECK, 64, &nitems,
                              &words) &&
            nitems == 1) {
...
void Fl_Window::fullscreen_x() {
    if (Fl_X::ewmh_supported()) {
        ...
        send_wm_event(fl_xid(this), fl_NET_WM_FULLSCREEN_MONITORS, top, bottom,
                      left, right);
...
static const int XEventMask =
    ...
    KeymapStateMask | FocusChangeMask | ButtonPressMask | ButtonReleaseMask |
    EnterWindowMask | LeaveWindowMask | PropertyChangeMask | PointerMotionMask;
...
void Fl_X::make_xid(Fl_Window *win, XVisualInfo *visual, Colormap colormap) {
    ...
    ulong root = win->parent() ? fl_xid(win->window())
    ...
    Fl_X *xp = set_xid(win, XCreateWindow(fl_display, root, X, Y, W, H,
                                          0,  // borderwidth
    ...
    XSetWMProperties(fl_display, xp->xid, NULL, NULL, NULL, 0, NULL, NULL,
                     NULL);

    // Set _NET_WM_PID
    long pid;
    pid = getpid();
    XChangeProperty(fl_display, xp->xid, fl_NET_WM_PID, XA_CARDINAL, 32, 0,
                    (unsigned char *)&pid, 1);
    ...
    if (!win->parent() && !attr.override_redirect) {
        ...
        XChangeProperty(fl_display, xp->xid, WM_PROTOCOLS, XA_ATOM, 32, 0,
                        (uchar *)&WM_DELETE_WINDOW, 1);
        ...
        if (win->non_modal() && xp->next && !fl_disable_transient_for) {
            ...
            if (win->modal()) {
                ...
                Atom net_wm_state_skip_taskbar =
                    XInternAtom(fl_display, "_NET_WM_STATE_MODAL", 0);
        ...
        if (!win->border()) {
            Atom net_wm_state = XInternAtom(fl_display, "_NET_WM_STATE", 0);
            Atom net_wm_state_skip_taskbar =
                XInternAtom(fl_display, "_NET_WM_STATE_SKIP_TASKBAR", 0);
            XChangeProperty(fl_display, xp->xid, net_wm_state, XA_ATOM, 32,
                            PropModeAppend,
                            (unsigned char *)&net_wm_state_skip_taskbar, 1);
        }
        ...
        if (win->fullscreen_active() && Fl_X::ewmh_supported()) {
            ...
            XChangeProperty(fl_display, xp->xid, fl_NET_WM_FULLSCREEN_MONITORS,
                            XA_ATOM, 32, PropModeReplace, (unsigned char *)data,
                            4);
            XChangeProperty(fl_display, xp->xid, fl_NET_WM_STATE, XA_ATOM, 32,
                            PropModeAppend,
                            (unsigned char *)&fl_NET_WM_STATE_FULLSCREEN, 1);
        }

        // Make it receptive to DnD:
        long version = 4;
        XChangeProperty(fl_display, xp->xid, fl_XdndAware, XA_ATOM,
                        sizeof(int) * 8, 0, (unsigned char *)&version, 1);
    ...
    if (win->menu_window() || win->tooltip_window()) {
        Atom net_wm_type =
            XInternAtom(fl_display, "_NET_WM_WINDOW_TYPE", False);
        Atom net_wm_type_kind =
            XInternAtom(fl_display, "_NET_WM_WINDOW_TYPE_MENU", False);
...
void Fl_X::sendxjunk() {
    ...
    if (hints->min_width != hints->max_width ||
        ...
        if (hints->max_width >= hints->min_width ||
            ...
            // window fit on screen when maximized, others will put it off
            // screen:
        ...
        prop[1] =
            1 | 2 | 16;  // MWM_FUNC_ALL | MWM_FUNC_RESIZE | MWM_FUNC_MAXIMIZE
    }
    ...
    XChangeProperty(fl_display, xid, fl_MOTIF_WM_HINTS, fl_MOTIF_WM_HINTS, 32,
                    0, (unsigned char *)prop, 5);
    XFree(hints);
}
...
static void icons_to_property(const Fl_RGB_Image *icons[], int count,
    ...
    sz = 0;
    for (int i = 0; i < count; i++) sz += 2 + icons[i]->w() * icons[i]->h();
    ...
    *len = sz;
    ...
    for (int i = 0; i < count; i++) {
        ...
        const int extra_data =
            image->ld() ? (image->ld() - image->w() * image->d()) : 0;
        ...
        for (int y = 0; y < image->h(); y++) {
            for (int x = 0; x < image->w(); x++) {
                switch (image->d()) {
                    case 1:
                        *data =
                            (0xff << 24) | (in[0] << 16) | (in[0] << 8) | in[0];
                        break;
                    case 2:
                        *data = (in[1] << 24) | (in[0] << 16) | (in[0] << 8) |
                                in[0];
                        break;
                    case 3:
                        *data =
                            (0xff << 24) | (in[0] << 16) | (in[1] << 8) | in[2];
                        break;
                    case 4:
                        *data = (in[3] << 24) | (in[0] << 16) | (in[1] << 8) |
                                in[2];
                        break;
                }
                in += image->d();
...
void Fl_X::set_default_icons(const Fl_RGB_Image *icons[], int count) {
    ...
    if (count > 0)
        icons_to_property(icons, count, &default_net_wm_icons,
...
void Fl_X::set_icons() {
    ...
    if (w->icon_->count) {
        icons_to_property((const Fl_RGB_Image **)w->icon_->icons,
                          w->icon_->count, &net_wm_icons, &net_wm_icons_size);
    } else {
    ...
    XChangeProperty(fl_display, xid, fl_NET_WM_ICON, XA_CARDINAL, 32,
                    PropModeReplace, (unsigned char *)net_wm_icons,
                    net_wm_icons_size);
...
void Fl_Window::label(const char *name, const char *iname) {
    ...
    if (shown() && !parent()) {
        ...
        XChangeProperty(fl_display, i->xid, fl_NET_WM_NAME, fl_XaUtf8String, 8,
                        0, (uchar *)name, namelen);  // utf8
        XChangeProperty(fl_display, i->xid, XA_WM_NAME, XA_STRING, 8, 0,
                        (uchar *)name, namelen);  // non-utf8
        XChangeProperty(fl_display, i->xid, fl_NET_WM_ICON_NAME,
                        fl_XaUtf8String, 8, 0, (uchar *)iname,
                        inamelen);  // utf8
        XChangeProperty(fl_display, i->xid, XA_WM_ICON_NAME, XA_STRING, 8, 0,
                        (uchar *)iname, inamelen);  // non-utf8
    }
}
...
void Fl_Paged_Device::draw_decorated_window(Fl_Window *win, int x_offset,
                                            int y_offset,
                                            Fl_Surface_Device *toset) {
    ...
    do_it = (XQueryTree(fl_display, fl_window, &root, &parent, &children, &n) !=
                 0 &&
             XTranslateCoordinates(fl_display, fl_window, parent, 0, 0, &bx,
                                   &bt, &child_win) == True);
    if (n) XFree(children);
    // hack to bypass STR #2648: when compiz is used, root and parent are the
    // same window and I don't know where to find the window decoration
    ...
    if (bx) {
        ...
        right_image =
            fl_read_image(NULL, win->w() + bx, bt, -bx, win->h() + bx);
        bottom_image =
            fl_read_image(NULL, 0, bt + win->h(), -(win->w() + 2 * bx), bx);
    }
    fl_window = from;
    toset->set_current();
    if (top_image) {
    ...
    if (bx) {
        if (left_image)
            fl_draw_image(left_image, x_offset, y_offset + bt, bx,
                          win->h() + bx, 3);
        if (right_image)
            fl_draw_image(right_image, x_offset + win->w() + bx, y_offset + bt,
                          bx, win->h() + bx, 3);
        if (bottom_image)
            fl_draw_image(bottom_image, x_offset, y_offset + bt + win->h(),
                          win->w() + 2 * bx, bx, 3);
...
// separate window contains also preparePrintFront call above
...
void copyFront(Fl_Widget *o, void *data) {
    ...
    Fl_Window *win = Fl::first_window();
    if (!win) return;
    Fl_Copy_Surface *surf =
        new Fl_Copy_Surface(win->decorated_w(), win->decorated_h());
    surf->set_current();
