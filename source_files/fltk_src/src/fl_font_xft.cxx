...
#if 1
    {" sans"},   {"Bsans"},   {"Isans"},   {"Psans"},
    {" mono"},   {"Bmono"},   {"Imono"},   {"Pmono"},
    {" serif"},  {"Bserif"},  {"Iserif"},  {"Pserif"},
...
static void fl_xft_font(Fl_Xlib_Graphics_Driver *driver, Fl_Font fnum,
                        Fl_Fontsize size, int angle) {
    ...
    for (f = font->first; f; f = f->next) {
        if (f->size == size &&
            f->angle == angle)  // && !strcasecmp(f->encoding, fl_encoding_))
            break;
    }
    if (!f) {
...
static XftFont *fontopen(const char *name, Fl_Fontsize size, bool core,
                         int angle) {
    ...
    if (len > 512)
        len = 512;  // ensure we are not passed an unbounded font name
    for (unsigned idx = 0; idx < len; idx++) {
        if (name[idx] == '-') hyphen_count++;  // check for XLFD hyphens
        if (name[idx] == ',') comma_count++;   // are there multiple names?
    }
    if (hyphen_count >= 14)
        is_xlfd = true;  // Not a robust check, but good enough?
    ...
    if (!is_xlfd) {                // Not an XLFD - open as a XFT style name
        ...
        /* This "converts" FLTK-style font names back into "regular" names,
         * extracting the BOLD and ITALIC codes as it does so - all FLTK font
         * names are prefixed by 'I' (italic) 'B' (bold) 'P' (bold italic) or '
         * ' (regular) modifiers. This gives a fairly limited font selection
         * ability, but is retained for compatibility reasons. If you really
         * need a more complex choice, you are best calling Fl::set_fonts(*)
         * then selecting the font by font-index rather than by name anyway.
         * Probably. If you want to load a font who's name does actually begin
         * with I, B or P, you MUST use a leading space OR simply use lowercase
         * for the name...
         */
        ...
        if (comma_count) {  // multiple comma-separated names were passed
            char *local_name = strdup(
                name);  // duplicate the full name so we can edit the copy
            ...
            do {
                ...
                // subsequent names we ignore this for... But we still need to
                // do the check.
                switch (*curr++) {
                    case 'I':
                        break;  // italic
        ...
        if (angle != 0) {
            XftMatrix m;
            XftMatrixInit(&m);
            XftMatrixRotate(&m, cos(M_PI * angle / 180.),
                            sin(M_PI * angle / 180.));
...
#if 0  // diagnostic to print the "full name" of the font we matched. This
       // works.
    ...
    free(picked_name);
        ...
        if (!match_pat || !the_font) {
            ...
            the_font = XftFontOpen(fl_display, fl_screen, XFT_FAMILY,
                                   XftTypeString, "sans", XFT_SIZE,
                                   XftTypeDouble, (double)size, NULL);
            XftPatternDestroy(fnt_pat);
            if (!the_font) {
                Fl::error(
                    "Unable to find fonts. Check your FontConfig "
                    "configuration.\n");
                exit(1);
            }
            return the_font;
    ...
    free(picked_name2);
        ...
        XftPatternDestroy(fnt_pat);
        //  XftPatternDestroy(match_pat); // FontConfig will destroy this
        //  resource for us. We must not!
         ...
         * list. But XftFontOpenXlfd can not do this, so if a list is passed, we
         * just terminate it at the first comma. A "better" solution might be to
         * use XftXlfdParse() on each of the passed XLFD's to construct a
         * "super-pattern" that incorporates attributes from all XLFD's and use
         * that to perform a XftFontMatch(). Maybe...
         */
...
static void utf8extents(Fl_Font_Descriptor *desc, const char *str, int n,
                        XGlyphInfo *extents) {
...
int Fl_Xlib_Graphics_Driver::height() {
    if (font_descriptor())
        return font_descriptor()->font->ascent +
...
int Fl_Xlib_Graphics_Driver::descent() {
    if (font_descriptor())
        return font_descriptor()->font->descent;
    else
        return -1;
}
...
void Fl_Xlib_Graphics_Driver::text_extents(const char *c, int n, int &dx,
                                           int &dy, int &w, int &h) {
    if (!font_descriptor()) {
        w = h = 0;
        dx = dy = 0;
...
// for GL or other bitmap font needs (we dont have an XglUseXftFont(...)
// function.) There's probably a better way to do this. I can't believe it is
// this hard... Anyway... This code attempts to make an XLFD out of the
// fltk-style font name it is passed, then tries to load that font.
// Surprisingly, this quite often works - boxes that have XFT generally also
// have a fontserver that can serve TTF and other fonts to X, and so the font
// name that fltk makes from the XFT name often also "exists" as an "core" X
// font... If this code fails to load the requested font, it falls back through
// a series of tried 'n tested alternatives, ultimately resorting to what the
...
static XFontStruct *load_xfont_for_xft2(Fl_Graphics_Driver *driver) {
    ...
    const char *name =
        pc;  // keep a handle to the original name for freeing later
    ...
    if (xgl_font) {  // the face exists, but can we get it in a suitable size?
        ...
        snprintf(xlfd, 128, "-*-%s-%s-%c-*--*-%d-*-*-*-*-*-*", name, weight,
                 slant, (size * 10));
        xgl_font = XLoadQueryFont(
            fl_display, xlfd);  // attempt to load the font at the right size
    }
    // puts(xlfd);
...
static XFontStruct *fl_xxfont(Fl_Graphics_Driver *driver) {
    ...
    if ((!xgl_font) || (glsize != driver->size()) ||
        (glfont != driver->font())) {
        // create a dummy XLFD for some font of the appropriate size...
        if (xgl_font)
            XFreeFont(fl_display, xgl_font);  // font already loaded, free it -
    ...
    if (driver->font_descriptor()->font->core) {
        return driver->font_descriptor()
            ->font->u.core
            .font;  // is the current font a "core" font? If so, use it.
    }
    static XftFont *xftfont;
    if (xftfont) XftFontClose(fl_display, xftfont);
    xftfont = fontopen(
        fl_fonts[driver->font()].name, driver->size(), true,
        0);  // else request XFT to load a suitable "core" font instead.
...
void Fl_Xlib_Graphics_Driver::draw(const char *str, int n, int x, int y) {
    ...
    if (fl_overlay) {
        if (!draw_)
            draw_ =
                XftDrawCreate(fl_display, draw_overlay_window = fl_window,
    ...
    const wchar_t *buffer = utf8reformat(str, n);
#ifdef __CYGWIN__
    XftDrawString16(draw_, &color, font_descriptor()->font, x, y,
                    (XftChar16 *)buffer, n);
#else
    XftDrawString32(draw_, &color, font_descriptor()->font, x, y,
                    (XftChar32 *)buffer, n);
#endif
}
...
static void fl_drawUCS4(Fl_Graphics_Driver *driver, const FcChar32 *str, int n,
                        int x, int y) {
#if USE_OVERLAY
    XftDraw *&draw_ = fl_overlay ? draw_overlay : ::draw_;
    if (fl_overlay) {
    ...
    XftDrawString32(draw_, &color, driver->font_descriptor()->font, x, y,
                    (FcChar32 *)str, n);
}
...
// FIXME: warning Need to improve this XFT right to left draw function
    ...
    // reversed... but you can't just byte-rev a UTF-8 string, that isn't valid.
    ...
    if (num_chars < n)
        n = num_chars;  // limit drawing to usable characters in input array
