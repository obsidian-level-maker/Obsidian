...
static void scroll_cb(Fl_Widget *, void *data) { ((Fl_Tree *)data)->redraw(); }
...
static char **parse_path(const char *path) {
    ...
    char *cp = new char[(len + 1)], *word = cp,
         *s = cp;  // freed below or in free_path()
...
/// Constructor.
Fl_Tree::Fl_Tree(int X, int Y, int W, int H, const char *L)
    : Fl_Group(X, Y, W, H, L) {
...
/// \version 1.3.3
///
int Fl_Tree::extend_selection_dir(Fl_Tree_Item *from, Fl_Tree_Item *to, int dir,
                                  int val, bool visible) {
    int changed = 0;
    for (Fl_Tree_Item *item = from; item;
         item = next_item(item, dir, visible)) {
        switch (val) {
            case 0:
...
int Fl_Tree::extend_selection(Fl_Tree_Item *from, Fl_Tree_Item *to, int val,
                              bool visible) {
#else
...
int Fl_Tree::extend_selection__(Fl_Tree_Item *from, Fl_Tree_Item *to, int val,
                                bool visible) {
#endif
    int changed = 0;
    if (from == to) {
    ...
    char on = 0;
    for (Fl_Tree_Item *item = first(); item;
         item = item->next_visible(_prefs)) {
...
void Fl_Tree::extend_selection(Fl_Tree_Item *from, Fl_Tree_Item *to) {
    ...
    const bool visible =
        false;  // true=only 'open' items, false='open' or 'closed'
    extend_selection__(from, to, val,
                       visible);  // use private method until we can release it
}
#endif
...
int Fl_Tree::handle(int e) {
    if (e == FL_NO_EVENT)
        return (0);  // XXX: optimize to prevent slow resizes on large trees!
    int ret = 0;
    char is_shift = Fl::event_state() & FL_SHIFT ? 1 : 0;
    char is_ctrl = Fl::event_state() & FL_CTRL ? 1 : 0;
    char is_command = Fl::event_state() & FL_COMMAND
                          ? 1
                          : 0;  // ctrl on win/lin, 'Command' on mac
#if FLTK_ABI_VERSION >= 10301
    ...
    switch (e) {
        case FL_FOCUS: {
            ...
            //     If a nav key was used to give us focus, and we've got no
            //     saved focus widget, determine which item gets focus depending
            //     on nav key.
            //
            if (!_item_focus) {  // no focus established yet?
                switch (
                    Fl::event_key()) {  // determine if focus was navigated..
                    case FL_Tab: {      // received focus via TAB?
                        int updown = is_shift
                                         ? FL_Up
                                         : FL_Down;  // SHIFT-TAB similar to Up,
                                                     // TAB similar to Down
            ...
            if ((Fl::focus() == this) &&  // tree has focus?
                ...
                if (_item_focus) {
                    ...
                    switch (ekey) {
                            ...
                            switch (_prefs.selectmode()) {
                                ...
                                case FL_TREE_SELECT_MULTI:
                                    if (is_ctrl) {
                                        select_toggle(
                                            _item_focus,
                                            when());  // CTRL-SPACE: (multi
                                                      // mode) toggle selection
                                    } else {
                                        select(_item_focus,
                                               when());  // SPACE: (multi-mode)
                                                         // select
                                    }
                                    _lastselect = _item_focus;
                                    return (1);  // done, we handled key
                        ...
                        case FL_Down: {  // DOWN: next item down, or extend
                                         // selection down
                            set_item_focus(next_visible_item(
                                _item_focus, ekey));  // next item up|dn
                            if (_item_focus) {        // item in focus?
                                                      // Autoscroll
                                int itemtop = _item_focus->y();
                                int itembot =
                                    _item_focus->y() + _item_focus->h();
                                if (itemtop < y()) {
                                ...
                                // Extend selection
                                if (_prefs.selectmode() ==
                                        FL_TREE_SELECT_MULTI &&  // multiselect
                                                                 // on?
                                    is_shift &&                  // shift key?
                                    !_item_focus
                                         ->is_selected()) {  // not already
                                                             // selected?
                                    select(_item_focus,
                                           when());  // extend selection..
                                    _lastselect = _item_focus;
                                }
                                return (1);
                            }
    ...
    switch (e) {
        case FL_PUSH: {               // clicked on tree
            ...
            set_item_focus(
                item);  // becomes new focus widget, calls redraw() if needed
            ret |= 1;   // handled
            if (Fl::event_button() == FL_LEFT_MOUSE) {
                if (item->event_on_collapse_icon(
                        _prefs)) {      // collapse icon clicked?
                    open_toggle(item);  // toggle open (handles redraw)
                } else if (item->event_on_label(_prefs) &&  // label clicked?
                           (!item->widget() ||
                            !Fl::event_inside(
                                item->widget()))) {  // not inside widget
                    switch (_prefs.selectmode()) {
                        case FL_TREE_SELECT_NONE:
                            break;
                        case FL_TREE_SELECT_SINGLE:
                        ...
                        case FL_TREE_SELECT_MULTI: {
                            if (is_shift) {  // SHIFT+PUSH?
                                if (_lastselect) {
                                    ...
                                    bool visible = true;
#if FLTK_ABI_VERSION >= 10303
                                    extend_selection(_lastselect, item, val,
                                                     visible);
#else
                                    extend_selection__(_lastselect, item, val,
                                                       visible);
#endif
                                } else {
                                ...
                                select_toggle(
                                    item, when());  // toggle selection state
                            } else {
        ...
        case FL_DRAG: {
...
#if FLTK_ABI_VERSION >= 10303
            Fl_Tree_Item *item =
                _root->find_clicked(_prefs, 1);  // item we're on, vertically
#else
            Fl_Tree_Item *item =
                _root->find_clicked(_prefs);  // item we're on, vertically
#endif
            ...
            if (item == _lastselect)
                break;  // same item as before? avoid reselect
            ...
            switch (_prefs.selectmode()) {
                ...
                case FL_TREE_SELECT_SINGLE: {
                    select_only(
                        item,
                        when());  // select only this item (handles redraw)
                    break;
                }
                case FL_TREE_SELECT_SINGLE_DRAGGABLE: {
                ...
                case FL_TREE_SELECT_MULTI: {
                    Fl_Tree_Item *from = next_visible_item(
                        _lastselect, dir);  // avoid reselecting item
                    Fl_Tree_Item *to = item;
                    int val =
                        is_ctrl ? 2 : 1;  // toggle_select() or just select()?
                    bool visible = true;
...
#if FLTK_ABI_VERSION >= 10303
                Fl_Tree_Item *item = _root->find_clicked(
                    _prefs, 1);  // item we're on, vertically
#else
                Fl_Tree_Item *item =
                    _root->find_clicked(_prefs);  // item we're on, vertically
#endif
                ...
                if (item && _lastselect && item != _lastselect &&
                    ...
                    if ((before && prev(item) != _lastselect) ||
                        ...
                        if (parent) {
                            ...
                            // Special case: trying to drop right before a
                            // folder
                            ...
                            // If we're moving inside the same parent, use the
                            // below/above methods
                            ...
                            redraw();
                            do_callback_for_item(_lastselect,
...
void Fl_Tree::calc_dimensions() {
    ...
    if (_tree_h >= 0 && _tree_w >= 0) {
        int scrollsize =
            _scrollbar_size ? _scrollbar_size : Fl::scrollbar_size();
        ...
        if (vshow) {
            _vscroll->show();
            _vscroll->resize(
                _tox + _tow - scrollsize, _toy, scrollsize,
                h() - Fl::box_dh(box()) - (hshow ? scrollsize : 0));
        } else {
            _vscroll->hide();
            _vscroll->value(0);
        }
...
void Fl_Tree::calc_tree() {
    // Set tree width and height to zero, and recalc just _tox/_toy/_tow/_toh
    // for now.
    _tree_w = _tree_h = -1;
    calc_dimensions();
    if (!_root) return;
    ...
    _root->draw(X, Y, W, 0, xmax, 1,
                render);  // descend into tree without drawing (render=0)
    ...
    // Calc tree dims again; now that tree_w/tree_h are known, scrollbars are
    // calculated.
    calc_dimensions();
}
#endif
...
void Fl_Tree::draw() {
    ...
    if (_vscroll->visible() && _hscroll->visible()) {
        ...
        fl_rectf(_hscroll->x() + _hscroll->w(), _vscroll->y() + _vscroll->h(),
                 _vscroll->w(), _hscroll->h());
    }
...
void Fl_Tree::draw() {
    ...
    if (_vscroll->visible() && ytoofar > 0) {
        ...
        Fl::add_timeout(.10, redraw_soon,
                        (void *)this);  // use timer to trigger redraw; we can't
    }
...
int Fl_Tree::draw_tree() {
    ...
    int ch = h() - Fl::box_dh(box());
    {
        ...
        int Y = cy + _prefs.margintop() -
                (_vscroll->visible() ? _vscroll->value() : 0);
...
#endif /*FLTK_ABI_VERSION*/
            int ydiff =
                (SY + _prefs.margintop()) - Ysave;  // ydiff=size of tree
            int ytoofar =
                (cy + ch) - SY;  // ytoofar -- if >0, scrolled beyond bottom
...
Fl_Tree_Item *Fl_Tree::root() { return (_root); }
...
///     "12/25/2010" tree->add("/Pathnames/c:\\\\Program Files\\\\MyApp"); //
///     Adds item "c:\Program Files\MyApp"
/// \endcode
...
///
Fl_Tree_Item *Fl_Tree::add(const char *path) { return add(path, 0); }
#endif
...
/// if (item) tree->insert_above(item, "New item");   // insert new item above
/// it \endcode
///
/// \param[in] above -- the item above which to insert the new item. Must not be
/// NULL. \param[in] name -- the name of the new item \returns The new item
/// added, or 0 if 'above' could not be found. \see insert()
///
...
/// if (item) tree->insert(item, "New item", 2);  // insert as a child of Aaa at
/// index #2 \endcode
///
/// \param[in] item The existing item to insert new child into. Must not be
/// NULL. \param[in] name The label for the new item \param[in] pos The position
/// of the new item in the child list \returns The new item added. \see
/// insert_above()
///
...
/// "Flintstones/Fred") \returns The item, or NULL if not found. \see
/// item_pathname()
///
...
/// Must not be zero. \param[in] item The item whose pathname is to be returned.
/// \returns
...
/// \see find_item()
///
int Fl_Tree::item_pathname(char *pathname, int pathnamelen,
    ...
    SAFE_RCAT('\0');
    while (item) {
        if (item->is_root() && showroot() == 0)
            break;  // don't include root in path if showroot() off
        // Find name of current item
        const char *name =
            item->label() ? item->label() : "???";  // name for this item
    ...
    if (s != pathname)
        memmove(pathname, s, slen);  // Shift down right-aligned string
    return (0);
}
...
///
void Fl_Tree::item_clicked(Fl_Tree_Item *item) { _callback_item = item; }
...
/// \deprecated in 1.3.3 ABI -- use callback_item() instead.
///
Fl_Tree_Item *Fl_Tree::item_clicked() { return (_callback_item); }
...
/// for ( Fl_Tree_Item *i=tree->first_visible_item(); i;
/// i=tree->next_visible_item(i, FL_Down) )
///     printf("Item: %s\n", i->label());
///
/// // Walk up the tree (backwards)
/// for ( Fl_Tree_Item *i=tree->last_visible_item(); i;
/// i=tree->next_visible_item(i, FL_Up) )
///     printf("Item: %s\n", i->label());
/// \endcode
...
/// specified \p item. \version 1.3.3
///
...
///
Fl_Tree_Item *Fl_Tree::first_visible() { return (first_visible_item()); }
...
/// \endcode
///
/// \param[in] item The item to use to find the previous item. If NULL, returns
/// 0. \returns Previous item in tree, or 0 if at first item.
///
/// \see first(), next(), last(), prev()
///
...
///
Fl_Tree_Item *Fl_Tree::last_visible() { return (last_visible_item()); }
...
/// for ( Fl_Tree_Item *i=tree->first_selected_item(); i;
/// i=tree->next_selected_item(i) )
...
///
Fl_Tree_Item *Fl_Tree::first_selected_item() { return (next_selected_item(0)); }
...
///
/// This is a convenience method; equivalent to next_selected_item(item,
/// FL_Down);
///
/// Use this to walk the tree forward (downward) looking for all the selected
/// items, e.g. \code for ( Fl_Tree_Item *i = tree->first_selected_item(); i; i
/// = tree->next_selected_item(i) )
///     printf("Selected item: %s\n", i->label());
/// \endcode
///
/// \param[in] item The item to use to find the next selected item. If NULL,
/// first() is used. \returns The next selected item, or 0 if there are no more
/// selected items. \see first_selected_item(), last_selected_item(),
/// next_selected_item()
///
...
/// for ( Fl_Tree_Item *i=tree->last_selected_item(); i;
/// i=tree->next_selected_item(i, FL_Up) )
///     printf("Selected item: %s\n", i->label());
/// \endcode
///
...
/// If \p 'visible' is true, only items whose parents are open() will be
/// returned. If \p 'visible' is false, even items whose parents are close()ed
/// will be returned.
///
...
/// for ( Fl_Tree_Item *i=tree->first_visible_item(); i; i=tree->next_item(i,
/// FL_Down, true) )
///     printf("Item: %s\n", i->label());
///
/// // Walk up the tree showing open(), visible items
/// for ( Fl_Tree_Item *i=tree->last_visible_item(); i; i=tree->next_item(i,
/// FL_Up, true) )
///     printf("Item: %s\n", i->label());
///
...
/// 0. \param[in] dir     Can be FL_Up or FL_Down (default=FL_Down or 'next')
...
Fl_Tree_Item *Fl_Tree::next_item(Fl_Tree_Item *item, int dir, bool visible) {
    ...
    switch (dir) {
        case FL_Up:
            if (visible)
                return (item->prev_visible(_prefs));
            else
                return (item->prev());
        case FL_Down:
            if (visible)
                return (item->next_visible(_prefs));
            else
                return (item->next());
    }
...
/// Returns the next selected item above or below \p 'item', depending on \p
/// 'dir'. If \p 'item' is 0, search starts at either first() or last(),
/// depending on \p 'dir': first() if \p 'dir' is FL_Down (default), last() if
/// \p 'dir' is FL_Up.
///
...
/// // Walk down the tree (forwards)
/// for ( Fl_Tree_Item *i=tree->first_selected_item(); i;
/// i=tree->next_selected_item(i, FL_Down) )
///     printf("Item: %s\n", i->label());
///
/// // Walk up the tree (backwards)
/// for ( Fl_Tree_Item *i=tree->last_selected_item(); i;
...
/// \endcode
///
/// \param[in] item The item above or below which we'll find the next selected
/// item.
...
#if FLTK_ABI_VERSION >= \
    10303 /* reason for this: Fl_Tree_Item_Array::manage_item_destroy() */
...
int Fl_Tree::get_selected_items(Fl_Tree_Item_Array &ret_items) {
    ret_items.clear();
    for (Fl_Tree_Item *i = first_selected_item(); i;
         i = next_selected_item(i)) {
        ret_items.add(i);
    }
...
/// determine the item changed and the reason the callback was called.
///
/// \param[in] item -- the item to be opened. Must not be NULL.
/// \param[in] docallback -- A flag that determines if the callback() is invoked
/// or not:
...
///
/// \see open(), close(), is_open(), is_close(), callback_item(),
/// callback_reason()
///
...
/// determine the item changed and the reason the callback was called.
///
/// \param[in] item -- the item whose open state is to be toggled. Must not be
/// NULL. \param[in] docallback -- A flag that determines if the callback() is
/// invoked or not:
///     -   0 - callback() is not invoked
///     -   1 - callback() is invoked (default), callback_reason() will be
///     either
...
///     -   0 : item is open
///
int Fl_Tree::is_close(Fl_Tree_Item *item) const { return (item->is_close()); }
...
/// docallback.<br> Handles calling redraw() if anything changed.
///
/// The callback can use callback_item() and callback_reason() respectively to
/// determine the item changed and the reason the callback was called.
///
...
void Fl_Tree::select_toggle(Fl_Tree_Item *item, int docallback) {
    ...
    set_changed();
    if (docallback) {
        do_callback_for_item(item, item->is_selected()
...
///     (default),
///             callback_reason() will be FL_TREE_REASON_DESELECTED
/// \returns Count of how many items were actually changed to the deselected
/// state.
///
int Fl_Tree::deselect_all(Fl_Tree_Item *item, int docallback) {
    ...
    // Deselect item
    if (item->is_selected())
        if (deselect(item, docallback)) ++count;
...
/// Select only the specified item, deselecting all others that might be
/// selected.
///
...
int Fl_Tree::select_all(Fl_Tree_Item *item, int docallback) {
    ...
    // Select item
    if (!item->is_selected())
        if (select(item, docallback)) ++count;
    // Select its children
    for (int t = 0; t < item->children(); t++) {
...
Fl_Tree_Item *Fl_Tree::get_item_focus() const { return (_item_focus); }
...
Fl_Fontsize Fl_Tree::item_labelsize() const { return (_prefs.labelsize()); }

/// Set the default label font size used for creating new items.
/// To change the font size on a per-item basis, use
/// Fl_Tree_Item::labelsize(Fl_Fontsize)
///
void Fl_Tree::item_labelsize(Fl_Fontsize val) { _prefs.labelsize(val); }

/// Get the default font face used for creating new items.
Fl_Font Fl_Tree::item_labelfont() const { return (_prefs.labelfont()); }

/// Set the default font face used for creating new items.
/// To change the font face on a per-item basis, use
/// Fl_Tree_Item::labelfont(Fl_Font)
///
void Fl_Tree::item_labelfont(Fl_Font val) { _prefs.labelfont(val); }
...
/// To change the foreground color on a per-item basis, use
/// Fl_Tree_Item::labelfgcolor(Fl_Color)
///
void Fl_Tree::item_labelfgcolor(Fl_Color val) { _prefs.labelfgcolor(val); }
...
/// A special case is made for color 0xffffffff (default) which is treated as
/// 'transparent'. To change the background color on a per-item basis, use
/// Fl_Tree_Item::labelbgcolor(Fl_Color)
///
void Fl_Tree::item_labelbgcolor(Fl_Color val) { _prefs.labelbgcolor(val); }

/// Get the connector color used for tree connection lines.
Fl_Color Fl_Tree::connectorcolor() const { return (_prefs.connectorcolor()); }

/// Set the connector color used for tree connection lines.
void Fl_Tree::connectorcolor(Fl_Color val) { _prefs.connectorcolor(val); }

/// Get the amount of white space (in pixels) that should appear
/// between the widget's left border and the tree's contents.
///
int Fl_Tree::marginleft() const { return (_prefs.marginleft()); }
...
///
int Fl_Tree::margintop() const { return (_prefs.margintop()); }
...
/// bottom.
///
int Fl_Tree::marginbottom() const { return (_prefs.marginbottom()); }
...
///
int Fl_Tree::linespacing() const { return (_prefs.linespacing()); }
...
int Fl_Tree::labelmarginleft() const { return (_prefs.labelmarginleft()); }
...
int Fl_Tree::widgetmarginleft() const { return (_prefs.widgetmarginleft()); }
...
///
int Fl_Tree::connectorwidth() const { return (_prefs.connectorwidth()); }
...
///
Fl_Image *Fl_Tree::usericon() const { return (_prefs.usericon()); }
...
/// a simple '[+]' icon.
///
Fl_Image *Fl_Tree::openicon() const { return (_prefs.openicon()); }
...
/// a simple '[-]' icon.
///
Fl_Image *Fl_Tree::closeicon() const { return (_prefs.closeicon()); }
...
int Fl_Tree::showcollapse() const { return (_prefs.showcollapse()); }
...
int Fl_Tree::showroot() const { return (_prefs.showroot()); }
...
///
Fl_Tree_Sort Fl_Tree::sortorder() const { return (_prefs.sortorder()); }
...
///
Fl_Boxtype Fl_Tree::selectbox() const { return (_prefs.selectbox()); }
...
///
Fl_Tree_Select Fl_Tree::selectmode() const { return (_prefs.selectmode()); }

/// Sets the tree's selection mode.
/// See ::Fl_Tree_Select for possible values.
///
void Fl_Tree::selectmode(Fl_Tree_Select val) { _prefs.selectmode(val); }
...
/// edges of the display window. This does NOT take into account the hide() /
...
int Fl_Tree::displayed(Fl_Tree_Item *item) {
    ...
    if (!item) return (0);
    return ((item->y() >= y()) && (item->y() <= (y() + h() - item->h())) ? 1
                                                                         : 0);
}
...
/// Adjust the vertical scrollbar so that \p 'item' is at the top of the
/// display.
///
/// \param[in] item The item to be shown. If NULL, first() is used.
///
...
/// The position returned is how many pixels of the tree are scrolled off the
/// top edge of the screen. \see vposition(int), hposition(), hposition(int)
///
int Fl_Tree::vposition() const { return ((int)_vscroll->value()); }
...
/// The position returned is how many pixels of the tree are scrolled off the
/// left edge of the screen. \see hposition(int), vposition(), vposition(int)
...
/// Use this to skip over the scrollbars when walking the child() array.
/// Example: \code for ( int i=0; i<tree->children(); i++ ) {    // walk
/// children
...
///
int Fl_Tree::scrollbar_size() const { return (_scrollbar_size); }
...
void Fl_Tree::scrollbar_size(int size) {
    ...
    if (_vscroll->w() != scrollsize) {
        _vscroll->resize(x() + w() - scrollsize, h(), scrollsize,
                         _vscroll->h());
    }
#if FLTK_ABI_VERSION >= 10303
    if (_hscroll->h() != scrollsize) {
        _hscroll->resize(x(), y() + h() - scrollsize, _hscroll->w(),
                         scrollsize);
    }
    // Changing scrollbar size affects _tiw/_tih + may affect scrollbar
    // visibility
    calc_dimensions();
#endif
}
...
///
void Fl_Tree::callback_item(Fl_Tree_Item *item) { _callback_item = item; }

/// Gets the item that caused the callback.
/// The callback() can use this value to see which item changed.
///
Fl_Tree_Item *Fl_Tree::callback_item() { return (_callback_item); }
...
///         Fl_Tree_Item *item = tree->callback_item();    // the item changed
///         (can be NULL if more than one item was changed!) switch (
///         tree->callback_reason() ) {           // reason callback was invoked
...
///             (double-clicked, etc).. case FL_TREE_REASON_DESELECTED: ..item
///             was deselected..
///         }
///     }
/// \endcode
...
///
Fl_Tree_Reason Fl_Tree::callback_reason() const { return (_callback_reason); }
...
void Fl_Tree::load(Fl_Preferences &prefs) {
    ...
    n = prefs.entries();
    for (i = 0; i < n; i++) {
        // We must remove all fwd slashes in the key and value strings. Replace
        // with backslash.
