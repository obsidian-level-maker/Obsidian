...
key\modifier   plain  Ctrl   Alt  Meta
  ...
  page down    12/8  24/0   0/12  0/0
    ...
    (adding the shift key extends the selection, all other combinations are
no-op)

  0: no-op
  1: move cursor to the left, at line beginning wrap to end of prev line, at doc
start no-op 2: move cursor to the right, at line end move to beginning of the
next line, at doc end no-op 3: move cursor up, at doc top no-op 4: move cursor
down, at doc bottom no-op 5: scroll display to top of text (cursor unchanged) 6:
scroll display to end of text (cursor unchanged) 7: scroll text down one page
(cursor unchanged) 8: scroll text up one page (cursor unchanged) 9: move cursor
to beginning of line 10: move cursor to end of line 11: move cursor up one page
and scroll down 12: move cursor down one page and scroll up 13: move to the
beginning of the word or the previous word 14: move to the end of the word or
the next word 15: if start of line: start of prev line, else start of this line
...
/**  Adds all of the default editor key bindings to the specified key binding
 * list.*/
void Fl_Text_Editor::add_default_key_bindings(Key_Binding **list) {
    for (int i = 0; default_key_bindings[i].key; i++) {
        add_key_binding(default_key_bindings[i].key,
                        ...
                        default_key_bindings[i].func, list);
    }
}
...
Fl_Text_Editor::Key_Func Fl_Text_Editor::bound_key_function(
    int key, int state, Key_Binding *list) const
#endif
{
    Key_Binding *cur;
...
/** Inserts a newline for key \p 'c' at the current cursor position in editor \p
   'e'. The key value \p 'c' is currently unused.
*/
...
/** Moves the text cursor in the direction indicated by key \p 'c' in editor \p
   'e'. Supported values for 'c' are currently: \code FL_Home      -- moves the
   cursor to the beginning of the current line FL_End       -- moves the cursor
   to the end of the current line FL_Left      -- moves the cursor left one
   character FL_Right     -- moves the cursor right one character FL_Up -- moves
   the cursor up one line FL_Down      -- moves the cursor down one line
...
int Fl_Text_Editor::kf_move(int c, Fl_Text_Editor *e) {
    ...
    if (!selected) e->dragPos = e->insert_position();
    e->buffer()->unselect();
    Fl::copy("", 0, 0);
    switch (c) {
        case FL_Home:
...
/** Extends the current selection in the direction of key \p 'c' in editor \p
   'e'. \see kf_move()
*/
...
/** Moves the current text cursor in the direction indicated by control key \p
   'c' in editor \p 'e'. Supported values for 'c' are currently: \code FL_Home
   -- moves the cursor to the beginning of the document FL_End       -- moves
   the cursor to the end of the document FL_Left      -- moves the cursor left
   one word FL_Right     -- moves the cursor right one word FL_Up        --
   scrolls up one line, without moving cursor FL_Down      -- scrolls down one
   line, without moving cursor FL_Page_Up   -- moves the cursor to the beginning
   of the top line on the current page FL_Page_Down -- moves the cursor to the
   beginning of the last line on the current page \endcode
*/
int Fl_Text_Editor::kf_ctrl_move(int c, Fl_Text_Editor *e) {
    if (!e->buffer()->selected()) e->dragPos = e->insert_position();
...
/** Moves the current text cursor in the direction indicated by meta key \p 'c'
   in editor \p 'e'. Supported values for 'c' are currently: \code FL_Up --
   moves cursor to the beginning of the current document FL_Down      -- moves
   cursor to the end of the current document FL_Left      -- moves the cursor to
   the beginning of the current line FL_Right     -- moves the cursor to the end
   of the current line \endcode
*/
...
/** Extends the current selection in the direction indicated by meta key \p 'c'
   in editor \p 'e'. \see kf_meta_move().
*/
...
/** Extends the current selection in the direction indicated by control key \p
...
/** Moves the text cursor one line up for editor \p 'e'.
    ...
    The key value \p 'c' is currently unused.
*/
int Fl_Text_Editor::kf_up(int, Fl_Text_Editor *e) { return kf_move(FL_Up, e); }
...
/** Does a delete of selected text or the current character in the current
   buffer of editor \p 'e'. The key value \p 'c' is currently unused.
*/
...
/** Does a copy of selected text or the current character in the current buffer
   of editor \p 'e'. The key value \p 'c' is currently unused.
*/
    ...
    switch (event) {
        ...
        case FL_PUSH:
            if (Fl::event_button() == 2) {
                ...
                int pos =
                    xy_to_position(Fl::event_x(), Fl::event_y(), CURSOR_POS);
                insert_position(pos);
                Fl::paste(*this, 0);
                Fl::focus(this);
                set_changed();
        ...
        case FL_DND_LEAVE:  // restore original cursor
            ...
            return 1;
        case FL_DND_RELEASE:  // keep insertion cursor and wait for the FL_PASTE
                              // event
            if (!dragging)
                buffer()->unselect();  // FL_PASTE must not destroy current
                                       // selection if drag comes from outside
            return 1;
    }
