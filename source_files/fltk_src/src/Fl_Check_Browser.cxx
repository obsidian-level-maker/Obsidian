...
void *Fl_Check_Browser::item_first() const { return first; }
...
int Fl_Check_Browser::item_height(void *) const { return textsize() + 2; }
...
void Fl_Check_Browser::item_draw(void *v, int X, int Y, int, int) const {
    ...
    fl_color(active_r() ? FL_FOREGROUND_COLOR
                        : fl_inactive(FL_FOREGROUND_COLOR));
    fl_loop(X, cy, X, cy + CHECK_SIZE, X + CHECK_SIZE, cy + CHECK_SIZE,
            X + CHECK_SIZE, cy);
    if (i->checked) {
        int tx = X + 3;
        int tw = CHECK_SIZE - 4;
...
*/
int Fl_Check_Browser::add(char *s) { return (add(s, 0)); }
...
int Fl_Check_Browser::value() const { return lineno((cb_item *)selection()); }
