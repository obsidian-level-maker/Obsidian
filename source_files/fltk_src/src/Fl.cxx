...
#define FD_ISSET(S, SET) (get_wsock_mod() ? fl_wsk_fd_is_set(S, SET) : 0)
#endif  // __MINGW32__

// recent versions of MinGW warn: "Please include winsock2.h before windows.h",
// hence we must include winsock2.h before FL/Fl.H (A.S. Dec. 2010, IMM May
// 2011)
...
const char *fl_local_alt = "Alt";    ///< string pointer used in shortcuts, you
                                     ///< can change it to another language
const char *fl_local_ctrl = "Ctrl";  ///< string pointer used in shortcuts, you
                                     ///< can change it to another language
const char *fl_local_meta = "Meta";  ///< string pointer used in shortcuts, you
                                     ///< can change it to another language
const char *fl_local_shift =
    "Shift";  ///< string pointer used in shortcuts, you can change it to
              ///< another language
#else
...
const char *fl_local_meta =
    "\xe2\x8c\x98\\";  // U+2318 (place of interest sign)
...
#ifndef FL_DOXYGEN
Fl_Widget *Fl::belowmouse_, *Fl::pushed_, *Fl::focus_, *Fl::selection_owner_;
int Fl::damage_, Fl::e_number, Fl::e_x, Fl::e_y, Fl::e_x_root, Fl::e_y_root,
    Fl::e_dx, Fl::e_dy, Fl::e_state, Fl::e_clicks, Fl::e_is_click, Fl::e_keysym,
    Fl::e_original_keysym, Fl::scrollbar_size_ = 16;
...
/**
  ...
  is useful for checking the version of a shared library.
*/
int Fl::api_version() { return FL_API_VERSION; }

/**
  Returns the compiled-in value of the FL_ABI_VERSION constant. This
  is useful for checking the version of a shared library.
*/
int Fl::abi_version() { return FL_ABI_VERSION; }
...
*/
int Fl::scrollbar_size() { return scrollbar_size_; }
...
*/
void Fl::scrollbar_size(int W) { scrollbar_size_ = W; }
...
bool fl_clipboard_notify_empty(void) { return clip_notify_list == NULL; }
    ...
    for (Fl_X **pp = &Fl_X::first; (window = *pp); pp = &window->next)
        if (window->xid == xid) {
            if (window != Fl_X::first && !Fl::modal()) {
                ...
                // this is not done if modal is true to avoid messing up modal
                // stack
                *pp = window->next;
...
void Fl::remove_handler(Fl_Event_Handler ha) {
    ...
    for (l = handlers, p = 0; l && l->handle != ha;
         p = l, l = l->next) { /*empty*/
    }
...
int fl_send_system_handlers(void *e) {
    for (const system_handler_link *hl = sys_handlers; hl; hl = hl->next) {
        if (hl->handle(e, hl->data)) return 1;
    }
    return 0;
}
...
/**
    ...
    handle()).
*/
void Fl::pushed(Fl_Widget *o) { pushed_ = o; }
 ...
 */
void Fl::event_dispatch(Fl_Event_Dispatch d) { e_dispatch = d; }

/**
 \brief Return the current event dispatch function.
 */
Fl_Event_Dispatch Fl::event_dispatch() { return e_dispatch; }
...
int Fl::handle_(int e, Fl_Window *window) {
    ...
    switch (e) {
        ...
        case FL_DRAG:
            fl_xmousewin =
                window;  // this should already be set, but just in case.
            if (pushed()) {
                wi = pushed();
                if (grab()) wi = grab();
                e_number = e = FL_DRAG;
            ...
            if (grab()) wi = grab();
            {
                ...
                if (fl_mac_os_version < 100500) {
                    // before 10.5, mouse moved events aren't sent to borderless
                    // windows such as tooltips
                    Fl_Window *tooltip = Fl_Tooltip::current_window();
                    int inside = 0;
                    if (tooltip &&
                        tooltip->shown()) {  // check if a tooltip window is
                                             // currently opened
                        // check if mouse is inside the tooltip
                        inside =
                            (Fl::event_x_root() >= tooltip->x() &&
                             Fl::event_x_root() < tooltip->x() + tooltip->w() &&
                             Fl::event_y_root() >= tooltip->y() &&
                             Fl::event_y_root() < tooltip->y() + tooltip->h());
                    }
                    // if inside, send event to tooltip window instead of
                    // background window
                    if (inside)
                        ret = send_event(e, tooltip, window);
                    else
        ...
        case FL_RELEASE: {
            //    printf("FL_RELEASE: window=%p, pushed() = %p, grab() = %p,
            //    modal() = %p\n",
        ...
        case FL_SHORTCUT:
            ...
            if (!wi) {
                ...
                if (send_event(FL_SHORTCUT, first_window(), first_window()))
                    return 1;
            }
...
void Fl_Window::hide() {
#ifdef WIN32
    // STR#3079: if there remains a window and a non-modal window, and the
    // window is deleted, the app remains running without any apparent window.
    // Bug mechanism: hiding an owner window unmaps the owned (non-modal)
    // window(s) but does not delete it(them) in FLTK. Fix for it: when hiding a
    // window, build list of windows it owns, and do hide/show on them.
    int count = 0;
    ...
    if (count) {
        ...
        for (win = Fl::first_window(); win && i; win = Fl::next_window(win)) {
            if (win->non_modal() &&
                GetWindow(fl_xid(win), GW_OWNER) == i->xid) {
                doit[count++] = win;
            }
        }
    }
    ...
    *pp = ip->next;
    ...
    if (this ==
        Fl::modal_) {  // we are closing the modal window, find next one:
        Fl_Window *W;
    ...
    if (GetClipboardOwner() == ip->xid) fl_update_clipboard();
    ...
    if (ip->xid == fl_window && fl_gc) {
        ...
        if (Fl::cairo_autolink_context())
            Fl::cairo_make_current((Fl_Window *)0);
#endif
    }
#elif defined(__APPLE_QUARTZ__)
    Fl_X::q_release_context(ip);
    if (ip->xid == fl_window) fl_window = 0;
#endif
...
#error unsupported platform
            ...
            case FL_HIDE:
                if (shown()) {
                    // Find what really turned invisible, if it was a parent
                    // window we do nothing.  We need to avoid unnecessary unmap
                    // calls because they cause the display to blink when the
                    // parent is remapped.  However if this or any intermediate
                    // non-window widget has really had hide() called directly
                    // on it, we must unmap because when the parent window is
                    // remapped we don't want to reappear.
                    if (visible()) {
                        Fl_Widget *p = parent();
...
*/
void Fl::paste(Fl_Widget &receiver) { Fl::paste(receiver, 0); }
...
void Fl_Widget::redraw_label() {
    if (window()) {
        ...
        if (align() && !(align() & FL_ALIGN_INSIDE) && window()->shown()) {
            ...
            // FIXME:
            // This assumes that measure() returns the correct outline, which it
            // does not in all possible cases of alignment combinedwith image
            // and symbols.
            switch (align() & 0x0f) {
                case FL_ALIGN_TOP_LEFT:
                    ...
                    break;
                case FL_ALIGN_TOP:
                    window()->damage(FL_DAMAGE_EXPOSE, x() + (w() - W) / 2,
                                     y() - H, W, H);
                    break;
                case FL_ALIGN_TOP_RIGHT:
                    window()->damage(FL_DAMAGE_EXPOSE, x() + w() - W, y() - H,
                                     W, H);
                    break;
                ...
                case FL_ALIGN_RIGHT_TOP:
                    ...
                    break;
                case FL_ALIGN_LEFT:
                    window()->damage(FL_DAMAGE_EXPOSE, x() - W,
                                     y() + (h() - H) / 2, W, H);
                    break;
                case FL_ALIGN_RIGHT:
                    window()->damage(FL_DAMAGE_EXPOSE, x() + w(),
                ...
                case FL_ALIGN_LEFT_BOTTOM:
                    window()->damage(FL_DAMAGE_EXPOSE, x() - W, y() + h() - H,
                                     W, H);
                    break;
            ...
            // The label is inside the widget, so just redraw the widget
            // itself...
...
void Fl_Widget::damage(uchar fl, int X, int Y, int W, int H) {
    ...
    if (wi->damage()) {
        ...
        if (i->region) {
            ...
            if (j >= i->region->count) {
                i->region->rects = (CGRect *)realloc(
                    i->region->rects, (++(i->region->count)) * sizeof(CGRect));
...
void Fl_Window::flush() {
    if (!shown()) return;
    make_current();
    // if (damage() == FL_DAMAGE_EXPOSE && can_boxcheat(box())) fl_boxcheat =
    // this;
...
void Fl::do_widget_deletion() {
    ...
    for (int i = 0; i < num_dwidgets; i++) delete dwidgets[i];
...
/**
  ...
  pointer can be queried, if it is NULL. \e If it is NULL, then the widget has
  been deleted during the callback and must not be accessed anymore. If the
  widget pointer is \e not NULL, then the widget has not been deleted and can be
  accessed safely.
  ...
  \code
    ...
    Fl::release_widget_pointer(wp);	// remove the pointer from the watch
  list \endcode
...
void Fl::watch_widget_pointer(Fl_Widget *&w) {
    ...
    if (num_widget_watch == max_widget_watch) {
        max_widget_watch += 8;
        widget_watch = (Fl_Widget ***)realloc(
            widget_watch, sizeof(Fl_Widget **) * max_widget_watch);
    }
    widget_watch[num_widget_watch++] = wp;
#ifdef DEBUG_WATCH
    printf("\nwatch_widget_pointer:   (%d/%d) %8p => %8p\n", num_widget_watch,
           num_widget_watch, wp, *wp);
    fflush(stdout);
#endif  // DEBUG_WATCH
}
...
/**
  ...
  This is called when a widget is destroyed (by its destructor). You should
...
bool Fl::option(Fl_Option opt) {
    if (!options_read_) {
        ...
        {  // first, read the system wide preferences
            ...
            // default: on options_[OPTION_NATIVE_FILECHOOSER] = tmp;
            // opt_prefs.get("FilechooserPreview", tmp, 1);            //
            // default: on options_[OPTION_FILECHOOSER_PREVIEW] = tmp;
    ...
    if (opt < 0 || opt >= OPTION_LAST) return false;
...
void Fl::option(Fl_Option opt, bool val) {
    if (opt < 0 || opt >= OPTION_LAST) return;
    if (!options_read_) {
