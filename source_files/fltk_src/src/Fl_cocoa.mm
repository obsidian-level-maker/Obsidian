...
static unsigned make_current_counts =
    0;  // if > 0, then Fl_Window::make_current() can be called only once
static NSBitmapImageRep *rect_to_NSBitmapImageRep(Fl_Window *win, int x, int y, int w, int h);

int fl_mac_os_version = Fl_X::calc_mac_os_version();  // the version number of the running Mac OS X
                                                      // (e.g., 100604 for 10.6.4)
int fl_mac_quit_early =
    1;  // set it to 0 so cmd-Q does not terminate app but merely terminates the event loop

// public variables
CGContextRef fl_gc = 0;
void *fl_capture = 0;  // (NSWindow*) we need this to compensate for a missing(?) mouse capture
bool fl_show_iconic;   // true if called from iconize() - shows the next created window in collapsed
                       // state
...
void Fl::add_fd(int fd, void (*cb)(int, void *), void *v) { dataready.AddFD(fd, POLLIN, cb, v); }

void Fl::remove_fd(int n, int events) { dataready.RemoveFD(n, events); }

void Fl::remove_fd(int n) { dataready.RemoveFD(n, -1); }
...
static void delete_timer(MacTimeout &t) {
    if (t.timer) {
        CFRunLoopRemoveTimer(CFRunLoopGetCurrent(), t.timer, kCFRunLoopDefaultMode);
        CFRelease(t.timer);
...
void Fl::add_timeout(double time, Fl_Timeout_Handler cb, void *data) {
    ...
    CFRunLoopTimerRef timerRef = CFRunLoopTimerCreate(
        kCFAllocatorDefault, CFAbsoluteTimeGetCurrent() + time, 1E30, 0, 0, do_timer, &context);
    if (timerRef) {
...
- (FLWindow *)initWithFl_W:(Fl_Window *)flw
    ...
    if (self) {
        ...
        if (fl_mac_os_version >= 100700) {
            ...
            static setIMP addr =
                (setIMP)[NSWindow instanceMethodForSelector:@selector(setRestorable:)];
...
- (NSRect)constrainFrameRect:(NSRect)frameRect toScreen:(NSScreen *)screen {
    ...
    return
        [super constrainFrameRect:frameRect
...
double fl_mac_flush_and_wait(double time_to_wait) {
    if (dropped_files_list) {  // when the list of dropped files is not empty, open one and remove
                               // it from list
...
#if FLTK_ABI_VERSION >= 10304
    if (b)
        mapped_to_retina_ |= windowDidResize_mask;
    else
        mapped_to_retina_ &= ~windowDidResize_mask;
#else
    if (b)
        xidChildren = (Fl_X *)((unsigned long)xidChildren | windowDidResize_mask);
    else
        xidChildren = (Fl_X *)((unsigned long)xidChildren & ~windowDidResize_mask);
#endif
}
...
#if FLTK_ABI_VERSION >= 10304
    if (b)
        mapped_to_retina_ |= mapped_mask;
    else
        mapped_to_retina_ &= ~mapped_mask;
#else
    if (b)
        xidChildren = (Fl_X *)((unsigned long)xidChildren | mapped_mask);
    else
        xidChildren = (Fl_X *)((unsigned long)xidChildren & ~mapped_mask);
#endif
}
...
#if FLTK_ABI_VERSION >= 10304
    if (b)
        mapped_to_retina_ |= changed_mask;
    else
        mapped_to_retina_ &= ~changed_mask;
#else
    if (b)
        xidChildren = (Fl_X *)((unsigned long)xidChildren | changed_mask);
    else
        xidChildren = (Fl_X *)((unsigned long)xidChildren & ~changed_mask);
#endif
}
...
- (void)windowDidResize:(NSNotification *)notif {
    ...
    if (views_use_CA && !window->as_gl_window())
        [(FLViewLayer *)[nsw contentView] viewFrameDidChange];
#endif
    fl_unlock_function();
}
- (void)windowDidResignKey:(NSNotification *)notif {
    ...
    if (window->fullscreen_active() && [NSApp keyWindow] &&
        [[NSApp keyWindow] level] <= [nsw level]) {
...
- (void)fl_windowMiniaturize:
    (NSNotification *)notif {  // Same as windowWillMiniaturize before 10.5,
...
- (BOOL)windowShouldClose:(id)fl {
    fl_lock_function();
    Fl::handle(FL_CLOSE,
               [(FLWindow *)fl getFl_Window]);  // this might or might not close the window
    fl_unlock_function();
...
- (void)applicationDidChangeScreenParameters:
    ...
    for (int i = 0; i < count; i++) {
        ...
        if ([win isKindOfClass:[FLWindow class]] && ![win parentWindow] && [win isVisible]) {
            [[NSNotificationCenter defaultCenter] postNotificationName:NSWindowDidMoveNotification
                                                                object:win];
        }
    }
...
- (BOOL)application:(NSApplication *)theApplication openFile:(NSString *)filename {
    if (fl_mac_os_version < 101300) {
        // without the next two statements, the opening of the 1st window is delayed by several
        // seconds under 10.8 ≤ Mac OS < 10.13 when a file is dragged on the application icon
...
static open_cb_f_type get_open_cb() { return ((FLAppDelegate *)[NSApp delegate])->open_cb; }
...
static void foreground_and_activate() {
    if (!is_bundled()) {  // only transform the application type for unbundled apps
        ...
        TransformProcessType(&cur_psn,
                             kProcessTransformToForegroundApplication);  // needs Mac OS 10.3
...
void fl_open_display() {
    ...
    if (!beenHereDoneThat) {
        ...
        FLAppDelegate *delegate =
            (fl_mac_os_version < 100500 ? [FLAppDelegateBefore10_5 alloc] : [FLAppDelegate alloc]);
        ...
        if (![NSThread isMultiThreaded]) {
            // With old OS X versions, it is necessary to create one thread for secondary pthreads
            // to be allowed to use cocoa, especially to create an NSAutoreleasePool. We create a
            // thread that does nothing so it completes very fast:
    ...
    if (retval == -1) {
        ...
        if (fl_mac_os_version >= 100500) {
            TSMGetActiveDocument =
                (TSMGetActiveDocument_type)Fl_X::get_carbon_function("TSMGetActiveDocument");
            TSMSetDocumentProperty =
                (TSMSetDocumentProperty_type)Fl_X::get_carbon_function("TSMSetDocumentProperty");
            TSMRemoveDocumentProperty = (TSMRemoveDocumentProperty_type)Fl_X::get_carbon_function(
                "TSMRemoveDocumentProperty");
            TISCreateASCIICapableInputSourceList =
                (TISCreateASCIICapableInputSourceList_type)Fl_X::get_carbon_function(
                    "TISCreateASCIICapableInputSourceList");
            retval = (TSMGetActiveDocument && TSMSetDocumentProperty && TSMRemoveDocumentProperty &&
                              TISCreateASCIICapableInputSourceList
                          ? 1
                          : 0);
        } else {
 ...
 */
int Fl::x() { return int([[[NSScreen screens] objectAtIndex:0] visibleFrame].origin.x); }
 ...
 */
int Fl::w() { return int([[[NSScreen screens] objectAtIndex:0] visibleFrame].size.width); }

/*
 * height of work area of menubar-containing display
 */
int Fl::h() { return int([[[NSScreen screens] objectAtIndex:0] visibleFrame].size.height); }
...
int Fl_X::fake_X_wm(const Fl_Window *w, int &X, int &Y, int &bt, int &bx, int &by) {
    ...
    for (i = 0; i < count; i++) {
        ...
        r.origin.y = main_screen_height -
                     (r.origin.y + r.size.height);  // use FLTK's multiscreen coordinates
...
/**                 How FLTK handles Mac OS text input
 ...
 key is pressed, and also when entering complex scripts (e.g., Chinese). Fl_X::next_marked_length
 gives the byte length of marked text before the FL_KEYBOARD event is processed. Fl::compose_state
 gives this length after this processing. Message insertText: is sent to enter text in the focused
 widget. If there's marked text, Fl::compose_state is > 0, and this marked text gets replaced by the
 inserted text. If there's no marked text, the new text is inserted at the insertion point. When the
 character palette is used to enter text, the system sends an insertText: message to myview. The
 in_key_event field of the FLView class allows to differentiate keyboard from palette inputs.
...
/* Implementation note for the support of layer-backed views.
 ...
 Each layer-backed non-OpenGL window has a single FLViewLayer object which itself has an associated
 CALayer. FLViewLayer implements displayLayer:. Consequently, FLViewLayer objects are drawn by the
 displayLayer: method. An FLViewLayer manages also a member variable CGContextRef layer_data, a
 bitmap context the size of the view (double on Retina). All Quartz drawings go to this bitmap.
 updateLayer finishes by using an image copy of the bitmap as the layer's contents. That step fills
 the window. FLViewLayer implements viewFrameDidChange which deletes the bitmap and zeros
 layer_data. This ensures the bitmap is recreated when the window is resized. viewFrameDidChange is
 also called when the window flips between low/high resolution displays.
...
static CGContextRef prepare_bitmap_for_layer(int w, int h) {
    ...
    CGContextRef gc =
        CGBitmapContextCreate(NULL, w, h, 8, 4 * w, cspace, kCGImageAlphaPremultipliedFirst);
...
- (void)drawRect:(NSRect)rect {
    ...
    i->flush();
    window->clear_damage();
    if (window->parent() && fl_mac_os_version < 101401)
        window->redraw();  // useful during resize of GL subwindow
    fl_unlock_function();
}
- (BOOL)wantsLayer {
    return YES;
...
- (BOOL)performKeyEquivalent:(NSEvent *)theEvent {
    ...
    if ((mods & NSControlKeyMask) || (mods & NSCommandKeyMask)) {
        ...
        if ((mods & NSShiftKeyMask) && (mods & NSCommandKeyMask)) {
            s = [s uppercaseString];  // US keyboards return lowercase letter in s if cmd-shift-key
                                      // is hit
        }
...
- (void)setMarkedText:(id)aString
    ...
    if (in_key_event && Fl_X::next_marked_length && Fl::e_length) {
        // if setMarkedText + setMarkedText is sent during handleEvent, text cannot be concatenated
        // in single FL_KEYBOARD event
        Fl::handle(FL_KEYBOARD, target);
        Fl::e_length = 0;
    }
    if (in_key_event && Fl::e_length)
...
void Fl_X::GLcontext_update(NSOpenGLContext *ctxt) { [ctxt update]; }

void Fl_X::GLcontext_flushbuffer(NSOpenGLContext *ctxt) { [ctxt flushBuffer]; }

void Fl_X::GLcontext_release(NSOpenGLContext *ctxt) { [ctxt release]; }

void Fl_X::GL_cleardrawable(void) { [[NSOpenGLContext currentContext] clearDrawable]; }

void Fl_X::GLcontext_makecurrent(NSOpenGLContext *ctxt) { [ctxt makeCurrentContext]; }
...
void Fl_Window::fullscreen_off_x(int X, int Y, int W, int H) {
    ...
    if (fl_mac_os_version < 101000) {
        ...
        NSUInteger winstyle =
            (border() ? (NSTitledWindowMask | NSClosableWindowMask | NSResizableWindowMask)
...
void Fl_X::make(Fl_Window *w) {
    ...
    if (fl_show_iconic) {
        ...
        [cw recursivelySendToSubwindows:@selector
            (display)];  // draw the window and its subwindows before its icon is computed
        [cw miniaturize:nil];
...
void Fl_Window::resize(int X, int Y, int W, int H) {
    ...
    //  printf("Fl_Window::resize(X=%d, Y=%d, W=%d, H=%d), is_a_resize=%d, resize_from_system=%p,
    //  this=%p\n",
    ...
    if ((resize_from_system != this) && shown()) {
        if (is_a_resize) {
            ...
            NSRect r = NSMakeRect(bx, main_screen_height - (by + H), W,
                                  H + ((border() && !fullscreen_active()) ? bt : 0));
 ...
 * make all drawing go into this window (called by subclass flush() impl.)
 ...
 to YES and calls handleUpdateEvent() that calls Fl_X::flush(). Fl_X::flush() sets
 through_Fl_X_flush to YES and calls Fl_Window::flush() that calls Fl_Window::make_current() that
 ...
 through_Fl_X_flush is set to YES. Fl_X::flush() locks the focus to the view and calls
 Fl_Window::flush() that calls Fl_Window::make_current() which uses the window's graphics context.
...
void Fl_Window::make_current() {
...
#endif
            nsgc = through_Fl_X_flush ? [NSGraphicsContext currentContext]
    ...
    if (Fl::cairo_autolink_context())
        Fl::cairo_make_current(
            this);  // capture gc changes automatically to update the cairo context adequately
#endif
    fl_clip_region(0);
...
#if defined(FLTK_USE_CAIRO)
    if (Fl::cairo_autolink_context())
        Fl::cairo_make_current((Fl_Window *)0);  // capture gc changes automatically to update the
                                                 // cairo context adequately
#endif
}
...
void Fl_X::q_begin_image(CGRect &rect, int cx, int cy, int w, int h) {
    ...
    if (at.a == at.d && at.b == 0 && at.c == 0) {  // proportional scaling, no rotation
        ...
        if (at.a == 2) {  // make .tx and .ty have even values
            ...
            if (Fl_Display_Device::high_resolution()) {  // make .tx and .ty have int or half-int
                                                         // values
                deltax = -(at.tx * 2 - round(at.tx * 2));
                deltay = (at.ty * 2 - round(at.ty * 2));
...
void Fl_X::q_end_image() { CGContextRestoreGState(fl_gc); }

void Fl_X::set_high_resolution(bool new_val) { Fl_Display_Device::high_res_window_ = new_val; }
...
static void resize_selection_buffer(int len, int clipboard) {
    if (len <= fl_selection_buffer_length[clipboard]) return;
...
static int get_plain_text_from_clipboard(int clipboard) {
    ...
    if (found) {
        ...
        if (data) {
            ...
            length = convert_crlf(fl_selection_buffer[clipboard],
                                  len - 1);  // turn all \r characters into \n:
...
static Fl_Image *get_image_from_clipboard(Fl_Widget *receiver) {
    ...
    NSArray *possible = [NSArray
        arrayWithObjects:TIFF_pasteboard_type, PDF_pasteboard_type, PICT_pasteboard_type, nil];
    NSString *found = nil;
    NSUInteger rank;
    ...
    if ([found isEqualToString:TIFF_pasteboard_type]) {
        ...
        bitmap = [[NSBitmapImageRep alloc]
            initWithFocusedViewRect:NSMakeRect(0, 0, [nsimg size].width, [nsimg size].height)];
        [nsimg unlockFocus];
        [nsimg release];
    }
    ...
    if (strcmp(type, Fl::clipboard_plain_text) == 0) {
        found = [[NSPasteboard generalPasteboard]
            availableTypeFromArray:[NSArray
                                       arrayWithObjects:UTF8_pasteboard_type,
                                                        @"public.utf16-plain-text",
                                                        @"com.apple.traditional-mac-plain-text",
                                                        nil]];
    } else if (strcmp(type, Fl::clipboard_image) == 0) {
        found = [[NSPasteboard generalPasteboard]
            availableTypeFromArray:[NSArray arrayWithObjects:TIFF_pasteboard_type,
                                                             PDF_pasteboard_type,
...
void Fl_X::map() {
    if (w && xid && ![xid parentWindow]) {  // 10.2
        // after a subwindow has been unmapped, it has lost its parent window and its frame may be
        // wrong
...
void Fl_X::collapse() { [xid miniaturize:nil]; }
...
- (BOOL)validateMenuItem:
    (NSMenuItem *)item {  // invalidate the Quit item of the application menu when running modal
    ...
    options = [NSDictionary
        dictionaryWithObjectsAndKeys:
            [[[NSAttributedString alloc]
                initWithString:[NSString stringWithFormat:@" GUI with FLTK %d.%d", FL_MAJOR_VERSION,
                                                          FL_MINOR_VERSION]] autorelease],
            @"Credits", nil];
...
static void createAppleMenu(void) {
    ...
    title =
        [NSString stringWithFormat:NSLocalizedString(
                                       [NSString stringWithUTF8String:Fl_Mac_App_Menu::about], nil),
                                   nsappname];
    ...
    if (fl_mac_os_version >= 100400) {  // services+hide+quit already in menu in OS 10.3
        ...
        title = [NSString
            stringWithFormat:NSLocalizedString(
                                 [NSString stringWithUTF8String:Fl_Mac_App_Menu::hide], nil),
                             nsappname];
...
void Fl_X::set_key_window() { [xid makeKeyWindow]; }
...
static NSBitmapImageRep *rect_to_NSBitmapImageRep_layer(
    Fl_Window *win, int x, int y, int w,
    int h) {  // capture window data for layer-based views because initWithFocusedViewRect: does not
              // work for them
    NSBitmapImageRep *bitmap = nil;
#if MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_8
 ...
 */
{
    ...
    while ((child = [enumerator nextObject]) != nil) {
        ...
        NSBitmapImageRep *childbitmap = rect_to_NSBitmapImageRep(
        ...
        if (childbitmap) {
            ...
            if ([bitmap pixelsWide] > w && [childbitmap pixelsWide] == clip.size.width)
                childbitmap = scale_nsbitmapimagerep(childbitmap, 2);
            write_bitmap_inside(bitmap, w, childbitmap, clip.origin.x - x,
                                win->h() - clip.origin.y - clip.size.height - y);
        }
        [childbitmap release];
    }
    return bitmap;
...
unsigned char *Fl_X::bitmap_from_window_rect(Fl_Window *win, int x, int y, int w, int h,
                                             int *bytesPerPixel)
 ...
 */
{
    ...
    *bytesPerPixel = [bitmap bitsPerPixel] / 8;
    ...
    if (ww > w) {  // with a retina display, we have to scale the image by a factor of 2
        ...
        if (convert) {  // duplicate the NSBitmapImageRep data taking care not to access beyond its
                        // end
            tocopy = ww * hh * 4;
        ...
        if (w == ww) {  // the NSBitmapImageRep data can be copied in one step
            ...
            for (int i = 0; i < hh; i++) {
                if (i == hh - 1 && convert)
                    tocopy--;  // take care not to access beyond the image end
                memcpy(q, p, tocopy);
                p += bpr;
                q += w * (*bytesPerPixel);
            }
 ...
 */
{
    ...
    if (fl_mac_os_version >= 100500) {
        ...
        img = CGImageCreate(
            [bitmap pixelsWide], [bitmap pixelsHigh], 8, [bitmap bitsPerPixel],
...
#if MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_6
    return fl_mac_os_version >= 101000
               ? [[[fl_xid(win) standardWindowButton:NSWindowCloseButton] superview] layer]
               : nil;  // 10.5
#else
    return nil;
#endif
...
void Fl_Paged_Device::print_window(Fl_Window *win, int x_offset, int y_offset) {
    ...
    if (layer) {          // if title bar uses a layer
        if (to_quartz) {  // to Quartz printer
            ...
            CGColorSpaceRef cspace = CGColorSpaceCreateDeviceRGB();
            CGContextRef gc = CGBitmapContextCreate(NULL, win->w(), bt, 8, 0, cspace,
                                                    kCGImageAlphaPremultipliedLast);
            CGColorSpaceRelease(cspace);
            CGContextClearRect(gc, CGRectMake(0, 0, win->w(), bt));
            Fl_X::draw_layer_to_context(layer, gc, win->w(), bt);
            Fl_RGB_Image *image =
                new Fl_RGB_Image((const uchar *)CGBitmapContextGetData(gc), win->w(), bt, 4,
...
#if MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_4
        if (fl_mac_os_version >= 100400 &&
            to_quartz) {  // use Cocoa string drawing with exact title bar font
            // the exact font is LucidaGrande 13 pts (and HelveticaNeueDeskInterface-Regular
            // with 10.10)
            NSGraphicsContext *current = [NSGraphicsContext currentContext];
            [NSGraphicsContext
                setCurrentContext:[NSGraphicsContext graphicsContextWithGraphicsPort:fl_gc
                                                                             flipped:YES]];  // 10.4
