...
static void clear_xmap(Fl_XMap &xmap) {
    if (xmap.pen) {
        ...
        HGDIOBJ oldpen =
            SelectObject(fl_gc, tmppen);  // Push out the current pen of the gc
        if (oldpen != xmap.pen)
            SelectObject(fl_gc, oldpen);  // Put it back if it is not the one we
                                          // are about to delete
...
static void set_xmap(Fl_XMap &xmap, COLORREF c) {
    ...
    if (xmap.pen) {
        HGDIOBJ oldpen = SelectObject(
            fl_gc,
            GetStockObject(BLACK_PEN));  // replace current pen with safe one
        if (oldpen != xmap.pen)
            SelectObject(
                fl_gc, oldpen);  // if old one not xmap.pen, need to put it back
    ...
    if (i & 0xffffff00) {
        ...
        if (!xmap.pen) {
            ...
            if (fl_palette) {
                ...
                set_xmap(xmap,
                         RGB(uchar(c >> 24), uchar(c >> 16), uchar(c >> 8)));
#if USE_COLORMAP
            }
#endif
        }
...
HBRUSH fl_brush() { return fl_brush_action(0); }
...
#define FL_N_BRUSH 16
    ...
    if (i != -1) {        // if the brush was allready allocated
        ...
        HGDIOBJ oldbrush =
            SelectObject(fl_gc, tmpbrush);  // load in into current context
        if (oldbrush != brushes[i].brush)
            SelectObject(fl_gc, oldbrush);  // reload old one
...
fl_select_palette(void) {
    ...
    if (!beenhere) {
        ...
        ulong foo[(sizeof(LOGPALETTE) + 256 * sizeof(PALETTEENTRY)) /
                      sizeof(ulong) +
                  1];
