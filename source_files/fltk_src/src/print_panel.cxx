...
static void cb_print_choice(Fl_Choice *, void *) { print_update_status(); }
...
static void cb_Save(Fl_Return_Button *, void *) {
    ...
    snprintf(name, sizeof(name), "%s/page_size",
             printer == NULL ? "" : printer);
    print_prefs.set(name, print_page_size->value());

    snprintf(name, sizeof(name), "%s/output_mode",
             printer == NULL ? "" : printer);
    for (val = 0; val < 4; val++) {
...
static void cb_Use(Fl_Button *, void *) { print_properties_panel->hide(); }
...
Fl_Double_Window *make_print_panel() {
    {
        print_panel = new Fl_Double_Window(465, 235, Fl_Printer::dialog_title);
        {
            ...
            {
                print_choice =
                    new Fl_Choice(133, 10, 181, 25, Fl_Printer::dialog_printer);
            ...
            }  // Fl_Button* print_properties
            {
                print_status = new Fl_Box(0, 41, print_panel_controls->w(), 17,
                                          "printer/job status");
                print_status->align(
                    Fl_Align(FL_ALIGN_CLIP | FL_ALIGN_INSIDE | FL_ALIGN_LEFT));
            }  // Fl_Box* print_status
            {
                Fl_Group *o =
                    new Fl_Group(10, 86, 227, 105, Fl_Printer::dialog_range);
                ...
                }  // Fl_Round_Button* print_pages
                {
                    ...
                    print_selection->callback(
                        (Fl_Callback *)cb_print_selection);
                ...
                {
                    print_to = new Fl_Int_Input(199, 126, 28, 25,
                                                Fl_Printer::dialog_to);
                    print_to->type(2);
                    print_to->textfont(4);
                    print_to->deactivate();
            ...
            {
                Fl_Group *o =
                    new Fl_Group(247, 86, 210, 105, Fl_Printer::dialog_copies);
                o->box(FL_THIN_DOWN_BOX);
                o->labelfont(1);
                o->align(Fl_Align(FL_ALIGN_TOP_LEFT));
                {
                ...
                }  // Fl_Spinner* print_copies
                {
                    ...
                    print_collate_button->callback(
                        (Fl_Callback *)cb_print_collate_button);
                ...
                }  // Fl_Check_Button* print_collate_button
                {
                    ...
                    print_collate_group[0]->deactivate();
                    {
                        ...
                        o->labelsize(11);
                        o->align(
                            Fl_Align(FL_ALIGN_BOTTOM_RIGHT | FL_ALIGN_INSIDE));
                        o->deactivate();
                    }  // Fl_Box* o
                    {
            ...
            }  // Fl_Group* o
            {
                Fl_Return_Button *o = new Fl_Return_Button(
                    279, 201, 100, 25, Fl_Printer::dialog_print_button);
    ...
    }  // Fl_Double_Window* print_panel
    {
        print_properties_panel =
            new Fl_Double_Window(290, 130, Fl_Printer::property_title);
        print_properties_panel->callback(
            (Fl_Callback *)cb_print_properties_panel);
        {
            print_page_size =
        ...
        {
            Fl_Group *o =
                new Fl_Group(110, 45, 170, 40, Fl_Printer::property_mode);
        ...
        {
            Fl_Return_Button *o =
                new Fl_Return_Button(93, 95, 99, 25, Fl_Printer::property_save);
            o->callback((Fl_Callback *)cb_Save);
        }  // Fl_Return_Button* o
        {
            Fl_Button *o =
                new Fl_Button(202, 95, 78, 25, Fl_Printer::property_cancel);
            o->callback((Fl_Callback *)cb_Cancel1);
        }  // Fl_Button* o
        {
            Fl_Button *o =
                new Fl_Button(10, 95, 73, 25, Fl_Printer::property_use);
...
printing_style
print_load() {  // return whether SystemV or BSD printing style is used
    FILE *lpstat;
    ...
    if ((lpstat = popen(
             "LC_MESSAGES=C LANG=C /bin/sh -c '(lpstat -p -d ) 2>&-'", "r")) !=
        NULL) {  // try first with SystemV printing system
    ...
    if (print_choice->size() == 2 &&
        (lpstat = fopen("/etc/printcap",
                        "r"))) {  // try next with BSD printing system
        while (fgets(line, sizeof(line),
                     lpstat)) {  // get names of all known printers
...
void print_update_status() {
    ...
    if (print_choice->value()) {
        ...
        snprintf(command, sizeof(command),
                 "/bin/sh -c \"(lpstat -p '%s' ) 2>&-\" ",
                 printer);  // try first with SystemV printing system
        if ((lpstat = popen(command, "r")) != NULL) {
            if (fgets(status, sizeof(status), lpstat) == 0) {  // if no reply
                pclose(lpstat);
                snprintf(command, sizeof(command), "lpq -P%s 2>&-",
                         printer);  // try next with BSD printing system
                if ((lpstat = popen(command, "r")) != NULL) {
                    if (fgets(status, sizeof(status), lpstat) ==
                        0) { /* ignore */
                    }
                }
            }
