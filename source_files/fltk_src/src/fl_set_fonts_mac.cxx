...
const char *Fl::get_font_name(Fl_Font fnum, int *ap) {
    ...
    if (!f->fontname[0]) {
        ...
        if (fl_mac_os_version >= Fl_X::CoreText_threshold) {
            CFStringRef cfname =
                CFStringCreateWithCString(NULL, f->name, kCFStringEncodingUTF8);
            CTFontRef ctfont = CTFontCreateWithName(cfname, 0, NULL);
            CFRelease(cfname);
            cfname = CTFontCopyFullName(ctfont);
            CFRelease(ctfont);
            CFStringGetCString(cfname, f->fontname, ENDOFBUFFER,
...
static int name_compare(const void *a, const void *b) {
    /* Compare PostScript font names.
     First compare font family names ignoring bold, italic and oblique
     qualifiers. When families are identical, order them according to regular,
...
#pragma unused(xstarname)
    ...
    if (fl_mac_os_version >= Fl_X::CoreText_threshold) {
        ...
        CFDictionaryRef dict = CFDictionaryCreate(
            NULL, (const void **)kCTFontCollectionRemoveDuplicatesOption,
        ...
        for (i = 0; i < count; i++) {
            ...
            CFStringGetCString(cfname, fname, sizeof(fname),
                               kCFStringEncodingUTF8);
    ...
    MAC_OS_X_VERSION_MAX_ALLOWED < MAC_OS_X_VERSION_10_11
        ...
        for (ItemCount idx = 0; idx < oFontCount; idx++) {
            ...
            //	Ptr oName;
            // How to get the name - Apples docs say call this twice, once to
            // get the length, then again to get the actual name...
            //    ATSUFindFontName (oFontIDs[idx], kFontFullName,
            ...
            // But who's got time for that nonsense? Let's just hard code a
            // fixed buffer (urgh!)
            ByteCount actualLength = 511;
            char oName[512];
            ATSUFindFontName(oFontIDs[idx], kFontFullName,
                             kFontMacintoshPlatform, kFontRomanScript,
                             kFontEnglishLanguage, actualLength, oName,
                             &actualLength, &oCountAgain);
