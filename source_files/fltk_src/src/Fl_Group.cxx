...
/**
  ...
  children() if the widget is NULL or not found.
*/
...
static int send(Fl_Widget *o, int event) {
    ...
    switch (event) {
        ...
        case FL_DND_DRAG:
            ...
            event =
                (o->contains(Fl::belowmouse())) ? FL_DND_DRAG : FL_DND_ENTER;
    }
    int save_x = Fl::e_x;
    Fl::e_x -= o->x();
    int save_y = Fl::e_y;
    ...
    switch (event) {
        case FL_FOCUS:
            switch (navkey()) {
                ...
                case FL_Down:
                    for (i = children(); i--;)
                        if ((*a++)->take_focus()) return 1;
                    break;
                case FL_Left:
                case FL_Up:
                    for (i = children(); i--;)
                        if (a[i]->take_focus()) return 1;
                    break;
            }
            return 0;
        ...
        case FL_PUSH:
            for (i = children(); i--;) {
                ...
                if (o->takesevents() && Fl::event_inside(o)) {
                    ...
                    if (send(o, FL_PUSH)) {
                        if (Fl::pushed() && wp.exists() &&
                            !o->contains(Fl::pushed()))
                            Fl::pushed(o);
                        return 1;
                    }
                }
        ...
        default:
            // For all other events, try to give to each child, starting at
            // focus:
...
void Fl_Group::clear() {
    ...
    if (contains(pushed))
        pushed = this;  // set it to be the group, if it's a child
...
/**
  ...
  This is useful, if you derived a widget from Fl_Group and want to draw a
  special border or background. You can call draw_children() from the derived
  draw() method after drawing the box, border, or background.
*/
