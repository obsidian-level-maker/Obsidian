                   ...
                   int filetype)          // I - Enumerated file type
{
    ...
    if (filetype == ANY) {
        ...
        if (!fl_stat(filename, &fileinfo)) {
            if (S_ISDIR(fileinfo.st_mode)) filetype = DIRECTORY;
#ifdef S_IFIFO
    ...
    // is found...
    for (current = first_; current != (Fl_File_Icon *)0;
...
*/
void Fl_File_Icon::draw(int x,        // I - Upper-lefthand X
                        ...
                        int active)   // I - Active or inactive?
{
    ...
    while (d < dend) switch (*d) {
            case END:
                if (prim) switch (*prim) {
                        ...
                        case OUTLINEPOLYGON:
                            ...
                            oc =
                                (Fl_Color)((((unsigned short *)prim)[1] << 16) |
            ...
            case COLOR:
                ...
                if (c == FL_ICON_COLOR) c = ic;

                if (!active) c = fl_inactive(c);
            ...
            case VERTEX:
                if (prim) fl_vertex(d[1] * 0.0001, d[2] * 0.0001);
                d += 3;
                break;
