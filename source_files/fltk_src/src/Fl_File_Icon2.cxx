...
//   Fl_File_Icon::load_system_icons() - Load the standard system
//   icons/filetypes. load_kde_icons()                  - Load KDE icon files.
//   load_kde_mimelnk()                - Load a KDE "mimelnk" file.
//   kde_to_fltk_pattern()             - Convert a KDE pattern to a FLTK
//   pattern. get_kde_val()                     - Get a KDE value.
//
...
void Fl_File_Icon::load(const char *f)  // I - File to read from
{
    ...
    if (i) {
        Fl::warning("Fl_File_Icon::load(): Unable to load icon file \"%s\".",
                    f);
        return;
    }
}
...
Fl_File_Icon::load_fti(const char *fti)  // I - File to read from
{
    ...
    if ((fp = fl_fopen(fti, "rb")) == NULL) {
        Fl::error("Fl_File_Icon::load_fti(): Unable to open \"%s\" - %s", fti,
                  strerror(errno));
        return -1;
    }
    ...
    while ((ch = getc(fp)) != EOF) {
        ...
        if (!isalpha(ch)) {
            Fl::error(
                "Fl_File_Icon::load_fti(): Expected a letter at file position "
                "%ld (saw '%c')",
                ftell(fp) - 1, ch);
            break;
        }
        ...
        *ptr++ = '\0';
        ...
        if (ch != '(') {
            Fl::error(
                "Fl_File_Icon::load_fti(): Expected a ( at file position %ld "
                "(saw '%c')",
                ftell(fp) - 1, ch);
            break;
        }
        ...
        if (strcmp(command, "color") == 0) {
            ...
            if (strcmp(params, "iconcolor") == 0)
                ...
                if (c < 0) {
                    // Composite color; compute average...
                    c = -c;
                    cval = fl_color_average((Fl_Color)(c >> 4),
                                            (Fl_Color)(c & 15), 0.5f);
                } else
                    cval = c;
            }
            ...
            if (sscanf(params, "%f,%f", &x, &y) != 2) break;

            add_vertex((short)(int)rint(x * 100.0),
                       (short)(int)rint(y * 100.0));
        } else {
            Fl::error(
                "Fl_File_Icon::load_fti(): Unknown command \"%s\" at file "
                "position %ld.",
                command, ftell(fp) - 1);
            break;
        }
...
#ifdef DEBUG
    printf("Icon File \"%s\":\n", fti);
    for (int i = 0; i < num_data_; i++) printf("    %d,\n", data_[i]);
#endif /* DEBUG */
...
int Fl_File_Icon::load_image(const char *ifile)  // I - File to read from
{
    ...
    if (img->count() == 1) {
        ...
        const int extra_data =
            img->ld() ? (img->ld() - img->w() * img->d()) : 0;

        // Loop through grayscale or RGB image...
        for (y = 0, row = (const uchar *)(*(img->data())); y < img->h();
             y++, row += extra_data) {
            for (x = 0, startx = 0, c = (Fl_Color)-1; x < img->w();
                 x++, row += img->d()) {
                switch (img->d()) {
                    case 1:
                ...
                if (temp != c) {
                    if (x > startx && c != (Fl_Color)-1) {
                        add_color(c);
                        add(POLYGON);
                        add_vertex(startx * 9000 / img->w() + 1000,
                                   9500 - y * 9000 / img->h());
                        add_vertex(x * 9000 / img->w() + 1000,
                                   9500 - y * 9000 / img->h());
                        add_vertex(x * 9000 / img->w() + 1000,
                                   9500 - (y + 1) * 9000 / img->h());
                        add_vertex(startx * 9000 / img->w() + 1000,
        ...
        if (ncolors < 0) {
            ...
            for (i = 0, cmapptr = (const uchar *)*ptr; i < ncolors;
                 i++, cmapptr += 4)
                colors[cmapptr[0]] =
            ...
            for (i = 0; i < ncolors; i++, ptr++) {
                ...
                if ((lineptr = strstr(lineptr, "c ")) == NULL) {
                    ...
                    colors[ch] =
                        fl_rgb_color((uchar)red, (uchar)green, (uchar)blue);
                } else {
                    // Read a color name...
...
void Fl_File_Icon::load_system_icons(void) {
    ...
    static short dir[] = {
        // Directory icon
        COLOR,  -1,       -1,     POLYGON, VERTEX, 1000,    1000,   VERTEX,
        1000,   7500,     VERTEX, 9000,    7500,   VERTEX,  9000,   1000,
        END,    POLYGON,  VERTEX, 1000,    7500,   VERTEX,  2500,   9000,
        VERTEX, 5000,     9000,   VERTEX,  6500,   7500,    END,    COLOR,
        0,      FL_WHITE, LINE,   VERTEX,  1500,   1500,    VERTEX, 1500,
        7000,   VERTEX,   9000,   7000,    END,    COLOR,   0,      FL_BLACK,
        LINE,   VERTEX,   9000,   7500,    VERTEX, 9000,    1000,   VERTEX,
        1000,   1000,     END,    COLOR,   0,      FL_GRAY, LINE,   VERTEX,
        1000,   1000,     VERTEX, 1000,    7500,   VERTEX,  2500,   9000,
        VERTEX, 5000,     9000,   VERTEX,  6500,   7500,    VERTEX, 9000,
        7500,   END,      END};
        ...
        if (!access(filename, F_OK)) {
            ...
            if (icondirs[i]) {
                snprintf(filename, sizeof(filename),
                         "%s/16x16/mimetypes/unknown.png", icondir);
            } else {
                snprintf(filename, sizeof(filename),
                         "%s/share/icons/unknown.xpm", kdedir);
            }
            ...
            snprintf(filename, sizeof(filename),
                     "%s/16x16/filesystems/link.png", icondir);
            ...
            icon = new Fl_File_Icon(
                "*.{bmp|bw|gif|jpg|pbm|pcd|pgm|ppm|png|ras|rgb|tif|xbm|xpm}",
                Fl_File_Icon::PLAIN);
            ...
            if (!access("/usr/lib/filetype/install/iconlib/acroread.doc.fti",
                        F_OK)) {
                icon = new Fl_File_Icon("*.{eps|ps}", Fl_File_Icon::PLAIN);
                icon->load_fti(
                    "/usr/lib/filetype/system/iconlib/"
                    "PostScriptFile.closed.fti");
            ...
            new Fl_File_Icon("*", Fl_File_Icon::PLAIN,
                             sizeof(plain) / sizeof(plain[0]), plain);
            new Fl_File_Icon(
...
static void load_kde_icons(const char *directory,  // I - Directory to load
    ...
    for (i = 0; i < n; i++) {
        if (entries[i]->d_name[0] != '.') {
            snprintf(full, sizeof(full), "%s/%s", directory,
                     entries[i]->d_name);
...
static void load_kde_mimelnk(const char *filename,   // I - mimelnk filename
    ...
    if ((fp = fl_fopen(filename, "rb")) != NULL) {
        ...
        if (iconfilename[0]) {
            if (iconfilename[0] == '/') {
                strlcpy(full_iconfilename, iconfilename,
...
*/	};
                ...
                // KDE 1.x icons
                snprintf(full_iconfilename, sizeof(full_iconfilename), "%s/%s",
                         tmp, iconfilename);
