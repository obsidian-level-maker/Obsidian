...
static int XConvertEucToUtf8(const char *locale, char *buffer_return, int len,
                             int bytes_buffer) {
    /* if (!locale) { */
...
int XUtf8LookupString(XIC ic, XKeyPressedEvent *event, char *buffer_return,
    ...
    long ucs = -1;
    int len;
    len = XmbLookupString(ic, event, buffer_return, bytes_buffer / 5, keysym,
                          status_return);
    ...
    if (ucs > 0) {
        ...
        im = XIMOfIC(ic);
        if (!im) return 0;
        len = XConvertEucToUtf8(XLocaleOfIM(im), buffer_return, len,
                                bytes_buffer);
    }
    return len;
}
