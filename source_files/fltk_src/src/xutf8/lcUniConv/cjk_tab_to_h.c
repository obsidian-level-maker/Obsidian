/* $XFree86: xc/lib/X11/lcUniConv/cjk_tab_to_h.c,v 1.2 2000/12/04 18:49:31 dawes
 * Exp $ */
...
typedef struct {
    ...
    /* You'll understand the terms "row" and "col" when you buy Ken Lunde's
       book. Once a row is fixed, choosing a "col" is the same as choosing a
       "cell". */
...
static void find_charset2uni_pages(Encoding *enc) {
    ...
    for (row = 0; row <= enc->rows; row++) enc->charsetpage[row] = 0;
...
static void invert(Encoding *enc) {
    ...
    for (j = 0; j < 0x10000; j++) enc->uni2charset[j] = 0;
...
static void output_uni2charset_dense(const char *name, Encoding *enc) {
    ...
    for (p = 0; p < 0x100; p++) pages[p] = false;
    for (row = 0; row < enc->rows; row++)
        for (col = 0; col < enc->cols; col++) {
            j = enc->charset2uni[row][col];
    ...
    printf(
        "static int\n%s_wctomb (conv_t conv, unsigned char *r, ucs4_t wc, int "
        "n)\n",
        name);
    printf("{\n");
    printf("  if (n >= 2) {\n");
...
static int row_byte_gbk1(int row) { return 0x81 + row; }
static int col_byte_gbk1(int col) { return (col >= 0x3f ? 0x41 : 0x40) + col; }
...
static int row_byte_gbk2(int row) { return 0x81 + row; }
static int col_byte_gbk2(int col) { return (col >= 0x3f ? 0x41 : 0x40) + col; }
...
static void read_table_ksc5601(Encoding *enc) {
    ...
    if (c == '#') {
        ...
        for (;;) {
            ...
            i2 = j & 0xff;
            if (scanf(" 0x%x", &j) != 1) exit(1);
            /* Take only the range covered by KS C 5601.1987-0 = KS C
               5601.1989-0 = KS X 1001.1992, ignore the rest. */
...
static int row_byte_big5(int row) { return 0xa1 + row; }
static int col_byte_big5(int col) { return (col >= 0x3f ? 0x62 : 0x40) + col; }
...
static int row_byte_johab_hangul(int row) { return 0x84 + row; }
...
static int row_byte_sjis(int row) { return (row >= 0x1f ? 0xc1 : 0x81) + row; }
static int col_byte_sjis(int col) { return (col >= 0x3f ? 0x41 : 0x40) + col; }
