/* $XFree86: xc/lib/X11/lcUniConv/8bit_tab_to_h.c,v 1.3 2001/02/09 00:02:54
 * dawes Exp $ */
...
int main(int argc, char *argv[]) {
...

    {
        ...
        if (c == '#') {
            ...
            for (i = 0; i < 0x100; i++) charset2uni[i] = 0xfffd;
            for (;;) {
                c = getc(stdin);
                if (c == EOF) break;
                if (c == '\n' || c == ' ' || c == '\t') continue;
    ...
    /* Write the output file. */
    {
...

        {
            ...
            if (tableno > 0) {
                int t;
                for (t = 0; t < tableno; t++) {
                    fprintf(f, "static const unsigned short %s_2uni",
                            c_charsetname);
                    if (tableno > 1) fprintf(f, "_%d", t + 1);
                    fprintf(f, "[%d] = {\n",
                            16 * (tables[t].maxline - tables[t].minline + 1));
                    for (i1 = tables[t].minline; i1 <= tables[t].maxline;
                         i1++) {
                        fprintf(f, "  /* 0x%02x */\n", 16 * i1);
                        for (i2 = 0; i2 < 2; i2++) {
                            fprintf(f, " ");
...

        {
            ...
            for (j = 0; j < 0x10000; j++) uni2charset[j] = 0;
            ...
            }
            {
                ...
                for (t = 0; t < tableno; t++)
                    if (tables[t].usecount > 1) {
                        p = 0;
                        fprintf(
                            f, "static const unsigned char %s_page%s[%d] = {\n",
                            c_charsetname, tables[t].suffix,
                ...
                if (t == -2) {
                    ...
                    if (t >= 0 && tables[t].usecount == 1) {
                        ...
                        if (j1 == 0) {
                            ...
                            fprintf(f, "if (wc >= 0x%04x && wc < 0x%04x)",
                                    8 * j1, 8 * j2);
                        }
                        if (t == -1) {
                            if (j1 == 0)
                                /* If wc == 0, the function must return 1, not
                                 * -1. */
                                fprintf(
                                    f,
                                    " {\n    *r = wc;\n    return 1;\n  }\n");
