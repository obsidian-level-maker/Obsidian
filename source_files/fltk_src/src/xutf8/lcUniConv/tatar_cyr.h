/* $XFree86: xc/lib/X11/lcUniConv/tatar_cyr.h,v 1.3 2000/12/04 18:49:42 dawes
 * Exp $ */
...
static int tatar_cyr_mbtowc(conv_t conv, ucs4_t *pwc, const unsigned char *s,
                            int n) {
    unsigned char c = *s;
    if (c < 0x80)
        *pwc = (ucs4_t)c;
...
static int tatar_cyr_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, int n) {
    unsigned char c = 0;
    if (wc < 0x0080) {
        *r = wc;
        return 1;
