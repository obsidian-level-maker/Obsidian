...
int main(int argc, char **argv) {
    ...
    XmbSetWMProperties(dpy, w, "test", "test", NULL, 0, NULL, &wm_hints,
                       &class_hints);
    ...
    xim_ic =
        XCreateIC(xim_im, XNInputStyle, (XIMPreeditNothing | XIMStatusNothing),
    ...
    while (1) {
        ...
        if (xevent.type == DestroyNotify) {
            ...
            xim_im = XOpenIM(dpy, NULL, "test", "Test");
            if (xim_im) {
                xim_ic = XCreateIC(xim_im, XNInputStyle,
        ...
        if (xevent.type == KeyPress && !filtered) {
            len = XUtf8LookupString(xim_ic, &xevent.xkey, buf, 127, &keysym,
                                    &status);
            ...
            XUtf8DrawString(dpy, w, fontset, gc, 50, 90, rtl_txt,
                            strlen(rtl_txt));
            XUtf8DrawRtlString(dpy, w, fontset, gc, 50, 90, rtl_txt,
                               strlen(rtl_txt));
            buf[len] = 0;
            printf("'%s' %d %x\n", buf, keysym, keysym);
            buf[0] = 0;
