...
/**
  ...
  one. \see item_first(), item_last(), item_next(), item_prev()
*/
...
/**
  ...
  \param[in] item The 'current' item
  \returns The previous item before \p item, or NULL if there are none before
  this one. \see item_first(), item_last(), item_next(), item_prev()
*/
...
FL_BLINE *Fl_Browser::find_line(int line) const {
    ...
    if (cacheline && line > (cacheline / 2) &&
        line < ((cacheline + lines) / 2)) {
        n = cacheline;
        l = cache;
...
/**
  Insert a new entry whose label is \p newtext \e above given \p line, optional
  data \p d.
...
/**
  ...
  removed) \param[in] from Line number of item to be moved
*/
...
/**
  ...
  \see item_height(), item_width(),\n
       incr_height(), full_height()
*/
int Fl_Browser::full_height() const { return full_height_; }
...
/**
  ...
  \see item_height(), item_width(),\n
       incr_height(), full_height()
*/
int Fl_Browser::incr_height() const { return textsize() + 2; }
...
void Fl_Browser::item_draw(void *item, int X, int Y, int W, int H) const {
            ...
            switch (*str++) {
                ...
                case 'B':
                    if (!(l->flags & SELECTED)) {
                        ...
                        while (isdigit(*str & 255)) str++;  // skip digits
                    break;
                case 'C':
    ...
    BREAK:
        fl_font(font, tsize);
        if (l->flags & SELECTED) lcol = fl_contrast(lcol, selection_color());
        if (!active_r()) lcol = fl_inactive(lcol);
        fl_color(lcol);
        fl_draw(str, X + 3, Y, w1 - 6, H,
                e ? Fl_Align(talign | FL_ALIGN_CLIP) : talign, 0, 0);
...
*/
int Fl_Browser::topline() const { return lineno(top()); }
...
void Fl_Browser::textsize(Fl_Fontsize newSize) {
    if (newSize == textsize()) return;  // avoid recalculation
    Fl_Browser_::textsize(newSize);
    new_list();
    full_height_ = 0;
    if (lines == 0) return;
    for (FL_BLINE *itm = (FL_BLINE *)item_first(); itm;
         itm = (FL_BLINE *)item_next(itm)) {
...
/**
  ...
  The text string \p newtext may contain format characters; see format_char()
  for details. \p newtext is copied using the strdup() function, and can be NULL
...
/**
  ...
  defined. The parameter \p line is 1 based (1 will be the first item in the
  list). \param[in] line The line number of the item whose data() is returned.
  (1 based) \returns The user data pointer (can be NULL)
...
/**
  ...
  state. \param[in] line The line in the browser to be tested. (1 based) \see
...
/**
  ...
  \see select(), selected(), value(), item_select(), item_selected()
*/
int Fl_Browser::value() const { return lineno(selection()); }
...
void Fl_Browser::icon(int line, Fl_Image *icon) {
    ...
    int old_h =
        bl->icon ? bl->icon->h() + 2 : 0;  // init with *old* icon height
...
*/
void Fl_Browser::remove_icon(int line) { icon(line, 0); }
