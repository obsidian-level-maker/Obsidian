...
int Fl_Menu_Item::measure(int *hp, const Fl_Menu_ *m) const {
    ...
    l.deimage = 0;
    l.type = labeltype_;
    l.font = labelsize_ || labelfont_ ? labelfont_
                                      : (m ? m->textfont() : FL_HELVETICA);
...
void Fl_Menu_Item::draw(int x, int y, int w, int h, const Fl_Menu_ *m,
    ...
    l.font = labelsize_ || labelfont_ ? labelfont_
                                      : (m ? m->textfont() : FL_HELVETICA);
    l.size = labelsize_ ? labelsize_ : m ? m->textsize() : FL_NORMAL_SIZE;
    l.color = labelcolor_ ? labelcolor_
              : m         ? m->textcolor()
                          : int(FL_FOREGROUND_COLOR);
    ...
    if (selected) {
        ...
        if (selected == 2) {  // menu title
            ...
            fl_draw_box(b, x + 1, y - (LEADING - 2) / 2, w - 2,
                        h + (LEADING - 2), r);
        }
    }
    ...
    if (flags & (FL_MENU_TOGGLE | FL_MENU_RADIO)) {
        ...
        if (flags & FL_MENU_RADIO) {
            fl_draw_box(FL_ROUND_DOWN_BOX, x + 2, y + d, W, W,
            ...
            if (value()) {
                ...
                if (Fl::is_scheme("gtk+")) {
                    ...
                    tW--;
                    fl_pie(x + td + 1, y + d + td - 1, tW + 3, tW + 3, 0.0,
                           360.0);
                    fl_color(
                        fl_color_average(FL_WHITE, FL_SELECTION_COLOR, 0.2f));
                } else
                ...
                if (Fl::is_scheme("gtk+")) {
                    fl_color(
                        fl_color_average(FL_WHITE, FL_SELECTION_COLOR, 0.5));
...
menutitle::menutitle(int X, int Y, int W, int H, const Fl_Menu_Item *L)
    ...
    set_menu_window();
    menu = L;
    if (L->labelcolor_ || Fl::scheme() || L->labeltype_ > FL_NO_LABEL)
        clear_overlay();
}
    ...
    if (m)
        for (; m->text; m = m->next()) {
            ...
            if (m->shortcut_) {
                ...
                if (fl_utf_nb_char((const unsigned char *)k, (int)strlen(k)) <=
                    4) {
                    // a regular shortcut has a right-justified modifier
                    // followed by a left-justified key
                    w1 = int(fl_width(s, (int)(k - s)));
                    if (w1 > hotModsw) hotModsw = w1;
                    w1 = int(fl_width(k)) + 4;
                    if (w1 > hotKeysw) hotKeysw = w1;
                } else {
                    // a shortcut with a long modifier is right-justified to the
                    // menu
                    w1 = int(fl_width(s)) + 4;
...
void menuwindow::drawentry(const Fl_Menu_Item *m, int n, int eraseit) {
    ...
    if (eraseit && n != selected) {
        fl_push_clip(xx + 1, yy - (LEADING - 2) / 2, ww - 2,
                     hh + (LEADING - 2));
    ...
    if (m->submenu()) {
        ...
        Fl_Font f = m->labelsize_ || m->labelfont_ ? (Fl_Font)m->labelfont_
                    : button                       ? button->textfont()
                                                   : FL_HELVETICA;
        fl_font(f, m->labelsize_ ? m->labelsize_
                   : button      ? button->textsize()
                                 : FL_NORMAL_SIZE);
        const char *k, *s = fl_shortcut_label(m->shortcut_, &k);
        if (fl_utf_nb_char((const unsigned char *)k, (int)strlen(k)) <= 4) {
            ...
            fl_draw(k, xx + ww - shortcutWidth, yy, shortcutWidth, hh,
                    FL_ALIGN_LEFT);
            free(buf);
        } else {
...
void menutitle::draw() { menu->draw(0, 0, w(), h(), button, 2); }
...
void menuwindow::draw() {
    if (damage() != FL_DAMAGE_CHILD) {  // complete redraw
        ...
        if (damage() & FL_DAMAGE_CHILD &&
            selected != drawn_selected) {  // change selection
...
int menuwindow::is_inside(int mx, int my) {
    if (mx < x_root() || mx >= x_root() + w() || my < y_root() ||
        my >= y_root() + h()) {
        return 0;
    }
...
int menustate::is_inside(int mx, int my) {
    ...
    for (i = nummenus - 1; i >= 0; i--) {
        if (p[i]->is_inside(mx, my)) return 1;
    }
    return 0;
}
...
int menuwindow::handle(int e) {
    ...
    if (pp.state == DONE_STATE) {
        ...
        if (pp.fakemenu) {
            pp.fakemenu->hide();
            if (pp.fakemenu->title) pp.fakemenu->title->hide();
        }
        int i = pp.nummenus;
        while (i > 0) {
    ...
    switch (e) {
        case FL_KEYBOARD:
            switch (Fl::event_key()) {
                ...
                case FL_Right:
                    if (pp.menubar &&
                        (pp.menu_number <= 0 ||
                         (pp.menu_number == 1 && pp.nummenus == 2)))
                        forward(0);
                    else if (pp.menu_number < pp.nummenus - 1)
                        forward(pp.menu_number + 1);
                    return 1;
                case FL_Left:
                    if (pp.menubar && pp.menu_number <= 1)
                        ...
                        setitem(pp.menu_number - 1,
                                pp.p[pp.menu_number - 1]->selected);
                    return 1;
                case FL_Enter:
                case FL_KP_Enter:
                case ' ':
        ...
        case FL_DRAG: {
            ...
            if ((!pp.menubar || mymenu) && !pp.is_inside(mx, my)) {
                setitem(0, -1, 0);
                if (e == FL_PUSH) pp.state = DONE_STATE;
                return 1;
            }
            for (mymenu = pp.nummenus - 1;; mymenu--) {
                ...
                if (item >= 0) break;
                if (mymenu <= 0) {
                    // buttons in menubars must be deselected if we move outside
                    // of them!
                    if (pp.menu_number == -1 && e == FL_PUSH) {
                        pp.state = DONE_STATE;
                        return 1;
                    }
                    if (pp.current_item && pp.menu_number == 0 &&
            ...
            if (e == FL_PUSH) {
                if (pp.current_item &&
                    pp.current_item->submenu()  // this is a menu title
                    &&
                    item != pp.p[mymenu]->selected  // and it is not already on
                    && !pp.current_item
                            ->callback_)  // and it does not have a callback
...
const Fl_Menu_Item *Fl_Menu_Item::pulldown(int X, int Y, int W, int H,
    ...
    for (;;) {
        ...
        // get events:
        {
            ...
            Fl::wait();
            if (pbutton &&
                wp.deleted())  // menu widget has been deleted (STR #3503)
                break;
        ...
        if (m->submenu()) {
            ...
            if (initial_item) {  // bring up submenu containing initial item:
                menuwindow *n = new menuwindow(
                    menutable, X, Y, W, H, initial_item, title, 0, 0, cw.x());
                pp.p[pp.nummenus++] = n;
                ...
                while (pp.nummenus > pp.menu_number + 1)
                    delete pp.p[--pp.nummenus];
                pp.p[pp.nummenus++] =
                    new menuwindow(menutable, nX, nY, title ? 1 : 0, 0, 0,
                                   title, 0, menubar, (title ? 0 : cw.x()));
            }
        } else {  // !m->submenu():
            ...
            if (!pp.menu_number && pp.menubar) {
                ...
                pp.fakemenu =
                    new menuwindow(0, cw.x() + cw.titlex(pp.item_number),
...
*/
const Fl_Menu_Item *Fl_Menu_Item::popup(int X, int Y, const char *title,
    ...
    if (m)
        for (; m->text; m = next_visible_or_not(m)) {
            if (m->active()) {
                ...
                // only return matches from lower menu if nothing found in top
                // menu:
                if (!ret && m->submenu()) {
                    const Fl_Menu_Item *s =
                        (m->flags & FL_SUBMENU)
                            ? m + 1
                            : (const Fl_Menu_Item *)m->user_data_;
                    ret = s->test_shortcut();
                }
            }
        }
