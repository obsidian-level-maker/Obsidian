...
int fl_scandir(const char *dirname, struct dirent ***namelist,
    ...
    if (!is_dir) { /* this file may still be a directory that we need to list */
        ...
        if (attr & FILE_ATTRIBUTE_DIRECTORY) strcpy(d, "\\*");
    }
    { /* Create a block to limit the scope while we find the initial "wide"
        ...
        unsigned wlen = fl_utf8toUtf16(findIn, (unsigned)strlen(findIn), NULL,
        ...
        h = FindFirstFileW(
            wbuf,
            &findw); /* get a handle to the first filename in the search */
        free(wbuf);  /* release the "wide" buffer before the pointer goes out of
                        scope */
    }
        ...
        if (!select || (*select)(selectDir)) {
            if (nDir == NDir) {
                struct dirent **tempDir = (struct dirent **)calloc(
                    sizeof(struct dirent *), (size_t)(NDir + 33));
    ...
    if (compar)
        qsort(dir, (size_t)nDir, sizeof(*dir),
