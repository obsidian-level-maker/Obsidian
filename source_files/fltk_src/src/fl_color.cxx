...
static void figure_out_visual() {
    ...
    for (i = 0, m = 1; m; i++, m <<= 1)
        if (fl_visual->green_mask & m) break;
    for (j = i; m; j++, m <<= 1)
        if (!(fl_visual->green_mask & m)) break;
    fl_greenshift = j - 8;
    ...
    for (i = 0, m = 1; m; i++, m <<= 1)
        if (fl_visual->blue_mask & m) break;
    for (j = i; m; j++, m <<= 1)
        if (!(fl_visual->blue_mask & m)) break;
    fl_blueshift = j - 8;
...
void Fl_Xlib_Graphics_Driver::color(Fl_Color i) {
    if (i & 0xffffff00) {
        ...
        if (!fl_gc)
            return;  // don't get a default gc if current window is not yet
                     // created/valid
...
#endif
    return (((r & fl_redmask) << fl_redshift) +
...
ulong fl_xpixel(Fl_Color i) {
    ...
    if (fl_overlay)
        ...
        return xmap.pixel = (((r & fl_redmask) << fl_redshift) +
                             ((g & fl_greenmask) << fl_greenshift) +
                             ((b & fl_bluemask) << fl_blueshift)) >>
                            fl_extrashift;
#if USE_COLORMAP
...
void Fl::set_color(Fl_Color i, uchar red, uchar green, uchar blue) {
    Fl::set_color((Fl_Color)(i & 255), ((unsigned)red << 24) +
                                           ((unsigned)green << 16) +
                                           ((unsigned)blue << 8));
}
/**
...
*/
Fl_Color fl_inactive(Fl_Color c) { return fl_color_average(c, FL_GRAY, .33f); }
...
Fl_Color fl_contrast(Fl_Color fg, Fl_Color bg) {
    ...
    l1 = ((c1 >> 24) * 30 + ((c1 >> 16) & 255) * 59 + ((c1 >> 8) & 255) * 11) /
         100;
    l2 = ((c2 >> 24) * 30 + ((c2 >> 16) & 255) * 59 + ((c2 >> 8) & 255) * 11) /
         100;
