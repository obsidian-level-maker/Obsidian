...
void Fl_Image::draw(int XP, int YP, int, int, int, int) { draw_empty(XP, YP); }
...
/**
  ...
  with it.
*/
Fl_Image *Fl_Image::copy(int W, int H) { return new Fl_Image(W, H, d()); }
...
/**
 ...
 \code
    ...
    switch ( jpg.fail() ) {
        ...
        case Fl_Image::ERR_FILE_ACCESS:
            fl_alert("/tmp/foo.jpg: %s", strerror(errno));    // shows actual os
 error to user exit(1); case Fl_Image::ERR_FORMAT: fl_alert("/tmp/foo.jpg:
 couldn't decode image"); exit(1);
    }
    box.image(jpg);
    [..]
 \endcode
 ...
 \return ERR_FILE_ACCESS if there was a file access related error (errno should
 be set) \return ERR_FORMAT if image decoding failed.
 */
int Fl_Image::fail() {
...
*/
void Fl_Image::RGB_scaling(Fl_RGB_Scaling method) { RGB_scaling_ = method; }

/** Returns the currently used RGB image scaling method. */
Fl_RGB_Scaling Fl_Image::RGB_scaling() { return RGB_scaling_; }
...
/**
  ...
  \see Fl_Image::data(), Fl_Image::w(), Fl_Image::h(), Fl_Image::d(),
  Fl_Image::ld(int)
*/
...
Fl_Image *Fl_RGB_Image::copy(int W, int H) {
    ...
    if (Fl_Image::RGB_scaling() == FL_RGB_SCALING_NEAREST) {
        ...
        for (dy = H, sy = 0, yerr = H, new_ptr = new_array; dy > 0; dy--) {
            for (dx = W, xerr = W, old_ptr = array + sy * line_d; dx > 0;
                 dx--) {
        ...
        for (dy = 0; dy < H; dy++) {
            ...
            for (dx = 0; dx < W; dx++) {
                ...
                memcpy(downright, array + drighty * line_d + drightx * d(),
                       d());
                ...
                if (d() == 4) {
                    for (i = 0; i < 3; i++) {
                        ...
                        downleft[i] =
                            (uchar)(downleft[i] * downleft[3] / 255.0f);
                        downright[i] =
                            (uchar)(downright[i] * downright[3] / 255.0f);
                    }
                }
                ...
                for (i = 0; i < d(); i++) {
                    new_ptr[i] =
                        (uchar)((left[i] * leftf + right[i] * rightf) * upf +
                                (downleft[i] * leftf + downright[i] * rightf) *
                                    downf);
                }
                ...
                if (d() == 4 && new_ptr[3]) {
                    for (i = 0; i < 3; i++) {
                        new_ptr[i] =
                            (uchar)(new_ptr[i] / (new_ptr[3] / 255.0f));
                    }
                }
            }
        }
...
void Fl_RGB_Image::color_average(Fl_Color c, float i) {
    ...
    const uchar *old_ptr;
    int x, y;
    int line_i = ld() ? ld() - (w() * d())
                      : 0;  // increment from line end to beginning of next line

    if (d() < 3) {
        ig = (r * 31 + g * 61 + b * 8) / 100 * (256 - ia);

        for (new_ptr = new_array, old_ptr = array, y = 0; y < h();
             y++, old_ptr += line_i)
            for (x = 0; x < w(); x++) {
...
void Fl_RGB_Image::desaturate() {
    ...
    for (new_ptr = new_array, old_ptr = array, y = 0; y < h();
        ...
        for (x = 0; x < w(); x++, old_ptr += d()) {
            *new_ptr++ =
                (uchar)((31 * old_ptr[0] + 61 * old_ptr[1] + 8 * old_ptr[2]) /
                        100);
...
static int start(Fl_RGB_Image *img, int XP, int YP, int WP, int HP, int w,
                 int h, int &cx, int &cy, int &X, int &Y, int &W, int &H) {
...
/** Draws an Fl_Image scaled to width \p W & height \p H with top-left corner at
 \em X,Y \return zero when the graphics driver doesn't implement scaled drawing,
...
void Fl_Quartz_Graphics_Driver::draw(Fl_RGB_Image *img, int XP, int YP, int WP,
    ...
    if (!img->id_) {
        CGColorSpaceRef lut = img->d() <= 2 ? CGColorSpaceCreateDeviceGray()
                                            : CGColorSpaceCreateDeviceRGB();
        int ld = img->ld();
        if (!ld) ld = img->w() * img->d();
        // If img->alloc_array == 0, the CGImage data provider must not release
        // the image data. If img->alloc_array != 0, the CGImage data provider
        // will take responsibilty of deleting RGB image data after use: when
        // the CGImage is deallocated, the release callback of its data provider
        // (imgProviderReleaseData) is called and can delete the RGB image data.
        // If the CGImage is printed, it is not deallocated until after the end
        // of the page, therefore, with img->alloc_array != 0, the RGB image can
        // be safely deleted any time after return from this function. The
        // previously unused mask_ member allows to make sure the RGB image data
        // is not deleted by Fl_RGB_Image::uncache().
        if (img->alloc_array) img->mask_ = new bool(true);
        CGDataProviderRef src = CGDataProviderCreateWithData(
            img->mask_, img->array, ld * img->h(),
            img->alloc_array ? imgProviderReleaseData : NULL);
        img->id_ = CGImageCreate(
            img->w(), img->h(), 8, img->d() * 8, ld, lut,
            (img->d() & 1) ? kCGImageAlphaNone : kCGImageAlphaLast, src, 0L,
    ...
    if (img->id_ && fl_gc) {
        if (!img->alloc_array &&
            Fl_Surface_Device::surface()->class_name() ==
                Fl_Printer::class_id &&
            !CGImageGetShouldInterpolate((CGImageRef)img->id_)) {
            // When printing, the image data is used when the page is completed,
            // that is, after return from this function. If the image has
            // alloc_array = 0, we must protect against image data being freed
            // before it is used: we duplicate the image data and have it
            // deleted after use by the release-callback of the CGImage data
            // provider
            ...
            CGColorSpaceRef lut = img->d() <= 2 ? CGColorSpaceCreateDeviceGray()
                                                : CGColorSpaceCreateDeviceRGB();
            CGDataProviderRef src = CGDataProviderCreateWithData(
                NULL, img_bytes, ld * img->h(), imgProviderReleaseData);
            img->id_ = CGImageCreate(
                img->w(), img->h(), 8, img->d() * 8, ld, lut,
                (img->d() & 1) ? kCGImageAlphaNone : kCGImageAlphaLast, src, 0L,
...
int Fl_Quartz_Graphics_Driver::draw_scaled(Fl_Image *img, int XP, int YP,
                                           int WP, int HP) {
    int X, Y, W, H;
    fl_clip_box(XP, YP, WP, HP, X, Y, W,
                H);  // X,Y,W,H will give the unclipped area of XP,YP,WP,HP
    if (W == 0 || H == 0) return 1;
    fl_push_no_clip();  // remove the FLTK clip that can't be rescaled
    CGContextSaveGState(fl_gc);
    CGContextClipToRect(
        fl_gc, CGRectMake(X, Y, W,
                          H));  // this clip path will be rescaled & translated
...
static Fl_Offscreen build_id(Fl_RGB_Image *img, void **pmask) {
    ...
    if ((img->d() == 2 || img->d() == 4) && fl_can_do_alpha_blending()) {
        ...
        fl_begin_offscreen(offs);
        fl_draw_image(img->array, 0, 0, img->w(), img->h(), img->d(),
                      img->ld());
        fl_end_offscreen();
        if (img->d() == 2 || img->d() == 4) {
            *pmask = fl_create_alphamask(img->w(), img->h(), img->d(),
                                         img->ld(), img->array);
        }
    }
    return offs;
...
int Fl_GDI_Printer_Graphics_Driver::draw_scaled(Fl_Image *img, int XP, int YP,
                                                int WP, int HP) {
    XFORM old_tr, tr;
...
void Fl_RGB_Image::label(Fl_Widget *widget) { widget->image(this); }
