...
Fl_GIF_Image::Fl_GIF_Image(const char *infname) : Fl_Pixmap((char *const *)0) {
    ...
    for (;;) {
        ...
        if (i == 0x21) {  // a "gif extension"
            ...
            ch = NEXTBYTE;
            ch = NEXTBYTE;  // GETSHORT(x_position);
            ch = NEXTBYTE;
            ch = NEXTBYTE;  // GETSHORT(y_position);
            GETSHORT(Width);
            GETSHORT(Height);
            ch = NEXTBYTE;
            Interlace = ((ch & 0x40) != 0);
    ...
    int length =
        sprintf((char *)(Suffix), "%d %d %d %d", Width, Height, -numcolors, 1);
