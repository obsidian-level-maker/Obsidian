...
// *name);
typedef void (*XX_gtk_file_chooser_set_current_name)(GtkFileChooser *,
                                                     const gchar *);
...
// gchar *name);
typedef void (*XX_gtk_file_chooser_set_current_folder)(GtkFileChooser *,
                                                       const gchar *);
...
Fl_GTK_File_Chooser::Fl_GTK_File_Chooser(int val) : Fl_FLTK_File_Chooser(-1) {
    ...
    gtkw_slist =
        NULL;  // will hold the returned file names in a multi-selection...
    gtkw_count = 0;  // How many items were selected?
    gtkw_filename =
        NULL;  // holds the last name we read back in a single file selection...
...
void Fl_GTK_File_Chooser::type(int val) { _btype = val; }

int Fl_GTK_File_Chooser::count() const { return gtkw_count; }

const char *Fl_GTK_File_Chooser::filename() const {
    if (gtkw_ptr) {
        if (fl_gtk_file_chooser_get_select_multiple(
                (GtkFileChooser *)gtkw_ptr) == FALSE) {
...
const char *Fl_GTK_File_Chooser::title() const { return gtkw_title; }

/* changes the extension of the outfile in the chooser according to newly
 * selected filter */
void Fl_GTK_File_Chooser::changed_output_type(const char *filter) {
    ...
    if (strchr(filter, '(') || strchr(filter, '{') || strchr(filter + 1, '*') ||
        ...
        return;
    const char *p =
        fl_gtk_file_chooser_get_filename((GtkFileChooser *)gtkw_ptr);
    if (!p) return;
...
gboolean Fl_GTK_File_Chooser::custom_gtk_filter_function(
    ...
    return (gboolean)fl_filename_match(fl_filename_name(info->filename),
                                       p->filter);
}

void Fl_GTK_File_Chooser::free_pair(Fl_GTK_File_Chooser::pair *p) { delete p; }
...
int Fl_GTK_File_Chooser::show() {
    // The point here is that after running a GTK dialog, the calling program's
    // current locale is modified. To avoid that, we memorize the calling
    // program's current locale, and the locale as modified by GTK after the
    // first dialog use. We restore the calling program's current locale before
    // returning, and we set the locale as modified by GTK before subsequent GTK
    // dialog uses.
...
int Fl_GTK_File_Chooser::fl_gtk_chooser_wrapper() {
        ...
        case Fl_Native_File_Chooser::BROWSE_SAVE_FILE:
            if (_preset_file)
                fl_gtk_file_chooser_set_current_name(
                    (GtkFileChooser *)gtkw_ptr, fl_filename_name(_preset_file));
            /* FALLTHROUGH */
        case Fl_Native_File_Chooser::BROWSE_SAVE_DIRECTORY:
            fl_gtk_file_chooser_set_create_folders((GtkFileChooser *)gtkw_ptr,
                                                   TRUE);
            fl_gtk_file_chooser_set_do_overwrite_confirmation(
                (GtkFileChooser *)gtkw_ptr,
                (_options & Fl_Native_File_Chooser::SAVEAS_CONFIRM) ? TRUE
                                                                    : FALSE);
            break;
    ...
    if (_directory && _directory[0]) {
        ...
        if (p)
            fl_gtk_file_chooser_set_current_folder((GtkFileChooser *)gtkw_ptr,
                                                   p);
    ...
    if (_parsedfilt) {
        ...
        while (p) {
            ...
            *q = 0;
            fl_gtk_file_filter_add_custom(
                ...
                (GtkFileFilterFunc)
                    Fl_GTK_File_Chooser::custom_gtk_filter_function,
        ...
        if (gtw_action_type == GTK_FILE_CHOOSER_ACTION_OPEN) {
            ...
            fl_gtk_file_filter_set_name(gfilter,
    ...
    if (firstw) {
        ...
        Window xw = fl_gdk_x11_drawable_get_xid(
            gdkw);  // get the X11 ref of the GTK window
        XSetTransientForHint(
            fl_display, xw,
            fl_xid(
                firstw));  // set the GTK window transient for the last FLTK win
    }
    gboolean state =
        fl_gtk_file_chooser_get_show_hidden((GtkFileChooser *)gtkw_ptr);
    fl_gtk_toggle_button_set_active((GtkToggleButton *)toggle, state);

    gint response_id = GTK_RESPONSE_NONE;
    fl_g_signal_connect_data(gtkw_ptr, "response",
                             G_CALLBACK(run_response_handler), &response_id,
                             NULL, (GConnectFlags)0);
    while (response_id ==
           GTK_RESPONSE_NONE) {   // loop that shows the GTK dialog window
        fl_gtk_main_iteration();  // one iteration of the GTK event loop
        while (XEventsQueued(fl_display,
                             QueuedAfterReading)) {  // emulate modal dialog
            XEvent xevent;
    ...
    if (response_id == GTK_RESPONSE_ACCEPT) {
        if (_parsedfilt) {
            GtkFileFilter *gfilter =
                fl_gtk_file_chooser_get_filter((GtkFileChooser *)gtkw_ptr);
        ...
        if (fl_gtk_file_chooser_get_select_multiple(
            ...
            gtkw_slist =
                fl_gtk_file_chooser_get_filenames((GtkFileChooser *)gtkw_ptr);
    ...
    // make sure the GtkFileChooserDialog is removed from the display correctly
