...
static XVisualInfo *gl3_getvisual(const int *blist, GLXFBConfig *pbestFB) {
    ...
    int fbcount;
    GLXFBConfig *fbc = glXChooseFBConfig(fl_display, DefaultScreen(fl_display),
                                         blist, &fbcount);
    if (!fbc) {
    ...
    for (i = 0; i < fbcount; ++i) {
        ...
        if (vi) {
            ...
            glXGetFBConfigAttrib(fl_display, fbc[i], GLX_SAMPLE_BUFFERS,
                                 &samp_buf);
            glXGetFBConfigAttrib(fl_display, fbc[i], GLX_SAMPLES, &samples);
            /*printf( "  Matching fbconfig %d, visual ID 0x%2lx: SAMPLE_BUFFERS
...
Fl_Gl_Choice *Fl_Gl_Choice::find(int m, const int *alistp) {
    ...
    for (g = first; g; g = g->next)
        if (g->mode == m && g->alist == alistp) return g;
...
#define DEBUG_PFD (0)  // 1 = PFD selection debug output, 0 = no debug output
    ...
    for (int i = 1;; i++) {
        ...
        if (pfd.iPixelType !=
            ((m & FL_INDEX) ? PFD_TYPE_COLORINDEX : PFD_TYPE_RGBA))
            continue;
        ...
        if (pixelformat) {
            // offering non-generic rendering is better (read: hardware
            // acceleration)
            ...
            // otherwise prefer a format that supports composition (STR #3119)
                ...
                continue;
            // otherwise more bit planes is better, but no more than 32 (8 bits
            // per channel):
            else if (pfd.cColorBits > 32 ||
                     chosen_pfd.cColorBits > pfd.cColorBits)
                continue;
    ...
    printf("Color bits = %d, Depth bits = %d\n", chosen_pfd.cColorBits,
           chosen_pfd.cDepthBits);
...
static void add_context(GLContext ctx) {
    ...
    if (nContext == NContext) {
        ...
        NContext *= 2;
        context_list =
            (GLContext *)realloc(context_list, NContext * sizeof(GLContext));
    }
...
GLContext fl_create_gl_context(Fl_Window *window, const Fl_Gl_Choice *g) {
    ...
    typedef GLContext (*glXCreateContextAttribsARBProc)(
        Display *, GLXFBConfig, GLContext, Bool, const int *);
    ...
    if (g->best_fb && strstr(glxExts, "GLX_ARB_create_context") &&
        ...
        ctx = glXCreateContextAttribsARB(fl_display, g->best_fb, shared_ctx,
...
GLContext fl_create_gl_context(XVisualInfo *vis) {
    ...
    if (context) add_context(context);
    return context;
}
...
GLContext fl_create_gl_context(Fl_Window *window, const Fl_Gl_Choice *g,
    ...
    context =
        Fl_X::create_GLcontext_for_window(g->pixelformat, shared_ctx, window);
    if (!context) return 0;
