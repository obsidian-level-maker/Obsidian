...
void *Fl_PostScript_Graphics_Driver::prepare85()  // prepare to produce
                                                  // ASCII85-encoded output
{
...
static int convert85(const uchar *bytes4, uchar *chars5) {
    ...
    unsigned val = bytes4[0] * (256 * 256 * 256) + bytes4[1] * (256 * 256) +
                   bytes4[2] * 256 + bytes4[3];
...
void Fl_PostScript_Graphics_Driver::write85(
    void *data, const uchar *p,
    int len)  // sends len input bytes for ASCII85 encoding
{
...
void Fl_PostScript_Graphics_Driver::close85(
    void *data)  // stops ASCII85-encoding after processing remaining unencoded
                 // input bytes, if any
{
    struct85 *big = (struct85 *)data;
    int l;
...
void *
Fl_PostScript_Graphics_Driver::prepare_rle85()  // prepare to produce
                                                // RLE+ASCII85-encoded output
{
...
void Fl_PostScript_Graphics_Driver::write_rle85(
    uchar b, void *data)  // sends one input byte to RLE+ASCII85 encoding
{
    struct_rle85 *rle = (struct_rle85 *)data;
    uchar c;
    if (rle->run_length > 0) {  // if within a run
        if (b == rle->buffer[0] &&
            rle->run_length < 128) {  // the run can be extended
            rle->run_length++;
            return;
        } else {  // output the run
    ...
    if (rle->count >= 2 && b == rle->buffer[rle->count - 1] &&
        b == rle->buffer[rle->count - 2]) {
        // about to begin a run
        if (rle->count >
            2) {  // there is non-run data before the run in the buffer
...
void Fl_PostScript_Graphics_Driver::close_rle85(
    void *data)  // stop doing RLE+ASCII85 encoding
{
    struct_rle85 *rle = (struct_rle85 *)data;
    uchar c;
...
//
// End of implementation of the /RunLengthEncode + /ASCII85Encode PostScript
// filter
//

int Fl_PostScript_Graphics_Driver::alpha_mask(const uchar *data, int w, int h,
                                              int D, int LD) {
    mask = 0;
    ...
    int V_ = 0;
    for (j = 0; j < h; j++) {
        for (i = 0; i < w; i++) switch (data[j * LD + D * i + D - 1]) {
                case 255:
                    V255 = 1;
                    break;
                case 0:
    ...
    for (i = 0; i < w * 4 + 2; i++)
        errors2[i] = 0;  // cleaning,after first swap will become current
    for (i = 0; i < w * 4 + 2; i++)
        errors1[i] = 0;  // cleaning,after first swap will become current
    ...
    for (j = 0; j < h; j++) {
        for (l = 0; l < 4;) {  // generating 4 rows of mask lines for 1 RGB line
            ...
            current = swap;
            *(next + 1) =
                0;  // must clean the first cell, next are overriden by *1
            for (i = 0; i < w; i++) {
                for (k = 0; k < 4; k++) {  // generating 4 x-pixels for 1 RGB
                    ...
                    if (val > 127) {
                        mask[jj * xx + ii / 8] |= 1 << (ii % 8);  // set mask
                                                                  // bit
                        error = val - 255;
                    } else
                        error = val;
                    ...
                    if (error > 0) {
                        ...
                        next[ii + 2] = o3 =
                            (error + 8) / 16;  // *1 - ok replacing (cleaning)
                    } else {
                        next[ii] += o1 = (error * 3 - 8) / 16;
                        current[ii + 2] += o2 = (error * 7 - 8) / 16;
...
void Fl_PostScript_Graphics_Driver::draw_image(const uchar *data, int ix,
    ...
    if (level2_mask) {
        ...
        for (j = 0; j < ih; j++) {
            ...
            for (i = 0; i < iw; i++) {
                ...
                if (lang_level_ < 3 &&
                    D > 3) {  // can do  mixing using bg_* colors)
...
void Fl_PostScript_Graphics_Driver::draw_image_mono(const uchar *data, int ix,
...
void Fl_PostScript_Graphics_Driver::draw_image_mono(Fl_Draw_Image_Cb call,
                                                    void *data, int ix, int iy,
                                                    int iw, int ih, int D) {
    double x = ix, y = iy, w = iw, h = ih;
...
void Fl_PostScript_Graphics_Driver::draw(Fl_Pixmap *pxm, int XP, int YP, int WP,
...
void Fl_PostScript_Graphics_Driver::draw(Fl_RGB_Image *rgb, int XP, int YP,
    ...
    mask = 0;
    if (lang_level_ >
        2)  // when not true, not making alphamask, mixing colors instead...
        if (alpha_mask(di, w, h, rgb->d(), rgb->ld()))
            return;  // everthing masked, no need for painting!
    push_clip(XP, YP, WP, HP);
...
int Fl_PostScript_Graphics_Driver::draw_scaled(Fl_Image *img, int XP, int YP,
                                               int WP, int HP) {
    int X, Y, W, H;
    clip_box(XP, YP, WP, HP, X, Y, W,
...
void Fl_PostScript_Graphics_Driver::draw(Fl_Bitmap *bitmap, int XP, int YP,
    ...
    if (WP >
        bitmap->w() - cx) {  // to assure that it does not go out of bounds;
        w = bitmap->w() - cx;
    ...
    int si =
        cx %
        8;  // small shift to be clipped, it is simpler than shifting whole mask
