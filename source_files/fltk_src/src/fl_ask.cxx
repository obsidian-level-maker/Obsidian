...
static Fl_Window *makeform() {
    ...
    {
        for (int b = 2; b >= 0; b--) w->add(button[b]);
    }
    w->begin();
...
static int innards(const char *fmt, va_list ap, const char *b0, const char *b1,
    ...
    Fl_Group *current_group =
        Fl_Group::current();  // make sure the dialog does not interfere with
                              // any active group
    message_form->show();
...
/** Shows a dialog displaying the \p fmt message,
   ...
   \param[in] fmt can be used as an sprintf-like format and variables for the
   message text \retval 0 if the no button is selected or another dialog box is
   still open \retval 1 if yes is selected

   \deprecated fl_ask() is deprecated since it uses "Yes" and "No" for the
   buttons which does not conform to the current FLTK Human Interface
   Guidelines. Use fl_choice() with the appropriate verbs instead.
 */
...
/** Shows a dialog displaying the printf style \p fmt message,
   ...
   "Delete", num_msgs) ) { case 0: .. // Send case 1: .. // Save (default) case
   2: .. // Delete
         ..
       }
   \endcode
   ...
   \param[in] fmt can be used as an sprintf-like format and variables for the
   message text \param[in] b0 text label of button 0 \param[in] b1 text label of
   button 1 (can be 0) \param[in] b2 text label of button 2 (can be 0) \retval 0
   if the first button with \p b0 text is pushed or another dialog box is still
   open \retval 1 if the second button with \p b1 text is pushed \retval 2 if
   the third button with \p b2 text is pushed
 */
...
/** Shows an input dialog displaying the \p fmt message
   ...
   \param[in] fmt can be used as an sprintf-like format and variables for the
   message text \param[in] defstr defines the default returned string if no text
   is entered \return the user string input if OK was pushed, NULL if Cancel was
   pushed or another dialog box was still open
 */
...
/** Shows an input dialog displaying the \p fmt message.
   ...
   message text \param[in] defstr defines the default returned string if no text
   is entered \return the user string input if OK was pushed, NULL if Cancel was
   pushed or aother dialog box was still open
 */
 ...
 */
int fl_message_hotspot(void) { return enableHotspot; }
...
void fl_message_title_default(const char *title) {
    ...
    if (title) message_title_default = strdup(title);
}
