    ...
    for (int n = 0; n < m; n++) {
        ...
        if (thissize == size && thislength < matchedlength) {
            ...
            ptsize = size;
                   ...
                   (thissize < ptsize &&
                    ptsize > size) ||  // current font too big
...
XFontStruct *Fl_XFont_On_Demand::value() { return ptr; }
...
void Fl_Xlib_Graphics_Driver::font(Fl_Font fnum, Fl_Fontsize size) {
    ...
    if (fnum == Fl_Graphics_Driver::font() &&
        size == Fl_Graphics_Driver::size())
        return;
...
double Fl_Xlib_Graphics_Driver::width(const char *c, int n) {
    if (font_descriptor())
        return (double)XUtf8TextWidth(font_descriptor()->font, c, n);
    else
        return -1;
}
...
double Fl_Xlib_Graphics_Driver::width(unsigned int c) {
    if (font_descriptor())
        return (double)XUtf8UcsWidth(font_descriptor()->font, c);
    else
        return -1;
}
...
void Fl_Xlib_Graphics_Driver::text_extents(const char *c, int n, int &dx,
    ...
    xx = yy = ww = hh = 0;
    if (fl_gc)
        XUtf8_measure_extents(fl_display, fl_window, font_descriptor()->font,
                              fl_gc, &xx, &yy, &ww, &hh, c, n);
...
void Fl_Xlib_Graphics_Driver::rtl_draw(const char *c, int n, int x, int y) {
    ...
    if (fl_gc)
        XUtf8DrawRtlString(fl_display, fl_window, font_descriptor()->font,
                           fl_gc, x, y, c, n);
}
#endif  // FL_DOXYGEN
//
