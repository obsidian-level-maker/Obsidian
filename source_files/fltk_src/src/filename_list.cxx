...
extern "C" {
#ifndef HAVE_SCANDIR
int fl_scandir(const char *dir, dirent ***namelist, int (*select)(dirent *),
...
/**
   ...
   nul-terminated file name. An pointers array to these dirent's is created and
   a pointer to the array is returned in *list. The number of entries is given
   as a return value. If there is an error reading the directory a number less
   than zero is returned, and errno has the reason; errno does not work under
   WIN32.
   ...
   \param[in] d the name of the directory to list.  It does not matter if it has
   a trailing slash. \param[out] list table containing the resulting directory
   listing \param[in] sort sorting functor:
   ...
   order, where an attempt is made to put unpadded numbers in consecutive order;
   ...
   where an attempt is made to put unpadded numbers in consecutive order; upper
   and lowercase letters are compared according to their ASCII ordering -
   uppercase before lowercase. \return the number of entries if no error, a
...
int fl_filename_list(const char *d, dirent ***list, Fl_File_Sort_F *sort) {
    ...
    int n = scandir(dirloc, list, 0,
                    (int (*)(const dirent **, const dirent **))sort);
    ...
    int n =
        scandir(dirloc, list, 0,
                (int (*)(const struct dirent **, const struct dirent **))sort);
#else
    ...
    char *fullname = (char *)malloc(
        dirlen + FL_PATH_MAX + 3);  // Add enough extra for two /'s and a nul
    ...
    for (i = 0; i < n; i++) {
...
#endif
        dirent *newde = (dirent *)malloc(de->d_name - (char *)de + newlen +
        ...
        if (de->d_name[len - 1] != '/' && len <= FL_PATH_MAX) {
            // Use memcpy for speed since we already know the length of the
            // string...
