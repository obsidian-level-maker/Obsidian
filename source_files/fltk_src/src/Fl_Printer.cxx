...
/**
 Starts a print job.
 Opens a platform-specific dialog window allowing the user to set several
 options including the desired printer and the page orientation. Optionally, the
 user can also select a range of pages to  be printed. This range is returned to
 the caller that is in charge of sending only these pages for printing.
 ...
 printed
 @return 0 if OK, non-zero if any error occurred or if the user cancelled the
 print request.
 */
...
int Fl_Printer::start_page(void) { return printer->start_page(); }
...
void Fl_Printer::origin(int *x, int *y) { printer->origin(x, y); }
...
void Fl_Printer::rotate(float angle) { printer->rotate(angle); }

void Fl_Printer::translate(int x, int y) { printer->translate(x, y); }

void Fl_Printer::untranslate(void) { printer->untranslate(); }

int Fl_Printer::end_page(void) { return printer->end_page(); }

void Fl_Printer::end_job(void) { printer->end_job(); }
...
void Fl_Printer::set_current(void) { printer->set_current(); }

Fl_Graphics_Driver *Fl_Printer::driver(void) { return printer->driver(); }

Fl_Printer::~Fl_Printer(void) { delete printer; }
