    ...
    for (;; p++) {
        ...
        if (!c || c == ' ' || c == '\n') {
            ...
            if (word_start < p && wrap) {
                ...
                if (word_end > buf &&
                    newwidth > maxw) {  // break before this word
                    o = word_end;
                    p = word_start;
                    break;
                }
        ...
        if (c == '\t') {
            ...
            *o++ = '^';
            *o++ = c ^ 0x40;
            /* This is in fact not useful: the point is that a valid UTF-8
            sequence for a non-ascii char contains no ascii char, thus no tab,
            space, control, & or @ we want to process differently. Also, invalid
            UTF-8 sequences are copied unchanged by this procedure. Therefore,
            checking for tab, space, control, & or @, and copying the byte
            otherwise, is enough. } else  if (handle_utf8_seq(p, o)) { // figure
            out if we have an utf8 valid sequence before we determine the nbsp
            test validity: #ifdef __APPLE__ } else if (c == 0xCA) { //
            non-breaking space in MacRoman #else } else if (c == 0xA0) { //
            non-breaking space in ISO 8859 #endif *o++ = ' ';*/
...
/**
 ...
 Sets width to the width of the string in the \ref drawing_fl_font "current
 font".
 */
...
/**
  ...
  the addition of the \p callthis parameter, which is a pointer to a text
  drawing function such as fl_draw(const char*, int, int, int) to do the real
  work
*/
void fl_draw(const char *str,             // the (multi-line) string
    ...
    if (draw_symbols) {
        if (str && str[0] == '@' && str[1] && str[1] != '@') {
            ...
            for (symptr = symbol[0];
                 *str && !isspace(*str) &&
                 symptr < (symbol[0] + sizeof(symbol[0]) - 1);
                 *symptr++ = *str++) { /*empty*/
            }
            *symptr = '\0';
            if (isspace(*str)) str++;
    ...
    if (img && !imgvert /* && (align & !FL_ALIGN_TEXT_NEXT_TO_IMAGE)*/) {
        if (align & FL_ALIGN_TEXT_OVER_IMAGE) {  // image is right of text
            ...
            if (align & FL_ALIGN_LEFT)
                ...
                xpos = x + (w - strw - symtotal - imgw[1]) / 2 + symwidth[0] +
                       strw + 1;
    ...
    if (str) {
        ...
        for (p = str;; ypos += height) {
            ...
            if (align & FL_ALIGN_LEFT)
                ...
                xpos =
                    x +
                    (w - (int)(width + .5) - symtotal - imgw[0] - imgw[1]) / 2 +
                    symwidth[0] + imgw[0];
            ...
            if (underline_at && underline_at >= linebuf &&
                ...
                callthis("_", 1,
                         xpos + int(fl_width(linebuf,
                                             (int)(underline_at - linebuf))),
                         ypos - desc);
    ...
    if (symwidth[0]) {
        ...
        fl_draw_symbol(symbol[0], xpos, ypos, symwidth[0], symwidth[0],
                       fl_color());
    }
    ...
    if (symwidth[1]) {
        ...
        fl_draw_symbol(symbol[1], xpos, ypos, symwidth[1], symwidth[1],
                       fl_color());
    }
}
...
/**
  ...
  fl_font(FL_HELVETICA, 14);   // set current font face/size to be used for
  measuring fl_measure(s, wi, hi);       // returns pixel width/height of string
  in current font \endcode
*/
void fl_measure(const char *str, int &w, int &h, int draw_symbols) {
    ...
    if (draw_symbols) {
        ...
        const char *sym2 = (str[0] == '@' && str[1] == '@')
                               ? str + 2
                               : str;  // sym2 check will skip leading @@
        if (str[0] == '@' && str[1] != '@') {
    ...
    for (p = str, lines = 0; p;) {
        //    e = expand(p, linebuf, w - symtotal, buflen, width, w != 0,
        //    draw_symbols);
        e = expand_text_(p, linebuf, 0, w - symtotal, buflen, width, w != 0,
                         draw_symbols);
