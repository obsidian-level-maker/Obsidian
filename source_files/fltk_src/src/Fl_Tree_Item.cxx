...
/// \deprecated in 1.3.3 ABI -- you must use Fl_Tree_Item(Fl_Tree*) for proper
/// horizontal scrollbar behavior.
#endif
///
Fl_Tree_Item::Fl_Tree_Item(const Fl_Tree_Prefs &prefs) { _Init(prefs, 0); }
...
void Fl_Tree_Item::_Init(const Fl_Tree_Prefs &prefs, Fl_Tree *tree) {
    ...
    _parent = 0;
#if FLTK_ABI_VERSION >= 10303
    _children.manage_item_destroy(
        1);  // let array's dtor manage destroying Fl_Tree_Items
#endif
#if FLTK_ABI_VERSION >= 10301
    _prev_sibling = 0;
    _next_sibling = 0;
...
///
/// This must be used instead of the older, deprecated
/// Fl_Tree_Item(Fl_Tree_Prefs) constructor for proper horizontal scrollbar
/// calculation.
///
/// \version 1.3.3 ABI feature
///
Fl_Tree_Item::Fl_Tree_Item(Fl_Tree *tree) { _Init(tree->_prefs, tree); }
#endif
...
void Fl_Tree_Item::show_self(const char *indent) const {
...
#if FLTK_ABI_VERSION >= 10301
    printf(
        "%s-%s (%d children, this=%p, parent=%p, prev=%p, next=%p, depth=%d)\n",
    ...
    printf("%s-%s (%d children, this=%p, parent=%p depth=%d)\n", indent,
           thelabel, children(), (void *)this, (void *)_parent, depth());
#endif /*FLTK_ABI_VERSION*/
    if (children()) {
...
/// Return the label.
const char *Fl_Tree_Item::label() const { return (_label); }
...
int Fl_Tree_Item::find_child(const char *name) {
    if (name) {
        for (int t = 0; t < children(); t++)
            if (child(t)->label())
                if (strcmp(child(t)->label(), name) == 0) return (t);
    }
    return (-1);
}
...
const Fl_Tree_Item *Fl_Tree_Item::find_child_item(const char *name) const {
    if (name)
        for (int t = 0; t < children(); t++)
            if (child(t)->label())
                if (strcmp(child(t)->label(), name) == 0) return (child(t));
    return (0);
}
...
///
Fl_Tree_Item *Fl_Tree_Item::add(const Fl_Tree_Prefs &prefs,
                                const char *new_label, Fl_Tree_Item *item) {
#if FLTK_ABI_VERSION >= 10303
    if (!item) {
...
Fl_Tree_Item *Fl_Tree_Item::add(const Fl_Tree_Prefs &prefs, char **arr,
    ...
    return (newchild = add(prefs, *arr))  // create new immediate child
               ? newchild->add(prefs, arr + 1,
                               newitem)  // it worked? recurse to add
               : 0;                      // failed? error
}
...
///
int Fl_Tree_Item::move(int to, int from) { return _children.move(to, from); }
...
int Fl_Tree_Item::move(Fl_Tree_Item *item, int op, int pos) {
    ...
    if (from_parent == to_parent) {  // same parent?
        ...
        if (to > to_parent->children())  // try to prevent a reparent() error
            return -4;
        if (from_parent->deparent(from) ==
            NULL)  // deparent self from current parent
            return -5;
        if (to_parent->reparent(this, to) <
            0) {  // reparent self to new parent at position 'to'
            to_parent->reparent(this,
                                0);  // failed? shouldn't happen, reparent at 0
            return -6;
        }
    }
    return 0;
...
///
int Fl_Tree_Item::move_above(Fl_Tree_Item *item) { return move(item, 0, 0); }
...
///          see move(Fl_Tree_Item*,int,int) for possible error codes.
///
int Fl_Tree_Item::move_below(Fl_Tree_Item *item) { return move(item, 1, 0); }
...
/// \version 1.3.3 ABI feature
///
const Fl_Tree_Prefs &Fl_Tree_Item::prefs() const { return (_tree->_prefs); }
...
///
void Fl_Tree_Item::swap_children(int ax, int bx) { _children.swap(ax, bx); }
...
///
void Fl_Tree_Item::draw_horizontal_connector(int x1, int x2, int y,
...
/// \param[in] prefs The Fl_Tree prefs
///
void Fl_Tree_Item::draw_vertical_connector(int x, int y1, int y2,
                                           const Fl_Tree_Prefs &prefs) {
    fl_color(prefs.connectorcolor());
    switch (prefs.connectorstyle()) {
        case FL_TREE_CONNECTOR_SOLID:
...
const Fl_Tree_Item *Fl_Tree_Item::find_clicked(const Fl_Tree_Prefs &prefs,
    ...
    if (is_open()) {  // open? check children of this item
        for (int t = 0; t < children(); t++) {
            ...
            if ((item = _children[t]->find_clicked(prefs, yonly)) !=
                NULL)           // recurse into child for descendents
                return (item);  // found?
        }
    }
    return (0);
...
/// Non-const version of Fl_Tree_Item::find_clicked(const Fl_Tree_Prefs&,int)
/// const
...
const Fl_Tree_Item *Fl_Tree_Item::find_clicked(
    ...
    if (is_open()) {  // open? check children of this item
        for (int t = 0; t < children(); t++) {
            const Fl_Tree_Item *item;
            if ((item = _children[t]->find_clicked(prefs)) !=
                NULL)           // recurse into child for descendents
                return (item);  // found?
        }
    }
...
static void draw_item_focus(Fl_Boxtype B, Fl_Color fg, Fl_Color bg, int X,
                            int Y, int W, int H) {
    ...
    fl_rect(X + Fl::box_dx(B), Y + Fl::box_dy(B), W - Fl::box_dw(B) - 1,
            H - Fl::box_dh(B) - 1);
    ...
    for (xx = 0, i = 1; xx < W; xx++, i++)
        if (i & 1) fl_point(X + xx, Y);
    for (yy = 0; yy < H; yy++, i++)
        if (i & 1) fl_point(X + W, Y + yy);
    for (xx = W; xx > 0; xx--, i++)
        if (i & 1) fl_point(X + xx, Y + H);
    for (yy = H; yy > 0; yy--, i++)
        if (i & 1) fl_point(X, Y + yy);
#endif
}
...
int Fl_Tree_Item::calc_item_height(const Fl_Tree_Prefs &prefs) const {
    ...
    if (_label) {
        ...
        H = _labelsize + fl_descent() +
            1;  // at least one pixel space below descender
    }
#if FLTK_ABI_VERSION >= 10301
    if (widget() &&
...
Fl_Color Fl_Tree_Item::drawbgcolor() const {
    ...
    return is_selected()                  ? is_active() && tree()->active_r()
...
///         // Draw label
///         fl_font(labelfont(), labelsize());      // use item's label
///         font/size fl_color(drawfgcolor());                // use recommended
///         fg color fl_draw(text, X,Y,W,H, FL_ALIGN_LEFT);  // draw the item's
///         label
///       }
...
/// \code
///     // Example #1: using add()
///     MyTreeItem *bart = new MyTreeItem(..);  // class derived from
///     Fl_Tree_Item tree->add("/Simpsons/Bart", bart);      // Add item as
///     /Simpsons/Bart
/// \endcode
///
...
///     MyTreeItem *marge = new MyTreeItem(..); // class derived from
///     Fl_Tree_Item item = tree->add("/Simpsons/Marge");    // create item
...
int Fl_Tree_Item::draw_item_content(int render) {
    ...
    if (render && (bg != tree()->color() || is_selected())) {
        if (is_selected()) {  // Selected? Use selectbox() style
            fl_draw_box(prefs.selectbox(), label_x(), label_y(), label_w(),
                        label_h(), bg);
        ...
        if (widget())
            widget()->damage(FL_DAMAGE_ALL);  // if there's a child widget, we
                                              // just damaged it
    }
    // Draw label
    if (_label && (!widget() || (prefs.item_draw_mode() &
        ...
        int ly =
            label_y() + (label_h() / 2) + (_labelsize / 2) - fl_descent() / 2;
        int lw = 0, lh = 0;
        fl_measure(_label, lw,
                   lh);  // get box around text (including white space)
...
/// \param[in,out] tree_item_xmax The tree's running xmax (right-most edge so
/// far).
...
///                               0: no rendering, just calculate size w/out
///                               drawing. 1: render item as well as size calc
///
...
void Fl_Tree_Item::draw(int X, int &Y, int W, Fl_Tree_Item *itemfocus,
    ...
    //   Note: calculate collapse icon's xywh for possible mouse click
    //   detection. We don't care about items clipped off the viewport; they
    //   won't get mouse events.
    //
    ...
    //   Must calculate these even if(clipped) because 'draw children' code
    //   (below) needs hconn_x_center value. (Otherwise, these calculations
    //   could be 'clipped')
    //
    ...
    int uicon_x =
        X + (icon_w / 2 - 1 + conn_w) +
        ((usericon() || prefs.usericon()) ? prefs.usericonmarginleft() : 0);
    int uicon_w = usericon()         ? usericon()->w()
                  : prefs.usericon() ? prefs.usericon()->w()
                                     : 0;
    ...
    if (widget()) {
        ...
        if (widget()->x() != wx || widget()->y() != wy || widget()->w() != ww ||
            widget()->h() != wh) {
    ...
    if (!render)
        clipped =
            0;  // NOT rendering? Then don't clip, so we calc unclipped items
#if FLTK_ABI_VERSION >= 10304
    ...
    if (!clipped) {
        ...
        //    'clipped' is an optimization to prevent drawing anything
        //    offscreen.
        //
        if (drawthis) {  // draw this item at all?
            if ((tree()->damage() & ~FL_DAMAGE_CHILD) ||
                !render) {  // non-child damage?
                // Draw connectors
                if (render &&
                    prefs.connectorstyle() != FL_TREE_CONNECTOR_NONE) {
                    // Horiz connector between center of icon and text
                    // if this is root, the connector should not dangle in thin
                    // air on the left
                    if (is_root())
                        draw_horizontal_connector(hconn_x_center, hconn_x2,
                                                  item_y_center, prefs);
                    else
                        draw_horizontal_connector(hconn_x, hconn_x2,
                                                  item_y_center, prefs);
                    // Small vertical line down to children
                    if (has_children() && is_open())
                        draw_vertical_connector(hconn_x_center, item_y_center,
                                                Y + H2, prefs);
                    // Connectors for last child
                    if (!is_root()) {
                        if (lastchild)
                            draw_vertical_connector(hconn_x, Y, item_y_center,
                                                    prefs);
                        else
                            draw_vertical_connector(hconn_x, Y, Y + H2, prefs);
                    }
                }
                // Draw collapse icon
                ...
                if (render && usericon()) {
                    ...
                    if (active)
                        ...
                        userdeicon()->draw(uicon_x, uicon_y);
                    ...
                    else if (prefs.userdeicon())
                        prefs.userdeicon()->draw(uicon_x, uicon_y);
                }
#else
                if (render && usericon()) {
                    // Item has user icon? Use it
            ...
            if (widget()) {
                if (render)
                    tree()->draw_child(
                        *widget());  // let group handle drawing child
                if (widget()->label() && render)
                    tree()->draw_outside_label(*widget());  // label too
                xmax = widget()->x() +
                       widget()->w();  // update max width of widget
            }
            // Draw focus box around item's bg last
            if (render && this == itemfocus && Fl::visible_focus() &&
                Fl::focus() == tree() &&
                prefs.selectmode() != FL_TREE_SELECT_NONE) {
                draw_item_focus(FL_NO_BOX, fg, bg, label_x() + 1, label_y() + 1,
                                label_w() - 1, label_h() - 1);
            }
        }                   // end drawthis
    }                       // end clipped
    ...
    if (xmax > tree_item_xmax) tree_item_xmax = xmax;
    // Draw child items (if any)
    if (has_children() && is_open()) {
        int child_x = drawthis ? (hconn_x_center - (icon_w / 2) +
                                  1)  // offset children to right,
                               : X;   // unless didn't drawthis
        int child_w = W - (child_x - X);
        int child_y_start = Y;
        for (int t = 0; t < children(); t++) {
            int lastchild = ((t + 1) == children()) ? 1 : 0;
            _children[t]->draw(child_x, Y, child_w, itemfocus, tree_item_xmax,
                               lastchild, render);
        }
        if (has_children() && is_open()) {
            Y +=
                prefs.openchild_marginbottom();  // offset below open child tree
        }
        if (!lastchild) {
...
void Fl_Tree_Item::draw(int X, int &Y, int W, Fl_Widget *tree,
                        Fl_Tree_Item *itemfocus, const Fl_Tree_Prefs &prefs,
                        int lastchild) {
    if (!is_visible()) return;
    int tree_top = tree->y();
    ...
    if (widget()) {
        ...
        if (_label &&
            !widget()) {  // back compat: don't draw label if widget() present
#endif /*FLTK_ABI_VERSION*/
            fl_font(_labelfont, _labelsize);  // fldescent() needs this
            int lw = 0, lh = 0;
            fl_measure(_label, lw,
    ...
    if (!clipped) {
        ...
        Fl_Color fg = is_selected()
                          ? fl_contrast(_labelfgcolor, tree->selection_color())
                      : active ? _labelfgcolor
                               : fl_inactive(_labelfgcolor);
        Fl_Color bg = is_selected() ? active
                                          ? tree->selection_color()
                                          : fl_inactive(tree->selection_color())
                      : _labelbgcolor == unspecified ? tree->color()
                                                     : _labelbgcolor;
        ...
        if (drawthis) {                               // draw this item at all?
            if (tree->damage() & ~FL_DAMAGE_CHILD) {  // non-child damage?
                ...
                // Draw background for the item.. only if different from tree's
                // bg color
                if (bg != tree->color() || is_selected()) {
                    if (is_selected()) {  // Selected? Use selectbox() style
                        fl_draw_box(prefs.selectbox(), bg_x, bg_y, bg_w, bg_h,
                                    bg);
                    } else {  // Not Selected? use plain filled rectangle
                        fl_color(bg);
                    ...
                    if (widget())
                        widget()->damage(
                            FL_DAMAGE_ALL);  // if there's a child widget, we
                                             // just damaged it
                }
                // Draw user icon (if any)
#if FLTK_ABI_VERSION >= 10304
...
#endif /*FLTK_ABI_VERSION*/
                {
                    ...
                    fl_font(_labelfont, _labelsize);
                    int label_y =
                        Y + (H / 2) + (_labelsize / 2) - fl_descent() / 2;
            ...
            if (widget()) {
                ((Fl_Tree *)tree)
                    ->draw_child(*widget());  // let group handle drawing child
                if (widget()->label())
                    ((Fl_Tree *)tree)
                        ->draw_outside_label(*widget());  // label too
            }
            // Draw focus box around item's bg last
            if (this == itemfocus && Fl::visible_focus() &&
                Fl::focus() == tree &&
                prefs.selectmode() != FL_TREE_SELECT_NONE) {
                draw_item_focus(FL_NO_BOX, fg, bg, bg_x + 1, bg_y + 1, bg_w - 1,
                                bg_h - 1);
            }
        }                   // end drawthis
    }                       // end clipped
    ...
    if (has_children() && is_open()) {
        ...
        for (int t = 0; t < children(); t++) {
            int lastchild = ((t + 1) == children()) ? 1 : 0;
            _children[t]->draw(child_x, Y, child_w, tree, itemfocus, prefs,
                               lastchild);
        }
        if (has_children() && is_open()) {
            Y +=
...
Fl_Tree_Item *Fl_Tree_Item::next() {
    ...
    while ((p = c->parent()) != NULL) {  // loop upwards through parents
        int t =
            p->find_child(c);  // find our position in parent's children[] array
...
Fl_Tree_Item *Fl_Tree_Item::prev() {
    ...
    while (p->has_children()) {       // item has children?
        p = p->child(p->children() -
                     1);  // descend hierarchy finding deepest 'last child'
    }
    return (p);
#else  /*FLTK_ABI_VERSION*/
    // OLD
    ...
    int t =
        p->find_child(this);  // find our position in parent's children[] array
...
Fl_Tree_Item *Fl_Tree_Item::next_sibling() {
    ...
    int index = parent()->find_child(
        this);                    // find our position in parent's child() array
    if (index == -1) return (0);  // parent doesn't know us? weird
    if ((index + 1) < parent()->children())  // is there a next child?
        return (parent()->child(
            index + 1));  // return next child if there's one below us
...
Fl_Tree_Item *Fl_Tree_Item::prev_sibling() {
    ...
    if (!parent()) return (0);  // No parent (root)? We have no siblings
    int index = parent()->find_child(
        this);  // find next position up in parent's child() array
    if (index == -1) return (0);  // parent doesn't know us? weird
    if (index > 0)
        return (parent()->child(
            index - 1));  // return previous child if there's one above us
...
///                  Special case if index=-1: become an orphan; null out all
///                  parent/sibling associations.
///
...
Fl_Tree_Item *Fl_Tree_Item::prev_visible(Fl_Tree_Prefs &prefs) {
    ...
    while (c) {
        ...
        if (c->is_root())  // root
            return ((prefs.showroot() && c->visible())
                        ? c
                        : 0);         // return root if visible
        if (!c->visible()) continue;  // item not visible? skip
        // Check all parents to be sure none are closed.
        // If closed, move up to that level and repeat until sure none are
        // closed.
        Fl_Tree_Item *p = c->parent();
        while (1) {
            if (!p || p->is_root())
                return (c);  // hit top? then we're displayed, return c
...
int Fl_Tree_Item::visible_r() const {
    if (!visible()) return (0);
    for (const Fl_Tree_Item *p = parent(); p;
         p = p->parent())  // move up through parents
        if (!p->visible() || p->is_close())
            return (0);  // any parent not visible or closed?
    return (1);
}
