...
const char *Fl::get_font_name(Fl_Font fnum, int *ap) {
    ...
    if (!f->fontname[0]) {
        ...
        // NOTE: This can cause duplications in fonts that already have Bold or
        // Italic in their "name". Maybe we need to find a cleverer way?
...
static int name_sort(const void *aa, const void *bb) {
    ...
    // duplicates as far as fltk is concerned, e.g. where there are ko or ja
    // variants that we can't distinguish (since we are not yet fully UTF-*) -
    // should we strip them here?
...
// This is just a mess - I should have tokenised the strings and gone from
// there, but I really thought this would be easier!
static void make_raw_name(char *raw, char *pretty) {
    ...
    // It is still possible that the "pretty" name has multiple comma separated
    // entries I've seen this often in CJK fonts, for example... Keep only the
    // first one... This is not ideal, the CJK fonts often have the name in utf8
    // in several languages. What we ought to do is use fontconfig to query the
    // available languages and pick one... But which?
  ...
  raw[LOCAL_RAW_NAME_MAX-1] = 0;
    ...
    strncat(raw, pretty,
            LOCAL_RAW_NAME_MAX -
                1);  // only copy MAX-1 chars, we have already set cell 0
...
#define BITALIC (BOLD | ITALIC)
        ...
        while ((style) && (style < last)) {
            ...
            switch (type) {
                // Things we might see: Regular Normal Bold Italic Oblique
                // (??what??) Medium Roman Light Demi Sans SemiCondensed
                // SuperBold Book... etc... Things we actually care about: Bold
                // Italic Oblique SuperBold - Others???
                case 'I':
...
Fl_Font Fl::set_fonts(const char *pattern_name) {
    ...
    if (fnt_set) {
        ...
        for (j = 0; j < font_count; j++) {
            ...
            // So the bit we want is up to the first comma - BUT some strings
            // have more than one name, separated by, guess what?, a comma...
            stop = start = first = 0;
            ...
            // Matt: Actually, there is no guarantee that the *first*
            // description is the English one. Matt: So we keep the entire
            // description, just in case.
            // if (stop)
            //{
            //  *stop = 0; // Terminate the string at the first comma, if there
            //  is one
            //}
        ...
        for (j = 0; j < font_count; j++) {
            if (full_list[j]) {
                ...
                // NOTE: This just adds on AFTER the default fonts - no attempt
                // is made to identify already loaded fonts. Is this bad?
                stored_name = strdup(xft_name);
                Fl::set_font((Fl_Font)(j + FL_FREE_FONT), stored_name);
                fl_free_font++;

                free(
                    full_list[j]);  // release that name from our internal array
            }
        }
...
int Fl::get_font_sizes(Fl_Font fnum, int *&sizep) {
    ...
    fl_open_display();
    XftFontSet *fs =
        XftListFonts(fl_display, fl_screen, XFT_FAMILY, XftTypeString,
                     s->name + 1, (void *)0, XFT_PIXEL_SIZE, (void *)0);
    static int *array = 0;
