...
static const uchar inactive_ramp[24] = {43, 43, 44, 44, 44, 45, 45, 46,
...
Fl_Box_Draw_F *Fl::get_boxtype(Fl_Boxtype t) { return fl_box_table[t].f; }
/** Sets the function to call to draw a specific boxtype. */
void Fl::set_boxtype(Fl_Boxtype t, Fl_Box_Draw_F *f, uchar a, uchar b, uchar c,
                     uchar d) {
    fl_box_table[t].f = f;
    fl_box_table[t].set = 1;
    fl_box_table[t].dx = a;
...
void Fl_Widget::draw_backdrop() const {
    if (align() & FL_ALIGN_IMAGE_BACKDROP) {
        ...
        if (img && deimage() && !active_r()) img = deimage();
        if (img)
            ((Fl_Image *)img)
                ->draw(x_ + (w_ - img->w()) / 2, y_ + (h_ - img->h()) / 2);
    }
}
...
void Fl_Widget::draw_box(Fl_Boxtype t, int X, int Y, int W, int H,
                         Fl_Color c) const {
