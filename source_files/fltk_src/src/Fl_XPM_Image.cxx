    ...
    while (fgets(buffer, MAXSIZE + 20, f)) {
        ...
        while (*q != '\"' && myp < buffer + MAXSIZE) {
            if (*q == '\\') switch (*++q) {
                    case '\r':
                    case '\n':
                        if (!fgets(q, (int)(buffer + MAXSIZE + 20 - q),
                                   f)) { /* no problem if we hit EOF */
                        }
                        break;
                    case 0:
                        break;
        ...
        *myp++ = 0;
        ...
        //   if ncolors < 0 this is FLTK (non standard) compressed colormap -
        //   all colors coded in single line of 4*ncolors bytes otherwise -
        //   ncolor lines of at least chars_per_pixel bytes
        // followed by pic segment: H lines of at least chars_per_pixel*W bytes
        // next line: would have loved to use measure_pixmap, but it doesn't
        // return all the data!
        if ((!i) && (sscanf(buffer, "%d%d%d%d", &W, &H, &ncolors,
                            &chars_per_pixel) < 4))
            goto bad_data;  // first line
        else if ((i > (ncolors < 0 ? 1 : ncolors)) &&
                 (myp - buffer - 1 < W * chars_per_pixel))
            goto bad_data;  // pic segment
        else if (myp - buffer - 1 <
                 (ncolors < 0 ? -ncolors * 4 : chars_per_pixel))
