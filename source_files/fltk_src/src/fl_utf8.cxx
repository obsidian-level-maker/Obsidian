...
*/
int fl_tolower(unsigned int ucs) { return XUtf8Tolower(ucs); }

/**
  Returns the Unicode upper case value of \p ucs.
*/
int fl_toupper(unsigned int ucs) { return Toupper(ucs); }
...
char *fl_utf8_to_locale(const char *s, int len, UINT codepage) {
    ...
    wbufa[l] = 0;
    buf[l] = 0;
    l = WideCharToMultiByte(codepage, 0, (WCHAR *)wbufa, l, buf, buf_len, NULL,
                            NULL);
    if (l < 0) l = 0;
    buf[l] = 0;
    return buf;
...
char *fl_utf2mbcs(const char *s) {
    ...
    l = fl_utf8toUtf16(s, (unsigned)l, (unsigned short *)mbwbuf,
                       wn);  // Convert string
    mbwbuf[l] = 0;
...
char *fl_getenv(const char *v) {
    ...
    wn = fl_utf8toUtf16(v, (unsigned)l, (unsigned short *)wbuf,
                        wn);  // Convert string
    wbuf[wn] = 0;
...
FILE *fl_fopen(const char *f, const char *mode) {
    ...
    wbuf = (xchar *)realloc(wbuf, sizeof(xchar) * wn);
    wn = fl_utf8toUtf16(f, (unsigned)l, (unsigned short *)wbuf,
                        wn);  // Convert string
    wbuf[wn] = 0;
    l = strlen(mode);
    wn = fl_utf8toUtf16(mode, (unsigned)l, NULL, 0) + 1;  // Query length
    wbuf1 = (xchar *)realloc(wbuf1, sizeof(xchar) * wn);
    wn = fl_utf8toUtf16(mode, (unsigned)l, (unsigned short *)wbuf1,
                        wn);  // Convert string
    wbuf1[wn] = 0;
...
/** Cross-platform function to run a system command with a UTF-8 encoded string.
  ...
  \return the return value of _wsystem() on Windows or system() on other
  platforms.
*/
...
int fl_system(const char *cmd) {
    ...
    size_t l = strlen(cmd);
    unsigned wn =
        fl_utf8toUtf16(cmd, (unsigned)l, NULL, 0) + 1;  // Query length
    wbuf = (xchar *)realloc(wbuf, sizeof(xchar) * wn);
    wn = fl_utf8toUtf16(cmd, (unsigned)l, (unsigned short *)wbuf,
                        wn);  // Convert string
    wbuf[wn] = 0;
...
int fl_execvp(const char *file, char *const *argv) {
    ...
    xchar **ar;
    unsigned wn =
        fl_utf8toUtf16(file, (unsigned)l, NULL, 0) + 1;  // Query length
    wbuf = (xchar *)realloc(wbuf, sizeof(xchar) * wn);
    wn = fl_utf8toUtf16(file, (unsigned)l, (unsigned short *)wbuf,
                        wn);  // Convert string
    wbuf[wn] = 0;
    ...
    while (i <= n) {
        ...
        wn = fl_utf8toUtf16(argv[i], (unsigned)l, (unsigned short *)ar[i],
                            wn);  // Convert string
        ar[i][wn] = 0;
        i++;
    }
...
/** Cross-platform function to set a files mode() with a UTF-8 encoded
 ...
 standard chmod() function fails with UTF-8 encoded non-ASCII filenames.
  ...
  \return    the return value of _wchmod() on Windows or chmod() on other
 platforms.
*/
...
/** Cross-platform function to test a files access() with a UTF-8 encoded
 ...
 standard access() function fails with UTF-8 encoded non-ASCII filenames.
  ...
  \return    the return value of _waccess() on Windows or access() on other
 platforms.
*/
...
/** Cross-platform function to stat() a file using a UTF-8 encoded
 ...
 standard stat() function fails with UTF-8 encoded non-ASCII filenames.
  ...
  \return    the return value of _wstat() on Windows or stat() on other
 platforms.
*/
...
/** Cross-platform function to unlink() (that is, delete) a file using
 ...
 standard function expects UTF-16 encoded non-ASCII filenames.
  ...
  \return    the return value of _wunlink() on Windows or unlink() on other
 platforms.
*/
...
/** Cross-platform function to create a directory with a UTF-8 encoded
 ...
 standard _wmkdir() function expects UTF-16 encoded non-ASCII filenames.
  ...
  \return    the return value of _wmkdir() on Windows or mkdir() on other
 platforms.
*/
...
/** Cross-platform function to remove a directory with a UTF-8 encoded
 ...
 standard _wrmdir() function expects UTF-16 encoded non-ASCII filenames.
  ...
  \return    the return value of _wrmdir() on Windows or rmdir() on other
 platforms.
*/
...
/** Cross-platform function to rename a filesystem object using
 ...
 standard _wrename() function expects UTF-16 encoded non-ASCII filenames.
  ...
  \return    the return value of _wrename() on Windows or rename() on other
 platforms.
*/
int fl_rename(const char *f, const char *n) {
    ...
    wbuf1 = (xchar *)realloc(wbuf1, sizeof(xchar) * wn);
    wn = fl_utf8toUtf16(n, (unsigned)l, (unsigned short *)wbuf1,
                        wn);  // Convert string
    wbuf1[wn] = 0;
