            ...
            if (event == FL_RELEASE) {
                ...
                if (o &&          // Released on a tab and..
                    (value(o) ||  // tab changed value or..
                     (when() & (FL_WHEN_NOT_CHANGED))  // ..no change but
                                                       // WHEN_NOT_CHANGED set,
...
int Fl_Tabs::push(Fl_Widget *o) {
    if (push_ == o) return 0;
    if ((push_ && !push_->visible()) || (o && !o->visible())) redraw_tabs();
    push_ = o;
    return 1;
}
...
void Fl_Tabs::draw() {
    ...
    if (damage() & FL_DAMAGE_ALL) {  // redraw the entire thing:
        ...
        draw_box(box(), x(), y() + (H >= 0 ? H : 0), w(),
                 h() - (H >= 0 ? H : -H), c);

        if (selection_color() != c) {
            // Draw the top or bottom SELECTION_BORDER lines of the tab pane in
            // the selection color so that the user knows which tab is
            // selected...
            int clip_y = (H >= 0) ? y() + H : y() + h() + H - SELECTION_BORDER;
            fl_push_clip(x(), clip_y, w(), SELECTION_BORDER);
            draw_box(box(), x(), clip_y, w(), SELECTION_BORDER,
                     selection_color());
            fl_pop_clip();
        }
        if (v) draw_child(*v);
    ...
    if (damage() & (FL_DAMAGE_SCROLL | FL_DAMAGE_ALL)) {
        ...
        for (i = 0; i < selected; i++)
            draw_tab(x() + tab_pos[i], x() + tab_pos[i + 1], tab_width[i], H,
                     a[i], LEFT);
...
Fl_Tabs::~Fl_Tabs() { clear_tab_positions(); }
