...
void Fl_Scroll::draw() {
    ...
    // Calculate where scrollbars should go, and draw them
    {
        ...
        if (si.vneeded && !scrollbar.visible()) {
            ...
            draw_clip(this, si.vscroll.x, si.vscroll.y, si.vscroll.w,
                      si.vscroll.h);
            d = FL_DAMAGE_ALL;
        }
        if (si.hneeded && !hscrollbar.visible()) {
            ...
            draw_clip(this, si.hscroll.x, si.hscroll.y, si.hscroll.w,
                      si.hscroll.h);
            d = FL_DAMAGE_ALL;
        } else if (hscrollbar.h() != si.scrollsize ||
                   scrollbar.w() != si.scrollsize) {
            // scrollsize changed
            d = FL_DAMAGE_ALL;
        }

        scrollbar.resize(si.vscroll.x, si.vscroll.y, si.vscroll.w,
                         si.vscroll.h);
        oldy = yposition_ = si.vscroll.pos;  // si.innerchild.y - si.child.t;
        scrollbar.value(si.vscroll.pos, si.vscroll.size, si.vscroll.first,
                        si.vscroll.total);

        hscrollbar.resize(si.hscroll.x, si.hscroll.y, si.hscroll.w,
                          si.hscroll.h);
...
void Fl_Scroll::resize(int X, int Y, int W, int H) {
    ...
    if (dw == 0 && dh == 0) {
        ...
        scrollbar.position(al ? X : X + W - scrollbar.w(),
                           (at && pad) ? Y + hscrollbar.h() : Y);
        hscrollbar.position((al && pad) ? X + scrollbar.w() : X,
...
Fl_Scroll::Fl_Scroll(int X, int Y, int W, int H, const char *L)
    : Fl_Group(X, Y, W, H, L),
      scrollbar(X + W - Fl::scrollbar_size(), Y, Fl::scrollbar_size(),
                H - Fl::scrollbar_size()),
      hscrollbar(X, Y + H - Fl::scrollbar_size(), W - Fl::scrollbar_size(),
                 Fl::scrollbar_size()) {
    type(BOTH);
    xposition_ = oldx = 0;
    yposition_ = oldy = 0;
