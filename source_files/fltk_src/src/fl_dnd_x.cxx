...
static int dnd_aware(Window &window) {
    ...
    XGetWindowProperty(fl_display, window, fl_XdndAware, 0, 4, False, XA_ATOM,
...
int Fl::dnd() {
    ...
    while (Fl::pushed()) {
        ...
        for (Window child = RootWindow(fl_display, fl_screen);;) {
            Window root;
            unsigned int junk3;
            XQueryPointer(fl_display, child, &root, &child, &e_x_root,
                          &e_y_root, &dest_x, &dest_y, &junk3);
            if (!child) {
        ...
        if (new_window != target_window) {
            if (local_window) {
                ...
                fl_sendClientMessage(target_window, fl_XdndLeave,
                                     source_window);
            }
            ...
            if (local_window) {
                ...
                if ((!strncmp(fl_selection_buffer[0], "file:///", 8) ||
                    ...
                    // Send file/URI list...
                    fl_sendClientMessage(target_window, fl_XdndEnter,
                                         source_window, dndversion << 24,
                                         fl_XdndURIList, XA_STRING, 0);
                } else {
                    // Send plain text...
                    fl_sendClientMessage(target_window, fl_XdndEnter,
                                         source_window, dndversion << 24,
    ...
    if (local_window) {
        ...
        fl_sendClientMessage(target_window, fl_XdndDrop, source_window, 0,
                             fl_event_time);
