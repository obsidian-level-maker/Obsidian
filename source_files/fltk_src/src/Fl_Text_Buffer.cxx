...
static int
    undoyankcut;  // length of valid contents of buffer, even if undocut=0
 ...
 */
void Fl_Text_Buffer::copy(Fl_Text_Buffer *fromBuf, int fromStart, int fromEnd,
                          int toPos) {
...
void Fl_Text_Buffer::canUndo(char flag) {
    ...
    if (!mCanUndo && undowidget == this) undowidget = 0;
}
 ...
 */
char *Fl_Text_Buffer::selection_text() { return selection_text_(&mPrimary); }

/*
 Remove the selected text.
 */
void Fl_Text_Buffer::remove_selection() { remove_selection_(&mPrimary); }
 ...
 */
char *Fl_Text_Buffer::highlight_text() { return selection_text_(&mHighlight); }
...
void Fl_Text_Buffer::remove_modify_callback(Fl_Text_Modify_Cb bufModifiedCB,
    ...
    if (toRemove == -1) {
        Fl::error(
            "Fl_Text_Buffer::remove_modify_callback(): Can't find modify CB to "
            "remove");
        return;
    }
...
void Fl_Text_Buffer::remove_predelete_callback(
    ...
    if (toRemove == -1) {
        Fl::error(
            "Fl_Text_Buffer::remove_predelete_callback(): Can't find "
            "pre-delete CB to remove");
        return;
    }

    /* Allocate new lists for remaining callback procs and args (if any are
     * left) */
    mNPredeleteProcs--;
    if (mNPredeleteProcs == 0) {
        delete[] mPredeleteProcs;
...
int Fl_Text_Buffer::word_start(int pos) const {
    ...
    // FIXME: character is ucs-4
    if (!(isalnum(char_at(pos)) || char_at(pos) == '_')) pos = next_char(pos);
    return pos;
}
    ...
    if (matchCase) {
        while (startPos < length()) {
            ...
            for (;;) {
                ...
                int l = fl_utf8len1(c);
                if (memcmp(sp, address(bp), l)) break;
                sp += l;
                bp += l;
            }
...
void Fl_Text_Buffer::remove_(int start, int end) {
    ...
    if (start > mGapStart) {
        ...
        if (mCanUndo) memcpy(undobuffer, mBuf + start, end - start);
        move_gap(end);
    } else {
...
void Fl_Text_Buffer::remove_selection_(Fl_Text_Selection *sel) {
    ...
    if (!sel->position(&start, &end)) return;
    remove(start, end);
    // undoyankcut = undocut;
}
...
/*
 ...
 Unicode safe.
 */
void Fl_Text_Buffer::call_modify_callbacks(int pos, int nDeleted, int nInserted,
                                           int nRestyled,
                                           const char *deletedText) const {
    IS_UTF8_ALIGNED2(this, pos)
    for (int i = 0; i < mNModifyProcs; i++)
        (*mModifyProcs[i])(pos, nInserted, nDeleted, nRestyled, deletedText,
                           mCbArgs[i]);
}
...
/*
 ...
 Unicode safe.
 */
void Fl_Text_Buffer::redisplay_selection(
    Fl_Text_Selection *oldSelection, Fl_Text_Selection *newSelection) const {
    ...
    if (!oldSelection->mSelected && !newSelection->mSelected) return;
...
void Fl_Text_Buffer::reallocate_with_gap(int newGapStart, int newGapLen) {
    ...
    if (newGapStart <= mGapStart) {
        ...
        memcpy(&newBuf[newGapEnd], &mBuf[newGapStart], mGapStart - newGapStart);
        memcpy(&newBuf[newGapEnd + mGapStart - newGapStart], &mBuf[mGapEnd],
               mLength - mGapStart);
    } else { /* newGapStart > mGapStart */
        memcpy(newBuf, mBuf, mGapStart);
        memcpy(&newBuf[mGapStart], &mBuf[mGapEnd], newGapStart - mGapStart);
        memcpy(&newBuf[newGapEnd], &mBuf[mGapEnd + newGapStart - mGapStart],
               mLength - newGapStart);
    }
    free((void *)mBuf);
    mBuf = newBuf;
...
/*
 ...
 Unicode safe. Pos must be at a character boundary.
 */
void Fl_Text_Buffer::update_selections(int pos, int nDeleted, int nInserted) {
...
int Fl_Text_Buffer::findchar_backward(int startPos, unsigned int searchChar,
    ...
    if (startPos > mLength) startPos = mLength;
...
static int general_input_filter(char *buffer, int buflen, char *line, int sline,
                                char *&endline, unsigned (*p_trf)(char *&),
                                FILE *fp) {
 ...
 *input_was_changed returns true if input was not strict UTF-8, so output
 differs from input.
 */
static int utf8_input_filter(
    char *buffer,    // result buffer we fill with utf8 encoded text
    ...
    int *input_was_changed)  // returned flag: 'true' if buffer[] different from
                             // file due to utf8 encoding
{
    ...
    while (q < buffer + buflen) {
        if (p >= endline) {  // walked off end of input file's line buffer?
            r = (int)fread(line, 1, sline,
                           fp);  // read another block of sline bytes from file
            endline = line + r;
            if (r == 0)
                return (int)(q -
                             buffer);  // EOF? return bytes read into buffer[]
            p = line;
        }
        // Predict length of utf8 sequence
        //    See if utf8 seq we're working on would extend off end of line
        //    buffer, and if so, adjust + load more data so that it doesn't.
        //
        l = fl_utf8len1(*p);    // anticipate length of utf8 sequence
        if (p + l > endline) {  // would walk off end of line buffer?
            memmove(line, p,
                    endline - p);  // re-jigger line buffer to get some room
            endline -= (p - line);
            r = (int)fread(endline, 1, sline - (endline - line),
                           fp);  // re-fill line buffer
            endline += r;
            p = line;
            if (endline - line < l)
        ...
        while (l > 0) {
            u = fl_utf8decode(
                p, p + l,
                &lp);  // get single utf8 encoded char as a Unicode value
            lq = fl_utf8encode(
                u,
                multibyte);  // re-encode Unicode value to utf8 in multibyte[]
            if (lp != l || lq != l) *input_was_changed = true;

            if (q + lq >
                buffer + buflen) {  // encoding would walk off end of buffer[]?
                memmove(line, p,
                        endline - p);  // re-jigger line[] buffer for next call
                endline -=
                    (p - line);  // adjust end of line[] buffer for next call
                return (int)(q - buffer);  // return what's decoded so far,
                                           // caller will consume buffer
            }
            memcpy(q, multibyte, lq);
            q += lq;
            p += lp;
...
int Fl_Text_Buffer::insertfile(const char *file, int pos, int buflen) {
    ...
    while (true) {
        ...
        l = general_input_filter(
            buffer, buflen, line, sizeof(line), endline,
            utf16toucs,  // use cp1252toucs to read CP1252-encoded files
            fp);
        input_file_was_transcoded = true;
#else
        l = utf8_input_filter(buffer, buflen, line, sizeof(line), endline, fp,
 ...
 */
int Fl_Text_Buffer::next_char_clipped(int pos) const { return next_char(pos); }
