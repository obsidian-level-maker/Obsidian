...
/* from fl_utf.c */
extern unsigned fl_utf8toUtf16(const char *src, unsigned srclen,
...
#define HAS_ATSU \
    (!__LP64__) && MAC_OS_X_VERSION_MAX_ALLOWED < MAC_OS_X_VERSION_10_11
...
Fl_Font_Descriptor::Fl_Font_Descriptor(const char *name, Fl_Fontsize Size) {
    ...
    if (fl_mac_os_version >= Fl_X::CoreText_threshold) {
        ...
        CTFontGetAdvancesForGlyphs(fontref, kCTFontHorizontalOrientation, glyph,
                                   advances, 2);
        w = advances[0].width;
        if (fabs(advances[0].width - advances[1].width) <
            ...
            w = CTFontGetAdvancesForGlyphs(
                fontref, kCTFontHorizontalOrientation, glyph, NULL, 1);
        }
        CFRelease(str);
        ...
        if (!attributes) {
            ...
            // character widths which allows fast fl_width() implementation.
        ...
        if (ascent == 0) {  // this may happen with some third party fonts
            ...
            CFAttributedStringRef mastr = CFAttributedStringCreate(
                kCFAllocatorDefault, CFSTR("Wj"), attributes);
        ...
        UniChar mTxt[2] = {65, 0};
        err = ATSUSetTextPointerLocation(layout, mTxt, kATSUFromTextBeginning,
                                         1, 1);
        err = ATSUCreateStyle(&style);
        err = ATSUSetRunStyle(layout, style, kATSUFromTextBeginning,
                              kATSUToTextEnd);
        // now set the actual font, size and attributes. We also set the font
        // matrix to render our font up-side-down, so when rendered through our
        // inverted CGContext, text will appear normal again.
        Fixed fsize = IntToFixed(Size);
        ATSUFontID fontID;
        ATSUFindFontFromName(name, strlen(name), kFontFullName,
                             kFontMacintoshPlatform, kFontNoScriptCode,
                             kFontEnglishLanguage, &fontID);

        // draw the font upside-down... Compensate for fltk/OSX origin
        // differences
        ...
        if (fontID != kATSUInvalidFontID)
            err = ATSUSetAttributes(style, 1, sTag, sBytes,
                                    sAttr);  // set the font attribute
        err = ATSUSetAttributes(
            style, 2, sTag + 1, sBytes + 1,
            sAttr + 1);  // then the size and matrix attributes
                         // next, make sure that Quartz will only render at
                         // integer coordinates
        ATSLineLayoutOptions llo =
        ...
        err = ATSUSetLineControls(layout, kATSUFromTextBeginning, 1, aTag,
                                  aBytes, aAttr);
        // now we are finally ready to measure some letter to get the bounding
        // box
        Fixed bBefore, bAfter, bAscent, bDescent;
        err = ATSUGetUnjustifiedBounds(layout, kATSUFromTextBeginning, 1,
                                       &bBefore, &bAfter, &bAscent, &bDescent);
        // Requesting a certain height font on Mac does not guarantee that
        // ascent+descent equal the requested height. fl_height will reflect the
        // actual height that we got. The font "Apple Chancery" is a pretty
        // extreme example of overlapping letters.
        ...
        if (w) q_width = FixedToInt(bAfter);

#define ENABLE_TRANSIENT_FONTS 1

#ifdef ENABLE_TRANSIENT_FONTS
        // Now, by way of experiment, try enabling Transient Font Matching, this
        // will cause ATSU to find a suitable font to render any chars the
        // current font can't do...
...
static Fl_Fontdesc built_in_table_PS[] =
    {  // PostScript font names preferred when Mac OS ≥ 10.5
        {"ArialMT"},
        {"Arial-BoldMT"},
        {"Arial-ItalicMT"},
        {"Arial-BoldItalicMT"},
...
static Fl_Fontdesc built_in_table_full[] =
    {  // full font names used before 10.5
        {"Arial"},
        {"Arial Bold"},
        {"Arial Italic"},
        {"Arial Bold Italic"},
...
static UniChar *mac_Utf8_to_Utf16(const char *txt, int len, int *new_len) {
    unsigned wlen =
        fl_utf8toUtf16(txt, len, (unsigned short *)utfWbuf, utfWlen);
    if (wlen >= utfWlen) {
        utfWlen = wlen + 100;
...
#if MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_5
    return (fl_mac_os_version >= Fl_X::CoreText_threshold
                ? built_in_table_PS
                : built_in_table_full);
#else
...
static CGFloat surrogate_width(const UniChar *txt,
    ...
    if (!b) {  // the current font doesn't contain this char
        CFStringRef str =
            CFStringCreateWithCharactersNoCopy(NULL, txt, 2, kCFAllocatorNull);
    ...
    if (b)
        CTFontGetAdvancesForGlyphs(font2, kCTFontHorizontalOrientation, glyphs,
                                   &a, 1);
    else
        a.width = fl_fontsize->q_width;
    if (must_release) CFRelease(font2);
    return a.width;
}

static CGFloat variation_selector_width(CFStringRef str16,
                                        Fl_Font_Descriptor *fl_fontsize) {
    CGFloat retval;
...
static double fl_mac_width(const UniChar *txt, int n,
                           Fl_Font_Descriptor *fl_fontsize) {
#if MAC_OS_X_VERSION_MAX_ALLOWED >= MAC_OS_X_VERSION_10_5
    if (fl_mac_os_version >= Fl_X::CoreText_threshold) {
        double retval = 0;
        ...
        for (i = 0; i < n; i++) {  // loop over txt
            uni = txt[i];
            if (uni >= 0xD800 &&
                uni <= 0xDBFF) {  // handles the surrogate range
                retval += surrogate_width(&txt[i], fl_fontsize);
                i++;  // because a pair of UniChar's represent a single
                      // character
                continue;
            }
            if (i + 1 < n && txt[i + 1] >= 0xFE00 &&
                txt[i + 1] <= 0xFE0F) {  // handles variation selectors
                CFStringRef substr =
                    CFStringCreateWithCharacters(NULL, txt + i, 2);
                retval += variation_selector_width(substr, fl_fontsize);
                CFRelease(substr);
                i++;
                continue;
            }
            const int block = 0x10000 / (sizeof(fl_fontsize->width) /
            ...
            if (!fl_fontsize
                     ->width[r]) {  // this character block has not been hit yet
                ...
                for (int j = 0; j < block; j++) {  // loop over the block
                    ...
                    bool b = CTFontGetGlyphsForCharacters(fl_fontsize->fontref,
                                                          &ii, &glyph, 1);
                    if (b)
                        CTFontGetAdvancesForGlyphs(fl_fontsize->fontref,
            ...
            if (wdt == -1e9) {
                ...
                if (!b) {  // the current font doesn't contain this char
                    CFStringRef str = CFStringCreateWithCharactersNoCopy(
                        NULL, &uni, 1, kCFAllocatorNull);
                    // find a font that contains it
                    font2 =
                        CTFontCreateForString(font2, str, CFRangeMake(0, 1));
                    must_release = true;
                    CFRelease(str);
                ...
                if (b)
                    ...
                    advance_size.width = 0.;
                // the width of the 'uni' character
                wdt = fl_fontsize->width[r][uni & (block - 1)] =
                    advance_size.width;
                if (must_release) CFRelease(font2);
            }
            retval += wdt;
        ...
        // Thing now collect our ATSU resources and measure our text string
        ...
        err = ATSUSetTextPointerLocation(layout, txt, kATSUFromTextBeginning, n,
                                         n);
        err = ATSUGetUnjustifiedBounds(layout, kATSUFromTextBeginning, n,
                                       &bBefore, &bAfter, &bAscent, &bDescent);
...
void Fl_Quartz_Graphics_Driver::text_extents(const char *str8, int n, int &dx,
                                             int &dy, int &w, int &h) {
    if (!font_descriptor()) font(FL_HELVETICA, FL_NORMAL_SIZE);
    Fl_Font_Descriptor *fl_fontsize = font_descriptor();
    ...
    if (fl_mac_os_version >= Fl_X::CoreText_threshold) {
        CFStringRef str16 =
            CFStringCreateWithCharactersNoCopy(NULL, txt, n, kCFAllocatorNull);
        CFDictionarySetValue(attributes, kCTFontAttributeName,
                             fl_fontsize->fontref);
        CFAttributedStringRef mastr =
            CFAttributedStringCreate(kCFAllocatorDefault, str16, attributes);
        ...
        err = ATSUSetTextPointerLocation(layout, txt, kATSUFromTextBeginning, n,
                                         n);
        Rect bbox;
        err = ATSUMeasureTextImage(layout, kATSUFromTextBeginning, n, 0, 0,
                                   &bbox);
...
static void fl_mac_draw(const char *str, int n, float x, float y,
    ...
    if (fl_mac_os_version >= Fl_X::CoreText_threshold) {
        ...
        CFDictionarySetValue(attributes, kCTFontAttributeName,
                             driver->font_descriptor()->fontref);
        ...
        err = ATSUSetTextPointerLocation(layout, uniStr, kATSUFromTextBeginning,
                                         n, n);
        CGContextSetShouldAntialias(fl_gc, true);
