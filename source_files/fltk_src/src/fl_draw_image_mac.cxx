...
static void innards(const uchar *buf, int X, int Y, int W, int H, int delta,
    ...
    // the image data must be kept until the page is closed. Thus tmpBuf can't
    // be deleted here. It's too early.
    CGDataProviderRef src = CGDataProviderCreateWithData(
        0L, array, abs(linedelta) * H, tmpBuf ? dataReleaseCB : NULL);
    CGImageRef img = CGImageCreate(
        W, H, 8, 8 * abs(delta), abs(linedelta), lut,
        abs(delta) & 1 ? kCGImageAlphaNone : kCGImageAlphaNoneSkipLast,
    ...
    // following the very save (and very slow) way to write the image into the
    // give port
...
void Fl_Quartz_Graphics_Driver::draw_image(const uchar *buf, int x, int y,
...
void Fl_Quartz_Graphics_Driver::draw_image_mono(const uchar *buf, int x, int y,
                                                int w, int h, int d, int l) {
