...
int Fl::add_awake_handler_(Fl_Awake_Handler func, void *data) {
    ...
    if (!awake_ring_) {
        ...
        awake_ring_ = (Fl_Awake_Handler *)malloc(awake_ring_size_ *
...
/**
 ...
 function. Registers a function that will be called by the main thread during
 the next message handling cycle. Returns 0 if the callback function was
 registered, and -1 if registration failed. Over a thousand awake callbacks can
...
void unlock_ring() { LeaveCriticalSection(cs_ring); }
...
static void unlock_function() { LeaveCriticalSection(&cs); }

//
// 'lock_function()' - Get the lock.
//

static void lock_function() { EnterCriticalSection(&cs); }
...
void Fl::unlock() { unlock_function(); }
...
static void lock_function_init_std() { pthread_mutex_init(&fltk_mutex, NULL); }
...
static void lock_function_rec() { pthread_mutex_lock(&fltk_mutex); }

static void unlock_function_rec() { pthread_mutex_unlock(&fltk_mutex); }
...
void Fl::unlock() { fl_unlock_function(); }

// Mutex code for the awake ring buffer
static pthread_mutex_t *ring_mutex;

void unlock_ring() { pthread_mutex_unlock(ring_mutex); }
...
int Fl::lock() { return 1; }

void Fl::unlock() {}

void *Fl::thread_message() { return NULL; }
