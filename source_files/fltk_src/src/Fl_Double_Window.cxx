...
static int can_xdbe() {
    ...
    if (!tried) {
        ...
        XdbeScreenVisualInfo *a =
            XdbeGetVisualInfo(fl_display, &root, &numscreens);
        if (!a) return 0;
...
/** Copy a rectangular area of the given offscreen buffer into the current
 drawing destination. \param x,y	position where to draw the copied
 rectangle \param w,h	size of the copied rectangle \param pixmap  offscreen
 buffer containing the rectangle to copy \param srcx,srcy origin in offscreen
 buffer of rectangle to copy
 */
#if FLTK_ABI_VERSION >= 10301
void fl_copy_offscreen(int x, int y, int w, int h, Fl_Offscreen pixmap,
                       int srcx, int srcy) {
...
#endif
#ifdef USE_X11
        ((Fl_Xlib_Graphics_Driver *)fl_graphics_driver)
            ->copy_offscreen(x, y, w, h, pixmap, srcx, srcy);
#elif defined(WIN32)
        ((Fl_GDI_Graphics_Driver *)fl_graphics_driver)
            ->copy_offscreen(x, y, w, h, pixmap, srcx, srcy);
#elif defined(__APPLE__)
    ((Fl_Quartz_Graphics_Driver *)fl_graphics_driver)
        ->copy_offscreen(x, y, w, h, pixmap, srcx, srcy);
#endif
...
void Fl_Xlib_Graphics_Driver::copy_offscreen_with_alpha(int x, int y, int w,
                                                        int h,
                                                        Fl_Offscreen pixmap,
                                                        int srcx, int srcy) {
#if HAVE_XRENDER
    ...
    static XRenderPictFormat *srcfmt =
        XRenderFindStandardFormat(fl_display, PictStandardARGB32);
    static XRenderPictFormat *dstfmt =
        XRenderFindStandardFormat(fl_display, PictStandardRGB24);

    Picture src = XRenderCreatePicture(fl_display, pixmap, srcfmt, 0, &srcattr);
    Picture dst =
        XRenderCreatePicture(fl_display, fl_window, dstfmt, 0, &srcattr);
    ...
    if (clipr) XRenderSetPictureClipRegion(fl_display, dst, clipr);
...
char fl_can_do_alpha_blending() { return Fl_X::xrender_supported(); }
#elif defined(WIN32)
...
typedef BOOL(WINAPI *fl_alpha_blend_func)(HDC, int, int, int, int, HDC, int,
                                          int, int, int, FL_BLENDFUNCTION);
...
char fl_can_do_alpha_blending() {
    ...
    BOOL alpha_ok =
        fl_alpha_blend(dc, 0, 0, 1, 1, new_gc, 0, 0, 1, 1, blendfunc);
...
void Fl_GDI_Graphics_Driver::copy_offscreen(int x, int y, int w, int h,
                                            HBITMAP bitmap, int srcx,
                                            int srcy) {
    HDC new_gc = CreateCompatibleDC(fl_gc);
    int save = SaveDC(new_gc);
    SelectObject(new_gc, bitmap);
...
void Fl_GDI_Graphics_Driver::copy_offscreen_with_alpha(int x, int y, int w,
...
char fl_can_do_alpha_blending() { return 1; }

#if !defined(FL_DOXYGEN)
Fl_Offscreen Fl_Quartz_Graphics_Driver::create_offscreen_with_alpha(int w,
                                                                    int h) {
    void *data = calloc(w * h, 4);
    CGColorSpaceRef lut = CGColorSpaceCreateDeviceRGB();
    CGContextRef ctx = CGBitmapContextCreate(data, w, h, 8, w * 4, lut,
...
void Fl_Quartz_Graphics_Driver::copy_offscreen(int x, int y, int w, int h,
                                               Fl_Offscreen osrc, int srcx,
                                               int srcy) {
    ...
    CGColorSpaceRef lut = CGColorSpaceCreateDeviceRGB();
    // when output goes to a Quartz printercontext, release of the bitmap must
    // be delayed after the end of the print page
    CFRetain(src);
    CGDataProviderRef src_bytes =
        CGDataProviderCreateWithData(src, data, sw * sh * 4, bmProviderRelease);
    CGImageRef img =
        CGImageCreate(sw, sh, 8, 4 * 8, 4 * sw, lut, alpha, src_bytes, 0L,
...
void fl_end_offscreen() {
    ...
    CGContextRestoreGState(
        fl_gc);  // matches CGContextSaveGState in fl_begin_offscreen()
...
#if USE_XDBE
        if (can_xdbe()) {
            myi->other_xid = XdbeAllocateBackBufferName(
                fl_display, fl_xid(this), XdbeCopied);
...
#error unsupported platform
    ...
    if (use_xdbe) {
        ...
        // capture gc changes automatically to update the cairo context
        // adequately # endif
#elif defined(__APPLE__)
        if (myi->other_xid) {
...
void Fl_Double_Window::resize(int X, int Y, int W, int H) {
    ...
    if (use_xdbe) {
        ...
        if (myi && myi->other_xid && (ow < w() || oh < h())) {
            // STR #2152: Deallocate the back buffer to force creation of a new
            // one.
