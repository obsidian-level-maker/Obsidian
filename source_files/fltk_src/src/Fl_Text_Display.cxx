...
*/
int Fl_Text_Display::linenumber_width() const { return mLineNumWidth; }

/**
 Set the font used for line numbers (if enabled).
 \version 1.3.3 ABI feature (ignored in 1.3.x unless FLTK_ABI_VERSION is 10303
 or higher)
*/
...
void Fl_Text_Display::resize(int X, int Y, int W, int H) {
    ...
    printf("            box_d* size(x=%d, y=%d, w=%d, h=%d)\n",
           Fl::box_dx(box()), Fl::box_dy(box()), Fl::box_dw(box()),
           Fl::box_dh(box()));
    printf("         text_area size(x=%d, y=%d, w=%d, h=%d)\n", text_area.x,
           text_area.y, text_area.w, text_area.h);
    ...
    for (i = 0, mMaxsize = fl_height(textfont(), textsize()); i < mNStyles; i++)
        mMaxsize =
            max(mMaxsize, fl_height(mStyleTable[i].font, mStyleTable[i].size));
    ...
    for (int again = 1; again;) {
        again = 0;
        /* In continuous wrap mode, a change in width affects the total number
         of lines in the buffer, and can leave the top line number incorrect,
         and the top character no longer pointing at a valid line start */
        ...
        if (scrollbar_width()) {
            ...
            /*
             ...
             wraps at the right edge, do not draw a horizontal scrollbar.
             Otherwise, if there is a vertical scrollbar, a horizontal is always
             created too.  This is because the alternatives are unattractive:
             * Dynamically creating a horizontal scrollbar based on the
             currently visible lines is what the original nedit does, but it
             always wastes space for the scrollbar even when it's not used.
             Since the FLTK widget dynamically allocates the space for the
             scrollbar and rearranges the widget to make room for it, this would
             create a very visually displeasing "bounce" effect when the
             vertical scrollbar is dragged.  Trust me, I tried it and it looks
             really bad.
            ...
            if (!mHScrollBar->visible() &&
                ...
                char wrap_at_bounds =
                    mContinuousWrap && (mWrapMarginPix < text_area.w);
                if (!wrap_at_bounds) {
                    mHScrollBar->set_visible();
                    text_area.h -= scrollbar_width();
                    again = 1;  // loop again to see if we now need vert. &
                                // recalc sizes
                }
            }
        }
    ...
    text_area.y = Y + TOP_MARGIN;
    if (mHScrollBar->visible() && scrollbar_align() & FL_ALIGN_TOP)
...
#endif
                                text_area.y - TOP_MARGIN, scrollbar_width(),
    ...
    if (mHScrollBar->visible()) {
        if (scrollbar_align() & FL_ALIGN_TOP) {
            ...
            mHScrollBar->resize(
                text_area.x - LEFT_MARGIN, Y + H - scrollbar_width(),
...
void Fl_Text_Display::redisplay_range(int startpos, int endpos) {
    ...
    } else if ((startpos >= damage_range1_start &&
                startpos <= damage_range1_end) ||
...
void Fl_Text_Display::draw_range(int startpos, int endpos) {
    ...
    startIndex =
        mLineStarts[startLine] == -1 ? 0 : startpos - mLineStarts[startLine];
    if (endpos >= mLastChar)
        endIndex = INT_MAX;
...
/**
 ...
 If \p wrap mode is not zero, this call enables automatic word wrapping at
 column \p wrapMargin. Word-wrapping does not change the text buffer itself,
 only the way the text is displayed. Different Text Displays can have different
 wrap modes, even if they share the same Text Buffer.
 ...
 width or average character width.
 */
...
void Fl_Text_Display::overstrike(const char *text) {
    ...
    mCursorToHint = NO_HINT;
    if (paddedText != NULL) delete[] paddedText;
}
...
int Fl_Text_Display::position_to_xy(int pos, int *X, int *Y) const {
    ...
    *X = text_area.x +
         handle_vline(GET_WIDTH, lineStartPos, pos - lineStartPos, 0, 0, 0, 0,
                      0, 0) -
         mHorizOffset;
    return 1;
}
 ...
 */
int Fl_Text_Display::position_to_linecol(int pos, int *lineNum,
                                         int *column) const {
    IS_UTF8_ALIGNED2(buffer(), pos)
    ...
    if (mContinuousWrap) {
        ...
        *column = buffer()->count_displayed_characters(
            buffer()->line_start(pos), pos);
        return 1;
    }
    ...
    if (retVal) {
        *column =
            mBuffer->count_displayed_characters(mLineStarts[*lineNum], pos);
        *lineNum += mTopLineNum;
    }
    return retVal;
}
...
int Fl_Text_Display::wrapped_column(int row, int column) const {
    ...
    if (!mContinuousWrap || row < 0 || row > mNVisibleLines) return column;
...
int Fl_Text_Display::wrapped_row(int row) const {
    if (!mContinuousWrap || row < 0 || row > mNVisibleLines) return row;
...
int Fl_Text_Display::move_up() {
    ...
    newPos = handle_vline(FIND_INDEX_FROM_ZERO, prevLineStartPos,
                          lineEnd - prevLineStartPos, 0, 0, 0, 0, 0, xPos);
...
int Fl_Text_Display::move_down() {
    ...
    newPos = handle_vline(FIND_INDEX_FROM_ZERO, nextLineStartPos,
                          lineEnd - nextLineStartPos, 0, 0, 0, 0, 0, xPos);
...
#ifdef DEBUG
    printf(
        "Fl_Text_Display::count_lines(startPos=%d, endPos=%d, "
        "startPosIsLineStart=%d\n",
        startPos, endPos, startPosIsLineStart);
#endif  // DEBUG

    /* If we're not wrapping use simple (and more efficient)
     * Fl_Text_Buffer::count_lines() */
    if (!mContinuousWrap) return buffer()->count_lines(startPos, endPos);
...
#ifdef DEBUG
    printf(
        "   # after WLC: retPos=%d, retLines=%d, retLineStart=%d, "
        "retLineEnd=%d\n",
...
int Fl_Text_Display::skip_lines(int startPos, int nLines,
    ...
    if (!mContinuousWrap) return buffer()->skip_lines(startPos, nLines);
    ...
    wrapped_line_counter(buffer(), startPos, buffer()->length(), nLines,
                         startPosIsLineStart, 0, &retPos, &retLines,
                         &retLineStart, &retLineEnd);
...
int Fl_Text_Display::line_end(int startPos, bool startPosIsLineStart) const {
    ...
    if (!mContinuousWrap) return buffer()->line_end(startPos);

    if (startPos == buffer()->length()) return startPos;
...
int Fl_Text_Display::line_start(int pos) const {
    ...
    if (!mContinuousWrap) return buffer()->line_start(pos);

    wrapped_line_counter(buffer(), buffer()->line_start(pos), pos, INT_MAX,
                         true, 0, &retPos, &retLines, &retLineStart,
                         &retLineEnd);
...
int Fl_Text_Display::rewind_lines(int startPos, int nLines) {
    ...
    if (!mContinuousWrap) return buf->rewind_lines(startPos, nLines);

    pos = startPos;
    for (;;) {
        lineStart = buf->line_start(pos);
        wrapped_line_counter(buf, lineStart, pos, INT_MAX, true, 0, &retPos,
                             &retLines, &retLineStart, &retLineEnd, false);
        if (retLines > nLines)
...
void Fl_Text_Display::next_word() {
    ...
    while (pos < buffer()->length() &&
           !fl_isseparator(buffer()->char_at(pos))) {
...
/**
 ...
 \param nRestyled ??
 \param deletedText this is what was removed, must not be NULL if nDeleted is
 set \param cbArg "this" pointer for static callback function
 */
void Fl_Text_Display::buffer_modified_cb(int pos, int nInserted, int nDeleted,
    ...
    if (textD->mContinuousWrap) {
        ...
        linesInserted =
            nInserted == 0 ? 0 : buf->count_lines(pos, pos + nInserted);
    ...
    if (nInserted != 0 || nDeleted != 0) {
        if (textD->mContinuousWrap) {
            textD->update_line_starts(wrapModStart, wrapModEnd - wrapModStart,
                                      nDeleted + pos - wrapModStart +
                                          (wrapModEnd - (pos + nInserted)),
    ...
    if (linesInserted == linesDeleted) {
        ...
        if (linesInserted > 1) {
            // textD->draw_line_numbers(false); // can't do this b/c not called
            // from virtual draw();
            textD->damage(::FL_DAMAGE_EXPOSE);
        }
    } else {
        ...
        // textD->draw_line_numbers(true);    // no, can't call this here, not
        // in draw() context -- ERCO / STR#2621
        // textD->damage(::FL_DAMAGE_EXPOSE);
    }
    IS_UTF8_ALIGNED2(buf, startDispPos)
...
int Fl_Text_Display::get_absolute_top_line_number() const {
    if (!mContinuousWrap) return mTopLineNum;
    if (maintaining_absolute_top_line_number()) return mAbsTopLineNum;
    return 0;
}
...
int Fl_Text_Display::maintaining_absolute_top_line_number() const {
    return mContinuousWrap && (mLineNumWidth != 0 || mNeedAbsTopLineNum);
}
        ...
        if (empty_vlines()) {
            if (mLastChar < mBuffer->length()) {
                if (!position_to_line(mLastChar, lineNum)) {
                    Fl::error(
                        "Fl_Text_Display::position_to_line(): Consistency "
                        "check ptvl failed");
                    return 0;
                }
                return ++(*lineNum) <= mNVisibleLines - 1;
            } else {
                position_to_line(buffer()->prev_char_clipped(mLastChar),
                                 lineNum);
                return 1;
            }
        }
...
/**
 ...
 \retval FIND_INDEX_FROM_ZERO index of character at given x position without
 scrolling and widget offsets \todo we need to handle hidden hyphens and tabs
 here! \todo we handle all styles and selections \todo we must provide code to
 get pixel positions of the middle of a character as well
 */
int Fl_Text_Display::handle_vline(int mode, int lineStartPos, int lineLen,
                                  int leftChar, int rightChar, int Y,
                                  int bottomClip, int leftClip,
                                  int rightClip) const {
    ...
    for (i = 0; i < lineLen;) {
        currChar =
            lineStr[i];  // one byte is enough to handele tabs and other cases
        ...
        if (charStyle != style || currChar == '\t' || prevChar == '\t') {
            ...
            if (prevChar == '\t') {
                ...
                int xAbs = (mode == GET_WIDTH)
                               ? startX
                               : startX + mHorizOffset - text_area.x;
                w = (((xAbs / tab) + 1) * tab) - xAbs;
                if (mode == DRAW_LINE)
                    draw_string(style | BG_ONLY_MASK, startX, Y, startX + w, 0,
                                0);
                ...
                if (mode == DRAW_LINE)
                    draw_string(style, startX, Y, startX + w,
    ...
    if (currChar == '\t') {
        ...
        int tab = (int)col_to_x(mBuffer->tab_distance());
        int xAbs =
            (mode == GET_WIDTH) ? startX : startX + mHorizOffset - text_area.x;
        w = (((xAbs / tab) + 1) * tab) - xAbs;
        if (mode == DRAW_LINE)
...
/**
 ...
 \param leftCharIndex, rightCharIndex index into line of segment that we want to
 draw
 */
void Fl_Text_Display::draw_vline(int visLineNum, int leftClip, int rightClip,
    ...
    handle_vline(DRAW_LINE, lineStartPos, lineLen, leftCharIndex,
                 rightCharIndex, Y, Y + fontHeight, leftClip, rightClip);
    return;
}
...
void Fl_Text_Display::draw_string(int style, int X, int Y, int toX,
    ...
    if (style & STYLE_LOOKUP_MASK) {
        ...
        if (style & PRIMARY_MASK) {
            ...
            background = color();
        foreground = (style & PRIMARY_MASK)
                         ? fl_contrast(styleRec->color, background)
                         : styleRec->color;
    ...
    if (!(style & BG_ONLY_MASK)) {
        ...
        if (Fl::compose_state && (style & PRIMARY_MASK)) {
            ...
            fl_line(X, Y + mMaxsize - 1, X + fl_width(string, nChars),
                    Y + mMaxsize - 1);
        }
#endif
#if !(defined(__APPLE__) || defined(WIN32)) && USE_XFT
 ...
 */
void Fl_Text_Display::clear_rect(int style, int X, int Y, int width,
                                 int height) const {
...
void Fl_Text_Display::draw_cursor(int X, int Y) {
    ...
    if (X < text_area.x - 1 || X > text_area.x + text_area.w) return;
 ...
 */
int Fl_Text_Display::position_style(int lineStartPos, int lineLen,
                                    int lineIndex) const {
    IS_UTF8_ALIGNED2(buffer(), lineStartPos)
    ...
    if (buf->primary_selection()->includes(pos)) style |= PRIMARY_MASK;
    if (buf->highlight_selection()->includes(pos)) style |= HIGHLIGHT_MASK;
    if (buf->secondary_selection()->includes(pos)) style |= SECONDARY_MASK;
    return style;
}
 ...
 */
double Fl_Text_Display::string_width(const char *string, int length,
                                     int style) const {
    IS_UTF8_ALIGNED(string)
...
int Fl_Text_Display::xy_to_position(int X, int Y, int posType) const {
    ...
    if (visLineNum < 0) return mFirstChar;
    if (visLineNum >= mNVisibleLines) visLineNum = mNVisibleLines - 1;
    ...
    if (lineStart == -1) return mBuffer->length();

    /* Get the line text and its length */
    lineLen = vline_length(visLineNum);

    return handle_vline(FIND_INDEX, lineStart, lineLen, 0, 0, 0, 0, text_area.x,
                        X);
}
...
/**
 ...
 \param posType CURSOR_POS or CHARACTER_POS
 */
void Fl_Text_Display::xy_to_rowcol(int X, int Y, int *row, int *column,
                                   int posType) const {
    int fontHeight = mMaxsize;
    ...
    *column = ((X - text_area.x) + mHorizOffset +
               (posType == CURSOR_POS ? fontWidth / 2 : 0)) /
              fontWidth;
...
void Fl_Text_Display::offset_line_starts(int newTopLineNum) {
    ...
    if (newTopLineNum < oldTopLineNum && newTopLineNum < -lineDelta) {
        ...
        mFirstChar =
            rewind_lines(buf->length(), mNBufferLines - newTopLineNum + 1);
    }
...
void Fl_Text_Display::update_line_starts(int pos, int charsInserted,
    ...
    if (pos < mFirstChar) {
        ...
        if (position_to_line(pos + charsDeleted, &lineOfEnd) &&
            ...
            mFirstChar =
                rewind_lines(lineStarts[lineOfEnd] + charDelta, lineOfEnd);
    ...
    if (pos <= mLastChar) {
        ...
        if (lineDelta < 0) calc_line_starts(nVisLines + lineDelta, nVisLines);
        /* calculate lastChar by finding the end of the last displayed line */
        calc_last_char();
        *scrolled = 0;
        return;
...
void Fl_Text_Display::calc_line_starts(int startLine, int endLine) {
    ...
    for (line = startLine; line <= endLine; line++) {
        ...
        if (startPos >= bufLen) {
            ...
            if (line == 0 ||
                (lineStarts[line - 1] != bufLen && lineEnd != nextLineStart)) {
...
int Fl_Text_Display::scroll_(int topLineNum, int horizOffset) {
    ...
    if (mHorizOffset == horizOffset && mTopLineNum == topLineNum) return 0;
...
/**
 ...
 \param clearAll -- (currently unused) If False, only draws the line number
 text, does not clear the area behind it. If True, clears the area and redraws
 the text. Use False to avoid a 'flash' for single buffered windows.
 */
...
void Fl_Text_Display::draw_line_numbers(bool /*clearAll*/) {
    ...
    if (mLineNumWidth <= 0 || !visible_r()) return;
    ...
    int yoff = Fl::box_dy(box()) +
               ((scrollbar_align() & FL_ALIGN_TOP) ? hscroll_h : 0);

#ifndef LINENUM_LEFT_OF_VSCROLL
    int vscroll_w = mVScrollBar->visible() ? mVScrollBar->w() : 0;
    if (scrollbar_align() & FL_ALIGN_LEFT) xoff += vscroll_w;
#endif

    Fl_Color fgcolor =
        isactive ? linenumber_fgcolor() : fl_inactive(linenumber_fgcolor());
    Fl_Color bgcolor =
        isactive ? linenumber_bgcolor() : fl_inactive(linenumber_bgcolor());
    fl_push_clip(x() + xoff, y() + yoff, mLineNumWidth,
                 h() - Fl::box_dw(box()) - hscroll_h);
    {
        ...
        for (visLine = 0; visLine < mNVisibleLines; visLine++) {
            ...
            if (lineStart != -1 &&
                ...
                int xx = x() + xoff + 3, yy = Y + 3,
                    ww = mLineNumWidth - (3 * 2), hh = lineHeight;
                fl_draw(lineNumString, xx, yy, ww, hh, linenumber_align(), 0,
                        0);
...
/**
 \brief Returns the width in pixels of the displayed line pointed to by
 "visLineNum". \param visLineNum index into visible lines array \return width of
 line in pixels
 */
...
/**
 \brief Return true if there are lines visible with no corresponding buffer
 text. \return 1 if there are empty lines
 */
...
int Fl_Text_Display::vline_length(int visLineNum) const {
    ...
    if (visLineNum < 0 || visLineNum >= mNVisibleLines) return (0);

    lineStartPos = mLineStarts[visLineNum];

    if (lineStartPos == -1) return 0;

    if (visLineNum + 1 >= mNVisibleLines) return mLastChar - lineStartPos;

    nextLineStart = mLineStarts[visLineNum + 1];
    if (nextLineStart == -1) return mLastChar - lineStartPos;
...
void Fl_Text_Display::find_wrap_range(const char *deletedText, int pos,
    ...
    *modRangeStart = countFrom;
    for (;;) {
        ...
        wrapped_line_counter(buf, lineStart, buf->length(), 1, true, 0, &retPos,
                             &retLines, &retLineStart, &retLineEnd);
        ...
        if (lineStart <= pos) {
            ...
            if (visLineNum < nVisLines && lineStarts[visLineNum] == lineStart) {
                countFrom = lineStart;
                nLines = 0;
                if (visLineNum + 1 < nVisLines &&
                    lineStarts[visLineNum + 1] != -1)
                    *modRangeStart =
                        min(pos, buf->prev_char(lineStarts[visLineNum + 1]));
                else
                    *modRangeStart = countFrom;
            } else
...
void Fl_Text_Display::measure_deleted_lines(int pos, int nDeleted) {
    ...
    if (pos >= mFirstChar && pos <= mLastChar) {
        for (i = nVisLines - 1; i > 0; i--)
            if (lineStarts[i] != -1 && pos >= lineStarts[i]) break;
        if (i > 0) {
            countFrom = lineStarts[i - 1];
        } else
            countFrom = buf->line_start(pos);
...
/**
 ...
 \param[in] startPosIsLineStart  Flag indicating if startPos is start of line.
                                 (If set, prevents our having to find the line
 start) \param[in] styleBufOffset Offset index position into style buffer.
...
void Fl_Text_Display::wrapped_line_counter(
    Fl_Text_Buffer *buf, int startPos, int maxPos, int maxLines,
    ...
    for (p = lineStart; p < buf->length(); p = buf->next_char(p)) {
        ...
        if (c == '\n') {
            ...
            colNum++;
            // FIXME: it is not a good idea to simply add character widths
            // because on some platforms, the width is a floating point value
            // and depends on the previous character as well.
            width += measure_proportional_character(s, (int)width,
                                                    p + styleBufOffset);
        }

        /* If character exceeded wrap margin, find the break point and wrap
         * there */
        if (width > wrapMarginPix) {
            ...
            for (b = p; b >= lineStart; b = buf->prev_char(b)) {
                ...
                if (c == '\t' || c == ' ') {
                    ...
                    for (i = buf->next_char(b); i < iMax;
                         i = buf->next_char(i)) {
                        width += measure_proportional_character(
                            buf->address(i), (int)width, i + styleBufOffset);
                        colNum++;
                    }
                    foundBreak = true;
                    break;
            ...
            if (!foundBreak) { /* no whitespace, just break at margin */
                ...
                if (b >= buf->length()) {  // STR #2730
                    ...
                    width = measure_proportional_character(s, 0,
                                                           p + styleBufOffset);
                }
            }
            if (p >= maxPos) {
                ...
                *retLineStart =
                    maxPos < newLineStart ? lineStart : newLineStart;
                *retLineEnd = maxPos;
                return;
            }
            nLines++;
            if (nLines >= maxLines) {
                *retPos = foundBreak ? buf->next_char(b)
                                     : max(p, buf->next_char(lineStart));
                *retLines = nLines;
                *retLineStart = lineStart;
 ...
 */
double Fl_Text_Display::measure_proportional_character(const char *s, int xPix,
                                                       int pos) const {
    IS_UTF8_ALIGNED(s)
...
void Fl_Text_Display::find_line_end(int startPos, bool startPosIsLineStart,
    ...
    wrapped_line_counter(buffer(), startPos, buffer()->length(), 1,
                         startPosIsLineStart, 0, nextLineStart, &retLines,
...
int Fl_Text_Display::wrap_uses_character(int lineEndPos) const {
    ...
    if (!mContinuousWrap || lineEndPos == buffer()->length()) return 1;

    c = buffer()->char_at(lineEndPos);
    return c == '\n' ||
           ((c == '\t' || c == ' ') && lineEndPos + 1 < buffer()->length());
}
...
void Fl_Text_Display::extend_range_for_styles(int *startpos, int *endpos) {
    ...
    /* The peculiar protocol used here is that modifications to the style
     buffer are marked by selecting them with the buffer's primary
     Fl_Text_Selection. The style buffer is usually modified in response to a
     modify callback on the text buffer BEFORE Fl_Text_Display.c's modify
     callback, so that it can keep the style buffer in step with the text
     buffer.  The style-update callback can't just call for a redraw, because
     Fl_Text_Display hasn't processed the original text changes yet.  Anyhow, to
     minimize redrawing and to avoid the complexity of scheduling redraws later,
     this simple protocol tells the text display's buffer modify callback to
     extend its redraw range to show the text color/and font changes as well. */
    if (sel->selected()) {
        if (sel->start() < *startpos) {
            *startpos = sel->start();
            // somewhere while deleting, alignment is lost. We do this just to
            // be sure.
            *startpos = buffer()->utf8_align(*startpos);
            IS_UTF8_ALIGNED2(buffer(), (*startpos))
            extended = 1;
    ...
    /* If the Fl_Text_Selection was extended due to a style change, and some of
     the fonts don't match in spacing, extend redraw area to end of line to
     redraw characters exposed by possible font size changes */
    if (extended) *endpos = mBuffer->line_end(*endpos) + 1;
...
void Fl_Text_Display::draw(void) {
    ...
    if (damage() & FL_DAMAGE_ALL) {
        ...
        if (Fl_Surface_Device::surface() !=
            ...
            fl_rectf(text_area.x, text_area.y, text_area.w, text_area.h,
                     bgcolor);
        }
        // draw the box()
        int W = w(), H = h();
        ...
        // top margin
        fl_rectf(text_area.x, text_area.y - TOP_MARGIN, text_area.w, TOP_MARGIN,
                 bgcolor);

        // bottom margin
        fl_rectf(text_area.x, text_area.y + text_area.h, text_area.w,
                 BOTTOM_MARGIN, bgcolor);

        // draw that little box in the corner of the scrollbars
        if (mVScrollBar->visible() && mHScrollBar->visible())
            fl_rectf(mVScrollBar->x(), mHScrollBar->y(), mVScrollBar->w(),
                     mHScrollBar->h(), FL_GRAY);
        ...
        //    mCursorOldY);
        // CET - FIXME - save old cursor position instead and just draw side
        // needed?
        fl_push_clip(text_area.x - LEFT_MARGIN, text_area.y,
                     text_area.w + LEFT_MARGIN + RIGHT_MARGIN, text_area.h);
        fl_rectf(text_area.x - LEFT_MARGIN, mCursorOldY, LEFT_MARGIN, mMaxsize,
                 bgcolor);
        fl_rectf(text_area.x + text_area.w, mCursorOldY, RIGHT_MARGIN, mMaxsize,
                 bgcolor);
        fl_pop_clip();
    }
    ...
    if (damage() & (FL_DAMAGE_ALL | FL_DAMAGE_EXPOSE)) {
        ...
        int X, Y, W, H;
        if (fl_clip_box(text_area.x, text_area.y, text_area.w, text_area.h, X,
                        Y, W, H)) {
...
void Fl_Text_Display::scroll_timer_cb(void *user_data) {
    ...
    switch (scroll_direction) {
        case 1:  // mouse is to the right, scroll left
            ...
            pos = w->xy_to_position(w->text_area.x + w->text_area.w, scroll_y,
                                    CURSOR_POS);
            break;
        ...
        case 4:  // mouse is below, scroll up
            w->scroll(w->mTopLineNum + scroll_amount, w->mHorizOffset);
            pos = w->xy_to_position(scroll_x, w->text_area.y + w->text_area.h,
                                    CURSOR_POS);
            break;
        default:
            return;
...
int Fl_Text_Display::handle(int event) {
    ...
    switch (event) {
        ...
        case FL_DRAG: {
            if (dragType == DRAG_NONE) return 1;
        ...
        case FL_RELEASE: {
            if (Fl::event_is_click() && (!Fl::event_clicks()) &&
                ...
                buffer()->unselect();  // clicking in the selection: unselect
                                       // and move cursor
