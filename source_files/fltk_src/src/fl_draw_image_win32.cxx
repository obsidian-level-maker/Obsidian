...
static void innards(const uchar *buf, int X, int Y, int W, int H, int delta,
                    int linedelta, int depth, Fl_Draw_Image_Cb cb,
                    void *userdata) {
    char indexed = 0;
    ...
    if (depth == 0) depth = 3;
    if (indexed || !fl_can_do_alpha_blending()) depth = (depth - 1) | 1;
    ...
    if (!bmi.bmiHeader.biSize) {
        bmi.bmiHeader.biSize =
            sizeof(bmi) - 4;  // does it use this to determine type?
    ...
    if (indexed) {
        ...
        if (depth < 3) {
        RGBQUAD *bmi_colors =
            &bmi.bmiColors[0];  // suppress warning (STR #3199)
        for (int i = 0; i < 256; i++) {
    ...
    {
        int size = linesize * h;
        // when printing, don't limit buffer size not to get a crash in
        // StretchDIBits
    ...
    for (int j = 0; j < h;) {
        ...
        if (Fl_Surface_Device::surface() !=
            ...
            // SetDIBitsToDevice does not do the expected job, whereas
            // StretchDIBits does it.
...
void Fl_GDI_Graphics_Driver::draw_image(Fl_Draw_Image_Cb cb, void *data, int x,
