...
int Fl_Glut_Window::handle(int event) {
    ...
    switch (event) {
        ...
        case FL_MOUSEWHEEL:
            button = Fl::event_dy();
            while (button < 0) {
                if (mouse) mouse(3, GLUT_DOWN, ex, ey);
                ++button;
            }
            while (button > 0) {
                if (mouse) mouse(4, GLUT_DOWN, ex, ey);
                --button;
            }
            return 1;
        ...
        case FL_KEYBOARD:
            if (Fl::event_text()[0]) {
                if (keyboard) {
                    keyboard(Fl::event_text()[0], ex, ey);
                    return 1;
                }
                break;
            } else {
...
void Fl_Glut_Window::_init() {
    for (number = 1; number < MAXWINDOWS; number++)
        if (!windows[number]) break;
    windows[number] = this;
...
void glutInitDisplayMode(unsigned int mode) { glut_mode = mode; }
...
void glutPostWindowRedisplay(int win) { windows[win]->redraw(); }

void glutSetWindow(int win) { windows[win]->make_current(); }
...
int glutGet(GLenum type) {
    switch (type) {
        case GLUT_RETURN_ZERO:
            return 0;
        case GLUT_WINDOW_X:
            return glut_window->x();
        case GLUT_WINDOW_Y:
            return glut_window->y();
        case GLUT_WINDOW_WIDTH:
            return glut_window->pixel_w();
        case GLUT_WINDOW_HEIGHT:
            return glut_window->pixel_h();
...
int glutLayerGet(GLenum type) {
    switch (type) {
        case GLUT_OVERLAY_POSSIBLE:
            return glut_window->can_do_overlay();
            // case GLUT_LAYER_IN_USE:
            // case GLUT_HAS_OVERLAY:
        case GLUT_TRANSPARENT_INDEX:
            return 0;  // true for SGI
        case GLUT_NORMAL_DAMAGED:
            return glut_window->damage();
