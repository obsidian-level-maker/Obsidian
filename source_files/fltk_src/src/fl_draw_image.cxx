...
static void color8_converter(const uchar *from, uchar *to, int w, int delta) {
    ...
    for (; w--; from += d, to += td) {
        ...
        if (b < 0)
            ...
            b = 255;
        Fl_Color i = fl_color_cube(r * FL_NUM_RED / 256, g * FL_NUM_GREEN / 256,
                                   b * FL_NUM_BLUE / 256);
...
static void color16_converter(const uchar *from, uchar *to, int w, int delta) {
    ...
    for (; w--; from += d, t += td) {
        ...
        if (b > 255) b = 255;
        OUTASSIGN((((r & fl_redmask) << fl_redshift) +
...
static void mono16_converter(const uchar *from, uchar *to, int w, int delta) {
    ...
    for (; w--; from += d, t += td) {
        ...
        uchar m = r & mask;
        OUTASSIGN(
            ((m << fl_redshift) + (m << fl_greenshift) + (m << fl_blueshift)) >>
            fl_extrashift);
    }
    ri = r;
}
...
static void argb_premul_converter(const uchar *from, uchar *to, int w,
                                  int delta) {
...
static void color32_converter(const uchar *from, uchar *to, int w, int delta) {
    INNARDS32((from[0] << fl_redshift) + (from[1] << fl_greenshift) +
              (from[2] << fl_blueshift));
}

static void mono32_converter(const uchar *from, uchar *to, int w, int delta) {
    INNARDS32((*from << fl_redshift) + (*from << fl_greenshift) +
              (*from << fl_blueshift));
}
...
static void innards(const uchar *buf, int X, int Y, int W, int H, int delta,
                    int linedelta, int mono, Fl_Draw_Image_Cb cb,
                    void *userdata, const bool alpha) {
    ...
    if (alpha) {
        // This flag states the destination format is ARGB32 (big-endian),
        // pre-multiplied.
        bytes_per_pixel = 4;
        ...
        if (fl_visual->depth != 32) {
            if (gc32 == None) gc32 = XCreateGC(fl_display, fl_window, 0, NULL);
            fl_gc = gc32;
        }
    }
    ...
    if (buf &&
      ...
      ||
        ...
        int linesize = ((w * bytes_per_pixel + scanline_add) & scanline_mask) /
                       sizeof(STORETYPE);
        int blocking = h;
        ...
        if (buf) {
            ...
            for (int j = 0; j < h;) {
                ...
                XPutImage(fl_display, fl_window, fl_gc, &xi, 0, 0, X + dx,
                          Y + dy + j - k, w, k);
            }
        } else {
            STORETYPE *linebuf =
                new STORETYPE[(W * delta + (sizeof(STORETYPE) - 1)) /
                              sizeof(STORETYPE)];
            for (int j = 0; j < h;) {
                STORETYPE *to = buffer;
                int k;
...
void Fl_Xlib_Graphics_Driver::draw_image(const uchar *buf, int x, int y, int w,
                                         int h, int d, int l) {
...
void Fl_Xlib_Graphics_Driver::draw_image(Fl_Draw_Image_Cb cb, void *data, int x,
