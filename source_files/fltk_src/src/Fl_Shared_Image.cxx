...
Fl_Shared_Handler *Fl_Shared_Image::handlers_ =
...
Fl_Shared_Image **Fl_Shared_Image::images() { return images_; }

/** Returns the total number of shared images in the array. */
int Fl_Shared_Image::num_images() { return num_images_; }
...
/**
  ...
  \returns	Whether the images match or their relative sort order (see
  text).
...
void Fl_Shared_Image::reload() {
    ...
    if (img) {
        ...
        if ((img->w() != w() && w()) || (img->h() != h() && h())) {
            // Make sure the reloaded image is the same size as the existing
            // one.
...
void Fl_Shared_Image::draw(int X, int Y, int W, int H, int cx, int cy) {
    ...
    if ((d() != 0 && count() < 2) ||
        (w() <= image_->w() && h() <= image_->h())) {
            ...
            fl_graphics_driver->draw_scaled(image_, X - cx, Y - cy, w(), h());
    }
    if (!done) {
        if (scaled_image_ &&
            (scaled_image_->w() != w() || scaled_image_->h() != h())) {
            delete scaled_image_;
            scaled_image_ = NULL;
        }
        if (!scaled_image_) {
            Fl_RGB_Scaling previous = RGB_scaling();
            RGB_scaling(scaling_algorithm_);  // useless but no harm if image_
                                              // is not an Fl_RGB_Image
            scaled_image_ = image_->copy(w(), h());
            RGB_scaling(previous);
        }
        scaled_image_->draw(X - cx, Y - cy, scaled_image_->w(),
                            scaled_image_->h(), 0, 0);
    }
    fl_pop_clip();
#else
...
/** Sets the drawing size of the shared image.
 This function gives the shared image its own size, independently from the size
 of the original image that is typically larger. This can be useful to draw a
 shared image on a drawing surface whose resolution is higher than the drawing
 unit for this surface: all pixels of the original image become available to
 fill an area of the drawing surface sized at <tt>width,height</tt>. Examples of
 such drawing surfaces: laser printers, PostScript files, PDF printers, retina
 displays on Apple hardware.

 \param width,height   maximum width and height (in drawing units) to use when
 drawing the shared image \param proportional   if not null, keep the width and
 height of the shared image proportional to those of its original image \param
 can_expand  if null, the width and height of the shared image will not exceed
 ...
 read a picture file shared->scale(b->w(), b->h(), 1); // set the drawing size
 of the shared image to the size of the box b->image(shared); // use the shared
 image as the box image b->align(FL_ALIGN_INSIDE | FL_ALIGN_CENTER |
 FL_ALIGN_CLIP); // the image is to be drawn centered in the box \endcode
 */
void Fl_Shared_Image::scale(int width, int height, int proportional,
                            int can_expand) {
#if FLTK_ABI_VERSION >= 10304
    w(width);
    h(height);
...
/**
  ...
  \see Fl_PNG_Image::Fl_PNG_Image (const char *name_png, const unsigned char
  *buffer, int maxsize)
*/
...
Fl_Shared_Image *Fl_Shared_Image::get(Fl_RGB_Image *rgb, int own_it) {
    Fl_Shared_Image *shared =
        new Fl_Shared_Image(Fl_Preferences::newUUID(), rgb);
...
void Fl_Shared_Image::add_handler(Fl_Shared_Handler f) {
    ...
    if (num_handlers_ >= alloc_handlers_) {
        ...
        if (alloc_handlers_) {
            memcpy(temp, handlers_,
                   alloc_handlers_ * sizeof(Fl_Shared_Handler));
