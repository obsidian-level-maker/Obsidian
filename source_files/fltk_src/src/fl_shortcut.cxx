...
int Fl::test_shortcut(unsigned int shortcut) {
    ...
    unsigned int firstChar = fl_utf8decode(
        Fl::event_text(), Fl::event_text() + Fl::event_length(), 0);
...
/**
  ...
  \param [in] shortcut the integer value containing the ascii character or
  extended keystroke plus modifiers \return a pointer to a static buffer
  containing human readable text for the shortcut
  */
...
/**
  ...
  \param [in] shortcut the integer value containing the ascii character or
  extended keystroke plus modifiers \param [in] eom if this pointer is set, it
  will receive a pointer to the end of the modifier text \return a pointer to a
  static buffer containing human readable text for the shortcut
...
const char *fl_shortcut_label(unsigned int shortcut, const char **eom) {
    ...
    // Note: if necessary we could change the order here depending on the
    // platform. However, as discussed in fltk.development, the order appears to
    // be the same on all platforms, with exceptions in _some_ Linux
    // applications.
...
#else
    const char *q;
    if (key == FL_Enter || key == '\r')
        q = "Enter";  // don't use Xlib's "Return":
...
/**
  ...
  \verbatim
   ...
   "@c"     -- Same as FL_COMMAND|'c' (see FL_COMMAND for platform specific
  behavior) "0x63"   -- Same as "c" (hex 63=='c') "99"     -- Same as "c" (dec
  99=='c') "0143"   -- Same as "c" (octal 0143=='c')
...
unsigned int fl_old_shortcut(const char *s) {
    if (!s || !*s) return 0;
    if (s[1] == 0 && strchr("@!", s[0]))
        return s[0];  // maintain legacy behavior for "!" and "@"
    unsigned int n = 0;
    if (*s == '#') {
        n |= FL_ALT;
        s++;
...
int Fl_Widget::test_shortcut(const char *t, const bool require_alt) {
    ...
    // and Linux.
    if (c && Fl::event_state(FL_ALT)) c = Fl::event_key();
#endif
    if (!c) return 0;
