 ...
 *
 ...
 origin() and by the optional delta_x and delta_y arguments. Its dimensions are
 in points unless there was a previous call to scale().
 ...
 relatively to the current origin of graphics functions.
 @param[in] delta_y Same as above, vertically.
 */
void Fl_Paged_Device::print_widget(Fl_Widget *widget, int delta_x,
                                   int delta_y) {
...
#ifdef __APPLE__  // for Mac OS X 10.6 and above, make window with rounded
                  // bottom corners
        if (fl_mac_os_version >= 100600 &&
            driver()->class_name() == Fl_Quartz_Graphics_Driver::class_id) {
            Fl_X::clip_to_rounded_corners(fl_gc, widget->w(), widget->h());
        }
#endif
    }
    // we do some trickery to recognize OpenGL windows and draw them via a
    // plugin
...
/**
 ...
 functions.
 *
 @param[out] x If non-null, *x is set to the horizontal page offset of graphics
 origin.
...
/**
 ...
 @param delta_x Optional horizontal offset from current graphics origin where to
 print the captured rectangle.
...
/**
 ...
 @param[in] pagecount the total number of pages of the job (or 0 if you don't
 know the number of pages)
 @param[out] frompage if non-null, *frompage is set to the first page the user
 wants printed
 @param[out] topage if non-null, *topage is set to the last page the user wants
 printed
...
/**
 ...
 printable_rect() call. That is, <tt>printable_rect(&w, &h); origin(w/2,
 0);</tt> sets the graphics origin at the top center of the page printable area.
...
/**
 ...
 printable page area. After a scale() call, do a printable_rect() call to get
 the new dimensions of the printable page area. Successive scale() calls don't
 ...
 scale(factor); is equivalent to scale(factor, factor);
 */
...
const Fl_Paged_Device::page_format
    Fl_Paged_Device::page_formats[NO_PAGE_FORMATS] = {
