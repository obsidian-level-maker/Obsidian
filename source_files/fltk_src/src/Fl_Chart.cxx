...
static void draw_barchart(int x, int y, int w, int h, int numb,
                          FL_CHART_ENTRY entries[], double min, double max,
                          int autosize, int maxnumb, Fl_Color textcolor)
   ...
   numb entries and min and max the boundaries. */
{
    ...
    for (i = 0; i < numb; i++) {
        ...
        if (hh < 0)
            ...
            fl_rectbound(x + i * bwidth, zeroh - hh, bwidth + 1, hh + 1,
                         (Fl_Color)entries[i].col);
    }
    /* Draw the labels */
    fl_color(textcolor);
    for (i = 0; i < numb; i++)
        fl_draw(entries[i].str, x + i * bwidth + bwidth / 2, zeroh, 0, 0,
                FL_ALIGN_TOP);
}

static void draw_horbarchart(int x, int y, int w, int h, int numb,
                             FL_CHART_ENTRY entries[], double min, double max,
                             int autosize, int maxnumb, Fl_Color textcolor)
   ...
   array of numb entries and min and max the boundaries. */
{
    ...
    for (i = 0; i < numb; i++) {
        ...
        if (ww > 0)
            ...
            fl_rectbound(zeroh + ww, y + i * bwidth, -ww + 1, bwidth + 1,
                         (Fl_Color)entries[i].col);
    }
    /* Draw the labels */
    fl_color(textcolor);
    for (i = 0; i < numb; i++)
        fl_draw(entries[i].str, zeroh - 2, y + i * bwidth + bwidth / 2, 0, 0,
                FL_ALIGN_RIGHT);
}
    ...
    for (i = 0; i < numb; i++) {
        ...
        if (type == FL_SPIKE_CHART) {
            ...
            if ((entries[i - 1].val > 0.0) != (entries[i].val > 0.0)) {
                double ttt =
                    entries[i - 1].val / (entries[i - 1].val - entries[i].val);
...
static void draw_piechart(int x, int y, int w, int h, int numb,
                          FL_CHART_ENTRY entries[], int special,
                          Fl_Color textcolor)
   ...
   numb entries */
{
    ...
    for (i = 0; i < numb; i++)
        if (entries[i].val > 0.0) {
            ...
            if (special && i == 0) {
                txc += 0.3 * rad *
                       cos(ARCINC * (curang + 0.5 * incr * entries[i].val));
                tyc -= 0.3 * rad *
                       sin(ARCINC * (curang + 0.5 * incr * entries[i].val));
            }
            fl_color((Fl_Color)entries[i].col);
            fl_begin_polygon();
            fl_vertex(txc, tyc);
            ...
            fl_draw(entries[i].str, (int)rint(xl),
                    (int)rint(tyc - 1.1 * rad * sin(ARCINC * curang)), 0, 0,
...
void Fl_Chart::draw() {
    ...
    switch (type()) {
        case FL_BAR_CHART:
            ...
            draw_barchart(xx, yy, ww, hh, numb, entries, min, max, autosize(),
                          maxnumb, textcolor());
            break;
...
Fl_Chart::Fl_Chart(int X, int Y, int W, int H, const char *L)
    ...
    entries =
        (FL_CHART_ENTRY *)calloc(sizeof(FL_CHART_ENTRY), FL_CHART_MAX + 1);
}

/**
  Destroys the Fl_Chart widget and all of its data.
 */
Fl_Chart::~Fl_Chart() { free(entries); }
...
void Fl_Chart::add(double val, const char *str, unsigned col) {
    ...
    if (numb >= sizenumb) {
        sizenumb += FL_CHART_MAX;
        entries = (FL_CHART_ENTRY *)realloc(
            entries, sizeof(FL_CHART_ENTRY) * (sizenumb + 1));
    }
...
void Fl_Chart::maxsize(int m) {
    ...
    if (numb > maxnumb) {
        for (i = 0; i < maxnumb; i++) entries[i] = entries[i + numb - maxnumb];
        numb = maxnumb;
        redraw();
    }
}
