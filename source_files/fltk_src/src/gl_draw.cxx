...
#if !defined(kCGBitmapByteOrder32Host)  // doc says available 10.4 but some 10.4
                                        // don't have it
...
#define GENLISTSIZE 0x10000
        ...
        /* OksiD had a fairly sophisticated scheme for storing multiple X fonts
         * in a XUtf8FontStruct, then sorting through them at draw time (for
         * normal X rendering) to find which one can render the current glyph...
         * But for now, just use the first font in the list for GL...
         */
    ...
    for (int j = 0; j < FL_FREE_FONT; ++j) {
        ...
        while (f != 0) {
            if (f->listbase) {
                ...
                // It would be nice if this next line was in a destructor
                // somewhere
...
/**
 ...
 \see On the Mac OS X platform, see gl_texture_pile_height(int)
  */
void gl_draw(const char *str) { gl_draw(str, strlen(str)); }

/**
  Draws a nul-terminated string in the current font at the given position
 \see On the Mac OS X platform, see gl_texture_pile_height(int)
  */
void gl_draw(const char *str, int x, int y) { gl_draw(str, strlen(str), x, y); }
...
/**
  ...
  */
void gl_draw(const char *str,             // the (multi-line) string
             int x, int y, int w, int h,  // bounding box
             Fl_Align align) {
    fl_draw(str, x, -y - h, w, h, align, gl_draw_invert, NULL, 0);
}

/** Measure how wide and tall the string will be when drawn by the gl_draw()
 * function */
...
void gl_draw_image(const uchar *b, int x, int y, int w, int h, int d, int ld) {
    ...
    glRasterPos2i(x, y);
    glDrawPixels(w, h, d < 4 ? GL_RGB : GL_RGBA, GL_UNSIGNED_BYTE,
                 (const ulong *)b);
}

#if defined(__APPLE__) || defined(FL_DOXYGEN)
/* Text drawing to an OpenGL scene under Mac OS X is implemented using textures,
 as recommended by Apple. This allows to use any font at any size, and any
 Unicode character. Some old Apple hardware doesn't implement the required
 GL_EXT_texture_rectangle extension. For these, glutStrokeString() is used to
 draw text. In that case, it's possible to vary text size, but not text font,
 and only ASCII characters can be drawn.
 */

static int gl_scale = 1;  // set to 2 for high resolution Fl_Gl_Window
static int has_texture_rectangle =
    0;  // true means GL_EXT_texture_rectangle is available
...
void gl_texture_fifo::display_texture(int rank) {
    ...
    // GL_COLOR_BUFFER_BIT for glBlendFunc, GL_ENABLE_BIT for glEnable /
    // glDisable
    glPushAttrib(GL_ENABLE_BIT | GL_TEXTURE_BIT | GL_COLOR_BUFFER_BIT);
    glDisable(
        GL_DEPTH_TEST);  // ensure text is not removed by depth buffer test.
    ...
    if (has_texture_rectangle) {
        ...
        GLint height;
        glGetTexLevelParameteriv(GL_TEXTURE_RECTANGLE_ARB, 0, GL_TEXTURE_HEIGHT,
                                 &height);
        CGRect bounds = CGRectMake(pos[0], pos[1] - gl_scale * fl_descent(),
                                   fifo[rank].width, height);
        ...
        glTexCoord2f(fifo[rank].width,
                     height);  // draw upper right in world coordinates
        glVertex2f(bounds.origin.x + bounds.size.width,
                   bounds.origin.y + bounds.size.height);

        glTexCoord2f(fifo[rank].width,
                     0.0f);  // draw lower right in world coordinates
...
int gl_texture_fifo::compute_texture(const char *str, int n) {
    ...
    if (has_texture_rectangle) {
        ...
        if (fl_mac_os_version < 100600)
            base = calloc(4 * fifo[current].width, h);
        CGContextRef save_gc = fl_gc;
        fl_gc = CGBitmapContextCreate(
            base, fifo[current].width, h, 8, fifo[current].width * 4, lut,
        ...
        glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA8, fifo[current].width,
                     h, 0, GL_BGRA_EXT, GL_UNSIGNED_INT_8_8_8_8_REV,
                     CGBitmapContextGetData(fl_gc));
        glPopAttrib();
        ...
        fifo[current].ratio =
            float(fifo[current].width) /
            glutStrokeLength(GLUT_STROKE_ROMAN, (uchar *)fifo[current].utf8);
    }
    return current;
}
...
static gl_texture_fifo *gl_fifo =
    NULL;  // points to the texture pile class instance
...
static void gl_draw_textures(const char *str, int n) {
    ...
    if (!gl_fifo->textures_generated) {
        has_texture_rectangle = gluCheckExtension(
            (GLubyte *)"GL_EXT_texture_rectangle", glGetString(GL_EXTENSIONS));
        if (has_texture_rectangle)
            for (int i = 0; i < gl_fifo->size_; i++)
                glGenTextures(1, &(gl_fifo->fifo[i].texName));
