 ...
 */
Fl_Native_File_Chooser::~Fl_Native_File_Chooser() { delete _x11_file_chooser; }
...
/**
 ...
 \p val is expected to be one or more Fl_Native_File_Chooser::Option flags ORed
together. Some platforms have OS-specific functions that can be enabled/disabled
via this method. <P> \code Flag              Description Win       Mac Other
 --------------    -----------------------------------------------   -------
-------   ------- NEW_FOLDER        Shows the 'New Folder' button. Ignored Used
Used PREVIEW           Enables the 'Preview' mode by default.            Ignored
Ignored   Used SAVEAS_CONFIRM    Confirm dialog if BROWSE_SAVE_FILE file exists.
Used      Used      Used USE_FILTER_EXT    Chooser filter pilots the output file
extension.  Ignored   Used      Used (GTK) \endcode
 */
...
/**
 ...
 Files\t*.txt") <P> The format of each filter is a wildcard, or an optional user
 description followed by '\\t' and the wildcard. <P> On most platforms, each
 filter is available to the user via a pulldown menu in the file chooser. The
 'All Files' option is always available to the user.
 */
...
/**
 ...
 This message should at least be flagged to the user in a dialog box, or to some
 kind of error log. Contents will be valid only for methods that document
 errmsg() will have info on failures.
 */
...
int Fl_FLTK_File_Chooser::type() const { return (_btype); }

void Fl_FLTK_File_Chooser::options(int val) { _options = val; }

int Fl_FLTK_File_Chooser::options() const { return (_options); }
...
int Fl_FLTK_File_Chooser::show() {
    ...
    // OPTIONS: PREVIEW
    _file_chooser->preview((options() & Fl_Native_File_Chooser::PREVIEW) ? 1
                                                                         : 0);

    // OPTIONS: NEW FOLDER
    if (options() & Fl_Native_File_Chooser::NEW_FOLDER)
        _file_chooser->type(_file_chooser->type() |
                            Fl_File_Chooser::CREATE);  // on
...
const char *Fl_FLTK_File_Chooser::filename(int i) const {
    if (i < _file_chooser->count())
        return (_file_chooser->value(
            i + 1));  // convert fltk 1 based to our 0 based
    return ("");
}

void Fl_FLTK_File_Chooser::title(const char *val) { _file_chooser->label(val); }
...
const char *Fl_FLTK_File_Chooser::filter() const { return (_filter); }

int Fl_FLTK_File_Chooser::filters() const { return (_nfilters); }

void Fl_FLTK_File_Chooser::filter_value(int val) { _filtvalue = val; }

int Fl_FLTK_File_Chooser::filter_value() const { return _filtvalue; }

int Fl_FLTK_File_Chooser::count() const { return _file_chooser->count(); }
...
const char *Fl_FLTK_File_Chooser::directory() const { return _directory; }
...
void Fl_FLTK_File_Chooser::parse_filter() {
    _parsedfilt =
        strfree(_parsedfilt);  // clear previous parsed filter (if any)
    _nfilters = 0;
    char *in = _filter;
    if (!in) return;
        ...
        switch (*in) {
            ...
            case '\0':
                ...
                if (wildcard[0]) {
                    ...
                    char comp[2048];
                    sprintf(comp, "%s%.511s(%.511s)",
                            ((_parsedfilt) ? "\t" : ""), name, wildcard);
...
const char *Fl_FLTK_File_Chooser::preset_file() const { return _preset_file; }
