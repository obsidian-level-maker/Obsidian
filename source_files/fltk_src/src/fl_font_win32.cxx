...
Fl_Font_Descriptor::Fl_Font_Descriptor(const char *name, Fl_Fontsize fsize) {
    ...
    // ...would be the right call, but is not implemented into Window95!
    // (WinNT?)
...
static Fl_Fontdesc built_in_table[] = {
    {" Arial"},           {"BArial"},           {"IArial"},
    {"PArial"},           {" Courier New"},     {"BCourier New"},
    {"ICourier New"},     {"PCourier New"},     {" Times New Roman"},
    {"BTimes New Roman"}, {"ITimes New Roman"}, {"PTimes New Roman"},
    {" Symbol"},          {" Terminal"},        {"BTerminal"},
    {" Wingdings"},
};
...
double Fl_GDI_Graphics_Driver::width(unsigned int c) {
    ...
    if (!gc)
        Fl::fatal(
            "Invalid graphic context: fl_width() failed because no valid HDC "
            "was found!");
...
/* Add function pointer to allow us to access GetGlyphIndicesW on systems that
 * have it, without crashing on systems that do not. */
/* DWORD WINAPI GetGlyphIndicesW(HDC,LPCWSTR,int,LPWORD,DWORD) */
typedef DWORD(WINAPI *fl_GetGlyphIndices_func)(HDC, LPCWSTR, int, LPWORD,
                                               DWORD);
...
static void GetGlyphIndices_init() {
    ...
    if (hMod) {
        ...
        fl_GetGlyphIndices =
            (fl_GetGlyphIndices_func)GetProcAddress(hMod, "GetGlyphIndicesW");
    }
    have_loaded_GetGlyphIndices = -1;  // set this non-zero when we have
                                       // attempted to load GetGlyphIndicesW
...
// string
void Fl_GDI_Graphics_Driver::text_extents(const char *c, int n, int &dx,
                                          int &dy, int &w, int &h) {
    Fl_Font_Descriptor *fl_fontsize = font_descriptor();
    ...
    static unsigned short *ext_buff =
        NULL;  // UTF-16 converted version of input UTF-8 string
    static WORD *w_buff = NULL;  // glyph indices array
    static unsigned wc_len = 0;  // current string buffer dimensions
    static const MAT2 matrix = {
    ...
    if (!fl_GetGlyphIndices)
        goto exit_error;  // No GetGlyphIndices function, use fallback mechanism
                          // instead
    ...
    // Are there surrogate-pairs in this string? If so GetGlyphIndicesW will
    // fail since it can only handle the BMP range. We ideally want to use
    // GetGlyphIndicesW, as it is the Right Thing, but it only works for the
    // BMP, so we leverage GetCharacterPlacementW instead, which is not ideal,
    // but works adequately well, and does handle surrogate pairs.
    has_surrogates = 0;
    ...
    if (has_surrogates) {
        ...
        DWORD dr = GetCharacterPlacementW(gc, (WCHAR *)ext_buff, len, 0,
                                          &gcp_res, GCP_GLYPHSHAPE);
        if (dr) {
            len = gcp_res.nGlyphs;
        } else
            goto exit_error;
...
void Fl_GDI_Graphics_Driver::draw(const char *str, int n, int x, int y) {
    ...
    if (wn >= wstr_len) {
        wstr =
            (unsigned short *)realloc(wstr, sizeof(unsigned short) * (wn + 1));
        wstr_len = wn + 1;
