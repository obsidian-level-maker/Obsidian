...
const char *Fl_Native_File_Chooser::errmsg() const { return (_errmsg ? _errmsg : "No error"); }
...
//     Returned value can be NULL if none set.
//
const char *Fl_Native_File_Chooser::preset_file() const { return (_preset_file); }

void Fl_Native_File_Chooser::filter_value(int val) { _filt_value = val; }

int Fl_Native_File_Chooser::filter_value() const { return (_filt_value); }

int Fl_Native_File_Chooser::filters() const { return (_filt_total); }
...
int Fl_Native_File_Chooser::get_saveas_basename(void) {
    ...
    if (!(_options & SAVEAS_CONFIRM)) {
        const char *d =
            [[[[(NSSavePanel *)_panel URL] path] stringByDeletingLastPathComponent] UTF8String];
...
- (BOOL)panel:(id)sender shouldShowFilename:(NSString *)filename {
    ...
    if (isdir) return YES;
    if (fl_filename_match([filename fileSystemRepresentation],
...
// correspondingly changes the extension of the output file name
{
    if (fl_mac_os_version < 100600)
        return;  // because of setNameFieldStringValue and nameFieldStringValue
...
static NSPopUpButton *createPopupAccessory(NSSavePanel *panel, const char *filter,
                                           const char *title, int rank) {
    NSPopUpButton *popup;
...
int Fl_Native_File_Chooser::runmodal() {
    ...
    if (fl_mac_os_version >= 100600) {
        if (dir)
            [(NSSavePanel *)_panel performSelector:@selector(setDirectoryURL:)
                                        withObject:[NSURL fileURLWithPath:dir]];
        if (fname)
            [(NSSavePanel *)_panel performSelector:@selector(setNameFieldStringValue:)
                                        withObject:fname];
...
int Fl_Native_File_Chooser::post() {
    ...
    if (_title) {
        SEL title_or_message = (is_open_panel && fl_mac_os_version >= 101200)
                                   ? @selector(setMessage:)
                                   : @selector(setTitle:);
        [(NSSavePanel *)_panel performSelector:title_or_message
                                    withObject:[NSString stringWithUTF8String:_title]];
    }
    switch (_btype) {
        case BROWSE_MULTI_FILE:
    ...
    if (is_open_panel) {
        if (_filt_total) {
            ...
            [popup
                addItemWithTitle:[NSString stringWithUTF8String:Fl_File_Chooser::all_files_label]];
        ...
        if (_filt_total) {
            ...
            popup = createPopupAccessory((NSSavePanel *)_panel, t,
                                         [[(NSSavePanel *)_panel nameFieldLabel] UTF8String],
                                         _filt_value);
            delete[] t;
