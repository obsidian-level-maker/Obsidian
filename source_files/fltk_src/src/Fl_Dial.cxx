...
void Fl_Dial::draw(int X, int Y, int W, int H) {
    ...
    H -= Fl::box_dh(box());
    double angle =
        (a2 - a1) * (value() - minimum()) / (maximum() - minimum()) + a1;
    if (type() == FL_FILL_DIAL) {
        ...
        if (active_r())
            ...
            fl_color(fl_inactive(color()));
        fl_pie(X, Y, W, H, 270 - a1,
               angle > a1 ? 360 + 270 - angle : 270 - 360 - angle);
        if (active_r())
...
int Fl_Dial::handle(int event, int X, int Y, int W, int H) {
    switch (event) {
        ...
        case FL_DRAG: {
            ...
            double oldangle =
                (a2 - a1) * (value() - minimum()) / (maximum() - minimum()) +
                a1;
            ...
            if ((a1 < a2) ? (angle <= a1) : (angle >= a1)) {
                ...
                val = minimum() +
                      (maximum() - minimum()) * (angle - a1) / (a2 - a1);
            }
...
*/
int Fl_Dial::handle(int e) { return handle(e, x(), y(), w(), h()); }
