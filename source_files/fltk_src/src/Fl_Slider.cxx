...
Fl_Slider::Fl_Slider(uchar t, int X, int Y, int W, int H, const char *L)
    : Fl_Valuator(X, Y, W, H, L) {
    type(t);
    box(t == FL_HOR_NICE_SLIDER || t == FL_VERT_NICE_SLIDER ? FL_FLAT_BOX
                                                            : FL_DOWN_BOX);
    _Fl_Slider();
}
...
int Fl_Slider::handle(int event) {
    ...
    return handle(event, x() + Fl::box_dx(box()), y() + Fl::box_dy(box()),
                  w() - Fl::box_dw(box()), h() - Fl::box_dh(box()));
}
...
Fl_Hor_Fill_Slider::Fl_Hor_Fill_Slider(int X, int Y, int W, int H,
                                       const char *L)
    : Fl_Slider(X, Y, W, H, L) {
    type(FL_HOR_FILL_SLIDER);
}

Fl_Hor_Nice_Slider::Fl_Hor_Nice_Slider(int X, int Y, int W, int H,
                                       const char *L)
    : Fl_Slider(X, Y, W, H, L) {
    type(FL_HOR_NICE_SLIDER);
    box(FL_FLAT_BOX);
