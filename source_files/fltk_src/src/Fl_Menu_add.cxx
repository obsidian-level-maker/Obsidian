...
*/
int Fl_Menu_Item::add(const char *mytext, int sc, Fl_Callback *cb, void *data,
                      int myflags) {
...
/**
 ...
 \returns the index into the menu() array, where the entry was added
*/
int Fl_Menu_Item::insert(int index, const char *mytext, int sc, Fl_Callback *cb,
                         void *data, int myflags) {
...
/**
  ...
  \param[in] callback	Optional callback invoked when user clicks the item.
                        Default 0 if none.
  \param[in] userdata	Optional user data passed as an argument to the
  callback. Default 0 if none. \param[in] flags	Optional flags that control the
  type of menu item; see below. Default is 0 for none.
  \returns		The index into the menu() array, where the entry was
  added.
  ...
  divider to be placed after that menu item. \par A label of the form
  "File/Quit" will create the submenu "File" with a menu item called "Quit". The
  "/" character is ignored if it appears as the first character of the label
  string, e.g.  "/File/Quit". \par The label string is copied to new memory and
  can be freed. The other arguments (including the shortcut) are copied into the
  menu item unchanged.
  \par
  ...
  \code
      ...
      FL_MENU_TOGGLE       // Item is a checkbox toggle (shows checkbox for
  on/off state) FL_MENU_VALUE        // The on/off state for checkbox/radio
  buttons (if set, state is 'on') FL_MENU_RADIO        // Item is a radio button
  (one checkbox of many can be on) FL_MENU_INVISIBLE    // Item will not show up
  (shortcut will work) FL_SUBMENU_POINTER   // Indicates user_data() is a
  pointer to another menu array FL_SUBMENU           // This item is a submenu
  to other items FL_MENU_DIVIDER      // Creates divider line below this item.
  Also ends a group of radio buttons. \endcode
  ...
  \todo Raw integer shortcut needs examples.
        Dependent on responses to
  http://fltk.org/newsgroups.php?gfltk.development+v:10086 and results of
  STR#2344
 */
int Fl_Menu_::add(const char *label, int shortcut, Fl_Callback *callback,
                  void *userdata, int flags) {
    return (
        insert(-1, label, shortcut, callback, userdata, flags));  // -1: append
}
...
/**
  ...
  To properly insert a menu item, \p label must be the name of the item (eg.
  "Quit"), and not a 'menu pathname' (eg. "File/Quit").  If a menu pathname is
  specified, the value of \p index is \em ignored, the new item's position
...
int Fl_Menu_::insert(int index, const char *label, int shortcut,
                     Fl_Callback *callback, void *userdata, int flags) {
    ...
    if (this != fl_menu_array_owner) {
        ...
        if (menu_) {
            ...
            if (local_array) {
                ...
                memset(local_array, 0,
                       sizeof(Fl_Menu_Item) * local_array_alloc);
            }
...
int Fl_Menu_::add(const char *str) {
    ...
    while (*str) {
        int sc = 0;
        char *c;
        for (c = buf; c < (buf + sizeof(buf) - 2) && *str && *str != '|';
             str++) {
            if (*str == '\t') {
                *c++ = 0;
