...
static Fl_RGB_Image *capture_gl_rectangle(Fl_Gl_Window *glw, int x, int y,
                                          int w, int h)
...
/**
 This class will make sure that OpenGL printing/screen capture is available if
 fltk_gl was linked to the program
 */
   ...
   public:
    ...
    virtual int print(Fl_Widget *w, int x, int y, int height /*useless*/) {
        ...
        if (Fl_Surface_Device::surface()->class_name() ==
            Fl_Printer::class_id) {
            // convert the image to CGImage, and draw it at full res (useful on
            // retina display)
            CGColorSpaceRef cSpace = CGColorSpaceCreateDeviceRGB();
            CGDataProviderRef provider = CGDataProviderCreateWithData(
                img, img->array, img->ld() * img->h(), imgProviderReleaseData);
            CGImageRef cgimg =
                CGImageCreate(img->w(), img->h(), 8, 24, img->ld(), cSpace,
                              (CGBitmapInfo)(kCGImageAlphaNone), provider, NULL,
                              false, kCGRenderingIntentDefault);
            CGColorSpaceRelease(cSpace);
            CGDataProviderRelease(provider);
            CGContextDrawImage(fl_gc, CGRectMake(0, 0, glw->w(), glw->h()),
                               cgimg);
            CFRelease(cgimg);
            return 1;
        } else if (img->w() > glw->w()) {
        ...
        fl_draw_image(img->array + (img->h() - 1) * ld, x, y, img->w(),
                      img->h(), 3, -ld);
        delete img;
        return 1;
    }
    virtual Fl_RGB_Image *rectangle_capture(Fl_Widget *widget, int x, int y,
                                            int w, int h) {
...
// The purpose of this variable, used in Fl_Gl_Window.cxx, is only to force this
// file to be loaded whenever Fl_Gl_Window.cxx is loaded, that is, whenever
// fltk_gl is.
