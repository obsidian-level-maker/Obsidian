...
static void gtk_color(Fl_Color c) { Fl::set_box_color(c); }
...
static void draw(int which, int x, int y, int w, int h, int inset) {
    ...
    if (which == FILL) {
        ...
        if (w < h) {
            if (which != UPPER_LEFT)
                fl_yxline(x + w - 1, y + d / 2 - 1, y + h - d / 2 + 1);
            if (which != LOWER_RIGHT)
                fl_yxline(x, y + d / 2 - 1, y + h - d / 2 + 1);
        } else if (w > h) {
            if (which != UPPER_LEFT)
                fl_xyline(x + d / 2 - 1, y + h - 1, x + w - d / 2 + 1);
            if (which != LOWER_RIGHT)
                fl_xyline(x + d / 2 - 1, y, x + w - d / 2 + 1);
        }
    }
}
