...
static const char *const broken_xpm[] = {
    "16 24 4 1", "@ c #000000", "  c #ffffff", "+ c none", "x c #ff0000",
    // pixels
    "@@@@@@@+++++++++", "@    @++++++++++", "@   @+++++++++++",
    "@   @++@++++++++", "@    @@+++++++++", "@     @+++@+++++",
    "@     @++@@++++@", "@ xxx  @@  @++@@", "@  xxx    xx@@ @",
...
void Fl_Help_View::hv_draw(const char *t, int x, int y,
                           int entity_extra_length) {
    if (selected && current_view == this && current_pos < selection_last &&
    ...
    if (draw_mode) {
        ...
        if (mouse_x >= x && mouse_x < x + w) {
            if (mouse_y >= y - fl_height() + fl_descent() &&
                mouse_y <= y + fl_descent()) {
                int f = (int)current_pos;
                int l =
                    (int)(f + strlen(t));  // use 'quote_char' to calculate the
                                           // true length of the HTML string
                if (draw_mode == 1) {
...
*/
HV_Edit_Buffer::HV_Edit_Buffer(int alloc, int ext) {
...
void HV_Edit_Buffer::check(int size) {
    ...
    int new_size = (allocated_ + size + extend_) &
                   (~(extend_ - 1));  // round to chunk size
                        ...
                        unsigned char border)  // I - Draw border?
{
    ...
    // printf("add_block(s = %p, xx = %d, yy = %d, ww = %d, hh = %d, border =
    // %d)\n",
    ...
    if (nblocks_ >= ablocks_) {
        ...
        if (ablocks_ == 16)
            ...
            blocks_ = (Fl_Help_Block *)realloc(
                blocks_, sizeof(Fl_Help_Block) * ablocks_);
    }
                            ...
                            int hh)         // I - Height of link text
{
    ...
    if (nlinks_ >= alinks_) {
        ...
        if (alinks_ == 16)
            ...
            links_ =
                (Fl_Help_Link *)realloc(links_, sizeof(Fl_Help_Link) * alinks_);
    }
                              ...
                              int yy)         // I - Y position of target
{
    ...
    if (ntargets_ >= atargets_) {
        ...
        if (atargets_ == 16)
            targets_ =
                (Fl_Help_Target *)malloc(sizeof(Fl_Help_Target) * atargets_);
        else
            targets_ = (Fl_Help_Target *)realloc(
                targets_, sizeof(Fl_Help_Target) * atargets_);
    }
...
void Fl_Help_View::draw() {
    ...
    if (hscrollbar_.visible() || scrollbar_.visible()) {
        ...
        // Scrollbar corner
        int scorn_x = x() + ww - (ver_vis ? scrollsize : 0) - Fl::box_dw(b) +
                      Fl::box_dx(b);
        int scorn_y = y() + hh - (hor_vis ? scrollsize : 0) - Fl::box_dh(b) +
                      Fl::box_dy(b);
        if (hor_vis) {
    ...
    fl_push_clip(x() + Fl::box_dx(b), y() + Fl::box_dy(b), ww - Fl::box_dw(b),
                 hh - Fl::box_dh(b));
    fl_color(textcolor_);
    ...
    for (i = 0, block = blocks_; i < nblocks_; i++, block++)
        if ((block->y + block->h) >= topline_ && block->y < (topline_ + h())) {
            ...
            for (ptr = block->start, buf.clear(); ptr < block->end;) {
                if ((*ptr == '<' || isspace((*ptr) & 255)) && buf.size() > 0) {
                    if (!head && !pre) {
                        ...
                        if (underline) {
                            xtra_ww =
                                isspace((*ptr) & 255) ? (int)fl_width(' ') : 0;
                        ...
                        while (isspace((*ptr) & 255)) {
                            if (*ptr == '\n') {
                                hv_draw(buf.c_str(), xx + x() - leftline_,
                                        yy + y());
                                if (underline)
                                    fl_xyline(
                                        xx + x() - leftline_, yy + y() + 1,
                ...
                if (*ptr == '<') {
                    ...
                    // end of command reached, set the supposed start of printed
                    // eord here
                    current_pos = (int)(ptr - value_);
                    if (buf.cmp("HEAD"))
                        head = 1;
                        ...
                        hh = 0;
                               ...
                               buf.cmp("H1") || buf.cmp("H2") ||
                               buf.cmp("H3") || buf.cmp("H4") ||
                               buf.cmp("H5") || buf.cmp("H6") ||
                               buf.cmp("UL") || buf.cmp("OL") ||
                               buf.cmp("DL") || buf.cmp("LI") ||
                               buf.cmp("DD") || buf.cmp("DT") ||
                               buf.cmp("PRE")) {
                        if (tolower(buf[0]) == 'h') {
                        ...
                        if (buf.cmp("LI")) {
                            // draw bullet (&bull;) Unicode: U+2022, UTF-8
                            // (hex): e2 80 a2
                            unsigned char bullet[4] = {0xe2, 0x80, 0xa2, 0x00};
                            hv_draw((char *)bullet,
                                    xx - fsize + x() - leftline_, yy + y());
                        }
                        ...
                        if (get_attr(attrs, "COLOR", attr, sizeof(attr)) !=
                            NULL) {
                            textcolor_ = get_color(attr, textcolor_);
                        }

                        if (get_attr(attrs, "FACE", attr, sizeof(attr)) !=
                            NULL) {
                            if (!strncasecmp(attr, "helvetica", 9) ||
                                !strncasecmp(attr, "arial", 5) ||
                                !strncasecmp(attr, "sans", 4))
                                font = FL_HELVETICA;
                            else if (!strncasecmp(attr, "times", 5) ||
                                     !strncasecmp(attr, "serif", 5))
                                font = FL_TIMES;
                            else if (!strncasecmp(attr, "symbol", 6))
                                font = FL_SYMBOL;
                            else
                                font = FL_COURIER;
                        }

                        if (get_attr(attrs, "SIZE", attr, sizeof(attr)) !=
                            NULL) {
                            if (isdigit(attr[0] & 255)) {
                                // Absolute size
                                fsize = (int)(textsize_ *
                                              pow(1.2, atof(attr) - 3.0));
                            } else {
                                // Relative size
                                fsize =
                                    (int)(fsize * pow(1.2, atof(attr) - 3.0));
                            }
                        }
                        ...
                        if (block->border) fl_rect(tx, ty, tw, th);
                        ...
                        head = 0;
                    else if (buf.cmp("/H1") || buf.cmp("/H2") ||
                             buf.cmp("/H3") || buf.cmp("/H4") ||
                             buf.cmp("/H5") || buf.cmp("/H6") ||
                             buf.cmp("/B") || buf.cmp("/STRONG") ||
                             buf.cmp("/I") || buf.cmp("/EM") ||
                             buf.cmp("/CODE") || buf.cmp("/TT") ||
                             buf.cmp("/KBD") || buf.cmp("/VAR"))
                        ...
                        if (!width || !height) {
                            if (get_attr(attrs, "ALT", attr, sizeof(attr)) ==
                                NULL) {
                                strcpy(attr, "IMG");
                            }
                        }
                ...
                } else if (*ptr == '&')  // process html entity
                {
                    ...
                    if (qch < 0)
                        ...
                        buf.add(qch);
                        utf8l =
                            buf.size() - utf8l;  // length of added UTF-8 text
                        const char *oldptr = ptr;
                        ptr = strchr(ptr, ';') + 1;
                        entity_extra_length += ptr - (oldptr - 1) -
                                               utf8l;  // extra length between
            ...
            if (buf.size() > 0 && !pre && !head) {
                ...
                if (needspace && xx > block->x) xx += (int)fl_width(' ');
                   ...
                   int p)          // I - Starting position
{
    ...
    for (i = nblocks_, b = blocks_; i > 0; i--, b++) {
        ...
        for (sp = s, bs = bp; *sp && *bp && bp < b->end; bp++) {
            ...
            // At this point c may be an arbitrary Unicode Code Point
            // corresponding to a quoted character (see above), i.e. it _can_ be
            // a multi byte UTF-8 sequence and must be compared with the
            // corresponding multi byte string in (*sp)... For instance:
            // "&euro;" == 0x20ac -> 0xe2 0x82 0xac (UTF-8: 3 bytes). Hint: use
            // fl_utf8encode() [see below]

            if (tolower(*sp) == tolower(c))
                sp++;
            else {
...
void Fl_Help_View::format() {
    ...
    while (!done) {
        ...
        for (ptr = value_, buf.clear(); *ptr;) {
            ...
            if ((*ptr == '<' || isspace((*ptr) & 255)) && buf.size() > 0) {
                ...
                if (!head && !pre) {
                    ...
                    if (needspace && xx > block->x) ww += (int)fl_width(' ');

                    //        printf("line = %d, xx = %d, ww = %d, block->x =
                    //        %d, block->w = %d\n",
                    ...
                    if (linkdest[0]) add_link(linkdest, xx, yy - hh, ww, hh);
            ...
            if (*ptr == '<') {
                ...
                if (buf.cmp("HEAD"))
                    ...
                    char *st;
                    for (st = title_; *ptr != '<' && *ptr &&
                                      st < (title_ + sizeof(title_) - 1);
                         *st++ = *ptr++) { /*empty*/
                    }
                    ...
                    *st = '\0';
                    ...
                    bgcolor_ = get_color(
                        get_attr(attrs, "BGCOLOR", attr, sizeof(attr)),
                        color());
                    textcolor_ =
                        get_color(get_attr(attrs, "TEXT", attr, sizeof(attr)),
                                  textcolor());
                    linkcolor_ =
                        get_color(get_attr(attrs, "LINK", attr, sizeof(attr)),
                    ...
                    yy += hh;
                    hh = 0;
                } else if (buf.cmp("CENTER") || buf.cmp("P") || buf.cmp("H1") ||
                           buf.cmp("H2") || buf.cmp("H3") || buf.cmp("H4") ||
                           buf.cmp("H5") || buf.cmp("H6") || buf.cmp("UL") ||
                           buf.cmp("OL") || buf.cmp("DL") || buf.cmp("LI") ||
                           buf.cmp("DD") || buf.cmp("DT") || buf.cmp("HR") ||
                           buf.cmp("PRE") || buf.cmp("TABLE")) {
                    block->end = start;
                    line = do_align(block, line, xx, newalign, links);
                    ...
                    if (buf.cmp("UL") || buf.cmp("OL") || buf.cmp("DL")) {
                        ...
                        switch (get_align(attrs, talign)) {
                            ...
                            case CENTER:
                                table_offset =
                                    (hsize_ - table_width) / 2 - textsize_;
                                break;
                    ...
                    if ((tolower(buf[0]) == 'h' && isdigit(buf[1])) ||
                        buf.cmp("DD") || buf.cmp("DT") || buf.cmp("P"))
                        yy += fsize + 2;
                    else if (buf.cmp("HR")) {
                        hh += 2 * fsize;
                        yy += fsize;
                    ...
                    if (buf.cmp("CENTER"))
                        ...
                        newalign = get_align(attrs, talign);
                           ...
                           buf.cmp("/PRE") || buf.cmp("/UL") ||
                           buf.cmp("/OL") || buf.cmp("/DL") ||
                           buf.cmp("/TABLE")) {
                    line = do_align(block, line, xx, newalign, links);
                    xx = block->x;
                    block->end = ptr;
                    ...
                    //#if defined(__GNUC__)
                    //#warning FIXME this isspace & 255 test will probably not
                    //work on a utf8 stream... And we use it everywhere! #endif
                    ///*__GNUC__*/
                    while (isspace((*ptr) & 255)) ptr++;
                    ...
                    rc = get_color(
                        get_attr(attrs, "BGCOLOR", attr, sizeof(attr)), tc);
                    ...
                    for (cell = blocks_ + row + 1; cell <= block; cell++)
                        if ((cell->y + cell->h) > yy) yy = cell->y + cell->h;
                    ...
                    for (i = 0; i < column; i++) xx += columns[i] + 6;
                    ...
                    line = 0;
                    newalign = get_align(
                        attrs, tolower(buf[1]) == 'h' ? CENTER : LEFT);
                    talign = newalign;
                    ...
                    block->bgcolor = get_color(
                        get_attr(attrs, "BGCOLOR", attr, sizeof(attr)), rc);
                } else if ((buf.cmp("/TD") || buf.cmp("/TH")) && row) {
                    line = do_align(block, line, xx, newalign, links);
                    popfont(font, fsize, fcolor);
                    xx = margins.pop();
                    ...
                    if (get_attr(attrs, "SIZE", attr, sizeof(attr)) != NULL) {
                        if (isdigit(attr[0] & 255)) {
                            // Absolute size
                            fsize =
                                (int)(textsize_ * pow(1.2, atoi(attr) - 3.0));
                        } else {
                            // Relative size
                    ...
                    pushfont(font = FL_COURIER_ITALIC, fsize);
                else if (buf.cmp("/B") || buf.cmp("/STRONG") || buf.cmp("/I") ||
                         buf.cmp("/EM") || buf.cmp("/CODE") || buf.cmp("/TT") ||
                         buf.cmp("/KBD") || buf.cmp("/VAR"))
                    popfont(font, fsize, fcolor);
                else if (buf.cmp("IMG")) {
                    Fl_Shared_Image *img = 0;
        ...
        if (buf.size() > 0 && !head) {
            ...
            //    printf("line = %d, xx = %d, ww = %d, block->x = %d, block->w =
            //    %d\n",
            //	   line, xx, ww, block->x, block->w);
            ...
            if (linkdest[0]) add_link(linkdest, xx, yy - fsize, ww, fsize);
    ...
    if (hsize_ > (w() - dw)) {
        ...
        if (size_ < (h() - dh)) {
            ...
            scrollbar_.show();
            scrollbar_.resize(x() + w() - ss - dx, y() + Fl::box_dy(b), ss,
                              h() - ss - Fl::box_dh(b));
            hscrollbar_.resize(x() + Fl::box_dx(b), y() + h() - ss - dy,
                               w() - ss - Fl::box_dw(b), ss);
        }
...
void Fl_Help_View::format_table(
    int *table_width,   // O - Total table width
    ...
    *table_width = 0;
    ...
    for (ptr = table, column = -1, width = 0, incell = 0; *ptr;) {
        if ((*ptr == '<' || isspace((*ptr) & 255)) && buf.size() > 0 &&
            ...
            if (temp_width > minwidths[column]) minwidths[column] = temp_width;
        ...
        if (*ptr == '<') {
            ...
            if (buf.cmp("BR") || buf.cmp("HR")) {
                ...
                break;
                     ...
                     buf.cmp("H5") || buf.cmp("H6") || buf.cmp("UL") ||
                     buf.cmp("OL") || buf.cmp("DL") || buf.cmp("LI") ||
                     buf.cmp("DD") || buf.cmp("DT") || buf.cmp("PRE")) {
                width = 0;
                needspace = 0;
                ...
                pushfont(font, fsize);
                       ...
                       buf.cmp("/H2") || buf.cmp("/H3") || buf.cmp("/H4") ||
                       buf.cmp("/H5") || buf.cmp("/H6") || buf.cmp("/PRE") ||
                ...
                //        printf("%s column = %d, colspan = %d, num_columns =
                //        %d\n",
                ...
                //        printf("BEFORE column = %d, colspan = %d, num_columns
                //        = %d\n",
                ...
                //        printf("AFTER column = %d, colspan = %d, num_columns =
                //        %d\n",
                //	       column, colspan, num_columns);
                ...
                if (iwidth > minwidths[column]) minwidths[column] = iwidth;

                width += iwidth;
                if (needspace) width += (int)fl_width(' ');
    ...
    for (column = 0; column < num_columns; column++)
        printf("    columns[%d] = %d, minwidths[%d] = %d\n", column,
               columns[column], column, minwidths[column]);
#endif  // DEBUG
    ...
    int scrollsize = scrollbar_size_ ? scrollbar_size_ : Fl::scrollbar_size();
    if (scale_width == 0) {
        if (width > (hsize_ - scrollsize))
            scale_width = hsize_ - scrollsize;
        else
            scale_width = width;
    }
                        ...
                        Fl_Color c)     // I - Default color value
{
    ...
    static const struct {  // Color name table
        const char *name;
        int r, g, b;
    } colors[] = {{"black", 0x00, 0x00, 0x00},   {"red", 0xff, 0x00, 0x00},
...
Fl_Help_Link *Fl_Help_View::find_link(int xx, int yy) {
    ...
    for (i = nlinks_, linkp = links_; i > 0; i--, linkp++) {
        if (xx >= linkp->x && xx < linkp->w && yy >= linkp->y && yy < linkp->h)
            break;
    }
    return i ? linkp : 0L;
}
    ...
    if (strcmp(linkp->filename, filename_) != 0 && linkp->filename[0]) {
        ...
        if (strchr(directory_, ':') != NULL &&
            ...
            if (directory_[0])
                ...
                fl_getcwd(dir, sizeof(dir));
                snprintf(temp, sizeof(temp), "file:%s/%s", dir,
                         linkp->filename);
            }
        } else
...
void Fl_Help_View::end_selection(int clipboard) {
    ...
    // convert the select part of our html text into some kind of somewhat
    // readable UTF-8 and store it in the selection buffer
    int p = 0;
    char pre = 0;
    ...
    for (;;) {
        ...
        if (n > selection_last)
            break;  // stop parsing html after end of selection
    }
    *d = 0;
...
Fl_Help_View::handle(int event)  // I - Event to handle
{
    ...
    switch (event) {
        ...
        case FL_DRAG:
            if (linkp) {
                if (Fl::event_is_click()) {
                    ...
                    fl_cursor(
                        FL_CURSOR_DEFAULT);  // should be "FL_CURSOR_CANCEL" if
                                             // we had it
                }
                return 1;
            }
        ...
        case FL_SHORTCUT: {
            int mods =
                Fl::event_state() & (FL_META | FL_CTRL | FL_ALT | FL_SHIFT);
            if (mods == FL_COMMAND) {
                switch (Fl::event_key()) {
                    case 'a':
                        select_all();
...
Fl_Help_View::Fl_Help_View(int xx,  // I - Left position
    ...
    : Fl_Group(xx, yy, ww, hh, l),
      scrollbar_(xx + ww - Fl::scrollbar_size(), yy, Fl::scrollbar_size(),
                 hh - Fl::scrollbar_size()),
      hscrollbar_(xx, yy + hh - Fl::scrollbar_size(), ww - Fl::scrollbar_size(),
                  Fl::scrollbar_size()) {
...
Fl_Help_View::load(
    const char *f)  // I - Filename to load (may also have target)
{
    ...
    if (strncmp(f, "ftp:", 4) == 0 || strncmp(f, "http:", 5) == 0 ||
        strncmp(f, "https:", 6) == 0 || strncmp(f, "ipp:", 4) == 0 ||
        strncmp(f, "mailto:", 7) == 0 || strncmp(f, "news:", 5) == 0) {
        ...
        if (fl_open_uri(f, urimsg, sizeof(urimsg)) == 0) {
            ...
            // Note: We do not support Windows backslashes, since they are
            // illegal
            //       in URLs...
                          ...
                          int hh)  // I - New height
{
    ...
    scrollbar_.resize(x() + w() - scrollsize - Fl::box_dw(b) + Fl::box_dx(b),
                      y() + Fl::box_dy(b), scrollsize,
                      h() - scrollsize - Fl::box_dh(b));
...
void Fl_Help_View::topline(const char *n)  // I - Target name
{
    ...
    target = (Fl_Help_Target *)bsearch(&key, targets_, ntargets_,
                                       sizeof(Fl_Help_Target),
                                       (compare_func_t)compare_targets);

    if (target != NULL) topline(target->y);
}
...
*/
void Fl_Help_View::topline(int top)  // I - Top line number
{
    if (!value_) return;
...
*/
void Fl_Help_View::leftline(int left)  // I - Left position
{
    if (!value_) return;
...
*/
void Fl_Help_View::value(const char *val)  // I - Text to view
{
    clear_selection();
    free_data();
    set_changed();
...
quote_char(const char *p) {  // I - Quoted string
    ...
    for (i = (int)(sizeof(names) / sizeof(names[0])), nameptr = names; i > 0;
         i--, nameptr++)
