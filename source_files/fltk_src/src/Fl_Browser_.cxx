...
static void hscrollbar_callback(Fl_Widget *s, void *) {
    ((Fl_Browser_ *)(s->parent()))
        ->hposition(int(((Fl_Scrollbar *)s)->value()));
}
...
void Fl_Browser_::resize(int X, int Y, int W, int H) {
    ...
    bbox(X, Y, W, H);
    scrollbar.resize(scrollbar.align() & FL_ALIGN_LEFT ? X - scrollsize : X + W,
                     Y, scrollsize, H);
    hscrollbar.resize(X,
                      scrollbar.align() & FL_ALIGN_TOP ? Y - scrollsize : Y + H,
                      W, scrollsize);
    max_width = 0;
}
...
/**
  ...
  list off the left edge of the screen. \param[in] pos The horizontal position
  (in pixels) to scroll the browser to. \see position(), hposition()
*/
...
/**
  ...
  displayed. Checks to see if the item's vertical position is within the top and
  bottom edges of the display window. This does NOT take into account the
  hide()/show() status of the widget or item. \param[in] item The item to check
...
void Fl_Browser_::display(void *item) {
    ...
    // 2nd special case - want to display item already displayed at top of
    // browser?
    if (l == item) {
        position(real_position_ + Y);
        return;
    }  // scroll up a bit
        ...
        if (l) {
            ...
            if (l == item) {
                if (Y <= H) {        // it is visible or right at bottom
                    ...
                    if (Y > 0)
                        position(real_position_ + Y);  // scroll down a bit
                } else {
...
J1:
    ...
    for (; l && yy < H; l = item_next(l)) {
        ...
        if ((damage() & (FL_DAMAGE_SCROLL | FL_DAMAGE_ALL)) || l == redraw1 ||
            ...
            if (item_selected(l)) {
                ...
                draw_box(box() ? box() : FL_DOWN_BOX, x(), y(), w(), h(),
                         color());
                fl_pop_clip();
            }
    ...
    if (scrollbar.visible()) {
        scrollbar.damage_resize(
            scrollbar.align() & FL_ALIGN_LEFT ? X - scrollsize : X + W, Y,
            scrollsize, H);
    ...
    if (hscrollbar.visible()) {
        hscrollbar.damage_resize(
            X, scrollbar.align() & FL_ALIGN_TOP ? Y - scrollsize : Y + H, W,
            scrollsize);
...
/**
  ...
  selection. This method does not actually replace the item, but handles the
  follow up bookkeeping after the item has just been replaced. \param[in] a Item
  being replaced \param[in] b Item to replace 'a'
*/
...
/**
  ...
  selection. This method does not actually swap items, but handles the follow up
    ...
    switch (event) {
        case FL_PUSH:
            ...
            if (type() == FL_NORMAL_BROWSER || !top_)
                ...
                TOGGLE:
                    ...
                    // see which of the new item or previous selection is
                    // earlier, by searching from the previous forward for this
                    // one:
                    int down;
                    if (!l)
                        down = 1;
        ...
        case FL_DRAG:
            ...
            if (type() == FL_NORMAL_BROWSER || !top_)
                ...
                void *l1 = selection_;
                void *l = (Fl::event_x() < x() || Fl::event_x() > x() + w())
                              ? selection_
                              : find_item(my);
                change = (l != l1);
...
/**
  ...
  call. \param[in] flags FL_SORT_ASCENDING -- sort in ascending order\n
...
void Fl_Browser_::sort(int flags) {
    ...
    //
    int i, j, n = -1,
              desc = ((flags & FL_SORT_DESCENDING) == FL_SORT_DESCENDING);
...
*/
int Fl_Browser_::incr_height() const { return item_quick_height(item_first()); }
...
int Fl_Browser_::full_height() const {
    int t = 0;
    for (void *p = item_first(); p; p = item_next(p)) t += item_quick_height(p);
    return t;
}
...
*/
int Fl_Browser_::full_width() const { return max_width; }
...
/**
  ...
  default) or 0 (de-select). \param[in] item The item to be selected \param[in]
  val The optional selection state; 1=select, 0=de-select.\n The default is to
  select the item (1).
*/
