...
static fl_gmi_func fl_gmi =
    NULL;  // used to get a proc pointer for GetMonitorInfoA
...
static BOOL CALLBACK screen_cb(HMONITOR mon, HDC, LPRECT r, LPARAM) {
    ...
    if (fl_gmi(mon, &mi)) {
        ...
        // If we also want to record the work area, we would also store
        // mi.rcWork at this point
...
static void screen_init() {
    ...
    if (hMod) {
        ...
        fl_edm_func fl_edm =
            (fl_edm_func)GetProcAddress(hMod, "EnumDisplayMonitors");
        ...
        if (fl_edm) {
            ...
            if (fl_gmi) {
                ...
                //      (but we use our self-acquired function pointer instead)
                //      NOTE: num_screens is incremented in screen_cb so we must
                //      first reset it here...
                num_screens = 0;
...
static void screen_init() {
    ...
    if (!XRRSizes_f) {
        ...
        if (!libxrandr_addr)
            libxrandr_addr = dlopen("/opt/X11/lib/libXrandr.dylib", RTLD_LAZY);
#endif
        if (libxrandr_addr)
            XRRSizes_f = (XRRSizes_type)dlsym(libxrandr_addr, "XRRSizes");
    }
    if (XRRSizes_f) {
        int nscreens;
        ...
        if (nscreens >
            0) {  // Note: XRRSizes() *may* return nscreens == 0, see docs
    ...
    if (XineramaIsActive(fl_display)) {
        XineramaScreenInfo *xsi =
            XineramaQueryScreens(fl_display, &num_screens);
...
#ifndef FL_DOXYGEN
void Fl::call_screen_init() { screen_init(); }
#endif
...
/**
  ...
  menubar/taskbar \param[out]  X,Y,W,H the corresponding screen bounding box
  \param[in] n the screen number (0 to Fl::screen_count() - 1)
  \see void screen_xywh(int &x, int &y, int &w, int &h, int mx, int my)
*/
...
static inline float fl_intersection(int x1, int y1, int w1, int h1, int x2,
