...
class FLDropTarget : public IDropTarget {
   ...
   public:
    ...
    HRESULT STDMETHODCALLTYPE DragEnter(IDataObject *pDataObj,
        ...
        /* Tricky point here: Not DPI-aware applications use different units for
         the 'POINTL pt' argument of the DragEnter, DragOver, and Drop member
         functions. DragEnter receives the mouse coordinates in unscaled screen
         units, whereas DragOver and Drop receive the mouse coordinates in
         scaled units. This looks like a Windows bug because these apps are
         supposed to always receive scaled units from the OS. Therefore, we ask
         here for the mouse coordinates.
        */
        POINT mp;
        GetCursorPos(&mp);
        pt.x = mp.x;
        ...
        // check if FLTK handles this drag and return if it can't (i.e. BMP drag
        // without filename)
        POINT ppt;
        ...
        if (fillCurrentDragData(pDataObj)) {
            // FLTK has no mechanism yet for the different drop effects, so we
            // allow move and copy
    ...
    HRESULT STDMETHODCALLTYPE DragOver(DWORD /*grfKeyState*/, POINTL pt,
        ...
        if (fillCurrentDragData(0)) {
            // Fl_Group will change DND_DRAG into DND_ENTER and DND_LEAVE if
            // needed
        ...
        py = pt.y;
        lastEffect = *pdwEffect;
        // show insert position if dnd'ing in the same window/process (STR
        // #3209)
        Fl::flush();
        return S_OK;
    }
    ...
    HRESULT STDMETHODCALLTYPE Drop(IDataObject *data, DWORD /*grfKeyState*/,
        ...
        if (!fl_dnd_target_window) return S_OK;
        ...
        if (!Fl::handle(FL_DND_RELEASE, target)) return S_OK;
        ...
        if (fillCurrentDragData(data)) {
            ...
            *b = 0;
            ...
            Fl::belowmouse()->handle(
                Fl::e_number =
                    FL_PASTE);  // e_text will be invalid after this call
   ...
   private:
    ...
    static char fillCurrentDragData(IDataObject *data) {
        ...
        if (!data) return currDragResult;
        // shortcut through this whole procedure if this is still the same drag
        // event
        // (* this is safe, because 'currDragRef' is cleared on Leave and Drop
        // events)
        if (data == currDragRef) return currDragResult;
        ...
        if (data->GetData(&fmt, &medium) == S_OK) {
            ...
            fl_utf8fromwc(currDragData, currDragSize + 1, wstuff,
                          srclen + 1);  // include null-byte
...
class FLEnum : public IEnumFORMATETC {
   public:
    ...
    ULONG __stdcall AddRef(void) { return InterlockedIncrement(&m_lRefCount); }
    ...
    virtual ~FLEnum(void) { n = 0; }
};

/**
  This is the actual object that FLTK can drop somewhere.

  The implementation is minimal, but it should work with all decent Win32 drop
  targets
*/
   ...
   public:
    ...
    HRESULT STDMETHODCALLTYPE GetData(FORMATETC *pformatetcIn,
        ...
        if ((pformatetcIn->dwAspect & DVASPECT_CONTENT) &&
            ...
            fl_utf8toUtf16(fl_selection_buffer[0], fl_selection_length[0],
                           (unsigned short *)pMem, utf16_len + 1);
            //      HGLOBAL gh = GlobalAlloc( GHND| GMEM_SHARE,
            //                            (fl_selection_length[0]+4) *
            //                            sizeof(short)
            //                            + sizeof(DROPFILES));
    ...
    //  HRESULT STDMETHODCALLTYPE EnumFormatEtc( DWORD dir, IEnumFORMATETC**
    //  ppenumFormatEtc) {
    ...
    HRESULT STDMETHODCALLTYPE GetDataHere(FORMATETC * /*pformatetcIn*/,
                                          STGMEDIUM * /*pmedium*/) {
        return E_NOTIMPL;
    }
    HRESULT STDMETHODCALLTYPE GetCanonicalFormatEtc(FORMATETC * /*in*/,
                                                    FORMATETC * /*out*/) {
        return E_NOTIMPL;
    }
    ...
    EnumFormatEtc(DWORD /*dir*/, IEnumFORMATETC ** /*ppenumFormatEtc*/) {
        return E_NOTIMPL;
    }
    //  HRESULT STDMETHODCALLTYPE EnumFormatEtc( DWORD dir, IEnumFORMATETC**
    //  ppenumFormatEtc) {*ppenumFormatEtc = m_EnumF; return S_OK;}
...
int Fl::dnd() {
    ...
    HRESULT ret = DoDragDrop(
        fdo, fds, DROPEFFECT_MOVE | DROPEFFECT_LINK | DROPEFFECT_COPY,
        &dropEffect);
