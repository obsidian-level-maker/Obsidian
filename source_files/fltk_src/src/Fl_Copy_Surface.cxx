...
Fl_Quartz_Surface_::Fl_Quartz_Surface_(int w, int h)
    : Fl_System_Printer(), width(w), height(h) {}
...
Fl_GDI_Surface_::~Fl_GDI_Surface_() { delete driver(); }
...
void Fl_GDI_Surface_::translate(int x, int y) {
    ...
    if (depth < sizeof(origins) / sizeof(POINT))
        depth++;
    else
        Fl::warning("Fl_GDI_Surface_: translate stack overflow!");
}
...
Fl_Copy_Surface::Fl_Copy_Surface(int w, int h) : Fl_Surface_Device(NULL) {
    ...
    driver(helper->driver());
    oldgc = fl_gc;
    // exact computation of factor from screen units to EnhMetaFile units (0.01
    // mm)
    HDC hdc = GetDC(NULL);
    ...
    int dhr =
        GetDeviceCaps(hdc, DESKTOPHORZRES);  // true number of pixels on display
    ReleaseDC(NULL, hdc);
    ...
    rect.left = 0;
    rect.top = 0;
    rect.right = (LONG)(w * factorw / scaling);
    rect.bottom = (LONG)(h * factorh / scaling);
...
size_t Fl_Copy_Surface::MyPutBytes(void *info, const void *buffer,
                                   size_t count) {
...
#endif
    {
        static CGDataConsumerCallbacks callbacks = {Fl_Copy_Surface::MyPutBytes,
                                                    NULL};
...
void Fl_Copy_Surface::draw_decorated_window(Fl_Window *win, int delta_x,
                                            int delta_y) {
    ...
    if (win->border()) {
        ...
        if (layer) {
            ...
            if (fl_mac_os_version < 101300) {
                // for unknown reason, rendering the layer to the
                // Fl_Copy_Surface pdf graphics context does not work; we use an
                // auxiliary bitmap context
                CGContextRef auxgc =
                    CGBitmapContextCreate(NULL, win->w(), bt, 8, 0, cspace,
                ...
                Fl_RGB_Image *image = new Fl_RGB_Image(
                    (const uchar *)CGBitmapContextGetData(auxgc), win->w(), bt,
                    4,
            ...
            CGImageRef img =
                Fl_X::CGImage_from_window_rect(win, 0, -bt, win->w(), bt);
...
class Fl_translated_Xlib_Graphics_Driver_ : public Fl_Xlib_Graphics_Driver {
    int offset_x, offset_y;  // translation between user and graphical
                             // coordinates: graphical = user + offset
    unsigned depth;          // depth of translation stack
    int stack_x[20],
        stack_y[20];  // translation stack allowing cumulative translations
   public:
    ...
    void translate_all(int dx,
                       int dy) {  // reversibly adds dx,dy to the offset between
                                  // user and graphical coordinates
        ...
        push_matrix();
        translate(dx, dy);
        if (depth < sizeof(stack_x) / sizeof(int))
            depth++;
        else
            Fl::warning("%s: translate stack overflow!", class_id);
    }
    ...
    void xyline(int x, int y, int x1) {
        Fl_Xlib_Graphics_Driver::xyline(x + offset_x, y + offset_y,
                                        x1 + offset_x);
    }
    ...
    void yxline(int x, int y, int y1) {
        Fl_Xlib_Graphics_Driver::yxline(x + offset_x, y + offset_y,
                                        y1 + offset_y);
    }
    void yxline(int x, int y, int y1, int x2) {
        Fl_Xlib_Graphics_Driver::yxline(x + offset_x, y + offset_y,
                                        y1 + offset_y, x2 + offset_x);
    }
    ...
    void line(int x, int y, int x1, int y1) {
        Fl_Xlib_Graphics_Driver::line(x + offset_x, y + offset_y, x1 + offset_x,
                                      y1 + offset_y);
    }
    ...
    void draw(int angle, const char *str, int n, int x, int y) {
        Fl_Xlib_Graphics_Driver::draw(angle, str, n, x + offset_x,
                                      y + offset_y);
    }
    ...
    void draw(Fl_RGB_Image *img, int XP, int YP, int WP, int HP, int cx,
              int cy) {
        XP += offset_x;
        YP += offset_y;
        translate_all(-offset_x, -offset_y);
    ...
    void draw_image(const uchar *buf, int X, int Y, int W, int H, int D = 3,
                    int L = 0) {
        X += offset_x;
        Y += offset_y;
        translate_all(-offset_x, -offset_y);
    ...
    void draw_image_mono(const uchar *buf, int X, int Y, int W, int H,
                         int D = 1, int L = 0) {
        X += offset_x;
        Y += offset_y;
        translate_all(-offset_x, -offset_y);
    ...
    void draw_image_mono(Fl_Draw_Image_Cb cb, void *data, int X, int Y, int W,
                         int H, int D = 1) {
        X += offset_x;
        Y += offset_y;
        translate_all(-offset_x, -offset_y);
    ...
    void copy_offscreen(int x, int y, int w, int h, Fl_Offscreen pixmap,
                        int srcx, int srcy) {
        Fl_Xlib_Graphics_Driver::copy_offscreen(x + offset_x, y + offset_y, w,
                                                h, pixmap, srcx, srcy);
    }
    ...
    int not_clipped(int x, int y, int w, int h) {
        return Fl_Xlib_Graphics_Driver::not_clipped(x + offset_x, y + offset_y,
                                                    w, h);
    };
    int clip_box(int x, int y, int w, int h, int &X, int &Y, int &W, int &H) {
        int retval = Fl_Xlib_Graphics_Driver::clip_box(
            x + offset_x, y + offset_y, w, h, X, Y, W, H);
        X -= offset_x;
        Y -= offset_y;
        return retval;
    }
    ...
    void polygon(int x0, int y0, int x1, int y1, int x2, int y2) {
        Fl_Xlib_Graphics_Driver::polygon(x0 + offset_x, y0 + offset_y,
                                         x1 + offset_x, y1 + offset_y,
                                         x2 + offset_x, y2 + offset_y);
    }
    void polygon(int x0, int y0, int x1, int y1, int x2, int y2, int x3,
                 int y3) {
        Fl_Xlib_Graphics_Driver::polygon(
            x0 + offset_x, y0 + offset_y, x1 + offset_x, y1 + offset_y,
            x2 + offset_x, y2 + offset_y, x3 + offset_x, y3 + offset_y);
    }
    void loop(int x0, int y0, int x1, int y1, int x2, int y2) {
        Fl_Xlib_Graphics_Driver::loop(x0 + offset_x, y0 + offset_y,
                                      x1 + offset_x, y1 + offset_y,
                                      x2 + offset_x, y2 + offset_y);
    }
    void loop(int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3) {
        Fl_Xlib_Graphics_Driver::loop(
            x0 + offset_x, y0 + offset_y, x1 + offset_x, y1 + offset_y,
            x2 + offset_x, y2 + offset_y, x3 + offset_x, y3 + offset_y);
    }
...
const char *Fl_translated_Xlib_Graphics_Driver_::class_id =
    "Fl_translated_Xlib_Graphics_Driver_";
...
Fl_Xlib_Surface_::~Fl_Xlib_Surface_() { delete driver(); }
