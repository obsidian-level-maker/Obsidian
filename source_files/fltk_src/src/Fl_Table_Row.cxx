...
int Fl_Table_Row::handle(int event) {
    ...
    // 05/18/03 if ( ret ) { _last_y = Fl::event_y(); return(1); }	// base
    // class 'handled' it (eg. column resize)

    int shiftstate = (_event_state & FL_CTRL)    ? FL_CTRL
                     : (_event_state & FL_SHIFT) ? FL_SHIFT
                                                 : 0;
    ...
    switch (event) {
            ...
            if (_event_button == 1) {
                ...
                if (context == CONTEXT_CELL) {
                    ...
                    switch (shiftstate) {
                        ...
                        default:
                            select_all_rows(
                                0);  // clear all previous selections
                            select_row(R, 1);
                            break;
                    }
        ...
        case FL_DRAG: {
            if (_dragging_select) {
                ...
                int offbot =
                    _last_y - (toy + toh);  // >0 if off bottom of table
                ...
                if (context == CONTEXT_CELL) {
                    switch (shiftstate) {
                        case FL_CTRL:
                            if (R !=
                                _last_row) {  // toggle if dragged to new row
        ...
        case FL_RELEASE:
            if (_event_button == 1) {
                ...
                //
                int databot = tiy + table_h, dataright = tix + table_w;
                if ((_last_push_x > dataright && _event_x > dataright) ||
