 ...
 */
void Fl_Color_Chooser::hsv2rgb(double H, double S, double V, double &R,
                               double &G, double &B) {
    if (S < 5.0e-6) {
        R = G = B = V;
    } else {
 ...
 */
void Fl_Color_Chooser::rgb2hsv(double R, double G, double B, double &H,
                               double &S, double &V) {
    double maxv = R > G ? R : G;
    if (B > maxv) maxv = B;
    V = maxv;
...
int Flcc_Value_Input::format(char *buf) {
    ...
    if (c->mode() == M_HEX)
        return sprintf(buf, "0x%02X", int(value()));
    else
...
int Flcc_HueBox::handle(int e) {
    ...
    switch (e) {
        ...
        case FL_DRAG: {
            double Xf, Yf, H, S;
            Xf = (Fl::event_x() - x() - Fl::box_dx(box())) /
                 double(w() - Fl::box_dw(box()));
            Yf = (Fl::event_y() - y() - Fl::box_dy(box())) /
                 double(h() - Fl::box_dh(box()));
            tohs(Xf, Yf, H, S);
...
void Flcc_HueBox::draw() {
    ...
    Fl_Color_Chooser *c = (Fl_Color_Chooser *)parent();
#ifdef CIRCLE
    int X = int(.5 * (cos(c->hue() * (M_PI / 3.0)) * c->saturation() + 1) *
                (w1 - 6));
    int Y = int(.5 * (1 - sin(c->hue() * (M_PI / 3.0)) * c->saturation()) *
                (h1 - 6));
#else
    ...
    draw_box(FL_UP_BOX, x1 + X, yy1 + Y, 6, 6,
             Fl::focus() == this ? FL_FOREGROUND_COLOR : FL_GRAY);
    px = X;
    py = Y;
}
#endif  // !FL_DOXYGEN
...
int Flcc_ValueBox::handle(int e) {
    ...
    switch (e) {
        ...
        case FL_DRAG: {
            double Yf;
            Yf = 1 - (Fl::event_y() - y() - Fl::box_dy(box())) /
                         double(h() - Fl::box_dh(box()));
...
void Flcc_ValueBox::draw() {
    ...
    if (Y < 0)
        ...
        Y = h1 - 6;
    draw_box(FL_UP_BOX, x1, yy1 + Y, w1, 6,
             Fl::focus() == this ? FL_FOREGROUND_COLOR : FL_GRAY);
    py = Y;
}
#endif  // !FL_DOXYGEN
...
void ColorChip::draw() {
    ...
    fl_rectf(x() + Fl::box_dx(box()), y() + Fl::box_dy(box()),
             w() - Fl::box_dw(box()), h() - Fl::box_dh(box()), r, g, b);
}
...
/**
  ...
  \param[in] cmode Optional mode for color chooser. See mode(int). Default -1 if
  none (rgb mode). \retval 1 if user confirms the selection \retval 0 if user
  cancels the dialog \relates Fl_Color_Chooser
 */
