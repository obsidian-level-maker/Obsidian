...
/**
 ...
 PostScript file.
 *
 Opens a file dialog entitled with Fl_PostScript_File_Device::file_chooser_title
 to select an output PostScript file.
 @param pagecount The total number of pages to be created. Use 0 if this number
 is unknown when this function is called.
...
int Fl_PostScript_File_Device::start_job(
    int pagecount, enum Fl_Paged_Device::Page_Format format,
...
extern "C" {
static int dont_close(FILE *f) { return 0; }
}
 ...
 *
 ...
 and that should not be closed until after end_job() has been called.
 @param pagecount The total number of pages to be created. Use 0 if this number
 is unknown when this function is called.
 @param format Desired page format.
 @param layout Desired page layout.
 ...
 */
int Fl_PostScript_File_Device::start_job(
    FILE *ps_output, int pagecount, enum Fl_Paged_Device::Page_Format format,
    enum Fl_Paged_Device::Page_Layout layout) {
    Fl_PostScript_Graphics_Driver *ps = driver();
    ps->output = ps_output;
...
/** Shields output PostScript data from modifications of the current locale.
 It typically avoids PostScript errors caused if the current locale uses comma
 instead of dot as "decimal point". \param format  directives controlling output
 PostScript data \return value returned by vfprintf() call
 */
...
static const char *prolog_2 =  // prolog relevant only if lang_level >1
    ...
    "/CII {GS /inter exch def /py exch def /px exch def /sy exch def /sx exch "
    "def \n"
    "translate \n"
    "sx sy scale\n"
    ...
    // gray image dict
    "/GII {GS /inter exch def /py exch def /px exch def /sy exch def /sx exch "
    "def \n"
    "translate \n"
    "sx sy scale\n"
    ...
    // Create a custom PostScript font derived from PostScript standard text
    // fonts The encoding of this custom font is as follows: 0000-00FF coincides
    // with Unicode, that is to ASCII + Latin-1 0100-017F  coincides with
    // Unicode, that is to Latin Extended-A 0180-01A6  encodes miscellaneous
    // characters present in PostScript standard text fonts

    // use ISOLatin1Encoding for all text fonts
    "/ToISO { dup findfont dup length dict copy begin /Encoding "
    ...
    "/Times-Roman ToISO /Times-Bold ToISO /Times-Italic ToISO "
    "/Times-BoldItalic ToISO \n"

    // define LatinExtA, the encoding of Latin-extended-A + some additional
    // characters see
    // http://partners.adobe.com/public/developer/en/opentype/glyphlist.txt for
    // their names
    "/LatinExtA \n"
    "[ "
    ...
    " /Itilde  /itilde  /Imacron  /imacron  /Ibreve  /ibreve  /Iogonek  "
    "/iogonek /Idotaccent  /dotlessi  \n"
    ...
    "/ToLatinExtA { findfont dup length dict copy begin /Encoding LatinExtA "
    ...
    "/CourierExt /Courier ToLatinExtA /Courier-BoldExt /Courier-Bold "
    "ToLatinExtA  \n"
    ...
    "/FontMatrix [1  0  0  1  0  0] def /FMapType 6 def /Encoding [ 0 1 0 ] "
    "def\n"
    ...
    "/Courier-BoldOblique2B /Courier-BoldObliqueExt /Courier-BoldOblique "
    "To2byte \n"
...
static const char *prolog_2_pixmap =  // prolog relevant only if lang_level == 2
                                      // for pixmaps/masked color images
...
static const char *prolog_3 =  // prolog relevant only if lang_level >2
    ...
    // masked color images
    "/CIM {GS /inter exch def /my exch def /mx exch def /py exch def /px exch "
    "def /sy exch def /sx exch def \n"
    "translate \n"
    "sx sy scale\n"
    "/DeviceRGB setcolorspace\n"
    ...
    //  masked gray images
    "/GIM {GS /inter exch def /my exch def /mx exch def /py exch def /px exch "
    "def /sy exch def /sx exch def \n"
    "translate \n"
    "sx sy scale\n"
    "/DeviceGray setcolorspace\n"
...
int Fl_PostScript_Graphics_Driver::start_postscript(
    int pagecount, enum Fl_Paged_Device::Page_Format format,
    enum Fl_Paged_Device::Page_Layout layout)
// returns 0 iff OK
{
    ...
    if (lang_level_ > 2) fputs(prolog_3, output);
    if (lang_level_ >= 3) {
...
void Fl_PostScript_Graphics_Driver::reset() {
    ...
    linestyle_ = FL_SOLID;
    strcpy(linedash_, "");
    Clip *c = clip_;  ////just not to have memory leaks for badly writen code
                      ///(forgotten clip popping)
...
void Fl_PostScript_Graphics_Driver::line(int x0, int y0, int x1, int y1, int x2,
                                         int y2) {
    fprintf(output, "GS\n");
    fprintf(output, "BP\n");
    fprintf(output, "%i %i MT\n", x0, y0);
...
void Fl_PostScript_Graphics_Driver::loop(int x0, int y0, int x1, int y1, int x2,
                                         int y2) {
    fprintf(output, "GS\n");
    fprintf(output, "BP\n");
    fprintf(output, "%i %i MT\n", x0, y0);
...
void Fl_PostScript_Graphics_Driver::polygon(int x0, int y0, int x1, int y1,
                                            int x2, int y2) {
    fprintf(output, "GS\n");
    fprintf(output, "BP\n");
    fprintf(output, "%i %i MT\n", x0, y0);
...
void Fl_PostScript_Graphics_Driver::point(int x, int y) { rectf(x, y, 1, 1); }
...
void Fl_PostScript_Graphics_Driver::line_style(int style, int width,
                                               char *dashes) {
    // line_styled_=1;
    ...
    // dashes_= dashes;
    if (dashes) {
        if (dashes != linedash_) strcpy(linedash_, dashes);
...
static const char *_fontNames[] = {"Helvetica2B",
                                   "Helvetica-Bold2B",
                                   "Helvetica-Oblique2B",
...
#if defined(USE_X11)
#if USE_XFT
        // Xft font height is sometimes larger than the required size (see STR
        // 2566). Increase the PostScript font size by 15% without exceeding the
        // display font height
...
void Fl_PostScript_Graphics_Driver::text_extents(const char *c, int n, int &dx,
                                                 int &dy, int &w, int &h) {
...
void Fl_PostScript_Graphics_Driver::draw(int angle, const char *str, int n,
                                         int x, int y) {
...
void Fl_PostScript_Graphics_Driver::transformed_draw_extra(const char *str,
                                                           int n, double x,
                                                           double y, int w,
                                                           bool rtl) {
    ...
    fl_color(text_color);
#if defined(USE_X11) && !USE_XFT
    // force seeing this font as new so it's applied to the offscreen graphics
    // context
    ...
    if (rtl)
        fl_rtl_draw(str, n, w2, (int)(h * 0.8));
    else
        fl_draw(str, n, 1, (int)(h * 0.8));
...
static int is_in_table(unsigned utf) {
    unsigned i;
    static unsigned extra_table_roman[] = {
...
void Fl_PostScript_Graphics_Driver::curve(double x, double y, double x1,
...
void Fl_PostScript_Graphics_Driver::arc(double x, double y, double r,
...
void Fl_PostScript_Graphics_Driver::arc(int x, int y, int w, int h, double a1,
                                        double a2) {
    if (w <= 1 || h <= 1) return;
    fprintf(output, "GS\n");
    // fprintf(output, "BP\n");
...
void Fl_PostScript_Graphics_Driver::pop_clip() {
    ...
    fprintf(output, "CR\nCS\n");
    if (clip_ && clip_->w > 0)
        clocale_printf("%g %g %i %i CL\n", clip_->x - 0.5, clip_->y - 0.5,
                       clip_->w, clip_->h);
    // uh, -0.5 is to match screen clipping, for floats there should be
    // something beter
    if (lang_level_ < 3) recover();
}

int Fl_PostScript_Graphics_Driver::clip_box(int x, int y, int w, int h, int &X,
                                            int &Y, int &W, int &H) {
    if (!clip_) {
        X = x;
        Y = y;
...
void Fl_PostScript_File_Device::margins(int *left, int *top, int *right,
                                        int *bottom)  // to implement
{
    Fl_PostScript_Graphics_Driver *ps = driver();
...
void Fl_PostScript_File_Device::origin(int x, int y) {
    ...
    ps->clocale_printf("GR GR GS %d %d TR  %f %f SC %d %d TR %f rotate GS\n",
                       ps->left_margin, ps->top_margin, ps->scale_x,
                       ps->scale_y, x, y, ps->angle);
}
...
void Fl_PostScript_File_Device::rotate(float rot_angle) {
    ...
    ps->clocale_printf("GR GR GS %d %d TR  %f %f SC %d %d TR %f rotate GS\n",
                       ps->left_margin, ps->top_margin, ps->scale_x,
                       ps->scale_y, x_offset, y_offset, ps->angle);
}
...
int Fl_PostScript_File_Device::end_page(void) { return 0; }
...
int Fl_PostScript_Printer::start_job(int pages, int *firstpage, int *lastpage) {
    ...
    if (print_output_mode[0]->value())
        layout = Fl_Paged_Device::PORTRAIT;
    else if (print_output_mode[1]->value())
        layout = Fl_Paged_Device::LANDSCAPE;
    else if (print_output_mode[2]->value())
        layout = Fl_Paged_Device::PORTRAIT;
    else
        layout = Fl_Paged_Device::LANDSCAPE;

    int print_pipe =
        print_choice->value();  // 0 = print to file, >0 = printer (pipe)
    ...
    if (style == SystemV)
        snprintf(command, sizeof(command),
                 "lp -s -d %s -n %d -t '%s' -o media=%s", printer,
                 print_collate_button->value()
                     ? 1
                     : (int)(print_copies->value() + 0.5),
                 "FLTK", media);
    else
        snprintf(command, sizeof(command), "lpr -h -P%s -#%d -T FLTK ", printer,
                 print_collate_button->value()
                     ? 1
                     : (int)(print_copies->value() + 0.5));
