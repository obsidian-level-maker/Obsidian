...
/** Expands a filename containing shell variables and tilde (~).
    ...
    \code
    "~username"               // if 'username' does not exist, result will be
   unchanged
    "~/file"
   ...
   out="/usr/fred/.cshrc" fl_filename_expand(out, sizeof(out), "~/.cshrc"); //
   out="/usr/<yourname>/.cshrc" fl_filename_expand(out, sizeof(out),
   "$TMPDIR/foo.txt");  // out="/var/tmp/foo.txt" \endcode \param[out] to
   resulting expanded filename \param[in]  tolen size of the expanded filename
   buffer \param[in]  from filename containing shell variables \return 0 if no
   change, non zero otherwise
 */
int fl_filename_expand(char *to, int tolen, const char *from) {
    ...
    for (char *a = temp; a < end;) {  // for each slash component
        ...
        if (value) {
            // substitutions that start with slash delete everything before
            // them:
