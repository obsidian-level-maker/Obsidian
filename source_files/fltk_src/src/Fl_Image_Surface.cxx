...
/** Constructor with optional high resolution.
 ...
 \param highres if non-zero, the surface pixel size is twice as high and wide as
 w and h, which is useful to draw it later on a high resolution display (e.g.,
 retina display). This is implemented for the Mac OS platform only. If \p
 highres is non-zero, use Fl_Image_Surface::highres_image() to get the image
 data. \version 1.3.4 and requires compilation with -DFL_ABI_VERSION=10304
 (1.3.3 without the highres parameter)
 */
Fl_Image_Surface::Fl_Image_Surface(int w, int h, int highres)
    : Fl_Surface_Device(NULL) {
...
/** Returns a possibly high resolution image made of all drawings sent to the
 Fl_Image_Surface object. The Fl_Image_Surface object should have been
 constructed with Fl_Image_Surface(W, H, 1). The returned image is scaled to a
 size of WxH drawing units and may have a pixel size twice as wide and high. The
 returned object should be deallocated with Fl_Shared_Image::release() after
 use. \version 1.3.4 and requires compilation with -DFL_ABI_VERSION=10304
 */
...
/** Draws a widget in the image surface
 ...
 \param widget any FLTK widget (e.g., standard, custom, window, GL view) to draw
 in the image \param delta_x and \param delta_y give the position in the image
 of the top-left corner of the widget
 */
...
Fl_Quartz_Flipped_Surface_::Fl_Quartz_Flipped_Surface_(int w, int h)
    : Fl_Quartz_Surface_(w, h) {}
...
void Fl_Image_Surface::draw_decorated_window(Fl_Window *win, int delta_x,
                                             int delta_y) {
...
void Fl_Image_Surface::draw_decorated_window(Fl_Window *win, int delta_x,
                                             int delta_y) {
#ifdef WIN32
    // draw_decorated_window() will change the current drawing surface, and set
    // it back to us; it's necessary to do some cleaning before
    fl_pop_clip();
    RestoreDC(fl_gc, _savedc);
    DeleteDC(fl_gc);
