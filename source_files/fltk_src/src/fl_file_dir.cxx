...
fl_file_chooser(const char *message,   // I - Message in titlebar
    ...
    if (!fc) {
        ...
        if (!fname) {  // null pointer reuses same filename if pattern didn't
                       // change
            if (!same_pattern && fc->value()) {
                ...
                if (p) {
                    // If the filename is "/foo", then the directory will be
                    // "/", not
                    // ""...
                    if (p == retname)
                        retname[1] = '\0';
        ...
        } else if (!*fname) {  // empty filename reuses directory with empty
                               // name
               ...
               int relative)         // I - 0 for absolute
{
    ...
    if (!fc) {
        ...
        fc = new Fl_File_Chooser(
            fname, "*", Fl_File_Chooser::CREATE | Fl_File_Chooser::DIRECTORY,
            message);
        fc->callback(callback, 0);
    } else {
