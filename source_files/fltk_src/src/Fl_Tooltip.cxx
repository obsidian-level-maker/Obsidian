...
Fl_Color Fl_Tooltip::color_ =
    fl_color_cube(FL_NUM_RED - 1, FL_NUM_GREEN - 1, FL_NUM_BLUE - 2);
...
void Fl_TooltipBox::draw() {
    ...
    fl_draw(tip, X, Y, W, H, Fl_Align(FL_ALIGN_LEFT | FL_ALIGN_WRAP), 0,
            draw_symbols_);
}
    ...
    if (!top_win_iconified_()) {  // no tooltip if top win iconified (STR #3157)
        if (!tip || !*tip) {
            ...
            if (condition) {
                ...
                window->layout();
                window->redraw();
                // printf("tooltip_timeout: Showing window %p with tooltip
                // \"%s\"...\n",
                //        window, tip ? tip : "(null)");
                window->show();
            }
...
/**
   ...
   instance if you want the tooltip to reappear when the mouse moves back in)
...
*/
void Fl_Tooltip::enter_area(Fl_Widget *wid, int x, int y, int w, int h,
                            const char *t) {
    (void)x;
    (void)w;
