...
typedef int(WINAPI *fl_wsk_select_f)(int, fd_set *, fd_set *, fd_set *,
...
static HMODULE get_wsock_mod() {
    ...
    if (!s_wsock_mod) {
        ...
        if (s_wsock_mod == NULL)
            Fl::fatal(
                "FLTK Lib Error: %s file not found! Please check your winsock "
                "dll accessibility.\n",
                WSCK_DLL_NAME);
        s_wsock_select = (fl_wsk_select_f)GetProcAddress(s_wsock_mod, "select");
        fl_wsk_fd_is_set =
            (fl_wsk_fd_is_set_f)GetProcAddress(s_wsock_mod, "__WSAFDIsSet");
    }
    return s_wsock_mod;
}
...
static void get_imm_module() {
    ...
    if (!s_imm_module)
        Fl::fatal(
            "FLTK Lib Error: IMM32.DLL file not found!\n\n"
            "Please check your input method manager library accessibility.");
    flImmAssociateContextEx = (flTypeImmAssociateContextEx)GetProcAddress(
        s_imm_module, "ImmAssociateContextEx");
    flImmGetContext =
        (flTypeImmGetContext)GetProcAddress(s_imm_module, "ImmGetContext");
    flImmSetCompositionWindow = (flTypeImmSetCompositionWindow)GetProcAddress(
        s_imm_module, "ImmSetCompositionWindow");
    flImmReleaseContext = (flTypeImmReleaseContext)GetProcAddress(
        s_imm_module, "ImmReleaseContext");
}
...
static struct FD {
    int fd;
    short events;
    void (*cb)(FL_SOCKET, void *);  // keep socket api opaque at this level to
                                    // reduce multiplatform deps headaches
    void *arg;
} *fd = 0;
...
int fl_wait(double time_to_wait) {
    ...
    while ((have_message = PeekMessageW(&fl_msg, NULL, 0, 0, PM_REMOVE)) > 0) {
        if (fl_send_system_handlers(&fl_msg)) continue;

        // Let applications treat WM_QUIT identical to SIGTERM on *nix
        if (fl_msg.message == WM_QUIT) raise(SIGTERM);
    ...
    // whilst they are being modified) we will call
    // process_awake_handler_requests() unnecessarily, but this has no harmful
    // consequences so is safe to do. Note also that if we miss the
    // PostThreadMessage(), then thread_message_ will not be updated, so this is
    // not a perfect solution, but it does recover and process any pending awake
    // callbacks. Normally the ring buffer head and tail indices will match and
    // this comparison will do nothing. Addresses STR #3143
...
class Fl_Win32_At_Exit {
   public:
    ...
    ~Fl_Win32_At_Exit() {
        ...
        if (clipboard_wnd != NULL) fl_clipboard_notify_untarget(clipboard_wnd);
    }
};
...
void fl_update_clipboard(void) {
    ...
    int utf16_len =
        fl_utf8toUtf16(fl_selection_buffer[1], fl_selection_length[1], 0, 0);

    HGLOBAL hMem = GlobalAlloc(
        GHND, utf16_len * 2 + 2);  // moveable and zero'ed mem alloc.
...
void Fl::copy(const char *stuff, int len, int clipboard, const char *type) {
    ...
    if (clipboard) fl_update_clipboard();
}

// Call this when a "paste" operation happens:
void Fl::paste(Fl_Widget &receiver, int clipboard, const char *type) {
    if (!clipboard || (fl_i_own_selection[clipboard] &&
                       strcmp(type, Fl::clipboard_plain_text) == 0)) {
        ...
        *o = 0;
        ...
        if (strcmp(type, Fl::clipboard_plain_text) ==
            0) {  // we want plain text from clipboard
            if ((h = GetClipboardData(
                     CF_UNICODETEXT))) {  // there's text in the clipboard
                wchar_t *memLock = (wchar_t *)GlobalLock(h);
                size_t utf16_len = wcslen(memLock);
                char *clip_text = new char[utf16_len * 4 + 1];
                unsigned utf8_len =
                    fl_utf8fromwc(clip_text, (unsigned)(utf16_len * 4), memLock,
                                  (unsigned)utf16_len);
                ...
                *b = 0;
                ...
                Fl::e_clipboard_type =
                    Fl::clipboard_plain_text;  // indicates that the paste event
                                               // is for plain UTF8 text
                receiver.handle(
                    FL_PASTE);  // send the FL_PASTE event to the widget
                delete[] clip_text;
                Fl::e_text = 0;
            }
        } else if (strcmp(type, Fl::clipboard_image) ==
                   0) {  // we want an image from clipboard
            uchar *rgb = NULL;
            int width = 0, height = 0, depth = 0;
            if ((h = GetClipboardData(
                     CF_DIB))) {  // if there's a DIB in clipboard
                ...
                if ((lpBI->bmiHeader.biBitCount == 24 ||
                    ...
                    lpBI->bmiHeader.biClrUsed ==
                        0) {  // direct use of the DIB data if it's RGB or RGBA
                    int linewidth;                           // row length
                    depth = lpBI->bmiHeader.biBitCount / 8;  // 3 or 4
                    if (depth == 3)
                        linewidth =
                            4 * ((3 * width + 3) /
                                 4);  // row length: series of groups of 3
                                      // bytes, rounded to multiple of 4 bytes
                    else
                        linewidth = 4 * width;
                    rgb = new uchar[width * height *
                                    depth];  // will hold the image data
                    uchar *p = rgb, *r, rr, gg, bb;
                    for (int i = height - 1; i >= 0;
                         i--) {  // for each row, from last to first
                        r = (uchar *)(lpBI->bmiColors) +
                            i * linewidth;  // beginning of pixel data for the
                                            // ith row
                        for (int j = 0; j < width;
                             j++) {     // for each pixel in a row
                            bb = *r++;  // BGR is in DIB
                            gg = *r++;
                            rr = *r++;
                            *p++ = rr;  // we want RGB
                    ...
                    SetDIBitsToDevice(fl_gc, 0, 0, width, height, 0, 0, 0,
                                      height, pDIBBits, lpBI, DIB_RGB_COLORS);
                ...
                GlobalUnlock(h);
            } else if ((h = GetClipboardData(
                            CF_ENHMETAFILE))) {  // if there's an enhanced
                                                 // metafile in clipboard
                ENHMETAHEADER header;
                GetEnhMetaFileHeader(
                    (HENHMETAFILE)h, sizeof(header),
                    &header);  // get structure containing metafile dimensions
                width = (header.rclFrame.right - header.rclFrame.left +
                         1);  // in .01 mm units
                height = (header.rclFrame.bottom - header.rclFrame.top + 1);
                HDC hdc = GetDC(NULL);  // get unit correspondance between .01
                                        // mm and screen pixels
                int hmm = GetDeviceCaps(hdc, HORZSIZE);
                int hdots = GetDeviceCaps(hdc, HORZRES);
                int dhr = GetDeviceCaps(
                    hdc, DESKTOPHORZRES);  // true number of pixels on display
                ReleaseDC(NULL, hdc);
                // Global display scaling factor: 1, 1.25, 1.5, 1.75, etc...
                float scaling = dhr / float(hdots);
                float factor = (100.f * hmm) / hdots;
                width = (int)(width * scaling / factor);
                height = (int)(height * scaling /
                               factor);  // convert to screen pixel unit
                RECT rect = {0, 0, width, height};
                Fl_Offscreen off = fl_create_offscreen(width, height);
                fl_begin_offscreen(off);
                fl_color(FL_WHITE);
                fl_rectf(0, 0, width, height);  // draw white background
                PlayEnhMetaFile(fl_gc, (HENHMETAFILE)h,
                                &rect);  // draw metafile to offscreen buffer
                rgb =
                    fl_read_image(NULL, 0, 0, width,
                                  height);  // read pixels from offscreen buffer
                depth = 3;
                fl_end_offscreen();
                fl_delete_offscreen(off);
            }
            if (rgb) {
                Fl_RGB_Image *image = new Fl_RGB_Image(
                    rgb, width, height,
                    depth);  // create new image from pixel data
                image->alloc_array = 1;
                Fl::e_clipboard_data = image;
                Fl::e_clipboard_type =
                    Fl::clipboard_image;  // indicates that the paste event is
                                          // for image data
                int done =
                    receiver.handle(FL_PASTE);  // send FL_PASTE event to widget
                Fl::e_clipboard_type = "";
                if (done == 0) {  // if widget did not handle the event, delete
                                  // the image
...
static void fl_clipboard_notify_untarget(HWND wnd) {
    ...
    if (IsWindow(wnd))
        ...
        tmp = CreateWindow("STATIC", "Temporary FLTK Clipboard Window", 0, 0, 0,
        ...
        if (head == NULL)
            ...
            SendMessage(head, WM_CHANGECBCHAIN, (WPARAM)wnd,
                        (LPARAM)next_clipboard_wnd);
...
static int mouse_event(Fl_Window *window, int what, int button, WPARAM wParam,
                       LPARAM lParam) {
  ...
  if (wParam & MK_CONTROL) state |= FL_CTRL;
    ...
    switch (what) {
        ...
        default:  // avoid compiler warning
            // MSWindows produces extra events even if mouse does not move,
            // ignore em:
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam,
    ...
    if (window) switch (uMsg) {
            ...
            case WM_NCPAINT:
            case WM_ERASEBKGND:
                // Andreas Weitl - WM_SYNCPAINT needs to be passed to
                // DefWindowProc so that Windows can generate the proper paint
                // messages... Similarly, WM_NCPAINT and WM_ERASEBKGND need
                // this, too...
                break;
            ...
            case WM_PAINT: {
                ...
                if (i->region) {
                    // Also tell WIN32 that we are drawing someplace else as
                    // well...
                ...
                window->clear_damage(
                    (uchar)(window->damage() | FL_DAMAGE_EXPOSE));
                // These next two statements should not be here, so that all
                // update is deferred until Fl::flush() is called during idle.
                // However WIN32 apparently is very unhappy if we don't obey it
                // and draw right now. Very annoying!
            ...
            case WM_MOUSELEAVE:
                if (track_mouse_win ==
                    window) {  // we left the top level window !
                    Fl_Window *tw = window;
                    while (tw->parent())
                        tw = tw->window();  // find top level window
                    Fl::belowmouse(0);
            ...
            case WM_KILLFOCUS:
                ...
                Fl::flush();  // it never returns to main loop when
                              // deactivated...
                break;
            ...
            case WM_ACTIVATEAPP:
                ...
                // messages to restore the correct state of the
                // shift/ctrl/alt/lock keys...  Added control, shift, alt, and
                // meta keys, and changed to use GetAsyncKeyState and do it when
                // wParam is 1 (that means we have focus...)
                if (wParam) {
                    ulong state = 0;
            ...
            case WM_SYSKEYUP:
                ...
                Fl::e_keysym = Fl::e_original_keysym =
                    ms2fltk(wParam, lParam & (1 << 24));
                // See if TranslateMessage turned it into a WM_*CHAR message:
                if (PeekMessageW(&fl_msg, hWnd, WM_CHAR, WM_SYSDEADCHAR,
                                 PM_REMOVE)) {
                    uMsg = fl_msg.message;
            ...
            case WM_SYSCHAR: {
                ulong state =
                    Fl::e_state & 0xff000000;  // keep the mouse button state
                // if GetKeyState is expensive we might want to comment some of
                // these out:
                ...
                if ((GetKeyState(VK_LWIN) | GetKeyState(VK_RWIN)) & ~1) {
                    // WIN32 bug?  GetKeyState returns garbage if the user hit
                    // the meta key to pop up start menu.  Sigh.
                    if ((GetAsyncKeyState(VK_LWIN) |
                         GetAsyncKeyState(VK_RWIN)) &
                        ~1)
                        state |= FL_META;
                ...
                } else if (Fl::e_keysym >= FL_KP &&
                           Fl::e_keysym <= FL_KP_Last) {
...
#define WM_MOUSEHWHEEL 0x020E
            ...
            case WM_CHANGECBCHAIN:
                if ((hWnd == clipboard_wnd) &&
                    ...
                    SendMessage(next_clipboard_wnd, WM_CHANGECBCHAIN, wParam,
                                lParam);
                return 0;
            ...
            case WM_DRAWCLIPBOARD:
                ...
                if (next_clipboard_wnd)
                    SendMessage(next_clipboard_wnd, WM_DRAWCLIPBOARD, wParam,
                                lParam);
...
static int fake_X_wm_style(const Fl_Window *w, int &X, int &Y, int &bt, int &bx,
                           int &by, DWORD style, DWORD styleEx, int w_maxw,
                           int w_minw, int w_maxh, int w_minh,
                           uchar w_size_range_set) {
    ...
    if (!w->parent()) {
        if (fl_xid(w) || style) {
            ...
            // required decorated window rectangle for a desired client
            // rectangle. If any part of the function above fails, we will drop
            // to a fallback to get the best guess which is always available.
    ...
    // This is the original (pre 1.1.7) routine to calculate window border
    // sizes.
    if (fallback) {
        if (w->border() && !w->parent()) {
            if (w_size_range_set && (w_maxw != w_minw || w_maxh != w_minh)) {
                ...
                bx = GetSystemMetrics(SM_CXFIXEDFRAME) +
                     (padding ? padding + ncm.iBorderWidth : 0);
...
int Fl_X::fake_X_wm(const Fl_Window *w, int &X, int &Y, int &bt, int &bx,
                    int &by) {
    return fake_X_wm_style(w, X, Y, bt, bx, by, 0, 0, w->maxw, w->minw, w->maxh,
                           w->minh, w->size_range_set);
}
...
void Fl_Window::resize(int X, int Y, int W, int H) {
    UINT flags =
        SWP_NOSENDCHANGING | SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOOWNERZORDER;
    ...
    if (is_a_resize) {
        ...
        if (visible_r()) {
            ...
            if (i && W > 0 && H > 0) i->wait_for_expose = 1;
        }
    } else {
        x(X);
        y(Y);
...
void Fl_X::make_fullscreen(int X, int Y, int W, int H) {
    ...
    SetWindowPos(xid, HWND_TOP, X, Y, W, H,
...
Fl_X *Fl_X::make(Fl_Window *w) {
    ...
    if (w->parent()) {
        ...
        if (!w->size_range_set) {
            if (w->resizable()) {
                ...
                w->size_range(w->w() - o->w() + minw, w->h() - o->h() + minh, 0,
                              0);
            } else {
        ...
        fake_X_wm_style(w, xwm, ywm, bt, bx, by, style, styleEx, w->maxw,
                        w->minw, w->maxh, w->minh, w->size_range_set);
        if (by + bt) {
            wp += 2 * bx;
            hp += 2 * by + bt;
        }
    ...
    if (w->label()) {
        ...
        //  lab[l] = 0;
        unsigned wlen = fl_utf8toUtf16(w->label(), (unsigned)l, NULL,
                                       0);  // Pass NULL to query length
        wlen++;
        lab = (WCHAR *)malloc(sizeof(WCHAR) * wlen);
        wlen = fl_utf8toUtf16(w->label(), (unsigned)l, (unsigned short *)lab,
                              wlen);
        lab[wlen] = 0;
    }
    x->xid = CreateWindowExW(styleEx, class_namew, lab, style, xp, yp, wp, hp,
                             parent,
                             NULL,  // menu
                             fl_display,
    ...
    if (w->fullscreen_active()) {
        ...
        x->make_fullscreen(rect.left, rect.top, rect.right - rect.left,
    ...
    ShowWindow(x->xid, !showit ? SW_SHOWMINNOACTIVE
                       : (Fl::grab() || (styleEx & WS_EX_TOOLWINDOW))
                           ? SW_SHOWNOACTIVATE
                           : SW_SHOWNORMAL);
    ...
    if (!im_enabled) flImmAssociateContextEx(x->xid, 0, 0);
...
static LRESULT CALLBACK s_TimerProc(HWND hwnd, UINT msg, WPARAM wParam,
                                    LPARAM lParam) {
    switch (msg) {
        case WM_TIMER: {
            unsigned int id = (unsigned)(wParam - 1);
            if (id < (unsigned int)win32_timer_used &&
                win32_timers[id].handle) {
...
void Fl::repeat_timeout(double time, Fl_Timeout_Handler cb, void *data) {
    ...
    if (!s_TimerWnd) {
        ...
        s_TimerWnd =
            CreateWindowEx(WS_EX_LEFT | WS_EX_TOOLWINDOW, timer_class, "",
                           WS_POPUP, 0, 0, 0, 0, NULL, NULL, fl_display, NULL);
        // just in case this OS won't let us create a 0x0 size window:
        if (!s_TimerWnd)
            s_TimerWnd = CreateWindowEx(WS_EX_LEFT | WS_EX_TOOLWINDOW,
                                        timer_class, "", WS_POPUP, 0, 0, 1, 1,
...
void Fl::remove_timeout(Fl_Timeout_Handler cb, void *data) {
    ...
    for (i = 0; i < win32_timer_used; ++i) {
        Win32Timer &t = win32_timers[i];
        if (t.handle && t.callback == cb && (t.data == data || data == NULL)) {
            delete_timer(t);
        }
    }
}
...
void Fl_Window::label(const char *name, const char *iname) {
    ...
    if (shown() && !parent()) {
        ...
        unsigned wlen = fl_utf8toUtf16(name, (unsigned)l, NULL,
                                       0);  // Pass NULL to query length
        wlen++;
        unsigned short *lab =
...
static HICON image_to_icon(const Fl_RGB_Image *image, bool is_icon, int hotx,
                           int hoty) {
    BITMAPV5HEADER bi;
    HBITMAP bitmap, mask;
    DWORD *bits;
    ...
    if (!is_icon) {
        if ((hotx < 0) || (hotx >= image->w())) return NULL;
        if ((hoty < 0) || (hoty >= image->h())) return NULL;
    }
    ...
    for (int i = 0; i < count; i++) {
        if (best == NULL)
            ...
            if (best->w() < ideal_width) {
                if (icons[i]->w() > best->w()) best = icons[i];
            } else {
...
void Fl_X::set_default_icons(const Fl_RGB_Image *icons[], int count) {
    ...
    if (default_big_icon != NULL) DestroyIcon(default_big_icon);
    if (default_small_icon != NULL) DestroyIcon(default_small_icon);
...
void Fl_X::set_default_icons(HICON big_icon, HICON small_icon) {
    ...
    if (big_icon != NULL) default_big_icon = CopyIcon(big_icon);
    if (small_icon != NULL) default_small_icon = CopyIcon(small_icon);
}
...
void Fl_X::set_icons() {
    ...
    if (w->icon_->count) {
        ...
        if (best_big != NULL) big_icon = image_to_icon(best_big, true, 0, 0);
        if (best_small != NULL)
...
void Fl_Window::icons(HICON big_icon, HICON small_icon) {
    ...
    if (big_icon != NULL) icon_->big_icon = CopyIcon(big_icon);
    if (small_icon != NULL) icon_->small_icon = CopyIcon(small_icon);
...
int Fl_X::set_cursor(Fl_Cursor c) {
    ...
    if (c == FL_CURSOR_NONE)
        ...
        if (new_cursor == NULL) return 0;
    }

    if ((cursor != NULL) && custom_cursor) DestroyIcon(cursor);
...
//  memory leaks. The following struct, global variable, and routines help
//  implement the above protocol for those cases where the GetDC and RestoreDC
//  are not in the same routine. For each GetDC, fl_save_dc is used to create an
//  entry in a linked list that saves the window handle, the DC handle, and the
//  initial state. When the DC is to be released, 'fl_release_dc' is called. It
//  restores the initial state and releases the DC. When the program exits,
//  'fl_cleanup_dc_list' frees any remaining nodes in the list.
...
static RECT border_width_title_bar_height(Fl_Window *win, int &bx, int &by,
                                          int &bt, float *pscaling = 0) {
    RECT r = {0, 0, 0, 0};
    bx = by = bt = 0;
    float scaling = 1;
    if (win->shown() && !win->parent() && win->border() && win->visible()) {
        static HMODULE dwmapi_dll = LoadLibrary("dwmapi.dll");
        typedef HRESULT(WINAPI * DwmGetWindowAttribute_type)(
            HWND hwnd, DWORD dwAttribute, PVOID pvAttribute, DWORD cbAttribute);
        static DwmGetWindowAttribute_type DwmGetWindowAttribute =
            dwmapi_dll ? (DwmGetWindowAttribute_type)GetProcAddress(
                             dwmapi_dll, "DwmGetWindowAttribute")
                       : NULL;
        int need_r = 1;
        if (DwmGetWindowAttribute) {
            const DWORD DWMWA_EXTENDED_FRAME_BOUNDS = 9;
            if (DwmGetWindowAttribute(fl_xid(win), DWMWA_EXTENDED_FRAME_BOUNDS,
                                      &r, sizeof(RECT)) == S_OK) {
                need_r = 0;
                // Compute the global display scaling factor:
                // 1, 1.25, 1.5, 1.75, etc... This factor can be set in Windows
                // 10 by "Change the size of text, apps and other items" in
                // display settings.
                HDC hdc = GetDC(NULL);
                int hr =
                    GetDeviceCaps(hdc, HORZRES);  // pixels visible to the app
...
#define DESKTOPHORZRES 118
                ...
                scaling = int(scaling * 100 + 0.5) /
                          100.;  // round to 2 digits after decimal point
            }
        }
        if (need_r) {
...
void Fl_Paged_Device::draw_decorated_window(Fl_Window *win, int x_offset,
    ...
    if (bt) {
        Fl_Display_Device::display_device()
            ->set_current();  // make window current
        win->show();
        Fl::check();
        win->make_current();
        HDC save_gc = fl_gc;
        ...
        fl_window =
            NULL;  // force use of read_win_rectangle() by fl_read_image()
        uchar *top_image =
            fl_read_image(NULL, r.left, r.top, r.right - r.left + 1, bt + by);
        uchar *left_image =
            bx ? fl_read_image(NULL, r.left, r.top, bx, wh) : NULL;
        uchar *right_image =
            bx ? fl_read_image(NULL, r.right - bx, r.top, bx, wh) : NULL;
        uchar *bottom_image =
            by ? fl_read_image(NULL, r.left, r.bottom - by, ww, by) : NULL;
        fl_window = save_win;
        ...
        Fl_RGB_Image *top_r =
            new Fl_RGB_Image(top_image, r.right - r.left + 1, bt + by, 3);
        top_r->alloc_array = 1;
        if (scaling > 1) {
            Fl_RGB_Scaling current = Fl_Image::RGB_scaling();
            Fl_Image::RGB_scaling(FL_RGB_SCALING_BILINEAR);
            Fl_RGB_Image *tmp_img =
                (Fl_RGB_Image *)top_r->copy(ww, (bt + by) / scaling);
        ...
        if (right_image) {
            fl_draw_image(right_image, x_offset + win->w() + bx, y_offset, bx,
                          wh, 3);
            delete right_image;
        }
        if (bottom_image) {
            fl_draw_image(bottom_image, x_offset, y_offset + win->h() + bt + by,
                          ww, by, 3);
            delete bottom_image;
        }
    }
