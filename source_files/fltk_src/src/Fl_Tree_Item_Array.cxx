...
Fl_Tree_Item_Array::~Fl_Tree_Item_Array() { clear(); }
...
Fl_Tree_Item_Array::Fl_Tree_Item_Array(const Fl_Tree_Item_Array *o) {
    ...
    for (int t = 0; t < o->_total; t++) {
        ...
        if (_flags & MANAGE_ITEM) {
            _items[t] =
                new Fl_Tree_Item(o->_items[t]);  // make new copy of item
            ++_total;
            _items[t]->update_prev_next(
                t);  // update uses _total's current value
        } else {
...
void Fl_Tree_Item_Array::enlarge(int count) {
    ...
    if (newtotal >= _size) {        // more than we have allocated?
        ...
        Fl_Tree_Item **newitems =
            (Fl_Tree_Item **)malloc(newsize * sizeof(Fl_Tree_Item *));
        if (_items) {
...
void Fl_Tree_Item_Array::insert(int pos, Fl_Tree_Item *new_item) {
    ...
    if (pos <= (_total - 1)) {  // need to move memory around?
        ...
        memmove(&_items[pos + 1], &_items[pos],
                sizeof(Fl_Tree_Item *) * nitems);
    }
    _items[pos] = new_item;
    _total++;
#if FLTK_ABI_VERSION >= 10303
...
#endif
    {
        _items[pos]->update_prev_next(
            pos);  // adjust item's prev/next and its neighbors
    }
}
...
///
void Fl_Tree_Item_Array::add(Fl_Tree_Item *val) { insert(_total, val); }
...
void Fl_Tree_Item_Array::remove(int index) {
...
#endif
    {
        if (index < _total) {  // removed item not last?
            _items[index]->update_prev_next(
                index);  // update next item's prev/next and neighbors
        } else if (((index - 1) >= 0) &&  // removed item IS last?
                   ((index - 1) < _total)) {
            _items[index - 1]->update_prev_next(
                index - 1);  // update prev item's prev/next and neighbors
        }
    }
}
...
///
///     \returns 0 on success, -1 on range error (e.g. if \p 'to' or \p 'from'
///     out of range)
///
int Fl_Tree_Item_Array::move(int to, int from) {
    ...
    if (from < to)
        ...
        for (int t = from; t > to && t > 0; t--) _items[t] = _items[t - 1];
    // Move to new position
    _items[to] = item;
    // Update all children
    for (int r = 0; r < _total; r++)  // XXX: excessive to do all children,
        _items[r]->update_prev_next(
            r);  // XXX: but avoids weird boundary issues
    return 0;
}
...
///     \returns 0 on success, -1 on error (e.g. if \p 'pos' out of range)
///
int Fl_Tree_Item_Array::reparent(Fl_Tree_Item *item, Fl_Tree_Item *newparent,
                                 int pos) {
    if (pos < 0 || pos > _total) return -1;
    // Add item to new parent
    enlarge(1);
    _total += 1;
    for (int t = _total - 1; t > pos;
         --t)  // shuffle array to make room for new entry
        _items[t] = _items[t - 1];
