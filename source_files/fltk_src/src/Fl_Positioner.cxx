...
int Fl_Positioner::xvalue(double X) { return (value(X, yvalue_)); }

/** Sets the Y axis coordinate.*/
int Fl_Positioner::yvalue(double Y) { return (value(xvalue_, Y)); }
    ...
    switch (event) {
        ...
        case FL_RELEASE: {
            ...
            if (!(when() & FL_WHEN_CHANGED ||
                  (when() & FL_WHEN_RELEASE && event == FL_RELEASE)))
                return 1;
...
int Fl_Positioner::handle(int e) { return handle(e, x(), y(), w(), h()); }
