...
Fl_Bitmask fl_create_bitmask(int w, int h, const uchar *array) {
    ...
    for (int i = rowBytes * h; i > 0; i--, src++) {
        *dst++ = ((reverse[*src & 0x0f] & 0xf0) |
                  (reverse[(*src >> 4) & 0x0f] & 0x0f)) ^
                 0xff;
    }
    CGDataProviderRef srcp =
        CGDataProviderCreateWithData(0L, bmask, rowBytes * h, 0L);
...
Fl_Bitmask fl_create_bitmask(int w, int h, const uchar *data) {
    ...
    Fl_Bitmask bm;
    static uchar hiNibble[16] = {0x00, 0x80, 0x40, 0xc0, 0x20, 0xa0,
                                 0x60, 0xe0, 0x10, 0x90, 0x50, 0xd0,
                                 0x30, 0xb0, 0x70, 0xf0};
    static uchar loNibble[16] = {0x00, 0x08, 0x04, 0x0c, 0x02, 0x0a,
                                 0x06, 0x0e, 0x01, 0x09, 0x05, 0x0d,
                                 0x03, 0x0b, 0x07, 0x0f};
    int np =
        GetDeviceCaps(fl_gc, PLANES);  //: was always one on sample machines
    int bpp =
        GetDeviceCaps(fl_gc, BITSPIXEL);  //: 1,4,8,16,24,32 and more odd stuff?
...
void fl_delete_bitmask(Fl_Bitmask bm) { DeleteObject((HGDIOBJ)bm); }
...
void fl_delete_bitmask(Fl_Bitmask bm) { fl_delete_offscreen((Fl_Offscreen)bm); }

#endif  // __APPLE__

// Create a 1-bit mask used for alpha blending
Fl_Bitmask fl_create_alphamask(int w, int h, int d, int ld,
                               const uchar *array) {
    Fl_Bitmask bm;
    int bmw = (w + 7) / 8;
    ...
    static uchar dither[16][16] = {
        ...
        {63, 191, 31, 159, 55, 183, 23, 151, 61, 189, 29, 157, 53, 181, 21,
         149},
        {254, 127, 223, 95, 247, 119, 215, 87, 253, 125, 221, 93, 245, 117, 213,
         85}};
    ...
    for (dataptr = array + d - 1, y = 0; y < h; y++, dataptr += ld)
        for (bitptr = bitmap + y * bmw, bit = 1, x = 0; x < w;
             x++, dataptr += d) {
            if (*dataptr > dither[x & 15][y & 15]) *bitptr |= bit;
            if (bit < 128)
                bit <<= 1;
            else {
                bit = 1;
...
int Fl_Bitmap::start(int XP, int YP, int WP, int HP, int &cx, int &cy, int &X,
                     int &Y, int &W, int &H) {
    if (!array) {
        draw_empty(XP, YP);
        return 1;
...
#elif defined(WIN32)
void Fl_GDI_Graphics_Driver::draw(Fl_Bitmap *bm, int XP, int YP, int WP, int HP,
                                  int cx, int cy) {
    int X, Y, W, H;
    if (bm->start(XP, YP, WP, HP, cx, cy, X, Y, W, H)) {
        return;
...
void Fl_GDI_Printer_Graphics_Driver::draw(Fl_Bitmap *bm, int XP, int YP, int WP,
                                          int HP, int cx, int cy) {
    int X, Y, W, H;
    ...
    b = 255 - b;
    Fl_Color background =
        fl_rgb_color(r, g, b);  // a color very different from the bitmap's
    ...
    BitBlt(fl_gc, 0, 0, W, H, tempdc, 0, 0,
    ...
    fl_TransparentBlt(fl_gc, X, Y, W, H, tempdc, cx, cy, bm->w(), bm->h(),
                      RGB(r, g, b));
...
#else  // Xlib
void Fl_Xlib_Graphics_Driver::draw(Fl_Bitmap *bm, int XP, int YP, int WP,
                                   int HP, int cx, int cy) {
    int X, Y, W, H;
    if (bm->start(XP, YP, WP, HP, cx, cy, X, Y, W, H)) {
        return;
...
Fl_Image *Fl_Bitmap::copy(int W, int H) {
    ...
    for (dy = H, sy = 0, yerr = H, new_ptr = new_array; dy > 0; dy--) {
        for (dx = W, xerr = W, old_ptr = array + sy * ((w() + 7) / 8), sx = 0,
            new_bit = 1;
             dx > 0; dx--) {
