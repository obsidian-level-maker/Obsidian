...
float Fl_Gl_Window::pixels_per_unit() {
    return (fl_mac_os_version >= 100700 && Fl::use_high_res_GL() &&
            Fl_X::i(this) && Fl_X::i(this)->mapped_to_retina())
               ? 2
               : 1;
}
...
int Fl_Gl_Window::mode(int m, const int *a) {
    ...
    if (a) {  // when the mode is set using the a array of system-dependent
              // values, and if asking for double buffer, the FL_DOUBLE flag
              // must be set in the mode_ member variable
        const int *aa = a;
        while (*aa) {
            if (*(aa++) ==
#if defined(__APPLE__)
    ...
    if (shown()) {
        ...
        if (!g || g->vis->visualid != oldg->vis->visualid ||
            (oldmode ^ m) & FL_DOUBLE) {
            hide();
            show();
        }
...
void Fl_Gl_Window::swap_buffers() {
    ...
    if (overlay != NULL) {
        ...
        glTranslatef(-wo / 2.0f, -ho / 2.0f,
                     0.0f);  // set transform so 0,0 is bottom/left of Gl_Window
    ...
    /* // nothing to do here under Cocoa because [NSOpenGLContext -flushBuffer]
     done later replaces it else aglSwapBuffers((AGLContext)context_);
     */
#else
#error unsupported platform
#endif
...
void Fl_Gl_Window::flush() {
    ...
    if (mode_ & FL_DOUBLE) {
        ...
        if (SWAP_TYPE == NODAMAGE) {
            ...
            if (overlay == this) {
                ...
                if (damage1_ || damage() != FL_DAMAGE_OVERLAY || !save_valid)
                    draw();
                // we use a separate context for the copy because rasterpos must
                // be 0 and depth test needs to be off:
        ...
        if (overlay == this &&
            SWAP_TYPE != SWAP) {  // fake overlay in front buffer
            glDrawBuffer(GL_FRONT);
            draw_overlay();
            glDrawBuffer(GL_BACK);
            glFlush();
...
void Fl_Gl_Window::draw() {
    Fl::fatal(
        "Fl_Gl_Window::draw() *must* be overriden. Please refer to the "
        "documentation.");
}
 ...
 */
int Fl_Gl_Window::handle(int event) { return Fl_Window::handle(event); }

// don't remove me! this serves only to force linking of Fl_Gl_Device_Plugin.o
int Fl_Gl_Window::gl_plugin_linkage() { return fl_gl_load_plugin; }
