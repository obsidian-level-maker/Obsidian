...
/*  Each MacOS system menu item contains a pointer to a record of type sys_menu_item defined below.
    The purpose of these records is to associate each MacOS system menu item with a relevant
   Fl_Menu_Item.
    ...
    Sending the getFlItem message to a MacOS system menu item (of class FLMenuItem) returns the
   address of the relevant Fl_Menu_Item.
*/
typedef struct {
    union {
        int rank;
...
void fl_mac_set_about(Fl_Callback *cb, void *user_data, int shortcut) {
    ...
    if (aboutItem.shortcut()) [item setFltkShortcut:aboutItem.shortcut()];
 ...
 * @param cb        - callback to be invoked when item selected (can be 0 for none, in which case
 * the menubar's callback() can be used instead)
 ...
 */
int Fl_Sys_Menu_Bar::add(const char *label, int shortcut, Fl_Callback *cb, void *user_data,
                         int flags) {
    fl_open_display();
 ...
 * @see Fl_Menu_::insert(int index, const char* label, int shortcut, Fl_Callback *cb, void
 * *user_data, int flags)
 */
int Fl_Sys_Menu_Bar::insert(int index, const char *label, int shortcut, Fl_Callback *cb,
 ...
 */
void Fl_Sys_Menu_Bar::update() { convertToMenuBar(Fl_Menu_::menu()); }
...
static int process_sys_menu_shortcuts(int event) {
    ...
    if (!item) return 0;
    if (item->visible())  // have the system menu process the shortcut, highlighting the
                          // corresponding menu
...
/**
 The constructor.
 On Mac OS X, all arguments are unused. On other platforms they are used as by
...
void Fl_Mac_App_Menu::custom_application_menu_items(const Fl_Menu_Item *m) {
    ...
    if ([[menu itemAtIndex:2] action] != @selector(printPanel)) {  // the 'Print' item was removed
        ...
        to_rank = 3;  // after the "Print Front Window" item
