...
void fl_scroll(int X, int Y, int W, int H, int dx, int dy,
...
#if defined(USE_X11)
    XCopyArea(fl_display, fl_window, fl_window, fl_gc, src_x, src_y, src_w,
              src_h, dest_x, dest_y);
    ...
    for (;;) {
        ...
        draw_area(data, e.xexpose.x, e.xexpose.y, e.xexpose.width,
                  e.xexpose.height);
    ...
    if (first_time) {
        ...
        if (hMod) {
            fl_GetRandomRgn =
                (fl_GetRandomRgn_func)GetProcAddress(hMod, "GetRandomRgn");
        }
        first_time = 0;
    }
    ...
    if (fl_GetRandomRgn) {
        ...
        HRGN src_rgn =
            CreateRectRgn(src_x, src_y, src_x + src_w, src_y + src_h);
        POINT offset = {0, 0};
