...
// Move up a page
int Fl_Input::kf_page_up() { return kf_lines_up(linesPerPage()); }

// Move down a page
int Fl_Input::kf_page_down() { return kf_lines_down(linesPerPage()); }
...
// Do a copy operation
int Fl_Input::kf_copy() { return copy(1); }
...
int Fl_Input::handle_key() {
    ...
    if (Fl::compose(del)) {
        ...
        if (input_type() == FL_FLOAT_INPUT || input_type() == FL_INT_INPUT) {
            ...
            // initialize the list of legal characters inside a floating point
            // number
#ifdef HAVE_LOCALECONV
            if (!legal_fp_chars) {
                ...
                if (lc) {
                    ...
                    if (lc->mon_decimal_point)
                        len += strlen(lc->mon_decimal_point);
                    if (lc->positive_sign) len += strlen(lc->positive_sign);
                    if (lc->negative_sign) len += strlen(lc->negative_sign);
                }
                // the following line is not a true memory leak because the
                // array is only allocated once if required, and automatically
                // freed when the program quits
                ...
                if (lc) {
                    ...
                    if (lc->mon_decimal_point)
                        strcat(chars, lc->mon_decimal_point);
    ...
    switch (Fl::event_key()) {
        case FL_Insert:
            ...
            if (mods == 0)
                return kf_insert_toggle();  // Insert         (Standard)
            if (mods == FL_CTRL)
        ...
        case FL_Delete: {
#ifdef __APPLE__
            if (mods == 0)
                return kf_delete_char_right();  // Delete (OSX-HIG,TE,SA,WOX)
            if (mods == FL_CTRL)
                return kf_delete_char_right();  // Ctrl-Delete    (???
                                                // TE,!SA,!WOX)
            if (mods == FL_ALT)
            ...
            if (mods == 0 && shift && selected)
                return kf_copy_cut();  // Shift-Delete with selection
                                       // (WP,NP,WOW,GE,KE,OF)
            if (mods == 0 && shift && !selected)
                return kf_delete_char_right();  // Shift-Delete no selection
                                                // (WP,NP,WOW,GE,KE,!OF)
            if (mods == 0)
                return kf_delete_char_right();  // Delete         (Standard)
            if (mods == FL_CTRL)
                return kf_delete_word_right();  // Ctrl-Delete
                                                // (WP,!NP,WOW,GE,KE,!OF)
            return 0;  // ignore other combos, pass to parent
#endif
        }
...
#ifdef __APPLE__
            if (mods == 0)
                return kf_move_char_left();  // Left           (OSX-HIG)
            if (mods == FL_ALT)
...
#else
            if (mods == 0)
                return kf_move_char_left();  // Left (WP,NP,WOW,GE,KE,OF)
            if (mods == FL_CTRL)
...
#ifdef __APPLE__
            if (mods == 0)
                return kf_move_char_right();  // Right          (OSX-HIG)
            if (mods == FL_ALT)
...
#else
            if (mods == 0)
                return kf_move_char_right();  // Right (WP,NP,WOW,GE,KE,OF)
            if (mods == FL_CTRL)
                return kf_move_word_right();  // Ctrl-Right
                                              // (WP,NP,WOW,GE,KE,!OF)
            if (mods == FL_META)
                return kf_move_char_right();  // Meta-Right
                                              // (WP,NP,?WOW,GE,KE,!OF)
            return 0;  // ignore other combos, pass to parent
#endif
...
#else
            if (mods == 0)
                return kf_lines_up(1);  // Up             (WP,NP,WOW,GE,KE,OF)
            if (mods == FL_CTRL)
...
#else
            if (mods == 0)
                return kf_lines_down(1);  // Dn             (WP,NP,WOW,GE,KE,OF)
            if (mods == FL_CTRL)
                return kf_move_down_and_eol();  // Ctrl-Down
                                                // (WP,!NP,WOW,GE,!KE,OF)
            return 0;  // ignore other combos, pass to parent
#endif
        ...
        case FL_Page_Up:
            ...
            // one page OSX-HIG recommends Alt increase one semantic unit, Meta
            // next higher..
#ifdef __APPLE__
...
#else
            if (mods == 0)
                return kf_page_up();  // PageUp         (WP,NP,WOW,GE,KE)
            if (mods == FL_CTRL)
...
#else
            if (mods == 0)
                return kf_page_down();  // PageDn         (WP,NP,WOW,GE,KE)
            if (mods == FL_CTRL)
...
#else
            if (mods == 0)
                return kf_move_sol();  // Home           (WP,NP,WOW,GE,KE,OF)
            if (mods == FL_CTRL)
...
#else
            if (mods == 0)
                return kf_move_eol();  // End            (WP,NP,WOW,GE,KE,OF)
            if (mods == FL_CTRL)
...
#ifdef __APPLE__
            if (mods == 0)
                return kf_delete_char_left();  // Backspace      (OSX-HIG)
            if (mods == FL_CTRL)
...
#else
            if (mods == 0)
                return kf_delete_char_left();  // Backspace (WP,NP,WOW,GE,KE,OF)
            if (mods == FL_CTRL)
                return kf_delete_word_left();  // Ctrl-Backspace
                                               // (WP,!NP,WOW,GE,KE,!OF)
            return 0;  // ignore other combos, pass to parent
#endif
        ...
        case FL_Tab:
            ...
            if (mods == 0)
                return 0;  // Tab, Shift-Tab? nav focus      (Standard/OSX-HIG)
            return 0;      // ignore other combos, pass to parent

        case 'a':
            if (mods == FL_COMMAND)
                return kf_select_all();  // Ctrl-A, Mac:Meta-A
                                         // (Standard/OSX-HIG)
        ...
        case 'x':
            ...
            break;
        case 'z':
            if (mods == FL_COMMAND && !shift)
                return kf_undo();  // Ctrl-Z, Mac:Meta-Z (Standard/OSX-HIG)
            if (mods == FL_COMMAND && shift)
                return kf_redo();  // Shift-Ctrl-Z, Mac:Shift-Meta-Z
                                   // (Standard/OSX-HIG)
            break;                 // handle other combos elsewhere
    }
...
int Fl_Input::handle(int event) {
    ...
    switch (event) {
        ...
        case FL_FOCUS:
            switch (Fl::event_key()) {
                ...
                default:
                    position(
                        position(),
                        mark());  // turns off the saved up/down arrow position
                    break;
            }
            break;
        ...
        case FL_KEYBOARD:
            // Handle special case for multiline input with 'old tab behavior'
            // where tab is entered as a character: make sure user attempt to
            // 'tab over' widget doesn't destroy the field, replacing it with a
            // tab character.
            //
            ...
            if (Fl::dnd_text_ops()) {
                if (drag_start >= 0) {
                    if (Fl::event_is_click())
                        return 1;  // debounce the mouse
                                   // save the position because sometimes we
                                   // don't get DND_ENTER:
        ...
        case FL_RELEASE:
            if (Fl::event_button() == 2) {
                ...
                // user clicked in the field and wants to reset the cursor
                // position...
...
#ifdef DND_OUT_XXXX
            if (Fl::focus() == this &&
                (p >= dnd_save_position && p <= dnd_save_mark ||
        ...
        case FL_DND_RELEASE:
            if (dnd_save_focus ==
                this) {  // if the dragged text comes from the same widget
                ...
                if (old_position > dnd_save_position)
                    position(old_position -
                             (dnd_save_position - dnd_save_mark));
                else
                    position(old_position);
            ...
            /* TODO: this will scroll the area, but stop if the cursor would
             become invisible. That clipping happens in drawtext(). Do we change
             the clipping or should we move the cursor (ouch)? case
             FL_MOUSEWHEEL: if (Fl::e_dy > 0) { yscroll( yscroll() - Fl::e_dy*15
             ); } else if (Fl::e_dy < 0) { yscroll( yscroll() - Fl::e_dy*15 );
             }
             return 1;
             */
    }
    Fl_Boxtype b = box();
    return Fl_Input_::handletext(event, x() + Fl::box_dx(b),
                                 y() + Fl::box_dy(b), w() - Fl::box_dw(b),
                                 h() - Fl::box_dh(b));
}
