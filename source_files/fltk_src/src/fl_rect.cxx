...
void Fl_Graphics_Driver::xyline(int x, int y, int x1) {
    ...
    if (Fl_Display_Device::high_resolution()) {
        /* On retina displays, all xyline() and yxline() functions produce lines
         that are half-unit (or one pixel) too short at both ends. This is
         corrected by filling at both ends rectangles of size one unit by
         line-width.
         */
        CGContextFillRect(fl_gc,
                          CGRectMake(x - 0.5, y - fl_quartz_line_width_ / 2, 1,
                                     fl_quartz_line_width_));
        CGContextFillRect(fl_gc,
                          CGRectMake(x1 - 0.5, y - fl_quartz_line_width_ / 2, 1,
                                     fl_quartz_line_width_));
    }
    if (USINGQUARTZPRINTER || fl_quartz_line_width_ > 1.5f)
        CGContextSetShouldAntialias(fl_gc, false);
    ...
    if (Fl_Display_Device::high_resolution()) {
        CGContextFillRect(fl_gc,
                          CGRectMake(x - 0.5, y - fl_quartz_line_width_ / 2, 1,
                                     fl_quartz_line_width_));
        CGContextFillRect(fl_gc,
                          CGRectMake(x1 - fl_quartz_line_width_ / 2, y2 - 0.5,
                                     fl_quartz_line_width_, 1));
    }
    if (USINGQUARTZPRINTER || fl_quartz_line_width_ > 1.5f)
        CGContextSetShouldAntialias(fl_gc, false);
#else
    ...
    if (Fl_Display_Device::high_resolution()) {
        CGContextFillRect(fl_gc,
                          CGRectMake(x - 0.5, y - fl_quartz_line_width_ / 2, 1,
                                     fl_quartz_line_width_));
        CGContextFillRect(
            fl_gc, CGRectMake(x3 - 0.5, y2 - fl_quartz_line_width_ / 2, 1,
                              fl_quartz_line_width_));
    }
    if (USINGQUARTZPRINTER || fl_quartz_line_width_ > 1.5f)
        CGContextSetShouldAntialias(fl_gc, false);
...
void Fl_Graphics_Driver::yxline(int x, int y, int y1) {
    ...
    CGContextStrokePath(fl_gc);
    if (Fl_Display_Device::high_resolution()) {
        CGContextFillRect(fl_gc, CGRectMake(x - fl_quartz_line_width_ / 2,
                                            y - 0.5, fl_quartz_line_width_, 1));
        CGContextFillRect(fl_gc,
                          CGRectMake(x - fl_quartz_line_width_ / 2, y1 - 0.5,
                                     fl_quartz_line_width_, 1));
    }
    if (USINGQUARTZPRINTER || fl_quartz_line_width_ > 1.5f)
        CGContextSetShouldAntialias(fl_gc, false);
...
void Fl_Graphics_Driver::yxline(int x, int y, int y1, int x2) {
    ...
    if (Fl_Display_Device::high_resolution()) {
        CGContextFillRect(fl_gc, CGRectMake(x - fl_quartz_line_width_ / 2,
                                            y - 0.5, fl_quartz_line_width_, 1));
        CGContextFillRect(
            fl_gc, CGRectMake(x2 - 0.5, y1 - fl_quartz_line_width_ / 2, 1,
                              fl_quartz_line_width_));
    }
    if (USINGQUARTZPRINTER || fl_quartz_line_width_ > 1.5f)
        CGContextSetShouldAntialias(fl_gc, false);
...
void Fl_Graphics_Driver::yxline(int x, int y, int y1, int x2, int y3) {
    ...
    if (Fl_Display_Device::high_resolution()) {
        CGContextFillRect(fl_gc, CGRectMake(x - fl_quartz_line_width_ / 2,
                                            y - 0.5, fl_quartz_line_width_, 1));
        CGContextFillRect(fl_gc,
                          CGRectMake(x2 - fl_quartz_line_width_ / 2, y3 - 0.5,
                                     fl_quartz_line_width_, 1));
    }
    if (USINGQUARTZPRINTER || fl_quartz_line_width_ > 1.5f)
        CGContextSetShouldAntialias(fl_gc, false);
...
#elif defined(__APPLE_QUARTZ__)
    if (fl_window ||
        fl_gc) {  // clipping for a true window or an offscreen buffer
...
Fl_Region Fl_Graphics_Driver::clip_region() { return rstack[rstackptr]; }
...
void Fl_Graphics_Driver::push_no_clip() {
    if (rstackptr < region_stack_max)
        ...
        Fl::warning("fl_push_no_clip: clip stack overflow!\n");
    fl_restore_clip();
}
...
void Fl_Graphics_Driver::pop_clip() {
    if (rstackptr > 0) {
        ...
        Fl::warning("fl_pop_clip: clip stack underflow!\n");
    fl_restore_clip();
}
...
int Fl_Graphics_Driver::not_clipped(int x, int y, int w, int h) {
    ...
    if (Fl_Surface_Device::surface() !=
        Fl_Display_Device::display_device()) {  // in case of print context,
                                                // convert coords from logical
                                                // to device
...
int Fl_Graphics_Driver::clip_box(int x, int y, int w, int h, int &X, int &Y,
                                 int &W, int &H) {
    X = x;
    Y = y;
    W = w;
    ...
    if (CombineRgn(temp, rr, r, RGN_AND) == NULLREGION) {  // disjoint
        ...
        GetRgnBox(temp, &rect);
        if (Fl_Surface_Device::surface() !=
            Fl_Display_Device::display_device()) {  // if print context, convert
                                                    // coords from device to
                                                    // logical
    ...
    for (int i = 0; i < r->count; i++) {
        ...
        if (!CGRectIsEmpty(test)) {
            if (CGRectIsEmpty(u))
                ...
                u = CGRectUnion(u, test);
        }
    }
    X = int(u.origin.x +
            0.5);  // reverse offset introduced by fl_cgrectmake_cocoa()
