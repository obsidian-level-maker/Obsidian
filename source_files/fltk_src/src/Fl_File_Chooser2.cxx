...
// fabien: ATTENTION: Only Out Of Source Gen. because cxx/H files are
...
/** \fn Fl_File_Chooser::Fl_File_Chooser(const char *pathname, const char
  *pattern, int type, const char *title) The constructor creates the
  Fl_File_Chooser dialog shown. The pathname argument can be a directory name or
  a complete file name (in which case the corresponding file is highlighted in
  the list and in the filename input field.)
...
/** \fn int Fl_File_Chooser::shown()
  Returns non-zero if the file chooser main window show() has been called (but
  not hide() see Fl_Window::shown()
*/

/** \fn void Fl_File_Chooser::callback(void (*cb)(Fl_File_Chooser *, void *),
  void *d = 0) Sets the file chooser callback cb and associated data d */
...
//   Fl_File_Chooser::directory()         - Set the directory in the file
//   chooser. Fl_File_Chooser::filter()            - Set the filter(s) for the
//   chooser. Fl_File_Chooser::newdir()            - Make a new directory.
...
//   Fl_File_Chooser::fileNameCB()        - Handle text entry in the
//   FileBrowser. Fl_File_Chooser::showChoiceCB()      - Handle show selections.
...
//
//   Fl_File_Chooser::add_extra()         - add extra widget at the bottom,
//   return pointer
//                                          to previous extra widget or NULL if
//                                          none, If argument is NULL extra
//                                          widget removed. NOTE! file chooser
//                                          does't delete extra widget in
...
const char *Fl_File_Chooser::existing_file_label =
...
Fl_File_Chooser::count() {
    ...
    for (i = 1, fcount = 0; i <= fileList->size(); i++)
        if (fileList->selected(i)) {
            ...
            // matt: why would we do that? It is perfectly legal to select
            // multiple directories in a DIR chooser. They are visually selected
            // and value(i) returns all of them as expected
...
void Fl_File_Chooser::directory(const char *d)  // I - Directory to change to
{
    ...
    if (strchr(d, '\\')) {
        ...
        for (slash = strchr(fixpath, '\\'); slash;
    ...
    if (Fl::event_clicks()) {
        ...
        if ((type_ & MULTI) && !(type_ & DIRECTORY)) {
            if (*filename == '/') {
                ...
                // Clicked on a file - see if there are other directories
                // selected...
                int i;
                const char *temp;
...
#if (defined(WIN32) && !defined(__CYGWIN__)) || defined(__EMX__)
        if ((isalpha(pathname[0] & 255) && pathname[1] == ':' &&
             !pathname[2]) ||
            (_fl_filename_isdir_quick(pathname) &&
        ...
        *slash++ = '\0';
        ...
        if (strcmp(pathname, directory_) &&
            ...
            if (filename[0]) {
                ...
                snprintf(tempname, sizeof(tempname), "%s/%s", directory_,
                         filename);
        ...
        if (first_line > 0 && min_match == max_match &&
            ...
            fileName->replace((int)(filename - pathname),
...
void Fl_File_Chooser::preview(int e) {
    ...
    if (e) {
        ...
        previewBox->resize(fileList->x() + w, previewBox->y(), p->w() - w,
                           previewBox->h());
        previewBox->show();
        update_preview();
    } else {
        fileList->resize(fileList->x(), fileList->y(), p->w(), fileList->h());
        previewBox->resize(p->x() + p->w(), previewBox->y(), 0,
                           previewBox->h());
        previewBox->hide();
    }
    p->init_sizes();
