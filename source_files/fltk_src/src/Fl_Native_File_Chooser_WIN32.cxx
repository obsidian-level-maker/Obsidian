...
#define FNFC_MAX_PATH \
    32768  // XXX: MAX_PATH under win32 is 260, too small for modern use
...
static void dnullcat(char *&wp, const char *string, int n = -1) {
    ...
    if (!wp) {
        ...
        *(wp + 1) = '\0';
        ...
        wp = tmp;                    // use new copy
        // DEBUG printf("DEBUG: dnullcat COPY: <"); dnullprint(wp); printf(">
        // (wplen=%d)\n", wplen);
    }
...
// SET TYPE OF BROWSER
void Fl_Native_File_Chooser::type(int val) { _btype = val; }

// GET TYPE OF BROWSER
int Fl_Native_File_Chooser::type() const { return (_btype); }

// SET OPTIONS
void Fl_Native_File_Chooser::options(int val) { _options = val; }

// GET OPTIONS
int Fl_Native_File_Chooser::options() const { return (_options); }
...
int Fl_Native_File_Chooser::showfile() {
    ...
    _ofn_ptr->Flags |=
        OFN_ENABLESIZING;  // allow window to be resized (hey, why not?)
    _ofn_ptr->Flags |= OFN_NOCHANGEDIR;  // XXX: docs say ineffective on
                                         // XP/2K/NT, but set it anyway..
    ...
    // FILTER
    if (_parsedfilt != NULL) {  // to convert a null-containing char string into
                                // a widechar string
#if FLTK_ABI_VERSION >= 10304
        // NEW
...
#else
        // OLD
        static WCHAR _wpattern[FNFC_MAX_PATH];  // yuck -- replace with managed
                                                // class member
#endif
        const char *p = _parsedfilt;
        while (*(p + strlen(p) + 1) != 0) p += strlen(p) + 1;
        p += strlen(p) + 2;
        MultiByteToWideChar(CP_UTF8, 0, _parsedfilt, (int)(p - _parsedfilt),
                            _wpattern, FNFC_MAX_PATH);
    ...
    switch (_btype) {
        ...
        case BROWSE_MULTI_FILE: {
            ...
            if (dirlen > 0) {
                ...
                for (const WCHAR *s = dirname + dirlen + 1; *s;
                     s += (wcslen(s) + 1)) {
...
int Fl_Native_File_Chooser::showdir() {
    ...
    //       from being visible in BROWSE_DIRECTORY mode. Walter Garm's
    //       comments:
    //
    //       --- Garms, Walter (GE EntSol, Security) wrote:
    //       With your help I was able to solve the problem of the network
    //       drives. For Version 6.0, at least, the BIF_SHAREABLE flag seems to
    //       have the opposite sense:  With BIF_SHAREABLE not set I see the
    //       mapped network drives, and with BIF_SHAREABLE set I do not.
    //       ---
...
int Fl_Native_File_Chooser::show() {
    int retval;
    if (_btype == BROWSE_DIRECTORY || _btype == BROWSE_MULTI_DIRECTORY ||
...
int Fl_Native_File_Chooser::count() const { return (_tpathnames); }
...
//
const char *Fl_Native_File_Chooser::directory() const { return (_directory); }
...
//    Can return NULL if none set.
//
const char *Fl_Native_File_Chooser::title() const { return (_title); }
...
//    Can return NULL if none set.
//
const char *Fl_Native_File_Chooser::filter() const { return (_filter); }
...
void Fl_Native_File_Chooser::parse_filter(const char *in) {
    ...
    char **wildcards =
        new char *[maxfilters];  // parsed wildcards (can be several)
    int t;
...
int Fl_Native_File_Chooser::filter_value() const {
    return (_ofn_ptr->nFilterIndex ? _ofn_ptr->nFilterIndex - 1
                                   : _nfilters + 1);
}
...
int Fl_Native_File_Chooser::filters() const { return (_nfilters); }
