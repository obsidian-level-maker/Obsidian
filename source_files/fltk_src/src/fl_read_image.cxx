...
static uchar *read_win_rectangle(uchar *p, int X, int Y, int w, int h,
                                 int alpha);

static void write_image_inside(Fl_RGB_Image *to, Fl_RGB_Image *from, int to_x,
                               int to_y)
/* Copy the image "from" inside image "to" with its top-left angle at
 coordinates to_x, to_y. Also, exchange top and bottom of "from". Image depth
 can differ between "to" and "from".
 */
{
    ...
    for (int i = from->h() - 1; i >= 0; i--) {
        if (from->d() == to->d())
            ...
            for (int j = 0; j < from->w(); j++) {
                memcpy(tobytes + j * to->d(), frombytes + j * from->d(),
                       from->d());
            }
        }
        tobytes += to_ld;
...
/* Captures rectangle x,y,w,h from a mapped window or GL window.
 All sub-GL-windows that intersect x,y,w,h, and their subwindows, are also
 captured.
 ...
 g: an Fl_Group
 p: as above
 x,y,w,h: a rectangle in g's coordinates if g is a window, or in g's parent
 window coords if g is a group alpha: as above full_img: NULL, or a previously
 captured image that encompasses the x,y,w,h rectangle and that will be
 partially overwritten with the new capture

 Return value:
 An Fl_RGB_Image* of depth 4 if alpha>0 or 3 if alpha = 0 containing the
 captured pixels.
 */
static Fl_RGB_Image *traverse_to_gl_subwindows(Fl_Group *g, uchar *p, int x,
                                               int y, int w, int h, int alpha,
    ...
    if (g->as_gl_window()) {
        ...
        Fl_RGB_Image *img =
            pi->rectangle_capture(g, x, y, w, h);  // bottom to top image
        if (full_img)
            full_img = img;  // top and bottom will be exchanged later
        else {  // exchange top and bottom to get a proper FLTK image
            uchar *data =
                (p ? p : new uchar[img->w() * img->h() * (alpha ? 4 : 3)]);
            full_img =
                new Fl_RGB_Image(data, img->w(), img->h(), alpha ? 4 : 3);
            ...
            delete img;
               ...
               (!full_img || (g->window() && g->window()->as_gl_window()))) {
        // the starting window or one inside a GL window
        if (full_img) g->as_window()->make_current();
        uchar *image_data;
        int alloc_img =
            (full_img != NULL ||
             p == NULL);  // false means use p, don't alloc new memory for image
#ifdef __APPLE_CC__
        // on Darwin + X11, read_win_rectangle() sometimes returns NULL when
        // there are subwindows
        do
            image_data =
                read_win_rectangle((alloc_img ? NULL : p), x, y, w, h, alpha);
        while (!image_data);
#else
        image_data =
            read_win_rectangle((alloc_img ? NULL : p), x, y, w, h, alpha);
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (c->as_window()) {
            int origin_x =
                x;  // compute intersection of x,y,w,h and the c window
            if (x < c->x()) origin_x = c->x();
            int origin_y = y;
            if (y < c->y()) origin_y = c->y();
            int width = c->w();
            if (origin_x + width > c->x() + c->w())
                width = c->x() + c->w() - origin_x;
            if (origin_x + width > x + w) width = x + w - origin_x;
            int height = c->w();
            if (origin_y + height > c->y() + c->h())
                height = c->y() + c->h() - origin_y;
            if (origin_y + height > y + h) height = y + h - origin_y;
            if (width > 0 && height > 0) {
                Fl_RGB_Image *img = traverse_to_gl_subwindows(
                    c->as_window(), p, origin_x - c->x(), origin_y - c->y(),
                    width, height, alpha, full_img);
            ...
            traverse_to_gl_subwindows(c->as_group(), p, x, y, w, h, alpha,
                                      full_img);
    }
    return full_img;
}
...
// buffer this is the version for X11 and WIN32. The mac version is in
// fl_read_image_mac.cxx

uchar *  // O - Pixel buffer or NULL if failed
fl_read_image(
    uchar *p,  // I - Pixel buffer or NULL to allocate
...
{
    if (w < 0 || fl_find(fl_window) ==
                     0) {  // read from off_screen buffer or title bar and frame
        return read_win_rectangle(
            p, X, Y, w, h,
            alpha);  // this function has an X11 and a WIN32 version
    }
    Fl_RGB_Image *img = traverse_to_gl_subwindows(Fl_Window::current(), p, X, Y,
                                                  w, h, alpha, NULL);
...
static uchar *read_win_rectangle(uchar *p, int X, int Y, int w, int h,
    ...
    unsigned index_mask, index_shift, red_mask, red_shift, green_mask,
        green_shift, blue_mask, blue_shift;
    ...
    //
    int allow_outside =
        w < 0;  // negative w allows negative X or Y, that is, window frame
    if (w < 0) w = -w;
    ...
    image = 0;
        ...
        if (allow_outside)
            ...
            win = fl_find(fl_window);
        if (win) {
            XTranslateCoordinates(fl_display, fl_window,
                                  RootWindow(fl_display, fl_screen), X, Y, &dx,
                                  &dy, &child_win);
            // screen dimensions
            Fl::screen_xywh(sx, sy, sw, sh, fl_screen);
        }
        if (!win ||
            (dx >= sx && dy >= sy && dx + w <= sx + sw && dy + h <= sy + sh)) {
            // the image is fully contained, we can use the traditional method
            // however, if the window is obscured etc. the function will still
            // fail. Make sure we catch the error and continue, otherwise an
            ...
            image =
                XCreateImage(fl_display, fl_visual->visual, fl_visual->depth,
                             ZPixmap, 0, buf, w, h, bpp, 0);
            if (!image) {
                if (buf) free(buf);
                return 0;
            }
            ...
            XErrorHandler old_handler = XSetErrorHandler(xgetimageerrhandler);
            XImage *subimg =
                XGetSubImage(fl_display, fl_window, X + noffx, Y + noffy, nw,
                             nh, AllPlanes, ZPixmap, image, noffx, noffy);
            XSetErrorHandler(old_handler);
            if (!subimg) {
                XDestroyImage(image);
                return 0;
            ...
            switch (image->bits_per_pixel) {
                case 8:
                    for (x = image->width, line_ptr = line; x > 0;
                        ...
                        line_ptr[0] =
                            255 * ((i >> red_shift) & red_mask) / red_mask;
                        line_ptr[1] = 255 * ((i >> green_shift) & green_mask) /
                                      green_mask;
                        line_ptr[2] =
                            255 * ((i >> blue_shift) & blue_mask) / blue_mask;
                    }
                    break;
