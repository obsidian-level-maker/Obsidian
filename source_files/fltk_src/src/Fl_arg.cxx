...
int Fl::arg(int argc, char **argv, int &i) {
    ...
    if (i >= argc - 1 || !v)
        return 0;  // all the rest need an argument, so if missing it is an
                   // error
...
/**
  ...
  A very simple command line parser can be found in
  <tt>examples/howto-parse-args.cxx</tt>
...
void Fl_Window::show(int argc, char **argv) {
    ...
    if (val)
        Fl::dnd_text_ops(strcasecmp(val, "true") == 0 ||
                         strcasecmp(val, "on") == 0 ||
                         strcasecmp(val, "yes") == 0);

    val = XGetDefault(fl_display, key, "tooltips");
    if (val)
        Fl_Tooltip::enable(strcasecmp(val, "true") == 0 ||
                           strcasecmp(val, "on") == 0 ||
                           strcasecmp(val, "yes") == 0);

    val = XGetDefault(fl_display, key, "visibleFocus");
    if (val)
        Fl::visible_focus(strcasecmp(val, "true") == 0 ||
                          strcasecmp(val, "on") == 0 ||
                          strcasecmp(val, "yes") == 0);
...
int XParseGeometry(const char *string, int *x, int *y, unsigned int *width,
                   unsigned int *height) {
    int mask = NoValue;
    char *strind;
