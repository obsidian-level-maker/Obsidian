...
// "$Id$"
//
// Implementation of Fl_Window::shape(Fl_Image*) for the Fast Light Tool Kit
// (FLTK).
//
...
static const unsigned char swapped[16] = {0, 8, 4, 12, 2, 10, 6, 14,
                                          1, 9, 5, 13, 3, 11, 7, 15};
...
static HRGN bitmap2region(Fl_Image *image) {
        ...
        for (int x = 0; x < image->w(); x++) {
            ...
            if (x > x0) {
                ...
                if (pData->rdh.nCount == 2000) {
                    HRGN h = ExtCreateRegion(
                        NULL, sizeof(RGNDATAHEADER) + (sizeof(RECT) * maxRects),
                        pData);
                    if (hRgn) {
    ...
    HRGN h = ExtCreateRegion(
        NULL, sizeof(RGNDATAHEADER) + (sizeof(RECT) * maxRects), pData);
    if (hRgn) {
        CombineRgn(hRgn, hRgn, h, RGN_OR);
        DeleteObject(h);
    } else
...
void Fl_Window::combine_mask() {
    typedef void (*XShapeCombineMask_type)(Display *, int, int, int, int,
                                           Pixmap, int);
    ...
    if (!beenhere) {
        ...
        XShapeCombineMask_f =
            (XShapeCombineMask_type)dlsym(handle, "XShapeCombineMask");
    ...
    shape_data_->lh_ = h();
    Fl_Image *temp =
        shape_data_->shape_->copy(shape_data_->lw_, shape_data_->lh_);
    Pixmap pbitmap = XCreateBitmapFromData(fl_display, fl_xid(this),
                                           (const char *)*temp->data(),
                                           temp->w(), temp->h());
    XShapeCombineMask_f(fl_display, fl_xid(this), ShapeBounding, 0, 0, pbitmap,
                        ShapeSet);
...
#if defined(__APPLE__)
    if (b) {
        // complement mask bits and perform bitwise inversion of all bytes and
        // also reverse top and bottom
        ...
        CGDataProviderRef provider = CGDataProviderCreateWithData(
            NULL, from, bytes_per_row * b->h(), MyProviderReleaseData);
        shape_data_->mask = CGImageMaskCreate(
            b->w(), b->h(), 1, 1, bytes_per_row, provider, NULL, false);
        CFRelease(provider);
    }
#endif
}
...
/* the image can be of any depth
 offset gives the byte offset from the pixel start to the byte used to construct
 the shape
 */
void Fl_Window::shape_alpha_(Fl_Image *img, int offset) {
    ...
    if (shape_data_->shape_) {
        // reverse top and bottom and convert to gray scale if img->d() == 3 and
        // complement bits
        ...
        CGDataProviderRef provider = CGDataProviderCreateWithData(
            NULL, from, w * h, MyProviderReleaseData);
        shape_data_->mask =
            CGImageMaskCreate(w, h, 8, 8, w, provider, NULL, false);
        CFRelease(provider);
    }
}
...
void Fl_Window::shape_alpha_(Fl_Image *img, int offset) {
    ...
    unsigned u;
    uchar byte, onebit;
    // build an Fl_Bitmap covering the non-fully transparent/black part of the
    // image
    const uchar *bits = new uchar[h * bytesperrow];  // to store the bitmap
    const uchar *alpha = (const uchar *)*img->data() +
                         offset;  // points to alpha value of rgba pixels
    for (i = 0; i < h; i++) {
        ...
        for (j = 0; j < w; j++) {
            ...
            onebit = onebit
                     << 1;  // move the single set bit one position to the left
            if (onebit == 0 || j == w - 1) {
                onebit = 1;
...
/** Assigns a non-rectangular shape to the window.
 This function gives an arbitrary shape (not just a rectangular region) to an
 Fl_Window. An Fl_Image of any dimension can be used as mask; it is rescaled to
 the window's dimension as needed.

 The layout and widgets inside are unaware of the mask shape, and most will act
 as though the window's rectangular bounding box is available to them. It is up
 to you to make sure they adhere to the bounds of their masking shape.

 The \p img argument can be an Fl_Bitmap, Fl_Pixmap, Fl_RGB_Image or
 Fl_Shared_Image: \li With Fl_Bitmap or Fl_Pixmap, the shaped window covers the
 image part where bitmap bits equal one, or where the pixmap is not fully
 transparent. \li With an Fl_RGB_Image with an alpha channel (depths 2 or 4),
 the shaped window covers the image part that is not fully transparent. \li With
 an Fl_RGB_Image of depth 1 (gray-scale) or 3 (RGB), the shaped window covers
 the non-black image part. \li With an Fl_Shared_Image, the shape is determined
 by rules above applied to the underlying image. The shared image should not
 have been scaled through Fl_Shared_Image::scale().

 Platform details:
 \li On the unix/linux platform, the SHAPE extension of the X server is
 required. This function does control the shape of Fl_Gl_Window instances. \li
 ...
 with depths 2 or 4, the image alpha channel becomes the shape mask such that
 areas with alpha = 0 are out of the shaped window; with depths 1 or 3, white
 and black are in and out of the shaped window, respectively, and other colors
 give intermediate masking scores. This function does nothing with class
 Fl_Gl_Window.

 The window borders and caption created by the window system are turned off by
 default. They can be re-enabled by calling Fl_Window::border(1).
...
void Fl_Window::draw() {
    if (shape_data_) {
        ...
        if (shape_data_->mask && (&CGContextClipToMask != NULL)) {
            CGContextClipToMask(fl_gc, CGRectMake(0, 0, w(), h()),
                                shape_data_->mask);  // requires Mac OS 10.4
        }
        CGContextSaveGState(fl_gc);
#elif defined(WIN32)
        if ((shape_data_->lw_ != w() || shape_data_->lh_ != h()) &&
            shape_data_->shape_) {
            ...
            SetWindowRgn(fl_xid(this), region,
                         TRUE);  // the system deletes the region when it's no
                                 // longer needed
            delete temp;
        }
    ...
    // drag the lower right corner. This code draws a little ribbed triangle for
    // dragging.
