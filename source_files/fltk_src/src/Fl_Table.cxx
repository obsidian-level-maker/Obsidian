...
// Ctor
Fl_Table::Fl_Table(int X, int Y, int W, int H, const char *l)
    : Fl_Group(X, Y, W, H, l) {
    _rows = 0;
    _cols = 0;
    _row_header_w = 40;
...
void Fl_Table::row_height(int row, int height) {
    ...
    if (row >= now_size) {
        ...
        while (now_size < row) _rowheights[now_size++] = height;
    }
    _rowheights[row] = height;
    table_resized();
    if (row <=
        botrow) {  // OPTIMIZATION: only redraw if onscreen or above screen
        redraw();
    }
    // ROW RESIZE CALLBACK
...
void Fl_Table::col_width(int col, int width) {
    ...
    table_resized();
    if (col <=
        rightcol) {  // OPTIMIZATION: only redraw if onscreen or to the left
        redraw();
    }
...
void Fl_Table::get_bounds(TableContext context, int &X, int &Y, int &W,
                          int &H) {
    switch (context) {
        case CONTEXT_COL_HEADER:
...
//
Fl_Table::TableContext Fl_Table::cursor2rowcol(int &R, int &C,
...
//
int Fl_Table::find_cell(TableContext context, int R, int C, int &X, int &Y,
                        int &W, int &H) {
...
void Fl_Table::_auto_drag_cb2(void *d) { ((Fl_Table *)d)->_auto_drag_cb(); }
...
void Fl_Table::_auto_drag_cb() {
    ...
    if (lx > x() + w() - 20) {
        ...
        if (hscrollbar->visible())
            ((Fl_Slider *)hscrollbar)
                ->value(hscrollbar->clamp(hscrollbar->value() + 30));
    ...
    if (ly > y() + h() - 20) {
        ...
        if (vscrollbar->visible()) {
            ((Fl_Slider *)vscrollbar)
                ->value(vscrollbar->clamp(vscrollbar->value() + 30));
        }
...
void Fl_Table::table_resized() {
    ...
    //    Resize scrollbars to enforce a constant trough width after a window
    //    resize.
    //
    {
        // Vertical scrollbar
        float vscrolltab =
            (table_h == 0 || tih > table_h) ? 1 : (float)tih / table_h;
        float hscrolltab =
            (table_w == 0 || tiw > table_w) ? 1 : (float)tiw / table_w;
#if FLTK_ABI_VERSION >= 10301
        // NEW
        int scrollsize =
            _scrollbar_size ? _scrollbar_size : Fl::scrollbar_size();
        ...
        vscrollbar->resize(wix + wiw - scrollsize, wiy, scrollsize,
...
void Fl_Table::cols(int val) {
    ...
    {
        int default_w =
            (_colwidths.size() > 0) ? _colwidths[_colwidths.size() - 1] : 80;
...
int Fl_Table::move_cursor(int R, int C) { return move_cursor(R, C, 1); }
...
int Fl_Table::handle(int event) {
    ...
    if (ret) {
        if (Fl::event_inside(hscrollbar) || Fl::event_inside(vscrollbar))
            return 1;
        if (context !=
                CONTEXT_ROW_HEADER &&  // mouse not in row header (STR#2742)
            context !=
                CONTEXT_COL_HEADER &&  // mouse not in col header (STR#2742)
    ...
    switch (event) {
        case FL_PUSH:
            ...
            if (_event_button == 1 && !_event_clicks) {
                ...
                damage_zone(current_row, current_col, select_row, select_col, R,
                            C);
            ...
            if (Fl_Widget::callback() &&      // callback defined?
                ...
                do_callback(context, R,
                            C);  // do callback with context (cell, header, etc)
            }
            // Handle selection if handling a left-click
            //    Use snapshot of _event_button we made before servicing user's
            //    cb's to avoid checking realtime state of buttons which may
            //    have changed during the user's callbacks.
            //
            switch (context) {
                case CONTEXT_CELL:
                    // FL_PUSH on a cell?
                ...
                case CONTEXT_NONE:
                    ...
                    if (_event_button == 1 &&
                        ...
                        current_row = 0;
                        select_row = rows() - 1;
                        damage_zone(current_row, current_col, select_row,
                                    select_col);
                        ret = 1;
                    }
                    break;
                ...
                case CONTEXT_COL_HEADER:
                    ...
                    if (_event_button == 1) {
                        ...
                        if (resizeflag) {
                            ...
                            //    "ret=1" ensures we get drag events from now
                            //    on. (C-1) is used if mouse is over the left
                            //    hand side of cell, so we resize the next
                            //    column on the left.
                            //
                            _resizing_col =
                                (resizeflag & RESIZE_COL_LEFT) ? C - 1 : C;
                            _resizing_row = -1;
                ...
                case CONTEXT_ROW_HEADER:
                    ...
                    if (_event_button == 1) {
                        ...
                        if (resizeflag) {
                            ...
                            //    "ret = 1" ensures we get drag events from now
                            //    on. (R-1) is used if mouse is over the top of
                            //    the cell, so that we resize the row above.
                            //
                            _resizing_row =
                                (resizeflag & RESIZE_ROW_ABOVE) ? R - 1 : R;
                            _resizing_col = -1;
        ...
        case FL_DRAG:
            ...
            if (_resizing_col > -1) {
                ...
                if (_event_button == 1 && _selecting == CONTEXT_CELL &&
                    ...
                    ret = 1;
                           ...
                           _selecting == CONTEXT_ROW_HEADER &&
                           context & (CONTEXT_ROW_HEADER | CONTEXT_COL_HEADER |
                                      CONTEXT_CELL)) {
                    if (select_row != R) {
                        damage_zone(current_row, current_col, select_row,
                                    select_col, R, C);
            ...
            // Enable autodrag if not resizing, and mouse has moved off table
            // edge
            ...
            switch (context) {
                ...
                case CONTEXT_TABLE:                  // release on dead zone
                    if (_resizing_col == -1 &&       // not resizing a column
                        ...
                        // Need this for eg. left clicking on a cell to select
                        // it
        ...
        case FL_KEYBOARD: {
            ...
            switch (_event_key) {
                ...
                case FL_Tab:
#if FLTK_ABI_VERSION >= 10303
                    if (!tab_cell_nav())
                        break;  // not navigating cells? let fltk handle it
                                // (STR#2862)
                    ...
                    break;
#else
                    break;  // without tab_cell_nav(), Fl_Table should default
                            // to navigating widgets, not cells
#endif
            }
...
*/
void Fl_Table::get_selection(int &row_top, int &col_left, int &row_bot,
                             int &col_right) {
    if (select_col > current_col) {
...
/**
  ...
  table. And set_selection(0,0,1,1) selects the four cells in rows 0 and 1,
  column 0 and 1.
...
*/
void Fl_Table::set_selection(int row_top, int col_left, int row_bot,
                             int col_right) {
...
void Fl_Table::draw() {
    ...
    // (unfortunately this clips FLTK's border, so we must draw it explicity
    // below)
    //
    ...
    fl_push_clip(wix, wiy, wiw, wih);
    {
        ...
        if (damage() & FL_DAMAGE_ALL) {
            ...
            if (row_header() && col_header()) {
                fl_rectf(wix, wiy, row_header_width(), col_header_height(),
                         color());
            }
            ...
            if (table_h < tih) {
                ...
                if (row_header()) {
                    // NOTE:
                    //     Careful with that lower corner; don't use tih; when
                    //     eg. table->box(FL_THIN_UP_FRAME) and hscrollbar
                    //     hidden, leaves a row of dead pixels.
                    //
        ...
        if (vscrollbar->visible() && hscrollbar->visible()) {
            fl_rectf(vscrollbar->x(), hscrollbar->y(), vscrollbar->w(),
                     hscrollbar->h(), color());
        }
        draw_cell(CONTEXT_ENDPAGE, 0, 0,  // let user's drawing
                  tix, tiy, tiw, tih);    // routines cleanup

        _redraw_leftcol = _redraw_rightcol = _redraw_toprow = _redraw_botrow =
            -1;
    }
    fl_pop_clip();
}
