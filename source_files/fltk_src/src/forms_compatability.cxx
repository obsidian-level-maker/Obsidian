...
void fl_show_form(Fl_Window *f, int place, int b, const char *n) {
    ...
    if (place & FL_PLACE_FULLSCREEN) f->fullscreen();

    if (place & (FL_PLACE_POSITION | FL_PLACE_GEOMETRY))
        f->position((f->x() < 0) ? Fl::w() - f->w() + f->x() - 1 : f->x(),
    ...
    if (place == FL_PLACE_FREE || place == FL_PLACE_SIZE) f->free_position();
...
int fl_show_question(const char *q1, const char *q2, const char *q3) {
    return fl_choice("%s\n%s\n%s", "No", "Yes", 0L, q1 ? q1 : "", q2 ? q2 : "",
                     q3 ? q3 : "");
}

int fl_show_choice(const char *q1, const char *q2, const char *q3,
                   int,  // number of buttons, ignored
                   const char *b0, const char *b1, const char *b2) {
    return fl_choice("%s\n%s\n%s", q1 ? q1 : "", q2 ? q2 : "", q3 ? q3 : "", b0,
                     b1, b2) +
           1;
}
