...
class MyData : public Fl_Group {
   ...
   public:
    ...
    void SetData(int col, const char *val) {
        if (col >= 0 && col < MAX_FIELDS) fields[col]->value(val);
    }
};

int main(int argc, char *argv[]) {
    Fl_Double_Window *win =
        new Fl_Double_Window(450, 400, "Tree As FLTK Widget Container");
    win->begin();
    {
        // Create the tree
        ...
        for (int t = 0; t < MAX_ROWS; t++) {
            ...
            {
                MyData *data =
                    new MyData(0, 0, FIELD_WIDTH * MAX_FIELDS, FIELD_HEIGHT);
                item->widget(data);
