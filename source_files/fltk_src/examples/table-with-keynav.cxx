...
class MyTable : public Fl_Table_Row {
    ...
    void draw_cell(TableContext context, int R = 0, int C = 0, int X = 0,
        ...
        switch (context) {
            ...
            case CONTEXT_CELL: {
                ...
                int selected =
                    G_rowselect->value() ? row_selected(R) : is_selected(R, C);
                fl_draw_box(FL_THIN_UP_BOX, X, Y, W, H,
                            selected ? FL_YELLOW : FL_WHITE);
   ...
   public:
    ...
    int GetSelectionSum() {
        ...
        for (int R = 0; R < rows(); R++) {
            for (int C = 0; C < cols(); C++) {
                if (G_rowselect->value() ? row_selected(R)
                                         : is_selected(R, C)) {
                    if (sum == -1) sum = 0;
                    sum += R * C;
                }
    ...
    void UpdateSum() {
        ...
        if (sum == -1) {
            sprintf(s, "(nothing selected)");
            G_sum->color(48);
        } else {
            sprintf(s, "%d", sum);
            G_sum->color(FL_WHITE);
        }
    ...
    int handle(int e) {
        ...
        switch (e) {
            ...
            case FL_KEYDOWN:
            case FL_DRAG: {
                // ret = 1;		// *don't* indicate we 'handled' these,
                // just update ('handling' prevents e.g. tab nav)
                UpdateSum();
                redraw();
                break;
            }
...
int main() {
    Fl::option(Fl::OPTION_ARROW_FOCUS,
               0);  // disable arrow focus nav (we want arrows to control cells)
    Fl_Double_Window win(862, 312, "table-with-keynav");
    win.begin();
    // Create table
    G_table = new MyTable(10, 30, win.w() - 20, win.h() - 70, "Times Table");
    G_table->tooltip(
        "Use mouse or Shift + Arrow Keys to make selections.\n"
    ...
    win.end();
    win.begin();
    G_sum = new Fl_Output(140, G_table->y() + G_table->h() + 10, 160, 25,
                          "Selection Sum:");
    G_sum->value("(nothing selected)");
    G_sum->color(48);
    G_sum->tooltip(
        "This field shows the sum of the selected cells in the table");
    win.end();
