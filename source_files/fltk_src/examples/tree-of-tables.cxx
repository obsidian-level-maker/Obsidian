...
class MyTable : public Fl_Table {
   ...
   public:
    MyTable(int X, int Y, int W, int H, const char *mode)
        : Fl_Table(X, Y, W, H) {
        rows(11);
        row_height_all(20);
        row_header(1);
        ...
        switch (context) {
            ...
            case CONTEXT_CELL: {  // Draw data in cells
                ...
                if (strcmp(mode, "Addition") == 0) {
                    sprintf(s, "%d", ROW + COL);
                } else if (strcmp(mode, "Subtract") == 0) {
                    sprintf(s, "%d", ROW - COL);
                } else if (strcmp(mode, "Multiply") == 0) {
                    sprintf(s, "%d", ROW * COL);
                } else if (strcmp(mode, "Divide") == 0) {
                    if (COL == 0)
                        sprintf(s, "N/A");
                    else
                        sprintf(s, "%.2f", (float)ROW / (float)COL);
                } else if (strcmp(mode, "Exponent") == 0) {
                    sprintf(s, "%g", powf((float)ROW, (float)COL));
                } else if (strcmp(mode, "SinCos") == 0) {
                    sprintf(s, "%.2f",
                            sin((ROW / 10.0) * PI) * cos((COL / 10.0) * PI));
                } else {
                    sprintf(s, "???");
                }
                fl_color(col);
                fl_rectf(X, Y, W, H);  // bg
                fl_color(FL_GRAY0);
                fl_draw(s, X, Y, W, H, FL_ALIGN_CENTER);  // text
                fl_color(color());
                fl_rect(X, Y, W, H);  // box
                fl_pop_clip();
                return;
...
int main(int argc, char *argv[]) {
    ...
    win->begin();
    {
        ...
        tree->item_draw_mode(
            tree->item_draw_mode() |
            FL_TREE_ITEM_DRAW_LABEL_AND_WIDGET |  // draw item with widget()
                                                  // next to it
            FL_TREE_ITEM_HEIGHT_FROM_WIDGET);     // make item height follow
                                                  // table's height
...
int main(int argc, char *argv[]) {
    fl_alert(
        "This example must have FLTK_ABI_VERSION enabled to work properly.");
    return 1;
}
#endif
