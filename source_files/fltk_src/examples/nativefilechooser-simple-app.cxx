...
class Application : public Fl_Window {
    ...
    // 'Open' the file
    void open(const char *filename) { printf("Open '%s'\n", filename); }
    // 'Save' the file
    ...
    void save(const char *filename) {
        ...
        if (!exist(filename)) {
            FILE *fp =
                fl_fopen(filename, "w");  // create file if it doesn't exist
            if (fp) {
    ...
    static void open_cb(Fl_Widget *w, void *v) {
        ...
        app->fc->title("Open");
        app->fc->type(Fl_Native_File_Chooser::BROWSE_FILE);  // only picks files
                                                             // that exist
        switch (app->fc->show()) {
            case -1:
                break;  // Error
    ...
    static void saveas_cb(Fl_Widget *w, void *v) {
        ...
        app->fc->type(
            Fl_Native_File_Chooser::BROWSE_SAVE_FILE);  // need this if file
                                                        // doesn't exist yet
        switch (app->fc->show()) {
            case -1:
                break;  // Error
