   ...
   public:
    ...
    void draw(void) {
        ...
        if (!shaderProgram) {
            GLuint vs;
            GLuint fs;
            int Mslv, mslv;  // major and minor version numbers of the shading
                             // language
      ...
      fragColour = colourV;\
            ...
            glDeleteShader(vs);
            glDeleteShader(fs);
            // Upload vertices (1st four values in a row) and colours (following
            // four values)
            ...
            glBufferData(GL_ARRAY_BUFFER, 4 * 8 * sizeof(GLfloat), vertexData,
                         GL_STATIC_DRAW);

            glEnableVertexAttribArray((GLuint)positionAttribute);
            glEnableVertexAttribArray((GLuint)colourAttribute);
            glVertexAttribPointer((GLuint)positionAttribute, 4, GL_FLOAT,
                                  GL_FALSE, 8 * sizeof(GLfloat), 0);
            glVertexAttribPointer((GLuint)colourAttribute, 4, GL_FLOAT,
    ...
    virtual int handle(int event) {
        ...
        if (event == FL_PUSH && gl_version_major >= 3) {
            ...
            data[0] *= factor;
            glBufferSubData(GL_ARRAY_BUFFER, 24 * sizeof(GLfloat),
                            4 * sizeof(GLfloat), data);
            redraw();
...
void output_win(SimpleGL3Window *gl) {
    ...
    Fl_Light_Button *lb =
        new Fl_Light_Button(300, 280, 500, 20, "Double-Buffered");
