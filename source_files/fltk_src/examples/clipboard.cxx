...
/* Displays and follows the content of the clipboard with either image or text
 * data
 */
...
class chess : public Fl_Box{
    ...
    public : chess(int x, int y, int w, int h) : Fl_Box(
        FL_FLAT_BOX, x, y, w, h, 0){align(FL_ALIGN_CENTER | FL_ALIGN_CLIP);
}
void draw() {
    draw_box();
    Fl_Image *im = image();
    if (im) {  // draw the chess pattern below the box centered image
        int X = x() + (w() - im->w()) / 2, Y = y() + (h() - im->h()) / 2,
            W = im->w(), H = im->h();
...
class clipboard_viewer
    : public Fl_Tabs {  // use tabs to display as appropriate the image or
                        // textual content of the clipboard
   public:
    clipboard_viewer(int x, int y, int w, int h) : Fl_Tabs(x, y, w, h){};
    virtual int handle(int event) {
        if (event != FL_PASTE) return Fl_Tabs::handle(event);
        if (strcmp(Fl::event_clipboard_type(), Fl::clipboard_image) ==
            0) {  // an image is being pasted
            Fl_Image *im = (Fl_Image *)
                Fl::event_clipboard();  // get it as an Fl_Image object
            if (!im) return 1;
            char title[300];
            sprintf(title, "%dx%d", im->w(),
                    im->h());  // display the image original size
#ifdef WIN32
            OpenClipboard(
                NULL);  // display extra technical info about clipboard content
            ...
            float scale_x =
                (float)im->w() /
                image_box
                    ->w();  // rescale the image if larger than the display box
...
void cb(Fl_Widget *wid, clipboard_viewer *tabs) {
    if (Fl::clipboard_contains(Fl::clipboard_image)) {
        Fl::paste(*tabs, 1,
                  Fl::clipboard_image);  // try to find image in the clipboard
        return;
    }
...
void clip_callback(int source,
                   void *data) {  // called after clipboard was changed or at
...
int main(int argc, char **argv) {
    ...
    Fl_Group *g = new Fl_Group(
        5, 30, 490, 460, Fl::clipboard_image);  // g will display the image form
    g->box(FL_FLAT_BOX);
    ...
    display = new Fl_Text_Display(
        5, 30, 490, 460,
        Fl::clipboard_plain_text);  // display will display the text form
    ...
    Fl_Button *refresh =
        new Fl_Button(10, 510, 200, 25, "Refresh from clipboard");
    ...
    Fl::add_clipboard_notify(clip_callback,
                             tabs);  // will update with new clipboard content
                                     // immediately or at application activation

    Fl_Image::RGB_scaling(
        FL_RGB_SCALING_BILINEAR);  // set bilinear image scaling method
    return Fl::run();
}
