...
//      implementation, submitted to erco for Fl_Table Fl_Table[1.10/LGPL]
//      05/17/03 Greg Ercolano     -- Small mods to follow changes to Fl_Table
//      Fl_Table[1.20/LGPL] 02/22/04 Jean-Marc Lienher -- Keyboard nav and mouse
//      selection Fl_Table[1.21/LGPL] 02/22/04 Greg Ercolano     -- Small
//      reformatting mods, comments
//         FLTK[1.3.0/LGPL] 10/26/10 Greg Ercolano     -- Moved from Fl_Table to
//         FLTK 1.3.x, CMP compliance
//
...
class Spreadsheet : public Fl_Table {
   ...
   protected:
    void draw_cell(TableContext context, int = 0, int = 0, int = 0, int = 0,
                   int = 0, int = 0);
    void event_callback2();  // table's event callback (instance)
    static void event_callback(Fl_Widget *,
                               void *v) {  // table's event callback (static)
   ...
   public:
    Spreadsheet(int X, int Y, int W, int H, const char *L = 0)
        ...
        input->when(FL_WHEN_ENTER_KEY_ALWAYS);  // callback triggered when user
                                                // hits Enter
    ...
    void set_value_hide() {
        ...
        input->hide();
        window()->cursor(FL_CURSOR_DEFAULT);  // XXX: if we don't do this,
                                              // cursor can disappear!
    }
    ...
    void start_editing(int R, int C) {
        ...
        char s[30];
        sprintf(s, "%d",
                values[R][C]);  // Load input widget with cell's current value
        input->value(s);
    ...
    int sum_rows(int C) {
        ...
        for (int r = 0; r < rows() - 1;
             ++r)  // -1: don't include cell data in 'totals' column
            sum += values[r][C];
        return (sum);
    }
    ...
    int sum_cols(int R) {
        ...
        for (int c = 0; c < cols() - 1;
             ++c)  // -1: don't include cell data in 'totals' column
            sum += values[R][c];
        return (sum);
    }
...
void Spreadsheet::draw_cell(TableContext context, int R, int C, int X, int Y,
    ...
    switch (context) {
        ...
        case CONTEXT_COL_HEADER:  // table wants us to draw a column heading (C
                                  // is column)
            fl_font(FL_HELVETICA | FL_BOLD,
        ...
        case CONTEXT_ROW_HEADER:  // table wants us to draw a row heading (R is
                                  // row)
            fl_font(FL_HELVETICA | FL_BOLD,
        ...
        case CONTEXT_CELL: {  // table wants us to draw a cell
            ...
            fl_push_clip(X + 3, Y + 3, W - 6, H - 6);
            {
                fl_color(FL_BLACK);
                if (C == cols() - 1 ||
                    R == rows() - 1) {  // Last row or col? Show total
                    fl_font(FL_HELVETICA | FL_BOLD, 14);  // ..in bold font
                    if (C == cols() - 1 &&
                        R == rows() - 1) {  // Last row+col? Total all cells
                        sprintf(s, "%d", sum_all());
        ...
        case CONTEXT_CELL: {         // A table event occurred on a cell
            switch (Fl::event()) {   // see what FLTK event caused it
                case FL_PUSH:        // mouse click?
                    ...
                    if (R != rows() - 1 &&
                        C != cols() -
                                 1)  // only edit cells not in total's columns
                ...
                case FL_KEYBOARD:  // key press in table?
                    ...
                    if (Fl::event() == FL_KEYBOARD && Fl::e_text[0] != '\r') {
                        input->handle(
                            Fl::event());  // pass keypress to input widget
                    }
                    return;
            }
            return;
...
int main() {
    Fl::option(Fl::OPTION_ARROW_FOCUS,
               1);  // we want arrow keys to navigate table's widgets
    Fl_Double_Window *win = new Fl_Double_Window(
        922, 382, "Fl_Table Spreadsheet with Keyboard Navigation");
