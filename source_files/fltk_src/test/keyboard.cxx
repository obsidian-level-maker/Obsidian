    ...
    while (Fl::wait()) {
        ...
        for (int i = 0; i < window->children(); i++) {
            ...
            if (b->callback() == (Fl_Callback *)key_cb) {
                ...
                if (btn->value() != state) btn->value(state);
        ...
        if (!k)
            ...
            for (int i = 0; i < int(sizeof(table) / sizeof(*table)); i++)
                if (table[i].n == k) {
                    keyname = table[i].text;
                    break;
                }
        }
        if (strcmp(key_output->value(), keyname)) key_output->value(keyname);

        str = Fl::event_text();
        if (strcmp(text_output->value(), str)) text_output->value(str);
    }
    return 0;
}
