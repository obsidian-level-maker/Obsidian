...
void close_callback(void) { exit(0); }

//
// 'create_callback()' - Handle clicks on the create button.
//

void create_callback(void) { fc->type(fc->type() ^ Fl_File_Chooser::CREATE); }

//
// 'dir_callback()' - Handle clicks on the directory button.
//

void dir_callback(void) { fc->type(fc->type() ^ Fl_File_Chooser::DIRECTORY); }
...
void multi_callback(void) { fc->type(fc->type() ^ Fl_File_Chooser::MULTI); }
          ...
          int)               // I - Length of header data (unused)
{
    ...
    if (memcmp(header, "%PDF", 4) != 0) return 0;
    ...
    sprintf(command,
            ...
            "-dFirstPage=1 -dLastPage=1 \'%s\' 2>/dev/null",
            preview, name);
...
void show_callback(void) {
    ...
    if (filter->value()[0]) fc->filter(filter->value());
