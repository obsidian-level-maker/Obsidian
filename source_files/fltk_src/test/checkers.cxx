...
const piece flip[9] = {EMPTY,     WHITE,     BLACK, 0,   0,
                       WHITEKING, BLACKKING, 0,     BLUE};
...
char check(int target, int direction) {
    ...
    tb[target] = EMPTY;
    tb[src] = EMPTY;
    int safe = ((tb[src - 4] & FRIEND && tb[src - 8] & ENEMY) ||
...
void analyzemove(int direction, int src) {
    ...
    if (!tb[target]) {
        ...
        piece a = tb[src];
        tb[src] = EMPTY;
        if (check(target, 4) || check(target, 5) || check(target, -4) ||
            check(target, -5) || (tb[src + 4] & ENEMY && check(src + 4, 4)) ||
...
void evaluateboard(node *n, int print) {
    ...
    for (i = 5; i < 40; i++) switch (tb[i]) {
            ...
            case FRIENDKING:
                ...
                if (tb[i + 4] & ENEMY && !tb[i + 8] &&
                    !(tb[i + 4] == ENEMYKING && !tb[i - 4]))
                    n->threat++;
                if (tb[i + 5] & ENEMY && !tb[i + 10] &&
                    !(tb[i + 5] == ENEMYKING && !tb[i - 5]))
                    n->threat++;
            case FRIEND:
                friendpieces++;
    ...
    total = spiece * (friendpieces - enemypieces) +
            (sking - spiece) * (friendkings - enemykings) +
            //	mobil?
            sdeny * (n->deny - f->deny) + spin * (n->pin - f->pin) +
            sthreat * (n->threat - f->threat) +
            sgrad * (n->gradient - f->gradient) + sback * (n->back - f->back) +
            smoc2 * (n->moc2 - f->moc2) + smoc3 * (n->moc3 - f->moc3) +
    ...
    if (print) {
        ...
        printf("cent\t%d\t%d\t%dn", enemycent, friendcent,
               scent * (friendcent - enemycent));
        printf("kcent\t%d\t%d\t%d\n", enemykcent, friendkcent,
               skcent * (friendkcent - enemykcent));
        printf("total:\t\t\t%d\n", total);
    } else {
        n->value = total;
        evaluated++;
...
void movepiece(node *f, int i, node *jnode) {
    ...
    for (int k = 0; k < 4; k++) {
        ...
        } else if (((b[j] ^ b[i]) & (WHITE | BLACK)) == (WHITE | BLACK) &&
                   !b[j + direction]) {
...
int fullexpand(node *f, int level) {
    if (didabort() || nodes > maxnodes - (maxply * 10) ||
        evaluated > maxevaluate)
        return (0);
    expandnode(f);
    if (!f->son) return (1);
    piece oldboard[45];
...
void newgame(void) {
    ...
    centralsquares[15] = centralsquares[16] = centralsquares[19] =
        centralsquares[20] = centralsquares[24] = centralsquares[25] =
...
void dumpnode(node *n, int help) {
    ...
    int y = n->to;
    if (help)
        printf("%c%c %c%c\t- ", usermoves(x, 1), usermoves(x, 2),
               usermoves(y, 1), usermoves(y, 2));
    printf("%s %ss from %c%c to %c%c", n->who ? "White" : "Black",
           n->jump ? "jump" : "move", usermoves(x, 1), usermoves(x, 2),
           usermoves(y, 1), usermoves(y, 2));
    if (n->jump) {
        for (int i = 0; i < 32; i++)
            if (n->jump & (1 << i))
...
void positioncursor(int i) {
    printf("\033[%d;%dH", usermoves(i, 2) - '0' + 1,
           2 * (usermoves(i, 1) - 'A') + 1);
}
...
node *getusermove(void) {
    ...
    switch (toupper(m1[0])) {
        ...
        case '+':
            ...
            goto J2;
        case '-':
            if (maxevaluate > 1) maxevaluate = maxnodes = maxevaluate / 2;
        J2:
...
int VT100main() {
    ...
    for (;;) {
        if (playing) {
            expandnode(root);
            if (!root->son) {
                printf("%s has no move.  Game over.",
                       root->who ? "Black" : "White");
                playing = autoplay = 0;
            }
        }
        node *move;
...
void Board::draw() {
    ...
    for (x = 0; x < 8; x++)
        for (int y = 0; y < 8; y++) {
            if (!((x ^ y) & 1))
                fl_rectf(BORDER + x * BOXSIZE, BORDER + y * BOXSIZE,
...
int Board::handle(int e) {
    ...
    switch (e) {
        ...
        case FL_SHORTCUT:
            ...
            return 0;
        case FL_DRAG:
            drag_piece(erase_this, Fl::event_x() - deltax,
                       Fl::event_y() - deltay);
            return 1;
        case FL_RELEASE:
