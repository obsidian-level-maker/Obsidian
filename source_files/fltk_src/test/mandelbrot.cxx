...
void idle(void *) {
    if (!mbrot.d->idle() && !(jbrot.d && jbrot.d->idle()))
        Fl::remove_idle(idle);
}

void set_idle() { Fl::add_idle(idle); }
...
void Drawing_Window::update_label() {
    char buffer[128];
    sprintf(buffer, "%+.10f", d->X);
    x_input->value(buffer);
    sprintf(buffer, "%+.10f", d->Y);
    y_input->value(buffer);
    sprintf(buffer, "%.2g", d->scale);
    w_input->value(buffer);
}
