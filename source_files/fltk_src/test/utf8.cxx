...
static void refresh_cb(Fl_Widget *, long) { main_win->redraw(); }

static void own_face_cb(Fl_Widget *, void *) {
    int font_idx;
    int cursor_restore = 0;
    static int i_was =
        -1;  // used to keep track of where we were in the list...

    if (i_was < 0) {  // not been here before
        i_was = 1;
    } else {
        i_was =
            fontobj->topline();  // record which was the topmost visible line
        fontobj->clear();
        // Populating the font widget can be slower than an old dog with three
        // legs on a bad day, show a wait cursor
    ...
    for (font_idx = first_free; font_idx < font_count; font_idx++) {
        ...
        if (own_face->value() == 0) {
            ...
            // Suppress subsequent formatting - some MS fonts have '@' in their
            // name
            *p++ = '@';
            *p++ = '.';
            strcpy(p, name);
...
class right_left_input : public Fl_Input {
   public:
    ...
    void drawtext(int X, int Y, int W, int H) {
        ...
        fl_rtl_draw(value(), strlen(value()), X + W,
...
int main(int argc, char **argv) {
    int l;
    const char *latin1 =
        "\x41\x42\x43\x61\x62\x63\xe0\xe8\xe9\xef\xe2\xee\xf6\xfc\xe3\x31\x32"
        "\x33";
    ...
    wchar_t r_to_l_txt1[] = {/*8238,*/
                             1610, 0x20, 1608, 0x20, 1606, 0x20, 1604,
