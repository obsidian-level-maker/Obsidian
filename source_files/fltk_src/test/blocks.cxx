...
Fl_Pixmap *normal_pixmaps[] = {&red_pixmap,    &green_pixmap, &blue_pixmap,
                               &yellow_pixmap, &cyan_pixmap,  &magenta_pixmap,
                               &gray_pixmap};
Fl_Pixmap *bomb_pixmaps[] = {&red_bomb_pixmap,  &green_bomb_pixmap,
                             &blue_bomb_pixmap, &yellow_bomb_pixmap,
                             &cyan_bomb_pixmap, &magenta_bomb_pixmap,
                             &gray_bomb_pixmap};

const unsigned char screen_bits[] = {0xff, 0x55, 0xff, 0xaa,
                                     0xff, 0x55, 0xff, 0xaa};
...
BlockSound::BlockSound() {
    ...
    data_handle =
        GlobalAlloc(GMEM_MOVEABLE | GMEM_SHARE, format.nSamplesPerSec * 4);
    ...
    if (sample_size) {
        ...
        *sample_ptr++ = 0;
        ...
        for (int j = max_sample; j > 0; j--, sample_ptr++) {
            ...
            *sample_ptr =
                (int)(volume * freq * sample + (1.0 - freq) * sample_ptr[-2]);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    BlockWindow *bw = new BlockWindow(
        BLOCK_COLS * BLOCK_SIZE, BLOCK_ROWS * BLOCK_SIZE + 20, "Block Attack!");
            ...
            if (interval_ < 0.0) {
                ...
                for (i = 0; i < 2; i++) {
                    ...
                    for (j = 0, sample_ptr = sound_->sample_data + i;
                         j < sound_->sample_size; j++, sample_ptr += 2)
...
int BlockWindow::handle(int event) {
    ...
    switch (event) {
        case FL_KEYBOARD:
            if (Fl::event_text()) {
                if (strcmp(Fl::event_text(), "+") == 0) up_level();
            }
            break;
        case FL_PUSH:
            ...
            for (j = 0, c = columns_; !count && j < num_columns_; j++, c++)
                for (k = 0, b = c->blocks; !count && k < c->num_blocks;
                     k++, b++)
                    if (mx >= c->x && mx < (c->x + BLOCK_SIZE) && my >= b->y &&
                        my < (b->y + BLOCK_SIZE)) {
                        if (b->bomb)
                            count = bomb(b->color);
                        else
...
void BlockWindow::timeout_cb(BlockWindow *bw) {
    ...
    printf("%.3f (%+f - %f)\n", curtime.tv_sec + 0.000001 * curtime.tv_usec,
           curtime.tv_sec - lasttime.tv_sec +
               0.000001 * (curtime.tv_usec - lasttime.tv_usec),
           bw->interval_);
    lasttime = curtime;
#endif  // DEBUG
            ...
            if (b->color > (BLOCK_BLAST + 1)) {
                ...
                if (!c->num_blocks) {
                    ...
                    if (i < bw->num_columns_) {
                        memmove(c, c + 1,
                                (bw->num_columns_ - i) * sizeof(Column));
                    }
    ...
    for (i = bw->num_columns_, c = bw->columns_, lastx = c->x; i > 0;
         i--, c++) {
        if (c->x > lastx) {
            c->x -= 8;
            bw->redraw();
        ...
        if (bw->count_ <= 0) {
            ...
            if (bw->num_columns_ == BLOCK_COLS) {
                ...
                for (j = 0, b = c->blocks; j < BLOCK_ROWS; j++, b++) {
                    b->bomb =
                        bw->num_colors_ > 3 && (rand() & 127) < bw->num_colors_;
