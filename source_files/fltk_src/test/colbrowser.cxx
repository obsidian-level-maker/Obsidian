...
int main(int argc, char *argv[]) {
    ...
    if (slash) strcpy(slash, "/../Resources/rgb.txt");
#else
...
static int search_entry(int r, int g, int b) {
    ...
    for (i = j = 0; db->r < 256; db++, i++) {
        ...
        diff = unsigned(3.0 * (FL_abs(r - db->r)) + (5.9 * FL_abs(g - db->g)) +
                        (1.1 * (FL_abs(b - db->b))));
#else
        diff = unsigned(3.0 * (diffr * diffr) + 5.9 * (diffg * diffg) +
                        1.1 * (diffb * diffb));
#endif
...
static void search_rgb(Fl_Widget *, long) {
    ...
    if (i < top || i > (top + 15)) colbr->topline(i - 8);
    colbr->select(i + 1, 1);
    // fl_unfreeze_form(cl);
}
...
static void done_cb(Fl_Widget *, long) { exit(0); }
