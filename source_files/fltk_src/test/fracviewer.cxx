...
GLfloat Ex = INIT_EX, /* flying parameters */
    Ey = INIT_EY, Ez = INIT_EZ, EyeMove = INIT_MOVE,

        EyeDist = INIT_DIST, /* polar params */
    AzSpin = INIT_AZ_SPIN, ElSpin = INIT_EL_SPIN,
...
void FlyLookFrom(GLfloat x, GLfloat y, GLfloat z, GLfloat az, GLfloat el) {
    ...
    gluLookAt(x, y, z, x + lookat[0], y + lookat[1], z + lookat[2], up[0],
              up[1], up[2]);
}
...
void MoveOn(int v) {
    if (v && ((MoveMode == FLYING && EyeMove != 0) ||
        ...
        agvMoving = 1;
        if (AllowIdle) glutIdleFunc(agvMove);
    } else {
        agvMoving = 0;
        if (AllowIdle) glutIdleFunc(NULL);
    }
}
...
void agvSetAllowIdle(int allowidle) {
    if ((AllowIdle = allowidle)) MoveOn(1);
}
    ...
    if (state == GLUT_DOWN && downb == -1) {
        ...
        switch (button) {
            case GLUT_LEFT_BUTTON:
                if (MoveMode != FLYING) {
                    AzSpin = -dAz;
                    if (AzSpin < MIN_AZSPIN && AzSpin > -MIN_AZSPIN) AzSpin = 0;
                    ElSpin = -dEl;
                    if (ElSpin < MIN_ELSPIN && ElSpin > -MIN_ELSPIN) ElSpin = 0;
                }
                AdjustingAzEl = 0;
                MoveOn(1);
                break;
