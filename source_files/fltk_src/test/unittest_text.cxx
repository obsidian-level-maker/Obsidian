...
class TextExtentsTest : public Fl_Widget {
    void DrawTextAndBoxes(const char *txt, int X, int Y) {
        ...
        // Then we draw the text to show how it fits insode each of the two
        // boxes
        fl_color(FL_BLACK);
        fl_draw(txt, X, Y);
    }
   ...
   public:
    static Fl_Widget *create() {
        return new TextExtentsTest(TESTAREA_X, TESTAREA_Y, TESTAREA_W,
                                   TESTAREA_H);
    }
    ...
    void draw(void) {
        ...
        fl_push_clip(x0, y0, w0, h0);  // reset local clipping
        {
            ...
            int xx = x0 + 55;
            int yy = y0 + 40;
            DrawTextAndBoxes("!abcdeABCDE\"#A", xx, yy);
            yy += 50;  // mixed string
            DrawTextAndBoxes("oacs", xx, yy);
            xx += 100;  // small glyphs
            DrawTextAndBoxes("qjgIPT", xx, yy);
            yy += 50;
            xx -= 100;  // glyphs with descenders
            DrawTextAndBoxes("````````", xx, yy);
            yy += 50;  // high small glyphs
            DrawTextAndBoxes("--------", xx, yy);
            yy += 50;  // mid small glyphs
            DrawTextAndBoxes("________", xx, yy);
            yy += 50;  // low small glyphs

            fl_font(FL_HELVETICA, 14);
            fl_color(FL_RED);
            fl_draw("fl_measure bounding box in RED", xx, yy);
            yy += 20;
            fl_color(FL_GREEN);
            fl_draw("fl_text_extents bounding box in GREEN", xx, yy);
            fl_color(FL_BLACK);
            xx = x0 + 10;
            yy += 30;
            fl_draw(
                "NOTE: On systems with text anti-aliasing (e.g. OSX Quartz)",
                xx, yy);
            w0 = h0 = 0;
            fl_measure("NOTE: ", w0, h0, 0);
            xx += w0;
            yy += h0;
            fl_draw("text may leak slightly outside the fl_text_extents()", xx,
                    yy);
        }
