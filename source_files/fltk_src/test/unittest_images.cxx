...
#define CB \
    (1)  // 1 to show the checker board background for alpha images,
         // 0 otherwise
#define DX \
    (0)  // additional (undefined (0)) pixels per line, must be >= 0
         // ignored (irrelevant), if LX == 0 (see below)
#define IMG \
    (1)  // 1 to use Fl_RGB_Image for drawing images with transparency,
...
#define LX \
    (0)  // 0 for default: ld() = 0, i.e. ld() defaults (internally) to w()*d()
         // +1: ld() = (w() + DX) * d()
         // -1 to flip image vertically: ld() = - ((w() + DX) * d())
#define FLIPH \
    (0)  // 1 = Flip image horizontally (only if IMG == 0)
...
class ImageTest : public Fl_Box {
   public:
    ...
    ImageTest(int x, int y, int w, int h) : Fl_Box(x, y, w, h) {
        ...
        align(FL_ALIGN_INSIDE | FL_ALIGN_BOTTOM | FL_ALIGN_LEFT |
              FL_ALIGN_WRAP);
        box(FL_BORDER_BOX);
    }
    void draw() {
        ...
        yy = y() + 10 + 134;
        fl_color(FL_BLACK);
        fl_rect(xx, yy, 130, 130);  // black frame
        fl_color(FL_WHITE);
        fl_rectf(xx + 1, yy + 1, 128, 128);  // white background
#if CB                                       // checker board
        fl_color(FL_BLACK);
        fl_rectf(xx + 65, yy + 1, 64, 64);
        fl_color(FL_BLACK);
        fl_rectf(xx + 1, yy + 65, 64, 64);
#endif
#if IMG
...
#endif
        fl_color(FL_BLACK);
        fl_draw("Gray+Alpha", xx + 134, yy + 64);
    }
};
