...
void update_screeninfo(Fl_Widget *b, void *p) {
    ...
    for (int n = 0; n < Fl::screen_count(); n++) {
        ...
        sprintf(line, "Screen %d: %dx%d@%d,%d DPI:%.1fx%.1f", n, w, h, x, y,
                dpih, dpiv);
        browser->add(line);
...
void exit_cb(Fl_Widget *, void *) { exit(0); }
...
int main(int argc, char **argv) {
    ...
    fullscreen_window window(400, 400 + 30 * NUMB);
    window.end();
    ...
    window.b3 =
        new Fl_Toggle_Light_Button(50, y, window.w() - 60, 30, "FullScreen");
    window.b3->callback(fullscreen_cb, w);
    y += 30;

    window.b4 =
        new Fl_Toggle_Light_Button(50, y, window.w() - 60, 30, "All Screens");
