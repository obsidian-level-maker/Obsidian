   ...
   protected:
    void draw() {
        ...
        if (rt_align & FL_ALIGN_LEFT) {
            ...
            dy = (int)(-sin(M_PI * (double)(rt_angle + 180) / 180.) *
                       (double)dx);
            dx =
                (int)(cos(M_PI * (double)(rt_angle + 180) / 180.) * (double)dx);
        } else {
        ...
        if (labeltype() == FL_SHADOW_LABEL)
            shadow_label(x() + w() / 2 + dx, y() + h() / 2 + dy);
        else if (labeltype() == FL_ENGRAVED_LABEL)
            engraved_label(x() + w() / 2 + dx, y() + h() / 2 + dy);
        else if (labeltype() == FL_EMBOSSED_LABEL)
    ...
    void engraved_label(int X, int Y) {
        static int data[7][3] = {{1, 0, FL_LIGHT3},  {1, 1, FL_LIGHT3},
...
Fl_Menu_Item choices[] = {{"FL_NORMAL_LABEL", 0, normal_cb},
