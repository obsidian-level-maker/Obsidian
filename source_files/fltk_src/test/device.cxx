...
class MyWidget2 : public Fl_Box {
   protected:
    void draw() {
        ...
        fl_color(200, 0, 200);
        fl_polygon(x() + 35, y() + 72, x() + 33, y() + 95, x() + 48, y() + 95,
                   x() + 43, y() + 72);
        fl_color(FL_GREEN);
        fl_loop(x() + 35, y() + 72, x() + 33, y() + 95, x() + 48, y() + 95,
                x() + 43, y() + 72);
...
void copy(Fl_Widget *, void *data) {
    ...
    if (strcmp(operation, "Fl_Printer") == 0) {
        ...
        if (!p->start_job(1)) {
            p->start_page();
            if (target->as_window())
                p->print_window(target->as_window());
            else
...
void target_cb(Fl_Widget *wid, void *data) { target = (Fl_Widget *)data; }

void operation_cb(Fl_Widget *wid, void *data) { operation = wid->label(); }
...
int main(int argc, char **argv) {
    ...
    b_bitmap.image(
        new Fl_Bitmap(sorceress_bits, sorceress_width, sorceress_height));
    ...
    Fl_Box tx(120, 492, 230, 50,
              "Background is not printed because\nencapsulating group, which "
    ...
    operation = rb->label();
    rb = new Fl_Radio_Round_Button(5, 22, 150, 12, "Fl_Copy_Surface");
    rb->callback(operation_cb, NULL);
    rb = new Fl_Radio_Round_Button(5, 39, 150, 12, "Fl_Printer");
    rb->callback(operation_cb, NULL);
    g1->end();
    ...
    target = w2;
    rb = new Fl_Radio_Round_Button(170, 22, 150, 12, "Sub-window");
    rb->callback(target_cb, w3);
    rb = new Fl_Radio_Round_Button(170, 39, 150, 12, "Group");
