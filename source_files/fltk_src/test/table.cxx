...
class DemoTable : public Fl_Table_Row {
   ...
   protected:
    void draw_cell(TableContext context,  // table cell drawing
                   int R = 0, int C = 0, int X = 0, int Y = 0, int W = 0,
                   int H = 0);
    static void event_callback(Fl_Widget *, void *);
    void event_callback2();  // callback for table events

   public:
    DemoTable(int x, int y, int w, int h, const char *l = 0)
...
void DemoTable::draw_cell(TableContext context, int R, int C, int X, int Y,
                          int W, int H) {
    static char s[40];
...
void DemoTable::event_callback2() {
    ...
    printf("Row=%d Col=%d Context=%d Event=%d InteractiveResize? %d\n", R, C,
           (int)context, (int)Fl::event(), (int)is_interactive_resize());
}
...
void setrowheadercolor_cb(Fl_Widget *w, void *data) {
    ...
    int val = atoi(in->value());
    if (val < 0) {
        fl_alert("Must be a color >0");
    } else {
        G_table->row_header_color(Fl_Color(val));
    }
}
...
void setcolheadercolor_cb(Fl_Widget *w, void *data) {
    ...
    if (val < 0) {
        ...
        G_table->col_header_color(Fl_Color(val));
    }
}
...
void setcellfgcolor_cb(Fl_Widget *w, void *data) {
    ...
    if (val < 0) {
        ...
        G_table->SetCellFGColor(Fl_Color(val));
    }
    G_table->redraw();
}
...
void setcellbgcolor_cb(Fl_Widget *w, void *data) {
    ...
    if (val < 0) {
        ...
        G_table->SetCellBGColor(Fl_Color(val));
    }
    G_table->redraw();
}
