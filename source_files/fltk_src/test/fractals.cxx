...
void InitRandTable(unsigned int seed) {
    ...
    srand48((long)seed);
    for (i = 0; i < NUMRANDS; i++) RandTable[i] = drand48() - 0.5;
}

/* calculate midpoint and displace it if required */
void Midpoint(GLfloat mid[3], GLfloat v1[3], GLfloat v2[3], int edge,
              int level) {
    unsigned hash;
    ...
    mid[2] = (v1[2] + v2[2]) / 2;
    if (!Pegged[edge] ||
        (fabs(xzslope(Verts[edge], mid) - Slopes[edge]) > 0.00001)) {
...
void FractalMountain(GLfloat v1[3], GLfloat v2[3], GLfloat v3[3],
    ...
    for (i = 1; i < Level; i++) {
        lengths[i] =
            lengths[i - 1] / 2; /* compute edge length for each level */
    }
 ...
 */
void NewMountain(void) { InitRandTable(time(NULL)); }
 ...
 */
void NewTree(void) { TreeSeed = time(NULL); }
