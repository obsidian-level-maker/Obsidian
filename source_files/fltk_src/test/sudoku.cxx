...
#define MAC_OS_X_VERSION_10_5 1050
    ...
    static OSStatus audio_cb(AudioDeviceID device,
                             ...
                             const AudioTimeStamp *time_out, void *client_data);
#elif defined(WIN32)
    HWAVEOUT device;
...
SudokuSound::SudokuSound() {
    ...
    if (AudioDeviceSetProperty(device, NULL, 0, false,
                               kAudioDevicePropertyStreamFormat, sizeof(format),
                               &format) != noErr)
        return;
    ...
    data_handle =
        GlobalAlloc(GMEM_MOVEABLE | GMEM_SHARE, format.nSamplesPerSec / 5);
    ...
    if (sample_size) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            for (int j = 0; j < sample_size; j++, sample_ptr += 2) {
                ...
                double val =
                    0.5 * sin(2.0 * M_PI * theta) + theta - (int)theta - 0.5;

                if (j < attack) {
                    *sample_ptr = (int)(32767 * val * j / attack);
                } else if (j > decay) {
                    *sample_ptr =
                        (int)(32767 * val * (sample_size - j + decay) /
                              sample_size);
                } else
...
OSStatus SudokuSound::audio_cb(AudioDeviceID device,
                               const AudioTimeStamp *current_time,
                               const AudioBufferList *data_in,
                               const AudioTimeStamp *time_in,
                               AudioBufferList *data_out,
...
void SudokuSound::play(char note) {
    ...
    XChangeKeyboardControl(
        fl_display, KBBellPercent | KBBellPitch | KBBellDuration, &control);
...
SudokuCell::SudokuCell(int X, int Y, int W, int H) : Fl_Widget(X, Y, W, H, 0) {
    value(0);
}

// Draw cell
void SudokuCell::draw() {
    static Fl_Align align[8] = {FL_ALIGN_TOP_LEFT,     FL_ALIGN_TOP,
                                FL_ALIGN_TOP_RIGHT,    FL_ALIGN_RIGHT,
                                FL_ALIGN_BOTTOM_RIGHT, FL_ALIGN_BOTTOM,
...
Sudoku::Sudoku()
    ...
    static Fl_Menu_Item items[] = {
        ...
        {"&Mute Sound", FL_COMMAND | 'm', mute_cb, 0,
...
#elif !defined(__APPLE__)
    fl_open_display();
    icon((char *)XCreateBitmapFromData(
        fl_display, DefaultRootWindow(fl_display), (char *)sudoku_bits,
        sudoku_width, sudoku_height));
#endif  // WIN32
            ...
            for (int jj = 0; jj < 3; jj++)
                for (int kk = 0; kk < 3; kk++) {
                    ...
                    for (jjj = 0; jjj < 3; jjj++) {
                        ...
                        for (kkk = 0; kkk < 3; kkk++)
                            if (jj != jjj && kk != kkk &&
                                grid_cells_[j + jjj][k + kkk]->value() == val)
                                break;
    ...
    if (diff != s->difficulty_) {
        ...
        if (diff > 1) {
            ...
            if (!val) {
                ...
                fl_alert(
                    "Note: 'Hard' and 'Impossible' puzzles may have more than "
...
void Sudoku::update_helpers() {
    ...
    for (j = 0; j < 81; j++) {
        ...
        for (m = 1, k = 0; m <= 9; m++) {
            if (!taken[m]) dst_cell->test_value(m, k++);
        }
    }
}
...
void Sudoku::help_cb(Fl_Widget *, void *) {
    if (!help_dialog_) {
        ...
        help_dialog_->value(
            ...
            "<P>This version of the puzzle is copyright 2005-2010 by Michael "
            "R\n"
            "Sweet.</P>\n"
            ...
            "interesting than the simple single solution variety. If you "
            "don't\n"
            ...
            "<P>At the start of a new game, Sudoku fills in a random "
            "selection\n"
            ...
            "keys to highlight individual cells and press a number from 1 to "
            "9\n"
...
void Sudoku::new_cb(Fl_Widget *widget, void *) {
    ...
    if (s->grid_cells_[0][0]->color() != FL_GREEN) {
        if (!fl_choice(
                "Are you sure you want to change the difficulty level and "
...
void Sudoku::set_title() {
    static const char *const titles[] = {"Sudoku - Easy", "Sudoku - Medium",
                                         "Sudoku - Hard",
                                         "Sudoku - Impossible"};
