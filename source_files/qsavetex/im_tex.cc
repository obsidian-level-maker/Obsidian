...
static void ExtractMipTex(read_func_F read_func) {
    ...
    if (!read_func(0, sizeof(bsp), &bsp)) Mip_Error("dheader_t");
    ...
    for (int i = 0; i < num_miptex; i++) {
        ...
        // -1 means unused slot
        if (data_ofs & 0x8000000U) continue;
        ...
        if (mip.offsets[0] == 0) continue;

        // already seen it?
        if (tex_db.find((const char *)mip.name) != tex_db.end()) continue;
...
void TEX_ExtractStart() { tex_db.clear(); }
