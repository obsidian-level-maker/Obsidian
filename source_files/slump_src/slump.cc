/* Copyright 2000 David Chess; Copyright 2005-2006 Sam Trenholme
 *
 ...
 * - Please do not call any derivative of this program SLIGE.
   ...
   SLUMP - a modern port of SLIGE: May 2005
   ...
   - If you make various changes and improvements to it and release
     a modified version yourself, see the above GPL license with two
     optional (but strongly desired by the development team) requests.
...
flat *new_flat(config *c, const char *name) {
    ...
    answer->gamemask =
        DOOM0_BIT | DOOM1_BIT | DOOM2_BIT | DOOMC_BIT | DOOMI_BIT;
    answer->compatible = 0;
    answer->props = 0;
...
style *copy_style(level *l, style *old, int themenumber, int vary, config *c) {
    ...
    if (!rollpercent(vary))
        ...
        if (rollpercent(50)) /* Something wider? */
    ...
    if (!rollpercent(vary))
        ...
        answer->auxspecial = rollpercent(80) ? 0 : RANDOM_BLINK;
    if (!rollpercent(vary))
...
style *new_style(level *l, style *old, boolean radical, config *c) {
    ...
    if ((!c->lock_themes) &&
        rollpercent(newtheme)) { /* Sometimes entirely new */
    ...
    if (conly) { /* config, seed, -v only; imperfect algorithm! Can be fooled.
        ...
        for (i = 1; i < argc; i++) {
            if (!slump_stricmp(argv[i], "-config")) {
                if (i < (argc -
                         1)) { /* If -config is the last arg, just ignore it */
        ...
        for (i = 1; i < argc; i++) {
            if (argv[i][0] != '-') {
                ...
                if (i <
                    ...
                    if (c->minlight == 0) {
                        fprintf(stderr,
                                "%s error: invalid -minlight arg <%s>.\n", s,
                                argv[i]);
                        return SLUMP_FALSE;
                    }
                } /* end if enough args */
                ...
                if (i <
                    ...
                    if ((mfac < 1) || (mfac > 100)) {
                        fprintf(
                            stderr,
                            "%s error: -macho must be in [1,100], not <%s>.\n",
                            s, argv[i]);
                        return SLUMP_FALSE;
                    }
                    c->machoh =
                        ((float)100 - (float)mfac / (float)4) / (float)100;
                    c->machou =
                        ((float)100 - (float)mfac / (float)2) / (float)100;
                } /* end if enough args */
                ...
                if (i <
                    ...
                    for (j = 0; j < (int)strlen(argv[i]); j++) {
                        if (argv[i][j] == 'C')
                            ...
                            c->gamemask |= DOOM0_BIT | DOOM1_BIT;
                        else if (argv[i][j] == '1')
                            c->gamemask |= DOOM1_BIT;
                        else if (argv[i][j] == '2')
                            c->gamemask |= DOOM2_BIT;
                            ...
                            fprintf(stderr,
                                    "%s error: invalid -restrict arg <%s>.\n",
                                    s, argv[i]);
                            return SLUMP_FALSE;
                        }
                ...
                if (2 != sscanf(argv[i], "-%*c%d%*c%d", &(c->episode),
                                &(c->mission))) {
                ...
                if (i <
                    ...
                    if (c->minrooms > 37) c->minrooms = 37;
                } /* end if enough args */
...
void stairify(level *l, linedef *ldf1, linedef *ldf2, linedef *lde1,
    ...
    for (i = 0; i < (stepcount - 1);
         i++) { /* Minus one, since one's already made */
...
linedef *lightbox(level *l, linedef *ld, genus *g, style *ThisStyle,
                  config *c) {
    int len;
    linedef *ldb;
    int x, y;
...
/* two given linedefs. */
void try_falling_core(level *l, linedef *ld1, linedef *ld2, haa *haa,
                      config *c) {
    int len, depth;
    boolean room1, room2;
...
void e_bl_inner(level *l, linedef *ldf1, linedef *ldf2, link *ThisLink,
    ...
    if (ThisLink->bits & LINK_STEPS) {
        ...
        if (ThisLink->bits & LINK_ANY_DOOR)
    ...
    if (((ThisLink->bits & LINK_CORE)) && (ThisLink->bits & LINK_ANY_DOOR)) {
        ...
        if (ThisLink->bits & LINK_FAR_DOOR) {
            ...
            /* Might as well fix the light-level on the far door while we're
             * here */
    ...
    if (empty_left_side(l, ld, rdepth)) {
        ...
        if (key) {
            ldedge2->right->middle_texture = ldedge1->right->middle_texture =
                tx;
        } else if ((ThisStyle->light_recesses) &&
                   (ThisStyle->walllight != NULL)) {
...
void arena_arrival(level *l, arena *a, haa *haa, config *c) {
    ...
    if (a->props & ARENA_NUKAGE) { /* Little stub health */
        place_object_in_region(l, minx, a->miny, maxx, a->maxy, c, ID_MEDIKIT,
                               16, 0, 0, 0, 7);
    }
}
...
boolean rising_room(level *l, sector *s, config *c, haa *haa,
                    quest *ThisQuest) {
    int minx, miny, maxx, maxy;
...
void close_quest(level *l, sector *s, quest *q, haa *haa, config *c) {
    ...
    switch (q->goal) {
        ...
        case LEVEL_END_GOAL:
            ...
            i = mark_decent_boundary_linedefs(l, s, 64);
            for (j = 0; j < 5;
                 j++) { /* Try a little harder to get recessible! */
                ld = random_marked_linedef(l, i);
...
thing *place_object_in_region(level *l, int minx, int miny, int maxx, int maxy,
    ...
    for (i = 0; i < 10; i++) {
        ...
        x = deck[n].x;
        y = deck[n].y;
        if (room_at(l, g, x, y, width,
                    c)) { /* Use first point with room we find */
...
void update_haa_for_monster(haa *haa, genus *m, int levels, int mno,
                            config *c) {
    int i, thisbit;
    float damage;
...
genus *timely_monster_ex(haa *haa, config *c, int *levels, boolean biggest,
    ...
    return proper_monster(monster_size_health, monster_size_ammo, *levels, haa,
                          mno, c->required_monster_bits + req,
...
void load_config(config *c) {
    ...
    if (f == NULL) {
        ...
        *(outc++) = '\0';
        printf("Loaded.\n");
        c->configdata =
            (char *)realloc(c->configdata, 1 + (outc - c->configdata));
        fclose(f);
    }
#if 0
...
construct *new_construct(config *c) {
    ...
    answer->height = 64;
    answer->gamemask =
        DOOM1_BIT | DOOM0_BIT | DOOM2_BIT | DOOMI_BIT | DOOMC_BIT;
    answer->compatible = 0;
...
void do_new_pillars(level *l, sector *oldsector, style *ThisStyle, haa *haa,
    ...
    if ((ThisStyle->do_constructs == SLUMP_FALSE) &&
        (rollpercent(100))) { /* 100? */
                        ...
                        if (rollpercent(15) &&
                            ...
                            if (rollpercent(50)) { /* Health or whatever */
                                ...
                                if ((!(haa->haas[2].has_chainsaw)) &&
                                    ...
                                    switch (roll(2)) { /* What about a cell? / a
                                                          rocket */
                                        case 0:
                                            bonustype = ID_CLIP;
                                            break;
            ...
            for (j = 1;; j++) {
                ...
                if (ld != NULL) {
                    ...
                    if (empty_left_side(l, ld, depth)) {
                        ...
                        if (pup < 25)
                            if (rollpercent(
                                    80)) { /* Put a secret thingie behind it! */
                                if (sync_doors)
            ...
            if (empty_left_side(l, ld, 72)) { /* "72" is from secret_closet() */
                ...
                if (NULL != ldc) {
                    if (switch_closet) {
                        ...
                        if (DOOM0_BIT & c->gamemask)
                            switch_ld->type = LINEDEF_S1_OPEN_DOOR;
                        else
                            switch_ld->type = LINEDEF_S1_BLAZE_O_DOOR;
                    ...
                    if (tag == -1) { /* Need a subtle hint here */
                        ...
                        if (!hinted) {
                            /* Make it just show on the automap (always
                             * possible) */
                            ld->flags &= ~SECRET_LINEDEF;
