...
typedef struct {
    ...
    Byte
        buffered_data[Z_BUFSIZE]; /* buffer contain compressed data to be writ*/
    uLong dosDate;
    uLong crc32;
    int encrypt;
...
int Write_LocalFileHeader(zip64_internal *zi, const char *filename,
    ...
    if (err == ZIP_OK) {
        if (zi->ci.zip64)
            err =
                zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)45,
                                    2); /* version needed to extract */
        else
            err =
                zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)20,
                                    2); /* version needed to extract */
    }
...
extern int ZEXPORT zipOpenNewFileInZip4_64(
    ...
    zip64local_putValue_inmemory(zi->ci.central_header,
                                 (uLong)CENTRALHEADERMAGIC, 4);
    /* version info */
    zip64local_putValue_inmemory(zi->ci.central_header + 4,
    ...
    zip64local_putValue_inmemory(zi->ci.central_header + 12,
                                 (uLong)zi->ci.dosDate, 4);
    zip64local_putValue_inmemory(zi->ci.central_header + 16, (uLong)0,
                                 4); /*crc*/
...
extern int ZEXPORT zipOpenNewFileInZip4(
    ...
    return zipOpenNewFileInZip4_64(
        ...
        windowBits, memLevel, strategy, password, crcForCrypting, versionMadeBy,
        flagBase, 0);
}
...
extern int ZEXPORT zipOpenNewFileInZip2(
    ...
    return zipOpenNewFileInZip4_64(
        file, filename, zipfi, extrafield_local, size_extrafield_local,
        extrafield_global, size_extrafield_global, comment, method, level, raw,
        -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY, NULL, 0, VERSIONMADEBY,
        0, 0);
}
    ...
    if (zi->ci.method == Z_BZIP2ED && (!zi->ci.raw)) {
        ...
        while ((err == BZ_RUN_OK) && (zi->ci.bstream.avail_in > 0)) {
            ...
            if ((zi->ci.method == Z_BZIP2ED) && (!zi->ci.raw)) {
                ...
                //          uLong uTotalOutBefore_hi =
                //          zi->ci.bstream.total_out_hi32;
...
extern int ZEXPORT zipCloseFileInZipRaw64(zipFile file,
    ...
    if (zi->ci.stream.data_type == Z_ASCII)
        zip64local_putValue_inmemory(zi->ci.central_header + 36, (uLong)Z_ASCII,
                                     2);
...
int Write_Zip64EndOfCentralDirectoryRecord(zip64_internal *zi,
    ...
    if (err == ZIP_OK) /* version made by */
        err =
            zip64local_putValue(&zi->z_filefunc, zi->filestream, (uLong)45, 2);
...
extern int ZEXPORT zipRemoveExtraInfoBlock(char *pData, int *dataLen,
    ...
    while (p < (pData + *dataLen)) {
        ...
        if (header == sHeader)  // Header found.
        {
            ...
            // Extra Info block should not be removed, So copy it to the temp
            // buffer.
