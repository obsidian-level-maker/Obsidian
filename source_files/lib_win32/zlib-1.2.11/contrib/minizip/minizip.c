...
void do_banner() {
    printf(
        "MiniZip 1.1, demo of zLib + MiniZip64 package, written by Gilles "
        "Vollant\n");
    printf(
        "more info on MiniZip at "
        "http://www.winimage.com/zLibDll/minizip.html\n\n");
}

void do_help() {
    printf(
        "Usage : minizip [-o] [-a] [-0 to -9] [-p password] [-j] file.zip "
        "[files_to_add]\n\n"
...
char *argv[];
{
    ...
    if (zipok == 1) {
        ...
        zf = zipOpen2_64(filename_try, (opt_overwrite == 2) ? 2 : 0, NULL,
                         &ffunc);
#else
            ...
            if (!((((*(argv[i])) == '-') || ((*(argv[i])) == '/')) &&
                ...
                */
                ...
                err = zipOpenNewFileInZip3_64(
                    zf, savefilenameinzip, &zi, NULL, 0, NULL, 0,
                    NULL /* comment*/,
                    (opt_compress_level != 0) ? Z_DEFLATED : 0,
