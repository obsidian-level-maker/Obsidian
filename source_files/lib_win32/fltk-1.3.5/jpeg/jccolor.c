...
rgb_ycc_start(j_compress_ptr cinfo) {
    ...
    for (i = 0; i <= MAXJSAMPLE; i++) {
        ...
        /*  B=>Cb and R=>Cr tables are the same
            rgb_ycc_tab[i+R_CR_OFF] = FIX(0.5) * i    + CBCR_OFFSET +
           ONE_HALF-1;
        */
...
METHODDEF(void)
rgb_rgb1_convert(j_compress_ptr cinfo, JSAMPARRAY input_buf,
                 JSAMPIMAGE output_buf, JDIMENSION output_row, int num_rows) {
    register int r, g, b;
    register JSAMPROW inptr;
    register JSAMPROW outptr0, outptr1, outptr2;
...
jinit_color_converter(j_compress_ptr cinfo) {
    ...
    switch (cinfo->jpeg_color_space) {
        ...
        case JCS_BG_YCC:
            ...
            switch (cinfo->in_color_space) {
                case JCS_RGB:
                    /* For conversion from normal RGB input to BG_YCC
                     * representation, the Cb/Cr values are first computed as
                     * usual, and then quantized further after DCT processing by
                     * a factor of 2 in reference to the nominal quantization
                     * factor.
                     */
