...
class Fl_Type {
   ...
   public:
    ...
    virtual const char *alt_type_name() {
        return type_name();
    }  // alternate type for FLTK2 code output
    ...
    void write_comment_h(const char *ind = "");  // write the commentary text
                                                 // into the header file
    void write_comment_c(const char *ind = "");  // write the commentary text
                                                 // into the source file
    void write_comment_inline_c(
        const char *ind = 0L);  // write the commentary text

    // live mode
    virtual Fl_Widget *enter_live_mode(
        int top = 0);                // build wdgets needed for live mode
    virtual void leave_live_mode();  // free allocated resources
    virtual void copy_properties();  // copy properties from this type into a
                                     // potetial live object
...
class Fl_Code_Type : public Fl_Type {
   ...
   public:
    ...
    int is_editing() { return editor_.is_editing(); }
    ...
    //
    int reap_editor() { return editor_.reap_editor(); }
    // Handle external editor file modifications
    // If changed, record keeping is updated and file's contents is loaded into
    // ram
    //
    // Returns:
    ...
    int handle_editor_changes() {
        ...
        switch (editor_.handle_changes(&newcode)) {
            ...
            case -1:
                return -1;  // (-1)=error -- couldn't read file (dialog showed
                            // reason)
            default:
                break;  // (0)=no change
        }
...
class Fl_Menu_Type : public Fl_Widget_Type {
    int textstuff(int w, Fl_Font &f, int &s, Fl_Color &c) {
        Fl_Menu_ *myo =
            (Fl_Menu_ *)(w == 4 ? ((Fl_Widget_Type *)this->factory)->o
                                : this->o);
        switch (w) {
            case 4:
            case 0:
...
class Fl_Input_Choice_Type : public Fl_Menu_Type {
    int textstuff(int w, Fl_Font &f, int &s, Fl_Color &c) {
        Fl_Input_Choice *myo =
            (Fl_Input_Choice *)(w == 4 ? ((Fl_Widget_Type *)this->factory)->o
                                       : this->o);
        switch (w) {
            case 4:
            case 0:
...
class Fl_Commandline_Plugin : public Fl_Plugin {
   public:
    Fl_Commandline_Plugin(const char *name) : Fl_Plugin(klass(), name) {}
    ...
    virtual int test(const char *a1 = 0L, const char *a2 = 0L,
                     const char *a3 = 0L) {
        return 0;
    }
