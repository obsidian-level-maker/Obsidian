...
static int readentry(DIR *dirp, struct dirent **entryp, size_t *len) {
    ...
    if (!pthread_mutex_lock(&scandir_mutex)) {
        ...
        if (NULL == e) {
            ...
            *len =
                offsetof(struct dirent, d_name) + strlen(e->d_name) + (size_t)1;
...
#ifdef HAVE_PTHREAD
        /*
         * In a multithreading environment the systems dirent buffer may be
...
int fl_scandir(const char *dir, struct dirent ***namelist,
    ...
    if (NULL != entries) {
        ...
        if (NULL != dirp) {
            ...
            closedir(dirp);
            /*
             * A standard compliant 'closedir()' is allowed to fail with
             * 'EINTR', but the state of the directory structure is undefined in
             * this case. Therefore we ignore the return value because we can't
             * call 'closedir()' again and must hope that the system has
             * released all ressources.
             */
        }
