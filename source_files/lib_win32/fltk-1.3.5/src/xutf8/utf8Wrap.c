...
static void load_fonts(Display *dpy, XUtf8FontStruct *font_set) {
    ...
    font_set->fonts =
        (XFontStruct **)malloc(sizeof(XFontStruct *) * font_set->nb_font);
    ...
    font_set->encodings = get_encodings(font_set->font_name_list,
        ...
        if (font_set->font_name_list[i]) {
            ...
            for (j = 0; j < i; j++) {
                if (font_set->font_name_list[j] &&
                    font_set->encodings[j] == font_set->encodings[i] &&
                    font_set->ranges[2 * j] == font_set->ranges[2 * i] &&
...
XUtf8FontStruct *XCreateUtf8FontStruct(Display *dpy,
    ...
    font_set->nb_font =
        get_font_list(base_font_name_list, &font_set->font_name_list);
...
/*****************************************************************************/
void XUtf8DrawRtlString(Display *display, Drawable d, XUtf8FontStruct *font_set,
    ...
    while (num_bytes > 0) {
        ...
        ulen = XFastConvertUtf8ToUcs((const unsigned char *)string, num_bytes,
                                     &ucs);
...
/*****************************************************************************/
void XUtf8_measure_extents(Display *display, Drawable d,
                           XUtf8FontStruct *font_set, GC gc,
    ...
    while (num_bytes > 0) {
        ...
        if (last_fnum != fnum || no_spc) {
            XSetFont(display, gc, fonts[last_fnum]->fid);
            /* res = */ XTextExtents16(fonts[last_fnum], buf, i, &dir_ret,
                                       &fnt_asc, &fnt_dsc, &sizes);
            /* recover the dimensions - should verify that res == 0 first! */
            wd += sizes.width;                 /* accumulate the width */
            hs = sizes.ascent + sizes.descent; /* total height */
            if (hs > ht) ht = hs; /* new height exceeds previous height */
...
/*****************************************************************************/
int fl_XGetUtf8FontAndGlyph(XUtf8FontStruct *font_set, unsigned int ucs,
                            XFontStruct **fnt, unsigned short *id) {
    int *encodings;      /* encodings array */
    XFontStruct **fonts; /* fonts array */
    int fnum;            /* index of the current font in the fonts array*/
    int first;           /* first valid font index */
