...
void PNGAPI png_write_info_before_PLTE(png_structrp png_ptr,
    ...
    if ((png_ptr->mode & PNG_WROTE_INFO_BEFORE_PLTE) == 0) {
         ...
         * and error and calls png_error while the color space is being set, yet
...
static void png_reset_filter_heuristics(
    png_structrp png_ptr); /* forward decl */
#endif
        ...
        if (png_ptr->row_buf != NULL) {
            ...
            if ((png_ptr->do_filter & PNG_FILTER_UP) != 0 &&
                ...
                if (png_ptr->prev_row == NULL) {
                    ...
                    png_ptr->do_filter =
                        (png_byte)(png_ptr->do_filter & ~PNG_FILTER_UP);
                }
            ...
            if ((png_ptr->do_filter & PNG_FILTER_AVG) != 0 &&
                ...
                if (png_ptr->prev_row == NULL) {
                    ...
                    png_ptr->do_filter =
                        (png_byte)(png_ptr->do_filter & ~PNG_FILTER_AVG);
                }
    ...
    if (heuristic_method == PNG_FILTER_HEURISTIC_WEIGHTED) {
        ...
        if (num_weights > 0) {
            png_ptr->prev_filters = (png_bytep)png_malloc(
                png_ptr, (png_uint_32)((sizeof(png_byte)) * num_weights));
            ...
            png_ptr->filter_weights = (png_uint_16p)png_malloc(
                png_ptr, (png_uint_32)((sizeof(png_uint_16)) * num_weights));
...
void PNGAPI png_set_filter_heuristics(png_structrp png_ptr,
    ...
    /* The internal API allocates all the arrays and ensures that the elements
     * of those arrays are set to the default value.
     */
    ...
    if (heuristic_method == PNG_FILTER_HEURISTIC_WEIGHTED) {
         ...
         * low compression levels, while it has a lower relative cost at higher
    ...
    if (heuristic_method == PNG_FILTER_HEURISTIC_WEIGHTED) {
        ...
        for (i = 0; i < num_weights; i++) {
            if (filter_weights[i] <= 0) {
                ...
                png_ptr->inv_filter_weights[i] = (png_uint_16)(
                    (PNG_WEIGHT_FACTOR * filter_weights[i] + PNG_FP_HALF) /
                    PNG_FP_1);
        ...
        for (i = 0; i < PNG_FILTER_VALUE_LAST; i++)
            if (filter_costs[i] >= PNG_FP_1) {
                 ...
                 * intermediate value will be a 32 bit *signed* integer (ANSI
                 * rules) and this will get the wrong answer on division.
                 */
...
void PNGAPI png_set_compression_window_bits(png_structrp png_ptr,
     ...
     * meant that negative window bits values could be selected which would
...
static png_byte png_unpremultiply(
    png_uint_32 component, png_uint_32 alpha,
...
static int png_image_write_main(png_voidp argument) {
...

    {
        ...
        if (linear != 0) row_bytes *= (sizeof(png_uint_16));
