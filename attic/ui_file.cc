...
UI_File::UI_File(int x, int y, int w, int h, const char *label)
    : Fl_Group(x, y, w, h, label), full_path(NULL) {
    ...
    ext = new Fl_Box(FL_FLAT_BOX, filename->x() + filename->w() + 16, cy, 80,
                     24, "Type: WAD");
    ...
    browse = new Fl_Button(dir_name->x() + dir_name->w() + 16, cy, 70, 24,
                           "Browse...");
...
void UI_File::browse_callback(Fl_Widget *w, void *data) {
    ...
    char *new_dir =
        fl_dir_chooser("Oblige: Select output folder", that->full_path);
