...
static void BlockAdd(int blk_num, int line_index) {
    ...
    // TODO: V781 https://www.viva64.com/en/w/v781/ The value of the 'blk_num'
    // variable is checked after it was used. Perhaps there is a mistake in
    // program logic. Check lines: 157, 163.
    ...
    if (!cur) {
        ...
        block_lines[blk_num] = cur =
            (uint16_g *)UtilCalloc(BK_QUANTUM * sizeof(uint16_g));
        cur[BK_NUM] = 0;
    ...
    if (BK_FIRST + cur[BK_NUM] == cur[BK_MAX]) {
        ...
        block_lines[blk_num] = cur =
            (uint16_g *)UtilRealloc(cur, cur[BK_MAX] * sizeof(uint16_g));
    }
...
static void CompressBlockmap(void) {
    ...
    for (i = 0; i < block_count; i++) block_dups[i] = i;
...
static void WriteBlockmap(void) {
    ...
    for (i = 0; i < block_count; i++) {
        ...
        if (ptr == 0) InternalError("WriteBlockmap: offset %d not set.", i);
    ...
    for (i = 0; i < block_count; i++) {
        ...
        if (blk == NULL) InternalError("WriteBlockmap: block %d is NULL !", i);

        AppendLevelLump(lump, &m_zero, sizeof(uint16_g));
        AppendLevelLump(
            lump, blk + BK_FIRST,
            blk[BK_NUM] *
                sizeof(uint16_g));  // TODO: V1004
                                    // https://www.viva64.com/en/w/v1004/ The
                                    // 'blk' pointer was used unsafely after it
                                    // was verified against nullptr. Check lines:
                                    // 457, 461.
