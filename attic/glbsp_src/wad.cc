...
static int CheckGLLumpName(const char *name) {
    ...
    for (i = 0; i < NUM_GL_LUMPS; i++) {
        if (strcmp(name, gl_lumps[i]) == 0) return TRUE;
    }
...
static void ProcessDirEntry(lump_t *lump) {
    ...
    if (cur_info->gwa_mode) lump->flags |= LUMP_IGNORE_ME;
...
static PHYSFS_EnumerateCallbackResult WadNamespace(void *userData,
                                                   const char *origDir,
                                                   const char *fname) {
    lump_t *lump;
    raw_wad_header_t header;
    ...
    if (!file) return PHYSFS_ENUM_OK;  // couldn't open file - skip
...
static PHYSFS_EnumerateCallbackResult TopLevel(void *userData,
                                               const char *origDir,
                                               const char *fname) {
    char check[16];
    char path[256];
...
glbsp_ret_e WriteWadFile(const char *filename) {
    ...
    if (cur_info->gwa_mode) wad.kind = PWAD;
...
void ZLibFinishLump(void) {
    ...
    for (;;) {
        ...
        if (err != Z_OK) FatalError("Trouble finishing compression (zlib)\n");
    ...
    if (left_over > 0) AppendLevelLump(zout_lump, zout_buffer, left_over);
...
void MarkV5Switch(int v5) { wad.current_level->lev_info->v5_switch |= v5; }
...
void ReportOneOverflow(const lump_t *lump, int limit, boolean_g hard) {
    ...
    switch (limit) {
        case LIMIT_VERTEXES:
            ...
            break;
        case LIMIT_SECTORS:
            PrintMsg("Number of Sectors %s.\n", msg);
            break;
        case LIMIT_SIDEDEFS:
            PrintMsg("Number of Sidedefs %s\n", msg);
            break;
        case LIMIT_LINEDEFS:
            PrintMsg("Number of Linedefs %s\n", msg);
            break;

        case LIMIT_SEGS:
            PrintMsg("Number of Segs %s.\n", msg);
            break;
        case LIMIT_SSECTORS:
            PrintMsg("Number of Subsectors %s.\n", msg);
            break;
        case LIMIT_NODES:
            PrintMsg("Number of Nodes %s.\n", msg);
            break;

        case LIMIT_GL_VERT:
            PrintMsg("Number of GL vertices %s.\n", msg);
            break;
        case LIMIT_GL_SEGS:
            PrintMsg("Number of GL segs %s.\n", msg);
            break;
        case LIMIT_GL_SSECT:
            PrintMsg("Number of GL subsectors %s.\n", msg);
            break;
        case LIMIT_GL_NODES:
            PrintMsg("Number of GL nodes %s.\n", msg);
            break;

        case LIMIT_BAD_SIDE:
            PrintMsg("One or more linedefs has a bad sidedef.\n");
            break;
        case LIMIT_BMAP_TRUNC:
            PrintMsg("Blockmap area was too big - truncated.\n");
            break;
        case LIMIT_BLOCKMAP:
            PrintMsg("Blockmap lump %s.\n", msg);
            break;
...
void ReportOverflows(boolean_g hard) {
    ...
    } else  // soft
    {
        PrintMsg(
            ...
            "modern Doom port, but may fail (or even crash) in older "
            "ports.\n\n");
    }
    ...
    for (cur = wad.dir_head; cur; cur = cur->next) {
        ...
        if (!(lev && !(lev->flags & LEVEL_IS_GL))) continue;

        limits = hard ? lev->hard_limit : lev->soft_limit;

        if (limits == 0) continue;

        for (one_lim = (1 << 20); one_lim; one_lim >>= 1) {
            if (limits & one_lim) ReportOneOverflow(cur, one_lim, hard);
        }
    }
}
...
void ReportFailedLevels(void) {
    ...
    for (cur = wad.dir_head; cur; cur = cur->next) {
        if (!(cur->lev_info && !(cur->lev_info->flags & LEVEL_IS_GL))) continue;
