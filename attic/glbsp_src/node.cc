...
void FreeSuper(superblock_t *block) {
    ...
    InternalError("FreeSuper: block contains segs");
        ...
        for (num = 0; num < 2; num++) {
            if (block->subs[num]) FreeSuper(block->subs[num]);
        }
...
superblock_t *CreateSegs(void) {
    ...
    for (i = 0; i < num_linedefs; i++) {
        ...
        if (line->self_ref && cur_info->skip_self_ref) continue;
...
static void ClockwiseOrder(subsec_t *sub) {
    ...
    for (cur = sub->seg_list; cur; cur = cur->next) total++;
    ...
    for (cur = sub->seg_list, i = 0; cur; cur = cur->next, i++) array[i] = cur;

    if (i != total) InternalError("ClockwiseOrder miscounted.");
    ...
    while (i + 1 < total) {
        ...
        angle1 = UtilComputeAngle(A->start->x - sub->mid_x,
                                  A->start->y - sub->mid_y);
        angle2 = UtilComputeAngle(B->start->x - sub->mid_x,
                                  B->start->y - sub->mid_y);
    ...
    for (cur = sub->seg_list; cur; cur = cur->next) {
        ...
        PrintDebug("  Seg %p: Angle %1.6f  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
                   cur, angle, cur->start->x, cur->start->y, cur->end->x,
                   cur->end->y);
    }
#endif
}
...
static void SanityCheckSameSector(subsec_t *sub) {
    ...
    for (compare = sub->seg_list; compare; compare = compare->next) {
        if (!compare->sector) continue;

        if (compare->sector->coalesce) continue;
    ...
    for (cur = compare->next; cur; cur = cur->next) {
        ...
        if (cur->sector->coalesce) continue;

        // prevent excessive number of warnings
        if (compare->sector->warned_facing == cur->sector->index) continue;

        compare->sector->warned_facing = cur->sector->index;

        if (cur->linedef)
            PrintMiniWarn(
                "Sector #%d has sidedef facing #%d (line #%d) "
                "near (%1.0f,%1.0f).\n",
                compare->sector->index, cur->sector->index, cur->linedef->index,
                sub->mid_x, sub->mid_y);
        else
            PrintMiniWarn(
                "Sector #%d has sidedef facing #%d "
                "near (%1.0f,%1.0f).\n",
                compare->sector->index, cur->sector->index, sub->mid_x,
                sub->mid_y);
    }
}
...
static void DebugShowSegs(superblock_t *seg_list) {
    ...
    for (cur = seg_list->segs; cur; cur = cur->next) {
        PrintDebug(
            "Build:   %sSEG %p  sector=%d  (%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
            cur->linedef ? "" : "MINI", cur, cur->sector->index, cur->start->x,
            cur->start->y, cur->end->x, cur->end->y);
    }

    for (num = 0; num < 2; num++) {
        if (seg_list->subs[num]) DebugShowSegs(seg_list->subs[num]);
    }
}
#endif
...
glbsp_ret_e GlbspBuildNodes(superblock_t *seg_list, node_t **N, subsec_t **S,
    ...
    *S = NULL;
    ...
    if (cur_comms->cancelled) return GLBSP_E_Cancelled;
    ...
    *N = node = NewNode();
    ...
    if (best->p_length >= 30000) {
        if (node->dx && node->dy && ((node->dx & 1) || (node->dy & 1))) {
            PrintMiniWarn(
                "Loss of accuracy on VERY long node: "
                "(%d,%d) -> (%d,%d)\n",
...
static void RoundOffSubsector(subsec_t *sub) {
    ...
    for (cur = sub->seg_list; cur; cur = cur->next) {
        ...
        if (cur->start->normal_dup) cur->start = cur->start->normal_dup;

        if (cur->end->normal_dup) cur->end = cur->end->normal_dup;
        ...
        if (I_ROUND(cur->start->x) == I_ROUND(cur->end->x) &&
            ...
            if (cur->linedef) last_real_degen = cur;

            degen_total++;
            continue;
        }

        if (cur->linedef) real_total++;
    }
    ...
    if (real_total == 0) {
        ...
        last_real_degen->end = NewVertexDegenerate(
            last_real_degen
                ->start,  // TODO: V1004 https://www.viva64.com/en/w/v1004/ The
                          // 'last_real_degen' pointer was used unsafely after it
                          // was verified against nullptr. Check lines: 1247,
                          // 1258.
            last_real_degen->end);
