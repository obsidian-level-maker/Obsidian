...
void TestForWindowEffect(linedef_t *L) {
    ...
    for (i = 0; i < num_linedefs; i++) {
        ...
        if (N == L || N->zero_len || N->overlap) continue;
    ...
    PrintDebug("back line: %d  back dist: %1.1f  back_open: %s\n", back_line,
               back_dist, back_open ? "OPEN" : "CLOSED");
    ...
    if (back_open && front_open && L->right->sector == front_open) {
        ...
        PrintMiniWarn(
            "Linedef #%d seems to be a One-Sided Window (back faces sector "
            "#%d).\n",
...
void DetectWindowEffects(void) {
    ...
    for (i = 0; i < num_linedefs; i++) {
        ...
        if (L->two_sided || L->zero_len || L->overlap || !L->right) continue;
...
#if DEBUG_WINDOW_FX
            PrintDebug(
                "FUNNY LINE %d : start vertex %d has odd number of "
                "one-siders\n",
                i, L->start->index);
#endif
...
#if DEBUG_WINDOW_FX
            PrintDebug(
                "FUNNY LINE %d : end vertex %d has odd number of one-siders\n",
                i, L->end->index);
#endif
            TestForWindowEffect(L);
        }
...
vertex_t *NewVertexDegenerate(vertex_t *start, vertex_t *end) {
    ...
    if (dlen == 0) InternalError("NewVertexDegenerate: bad delta !");
