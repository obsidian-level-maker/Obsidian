...
static LEVELARRAY(seg_t, segs, num_segs) static LEVELARRAY(
    subsec_t, subsecs,
    num_subsecs) static LEVELARRAY(node_t, nodes,
                                   num_nodes) static LEVELARRAY(wall_tip_t,
                                                                wall_tips,
                                                                num_wall_tips)
...
#define ALLIGATOR(TYPE, BASEVAR, NUMVAR)                            \
    {                                                               \
        if ((NUMVAR % ALLOC_BLKNUM) == 0) {                         \
            BASEVAR = (TYPE **)UtilRealloc(                         \
                BASEVAR, (NUMVAR + ALLOC_BLKNUM) * sizeof(TYPE *)); \
        }                                                           \
...
vertex_t *NewVertex(void) ALLIGATOR(vertex_t, lev_vertices, num_vertices)

    linedef_t *NewLinedef(void) ALLIGATOR(linedef_t, lev_linedefs, num_linedefs)

        sidedef_t *NewSidedef(void) ALLIGATOR(sidedef_t, lev_sidedefs,
                                              num_sidedefs)

            sector_t *NewSector(void) ALLIGATOR(sector_t, lev_sectors,
                                                num_sectors)

                thing_t *NewThing(void) ALLIGATOR(thing_t, lev_things,
                                                  num_things)

                    seg_t *NewSeg(void) ALLIGATOR(seg_t, segs, num_segs)

                        subsec_t *NewSubsec(void) ALLIGATOR(subsec_t, subsecs,
                                                            num_subsecs)

                            node_t *NewNode(void) ALLIGATOR(node_t, nodes,
                                                            num_nodes)

                                wall_tip_t *NewWallTip(void) ALLIGATOR(
                                    wall_tip_t, wall_tips, num_wall_tips)
/* ----- free routines ---------------------------- */

#define FREEMASON(TYPE, BASEVAR, NUMVAR)                   \
    {                                                      \
        int i;                                             \
        for (i = 0; i < NUMVAR; i++) UtilFree(BASEVAR[i]); \
        if (BASEVAR) UtilFree(BASEVAR);                    \
        BASEVAR = NULL;                                    \
        NUMVAR = 0;                                        \
    }

                                    void FreeVertices(void) FREEMASON(
                                        vertex_t, lev_vertices, num_vertices)

                                        void FreeLinedefs(void) FREEMASON(
                                            linedef_t, lev_linedefs,
                                            num_linedefs)

                                            void FreeSidedefs(void) FREEMASON(
                                                sidedef_t, lev_sidedefs,
                                                num_sidedefs)

                                                void FreeSectors(
                                                    void) FREEMASON(sector_t,
                                                                    lev_sectors,
                                                                    num_sectors)

                                                    void FreeThings(void) FREEMASON(
                                                        thing_t, lev_things,
                                                        num_things)

                                                        void FreeSegs(void) FREEMASON(
                                                            seg_t, segs,
                                                            num_segs)

                                                            void FreeSubsecs(void) FREEMASON(
                                                                subsec_t,
                                                                subsecs,
                                                                num_subsecs)

                                                                void FreeNodes(void) FREEMASON(
                                                                    node_t,
                                                                    nodes,
                                                                    num_nodes)

                                                                    void FreeWallTips(void) FREEMASON(
                                                                        wall_tip_t,
                                                                        wall_tips,
                                                                        num_wall_tips)
...
#define LOOKERUPPER(BASEVAR, NUMVAR, NAMESTR)                    \
    {                                                            \
        ...
        return BASEVAR[index];                                   \
                                                                        ...
                                                                        vertex_t *LookupVertex(int index) LOOKERUPPER(
                                                                            lev_vertices,
                                                                            num_vertices,
                                                                            "ve"
                                                                            "rt"
                                                                            ...
                                                                            linedef_t *LookupLinedef(int index) LOOKERUPPER(
                                                                                lev_linedefs,
                                                                                num_linedefs,
                                                                                "linedef")

                                                                                sidedef_t *LookupSidedef(int index) LOOKERUPPER(
                                                                                    lev_sidedefs,
                                                                                    num_sidedefs,
                                                                                    "sidedef")

                                                                                    sector_t *LookupSector(int index) LOOKERUPPER(
                                                                                        lev_sectors,
                                                                                        num_sectors,
                                                                                        "sector")

                                                                                        thing_t *LookupThing(int index) LOOKERUPPER(
                                                                                            lev_things,
                                                                                            num_things,
                                                                                            "thing")

                                                                                            seg_t *LookupSeg(int index) LOOKERUPPER(
                                                                                                segs,
                                                                                                num_segs,
                                                                                                "seg")

                                                                                                subsec_t *LookupSubsec(
                                                                                                    int index)
                                                                                                    LOOKERUPPER(
                                                                                                        subsecs,
                                                                                                        num_subsecs,
    ...
    if (FindLevelLump("NODES") == NULL) return FALSE;

    lump = FindLevelLump("SEGS");

    if (!lump || lump->length == 0 || CheckLevelLumpZero(lump)) return FALSE;
...
void GetVertices(void) {
    ...
    if (lump) count = lump->length / sizeof(raw_vertex_t);
    ...
    if (!lump || count == 0) FatalError("Couldn't find any Vertices");

    raw = (raw_vertex_t *)
              lump->data;  // TODO: V1004 https://www.viva64.com/en/w/v1004/ The
                           // 'lump' pointer was used unsafely after it was
                           // verified against nullptr. Check lines: 239, 251.
...
void GetSectors(void) {
    ...
    if (lump) count = lump->length / sizeof(raw_sector_t);

    if (!lump || count == 0) FatalError("Couldn't find any Sectors");
    ...
    for (i = 0; i < count; i++, raw++) {
        ...
        sector->coalesce =
            (sector->tag >= 900 && sector->tag < 1000) ? TRUE : FALSE;
...
void GetThings(void) {
    ...
    if (lump) count = lump->length / sizeof(raw_thing_t);
...
void GetThingsHexen(void) {
    ...
    if (lump) count = lump->length / sizeof(raw_hexen_thing_t);
...
void GetSidedefs(void) {
    ...
    if (lump) count = lump->length / sizeof(raw_sidedef_t);

    if (!lump || count == 0) FatalError("Couldn't find any Sidedefs");
    ...
    for (i = 0; i < count; i++, raw++) {
        ...
        side->sector = (SINT16(raw->sector) == -1)
                           ? NULL
...
static INLINE_G sidedef_t *SafeLookupSidedef(uint16_g num) {
    ...
    if ((int)num >= num_sidedefs && (sint16_g)(num) < 0) return NULL;
...
void GetLinedefs(void) {
    ...
    if (lump) count = lump->length / sizeof(raw_linedef_t);

    if (!lump || count == 0) FatalError("Couldn't find any Linedefs");
    ...
    for (i = 0; i < count; i++, raw++) {
        ...
        line->is_precious =
            (line->tag >= 900 && line->tag < 1000) ? TRUE : FALSE;
...
void GetLinedefsHexen(void) {
    ...
    if (lump) count = lump->length / sizeof(raw_hexen_linedef_t);
    ...
    for (i = 0; i < count; i++, raw++) {
        ...
        /* read specials */
        for (j = 0; j < 5; j++) line->specials[j] = UINT8(raw->specials[j]);
...
static int SegCompare(const void *p1, const void *p2) {
    ...
    if (A->index < 0) InternalError("Seg %p never reached a subsector !", A);

    if (B->index < 0) InternalError("Seg %p never reached a subsector !", B);
...
void PutV2Vertices(int do_v5) {
    ...
    if (count > 32767) MarkSoftFailure(LIMIT_GL_VERT);
}
...
void PutSidedefs(void) {
    ...
    for (i = 0; i < num_sidedefs; i++) {
        ...
        raw.sector =
            (side->sector == NULL) ? SINT16(-1) : UINT16(side->sector->index);
...
void PutLinedefsHexen(void) {
    ...
    for (i = 0; i < num_linedefs; i++) {
        ...
        // write specials
        for (j = 0; j < 5; j++) raw.specials[j] = UINT8(line->specials[j]);
...
void PutSegs(void) {
    ...
    for (i = 0, count = 0; i < num_segs; i++) {
        ...
        if (!seg->linedef || seg->degenerate) continue;
        ...
        PrintDebug(
            ...
            seg->index, UINT16(raw.start), UINT16(raw.end), UINT16(raw.linedef),
            seg->side ? "L" : "R", UINT16(raw.angle), seg->start->x,
            seg->start->y, seg->end->x, seg->end->y);
#endif
    }

    if (count != num_complete_seg)
        InternalError("PutSegs miscounted (%d != %d)", count, num_complete_seg);
...
void PutGLSegs(void) {
    ...
    for (i = 0, count = 0; i < num_segs; i++) {
        ...
        PrintDebug(
            ...
            "(%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
            seg->index, UINT16(raw.linedef), seg->side ? "L" : "R",
            UINT16(raw.partner), seg->start->x, seg->start->y, seg->end->x,
            seg->end->y);
#endif
    }
...
void PutV3Segs(int do_v5) {
    ...
    if (!do_v5) AppendLevelLump(lump, lev_v3_magic, 4);
    ...
    for (i = 0, count = 0; i < num_segs; i++) {
        ...
        PrintDebug(
            ...
            "(%1.1f,%1.1f) -> (%1.1f,%1.1f)\n",
            seg->index, UINT16(raw.linedef), seg->side ? "L" : "R",
            UINT32(raw.partner), seg->start->x, seg->start->y, seg->end->x,
            seg->end->y);
#endif
    }
...
void PutSubsecs(const char *name, int do_gl) {
    ...
    for (i = 0; i < num_subsecs; i++) {
        ...
        PrintDebug("PUT SUBSEC %04X  First %04X  Num %04X\n", sub->index,
                   UINT16(raw.first), UINT16(raw.num));
#endif
    }
...
void PutV3Subsecs(int do_v5) {
    ...
    for (i = 0; i < num_subsecs; i++) {
        ...
        PrintDebug("PUT V3 SUBSEC %06X  First %06X  Num %06X\n", sub->index,
                   UINT32(raw.first), UINT32(raw.num));
#endif
    }

    if (!do_v5 && num_subsecs > 32767) MarkHardFailure(LIMIT_GL_SSECT);
}
...
static void PutOneNode(node_t *node, lump_t *lump) {
    ...
    if (node->r.node) PutOneNode(node->r.node, lump);

    if (node->l.node) PutOneNode(node->l.node, lump);
    ...
    PrintDebug(
        ...
        "(%d,%d) -> (%d,%d)\n",
        node->index, UINT16(raw.left), UINT16(raw.right), node->x, node->y,
...
static void PutOneV5Node(node_t *node, lump_t *lump) {
    ...
    if (node->r.node) PutOneV5Node(node->r.node, lump);

    if (node->l.node) PutOneV5Node(node->l.node, lump);
    ...
    PrintDebug(
        ...
        "(%d,%d) -> (%d,%d)\n",
        node->index, UINT32(raw.left), UINT32(raw.right), node->x, node->y,
        node->x + node->dx, node->y + node->dy);
#endif
}
...
void PutNodes(const char *name, int do_gl, int do_v5, node_t *root) {
    ...
    if (node_cur_index != num_nodes)
        InternalError("PutNodes miscounted (%d != %d)", node_cur_index,
                      num_nodes);

    if (!do_v5 && node_cur_index > 32767) MarkHardFailure(LIMIT_NODES);
}
...
void PutZSubsecs(void) {
    ...
    for (i = 0; i < num_subsecs; i++) {
        ...
        for (seg = sub->seg_list; seg; seg = seg->next, cur_seg_index++) {
            ...
            if (cur_seg_index != seg->index)
                InternalError(
                    "PutZSubsecs: seg index mismatch in sub %d (%d != %d)\n", i,
                    cur_seg_index, seg->index);
    ...
    if (cur_seg_index != num_complete_seg)
        InternalError("PutZSubsecs miscounted segs (%d != %d)", cur_seg_index,
                      num_complete_seg);
}
...
void PutZSegs(void) {
    ...
    if (count != num_complete_seg)
        InternalError("PutZSegs miscounted (%d != %d)", count,
                      num_complete_seg);
}
...
static void PutOneZNode(node_t *node) {
    ...
    if (node->r.node) PutOneZNode(node->r.node);

    if (node->l.node) PutOneZNode(node->l.node);
    ...
    ZLibAppendLump(
        &raw.right,
        4);  // TODO: V614 https://www.viva64.com/en/w/v614/ Potentially
             // uninitialized variable 'raw.right' used. Consider checking the
             // first actual argument of the 'ZLibAppendLump' function.
    ZLibAppendLump(
        &raw.left,
        4);  // TODO: V614 https://www.viva64.com/en/w/v614/ Potentially
             // uninitialized variable 'raw.left' used. Consider checking the
             // first actual argument of the 'ZLibAppendLump' function.
...
void PutZNodes(node_t *root) {
    ...
    if (node_cur_index != num_nodes)
        InternalError("PutZNodes miscounted (%d != %d)", node_cur_index,
                      num_nodes);
}
...
void LoadLevel(void) {
    ...
    lev_doing_normal =
        !cur_info->gwa_mode &&
        (cur_info->force_normal || (!cur_info->no_normal && !normal_exists));

    // -JL- Identify Hexen mode by presence of BEHAVIOR lump
    lev_doing_hexen = (FindLevelLump("BEHAVIOR") != NULL);

    if (lev_doing_normal)
        message = UtilFormat("Building normal and GL nodes on %s%s", level_name,
                             lev_doing_hexen ? " (Hexen)" : "");
    else
        message = UtilFormat("Building GL nodes on %s%s", level_name,
                             lev_doing_hexen ? " (Hexen)" : "");
    ...
    if (lev_doing_normal) {
        ...
        if (cur_info->pack_sides) DetectDuplicateSidedefs();

        if (cur_info->merge_vert) DetectDuplicateVertices();

        if (!cur_info->no_prune) PruneLinedefs();

        // always prune vertices (ignore -noprune), otherwise all the
        // unused vertices from seg splits would keep accumulating.
        PruneVertices();

        if (!cur_info->no_prune) PruneSidedefs();

        if (cur_info->prune_sect) PruneSectors();
    }
...
void PutGLOptions(void) {
    ...
    sprintf(option_buf, "-v%d -factor %d", cur_info->spec_version,
            cur_info->factor);
...
void SaveLevel(node_t *root_node) {
    ...
    if (cur_info->spec_version == 1) RoundOffBspTree(root_node);
    ...
    if (lev_doing_normal) {
        ...
        if (lev_force_v5) {
            ...
            if (cur_info->spec_version < 5) MarkZDSwitch();
        ...
        if (!cur_info->no_reject || !FindLevelLump("REJECT")) PutReject();
    }
