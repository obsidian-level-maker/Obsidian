...
static seg_t *SplitSeg(seg_t *old_seg, float_g x, float_g y) {
    ...
    if (old_seg->block) SplitSegInSuper(old_seg->block, old_seg);
...
// skipped altogether.
//
static int EvalPartition(superblock_t *seg_list, seg_t *part, int best_cost) {
    eval_info_t info;
...
static void EvaluateFastWorker(superblock_t *seg_list, seg_t **best_H,
                               seg_t **best_V, int mid_x, int mid_y) {
    seg_t *part;
    int num;
...
static seg_t *FindFastSeg(superblock_t *seg_list, const bbox_t *bbox) {
    ...
    if (best_H) H_cost = EvalPartition(seg_list, best_H, 99999999);

    if (best_V) V_cost = EvalPartition(seg_list, best_V, 99999999);
...
static int PickNodeWorker(superblock_t *part_list, superblock_t *seg_list,
                          seg_t **best, int *best_cost, int *progress,
                          int prog_step) {
    seg_t *part;
...
//       or be split.
//
void DivideOneSeg(seg_t *cur, seg_t *part, superblock_t *left_list,
                  superblock_t *right_list, intersection_t **cut_list) {
    seg_t *new_seg;
...
// SeparateSegs
//
void SeparateSegs(superblock_t *seg_list, seg_t *part, superblock_t *lefts,
                  superblock_t *rights, intersection_t **cut_list) {
    int num;
...
static void FindLimitWorker(superblock_t *block, bbox_t *bbox) {
    ...
    for (num = 0; num < 2; num++) {
        if (block->subs[num]) FindLimitWorker(block->subs[num], bbox);
    }
}
...
// AddMinisegs
//
void AddMinisegs(seg_t *part, superblock_t *left_list, superblock_t *right_list,
    ...
    for (cur = cut_list; cur; cur = cur->next) {
        PrintDebug("  Vertex %8X (%1.1f,%1.1f)  Along %1.2f  [%d/%d]  %s\n",
                   ...
                   cur->along_dist, cur->before ? cur->before->index : -1,
    ...
    while (cur && next) {
        ...
        PrintDebug(
            ...
            cur->after ? cur->after->index : -1, next->vertex, next->vertex->x,
            next->vertex->y, next->before ? next->before->index : -1,
        ...
        PrintDebug("---> merged (%1.0f,%1.0f) [%d/%d] %s\n", cur->vertex->x,
                   cur->vertex->y, cur->before ? cur->before->index : -1,
                   cur->after ? cur->after->index : -1,
                   cur->self_ref ? "SELFREF" : "");
#endif
    ...
    for (cur = cut_list; cur && cur->next; cur = cur->next) {
        ...
        if (cur->after != next->before) {
            if (!cur->self_ref && !next->self_ref)
                PrintMiniWarn(
                    "Sector mismatch: #%d (%1.1f,%1.1f) != #%d (%1.1f,%1.1f)\n",
