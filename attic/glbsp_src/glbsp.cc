...
static void AddExtraFile(nodebuildinfo_t *info, const char *str) {
    ...
    if (!info->extra_files) {
        info->extra_files =
            (const char **)UtilCalloc(EXTRA_BLOCK * sizeof(const char *));
    ...
    while (info->extra_files[count]) count++;

    space = EXTRA_BLOCK - 1 - (count % EXTRA_BLOCK);

    if (space == 0) {
        info->extra_files = (const char **)UtilRealloc(
            (void *)info->extra_files,
    ...
    while (argc > 0) {
        if (argv[0][0] != '-') {
            ...
            if (UtilCheckExtension(argv[0], "gwa")) {
                SetErrorMsg(
                    "Input file cannot be GWA (contains nothing to build)");
                cur_comms = NULL;
                return GLBSP_E_BadArgs;
            }
...
glbsp_ret_e GlbspCheckInfo(nodebuildinfo_t *info,
    ...
    if (!info->output_file || info->output_file[0] == 0) {
        GlbspFree(info->output_file);
        info->output_file =
            GlbspStrDup(UtilReplaceExtension(info->input_file, "gwa"));

        info->gwa_mode = TRUE;
        info->missing_output = TRUE;
    } else /* has output filename */
    {
        if (UtilCheckExtension(info->output_file, "gwa")) info->gwa_mode = TRUE;
    }
