...
char *UtilStrDup(const char *str) {
    ...
    if (len > 0)
        memcpy(result, str,
               len);  // TODO: V575 https://www.viva64.com/en/w/v575/ The
                      // potential null pointer is passed into 'memcpy' function.
...
char *UtilFormat(const char *str, ...) {
    ...
    for (;;) {
        ...
        buf = (char *)realloc(
            buf,
            buf_size);  // TODO: V701 https://www.viva64.com/en/w/v701/
                        // realloc() possible leak: when realloc() fails in
                        // allocating memory, original pointer 'buf' is lost.
