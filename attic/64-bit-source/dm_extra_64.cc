...
int DM_wad_logo_gfx(lua_State *L) {
    ...
    if (map->size < 2) return luaL_error(L, "wad_logo_gfx: colormap too small");
...
int DM_fsky_add_stars(lua_State *L) {
    ...
    if (!lua_isnil(L, -4)) seed = (unsigned long long)luaL_checkinteger(L, -4);
    ...
    if (thresh > 0.98) return luaL_error(L, "fsky_add_stars: bad thresh value");
...
int DM_fsky_add_clouds(lua_State *L) {
    ...
    if (!lua_isnil(L, -2)) seed = (unsigned long long)luaL_checkinteger(L, -2);
...
int DM_title_draw_clouds(lua_State *L) {
    ...
    uint_fast64_t seed = luaL_checkinteger(L, 1);
