...
W_Info::W_Info(int X, int Y, int W, int H, const char *label)
    : Fl_Group(X, Y, W, H, label), num_segs(0) {
...
int W_Info::handle(int event) { return Fl_Group::handle(event); }
...
void W_Info::SetNodes(const char *type) { node_type->value(type); }
...
void W_Info::SetCurBBox(const bbox_t *bbox) {
    ...
    sprintf(buffer, "%1.0f", bbox->minx);
    bb_x1->value(buffer);
    sprintf(buffer, "%1.0f", bbox->miny);
    bb_y1->value(buffer);
    sprintf(buffer, "%1.0f", bbox->maxx);
    bb_x2->value(buffer);
    sprintf(buffer, "%1.0f", bbox->maxy);
    bb_y2->value(buffer);
}
...
void W_Info::SetPartition(const node_c *part) {
    ...
    sprintf(buffer, "%1.1f", part->x1);
    pt_x->value(buffer);
    sprintf(buffer, "%1.1f", part->y1);
    pt_y->value(buffer);
    sprintf(buffer, "%1.1f", part->x2 - part->x1);
    pt_dx->value(buffer);
    sprintf(buffer, "%1.1f", part->y2 - part->y1);
    pt_dy->value(buffer);
}

void W_Info::SetMouse(double mx, double my) {
    if (mx < -32767.0 || mx > 32767.0 || my < -32767.0 || my > 32767.0) {
...
void W_Info::BeginSegList() { num_segs = 0; }
