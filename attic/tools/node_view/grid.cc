...
// W_Grid Destructor
//
W_Grid::~W_Grid() { delete[] visit_route; }

void W_Grid::SetZoom(int new_zoom) {
    if (new_zoom < MIN_GRID_ZOOM) new_zoom = MIN_GRID_ZOOM;

    if (new_zoom > MAX_GRID_ZOOM) new_zoom = MAX_GRID_ZOOM;
...
void W_Grid::FitBBox(double lx, double ly, double hx, double hy) {
    ...
    for (; zoom > MIN_GRID_ZOOM; zoom--) {
        ...
        if (dx * zoom_mul < w() && dy * zoom_mul < h()) break;
    }
...
void W_Grid::draw() {
    ...
    if (partition_MODE == 1) draw_all_partitions();

    draw_node(root, 0, true);

    if (partition_MODE == 2) draw_all_partitions();

    fl_pop_clip();
}

void W_Grid::draw_grid(double spacing, int ity) {
    if (grid_MODE == 0) return;

    if (!isdigit(ity)) return;
...
void W_Grid::draw_partition(const node_c *nd, int ity) {
    ...
    if (ABS(ndx) > ABS(ndy)) {
        ...
        if (MAX(tly, thy) < mly || MIN(tly, thy) > mhy) return;
    } else {
        ...
        if (MAX(tlx, thx) < mlx || MIN(tlx, thx) > mhx) return;
    }
    ...
    for (float u = 0.1; u <= 0.91; u += 0.16) {
        ...
        fl_line(ax, ay, ax + int((-pdy - pdx * 1.0) * 8),
                ay + int((pdx - pdy * 1.0) * 8));
        fl_line(ax, ay, ax + int((pdy - pdx * 1.0) * 8),
                ay + int((-pdx - pdy * 1.0) * 8));
    }
}
...
void W_Grid::draw_bbox(const bbox_t *bbox, int ity) {
    ...
    if (bbox->maxx < mlx || bbox->minx > mhx || bbox->maxy < mly ||
        bbox->miny > mhy) {
        return;
    }
...
void W_Grid::draw_all_partitions() {
    ...
    for (int rt_idx = 0; rt_idx < route_len; rt_idx++) {
        ...
        child_c *next_ch =
            (visit_route[rt_idx] == RT_LEFT) ? &cur->back : &cur->front;
    ...
    for (int n_idx = 2; n_idx <= 3; n_idx++) {
        if (bbox_MODE == 1)
            if (bboxs[n_idx]) draw_bbox(bboxs[n_idx], n_idx + 1);

        if (nodes[n_idx]) draw_partition(nodes[n_idx], n_idx + 1);
    }
}
...
bool W_Grid::set_seg_color(side_c *sd, bool on) {
    if (shade_MODE == 0 && !on) return false;

    if (shade_MODE == 2) on = true;

    int ity = on ? 255 : 144;

    if (sd->miniseg) {
        if (miniseg_MODE < 2) return false;
...
void W_Grid::draw_line(double x1, double y1, double x2, double y2) {
    ...
    /// mhy); PrintDebug("  out1 = %d  out2 = %d\n", out1, out2);

    while ((out1 & out2) == 0 && (out1 | out2) != 0) {
        /// PrintDebug("> LINE (%1.3f,%1.3f) --> (%1.3f,%1.3f)\n", x1, y1, x2,
        /// y2); PrintDebug(">   out1 = %d  out2 = %d\n", out1, out2);
...
void W_Grid::scroll(int dx, int dy) {
    ...
    // fprintf(stderr, "Scroll pix (%d,%d) map (%1.1f, %1.1f) mid (%1.1f,
...
bool W_Grid::descend_tree(char side) {
    ...
    if (route_len >= MAX_ROUTE) return false;
