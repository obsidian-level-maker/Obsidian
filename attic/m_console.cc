...
class UI_ConLine : public Fl_Group {
   ...
   public:
    int CalcHeight() const { return LINE_H; }
   ...
   private:
    static void callback_Button(Fl_Widget *w, void *data) {
        ...
        if (datum[0] == 'e' && datum[1] == ':') {
            ...
            Script_RunString("ob_console_dump { tab_ref=%s, indent=%d }", datum,
                             indent);
...
class UI_ConInput : public Fl_Group {
   ...
   public:
    UI_ConInput(int x, int y, int w, int h)
        : Fl_Group(x, y, w, h), hist_used(0), browse_pos(-1), saved_input() {
        end();  // cancel begin() in Fl_Group constructor

        for (int i = 0; i < CON_MAX_HISTORY; i++) cmd_history[i] = NULL;
    ...
    virtual ~UI_ConInput() {
        for (int i = 0; i < hist_used; i++) StringFree(cmd_history[i]);
    }
   ...
   private:
    void AddHistory(const char *s) {
        ...
        if (hist_used > 0)
            if (strcmp(s, cmd_history[0]) == 0) return;
        ...
        if (hist_used < CON_MAX_HISTORY) hist_used++;
    }

    bool HasInput() const { return strlen(input->value()) > 0; }

    void HistoryUp() {
        if (browse_pos >= hist_used - 1) return;

        if (browse_pos == -1) saved_input = input->value();
...
class UI_Console : public Fl_Double_Window {
   ...
   public:
    UI_Console(int x, int y, int w, int h, const char *label = NULL)
        ...
        if (!alternate_look) sbar->color(FL_DARK3 + 1, FL_DARK3 + 1);
...
void UI_Console::RepositionAll(bool jump_bottom, UI_ConLine *focus) {
    ...
    if (!focus) focus = LowestVisibleLine();
    ...
    if (new_height <= mh || jump_bottom) {
        ...
        if (offset_y > new_height - mh) offset_y = new_height - mh;
    } else {
...
void UI_OpenConsole() {
    ...
    if (!console_win) {
        ...
        console_win =
            new UI_Console(0, 0, con_w, con_h, "OBLIGE DEBUG CONSOLE");
    }
...
//
const con_cmd_t builtin_commands[] = {{"args", CMD_Args},
                                      {"clear", CMD_Clear},
                                      {"help", CMD_Help},
...
static int FindCommand(const char *line) {
    for (int i = 0; builtin_commands[i].name; i++) {
        ...
        if (StringCaseCmpPartial(line, name) == 0) return i;
    }
...
static void ParseArgs(const char *line) {
    ...
    for (;;) {
        ...
        if (console_argc >= CON_MAX_ARGS) break;
