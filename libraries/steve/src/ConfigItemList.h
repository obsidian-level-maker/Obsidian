...
namespace steve
{
template <class T> class ConfigItemList
{
  protected:
  ...
  public:
    inline auto get_all() const
    {
        return _items;
    }
    inline auto get_allowed() const
    {
    ...
    template <typename F> std::shared_ptr<T> get_random_item(F predicate) const;
};

template <class T> void ConfigItemList<T>::compute_cache()
{
...
template <class T> std::shared_ptr<T> ConfigItemList<T>::get_item(const std::string &name)
{
...
template <class T> std::shared_ptr<T> ConfigItemList<T>::get_random_item() const
{
...
template <class T> template <typename F> std::shared_ptr<T> ConfigItemList<T>::get_random_item(F predicate) const
{
