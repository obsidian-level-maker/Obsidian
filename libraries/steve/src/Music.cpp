...
Music::Music(const Config &config)
    : _config(config), _scale(Scale(_config.get_scales().get_random_item(), Rand::next(0, 11))),
      _tempo(_config.get_random_tempo()), _signature(_config.get_signatures().get_random_item())
{
    do
    {
...
void Music::write_mid(std::ostream &s) const
{
    ...
    auto  process_end_notes = [&s, &last, &end_notes](uint32_t next_time) {
        while (!end_notes.empty())
        {
            ...
            {
                s << VarLength(next_end->first - last) << uint8_t(0x80 | next_end->second.channel)
                  << uint8_t(next_end->second.tone) << uint8_t(next_end->second.velocity); // Note off
                last = next_end->first;
    ...
    for (const auto &note : _notes)
    {
        ...
        s << VarLength(note.first - last) << uint8_t(0x90 | note.second.channel) << uint8_t(note.second.tone)
          << uint8_t(note.second.velocity); // Note on
        ...
        if (note.first != last_chord && note.first != _size && note.first % get_bar_ticks() == 0)
        {
            ...
            const std::string chord_str =
                chord_at(note.first).to_short_string() + " (" + std::to_string(degree + 1) + ")";
...
void Music::write_txt(std::ostream &s) const
{
    s << "Scale: " << key_name(scale().key) << " " << scale().desc->name << std::endl
      ...
      << "Signature: " << _signature->beats_per_bar << "/"
      << (1 << (uint32_t(NoteValue::whole) - uint32_t(get_beat_value()))) << std::endl
...

    {
        ...
        {
            s << " - " << _scale.get_degree_string_for_chord(chord) << " (" << chord.to_short_string() << ")"
              << std::endl;
        }
        s << std::endl;
    }
