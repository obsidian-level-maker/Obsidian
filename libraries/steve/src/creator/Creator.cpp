...
void Creator::init()
{
    ...
    _instrument = _music->get_config().get_instruments().get_random_item(
        [this](std::shared_ptr<Instrument> candidate) { return is_valid_instrument(*candidate); });
    const uint8_t ambitus_half_range(Rand::next(6, 10));
    const uint8_t instrument_range(_instrument->max_tone - _instrument->min_tone);

    {
        const uint8_t mid_tone(_instrument->min_tone + ambitus_half_range +
                               Rand::next_normal() * float(instrument_range - ambitus_half_range * 2));
...
Notes Creator::compose()
{
    ...
    while (i < _music->size())
    {
        ...
        for (uintptr_t phrase_index(0); phrase_index < _phrases.size(); phrase_index++)
        {                                             // Iterate through figures already created
            ...
            if (i % phrase.size == 0                  // Its size is good for the place it would be in
                ...
                && harmony(_music->tones().data() + i, phrase.tones.data(), phrase.tones.size()))
            {                                         // It's in harmony with the current music
        ...
        if (new_phrase_needed)
        {                                                                    // Needs to create a new phrase of music
            Phrase phrase;
...
uintptr_t Creator::time(uintptr_t i, size_t size) const
{
    std::vector<uintptr_t> candidates =
        _music->beats_inside(i + ticks_for(_min_time), i + std::min<uintptr_t>(ticks_for(_max_time), size));
    ...
    candidates.erase(
        std::remove_if(candidates.begin(), candidates.end(), [score](uintptr_t c) { return score(c) == 0; }),
        candidates.end());
    if (candidates.empty())
    {
        return 0;
    ...
    std::sort(candidates.begin(), candidates.end(), [score](uintptr_t a, uintptr_t b) { return score(a) > score(b); });
