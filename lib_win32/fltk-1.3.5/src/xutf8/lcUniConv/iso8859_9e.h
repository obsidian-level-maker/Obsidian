/* $XFree86: xc/lib/X11/lcUniConv/iso8859_9e.h,v 1.3 2000/11/28 16:10:28 dawes
 * Exp $ */
...
static int iso8859_9e_mbtowc(conv_t conv, ucs4_t *pwc, const unsigned char *s,
                             int n) {
    unsigned char c = *s;
    if (c >= 0xa0)
...
static int iso8859_9e_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, int n) {
    unsigned char c = 0;
    if (wc < 0x00a0) {
        *r = wc;
        return 1;
