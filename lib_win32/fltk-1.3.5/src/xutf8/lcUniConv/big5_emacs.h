/* $XFree86: xc/lib/X11/lcUniConv/big5_emacs.h,v 1.1 2000/11/28 18:50:06 dawes
 * Exp $ */
...
static int big5_0_mbtowc(conv_t conv, ucs4_t *pwc, const unsigned char *s,
                         int n) {
...
static int big5_1_mbtowc(conv_t conv, ucs4_t *pwc, const unsigned char *s,
                         int n) {
    unsigned char c1 = s[0];
    if (c1 >= 0x21 && c1 <= 0x72) {
        if (n >= 2) {
...
static int big5_0_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, int n) {
    if (n >= 2) {
        unsigned char buf[2];
...
static int big5_1_wctomb(conv_t conv, unsigned char *r, ucs4_t wc, int n) {
    if (n >= 2) {
        unsigned char buf[2];
        int ret = big5_wctomb(conv, buf, wc, 2);
        if (ret != RET_ILSEQ) {
