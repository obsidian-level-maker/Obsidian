...
/* Do computations that are needed before master selection phase */
{
    ...
    if (cinfo->scale_num >= cinfo->scale_denom * cinfo->block_size) {
        ...
        cinfo->jpeg_width = (JDIMENSION)jdiv_round_up(
            (long)cinfo->image_width * cinfo->block_size, 2L);
        cinfo->jpeg_height = (JDIMENSION)jdiv_round_up(
...
jpeg_calc_trans_dimensions(j_compress_ptr cinfo) {
    if (cinfo->min_DCT_h_scaled_size != cinfo->min_DCT_v_scaled_size)
        ERREXIT2(cinfo, JERR_BAD_DCTSIZE, cinfo->min_DCT_h_scaled_size,
...
/* Do computations that are needed before master selection phase */
{
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
         ...
         * scale down the chroma components via DCT scaling rather than
         * downsampling. This saves time if the downsampler gets to use 1:1
         * scaling. Note this code adapts subsampling ratios which are powers
         * of 2.
         */
        ssize = 1;
#ifdef DCT_SCALING_SUPPORTED
 ...
 */
{
    ...
    for (idxin = 0; idxin < cinfo->num_scans; idxin++) {
        ...
        if (scanptr[idxout].Ss > cinfo->lim_Se)
            ...
            continue;
        if (scanptr[idxout].Se > cinfo->lim_Se) /* Limit scan to end of block */
...
jinit_c_master_control(j_compress_ptr cinfo, boolean transcode_only) {
    ...
    if (cinfo->scan_info != NULL) {
        ...
        if (cinfo->block_size < DCTSIZE) reduce_script(cinfo);
#else
        ERREXIT(cinfo, JERR_NOT_COMPILED);
#endif
    } else {
