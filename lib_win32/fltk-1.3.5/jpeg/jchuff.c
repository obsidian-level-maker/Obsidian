...
#define emit_byte_e(entropy, val)                                     \
    {                                                                 \
        *(entropy)->next_output_byte++ = (JOCTET)(val);               \
        if (--(entropy)->free_in_buffer == 0) dump_buffer_e(entropy); \
    }
...
emit_eobrun(huff_entropy_ptr entropy) {
    ...
    if (entropy->EOBRUN > 0) { /* if there is any pending EOBRUN */
        ...
        if (nbits) emit_bits_e(entropy, entropy->EOBRUN, nbits);
...
emit_restart_s(working_state *state, int restart_num) {
    ...
    if (!flush_bits_s(state)) return FALSE;
...
encode_one_block(working_state *state, JCOEFPTR block, int last_dc_val,
    ...
    /* or the complement of its magnitude, if negative. */
    if (nbits) /* emit_bits rejects calls with size 0 */
        if (!emit_bits_s(state, (unsigned int)temp2, nbits)) return FALSE;
    ...
    for (k = 1; k <= Se; k++) {
        if ((temp2 = block[natural_order[k]]) == 0) {
             ...
             */
            while (r > 15) {
                if (!emit_bits_s(state, actbl->ehufco[0xF0],
                                 actbl->ehufsi[0xF0]))
                    return FALSE;
                r -= 16;
            }
...
finish_pass_huff(j_compress_ptr cinfo) {
    ...
    if (cinfo->progressive_mode) {
        ...
        if (!flush_bits_s(&state)) ERREXIT(cinfo, JERR_CANT_SUSPEND);
        ...
        if (cinfo->Ss == 0 && cinfo->Ah == 0) {
            ...
            if (gather_statistics) {
                ...
                if (entropy->dc_count_ptrs[tbl] == NULL)
                    entropy->dc_count_ptrs[tbl] =
                        (long *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo,
                                                           JPOOL_IMAGE,
                                                           257 * SIZEOF(long));
