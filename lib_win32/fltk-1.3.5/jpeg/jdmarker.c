...
/* Process a SOS marker */
{
    ...
    if (!cinfo->marker->saw_SOF) ERREXITS(cinfo, JERR_SOF_BEFORE, "SOS");
    ...
    for (i = 0; i < n; i++) {
        ...
        for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
             ci++, compptr++) {
            if (c == compptr->component_id) goto id_found;
        }
...
/* Process an LSE marker */
{
    ...
    if (!cinfo->marker->saw_SOF) ERREXITS(cinfo, JERR_SOF_BEFORE, "LSE");

    if (cinfo->num_components < 3) goto bad;

    INPUT_2BYTES(cinfo, length, return FALSE);

    if (length != 24) ERREXIT(cinfo, JERR_BAD_LENGTH);
    ...
    for (;;) {
        ...
        switch (cinfo->unread_marker) {
            ...
            case M_SOF0: /* Baseline */
                if (!get_sof(cinfo, TRUE, FALSE, FALSE)) return JPEG_SUSPENDED;
                break;
            ...
            case M_JPG8:
                if (!get_lse(cinfo)) return JPEG_SUSPENDED;
                break;
