...
int DM_wad_logo_gfx(lua_State *L)
{
	...
	int new_W  = (int)luaL_checkinteger(L, 4);
	int new_H  = (int)luaL_checkinteger(L, 5);
	int map_id = (int)luaL_checkinteger(L, 6);
...
int DM_fsky_solid_box(lua_State *L)
{
	...
	int x2 = x1 + (int)luaL_checkinteger(L, 3);
	int y2 = y1 + (int)luaL_checkinteger(L, 4);
...
int DM_fsky_add_stars(lua_State *L)
{
	...
	if (! lua_isnil(L, -4)) seed     = (int)luaL_checkinteger(L, -4);
	if (! lua_isnil(L, -3)) map_id   = (int)luaL_checkinteger(L, -3);
...
int DM_fsky_add_clouds(lua_State *L)
{
	...
	if (! lua_isnil(L, -2)) seed     = (int)luaL_checkinteger(L, -2);
	if (! lua_isnil(L, -1)) map_id   = (int)luaL_checkinteger(L, -1);
...
int DM_wad_name_gfx(lua_State *L)
{
	...
	int map_id = (int)luaL_checkinteger(L, 3);
...
int DM_title_property(lua_State *L)
{
	...
	if (strcmp(propname, "reset") == 0)
		...
		title_drawctx.box_w = (int)luaL_checkinteger(L, 2);
	else if (strcmp(propname, "box_h") == 0)
		title_drawctx.box_h = (int)luaL_checkinteger(L, 2);

	else if (strcmp(propname, "grad_y1") == 0)
		title_drawctx.grad_y1 = (int)luaL_checkinteger(L, 2);
	else if (strcmp(propname, "grad_y2") == 0)
		title_drawctx.grad_y2 = (int)luaL_checkinteger(L, 2);
...
int DM_title_draw_planet(lua_State *L)
{
	...
	int ph = (int)luaL_checkinteger(L, 3);
	int pw = ph * 5 / 4;
