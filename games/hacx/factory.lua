...
HACX.FACTORY.DM_SCENERY_PREFABS =
{
  pillar_COMPWERD =
  {
...
HACX.FACTORY.DM_FEATURE_PREFABS =
{
...
HACX.FACTORY.DM_WINDOW_PREFABS =
{
  window_narrow =
  {
...
HACX.FACTORY.DM_DEATHMATCH_EXITS =
{
...
HACX.FACTORY.DM_QUEST_LEN_PROBS =
{
  ----------  2   3   4   5   6   7   8  9  10  -------
...
HACX.FACTORY.DM_MONSTER_WEAPON_PREFS =
{
  zombie  = { shotty=6.0 },
  shooter = { shotty=6.0 },
  imp     = { shotty=6.0 },
...
HACX.FACTORY.D2_FEATURE_PREFABS =
{
  overhang3_METAL6 =
  {
    prefab = "OVERHANG_3",
...
HACX.FACTORY.D2_THEMES =
{
}

------------------------------------------------------------

HACX.FACTORY.D2_QUESTS =
{
  key =
  {
...
HACX.FACTORY.D2_EPISODE_THEMES =
{
...
HACX.FACTORY.D2_SECRET_EXITS =
{
  MAP15 = true,
  MAP31 = true,
}

HACX.FACTORY.D2_LEVEL_BOSSES =
{
  MAP07 = "mancubus",
  MAP20 = "spider",
  MAP30 = "boss_brain",
...
function HACX.get_factory_levels(episode)

  assert(GAME.FACTORY.sky_info)

  local level_list = {}

  local theme_probs = HACX.FACTORY.D2_EPISODE_THEMES[episode]
  if OB_CONFIG.length ~= "full" then
    theme_probs = HACX.FACTORY.D2_EPISODE_THEMES[4]
  end
  assert(theme_probs)

  local ep_start  = HACX.FACTORY.D2_EPISODE_INFO[episode].start
  local ep_length = HACX.FACTORY.D2_EPISODE_INFO[episode].len
  ...
  for map = 1,ep_length do
    ...
    if episode == 2 and map >= 10 then
      ...
      Level.sky_info = HACX.FACTORY.D2_SKY_INFO[3]
      Level.theme_probs = { WOLF=10 }
      Level.toughness_factor = 1.2
    end

---!!! Level.boss_kind   = HACX.FACTORY.D2_LEVEL_BOSSES[Level.name]
    Level.secret_kind = HACX.FACTORY.D2_SECRET_KINDS[Level.name]
    Level.secret_exit = HACX.FACTORY.D2_SECRET_EXITS[Level.name]

    std_decide_quests(Level, HACX.FACTORY.D2_QUESTS, HACX.FACTORY.DM_QUEST_LEN_PROBS)
...
function HACX.factory_setup()
  ...
  GAME.FACTORY = 
  {
    ...
    monsters = HACX.FACTORY.DM_MONSTERS,
    bosses   = HACX.FACTORY.DM_BOSSES,
    weapons  = HACX.FACTORY.DM_WEAPONS,

    things = HACX.FACTORY.DM_THINGS,

    mon_give       = HACX.FACTORY.DM_MONSTER_GIVE,
    mon_weap_prefs = HACX.FACTORY.DM_MONSTER_WEAPON_PREFS,
    initial_model  = HACX.FACTORY.DM_INITIAL_MODEL,

    pickups = HACX.FACTORY.DM_PICKUPS,
    pickup_stats = { "health", "bullet", "shell", "rocket", "cell" },
    niceness = HACX.FACTORY.DM_NICENESS,

    dm = HACX.FACTORY.DM_DEATHMATCH,
    dm_exits = HACX.FACTORY.DM_DEATHMATCH_EXITS,

    combos    = HACX.FACTORY.DM_COMBOS,
    exits     = HACX.FACTORY.DM_EXITS,
    hallways  = HACX.FACTORY.DM_HALLWAYS,

    hangs     = HACX.FACTORY.DM_OVERHANGS,
    pedestals = HACX.FACTORY.DM_PEDESTALS,
    mats      = HACX.FACTORY.DM_MATS,
    crates    = HACX.FACTORY.DM_CRATES,

    liquids   = HACX.FACTORY.DM_LIQUIDS,
    switches  = HACX.FACTORY.DM_SWITCHES,
    doors     = HACX.FACTORY.DM_DOORS,
    key_doors = HACX.FACTORY.DM_KEY_DOORS,
    lifts     = HACX.FACTORY.DM_LIFTS,

    images    = HACX.FACTORY.DM_IMAGES,
    lights    = HACX.FACTORY.DM_LIGHTS,

    rooms     = HACX.FACTORY.DM_ROOMS,
    themes    = HACX.FACTORY.DM_THEMES,

    sc_fabs   = HACX.FACTORY.DM_SCENERY_PREFABS,
    feat_fabs = HACX.FACTORY.DM_FEATURE_PREFABS,
    wall_fabs = HACX.FACTORY.DM_WALL_PREFABS,

    door_fabs = HACX.FACTORY.DM_DOOR_PREFABS,
    arch_fabs = HACX.FACTORY.DM_ARCH_PREFABS,
    win_fabs  = HACX.FACTORY.DM_WINDOW_PREFABS,
    misc_fabs = HACX.FACTORY.DM_MISC_PREFABS,
  ...
  GAME.FACTORY.level_func = HACX.get_factory_levels

  GAME.FACTORY.quests   = HACX.FACTORY.D2_QUESTS
  GAME.FACTORY.sky_info = HACX.FACTORY.D2_SKY_INFO

  GAME.FACTORY.themes   = table.merge_w_copy(GAME.FACTORY.themes,   HACX.FACTORY.D2_THEMES)
  GAME.FACTORY.rooms    = table.merge_w_copy(GAME.FACTORY.rooms,    HACX.FACTORY.D2_ROOMS)
  GAME.FACTORY.monsters = table.merge_w_copy(GAME.FACTORY.monsters, HACX.FACTORY.D2_MONSTERS)

  GAME.FACTORY.combos   = table.merge_w_copy(GAME.FACTORY.combos,   HACX.FACTORY.D2_COMBOS)
  GAME.FACTORY.hallways = table.merge_w_copy(GAME.FACTORY.hallways, HACX.FACTORY.D2_HALLWAYS)
  GAME.FACTORY.exits    = table.merge_w_copy(GAME.FACTORY.exits,    HACX.FACTORY.D2_EXITS)

  GAME.FACTORY.rails = HACX.FACTORY.D2_RAILS

  GAME.FACTORY.hangs   = table.merge_w_copy(GAME.FACTORY.hangs,   HACX.FACTORY.D2_OVERHANGS)
  GAME.FACTORY.crates  = table.merge_w_copy(GAME.FACTORY.crates,  HACX.FACTORY.D2_CRATES)
  GAME.FACTORY.mats    = table.merge_w_copy(GAME.FACTORY.mats,    HACX.FACTORY.D2_MATS)
  GAME.FACTORY.doors   = table.merge_w_copy(GAME.FACTORY.doors,   HACX.FACTORY.D2_DOORS)
  GAME.FACTORY.lights  = table.merge_w_copy(GAME.FACTORY.lights,  HACX.FACTORY.D2_LIGHTS)
  GAME.FACTORY.liquids = table.merge_w_copy(GAME.FACTORY.liquids, HACX.FACTORY.D2_LIQUIDS)

  GAME.FACTORY.sc_fabs   = table.merge_w_copy(GAME.FACTORY.sc_fabs,   HACX.FACTORY.D2_SCENERY_PREFABS)
  GAME.FACTORY.feat_fabs = table.merge_w_copy(GAME.FACTORY.feat_fabs, HACX.FACTORY.D2_FEATURE_PREFABS)
  GAME.FACTORY.wall_fabs = table.merge_w_copy(GAME.FACTORY.wall_fabs, HACX.FACTORY.D2_WALL_PREFABS)
  GAME.FACTORY.door_fabs = table.merge_w_copy(GAME.FACTORY.door_fabs, HACX.FACTORY.D2_DOOR_PREFABS)
  GAME.FACTORY.misc_fabs = table.merge_w_copy(GAME.FACTORY.misc_fabs, HACX.FACTORY.D2_MISC_PREFABS)
  ...
  GAME.FACTORY.PREFABS = HACX.FACTORY.PREFABS
