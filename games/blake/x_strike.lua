----------------------------------------------------------------
-- GAME DEF : Blake Stone: Planet Strike
----------------------------------------------------------------
--
...
function STRIKE.get_factory_levels(episode)

  local level_list = {}

  local theme_probs = STRIKE.FACTORY.EPISODE_THEMES[episode]
  if OB_CONFIG.length ~= "full" then
    theme_probs = STRIKE.FACTORY.EPISODE_THEMES[rand.irange(1,4)]
  end

  local ep_start  = STRIKE.FACTORY.EPISODE_INFO[episode].start
  local ep_length = STRIKE.FACTORY.EPISODE_INFO[episode].len
  ...
  for map = 1,ep_length do
    ...
    if (map == ep_length) and episode ~= 5 then
      Level.boss_kind = STRIKE.FACTORY.EPISODE_BOSSES[episode]
    end
  ...
  BLAKE.decide_quests(level_list, "blake")
...
function STRIKE.factory_setup()

  BLAKE.factory_setup()

  GAME.FACTORY.episodes   = 6
  GAME.FACTORY.level_func = STRIKE.get_factory_levels

  GAME.FACTORY.bosses   = STRIKE.FACTORY.BOSSES

  GAME.FACTORY.combos   = table.merge_w_copy(GAME.FACTORY.combos,   STRIKE.FACTORY.COMBOS)
  GAME.FACTORY.pickups  = table.merge_w_copy(GAME.FACTORY.pickups,  STRIKE.FACTORY.PICKUPS)

  GAME.FACTORY.themes   = STRIKE.FACTORY.THEMES
  GAME.FACTORY.rooms    = table.merge_w_copy(GAME.FACTORY.rooms,    STRIKE.FACTORY.ROOMS)
  GAME.FACTORY.things    = table.merge_w_copy(GAME.FACTORY.things,    STRIKE.FACTORY.THINGS)

end
UNFINISHED["strike"] =
{
	label = _("Blake Stone 2"),
	priority = 47,
  ...
  engine = "idtech_0",
	...
	hooks =
	{
      factory_setup = STRIKE.factory_setup,
	},
}
