...
function Weird_generate(point_W, point_H)
  ...
  local function would_be_90_degree(P, dir)
    ...
    for _,dir2 in pairs(geom.ALL_DIRS) do
      if P.edge[dir2] then
...
gui.printf("  loop %d\n", alloc_id("flood_loop"))
  ...
  local function flood_inner_areas(A)
    ...
    for _,dir in pairs(geom.ALL_DIRS) do
      ...
      if not (N and N.area) then goto continue end

      if N.area.is_inner then goto continue end

      if S.border[dir].edge_kind == "boundary" then goto continue end
  ...
  local function mark_boundary_areas()
    ...
    for _,area in pairs(LEVEL.areas) do
