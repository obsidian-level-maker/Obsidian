...
// Returns first value (after shuffling).
//
int RandomShuffle(int *values, int size, bool fill)
{
	SYS_ASSERT(size > 0);
...
static int Int_Comparer(const void *A_void, const void *B_void)
{
...
void UtilSort(int *values, int size)
{
	...
	qsort(values, size, sizeof(int), Int_Comparer);
}
