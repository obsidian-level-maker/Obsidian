----------------------------------------------------------------
--  MODULE: ZDoom Special Addons for Strife
----------------------------------------------------------------
--
--  Copyright (C) 2019 MsrSgtShooterPerson
--  Adapted for Strife by Dashodanger
--
...
ZDOOM_SPECIALS_STRIFE = { }

ZDOOM_SPECIALS_STRIFE.MUSIC_SHUFFLER_CHOICES =
{
  "yes", _("Yes"),
  "no", _("No"),
}

ZDOOM_SPECIALS_STRIFE.FOG_GEN_CHOICES =
{
...
ZDOOM_SPECIALS_STRIFE.FOG_ENV_CHOICES =
{
  "all",     _("All"),
  "outdoor", _("Outdoors Only"),
}

ZDOOM_SPECIALS_STRIFE.FOG_DENSITY_CHOICES =
{
  "subtle",  _("Subtle"),
  "misty",  _("Misty"),
  "smoky",  _("Smoky"),
...
ZDOOM_SPECIALS_STRIFE.STORY_CHOICES =
{
  "generic", _("Generic"),
  "proc",    _("Generated"),
  "none",    _("NONE"),
...
ZDOOM_SPECIALS_STRIFE.INTERPICS =
{
--  OBDNLOAD = 50,
--  OBDNLOA2 = 50,
--  OBDNLOA3 = 50,
...
ZDOOM_SPECIALS_STRIFE.INTERPIC_MUSIC =
{
  "MUS_INTR", _("Universal Intermission")
}

ZDOOM_SPECIALS_STRIFE.DYNAMIC_LIGHT_DECORATE =
...
ZDOOM_SPECIALS_STRIFE.DYNAMIC_LIGHT_GLDEFS =
[[
...
ZDOOM_SPECIALS_STRIFE.GLOWING_FLATS_GLDEFS =
[[
Glow
{
  Flats
...
ZDOOM_SPECIALS_STRIFE.MUSIC_SELECTION = {}

ZDOOM_SPECIALS_STRIFE.FOG_COLORS = 
{
  SKY_CLOUDS = "00 00 ff",
...
function ZDOOM_SPECIALS_STRIFE.setup(self)
  gui.printf("\n--== ZDoom Special Addons module active ==--\n\n")

  module_param_up(self)
end

function ZDOOM_SPECIALS_STRIFE.shuffle_music()

  local music_table = ZDOOM_SPECIALS_STRIFE.MUSIC

  if PARAM.mapinfo_music_shuffler ~= "no" then
    rand.shuffle(music_table)
  end

  ZDOOM_SPECIALS_STRIFE.MUSIC_SELECTION = music_table

end

function ZDOOM_SPECIALS_STRIFE.do_special_stuff()
  ...
  --[[local function pick_sky_color_from_skygen_map(epi_num)
    ...
    if ZDOOM_SPECIALS_STRIFE.FOG_COLORS[skyname] then
      color = ZDOOM_SPECIALS_STRIFE.FOG_COLORS[skyname] 
    end
  ...
  local function add_mapinfo(mapinfo_tab)
    ...
    elseif map_num == 39 then
      ...
      if map_num == 9 then
        next_level_line = '  next = ' .. ZDOOM_SPECIALS_STRIFE.MAP_NOMENCLATURE[7] .. "\n"
      elseif map_num == 18 then
        next_level_line = '  next = ' .. ZDOOM_SPECIALS_STRIFE.MAP_NOMENCLATURE[14] .. "\n"
      elseif map_num == 27 then
        next_level_line = '  next = ' .. ZDOOM_SPECIALS_STRIFE.MAP_NOMENCLATURE[23] .. "\n"
      elseif map_num == 36 then
        next_level_line = '  next = ' .. ZDOOM_SPECIALS_STRIFE.MAP_NOMENCLATURE[32] .. "\n"
      elseif map_num == 45 then
        next_level_line = '  next = ' .. ZDOOM_SPECIALS_STRIFE.MAP_NOMENCLATURE[40] .. "\n"
      end
    ...
    local name_string_map_id = map_num

    -- special tags for Strife stuff
  ...
  local function add_episodedef(map_num)
    local episodedef = {''}
    local map_string = ZDOOM_SPECIALS_STRIFE.MAP_NOMENCLATURE[map_num]
...
--  local ipic = ZDOOM_SPECIALS_STRIFE.INTERPICS[cluster_num]
  ...
  for i=1, #GAME.levels do
    ...
    info.map_num = i
    --[[if i >= 1 and i <= 8 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[1]
    elseif i > 9 and i <= 17 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[2]
    elseif i > 18 and i <= 26 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[3]
    elseif i > 27 and i <= 35 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[4]
    elseif i > 36 and i <= 44 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[5]
    elseif i == 9 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[6]
    elseif i == 18 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[7]
    elseif i == 27 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[8]
    elseif i == 36 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[9]
    elseif i == 45 then
      info.interpic = ZDOOM_SPECIALS_STRIFE.INTERPICS[10]
    end
...
UNFINISHED["zdoom_specials_strife"] =
{
  ...
  hooks =
  {
    --setup = ZDOOM_SPECIALS_STRIFE.setup,
    --get_levels = ZDOOM_SPECIALS_STRIFE.shuffle_music,
    all_done = ZDOOM_SPECIALS_STRIFE.do_special_stuff
  },

  tooltip = "This will write the generated level names to ZDoom's MAPINFO lump.",
  ...
  {
    {
      ...
      priority = 12,
      choices = ZDOOM_SPECIALS_STRIFE.FOG_GEN_CHOICES,
      default = "no",
...

    {
      ...
      priority = 11,
      choices = ZDOOM_SPECIALS_STRIFE.FOG_ENV_CHOICES,
      default = "all",
...

    {
      ...
      priority = 10,
      choices = ZDOOM_SPECIALS_STRIFE.FOG_DENSITY_CHOICES,
      default = "subtle",
...

    {
      ...
      priority = 6,
      choices = ZDOOM_SPECIALS_STRIFE.MUSIC_SHUFFLER_CHOICES,
      default = "no",
...

    {
      ...
      priority = 5,
      choices = ZDOOM_SPECIALS_STRIFE.STORY_CHOICES,
      default = "proc",
...

    {
      ...
      priority = 3,
      choices = ZDOOM_SPECIALS_STRIFE.INTERPIC_MUSIC,
      default = "MUS_INTR",
