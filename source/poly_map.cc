...
bool LoadVertices()
{
    ...
    int count = length / sizeof(RawVertex);

#if DEBUG_LOAD
    DebugPrintf("LoadVertices: num = %d\n", count);
#endif

    RawVertex *raw = (RawVertex *)data;
...
bool LoadSectors()
{
    ...
    int count = length / sizeof(RawSector);

#if DEBUG_LOAD
    DebugPrintf("LoadSectors: num = %d\n", count);
#endif

    RawSector *raw = (RawSector *)data;

    for (int i = 0; i < count; i++, raw++)
    {
        sector_c *sector = NewSector();

        sector->floor_h = AlignedLittleEndianS16(raw->floorh);
        sector->ceil_h  = AlignedLittleEndianS16(raw->ceilh);
        ...
        sector->special = AlignedLittleEndianU16(raw->type);
...
bool LoadThings()
{
    ...
    int count = length / sizeof(RawThing);

#if DEBUG_LOAD
    DebugPrintf("LoadThings: num = %d\n", count);
#endif

    RawThing *raw = (RawThing *)data;
...
bool LoadThingsHexen()
{
    ...
    int count = length / sizeof(RawHexenThing);

#if DEBUG_LOAD
    DebugPrintf("LoadThingsHexen: num = %d\n", count);
#endif

    RawHexenThing *raw = (RawHexenThing *)data;
...
bool LoadSidedefs()
{
    ...
    int count = length / sizeof(RawSidedef);

#if DEBUG_LOAD
    DebugPrintf("LoadSidedefs: num = %d\n", count);
#endif

    RawSidedef *raw = (RawSidedef *)data;
...
bool LoadLinedefs()
{
    ...
    int count = length / sizeof(RawLinedef);

#if DEBUG_LOAD
    DebugPrintf("LoadLinedefs: num = %d\n", count);
#endif

    RawLinedef *raw = (RawLinedef *)data;
...
bool LoadLinedefsHexen()
{
    ...
    int count = length / sizeof(RawHexenLinedef);

#if DEBUG_LOAD
    DebugPrintf("LoadLinedefsHexen: num = %d\n", count);
#endif

    RawHexenLinedef *raw = (RawHexenLinedef *)data;
