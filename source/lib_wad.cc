...
static RawWadHeader wad_R_header;
static RawWadEntry *wad_R_dir;
...
bool WAD_OpenRead(std::filesystem::path filename)
{
    ...
    wad_R_header.num_entries = AlignedLittleEndianU32(wad_R_header.num_entries);
    ...
    wad_R_dir = new RawWadEntry[wad_R_header.num_entries + 1];
    ...
    for (int i = 0; i < (int)wad_R_header.num_entries; i++)
    {
        ...
        L->pos  = AlignedLittleEndianU32(L->pos);
...
static std::list<RawWadEntry> wad_W_directory;

static RawWadEntry wad_W_lump;
...
bool WAD_OpenWrite(std::filesystem::path filename)
{
    ...
    fwrite(&header, sizeof(RawWadHeader), 1, wad_W_fp);
    fflush(wad_W_fp);
...
void WAD_CloseWrite(void)
{
    ...
    std::list<RawWadEntry>::iterator WDI;

    for (WDI = wad_W_directory.begin(); WDI != wad_W_directory.end(); ++WDI)
    {
        RawWadEntry *L = &(*WDI);

        fwrite(L, sizeof(RawWadEntry), 1, wad_W_fp);
        fflush(wad_W_fp);
    ...
    header.num_entries = AlignedLittleEndianU32(header.num_entries);
...
void WAD_FinishLump(void)
{
    ...
    // fix endianness
    wad_W_lump.pos  = AlignedLittleEndianU32(wad_W_lump.pos);
    wad_W_lump.size = AlignedLittleEndianU32(len);
...
bool WAD2_OpenRead(const char *filename)
{
    ...
    wad2_R_dir = new raw_wad2_lump_t[wad2_R_header.num_entries + 1];

    for (int i = 0; i < (int)wad2_R_header.num_entries; i++)
    {
