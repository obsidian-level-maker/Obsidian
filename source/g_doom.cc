...
extern bool slump_main(std::filesystem::path filename);
...
void Doom::AddSector(int f_h, std::string f_tex, int c_h, std::string c_tex, int light, int special, int tag)
{
    if (!UDMF_mode)
    {
        ...
        sec.floorh = AlignedLittleEndianS16(f_h);
        sec.ceilh  = AlignedLittleEndianS16(c_h);
        ...
        sec.type  = AlignedLittleEndianU16(special);
...
int Doom::NumLinedefs()
{
    if (!UDMF_mode)
    {
        if (sub_format == SUBFMT_Hexen)
        {
            return linedef_lump->GetSize() / sizeof(RawHexenLinedef);
        }

        return linedef_lump->GetSize() / sizeof(RawLinedef);
    }
...
int Doom::NumThings()
{
    if (!UDMF_mode)
    {
        if (sub_format == SUBFMT_Hexen)
        {
            return thing_lump->GetSize() / sizeof(RawHexenThing);
        }

        return thing_lump->GetSize() / sizeof(RawThing);
    }
    return udmf_things;
}
